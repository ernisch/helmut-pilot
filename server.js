...
async function handleRequest(request, response) {
  ...
  if (url.pathname === "/api/cron/health-report") {
    ...
    return handleAsync(response, async () => {
      ...
      if (!tenantIds.length) {
        ...
        if (!ladeStoerung) return { ok: true, skipped: true, reason, tenants: 0, reports: [] };
        // MANDANTENLISTE NICHT LESBAR (Abnahme 26.08., Pflichtprüfung 3): der Bot darf
        // hier weder „0 von 0" noch einen gesunden Zustand behaupten — und er darf vor
        // allem NICHT schweigen. Vorher endete der Lauf hier ohne jede Meldung: genau
        // dann, wenn der Betreiber nichts mehr sieht, kam auch keine Nachricht mehr.
        ...
        const unbestimmtText = "Briefingstufen: Status nicht bestimmbar — die Liste der aktiven Mandate"
          + " ist nicht lesbar. Weder Vorbereitung noch Push-Empfänger noch Versand sind belegbar;"
          + " das ist ausdrücklich KEIN Nachweis, dass es keine Mandate oder keine Empfänger gibt.";
        const unbestimmtAggregat = {
          ok: false, text: unbestimmtText,
          state: "Status nicht bestimmbar", severity: "unbestimmt",
          healthBlockers: [], healthWarnings: ["mandantenliste-nicht-lesbar", "briefingstufen-nicht-bestimmbar"],
          rollingCrawl: null, overdueCrons: [], googleUrlResolutionRate: null
        };
        if (dryRun) {
          return { dryRun: true, ok: false, skipped: true, reason, tenants: 0, text: unbestimmtText, reports: [] };
        }
        const [dl, wh] = await Promise.all([
          sendCallMeBotMessage(buildAlarmText(unbestimmtAggregat)),
          sendMonitoringWebhook(unbestimmtAggregat)
        ]);
        return { ok: false, skipped: true, reason, tenants: 0, text: unbestimmtText, delivery: dl, webhook: wh, reports: [] };
      }
      // GEBÜNDELTER Frischebeleg für ALLE aktiven Mandate: EIN Speicherzugriff je
      // Stapel statt zwei Abfragen je Mandat (Skalierung 25/100, Abnahme 26.08.).
      // Der Frischevertrag ist der EINZIGE Schreiber dieser Belegzeile; ist er
      // abgeschaltet, existiert für kein Mandat je ein Beleg — daraus darf weder Grün
      // noch Rot folgen, sondern nur eine benannte Messlücke.
      const motorPfadAktiv = scalablePipeline.skalierbarerPfadAktiv(process.env);
      const berlinTagKeyLauf = briefingFrische.berlinTagKey(new Date());
      // Mandantenneutrale Motorsignale EINMAL je Lauf (Skalierung, Abnahme 26.08.).
      const motorKontext = motorPfadAktiv ? await motorLaufKontext({}) : null;
      const briefingBelege = motorPfadAktiv
        ? (briefingFrische.vertragAktiv()
          ? await briefingLauf.ladeTageslaeufe(storageModul, tenantIds, berlinTagKeyLauf)
      ...
      const briefingEinzelStatus = motorPfadAktiv
        ? tenantIds.map((tenantId) => briefingProMandat.get(tenantId) || {
          // Messlücke: der Report dieses Mandats ist geworfen. NICHTS davon ist belegt —
          // weder Vorbereitung noch Push-Ereignis noch Empfänger. Alles bleibt `null`.
          mandat: tenantId, vorbereitet: null, quelle: null, status: null, signatur: null,
          ablageKorrekt: null, pushErzeugt: null, empfaenger: null,
          versandBestaetigt: 0, zustellfehler: 0, belegLesefehler: "report-fehler"
        })
        : [];
      ...
      const morgenSlotMs = (() => {
        const teile = /^(\d{4})-(\d{2})-(\d{2})$/.exec(String(berlinTagKeyLauf || ""));
  ...
  // ═══ ZWEITER MORGENSLOT (Kapazitaetssprint 2026-08-09) ═══════════════════════════════════
  // WOFUER: ein Morgenslot traegt begrenzt viele Lage-Narrative (230 s Arbeitsbudget je
  // Worker, hoechstens 8 Worker). Fuer 200 Mandate mit Reserve reicht ein Slot nicht — die
  // Rechnung steht in `docs/betrieb/op30-kapazitaet-morgenslots-2026-08-09.md`. Der zweite
  // Slot holt, was der erste nicht geschafft hat, und baut Rueckstand aus der Nacht ab.
  //
  // WARUM EINE EIGENE ROUTE UND NICHT EIN ZWEITER ZEITEINTRAG AUF `lage-briefing`:
  // bei AUSGESCHALTETEN Flags liefe `lage-briefing` seine Direktschleife — ein zweiter
  // Zeiteintrag wuerde die BESTEHENDE Production-Verarbeitung verdoppeln. Diese Route
  // dagegen ist ein reiner Warteschlangen-Slot: sie hat KEINEN Altpfad.
  //
  // DIE DREI RIEGEL, in dieser Reihenfolge und VOR jedem Datenbankschreibzugriff und jedem
  // Modellaufruf:
  //   1. `authorizeCron` (fail closed: 503 ohne Secret, 403 bei falschem Secret),
  //   2. `narrativUeberWarteschlange()` — beide Flags muessen an sein,
  //   3. der Handler selbst prueft die Flaggrenze noch einmal (scalable-pipeline).
  // Ein versehentlich ausgeloester Slot bei ausgeschalteten Flags schreibt NICHTS: keine
  // Lauftelemetrie, keine Warteschlangenzeile, kein Modellaufruf. Er antwortet und endet.
  //
  // AUFTEILUNG DER ARBEIT: es gibt keine. Beide Slots ziehen aus DERSELBEN Warteschlange,
  // und `helmut_claim_jobs` reserviert atomar mit `for update skip locked` — zwei Slots
  // koennen denselben Auftrag nicht bekommen, auch wenn sie sich zeitlich ueberlappen. Eine
  // Partitionierung waere zusaetzliche Mechanik ohne zusaetzliche Sicherheit.
  if (url.pathname === "/api/cron/lage-briefing-nachlauf") {
    if (!authorizeCron(request, url, response)) return;
    return handleAsync(response, async () => {
      if (!scalablePipeline.narrativUeberWarteschlange()) {
        // KEIN Schreibvorgang, kein Modellaufruf, keine Telemetriezeile — und ein ehrliches
        // Wort statt eines stillen 200ers.
        console.log("[cron/lage-briefing-nachlauf] uebersprungen — OP-30-Flags aus, keine Verarbeitung");
        return {
          pfad: "aus", ok: true, uebersprungen: true, verarbeitet: 0,
          grund: "flags-aus: HELMUT_SCALABLE_PIPELINE und HELMUT_NARRATIV_QUEUE muessen beide an sein"
        };
      }
      const nachlaufStartMs = Date.now();
      return narrativSlotLauf({
        slot: "nachlauf",
        startMs: nachlaufStartMs,
        runId: helmutRunId("briefing-lage-nachlauf", nachlaufStartMs)
      });
    });
  }
...
// Mandantenneutrale Signale des Motorberichts — GENAU EINMAL je Lauf. Keine dieser
...
async function buildMotorHealthReport(politicianId, {
  ...
  // MANDANTENNEUTRALE Signale werden EINMAL JE LAUF geholt, nicht einmal je Mandat:
  // keines dieser acht Signale nimmt eine Mandantenkennung entgegen, und bei 25 oder
  // 100 Mandaten wären es sonst 8x25 bzw. 8x100 identische Abfragen (Abnahme 26.08.,
  // Pflichtprüfung 4). Der Aufrufer reicht sie über `laufKontext` durch; fehlt der
  ...
  // Einzelpfad (`ausZeile` verwirft eine fremde Zeile). Fehlt der Baustein ganz, ist
  // das eine Messlücke und weder Grün noch Rot.
  const briefingBeleg = briefingTageslauf || { lauf: null, quelle: null, fehler: "briefingbeleg-nicht-uebergeben" };
  const pushEvents = pushUebersicht ? pushUebersicht.events : null;
  const pushAbos = pushUebersicht ? pushUebersicht.subscriptions : null;
  ...
  // `null` heisst „Push-Speicher nicht lesbar" — dann wird KEINE Null behauptet.
  const pushSent24 = pushEvents == null
    ? null
    : pushEvents.filter((e) => e.createdAt && (now - new Date(e.createdAt).getTime()) < day && Number(e.delivered) > 0).length;
  ...
  // Derselbe Berliner Tagesschlüssel wie beim gebündelten Frischebeleg des Laufs —
  // sonst könnten Beleg und Push-Filter am Tageswechsel auseinanderlaufen.
  const berlinTagKey = berlinTag || briefingFrische.berlinTagKey(new Date(now));
  const briefingPushPraefix = `briefing-push:${politicianId}:${berlinTagKey}:`;
  const briefingPushes = pushEvents == null
    ? null
    : pushEvents.filter((e) => String(e.dedupeKey || "").startsWith(briefingPushPraefix));
  ...
  const belegVerworfen = Boolean(briefingBeleg && briefingBeleg.fehler === "beleg-passt-nicht");
  const belegLesefehler = Boolean(briefingBeleg && briefingBeleg.fehler && !belegVerworfen);
  const briefingEinzel = {
    mandat: politicianId,
    vorbereitet: Boolean(briefingBeleg && briefingBeleg.lauf
      && briefingBeleg.lauf.status === briefingLauf.STATUS_ERFOLG),
    quelle: (briefingBeleg && briefingBeleg.quelle) || null,
    status: (briefingBeleg && briefingBeleg.lauf && briefingBeleg.lauf.status) || null,
    signatur: (briefingBeleg && briefingBeleg.lauf && briefingBeleg.lauf.signatur) || null,
    // true = geprüft korrekt abgelegt · false = fremder Mandant/Tag verworfen · null = kein Beleg
    ablageKorrekt: briefingBeleg && briefingBeleg.lauf ? true : (belegVerworfen ? false : null),
    pushErzeugt: briefingPushes == null ? null : briefingPushes.length > 0,
    // Ein Lesefehler der Abos ergibt `null` = „nicht lesbar" — NICHT 0 = „kein Empfänger".
    empfaenger: Array.isArray(pushAbos) ? pushAbos.length : null,
    // `delivered` zählt die vom PUSH-DIENST angenommenen Sendungen (HTTP 2xx des
    // Endpunkts), NICHT eine Bestätigung des Endgeräts — daher `versandBestaetigt`.
    versandBestaetigt: (briefingPushes || []).reduce((n, e) => n + (Number(e.delivered) || 0), 0),
    zustellfehler: (briefingPushes || []).reduce((n, e) => n + (Number(e.failed) || 0), 0),
    belegLesefehler: belegLesefehler ? String(briefingBeleg.fehler) : null
  };
  ...
  const text = [
    ...
    `👤 ${active7} aktiv (7T) · 💬 ${feedback24} Feedback · 📲 ${pushSent24 == null ? "Push-Versand nicht lesbar" : `${pushSent24} Push-Versand angenommen`} (24h)`,
...
// ── DER NARRATIVSLOT — EINE Umsetzung fuer BEIDE Morgenslots ────────────────────────────────
// Frueher stand dieser Ablauf inline in `/api/cron/lage-briefing`. Mit einem zweiten Slot
// (`/api/cron/lage-briefing-nachlauf`) waere daraus eine Kopie geworden — und Kopien laufen
// auseinander. Genau ein Ablauf, zwei Aufrufer; der einzige Unterschied ist der Name des
// Slots in Telemetrie und Log.
//
// PARALLELITAET: die Morgenslots duerfen eine EIGENE bekommen. Der allgemeine Standard
// (`HELMUT_WORKER_PARALLEL`, Vorgabe 2) ist bewusst klein, weil `source_fetch` gegen die
// Google-Drosselung laeuft — dort schaden mehr Worker. Das Lage-Narrativ ruft kein Google
// an; es wartet auf das Modell. `HELMUT_NARRATIV_PARALLEL` hebt deshalb NUR die Morgenslots
// an, ohne die Crawl-/Pipeline-Slots zu beruehren. Ohne die Variable gilt unveraendert der
// allgemeine Wert. Die harte Klemmung (1..8) von `worker-betrieb.grenzenAusEnv` gilt fuer
// beide Wege — ein Aufrufer kann sie nicht umgehen.
function narrativSlotGrenzen() {
  ...
  return {
    ...
    // fehlende war KEIN Kapazitaetsproblem, sondern eine Wiederholung, die kurz nach dem
    // Slotende faellig wurde. 270 s sind gegenueber 300 s dieselbe Sicherheitsmarge, die der
    // Bestand schon faehrt — kein neues Risiko, nur eine ungenutzte Reserve gehoben.
    budgetMs: 270000,
    leaseMs: 300000,
    stapel: Math.max(1, Number(process.env.HELMUT_WORKER_BATCH) || 10),
    // LEERLAUFWARTEN, nur hier: ein Worker, der selbst Arbeit zurueckgestellt hat, wartet
    // 20 s und schaut noch einmal nach, statt den Slot mit ungenutztem Budget zu beenden.
    // Der Wiederholungs-Backoff beginnt bei 15–30 s (`scalable-pipeline.backoffMs`) — genau
    // die Spanne, die sonst zwischen zwei Morgenslots verlorenginge. Ohne eigene
    // Zurueckstellung wartet niemand; ein leerer Morgen kostet keine Sekunde extra.
    leerlaufWarteMs: 20000,
    ...(Number.isFinite(parallel) && parallel > 0 ? { parallel } : {})
  };
}

async function narrativSlotLauf({ slot = "morgen", startMs = Date.now(), runId = null } = {}) {
  ...
  // `llm-budget-fair` wird nie gezogen und es entsteht kein Bereichsverbrauch. Genau
  // Befund O1, bei dem Verbraucher, fuer den O1 gebaut wurde. `tagesplanFuerLauf` plant
  // NICHTS — es liest die aktiven Profile und rechnet die Zuteilung. Schlaegt es fehl, ist
  // der Plan `null` und der Worker meldet das ehrlich als `budgetSchicht: "ohne-tagesplan"`.
  const narrativTagesplan = await scalablePipeline
    .tagesplanFuerLauf({ jetztMs: startMs })
    .catch(() => null);
  const durchlauf = await workerBetrieb.durchlauf({
    kennung: runId,
    grenzen: narrativSlotGrenzen(),
    tagesplan: narrativTagesplan,
    typen: ["tenant_narrative"]
  });
  // BELEGTER FEHLER (Abschlussreview PR #236, Befund R3): der Status stand auf `success`,
  // sobald der Worker ueberhaupt STARTETE. Fehlte die Migration, war die Warteschlange
  // unerreichbar, der Altpfad durch das Flag abgeschaltet und es entstand KEIN einziges
  // Narrativ — gemeldet wurde trotzdem ein Erfolg. Das ist genau das falsche Gruen, das
  // CLAUDE.md §4.4 verbietet.
  ...
  const lageTelemetrie = await recordProcessRun({
    process: slot === "nachlauf" ? "briefing-lage-nachlauf" : "briefing-lage",
    runId, mode: "warteschlange", location: helmutExecLocation(),
    startedAt: new Date(startMs).toISOString(), finishedAt: new Date().toISOString(),
  ...
  console.log(`[cron/lage-briefing/${slot}] ${Date.now() - startMs}ms`
    + ` worker=${durchlauf.worker} reserviert=${durchlauf.reserviert || 0} erledigt=${durchlauf.erledigt || 0}`
    + ` zurueckgestellt=${durchlauf.zurueckgestellt || 0}`
    + ` verfuegbar=${warteschlangeVerfuegbar}${narrativGrund ? ` grund=${narrativGrund}` : ""}`
    + ` tagesplan=${narrativTagesplan ? "ja" : "nein"} lauf=${runId}`);
  return {
    ...durchlauf,
    pfad: "warteschlange",
    slot,
