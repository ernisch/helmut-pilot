-- Helmut — Berliner Abnahmeprofil · Schritt 2 von 4 · Rueckweg Stufe 0 · Profil deaktivieren (schnellster Rueckweg)
...
--
-- Nimmt das Profil sofort aus der Referenzzaehlung. Danach hat Berlin kein berechtigtes
-- Landtagsmandat mehr — alle Berliner Wege fallen aus dem Crawl-Plan, unabhaengig vom Flag und
-- unabhaengig vom Wegzustand. Kein Datenverlust, Auditspur bleibt.
--
...
begin
  ...
  if ist not in (1) then
    raise exception 'VORBEDINGUNG VERLETZT: Es gibt keine Mandatszeile unter dieser Id — nichts zu deaktivieren (ist: %, erlaubt: 1)', ist;
  end if;
end $$;

-- ---- MUTATION ----
-- Deaktivieren schlägt Löschen: die Zeile bleibt als Auditspur stehen.
update public.mandate_profiles set aktiv = false, updated_at = now()
