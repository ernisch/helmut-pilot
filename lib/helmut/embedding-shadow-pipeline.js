...
//   deployment?, apiVersion?, modelVersion? (Metadaten für den Datenvertrag),
//   nachBatch? (async Hook nach jedem Batch — z. B. Lock-Erneuerung/Fortschritt;
//   Rückgabe { abbrechen: true, grund? } bricht den Lauf kontrolliert ab)
// provider: async ({ model, dim, inputs: string[] }) =>
//   { vectors: number[][], usage?: { prompt_tokens?, total_tokens? } }
async function runShadowPipeline(objekte, store, provider, cfg = {}) {
  ...
  const bericht = {
    ...
    berechnet: 0,
    apiAufrufe: 0,
    tokensProvider: null, // vom Provider gemeldete Ist-Tokens (null = nicht gemeldet)
    fehlgeschlagen: [],
    abgebrochen: false,
    abbruchGrund: null
  };
  ...
  for (let batchIndex = 0; batchIndex < batches.length; batchIndex += 1) {
    ...
    if (providerFehler) {
      ...
      for (const auftrag of batch) schreibeFehler(store, auftrag, cfg, jetzt, `provider: ${providerFehler.message}`);
      bericht.fehlgeschlagen.push(...batch.map((a) => ({ id: a.id, grund: `provider: ${providerFehler.message}` })));
      providerFehlerInFolge += 1;
      if (providerFehlerInFolge >= 2) {
        bericht.abgebrochen = true;
        bericht.abbruchGrund = "provider-fehler-in-folge";
        await store.speichern();
        break;
      }
    } else {
      providerFehlerInFolge = 0;
      const usage = antwort && antwort.usage;
      const istTokens = usage == null ? NaN
        : Number(usage.total_tokens != null ? usage.total_tokens : usage.prompt_tokens);
      if (Number.isFinite(istTokens)) {
        bericht.tokensProvider = Number(bericht.tokensProvider || 0) + istTokens;
      }
      ...
      await store.speichern(); // nach jedem Batch: Abbruch verliert höchstens einen Batch
    }
    if (typeof cfg.nachBatch === "function") {
      const rueckmeldung = await cfg.nachBatch({
        batch: batchIndex + 1, batches: batches.length, bericht
      });
      if (rueckmeldung && rueckmeldung.abbrechen) {
        bericht.abgebrochen = true;
        bericht.abbruchGrund = rueckmeldung.grund || "nach-batch-hook";
        break;
      }
    }
  }
