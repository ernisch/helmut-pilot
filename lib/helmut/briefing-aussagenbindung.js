...
const ENUMS = {
  ...
  recommendedChannel: ["internal", "none", "unknown"], recommendedFormat: ["none", "unknown"], actionType: ["unknown"],
  helmutQualityStatus: ["partial", "complete", "error"], qualityStatus: ["partial", "complete", "error", "empty"],
...
const AUSGABEPFAD = /^(?:\/helmutAssessment\/(?:greeting|assessment)|\/executiveSummary|\/currentRadarState\/(?:anzeige\/)?summary\/(?:line1|line2|text)|\/currentHelmutState\/emptyState\/(?:headline|detail)|\/pruefumfang\/hinweis)$/;
...
function texte(briefing, { kos = [], quellen = [] } = {}) {
  ...
  function walk(value, path = "", vorgangId = null) {
    ...
    for (const [key, v] of Object.entries(value)) {
      ...
      if (path === "/pruefumfang" && key === "ergaenzteVorgaenge"
        && Array.isArray(v) && v.every(x => typeof x === "string" && vorgangIds.has(x))) continue;
      if (path === "/currentHelmutState/emptyState" && key === "kind" && v === "quiet") continue;
      ...
      // Nur die neuen, strukturiert belegten Rollenhinweise sind Aussagen über
      // Auswahl/Quellenmetadaten. Freie politische Prosa bleibt Fachaussage.
      const rollenArt = /\/(?:tagesAnlass|beobachtungsBeleg)$/.test(path) && key === "art"
        && ["neue-quelle", "heutige-frist", "kommende-frist", "weitere-berichte", "eigenerwaehnung"].includes(v);
      if (rollenArt) continue;
      const rollenHinweis = (key === "whyItMatters" && path === "/currentHelmutState" && value.tagesAnlass)
        || (key === "whyRelevant" && /^\/currentHelmutState\/items\/\d+$/.test(path) && value.tagesAnlass)
        || (key === "text" && /\/(?:tagesAnlass|beobachtungsBeleg)$/.test(path))
        || (key === "evidence" && path.startsWith("/currentRadarState/anzeige/") && value.beobachtungsBeleg);
      const ausgabe = AUSGABEPFAD.test(p) || Boolean(rollenHinweis);
      ...
      if (typeof v === "string" && v.trim())
        out.push({ pfad: p, text: v, vorgangId: ausgabe ? null : auswertungsBezug || bezug,
          art: ausgabe ? "ausgabe" : "fachaussage" });
...
function quellenVertrag(sourcesByVorgang = {}) {
  return Object.entries(sourcesByVorgang).sort(([a], [b]) => a.localeCompare(b))
    .flatMap(([vorgangId, docs]) => (docs || []).map(d => {
      const dokumentangaben = require("./dip-quellfelder").quellenangaben(d);
      return { vorgangId, documentId: d.id,
...
function baueEingabe({ briefing, sourcesByVorgang = {}, profile, userId, day, kos = [], korrekturHash = null }) {
  ...
  const inhalt = { version: VERSION, mandat: userId, tag: day, profilHash: profilHash(profile),
    ...
    // JSON Objektschluessel haben keine fachliche Reihenfolge. Nur die
    // Pruefaussagen ordnen; Kartenpositionen bleiben in Pfad und Darstellung gebunden.
    aussagen: texte(briefing, { kos, quellen }).sort((a, b) => a.pfad < b.pfad ? -1 : a.pfad > b.pfad ? 1 : 0),
    quellen, korrekturHash,
...
async function leseFuerNachlauf({ profile, userId, build, storage = require("./storage"), now = new Date() }) {
  ...
  let row = await storage.getRenderedBriefingV3(userId, SLOT, day, { strict: true });
  ...
  let nachfolger = false;
  const altUrteil = row?.payload?.importbeleg ? row.payload.urteil : row?.payload;
  if (altUrteil?.korrektur && altUrteil.korrektur.ursprungHash !== original.eingabe.eingabeHash) {
    const next = await require("./briefing-urteilsnachfolger").lese({ userId, tag: day,
      ursprungHash: original.eingabe.eingabeHash, storage });
    if (next) { row = next; nachfolger = true; }
  }
  if (!row) return { bereit: false, grund: "briefing-aussagenpruefung-fehlt" };
  if (row.user_id !== userId || row.slot !== SLOT || row.id !== (nachfolger ? require("./briefing-urteilsnachfolger").kennung(userId, day, original.eingabe.eingabeHash) : `bf-${userId}-${SLOT}-${day}`))
    throw new Error("briefing-aussagen-kontext-abweichend");
  const payload = row.payload?.importbeleg ? row.payload.urteil : row.payload;
  if (payload?.korrektur) {
    const result = await build(profile, userId, { aussagenEingabe: true, now,
      aussagenKorrektur: payload.korrektur });
    if (payload.korrektur.ursprungHash !== original.eingabe.eingabeHash)
      throw new Error("briefing-korrektur-abweichend");
    if (row.payload?.importbeleg) require("./briefing-urteilsimport").pruefeZeile(row, result, { gelesen: true, nachfolger });
    const einzel = pruefe(result.eingabe, payload);
    const check = einzel.bereit ? { ...einzel,
      ...require("./briefing-fachurteil").pruefe(result, payload) } : einzel;
    ...
    return { ...check, ...(check.bereit ? { briefing: result.briefing,
      lageEingabe: require("./briefing-lagebindung").baue(result, payload) } : {}) };
  }
  const ids = auslassungen(original.eingabe, payload);
