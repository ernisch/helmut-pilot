...
function texte(briefing) {
  const out = [];
  function walk(value, path = "", vorgangId = null) {
    if (!value || typeof value !== "object") return;
    const bezug = value.vorgangId || value.vorgang_id || vorgangId;
    for (const [key, v] of Object.entries(value)) {
      ...
      if (typeof v === "string" && v.trim() && !META.has(key) && !TECHNISCH.test(key))
        out.push({ pfad: p, text: v, vorgangId: bezug });
...
function baueEingabe({ briefing, sourcesByVorgang = {}, profile, userId, day }) {
  if (!profile || profile.id !== userId || !/^\d{4}-\d{2}-\d{2}$/.test(day || ""))
    throw new Error("briefing-aussagen-kontext-abweichend");
  const quellen = Object.entries(sourcesByVorgang).sort(([a], [b]) => a.localeCompare(b))
  ...
  const inhalt = { version: VERSION, mandat: userId, tag: day, profilHash: profilHash(profile),
    ...
    aussagen: texte(briefing), quellen };
...
module.exports = { VERSION, SLOT, texte, baueEingabe, pruefe, auslassungen, leseFuerNachlauf };
