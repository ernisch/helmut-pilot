...
function buildCurrentHelmutState({ profile = {}, decisions = [], kosById = {}, sourcesByVorgang = {}, now = new Date(), tenantId = null, briefingType = "daily" } = {}) {
  ...
  if (!candidates.length) {
    ...
    // (frisch, aber kein Handlungsansatz fuers Mandat).
    const es = scoring.scoringActive()
      ? scoring.tabEmptyState("helmut", { kos: kosValuesForFreshness(kosById), hasRankedItems: false, now: nowDate.getTime() })
      : null;
...
function toBriefingContractV3({ profile = {}, decisions = [], kosById = {}, sourcesByVorgang = {}, now = new Date(), reason = null, briefingType, knowledgeObjects = null } = {}) {
  const nowDate = now instanceof Date ? now : new Date(now);
  const generatedAt = nowDate.toISOString();
  const getKo = (id) => (kosById instanceof Map ? kosById.get(id) : kosById[id]);
