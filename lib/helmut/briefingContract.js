...
const { normalizeBriefingType } = require("./briefingLanguage");
...
//     keine Cem-Logik. contextChips/riskSummary etc. stammen NUR aus KO-/Decision-Daten.
...
function emptyCurrentHelmutState({ generatedAt, briefingType, profileId = null, tenantId = null } = {}) {
  return {
...
function buildCurrentHelmutState({ profile = {}, decisions = [], kosById = {}, sourcesByVorgang = {}, now = new Date(), tenantId = null, briefingType = "daily" } = {}) {
  ...
  if (!candidates.length) return emptyCurrentHelmutState(meta);

  const primary = candidates[0];
  // Weitere relevante Vorgaenge: die naechsten NICHT-ignorierten, wenige (keine Monitoring-Liste).
  const related = candidates.slice(1)
    .filter((c) => c.d.decision !== "Ignorieren")
    .slice(0, HELMUT_RELATED_CAP);
  ...
  return {
    ...
    generatedAt,
    briefingType: slot,
    status: publicStatusFromQuality(qualityStatus),
    ...
    qualityStatus,
    staleState: qualityStatus === "stale",
...
function toBriefingContractV3({ profile = {}, decisions = [], kosById = {}, sourcesByVorgang = {}, now = new Date(), reason = null, briefingType } = {}) {
  const nowDate = now instanceof Date ? now : new Date(now);
  const generatedAt = nowDate.toISOString();
  const getKo = (id) => (kosById instanceof Map ? kosById.get(id) : kosById[id]);
  ...
  return {
    ...
    currentHelmutState: buildCurrentHelmutState({ profile, decisions: ordered, kosById, sourcesByVorgang, now: nowDate, ...(briefingType !== undefined ? { briefingType } : {}) }),
