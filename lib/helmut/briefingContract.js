...
const { HELMUT_STAFF_PROSE_FIELDS, HELMUT_STAFF_LIST_FIELDS } = require("./understanding-schema");
...
//   empty   = kein Stabschef-Feld befuellt (oder Vorgang noch 'pending')
//   stale   = Felder vorhanden, aber KO aelter als HELMUT_STAFF_STALE_DAYS
//   partial = nur ein Teil der Felder befuellt
//   valid   = alle Stabschef-Felder befuellt UND frisch
function deriveHelmutQualityStatus(ko = {}, now = new Date()) {
  ...
  let present = 0;
  for (const f of HELMUT_STAFF_PROSE_FIELDS) if (cleanStr(ko[f])) present += 1;
  for (const f of HELMUT_STAFF_LIST_FIELDS) if (Array.isArray(ko[f]) && ko[f].some((x) => cleanStr(x))) present += 1;
  const total = HELMUT_STAFF_PROSE_FIELDS.length + HELMUT_STAFF_LIST_FIELDS.length;
  ...
  return present === total ? "valid" : "partial";
}

// Die vier Stabschef-Felder als camelCase-Vertragsform (fuer CurrentHelmutState).
// Kommen AUSSCHLIESSLICH aus dem V3-Motor (KO). KEIN Demo-Text, KEIN politischer
// Fallback: fehlt ein Feld -> "" bzw. [] (der helmutQualityStatus macht das ehrlich
// sichtbar). Es wird hier NICHTS erfunden und keine Partei/Person hartkodiert.
function koStaffFields(ko = {}, now = new Date()) {
  const actionItems = Array.isArray(ko.action_items)
    ? ko.action_items.map((x) => cleanStr(x)).filter(Boolean)
    : [];
  return {
    riskOfNoAction: cleanStr(ko.risk_of_no_action),
    opportunitySummary: cleanStr(ko.opportunity_summary),
    recommendedCommunication: cleanStr(ko.recommended_communication),
    actionItems,
