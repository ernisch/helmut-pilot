...
const { briefingArticleIdentity } = require("./dedup");
...
function firstSentence(text, maxLen = 160) {
  ...
  // Keine Aussage ohne ihre spaetere Voraussetzung oder Verneinung erzeugen.
...
// Ersatztitel nur an der bestehenden Satzgrenze beenden. Die fruehere Grenze
// von 90 Zeichen erzeugte mitten im Satz abbrechende Titel (500er Test 15.09.).
// Keine Woerter ergaenzen, keine andere Quelle vorziehen, keine Eingabe aendern.
...
// Nur vorhandene ganze Empfehlungstexte verwenden; Fragmente nicht ergaenzen.
function koAction(ko = {}) {
  const { ohneAbschneidung } = require("./empfehlungs-text");
  const max = require("./understanding-schema").KNOWLEDGE_OBJECT_SCHEMA.properties.recommendation.maxLength;
  return ohneAbschneidung(ko.recommendation, max)
    || ohneAbschneidung(ko.handlungsempfehlung, 800); // bestehende PROSE_FIELDS Speichergrenze
}
...
// Globale KO-Prosa ist kein individueller Auftrag. Insbesondere darf eine
// ignorierte Karte ohne jede Profilzuordnung keine Wahlkreisbetroffenheit oder
// persoenliche Handlung aus dem globalen Understanding uebernehmen. Quellen und
// Sachzusammenfassung bleiben sichtbar und weiterhin fachlich pruefpflichtig.
function unzugeordnetIgnoriert(decision = {}) {
  return decision.decision === "Ignorieren"
    && (!Array.isArray(decision.matched_features) || decision.matched_features.length === 0);
}
const OHNE_PROFILBEZUG = "Für diesen Vorgang ist kein konkreter Bezug zum gespeicherten Profil belegt.";
...
function buildSources(ko = {}, docs = []) {
  ...
  const list = (Array.isArray(docs) ? docs : [])
    .map((d) => {
      ...
      documentIds.set(source, d.id || null);
      return source;
    })
    .filter((s) => s.url);
  if (!list.length && ko.best_source_url) {
    list.push({
      ...
      sourceType: null,
      // EHRLICHKEIT (Quellenpflicht-Sprint, wie lage.js loadSourcesForVorgaenge):
      // ko.updated_at ist Helmuts ANALYSE-Zeitpunkt, nicht der Veroeffentlichungs-
      // zeitpunkt der Quelle. Ohne echtes Veroeffentlichungsdatum bleibt der Wert leer.
      publishedAt: null
    });
  }
  // Fassungen desselben belegten Artikels sind keine unabhaengigen Quellen.
  // Erstes vorhandenes Linkziel und seine Texte bleiben zusammen. Alle Varianten
  // bleiben nachvollziehbar; widerspruechliche Zeiten werden NICHT aufgeloest.
  const groups = new Map();
  const result = [];
  for (const source of list) {
    const identity = briefingArticleIdentity(source.url);
    if (!identity) { result.push(source); continue; }
    const group = groups.get(identity);
    if (group) group.push(source);
    else { groups.set(identity, [source]); result.push(source); }
  }
  return result.map(source => {
    const identity = briefingArticleIdentity(source.url);
    const group = groups.get(identity);
    if (!group || group.length < 2) return source;
    const times = group.filter(s => s.publishedAt);
    const keys = new Set(times.map(s => {
      const ms = Date.parse(s.publishedAt);
      return Number.isFinite(ms) ? ms : `invalid:${s.publishedAt}`;
    }));
    const conflict = keys.size > 1;
    const publishedAt = !conflict && times.length && Number.isFinite(Date.parse(times[0].publishedAt))
      ? times[0].publishedAt : null;
    return { ...source, publishedAt, publishedAtConflict: conflict, articleIdentity: identity,
      variants: group.map(s => ({ ...s, documentId: documentIds.get(s) || null })) };
  });
}
...
function toItem(decision, ko, sources) {
  ...
  const ohneBezug = unzugeordnetIgnoriert(decision);
  return {
    ...
    priority: decision.decision === "Ignorieren" ? "Ignorieren" : priorityLabelFromScore(score),
    priorityType: decision.priority_type,
    finalScore: score,
    relevance_score: score,
    recommendedAction: ohneBezug ? "" : koAction(ko),
    whyItMatters: ohneBezug ? OHNE_PROFILBEZUG : koWhy(ko),
    summary: koSummary(ko, primarySource),
    riskNote: ohneBezug ? "" : decision.risk || "",
    opportunityNote: ohneBezug ? "" : decision.chance || "",
...
function toRecommendation(decision, ko, sources, now = new Date()) {
  ...
  return {
    ...
    ...koStaffFields(ohneBezug ? { updated_at: ko.updated_at, created_at: ko.created_at, status: ko.status, understanding_status: ko.understanding_status } : ko, now),
    ...
    decision: decision.decision,
    priorityType: decision.priority_type,
    current_priority: decision.decision === "Ignorieren" ? "Ignorieren" : priorityLabelFromScore(score),
    previous_priority: null,
    recommended_action: ohneBezug ? "" : koAction(ko),
    personal_relevance_explanation: ohneBezug ? OHNE_PROFILBEZUG : koWhy(ko),
    consequence_if_ignored: ohneBezug ? "" : decision.risk || "",
    possible_upside: ohneBezug ? "" : decision.chance || "",
...
function buildHelmutAssessment(profile, items, generatedAt, top) {
  ...
  return {
    ...
    assessment: top
      ...
      : "Kein belegter Hauptvorgang verfügbar.",
...
function buildPersonMentions(kos, sourcesByVorgang = {}) {
  const { artikelUrl } = require("./lage-quellenbeleg");
  const norm = s => s.normalize("NFC").toLocaleLowerCase("de").replace(/\s+/gu, " ").trim();
  const seen = new Map();
  for (const ko of kos) {
    // Ein Analysefeld ist kein Namensbeleg. Nur die bereits geladenen Texte
    // desselben Vorgangs zaehlen; weder KO-Text noch URL/Herausgeber ersetzen sie.
    const docs = sourcesByVorgang[ko.vorgang_id];
    const texte = (Array.isArray(docs) ? docs : [])
      .filter(d => d && artikelUrl(d.url || d.canonical_url))
      .flatMap(d => [d.title, d.summary]).filter(t => typeof t === "string").map(norm);
    for (const name of [].concat(ko.mentioned_mps || [], ko.mentioned_people || [])) {
      if (typeof name !== "string") continue;
      // Rollen/Parteien in Klammern werden nicht als Personenname vererbt.
      // Fehlende Vornamen werden weder ergaenzt noch auf Nachnamen reduziert.
      const label = name.replace(/\s*\([^()]*\)\s*$/u, "").replace(/\s+/gu, " ").trim();
      const key = norm(label);
      if (!key) continue;
      const escaped = key.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
      const muster = new RegExp(`(?<![\\p{L}\\p{M}\\p{N}_])${escaped}(?![\\p{L}\\p{M}\\p{N}_])`, "u");
      if (!texte.some(t => muster.test(t))) continue;
      if (!seen.has(key)) seen.set(key, { name: label, vorgaenge: new Set() });
      seen.get(key).vorgaenge.add(ko.vorgang_id);
    }
  }
  return [...seen.values()].map(r => ({ name: r.name, mentions: r.vorgaenge.size }))
...
// Nur bei tatsaechlich zusammengefuehrten Varianten ersetzt die bereinigte
// Quellenmenge die alten Dokumentzeiten. null heisst hier ausdruecklich:
// kein eindeutiger Publikationszeitpunkt belegt, kein Fallback auf Analysezeit.
// undefined erhaelt das Verhalten aller nicht betroffenen Vorgaenge.
function artikelMeldungAt(ko, docs) {
  const sources = buildSources(ko, docs);
  if (!sources.some(s => s.variants)) return undefined;
  const dated = sources.filter(s => Number.isFinite(Date.parse(s.publishedAt || "")));
  if (dated.length) return dated.sort((a, b) => Date.parse(b.publishedAt) - Date.parse(a.publishedAt))[0].publishedAt;
  return null;
}
...
function helmutLastUpdated(ko = {}, docs = []) {
  // Das Feld ist auch Anzeigeersatz fuer meldungAt. Ein Quellenkonflikt darf
  // daher weder ueber KO-Schreibzeit noch ueber Variantenzeiten frisch wirken.
  if (artikelMeldungAt(ko, docs) === null) return null;
  const candidates = [];
...
// BELEGTER MELDUNGSZEITPUNKT (Audit 2026-08-10, Befund F2).
// `helmutLastUpdated` ist ein MAXIMUM ueber `ko.updated_at` — und das ist Helmuts
// EIGENER Schreibzeitpunkt, nicht der Zeitpunkt einer Entwicklung. Jede Reklassifizierung,
// jeder Backfill (presentation-/staff-backfill) und jedes erneute Speichern hebt ihn auf
// „heute". Wird die Frischeklasse daran gemessen, rutscht ein wochenalter Vorgang unter
// „Neu seit dem letzten Briefing" und traegt dort sogar das Label „Heute" — also genau
// die alte Lage im Gewand der heutigen. Deshalb gibt es hier einen ZWEITEN, ehrlichen
// Zeitstempel: den juengsten BELEGTEN Veroeffentlichungszeitpunkt der Quelldokumente,
// hilfsweise die Ersterfassung des Vorgangs (`created_at`, aendert sich nie nachtraeglich).
// NIEMALS `updated_at`, wenn ein Meldungsdatum belegt ist. Frischeauswahl,
// Status und Anzeige folgen diesem Datum; `lastUpdated` bleibt als Metadatum erhalten.
function helmutMeldungAt(ko = {}, docs = []) {
  const articleDate = artikelMeldungAt(ko, docs);
  if (articleDate !== undefined) return articleDate;
  const belegt = [];
  for (const d of (Array.isArray(docs) ? docs : [])) {
    if (d && d.published_at) belegt.push(d.published_at);
  }
  const times = belegt.map((t) => ({ t, ms: Date.parse(t) })).filter((x) => Number.isFinite(x.ms));
  if (times.length) return times.sort((a, b) => b.ms - a.ms)[0].t;
  return (ko && ko.created_at) || null;
}

// QUELLENPFLICHT (P0-Schliessungssprint 2026-08-22): ein Vorgang traegt den sichtbaren
// Stabschefstand (Briefing-Tab) nur mit mindestens EINER echten oeffnenden https-Quelle
// (radarState.oeffnendeArtikelUrl: https, keine Herausgeber-Startseite, kein Google-
// Proxy). Ein blosser Zaehlerwert (source_document_count) genuegt NICHT — gleiche Regel
// wie in der Lage. Ohne oeffnende Quelle: ehrlicher Leerzustand statt Inhaltskarte.
function oeffnendeHelmutQuelle(ko = {}, docs = []) {
  for (const s of buildSources(ko, docs)) {
    const url = radarState.oeffnendeArtikelUrl(s.url);
    if (url) return { url, name: s.name || "" };
  }
  return null;
}
...
function toHelmutVorgangItem(ko = {}, docs = [], now = new Date(), opts = {}) {
  ...
  const sources = buildSources(ko, docs);
  const primarySource = sources[0] || null;
  // Quellenpflicht: oeffnende Primaerquelle als Anzeige-Link des Briefing-Tabs.
  // Leer nur bei Altbestand — der Stand-Adapter laesst Vorgaenge ohne oeffnende
  // Quelle gar nicht mehr in den Stand (Kandidaten-Gate in buildCurrentHelmutState).
  const quelle = oeffnendeHelmutQuelle(ko, docs);
  const item = {
    ...
    sourceIds,
    sourceCount: sources.some(s => s.variants) ? sources.length : helmutSourceCount(ko, sourceIds),
    sourceUrl: quelle ? quelle.url : "",
    sourceName: quelle ? quelle.name : "",
    lastUpdated: helmutLastUpdated(ko, docs),
    // Belegter Meldungszeitpunkt (siehe helmutMeldungAt) — Grundlage der Frischeklasse.
    meldungAt: helmutMeldungAt(ko, docs),
...
function helmutCompare(a, b) {
  if (Number.isInteger(a.d.redaktionellerRang) && Number.isInteger(b.d.redaktionellerRang))
    return a.d.redaktionellerRang - b.d.redaktionellerRang;
...
function selectFreshAwarePrimary(candidates = [], now = new Date()) {
  ...
  // Ein belegter heutiger Termin verliert seine Tagesprioritaet nicht allein,
  // weil der zugehoerige Artikel vor mehreren Tagen publiziert wurde.
  if (top.tagesAnlass?.art === "heutige-frist") return { primary: top, displaced: null };
  const todayKey = dateKeyInTimezone(now, DEFAULT_TIMEZONE);
  const dayKeyOf = (c) => dateKeyInTimezone(helmutMeldungAt(c.ko, c.docs) || helmutLastUpdated(c.ko, c.docs), DEFAULT_TIMEZONE);
...
function buildPrimarySelectionDebug({ knowledgeObjectsLoaded = null, understood = [], decisionsBefore = [], decisionsAfter = [], kosById = {}, sourcesByVorgang = {}, now = new Date(), state = null, frischeFenster = null } = {}) {
  ...
  const freshOf = (ko) => {
    const key = dateKeyInTimezone(helmutMeldungAt(ko, docsOf(ko)) || helmutLastUpdated(ko, docsOf(ko)), DEFAULT_TIMEZONE);
  ...
  const candidatesAfter = (Array.isArray(decisionsAfter) ? decisionsAfter : [])
    .map((d) => { const ko = getKo(d.knowledge_object_id); return ko ? { d, ko, docs: docsOf(ko),
      tagesAnlass: require("./briefing-bereichsvertrag").tagesAnlass(docsOf(ko), nowDate, frischeFenster, ko),
      quality: deriveHelmutQualityStatus(ko, nowDate) } : null; })
    .filter(c => c && c.d.decision !== "Ignorieren" && oeffnendeHelmutQuelle(c.ko, c.docs) && c.tagesAnlass)
    .sort(helmutCompare);
  ...
  const annotate = (ko) => {
    ...
    else if (!require("./briefing-bereichsvertrag").tagesAnlass(docsOf(ko), nowDate, frischeFenster, ko)) reason = "kein belegter Tagesanlass";
  ...
  const selectedBecause = !primary ? "kein Kandidat mit belegtem Tagesanlass vorhanden"
    : displaced ? "frischer relevanter Vorgang (Datenstand heute) hat den stale Top-Vorgang verdraengt"
    : "Top-Vorgang mit belegtem Tagesanlass nach Entscheidung, Score und Qualität";
...
function buildCurrentHelmutState({ profile = {}, decisions = [], kosById = {}, sourcesByVorgang = {}, now = new Date(), tenantId = null, briefingType = "daily", knowledgeObjects = null, frischeFenster = null } = {}) {
  ...
  // QUELLENPFLICHT: nur Vorgaenge mit mindestens einer oeffnenden https-Quelle tragen
  // den sichtbaren Stand (auch die per augmentFreshCandidates nachgereichten) — ein
  // blosser source_document_count ohne oeffnende Quelle genuegt nicht mehr.
  let ignoredWithSource = false;
  const candidates = (Array.isArray(decisions) ? decisions : [])
    .map((d) => {
      ...
      if (!oeffnendeHelmutQuelle(ko, docs)) return null;
      // Ignorieren bleibt in den Vertragskarten erhalten, ist aber kein Hauptthema.
      if (d.decision === "Ignorieren") { ignoredWithSource = true; return null; }
      const tagesAnlass = require("./briefing-bereichsvertrag").tagesAnlass(docs, nowDate, frischeFenster, ko);
      if (!tagesAnlass) { ignoredWithSource = true; return null; }
      return { d, ko, docs, tagesAnlass, quality: deriveHelmutQualityStatus(ko, nowDate) };
    })
    .filter(Boolean)
    .sort(helmutCompare);
  ...
  if (!candidates.length) {
    if (ignoredWithSource) return emptyCurrentHelmutState(meta, {
      kind: "quiet", headline: "Kein belegter Tagesanlass für ein Briefing",
      detail: "Die vorhandenen Vorgänge belegen derzeit keine neue Tagespriorität. Sachstand und Einordnung bleiben in der Lage; daraus folgt keine Entwarnung."
    });
  ...
  const seenVorgaenge = new Set([primary.ko.vorgang_id]);
  const seenQuellen = new Set(primary.docs.flatMap(d => [d.id, require("./dedup").canonicalizeUrl(d.url || d.canonical_url), require("./dedup").briefingArticleIdentity(d.url || d.canonical_url)]).filter(Boolean));
  const relatedRest = (displaced ? [displaced, ...candidates.filter(c => c !== displaced)] : candidates).filter((c) => {
    if (c === primary || c.d.decision === "Ignorieren") return false;
    const keys = c.docs.flatMap(d => [d.id, require("./dedup").canonicalizeUrl(d.url || d.canonical_url), require("./dedup").briefingArticleIdentity(d.url || d.canonical_url)]).filter(Boolean);
    if (seenVorgaenge.has(c.ko.vorgang_id) || keys.some(k => seenQuellen.has(k))) return false;
    seenVorgaenge.add(c.ko.vorgang_id); keys.forEach(k => seenQuellen.add(k));
    return true;
  });
  const related = relatedRest.slice(0, HELMUT_RELATED_CAP);
  ...
  const items = related.map((c) => ({ ...toHelmutVorgangItem(c.ko, c.docs, nowDate),
    whyRelevant: c.tagesAnlass.text, tagesAnlass: c.tagesAnlass }));
  ...
  const displayedTs = primaryItem.meldungAt || primaryItem.lastUpdated || null;
  const todayKey = dateKeyInTimezone(nowDate, DEFAULT_TIMEZONE);
  const dataKey = Number.isFinite(Date.parse(displayedTs || ""))
    ? dateKeyInTimezone(displayedTs, DEFAULT_TIMEZONE) : null;
  ...
  // Der Erzeugungszeitpunkt ersetzt keinen fehlenden Datenstand.
  const publicStatus = (baseStatus === "fresh" && (dayStale || !Number.isFinite(displayedMs))) ? "stale" : baseStatus;
  ...
  // OPTISCHE EHRLICHKEIT (Audit 2026-08-10, Befund F3). Der Kopf darf den Namen des
  // heutigen Slots ("Morgenbriefing") nur tragen, wenn der GEZEIGTE Vorgang auch vom
  // heutigen Berliner Kalendertag ist. Mit Briefingfenster kann `publicStatus` zu Recht
  // `fresh` bleiben, obwohl der Vorgang von gestern Abend stammt ("neu seit dem letzten
  // Briefing") — dann wuerde der Kopf ohne diese Angabe "Morgenbriefing · gestern" mischen
  // und wieder wie eine heutige Lage aussehen. Reine ANZEIGE-Angabe, sie stuft nichts um.
  // Grundlage ist der BELEGTE Meldungszeitpunkt, nicht Helmuts Schreibzeitpunkt.
  const belegterStandKey = dataKey;
  ...
  return {
    ...
    // Keine zweite Sachmeldung oder nur umformulierte Lage-Einordnung.
    // Der Briefinggrund benennt den Auswahl-/Tagesanlass, nicht nochmals Fakten.
    whyItMatters: primary.tagesAnlass.text,
    tagesAnlass: primary.tagesAnlass,
    ...
    // Anzeige-Wahrheit (F3): Ist der gezeigte Vorgang vom heutigen Berliner Tag?
    datenstandTag: belegterStandKey || null,
    datenstandVonHeute: Boolean(todayKey && belegterStandKey && belegterStandKey === todayKey),
...
function toBriefingContractV3({ profile = {}, decisions = [], kosById = {}, sourcesByVorgang = {}, now = new Date(), reason = null, briefingType, knowledgeObjects = null, frischeFenster = null } = {}) {
  ...
  // Grundreihenfolge nach Score. Der vorhandene Helmut Hauptvorgang wird unten
  // einmalig vorangestellt, damit Kopf und hervorgehobene Texte zusammenpassen.
  const ordered = [...(Array.isArray(decisions) ? decisions : [])]
    .filter((d) => d && d.knowledge_object_id && getKo(d.knowledge_object_id))
    .sort((a, b) => (Number.isInteger(a.redaktionellerRang) && Number.isInteger(b.redaktionellerRang)
      ? a.redaktionellerRang - b.redaktionellerRang : b.score - a.score)
  ...
  // Einmalige Auswahl inklusive Quellenpflicht und bestehender Frischeauswahl.
  // Alle hervorgehobenen Texte folgen diesem Hauptvorgang, ohne neue Gewichte.
  const currentHelmutState = buildCurrentHelmutState({ profile, decisions: ordered, kosById, sourcesByVorgang, knowledgeObjects, now: nowDate, frischeFenster, ...(briefingType !== undefined ? { briefingType } : {}) });
  const primaryIndex = currentHelmutState.primaryVorgangId
    ? items.findIndex((i) => i.vorgangId === currentHelmutState.primaryVorgangId)
    : -1;
  if (primaryIndex > 0) {
    items.unshift(items.splice(primaryIndex, 1)[0]);
    recommendations.unshift(recommendations.splice(primaryIndex, 1)[0]);
  }
  const themeOfDay = primaryIndex >= 0 ? items[0] : null;
  const assessment = buildHelmutAssessment(profile, items, generatedAt, themeOfDay);
  // Risiko und Chance bleiben eigenständige Kategorien der bewerteten Vorgänge.
  ...
  return {
    ...
    personMentions: buildPersonMentions(usedKos, sourcesByVorgang),
    ...
    status: contractStatus(currentHelmutState),
    generatedAt
  };
}

// Auch direkte Vertragsleser erhalten einen belegten Status. Der App Decorator
// darf diesen anschließend mit dem Laufnachweis weiter herabstufen.
function contractStatus(state) {
  if (state.errorState || state.status === "error") return "Stand nicht verfügbar";
  if (!state.primaryItem || state.status === "empty") return "Keine aktuellen Vorgänge";
  const timestamp = state.primaryItem.meldungAt || state.primaryItem.lastUpdated;
  if (!Number.isFinite(Date.parse(timestamp || ""))) return "Datenstand unbekannt";
  if (state.staleState || state.status === "stale") return "Veraltet";
  if (state.status === "fresh") return "Aktuell";
