...
function buildCurrentHelmutState({ profile = {}, decisions = [], kosById = {}, sourcesByVorgang = {}, now = new Date(), tenantId = null, briefingType = "daily", knowledgeObjects = null } = {}) {
  const nowDate = now instanceof Date ? now : new Date(now);
...
function toBriefingContractV3({ profile = {}, decisions = [], kosById = {}, sourcesByVorgang = {}, now = new Date(), reason = null, briefingType, knowledgeObjects = null } = {}) {
  ...
  return {
    ...
    currentHelmutState: buildCurrentHelmutState({ profile, decisions: ordered, kosById, sourcesByVorgang, knowledgeObjects, now: nowDate, ...(briefingType !== undefined ? { briefingType } : {}) }),
