...
// 'aggregator-gedrosselt' = zentrale Provider-Drosselung (Breaker hat gegriffen).
// Das IST eine echte Degradation der Datenzufuhr und bleibt alarmierend sichtbar —
// aber sie wird nicht mehr als Defekt von N Einzelquellen dargestellt.
const DEGRADED_STATES = new Set(["teilweise-degradiert", "stark-degradiert", "fehlgeschlagen", "aggregator-gedrosselt"]);
const SEVERITY = {
  "gesund": 0,
  "cooldown-reduziert": 1,
  "unbekannt": 1,
  "teilweise-degradiert": 2,
  "aggregator-gedrosselt": 3,
  "stark-degradiert": 3,
  "fehlgeschlagen": 4
};
...
function stateOfRun(run, thresholds) {
  ...
  return classifyCrawlRunState({
    ...
    circuitOpenSources: run && run.circuitOpenSources,
...
// Ist der Lauf bewusst REDUZIERT gelaufen (Schutzmaßnahme, keine Störung)?
// Solche Läufe taugen nicht als Basis für die Frische-/Qualitätsachsen: ihre
// absoluten Zähler (successfulSources) sind konstruktionsbedingt klein und würden
// die Watchdog-Minima reißen. Zwei Fälle:
//   - cooldown.skipGoogle : Abstands-/Degradations-Schutz (bestehend)
//   - sharedSkippedSources: die geteilten Abrufwege hat in diesem Cron-Durchlauf
//     schon ein anderes Mandat geholt (Incident 2026-07-25) — die Dokumente sind
//     im globalen Korpus, dieser Lauf beschreibt die Datenzufuhr also nicht.
function isReducedRun(run) {
  if (!run || typeof run !== "object") return false;
  if (run.cooldown && run.cooldown.skipGoogle) return true;
  return Number(run.sharedSkippedSources || 0) > 0;
}

module.exports = { buildRollingCrawlHealth, stateOfRun, isReducedRun, DEGRADED_STATES };
