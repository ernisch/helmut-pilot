...
// unbekannt oder liegt zu einem mandatsgebundenen Kanal ÜBERHAUPT KEINE
// zuordenbare Angabe vor (weder Kennung noch Empfänger noch Ziel), wird
// gesperrt — nie durchgelassen.
//
// ─── PRÄZISIERUNG 02.09. (adversarialer Review, bestätigter Befund) ──────────
// Hier stand vorher „ist der Empfänger nicht bestimmbar, wird gesperrt". Das
// war ZU GRÜN und ist zurückgenommen: gesperrt wird, wenn ALLE drei Angaben
// fehlen. Liegt eine Empfängeradresse vor, aber keine Mandatskennung, gilt der
// Vorgang als real und wird im Standardmodus durchgelassen.
//
// Das ist eine ABSICHTLICHE Grenze, keine Lücke — und sie kostet für den
// 500er-Funktionstest nichts:
//   * Für die Kohorte trägt die Kennungsfamilie (Test B1) und zusätzlich die
//     reservierte Maildomain. Alle vier `sendAccessMail`-Aufrufer und beide
//     Push-Aufrufer im Produktionscode reichen die Mandatskennung durch
//     (Vertragstest §H) — der strenge Pfad ist also der, der tatsächlich läuft.
//   * Am Testtag steht der Riegel ohnehin auf MODUS_TESTFENSTER und sperrt
//     JEDEN Kanal, auch reale Empfänger.
//   * Eine Verschärfung auf „keine Kennung ⇒ gesperrt" würde das Verhalten des
//     ECHTEN Mailwegs ändern (Einladungen an Betreiber-/Referentenkonten tragen
//     bauartbedingt keine Mandatskennung, `accounts.createUser` setzt
//     `politicianId` nur für die Rolle „abgeordneter"). Genau diese Art
//     Nebenwirkung hat schon einmal vier gepinnte Mailverträge gebrochen
//     (Beleg §14) — das Verhalten des realen Mailwegs zu ändern ist nicht
//     Aufgabe dieses Riegels.
...
//
// EINE KANONISCHE STELLE (Sprint 02.09.): dieselbe Liste wurde inzwischen auch
// von der Warteschlangen-Priorisierung und vom KI-Budget gebraucht. Zwei Kopien
// derselben Sicherheitsliste sind ein Fehler, der irgendwann auseinanderläuft
// (CLAUDE.md §7.7). Sie lebt deshalb jetzt in `lib/helmut/mandatsklasse.js`;
// hier steht nur noch der Bezug. Das VERHALTEN dieses Riegels ist unverändert —
// testgesichert durch den Gleichheitsvertrag in
// `scripts/kommunikationsriegel-test.js` und `scripts/mandatsklasse-test.js`.
const mandatsklasse = require("./mandatsklasse");
const SYNTHETISCHE_KENNUNGSFAMILIEN = mandatsklasse.KENNUNGSFAMILIEN_SYNTHETISCH;
...
function kennungIstSynthetisch(kennung) {
  return mandatsklasse.istSynthetischeKennung(kennung);
}
...
function klassifiziere({ kanal, kennung, empfaenger, ziel } = {}) {
  ...
  // ALLE DREI müssen fehlen — siehe die Präzisierung im Kopf dieses Moduls.
