...
async function resolveEntryUrls(entries) {
  return mapWithConcurrency(entries, 4, async (entry) => {
    ...
    const articleUrl = await resolveArticleUrl(entry.url);
...
async function resolveArticleUrl(url) {
  ...
  try {
    ...
    if (isUsableArticleUrl(page.finalUrl)) return page.finalUrl;
...
function extractPublisherUrl(html) {
  ...
  return candidates.find(isUsableArticleUrl) || "";
}
...
function isUsableArticleUrl(candidate) {
  ...
  try {
    ...
    if (/\.(png|jpe?g|webp|gif|svg|ico)(\?|$)/i.test(parsed.pathname)) return false;
    return true;
  } catch {
    return false;
  }
