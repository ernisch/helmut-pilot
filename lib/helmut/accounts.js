...
const SESSION_TTL_MS = Number(process.env.HELMUT_SESSION_TTL_MS || 12 * 60 * 60 * 1000);
...
async function getSetupStatus() {
  return {
    ...
    userCount: (await readStore()).users?.length || 0,
    storageBackend: getStorageStatus().backend,
    seed: lastSeedResult,
    envAdminConfigured: Boolean(normalizeEmail(process.env.HELMUT_ADMIN_EMAIL) && process.env.HELMUT_ADMIN_PASSWORD)
  };
}
