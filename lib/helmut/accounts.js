...
// readStore/writeStore. Der Supabase Schreiber verlangt einen frischen Lesestand;
// Registrierungsfolgen verwenden wiederholbare Aenderungen ueber mutateAuthStore.
const { readAuthStore: readStore, writeAuthStore: writeStore, mutateAuthStore, getStorageStatus } = require("./storage");
...
// Historisch war der Auth-Store ein Voll-Upsert-JSON-Blob ohne Schreibschutz (siehe
// Kommentar bei deleteAuthDataForPolitician). Der heutige CAS Schutz verweigert
// veraltete Supabase Schreibstaende; die nachfolgende Verifikation bleibt als
// Zusatzschutz erhalten. Historischer Anlass: ein paralleler Schreiber (Session-
...
// Reiner In-Store-Anteil der Session-Anlage: kryptografisch zufaelliges Rohtoken,
// gespeichert wird ausschliesslich dessen SHA-256-Hash. Das Rohtoken wird nur an
// den Aufrufer zurueckgegeben und gelangt nie in den Store.
function pushSession(store, userId, meta = {}) {
...
async function createSession(userId, meta = {}) {
  return mutateAuthStore((store) => pushSession(store, userId, meta));
}
...
// Reiner In-Store-Anteil der Login-Statistik: lastLoginAt/lastSeenAt setzen,
// loginCount und openCount genau einmal erhoehen.
function markLoginInStore(store, userId) {
  const user = (store.users || []).find((entry) => entry.id === userId);
  if (!user) return;
  const now = new Date().toISOString();
...
async function markLogin(userId) {
  return mutateAuthStore((store) => markLoginInStore(store, userId));
}
...
// Reiner In-Store-Anteil des Audit-Events (neuestes zuerst).
function pushAuditEvent(store, event = {}) {
...
async function recordAudit(event = {}) {
  return mutateAuthStore((store) => pushAuditEvent(store, event));
}

// Ein erfolgreicher Login ist EINE einzige atomare Kontospeicher-Mutation:
// Session anlegen + Login-Statistik + Audit-Event. Vorher waren es drei getrennte
// CAS-Lese-Schreib-Zyklen gegen denselben (inzwischen ~800 KB grossen) Blob —
// drei Konfliktfenster und bis zu dreimal die Eingangsfrist. createSession/
// markLogin/recordAudit bleiben fuer alle uebrigen Aufrufer unveraendert.
async function loginUser(userId, meta = {}) {
  return mutateAuthStore((store) => {
    const session = pushSession(store, userId, meta);
    markLoginInStore(store, userId);
    pushAuditEvent(store, {
      action: "auth.login",
      userId,
...
async function ensureAdminSeed() {
  ...
  // Ein fehlgeschlagener Abruf ist kein fertiger Seed. Ein spaeterer expliziter
  // Login darf nach Erholung erneut lesen; keine automatische Wiederholung.
  if (!seedPromise) seedPromise = seedAdminOnce().catch(error => {
    seedPromise = null;
    throw error;
  });
  return seedPromise;
