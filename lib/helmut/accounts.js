...
// Kunden"-Timeout — siehe buildAdminOverview/buildAdminCustomers in server.js).
// Ohne Angabe unveraendertes Verhalten (eigener frischer readStore()-Aufruf).
async function listUsers(deps = {}) {
  const store = deps.store || await readStore();
...
// Boolesche Melde-Kategorien (an/aus). Die Sofort-Melden-Auswahl der
// Erstkonfiguration (S9) bildet auf crisis/opportunity ab und ergaenzt zwei
// zusaetzliche Kategorien (deadlines/votes) — alle rein boolesch.
const NOTIFICATION_BOOLEAN_KEYS = [
  "briefing", "lage", "radar", "crisis", "opportunity", "statement", "deadlines", "votes"
];
const BRIEFING_TIME_VALUES = new Set(["06:00", "07:00", "08:00"]);
const BRIEFING_DEPTH_VALUES = new Set(["knapp", "standard", "ausfuehrlich"]);
const BRIEFING_MAX_PRIO_VALUES = new Set([3, 5, 7]);
...
function defaultNotificationSettings() {
  return {
    ...
    deadlines: true,
    votes: true,
    // Strukturierte Briefing-Praeferenzen (Erstkonfiguration S9, Account-Ebene —
    // bewusst NICHT in mandate_profiles). Steuern Zeit/Tiefe/max. Prioritaeten.
    briefingTime: "07:00",
...
async function updateUser(id, patch = {}) {
  ...
  if (patch.notificationSettings !== undefined && typeof patch.notificationSettings === "object") {
    const merged = { ...defaultNotificationSettings(), ...(user.notificationSettings || {}) };
    const ns = patch.notificationSettings;
    // Boolesche Kategorien: nur die bekannten Schluessel, hart nach Boolean casten.
    for (const key of NOTIFICATION_BOOLEAN_KEYS) {
      if (ns[key] !== undefined) merged[key] = !!ns[key];
    }
    // Strukturierte Briefing-Praeferenzen (S9): nur zulaessige Enum-/Zahlenwerte
    // uebernehmen, sonst Bestand behalten (nie einen ungueltigen Wert speichern).
    if (ns.briefingTime !== undefined && BRIEFING_TIME_VALUES.has(String(ns.briefingTime))) {
      merged.briefingTime = String(ns.briefingTime);
    }
    if (ns.briefingDepth !== undefined && BRIEFING_DEPTH_VALUES.has(String(ns.briefingDepth))) {
      merged.briefingDepth = String(ns.briefingDepth);
    }
    if (ns.maxPriorities !== undefined && BRIEFING_MAX_PRIO_VALUES.has(Number(ns.maxPriorities))) {
      merged.maxPriorities = Number(ns.maxPriorities);
    }
...
async function countActiveSessions(deps = {}) {
  const store = deps.store || await readStore();
...
async function listPendingInvites(deps = {}) {
  const store = deps.store || await readStore();
  const now = Date.now();
  const invites = {};
...
async function listAssignments(deps = {}) {
  const store = deps.store || await readStore();
...
async function listAuditEvents(limit = 100, deps = {}) {
  const store = deps.store || await readStore();
  return (store.auditEvents || []).slice(0, limit);
}

async function listSystemErrors(limit = 100, deps = {}) {
  const store = deps.store || await readStore();
