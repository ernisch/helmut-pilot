...
const USER_STATUSES = Object.freeze(["aktiv", "testphase", "pausiert", "gekuendigt", "deaktiviert"]);
...
async function removeDailyInput(id, politicianId) {
  ...
  // Nur die Eintraege des aufrufenden Mandats zurueckgeben — sonst sieht jeder Referent/Admin,
  // der hier aufruft, die Termine/Themen aller anderen Mandate mit in der Antwort.
  return politicianId ? store.dailyInputs.filter((entry) => entry.politicianId === politicianId) : store.dailyInputs;
}

// Entfernt ALLE Tageseintraege eines Mandats (DSGVO-Loeschung, siehe deleteProfileData in
// storage.js — dailyInputs leben im geteilten Auth-Store, nicht im Politiker-Blob, und wurden
// von der bisherigen Loeschung nicht erfasst).
async function removeAllDailyInputsForPolitician(politicianId) {
  if (!politicianId) return 0;
  const store = await readStore();
  const before = (store.dailyInputs || []).length;
  store.dailyInputs = (store.dailyInputs || []).filter((entry) => entry.politicianId !== politicianId);
  const removed = before - store.dailyInputs.length;
...
module.exports = {
  ...
  addDailyInput,
  removeDailyInput,
  removeAllDailyInputsForPolitician,
  ensureAdminSeed,
  adminExists,
  getSetupStatus,
  httpError,
