...
function buildRetrievalPath(source, publisherId, method) {
  ...
  // VORBEREITETE Pflichtquellen (source.preparedOnly): status 'paused' + activation_mode
  // 'manual'. 'paused' ist der einzige Schalter, der in BEIDEN Auswertungen greift —
  // buildRelationalCrawlPlan schliesst ihn in Regel 4 aus ("nicht-reaktiviert"), und
  // model.isPathActive liefert false. Der Weg ist damit im Paket und fachlich zaehlbar, wird
  // aber garantiert nicht abgerufen; die Aktivierung ist eine eigene Freigabeentscheidung.
  const prepared = source.preparedOnly === true;
  const status = prepared ? "paused" : (KNOWN_PATH_HEALTH[source.id] || "needs_review");
  return {
    id: `rp-${source.id}`,
    ...
    status,
    activation_mode: prepared ? "manual" : (ALWAYS_ON_LEGACY_IDS.has(source.id) ? "always_on" : "auto"),
