...
// Pilot-Fachthemenpaket "wohnen-bauen-stadtentwicklung-bund": vorbereitete (INAKTIVE)
// Bund-Primaerquellen, additiv ins Modell gemischt (analog zum DIP-Abrufweg unten).
const { buildWohnenBauenStadtentwicklungSeed } = require("./seeds/wohnen-bauen-stadtentwicklung");
...
function buildCatalog({ sources } = {}) {
  ...
  // Pilot-Fachthemenpaket "wohnen-bauen-stadtentwicklung-bund" additiv einmischen.
  // DEDUP-BEWUSST: bestehende Herausgeber (destatis.de/bundestag.de) werden NICHT
  // ueberschrieben (nur hinzugefuegt, wenn ihre id noch nicht im Register ist). Alle
  // WBSB-Abrufwege bleiben status "needs_review" + activation_mode "manual" (technisch
  // inaktiv); ihr Paket traegt status "prepared" -> keine automatische Aktivierung.
  {
    const wbsb = buildWohnenBauenStadtentwicklungSeed();
    for (const p of wbsb.publishers) {
      if (publishers.has(p.id)) continue; // bestehenden Herausgeber wiederverwenden
      const { reuse, ...pub } = p; // In-Memory-Flag nicht in den Datensatz uebernehmen
      publishers.set(p.id, pub);
    }
    for (const path of wbsb.retrievalPaths) retrievalPaths.push(path);
    for (const pp of wbsb.packagePaths) {
      // Paket muss in PACKAGE_DEFINITIONS existieren (sonst als unmapped melden).
      const exists = PACKAGE_DEFINITIONS.some((d) => d.id === pp.package_id);
      if (exists) packagePaths.push(pp);
      else unmapped.push(`wbsb->${pp.package_id}`);
    }
  }
