...
// BETRIEBSENTSCHEIDUNG, KEIN GESUNDHEITSURTEIL (A-1 der Seed-Fachpruefung,
// betrieb/quellen-seed-einspielung.md §8.5). Diese Wege sind technisch in Ordnung, werden
// aber bewusst NICHT mit dem Seed reaktiviert. Getrennt von KNOWN_PATH_HEALTH gehalten,
// damit niemand die Angabe als Messwert missversteht.
//
// ausschuss-arbeit-soziales: Der Ersatzweg ist die Google-News-Suche
// 'site:bundestag.de "Ausschuss fuer Arbeit und Soziales"'. Er holt ueber einen Aggregator
// genau die Domain, die Helmut nach der Reparatur von rp-bundestag wieder DIREKT abruft
// (pressemitteilungen.rss + presse/hib/rss, beide in sources.js). Googles Index von
// bundestag.de ist gegenueber diesen Feeds langsamer und unvollstaendiger; zusaetzlich
// decken 6 rp-bundle-ausschuss-*-Suchen dasselbe Themenfeld ab. Damit waere es der einzige
// der vier Google-Ersatzwege ohne belegten Eigenertrag — bei 92 % Google-Anteil im Katalog
// (Klumpenrisiko B1) wird er nicht mitaktiviert.
//
// Warum 'broken' und nicht 'paused': 'broken' ist der Wert, der in Production heute schon in
// der Zeile steht. Der Seed laesst diese Zeile damit unveraendert (Null-Delta) statt sie fuer
// eine reine Etikettierung anzufassen. Semantisch waere 'paused' ("nicht-reaktiviert",
// source-mode.js Schritt 4) treffender — beide Werte schliessen den Weg gleichermassen aus
// dem Crawl-Plan aus. Eine Umstellung auf 'paused' ist eine eigene Betreiberentscheidung.
const PATH_STATUS_OVERRIDE = {
...
function buildRetrievalPath(source, publisherId, method) {
  ...
  const status = PATH_STATUS_OVERRIDE[source.id] || KNOWN_PATH_HEALTH[source.id] || "needs_review";
  return {
    id: `rp-${source.id}`,
