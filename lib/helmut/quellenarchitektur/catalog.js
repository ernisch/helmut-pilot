...
function buildCatalog({ sources } = {}) {
  ...
  // DIP als API-Abrufweg des Bundestag-Herausgebers ergaenzen (Bund Basis).
  {
    ...
    // P3-15 (Architektur-Audit 29): EINE Quelle der Wahrheit fuer die DIP-Wegfelder — die
    // identitaetsstiftenden Felder (legacy_source_id/method/url/query/priority) stammen aus
    // der DIP_PATH-Konstante oben statt aus einer zweiten, driftgefaehrdeten Inline-Kopie.
    const dipPath = {
      id: `rp-${DIP_PATH.legacy_source_id}`, publisher_id: dipPublisherId,
      legacy_source_id: DIP_PATH.legacy_source_id,
      name: DIP_PATH.name, method: DIP_PATH.method, url: DIP_PATH.url, query: DIP_PATH.query,
      parser: "dip-json", priority: DIP_PATH.priority, status: "healthy",
      activation_mode: "always_on", is_critical: true, max_items: null, represents_type: null
    };
