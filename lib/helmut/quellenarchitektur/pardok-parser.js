...
function parseBrandenburgVorgang(vorgangXml, opts = {}) {
  ...
  const documents = dokBloecke.map((dok, i) => {
    ...
    // Identitaet: Vorgangsnummer + Dokument-Diskriminator (DokNr, sonst ReihNr, sonst Index).
    const disc = dokNr || (reihNr ? `r${reihNr}` : `i${i}`);
    const externeId = vnr ? `${vnr}:${disc}` : (dokNr ? `bra:${dokNr}` : null);
