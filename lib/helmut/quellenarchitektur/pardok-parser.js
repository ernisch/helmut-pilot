...
function parseBerlinVorgang(vorgangXml, opts = {}) {
  ...
  const vnr = firstField(vorgangXml, "VNr") || firstField(vorgangXml, "VID");
  ...
  const vorgang = { vnr: vnr || null, vtyp: vTypL || null, stichworte: allFields(vorgangXml, "Desk") };
  return { skipped: null, vnr, documents: dokBloecke.map((dok) => parseBerlinDokument(dok, opts, vorgang)) };
}
