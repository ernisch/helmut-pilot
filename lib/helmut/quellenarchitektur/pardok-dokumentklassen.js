...
const BELEGE = Object.freeze({
  ...
  "sonde-be-vorgang-2026-07-29":
    "Struktur-Sonde mit PP_RECORD_TAG=Vorgang gegen pardok-wp19.xml (GitHub-Actions-Lauf " +
    "30483735900, 29.07.2026, 500 von 41 854 <Vorgang>-Records). Belegt die Berliner " +
    "Vorgangsstruktur (<VNr>/<VID> je 100 %, <VTyp>/<VTypL> 46 %, ~50 % delete-Stubs) und den " +
    "Dokumenttyp <DokTyp>MdlAnfr</DokTyp> / 'Muendliche Anfrage'.",
  "gold-fixture-bb": "test/fixtures/pardok/brandenburg-gold.xml (versionierte Gold-Fixture aus derselben Quelle).",
  "shadow-be-2026-07-29":
    "Shadow-Parser-Test gegen die Live-Datei pardok-wp19.xml (GitHub-Actions-Lauf 30481670298, " +
    "29.07.2026, 800 Records, 5 Wiederholungen identisch). Gemessene Dokumentart-Verteilung: " +
    "Drucksache 534 · Plenarprotokoll 206 · Ausschussprotokoll 34 · Gesetz- und Verordnungsblatt 26 " +
    "— insgesamt genau VIER Dokumentarten.",
  "shadow-bb-2026-07-29":
    "Shadow-Parser-Test gegen die Live-Datei exportWP8.xml (GitHub-Actions-Lauf 30481670298, " +
    "29.07.2026, 800 Vorgaenge -> 816 Dokumente, 5 Wiederholungen identisch). Gemessene " +
    "Dokumentart-Verteilung: Drucksache 411 · Ausschussprotokoll 221 · Plenarprotokoll 70 · " +
    "Unterrichtung 34 · Praesidiumsprotokoll 28 · Information 22 · Einladung 11 · " +
    "Fruehwarndokument 10 · Gesetz- und Verordnungsblatt 6 · Zuschrift 2 · Gutachten 1 " +
    "— insgesamt ELF Dokumentarten, deutlich mehr als Berlin.",
...
const BERLIN_TYPEN = Object.freeze({
  ...
  "muendliche anfrage": eintrag("anfrage", "sonde-be-vorgang-2026-07-29"),
  "mdlanfr": eintrag("anfrage", "sonde-be-vorgang-2026-07-29"),
...
// Brandenburg fuehrt ELF Dokumentarten (Berlin nur vier) — gemessen ueber 800 Records am
// 29.07.2026. Die ersten vier Eintraege stammen aus der Struktur-Sonde, die uebrigen aus dieser
// Verteilungsmessung. Ohne sie waeren 290 von 816 Dokumenten (35,5 %) faelschlich `unbekannt`.
const BRANDENBURG_ARTEN = Object.freeze({
  ...
  "ausschussprotokoll": eintrag("sitzung", "shadow-bb-2026-07-29"),
  // Protokoll einer Praesidiumssitzung — dieselbe Gattung wie Plenar-/Ausschussprotokoll.
  "praesidiumsprotokoll": eintrag("sitzung", "shadow-bb-2026-07-29"),
  // Einladung zu einer Sitzung; sie traegt die Tagesordnung. Berlin liefert dazu nichts.
  "einladung": eintrag("tagesordnung", "shadow-bb-2026-07-29"),
  "unterrichtung": eintrag("drucksache", "shadow-bb-2026-07-29"),
  "gesetz und verordnungsblatt": eintrag("sonstiges", "shadow-bb-2026-07-29"),
  // Bewusst `sonstiges` statt einer stärkeren Behauptung: der Wert ist belegt beobachtet, aber
  // es waere geraten, ihn zur Drucksache, Anfrage oder Sitzung zu erklaeren.
  "information": eintrag("sonstiges", "shadow-bb-2026-07-29"),
  "fruehwarndokument": eintrag("sonstiges", "shadow-bb-2026-07-29"),
  "gutachten": eintrag("sonstiges", "shadow-bb-2026-07-29"),
  "zuschrift": eintrag("sonstiges", "shadow-bb-2026-07-29")
});
...
const KLASSEN_AUSNAHMEN = Object.freeze([
  Object.freeze({
    ...
    grund: "Der PARDOK-Export ist eine Dokumentendatenbank abgeschlossener Vorgaenge. Ueber 800 " +
      "gemessene Records fuehrt Berlin GENAU VIER Dokumentarten (Drucksache, Plenarprotokoll, " +
      "Ausschussprotokoll, Gesetz- und Verordnungsblatt) — keine davon ist eine Tagesordnung oder " +
      "ein Termin. Brandenburg liefert hier sehr wohl etwas (Dokumentart 'Einladung'); der " +
      "Unterschied ist real und wird nicht durch eine gemeinsame Annahme eingeebnet.",
    beleg: "shadow-be-2026-07-29", ersatzweg: "rp-be-landesparlament (Suchweg, liefert sie als sonstigen Inhalt ohne document_type)"
  }),
  // KORREKTUR 29.07.2026: Brandenburg wurde hier zunaechst faelschlich als Ausnahme gefuehrt.
  // Die Verteilungsmessung ueber 800 Records zeigt die Dokumentart "Einladung" (11 Treffer) —
  // die Einladung zu einer Sitzung traegt die Tagesordnung. Die Klasse ist damit BELEGT und
  // steht als regulaerer Eintrag in BRANDENBURG_ARTEN. Nur Berlin bleibt hier eine Ausnahme.
  Object.freeze({
...
const VORGANGSBEZUG = Object.freeze({
  berlin: Object.freeze({
    verfuegbar: true, felder: ["vorgangsnummer (<VNr>/<VID>)", "vorgangstyp (<VTypL>/<VTyp>)"],
    grund: "Am 29.07.2026 mit PP_RECORD_TAG=Vorgang belegt: der Berliner Export ist ebenfalls " +
      "vorgangsstrukturiert (41 854 <Vorgang>, <VNr>/<VID> je 100 %, ~50 % delete-Stubs, " +
      "verschachtelte <Dokument>). Belegt an V-351039, das die Drucksache D-351040 UND das " +
      "Plenarprotokoll D-351042 traegt. Die Dokumentidentitaet bleibt die <DBID>; der Vorgang " +
      "liefert nur den Bezug. Die frueher hier notierte Aussage 'Berlin liefert keinen " +
      "Vorgangsbezug' ist damit WIDERLEGT.",
    beleg: "sonde-be-vorgang-2026-07-29"
  }),
