...
// Flache findings-Liste [{source_id, primary_source_id}] in das Dokument-Modell falten.
function normalizeFindings(findings = []) {
  if (!findings.length) return [];
  const byPrimary = new Map();
  for (const f of findings) {
    const primary = f.primary_source_id || f.primarySourceId || null;
    const key = primary || f.raw_document_id || f.rawDocumentId || JSON.stringify(f);
    if (!byPrimary.has(key)) byPrimary.set(key, { primary_source_id: primary, findings: [] });
    byPrimary.get(key).findings.push({ source_id: f.source_id || f.sourceId });
  }
...
function assessRetrievalPaths({ paths = [], sourceMetrics = {}, activePathIds = [], now = Date.now(), thresholds = {} } = {}) {
  ...
  return paths.map((p) => {
    ...
    // --- inhaltlicher Ertrag ---
    const contentYield = { documentCount: m.documentCount, latestAgeHours: round1(latestAgeH), available: true };
    ...
    else if (dupAvailable) productValue = "ohne_ko"; // Dokumente, aber kein KO und keine reine Duplikatlage
    else productValue = "ohne_ko_unbestaetigt"; // Dokumente, aber KO-/Dedup-Lage (noch) nicht belastbar
...
function buildWatchdog({ signals = {}, thresholds = {}, packageSupply = [], pathQuality = [], now = Date.now() } = {}) {
  ...
  const severity = dead.length ? "alarm" : (warn.length || unknown.length ? "watch" : "ok");
  const state = !signals.storageOk ? "kritisch"
  ...
  return {
    ...
    severity,
    state,
    storageOk: signals.storageOk !== false,
    axes,
