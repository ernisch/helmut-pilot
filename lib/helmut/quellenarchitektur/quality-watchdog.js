...
function assessRetrievalPaths({ paths = [], sourceMetrics = {}, activePathIds = [], now = Date.now(), thresholds = {} } = {}) {
  ...
  return paths.map((p) => {
    ...
    let productValue;
    // Ohne geladene Dokumentdaten KEINE Aussage „keine Dokumente" behaupten -> 'unbestaetigt'.
    if (!docsAvailable) productValue = "ohne_ko_unbestaetigt";
    else if (!hasDocs) productValue = "unbekannt";
    ...
    return {
      ...
      recommendedAction: pathAction({ technicalHealth, productValue, isCritical, active, dupAvailable, docsAvailable })
    };
  });
}

function pathAction({ technicalHealth, productValue, isCritical, active, dupAvailable, docsAvailable = true }) {
  // 'defekt' kommt aus dem Status (broken) und gilt IMMER — auch ohne Dokumentdaten.
  ...
  // Ohne geladene Dokumentdaten sind 'pruefen'/'ruht' KEINE belastbaren Befunde (dieselbe
  // Ursache: nichts ingestet) -> keine erfundene „liefert keine Dokumente"-Handlung.
  if (!docsAvailable && (technicalHealth === "pruefen" || technicalHealth === "ruht")) return { severity: "keine", text: "Keine." };
...
function assessProfiles({ profiles = [], packages = [], packagePaths = [] } = {}) {
  return profiles.map((profile) => {
    const p = (profile && typeof profile === "object") ? profile : {}; // null-Element/Fremdtyp null-sicher
    const st = profileSupplyStatus(p, { packages, packagePaths });
    return {
      profileId: p.id || p.user_id || null,
