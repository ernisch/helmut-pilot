...
function deriveDecisionLevel(ko = {}) {
  // Landtag-Sprint (P2-5): amtliche Quell-Ebene als autoritatives Signal. Ein Dokument
  // aus einer amtlichen LANDES-Primaerquelle (PARDOK/parldok — Parser setzt
  // politische_ebene='land') ist per Herkunft Landesebene; die Text-Heuristik unten
  // (jede Ausschussnennung => Bund-Signal) darf das nicht ueberstimmen. Bundesinhalte
  // tragen dieses Feld nicht — Verhalten fuer Bund unveraendert.
  const quellEbene = String(ko.quell_politische_ebene || ko.source_political_level || "").toLowerCase();
  if (quellEbene === "land") return { level: "land", confidence: "high", signal: "amtliche-landesquelle" };
