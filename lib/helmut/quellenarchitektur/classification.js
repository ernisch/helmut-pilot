...
function geografienAusText(haystack) {
  const out = [];
  for (const eintrag of SUBNATIONALE_INSTITUTIONSPHRASEN) {
    if (eintrag.phrase && haystack.includes(eintrag.phrase)) out.push(eintrag.geo);
  }
  return out;
}
...
function sammleGeografieKandidaten(ko = {}, ai = {}, opts = {}) {
  ...
  for (const g of geografienAusText(koHaystack(ko))) {
