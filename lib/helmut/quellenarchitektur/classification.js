...
function resolveEntity(name, typeHint) {
  ...
  if (typeHint === "committee") {
    ...
    // Ein Themen-Schluessel ist keine Institutionsidentitaet. Eine verkuerzte
    // vollstaendige Fachbezeichnung ist erlaubt, zusaetzliche Rollen/Traeger
    // oder fehlende Namensbestandteile duerfen aber nicht verschwinden.
    const fachname = value => normText(value).replace(/^ausschuss (?:fuer )?/, "");
    const name = ent && fachname(ent.name);
    const bezeichnungen = name ? [name, `${name}ausschuss`, `${name}sausschuss`] : [];
    if (ent && bezeichnungen.includes(fachname(raw))) {
      return { name: ent.name, type: ent.entity_type, entity_id: ent.id, confidence: "medium" };
    }
  }
...
function koHaystack(ko = {}, mitErwaehnungen = true) {
  return normText([
...
function deriveDecisionLevel(ko = {}) {
  // Auch der Altpfad darf eine explizite Erwaehnungsrolle nicht aufwerten.
  // Beruehrte Ebenen verwenden diese Listen weiterhin getrennt.
  const hay = koHaystack(ko, false);
  // Ausschuesse gibt es auf mehreren Ebenen. Ein Fachname allein belegt Bund
  // nicht. Eindeutige gelieferte Ministeriumsaliase behalten ihre Katalogebene.
  const bundesministerium = (ko.ministerien || []).some(name => {
    const ent = ENTITY_BY_NAME.get(normText(name));
    return ent?.entity_type === "ministry" && ent.level === "bund";
  });
  const hasBundInst = anyTermIn(hay, BUND_INSTITUTION_TERMS) || bundesministerium;
...
// related_entities  = erwaehnte Akteure und Institutionen.
// Die Quellenbindung bestaetigt nur die Nennung. Eine ausdrueckliche
// mentioned_* Rolle darf danach nicht wieder zur Beteiligung aufgewertet werden.
...
// fuellt fehlende Felder; gueltige KI-Werte einschliesslich unknown bleiben erhalten.
//
...
function classifyKnowledgeObject(ko = {}, ai = {}, opts = {}) {
  ...
  // Eine ausdrueckliche Enthaltung ist eine vorhandene Antwort, keine Luecke.
  // Sonst macht der Deriver aus einer Hintergrundnennung wieder Gewissheit.
  // Fehlende Alt-Felder behalten ihren bisherigen Ableitungspfad; ein bereits
  // ermittelter Bestand bleibt durch entscheideEbene unveraendert geschuetzt.
  const neu = ai.decision_level === "unknown"
    ? { level: "unknown", quelle: "ki", konfidenz: "unknown" }
    : aiLevel
  ...
  const event_type = EVENT_TYPES.includes(ai.event_type)
...
function sammleGeografieKandidaten(ko = {}, ai = {}, opts = {}) {
  ...
  // Eine gelieferte Betroffenheitsliste (auch []) darf nicht aus blossen
  // Institutionsnennungen erweitert werden. Diese bleiben dann als Erwaehnung
  // erhalten. Parser, amtliche Belege und das bestehende Gedaechtnis bleiben
  // unabhaengig davon staerkere Nachweise. Ohne Liste bleibt der Altpfad bestehen.
  const inhaltsRolle = Array.isArray(ai.affected_geographies)
    ? { herkunft: "erwaehnung", rolle: "erwaehnt" }
    : { herkunft: "inhalt", rolle: "betroffen" };
  const institutionsNennungen = [...(ko.ausschuesse || []), ...(ko.ministerien || [])];
  for (const bezeichnung of institutionsNennungen) {
    const g = geografieAusInstitutionsnennung(bezeichnung);
    if (g) kandidaten.push({ ...g, ...inhaltsRolle });
  }
  for (const g of geografienAusText(ko)) {
    kandidaten.push({ ...g, ...inhaltsRolle });
  }

  // Die Rolle dieser Listen ist bereits bekannt, auch ohne optionale KI-Liste.
  for (const bezeichnung of [
    ...(ko.mentioned_committees || []), ...(ko.mentioned_ministries || []),
    ...(ko.mentioned_organizations || [])
  ]) {
    const g = geografieAusInstitutionsnennung(bezeichnung);
    if (g) kandidaten.push({ ...g, herkunft: "erwaehnung", rolle: "erwaehnt" });
  }
