...
// --- 4) Bundestagsfraktionen (mit belegbarer Direktdomain; BSW/SSW-Gruppen s. Bericht) -------
const GROUP_DEFS = [
  ["group-bt-cdu-csu", "cducsu.de"], ["group-bt-spd", "spdfraktion.de"],
  ["group-bt-gruene", "gruene-bundestag.de"], ["group-bt-linke", "linksfraktion.de"],
  ["group-bt-afd", "afdbundestag.de"], ["group-bt-fdp", "fdpbt.de"]
].map(([group, d]) => def({
  id: `seed-${group}`, d, m: "html", t: "fraktion", group, rev: "active", lic: "presse_erlaubt",
  orig: "official_directory", ev: `https://www.${d}`, tr: "mittel", stance: "position"
}));
...
const SEED_SOURCE_DEFS = [
  ...
  ...EXPERT, ...LAND_PORTAL_SOURCES, ...LAND_OFFICIAL
];
...
function buildSeedCatalog(opts = {}) {
  ...
  const records = SEED_SOURCE_DEFS.map((o) => {
    ...
    return buildSourceRecord({
      ...
      privacy_status: o.priv || "oeffentliche_mandatsdaten",
