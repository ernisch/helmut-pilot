...
// Konsolidierung: KEINE Redeklaration mehr — die eine Wahrheit ist ../model.TRUST_LEVELS (Basis
// bereits importiert). Wertgleich, aus einer Quelle bezogen (Doppelung entfernt).
const TRUST_LEVELS = Object.freeze([...base.TRUST_LEVELS]);
...
function publisherIdForDomain(domain) {
  // Erst kleinschreiben, DANN www. entfernen — sonst bliebe "WWW."-Praefix stehen und spaltete
  // einen Herausgeber in zwei IDs (Review-Fund).
...
// Erkennt eine Suchanbieter-Quelle ROBUST (nicht nur am Methoden-Substring "googlenews"):
// Aggregator-Typ ODER Aggregator-Herausgeber ODER Such-/GoogleNews-Methode ODER Google-News-URL.
// Zentral, damit Versorgungsstandard, Zuweisung und Coverage dieselbe Wahrheit nutzen (Abnahme §9).
function isSearchProviderSource(rec = {}) {
  if (taxonomy.isSearchProviderType(rec.source_type)) return true;
  if (rec.publisher_id === base.GOOGLE_NEWS_PUBLISHER_ID) return true;
  const method = String((rec.retrieval && rec.retrieval.method) || rec.method || "").toLowerCase();
  if (method === "googlenews_search" || method === "search" || method.includes("googlenews")) return true;
  const url = rec.canonical_url || (rec.retrieval && rec.retrieval.url) || rec.url || "";
  if (url && base.isGoogleNewsUrl(url)) return true;
  return false;
}
...
function canonicalSourceKey(input = {}) {
  ...
  // Schema-insensitiv (http/https auf denselben Schluessel), wie der Kopfkommentar zusagt —
  // sonst erzeugten http- und https-Varianten derselben Quelle zwei Records (Review-Fund).
  const canon = canonicalUrl(rawUrl).replace(/^https?:\/\//i, "");
...
module.exports = {
  ...
  SOURCE_ATTRIBUTES,
  publisherIdForDomain, isSearchProviderSource,
  canonicalUrl, canonicalSourceKey, normalizeSearchQuery, dedupeCandidates,
