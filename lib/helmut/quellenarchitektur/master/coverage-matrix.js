...
function buildCoverageMatrix(input = {}) {
  ...
  const byBundesland = laender.map((l) => {
    const recs = records.filter((r) => (r.region_ids || []).some((rid) => rid === l.id || String(rid).startsWith(l.id + "-") || childOfLand(rid, l.id)));
    const stats = bucketStats(recs);
    const flags = classifyBucket(stats, th);
...
function childOfLand(regionId, landId) {
  // geo-bezirk-berlin-… / geo-kreis-bb-… -> gehoert zum Land, wenn der Land-Slug im Kind-Slug steckt.
  const landSlug = String(landId).replace(/^geo-land-/, "");
  return new RegExp(`(^|[-])${landSlug}([-]|$)`).test(String(regionId));
}
