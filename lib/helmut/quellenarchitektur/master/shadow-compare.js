...
// Abrufmethode + dieselbe inhaltliche Rolle sollte auf DIESELBE kanonische URL zeigen.
function identityAnchor(rec) {
  const method = String(rec.retrieval && rec.retrieval.method || "").toLowerCase();
  const anchor = rec.party_id || rec.group_id || (rec.committee_ids || []).join("+") || rec.institution_id || rec.source_type || "";
  return `${rec.publisher_id}|${method}|${anchor}`;
}
...
function compareCatalogs(input = {}) {
  ...
  // 5 likelyDuplicates — kanonische Dubletten + Beinah-Dubletten in der Vereinigung.
  const dedup = model.dedupeCandidates([...oldRecords, ...newRecords]);
  const likelyDuplicates = dedup.duplicates.map((d) => ({ key: d.key, duplicateOf: d.of }));
