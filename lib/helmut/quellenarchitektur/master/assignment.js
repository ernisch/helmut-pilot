...
function matchesCriteria(record = {}, criteria = {}) {
  ...
  // (search_provider_ok) — nie als stille Grundversorgung (Abnahme §9).
  const isSearch = taxonomy.isSearchProviderType(record.source_type) ||
    String(record.retrieval && record.retrieval.method || "").toLowerCase().includes("googlenews");
