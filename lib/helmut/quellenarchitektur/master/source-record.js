...
// Heuristische Muster fuer versehentlich eingeschleuste private Kontaktdaten in Freitextwerten.
// Bewusst konservativ: amtliche Sammel-/Presseadressen (info@, presse@) sind erlaubt.
const PRIVATE_EMAIL_RE = /\b[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}\b/i;
const ALLOWED_ROLE_MAILBOXES = /^(presse|press|info|kontakt|redaktion|pressestelle|kommunikation|office|media|newsroom)@/i;
const PHONE_RE = /(\+?\d[\d\s/().-]{7,}\d)/;

// Freitext-Felder, die auf private Kontaktdaten geprueft werden (der Rest sind IDs/Enums/URLs).
const TEXT_FIELDS_TO_SCAN = Object.freeze(["responsible", "note", "publisher_name"]);
...
function scanForPrivatePii(record = {}) {
  ...
  for (const field of TEXT_FIELDS_TO_SCAN) {
    ...
    if (!val) continue;
    const mail = val.match(PRIVATE_EMAIL_RE);
    if (mail && !ALLOWED_ROLE_MAILBOXES.test(mail[0])) {
      found.push(`moegliche private E-Mail in ${field}`);
    }
    if (PHONE_RE.test(val)) found.push(`moegliche Telefonnummer in ${field}`);
  }
  return found;
}
