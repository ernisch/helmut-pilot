...
function validateMigration({ catalog, orphanClassification = {}, observedSources = null } = {}) {
  ...
  for (const row of observed) {
    ...
    } else if (Object.prototype.hasOwnProperty.call(orphanClassification, id)) {
      // NUR die explizite Karte des Aufrufers erklaert Orphans (kein Muster-Fallback,
      // sonst wuerden lebende Mandats-Mehrfachsuchen als Legacy maskiert).
      orphanObserved.push({ source_id: id, classification: orphanClassification[id], count });
    } else {
