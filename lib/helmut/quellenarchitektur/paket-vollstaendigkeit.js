...
const {
  BUNDESTAG_21, fraktionsLage, vergleicheFraktionen, pruefstand
...
function pruefeAusnahme(klasse, a, opts = {}) {
  const basis = {
  ...
  // Pruefstand der zugrunde liegenden Parlamentsdaten mitfuehren (Phase-1-Punkt 15):
  // WANN wurde dieses Bild der Wirklichkeit zuletzt bewusst geprueft, und ist eine
  // Nachpruefung faellig? Das ist ein BERICHTSFELD — es kippt `bestaetigt` NICHT, damit
  // CI nicht an einem Stichtag ohne fachlichen Anlass rot wird. Der harte Driftschutz
  // bleibt die Voraussetzungspruefung oben.
  const stand = pruefstand(v.parlament, opts.heute);

  return zutreffend
    ? { ...basis, bestaetigt: true, pruefstand: stand, geprueft: { parlament: lage.parlamentName, wahlperiode: lage.wahlperiode, partei: v.partei, pruefung: v.pruefung, belege: lage.belege } }
    : { ...basis, bestaetigt: false, pruefstand: stand, geprueft: lage, grundAblehnung: `Voraussetzung trifft NICHT zu: '${v.partei}' hat in ${lage.parlamentName} (${lage.wahlperiode}. WP) ${lage.hatFraktion ? "eine Fraktion" : "Mandate"}` };
}
