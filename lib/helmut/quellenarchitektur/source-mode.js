...
//                    QUELLEN-CUTOVER und in diesem Auftrag NICHT aktiviert (Datei-Flag nicht
//                    gesetzt; Aktivierung erfordert die Gründer-Freigabe „Go Quellen Cutover").
//
...
const LANDESMODUL_PACKAGE_KEYS = new Set(["berlin-basis", "brandenburg-basis"]);
const LANDESMODUL_PATH_PREFIXES = ["rp-be-", "rp-bb-"];
...
// Ist ein Abrufweg Teil eines (nicht freigegebenen) Landesmoduls Berlin/Brandenburg?
function isLandesmodulPath(path = {}, packageKeysForPath = []) {
  const id = String(path.id || "").toLowerCase();
  if (LANDESMODUL_PATH_PREFIXES.some((p) => id.startsWith(p))) return true;
  const legacy = String(path.legacy_source_id || "").toLowerCase();
  if (legacy.startsWith("be-") || legacy.startsWith("bb-")) return true;
  return (packageKeysForPath || []).some((k) => LANDESMODUL_PACKAGE_KEYS.has(String(k || "").toLowerCase()));
}
...
function buildRelationalCrawlPlan({
  ...
  for (const path of ordered) {
    ...
    // 1) HARTES Landesmodul-Gate (Berlin/Brandenburg) — unabhängig von Status/Paketen.
    if (isLandesmodulPath(path, packageKeys)) {
      ausgeschlossen.push({ id: path.id, grund: "landesmodul-gesperrt (Berlin/Brandenburg nur mit eigener Freigabe)", packageKeys });
      continue;
    }
...
module.exports = { sourceMode, buildRelationalCrawlPlan, comparePlans, toCrawlerSource, isLandesmodulPath, mergeProfileAndPlanSources };
