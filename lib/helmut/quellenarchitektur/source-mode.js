...
//
// EXAKTE KENNUNGEN (`kennungen`, Punkt 14A): der zweilaendrige rbb24-Weg traegt WEDER ein
// Id-Praefix NOCH ein legacy-Praefix eines Landes — er war allein ueber seine Paketzeilen als
// Landesweg erkennbar. Damit haette ein Verlust oder eine Umhaengung seiner `package_paths`-Zeilen
// das Gate umgangen (fail-open): ohne erkanntes Landesmodul greift Regel 1 nicht mehr. Deshalb
// stehen solche Wege zusaetzlich namentlich hier. Das ist eine QUELLEN-Kennung, kein Mandant.
// `scripts/landesmodul-mandatsgate-test.js` erzwingt, dass JEDER Landesmodul-Weg des Seeds auch
// OHNE Paketzeilen erkannt wird — ein neuer Weg ohne Praefix faellt dort auf.
const LANDESMODULE = Object.freeze({
  berlin: Object.freeze({
    ...
    kennungen: Object.freeze(["rp-rbb24-politik", "rbb24-politik"])
  }),
...
// MANDATSBINDUNG des Landesmodul-Gates (Befund V-2, 2026-07-26).
// ---------------------------------------------------------------------------------------------
// Welche Laender haben mindestens EIN aktivierungsberechtigtes Landesmandat? Gezaehlt wird
// ueber die Paketaufloesung des Profils (resolveProfilePackages), also GENAU die Logik, die
// auch die Referenzzaehlung benutzt — nicht ueber ein Profilfeld. Damit gilt:
//   - ein BUNDESTAGSmandat mit bundesland='Berlin' berechtigt Berlin NICHT (es loest kein
//     Landespaket auf; gemessen 2026-07-26: 4 der 6 aktiven Production-Profile sind genau das),
//   - ein deaktiviertes/geloeschtes/unbrauchbares Profil zaehlt nicht (isActivationEligible),
//   - ein Brandenburger Mandat berechtigt Berlin nicht und umgekehrt.
// Kein Mandant ist hier hinterlegt: die Bindung entsteht zur Laufzeit aus dem Profil.
function laenderMitBerechtigtemMandat(profiles = []) {
  const out = new Set();
  for (const profile of Array.isArray(profiles) ? profiles : []) {
    if (!isActivationEligible(profile)) continue;
    const keys = new Set(resolveProfilePackages(profile).all.map((k) => String(k || "").toLowerCase()));
    for (const land of LANDESMODUL_LAENDER) {
      if (LANDESMODULE[land].paketKeys.some((k) => keys.has(k))) out.add(land);
    }
  }
  return out;
}

// WIRKSAM freigegeben = ausdruecklich freigegeben (Betreiberflag) UND mit berechtigtem
// Landesmandat (Produktbedingung). Beides ist noetig — das war vor V-2 nicht so: das Flag
// allein liess die Wege eines Landes in den Plan, sobald IRGENDEIN aktives Paket sie
// referenzierte. Beim zweilaendrigen rbb24-Weg (haengt in berlin-basis UND
// brandenburg-basis) genuegte dafuer ein BRANDENBURGER Mandat, obwohl nur Berlin
// freigegeben war. Fail-closed: fehlt eines von beiden, bleibt das Land gesperrt.
function wirksameLandesmodule({ profiles = [], env = process.env } = {}) {
  const freigegeben = freigegebeneLandesmodule(env);
  const mitMandat = laenderMitBerechtigtemMandat(profiles);
  const wirksam = new Set([...freigegeben].filter((land) => mitMandat.has(land)));
  return { freigegeben, mitMandat, wirksam };
}

// Ausschlussgrund je Weg — nennt AUSDRUECKLICH, welche der beiden Bedingungen fehlt.
// Ohne diese Unterscheidung liesse sich ein „Flag gesetzt, aber niemand berechtigt"
// nicht von einer echten Sperre unterscheiden.
function landesmodulSperrgrund(landesmodule, freigegeben, mitMandat) {
  const teile = landesmodule.map((land) => {
    if (!freigegeben.has(land)) return `${land}: nicht freigegeben`;
    if (!mitMandat.has(land)) return `${land}: freigegeben, aber kein berechtigtes Landesmandat`;
    return `${land}: wirksam`;
  });
  return `landesmodul-gesperrt (${teile.join("; ")})`;
}
...
function buildRelationalCrawlPlan({
  ...
  const { freigegeben, mitMandat, wirksam } = wirksameLandesmodule({ profiles, env });
  ...
  for (const path of ordered) {
    ...
    //    NUR, wenn mindestens eines seiner Länder WIRKSAM freigegeben ist: ausdrückliche
    //    Freigabe (Flag) UND mindestens ein aktivierungsberechtigtes Landesmandat.
    //    Default: kein Land — weder freigegeben noch bemandatiert.
    const landesmodule = landesmoduleOf(path, packageKeys);
    if (landesmodule.length && !landesmodule.some((land) => wirksam.has(land))) {
      ausgeschlossen.push({
        id: path.id,
        grund: landesmodulSperrgrund(landesmodule, freigegeben, mitMandat),
  ...
  const mehrlaendrig = landesmodulAktiv
    .filter((a) => a.landesmodule.some((land) => !wirksam.has(land)))
    .map((a) => ({ id: a.id, landesmodule: a.landesmodule, fremdlaender: a.landesmodule.filter((l) => !wirksam.has(l)) }));
  ...
  return {
    ...
    landesmodule: {
      ...
      // Ehrliche Trennung der beiden Bedingungen: ein Flag ohne berechtigtes Mandat ist
      // KEINE Aktivierung und darf im Bericht nicht wie eine aussehen.
      mitBerechtigtemMandat: [...mitMandat].sort(),
      wirksam: [...wirksam].sort(),
      freigegebenOhneMandat: [...freigegeben].filter((l) => !mitMandat.has(l)).sort(),
      bemandatiertOhneFreigabe: [...mitMandat].filter((l) => !freigegeben.has(l)).sort(),
...
// PROFILBEZOGENE VERSORGUNG (Befund V-2, 2026-07-26).
// =============================================================================================
// Der relationale Plan ist und bleibt GLOBAL — ein Abrufweg läuft systemweit genau einmal
// (Grundsatz oben, Referenzzählung statt Kopien). Das ist für Bundes-/neutrale Wege gewollt:
// der Rohkorpus (`raw_documents`) und die Knowledge Objects sind mandantenneutral, mandats-
// scharf ist erst die Auswahl stromabwärts (Briefing/Radar).
//
// LANDESMODULE sind davon ausgenommen. Vor V-2 mischte getSourcesForProfile den kompletten
// globalen Plan in die Quellenliste JEDES Profils — ein einziges Berliner Landtagsmandat hätte
// die Berliner Wege damit in den Abruf jedes Bundestagsmandats gelegt. Diese Funktion
// beschränkt die Landesmodul-Wege des Plans auf die Länder, für die DIESES Profil ein
// berechtigtes Mandat trägt. Alles andere bleibt unverändert geteilt.
//
// Die Vereinigung über alle berechtigten Profile ergibt wieder exakt `plan.aktiv` — es fällt
// also kein Weg aus dem Gesamtcrawl heraus (testgesichert), er läuft nur im Lauf des Mandats,
// das ihn berechtigt.
function planQuellenFuerProfil(plan, profile) {
  const eintraege = plan && Array.isArray(plan.aktiv) ? plan.aktiv : [];
  const erlaubt = laenderMitBerechtigtemMandat([profile]);
  return eintraege
    .filter((a) => {
      const lm = Array.isArray(a.landesmodule) ? a.landesmodule : [];
      return lm.length === 0 || lm.some((land) => erlaubt.has(land));
    })
    .map((a) => a.source);
}

// FALLBACK-RIEGEL (Befund V-2, adversarialer Punkt 5): der alte Katalog ist weiterhin
// Fallback, wenn der relationale Plan leer/nicht ladbar ist — und er kennt das
// Landesmodul-Gate nicht. Heute enthält er nachweislich keine Landesmodul-Quelle
// (testgesichert), das ist aber Datenlage, keine Sperre. Dieser Riegel macht sie strukturell:
// eine Katalogquelle, deren Kennung ein Landespräfix trägt, läuft im Fallback nur, wenn ihr
// Land für dieses Profil wirksam ist. Fail-closed ohne Profil/ohne Freigabe.
function landesmodulQuelleGesperrt(source = {}, erlaubteLaender = new Set()) {
  const lm = landesmoduleOf({ id: source.id, legacy_source_id: source.id }, []);
  if (!lm.length) return false;
  return !lm.some((land) => erlaubteLaender.has(land));
}
...
module.exports = {
  ...
  laenderMitBerechtigtemMandat, wirksameLandesmodule, planQuellenFuerProfil, landesmodulQuelleGesperrt,
