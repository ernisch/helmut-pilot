...
// GROSSEREIGNIS (Stufe 0) ist seit dem Sprint zu Betriebsbefund B4 die hoechste
// Stufe: ein tödlicher Anschlag darf nicht hinter einer Drucksache anstehen.
const TIER = { GROSSEREIGNIS: 0, AMTLICH: 1, HOHE_RELEVANZ: 2, FRIST: 3, WICHTIGKEIT: 4, REGIONAL: 5, GRENZFALL: 6, NIEDRIG: 7 };

function num(x, d = 0) { const n = Number(x); return Number.isFinite(n) ? n : d; }

// --- Grossereignis-Erkennung (rein deterministisch, KEINE neue Relevanzmaschine) ---
// Zweck (aus dem CSD-2026-Befund): ein politisches Grossereignis darf nicht
// untergehen, weil seine Einzelmeldungen schwach, fragmentiert oder frueh und
// unvollstaendig sind. Es werden ausschliesslich Signale genutzt, die im Cluster
// ohnehin vorliegen — kein Modellaufruf, kein Netz, keine zusaetzliche Quelle.
//
// EHRLICHE GRENZE: das ist eine VORFAHRTSREGEL, keine Relevanzbewertung. Sie
// entscheidet die Reihenfolge, nie den Inhalt und nie den Ausschluss.
const SICHERHEITSBEZUG = /\b(anschlag|anschläge|terror|terroristisch|islamistisch|attentat|amoklauf|schüsse|explosion|getötet|getoetet|tote[nrs]?\b|todesopfer|verletzte|opfer|festnahme|tatverdächtig|tatverdaechtig|geisel|evakuier)/i;
const OFFIZIELLE_REAKTION = /\b(bundeskanzler|kanzler|bundespräsident|bundespraesident|bundesinnenminister|innenminister|regierender bürgermeister|regierender buergermeister|ministerpräsident|ministerpraesident|generalbundesanwalt|krisenstab|sondersitzung|staatsakt|trauerbeflaggung)/i;
const INSTITUTION = /\b(bundestag|bundesrat|bundesregierung|kabinett|verfassungsgericht|bundesanwaltschaft|innenministerium|abgeordnetenhaus|landtag)/i;

// Zeitliche Verdichtung: mindestens `mindestens` Dokumente innerhalb `stundenFenster`.
const VERDICHTUNG_FENSTER_H = 12;
const VERDICHTUNG_MIN_DOKUMENTE = 3;
const UNABHAENGIGE_QUELLEN_MIN = 3;

function clusterText(cluster = {}) {
  return ((cluster && cluster.documents) || [])
    .map((d) => `${(d && d.title) || ""} ${(d && d.summary) || ""}`)
    .join(" \n ");
}

// Signale eines moeglichen Grossereignisses. `grossereignis` ist bewusst
// konservativ: Sicherheits-/Opferbezug ODER offizielle Reaktion muss zusammen mit
// einer der Verbreitungsbedingungen (mehrere unabhaengige Quellen ODER zeitliche
// Verdichtung) auftreten. Ein einzelner reisserischer Titel genuegt nie.
function grossereignisSignale(cluster = {}) {
  const docs = ((cluster && cluster.documents) || []).filter(Boolean);
  const text = clusterText(cluster);
  const quellen = new Set(docs.map((d) => d.source_id || d.source_name).filter(Boolean)).size;
  const zeiten = docs.map((d) => Date.parse(d.published_at || d.created_at || "")).filter((t) => Number.isFinite(t)).sort((a, b) => a - b);
  let verdichtung = false;
  for (let i = 0; i + VERDICHTUNG_MIN_DOKUMENTE - 1 < zeiten.length; i += 1) {
    const j = i + VERDICHTUNG_MIN_DOKUMENTE - 1;
    if (zeiten[j] - zeiten[i] <= VERDICHTUNG_FENSTER_H * 3600 * 1000) { verdichtung = true; break; }
  }
  const sicherheitsbezug = SICHERHEITSBEZUG.test(text);
  const reaktionen = OFFIZIELLE_REAKTION.test(text);
  const institutionen = INSTITUTION.test(text);
  const verbreitung = quellen >= UNABHAENGIGE_QUELLEN_MIN || verdichtung;
  const gewicht = sicherheitsbezug || reaktionen;
  return {
    quellen, verdichtung, sicherheitsbezug, reaktionen, institutionen,
    dokumente: docs.length,
    grossereignis: Boolean(gewicht && verbreitung)
  };
}

// Reihenfolge-Vorzug fuer Grossereignisse — wirkt UNABHAENGIG von
// HELMUT_UNDERSTANDING_PRIORITY. Begruendung: die vollstaendige Relevanzsortierung
// ist eine freigabepflichtige Produktentscheidung (P1-3), das Nicht-Verlieren eines
// toedlichen Anschlags ist keine. Ohne erkanntes Grossereignis bleibt die
// Reihenfolge byte-identisch zur Ankunftsreihenfolge.
function grossereignisseZuerst(clusters = []) {
  const liste = Array.isArray(clusters) ? clusters : [];
  const bewertet = liste.map((cluster, i) => ({ cluster, i, sig: grossereignisSignale(cluster) }));
  if (!bewertet.some((b) => b.sig.grossereignis)) return liste;
  return bewertet
    .sort((a, b) => (Number(b.sig.grossereignis) - Number(a.sig.grossereignis)) || (a.i - b.i))
    .map((b) => b.cluster);
}
...
function priorityTier(c = {}) {
  // Stufe 0: erkanntes Grossereignis (Betriebsbefund B4). Steht vor allem anderen.
  if (c.grossereignis === true) return TIER.GROSSEREIGNIS;
...
function clusterToCandidate(cluster = {}, assessed = {}) {
  ...
  const sig = grossereignisSignale(cluster);
  return {
    amtlich,
    grossereignis: sig.grossereignis,
...
module.exports = {
  TIER, priorityTier, priorityScore, secondaryScore, orderByPriority, selectWithinBudget,
  clusterToCandidate, clustersInPriorityOrder,
  grossereignisSignale, grossereignisseZuerst,
  VERDICHTUNG_FENSTER_H, VERDICHTUNG_MIN_DOKUMENTE, UNABHAENGIGE_QUELLEN_MIN
};
