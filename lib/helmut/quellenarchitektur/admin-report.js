...
function buildSourceAdminReport(inputs = {}) {
  ...
  // Reifegrad-Rollup je geography_id aus den Kandidaten-Seeds (Sprint 9): unterscheidet
  // Kandidat / verifiziert / bereit / aktiv (+ unbesetzt). „Kandidatenabdeckung" ist NICHT
  // Einsatzbereitschaft — heute ist NICHTS verifiziert/bereit/aktiv (Egress-Block). Rein
  // additiv; fehlt der Rollup (z. B. Niedersachsen), bleibt readiness null.
  const candidateReadiness = inputs.candidateReadiness || {};
  const laender = geographies.filter((g) => g.level === "land").map((g) => {
    ...
    // Pflichtklassen: required_classes UEBER ALLE Pakete dieser Geografie (Basis- + z. B.
    // Partei-Paket) — seit P0-2 leben Partei-/Personenklassen NICHT mehr im is_base-Paket,
    // zaehlen aber weiterhin zur Landesmodul-Pflichtklassenabdeckung.
    //
    // `present: 0` ist hier bewusst konservativ, NICHT die fachliche Wahrheit: dieser Report
    // arbeitet auf `buildFullModel()`, und das kennt fuer Berlin/Brandenburg keine Abrufwege
    // (die liegen im Landesmodul-Seed). Die tatsaechliche Klassenabdeckung wird seit Punkt 13
    // in `quellenarchitektur/paket-vollstaendigkeit.js` gegen das VEREINIGTE Modell gemessen
    // (Berlin 12/12, Brandenburg 12/12) und durch `scripts/paketvollstaendigkeit-test.js`
    // abgesichert; Nachweis: `docs/quellenarchitektur/31-paketvollstaendigkeit.md`.
    // Diese Anzeige an das vereinigte Modell zu haengen ist ein eigener Schritt (OP-23), weil
    // derselbe `catalog`-Eingang auch Aktivierung und Qualitaetsbericht speist — dort wuerden
    // 18 vorbereitete Landeswege die Kennzahlen verschieben. Bis dahin untertreibt die Anzeige
    // eher, als dass sie falsches Gruen erzeugt.
    const required = [...new Set(pkgs.flatMap((p) => Array.isArray(p.required_classes) ? p.required_classes : []))];
    ...
    const rd = candidateReadiness[g.id] || null;
    // Kandidatenabdeckung (aus Seeds) vs. Einsatzbereitschaft (verifiziert+bereit+aktiv).
    // Zahlenfelder defensiv auf Number normalisieren -> nie „undefined" im Client.
    const nz = (x) => Number.isFinite(Number(x)) ? Number(x) : 0;
    const readiness = rd
      ? {
          available: true,
          klassenGesamt: nz(rd.klassenGesamt),
          // Abdeckungs-Zaehler = DISTINCT Klassen mit Kandidat (Fallback auf besetzt fuer Altdaten).
          klassenAbgedeckt: nz(rd.klassenAbgedeckt != null ? rd.klassenAbgedeckt : rd.besetzt),
          besetzt: nz(rd.besetzt), unbesetzt: nz(rd.unbesetzt),
          unbesetzteKlassen: Array.isArray(rd.unbesetzteKlassen) ? rd.unbesetzteKlassen : [],
          kandidat: nz(rd.kandidat), verifiziert: nz(rd.verifiziert), bereit: nz(rd.bereit), aktiv: nz(rd.aktiv),
          einsatzbereit: nz(rd.einsatzbereit), hoechsteStufe: rd.hoechsteStufe || "unbesetzt"
        }
      : { available: false };
    return { geography_id: g.id, name: g.name, status, hasPackage: !!anyPkg, packageKey: anyPkg ? anyPkg.key : null, pflichtklassen, readiness };
  });
  ...
  const pakete = packages.map((pk) => {
    ...
    return {
      ...
      supply, pathCount: h.pathCount, healthyPathCount: h.gesund, defektPathCount: h.defekt, inaktivPathCount: h.inaktiv, unknownPathCount: h.unbekannt,
  ...
  const pathViews = paths.map((p) => {
    // Fallback (fehlender pathQuality-Eintrag): NICHT hart 'unbekannt', sondern den Katalog-
    // Status ehren — ein als 'broken' markierter (kritischer) Weg bleibt sichtbar defekt.
    const pq = pathQualityById.get(p.id) || {
      technicalHealth: p.status === "broken" ? "defekt" : (p.status === "paused" || p.status === "archived" || p.activation_mode === "dev_only" ? "inaktiv" : "unbekannt"),
      productValue: "unbekannt", contentYield: { documentCount: 0 }, koCount: 0, duplicateCount: null,
      is_critical: p.is_critical === true, active: false,
      recommendedAction: p.status === "broken"
        ? { severity: p.is_critical ? "hoch" : "mittel", text: p.is_critical ? "Defekter PFLICHT-Abrufweg: Ersatzweg suchen, NICHT still archivieren." : "Defekter Abrufweg: reparieren oder deaktivieren." }
        : { severity: "keine", text: "Keine." }
    };
    const pub = pubById.get(p.publisher_id) || null;
    const health = healthBucket(pq);
    const value = valueBucket(pq);
    // Fix A: Kosten je Quelle nur, wenn diese Quelle TATSÄCHLICH attribuiert ist (im costBySource).
    // Bei partieller Attribution sonst ein erfundenes „$0" für nicht zugeordnete Quellen.
    const attributed = !!costBySource && Object.prototype.hasOwnProperty.call(costBySource, p.legacy_source_id);
    return {
      ...
      cost: { available: attributed, usd: attributed ? round6(costBySource[p.legacy_source_id]) : null },
  ...
  for (const pk of pakete) if (pk.supply === "unterversorgt") {
    // Ehrlich nach tatsächlicher Ursache formulieren: defekt (ersetzen) vs. inaktiv (reaktivieren).
    const n = pk.pathCount, weg = n === 1 ? "Abrufweg" : "Abrufwege";
    let text;
    if (pk.defektPathCount === n) text = `Paket „${pk.key}“: ${n === 1 ? "der einzige" : "alle " + n} ${weg} defekt — Quellen ersetzen.`;
    else if (pk.inaktivPathCount === n) text = `Paket „${pk.key}“: ${n === 1 ? "der einzige" : "alle " + n} ${weg} inaktiv/pausiert — reaktivieren.`;
    else text = `Paket „${pk.key}“: kein nutzbarer Abrufweg (${pk.defektPathCount} defekt, ${pk.inaktivPathCount} inaktiv) — ersetzen/reaktivieren.`;
    pruefActions.push({ area: "paket", ref: pk.key, severity: pk.is_base ? "hoch" : "mittel", text });
  }
  ...
  return {
    ...
    counts: {
      ...
      // Reifegrad-Aggregat ueber Länder MIT Kandidaten-Rollup (ehrlich: Kandidat != einsatzbereit).
      kandidatKlassen: laender.reduce((n, l) => n + (l.readiness.available ? l.readiness.kandidat : 0), 0),
      einsatzbereiteKlassen: laender.reduce((n, l) => n + (l.readiness.available ? l.readiness.einsatzbereit : 0), 0),
      unbesetzteKlassen: laender.reduce((n, l) => n + (l.readiness.available ? l.readiness.unbesetzt : 0), 0)
    },
    views: {
