...
// Bundesland -> Landespaket-Key. Landtag-Sprint (Phase 7): abgeleitet aus der EINEN
// Landesmodul-Registry (landesmodule.js) statt eigener Kopie — ein weiteres Bundesland
// ist damit EIN Registry-Eintrag (Daten), keine Doppelpflege. Nur Laender mit
// (vorbereitetem) Landespaket; Landtagsprofile anderer Laender bleiben bewusst nicht
// vollstaendig aktivierbar (requiredMissing).
const { LANDESMODULE } = require("./landesmodule");
const LANDESPAKET_BY_BUNDESLAND = Object.freeze(Object.fromEntries(
  Object.entries(LANDESMODULE).map(([land, mod]) => [land, mod.paketKeys[0]])
));

// Personenbezogene Pakete: an EIN konkretes Profil (Pilot-Account) gebunden, NICHT aus
// Sachfeldern ableitbar (personenbezogene Nachrichtensuche nur fuer dieses Profil, Auftrag §7.4).
// Ohne diese explizite Bindung waere das Paket (und sein demoOnly-Abrufweg) unerreichbar:
// resolveProfilePackages wuerde es nie erzeugen -> es wuerde nie referenziert -> nie aktiviert.
// Gebunden an die konkrete Profil-ID, nicht an den Namen: ein anderes "Cem"-Profil bekommt es NICHT.
const PERSONAL_PACKAGE_BY_PROFILE = {
  "cem-ince": "profil-cem-ince"
};
...
function resolveProfilePackages(profile = {}) {
  ...
  // Macht das sonst unerreichbare Pilot-Paket ueber die Referenzzaehlung aktivierbar.
  const profileId = String(profile.id || profile.user_id || profile.politicianId || "").trim().toLowerCase();
  const personalPackage = PERSONAL_PACKAGE_BY_PROFILE[profileId];
