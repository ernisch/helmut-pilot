...
function computeGlobalActivation(input = {}) {
  ...
  for (const p of activeProfiles) {
    // P3-18 (Architektur-Audit 29): Fallback-Schluessel fuer id-lose Profile ohne die alte
    // slice(0,40)-Verkuerzung — zwei DISTINKTE Profile, die in den ersten 40 JSON-Zeichen
    // uebereinstimmen (z. B. gleiche fuehrende Felder), waeren sonst als EIN Profil gezaehlt
    // und haetten die Referenzzaehlung verfaelscht. Voller JSON-Abdruck = kollisionsfrei fuer
    // unterscheidbare Profile (strukturell identische anonyme Profile zaehlen bewusst als eins).
    const pid = p.id || p.user_id || p.politicianId || `anon:${JSON.stringify(p)}`;
