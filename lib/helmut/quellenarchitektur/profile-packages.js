...
// Robuste Ebenen-Aufloesung: liest VERBINDLICH mandate_profiles.politische_ebene
// ('bundestag'/'landtag') direkt — config.parliamentTypeOf wertet dieses Feld NICHT aus
// (nur parliamentType/level). Fallback auf parliamentTypeOf fuer weitere Aliase/Rollen.
function resolveParliamentType(profile = {}) {
  const raw = norm(profile.politische_ebene || profile.parliamentType || profile.ebene || profile.level || "");
  if (raw.includes("landtag") || raw.startsWith("land")) return "Landtag";
  if (raw.includes("bundestag") || raw.startsWith("bund")) return "Bundestag";
  return parliamentTypeOf(profile);
}

// Robuste Feldextraktion (mandate_profiles-deutsch ODER gemapptes Objekt-englisch).
function profileFieldset(profile = {}) {
  return {
    parliamentType: resolveParliamentType(profile), // "Bundestag" | "Landtag" | ""
...
function resolveProfilePackages(profile = {}) {
  ...
  const socialByTopic = f.themen.some((t) => SOCIAL_TERMS.some((term) => normRaw(t).includes(term)));
...
//   - Paket technisch aktiv, wenn (>=1 aktives Profil braucht es ODER always_on) UND status==='active'.
//     (prepared/draft-Pakete werden NIE technisch aktiviert -> keine Quellen; angeforderte
//      prepared-Pakete erscheinen als 'requested_unsupplied'.)
//   - Abrufweg aktiv per Referenzzaehlung (>=1 aktives Paket via package_paths) ODER always_on.
function computeGlobalActivation(input = {}) {
  ...
  const packageStatus = packages.map((pk) => {
    ...
    const alwaysOn = pk.always_on === true;
    let activation;
    if (pk.status === "active" && (neededByProfile || alwaysOn)) activation = "active";
    ...
    return { id: pk.id, key: pk.key, status: pk.status, always_on: alwaysOn, refCount, activation };
  });
