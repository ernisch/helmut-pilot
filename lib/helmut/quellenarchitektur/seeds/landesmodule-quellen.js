...
// P0-2 (Architektur-Audit 29): Partei-/Fraktions-/Person-Pilot-Klassen gehoeren NIEMALS ins
// verpflichtende is_base-Basispaket (jedes Landtagsprofil erhaelt es zwingend) — sie leben im
// eigenen, optionalen Partei-Paket (analog "die-linke-bund" auf Bundesebene).
const LAND_PARTEI_PACKAGE = Object.freeze({ berlin: "pkg-die-linke-berlin", brandenburg: "pkg-die-linke-brandenburg" });
const PILOT_KLASSEN = new Set(["partei_pilot", "fraktion_pilot", "person_pilot"]);
function packageIdFor(land, klasse) {
  return PILOT_KLASSEN.has(klasse) ? LAND_PARTEI_PACKAGE[land] : LAND_PACKAGE[land];
}
...
// Herausgeber-evidence_role aus dem Publisher-Typ ableiten (DB-CHECK-konform):
// amtlich (Parlament/Regierung/Ministerium) -> official_primary; Medien -> journalistic;
// Partei/Fraktion/Person (Eigeninteresse) -> direct_interest; Aggregator -> aggregator.
function publisherEvidenceRole(type) {
  if (type === "aggregator") return "aggregator";
  if (type === "media") return "journalistic";
  if (type === "party" || type === "parliamentary_group" || type === "person") return "direct_interest";
...
function buildLandesmodulSeed() {
  ...
  for (const land of ["berlin", "brandenburg"]) {
    ...
    for (const c of LANDESMODUL_KANDIDATEN[land]) {
      ...
      const pkgId = packageIdFor(land, c.klasse); // P0-2: Pilotklassen -> Partei-Paket, sonst Basispaket
