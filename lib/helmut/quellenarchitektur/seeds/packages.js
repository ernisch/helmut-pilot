...
// Pflichtklassen eines REGIONS-Pakets (Punkt 13, Nachbesserung). Vorher stand hier nur
// "regionalquellen" — das war mit vier Google-News-Themensuchen erfuellbar, ohne einen einzigen
// benannten regionalen Herausgeber. Die Sollmenge unterscheidet jetzt Ebenen und
// Herausgeberarten; `regionalquellen` (Aggregator-Themensuchen) bleibt ERGAENZEND, ist aber
// allein nicht ausreichend.
const REGIONALPAKET_PFLICHTKLASSEN = [
  "landesparlament",              // amtliche parlamentarische Ebene des Landes
  "landesregierung",              // Landesregierung / Landesverwaltung
  "regionale_leitmedien",         // benannte journalistische Landesquelle
  "oer_landesberichterstattung",  // oeffentlich-rechtliche Regionalredaktion
  "wahlkreismedien",              // benannte journalistische Quelle der Wahlkreisregion
  "regionalquellen"               // ergaenzende Themensuchen (nie die einzige Grundlage)
];
...
const PACKAGE_DEFINITIONS = [
  ...
  },
  {
    ...
    // Zweck praezisiert (Punkt 13): das Paket ist REGIONS-, nicht themengebunden — symmetrisch
    // zur Klarstellung "Fachthema, NICHT Region" bei arbeit-und-soziales. Die benannte Basis
    // (Landtag, Landesregierung, Landes- und Wahlkreismedien) ist themenoffen; nur die
    // ergaenzenden Themensuchen tragen eine Fachbindung.
    purpose: "Regionale Beobachtung Niedersachsen (Salzgitter/Braunschweig/Wolfenbuettel): amtliche Landesebene, Landes- und Wahlkreismedien, ergaenzende Themensuchen. Region, NICHT Fachthema.",
...
function regionalPackageKeyForSource(source = {}) {
  // EXPLIZITE Paketbindung hat Vorrang vor dem Begriffsabgleich: die benannten Pflichtquellen
  // eines Regionalpakets tragen `regionalPaket`. Der Begriffsabgleich unten bleibt fuer die
  // Bestands-Themensuchen, deren Region nur im Namen steht.
  const explizit = String(source.regionalPaket || "").trim();
  if (explizit) return REGION_TERMS_BY_PACKAGE[explizit] ? explizit : "";
