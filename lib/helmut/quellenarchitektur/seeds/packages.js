...
const PACKAGE_DEFINITIONS = [
  ...
  // --- Strukturell vorbereitetes Fachthemenpaket Gesundheit Bund (Auftrag: Quellenpaket
  // gesundheit-bund), noch INAKTIV. Abrufwege liegen (wie bei den Landesmodulen) in einem
  // eigenen standalone-Seed (seeds/gesundheit-bund-quellen.js), status needs_review + manual.
  // is_base:false -> Fachthema (kein Pflicht-Basispaket); kein Profil-Mapping mappt darauf
  // (packages.packageKeysForSource + profile-packages.resolveProfilePackages unveraendert)
  // -> das Paket wird von computeGlobalActivation NIE aktiviert.
  {
    id: "pkg-gesundheit-bund", key: "gesundheit-bund", name: "Gesundheit Bund",
    purpose: "Fachthemenpaket Gesundheitspolitik (Bund): normative Primärquellen (BMG, Bundesrat-Gesundheitsausschuss, G-BA), amtliche Datenquellen (RKI, Destatis), gesetzliche Evidenz (IQWiG, SVR), Zulassungs-/Sicherheitsbehörden (BfArM, PEI) und Akteurspositionen (GKV-SV, KBV, DKG, BÄK, Pflegerat). Parlamentarischer Prozess über Bestand (DIP-API + Ausschuss Gesundheit) abgedeckt. Struktur vorbereitet — Quellen INAKTIV bis Byte-Verifikation + Freigabe.",
