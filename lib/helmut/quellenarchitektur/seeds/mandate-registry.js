...
const COMMITTEE_RELATIONS = {
  ...
  "umwelt": {
    ...
    aliases: ["klima und umwelt", "umwelt naturschutz nukleare sicherheit und verbraucherschutz", "klima", "klimaausschuss", "klimaschutz", "klimaschutzausschuss", "umweltausschuss"]
  },
  "ernaehrung": {
  ...
  "wohnen": {
    ...
    aliases: ["bauen und wohnen", "wohnen stadtentwicklung bauwesen und kommunen", "bau", "wohnungs", "wohnungsausschuss", "bau wohnungs", "bau wohnungsausschuss", "bauausschuss"]
  },
  "menschenrechte": {
  ...
  "petitionen": {
    // Verfahrensgremium ohne Politikfeld — aber als Ausschuss ERKANNT (known=true),
    // damit ein Petitionsausschuss-Mitglied nicht durchs Register faellt.
    policyField: null, ministry: null, ministryEntity: null,
    themeTerms: [], aliases: ["petitions", "petitionsausschuss"]
  }
};
...
function buildPartyIndex() {
  ...
  // "Union" / "CDU/CSU" ist eine anerkannte SAMMELBEZEICHNUNG (normalizeParty -> "union"),
  // die die gemeinsame Fraktion cdu-csu traegt. Ohne diesen Eintrag waere resolveParty("Union")
  // unbekannt, obwohl die Fraktion aufloest — genau die vom Review bemaengelte Asymmetrie.
  // isGroup:true -> erscheint NICHT in listParties()/registrySummary (bleibt 9 echte Parteien),
  // ist aber ein gueltiger, bekannter Registry-Schluessel (kein "geratener" Fremdschluessel).
  byKey.set("union", {
    key: "union", name: "CDU/CSU (Unionsparteien)", entity_id: null,
    aliases: ["Union", "CDU/CSU", "Unionsparteien"], fractionKey: "cdu-csu", isGroup: true
  });
  return byKey;
}
...
function listParties() { return [...PARTY_INDEX.values()].filter((p) => !p.isGroup); }
...
function registrySummary() {
  ...
  return {
    ...
    parties: listParties().length,   // 9 echte Parteien (Sammelbezeichnung "union" ausgenommen)
