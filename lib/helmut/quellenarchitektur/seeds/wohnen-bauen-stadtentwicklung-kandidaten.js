...
const WBSB_KANDIDATEN = [
  ...
  // FACHREVIEW 2026-07-24: die Erst-URLs lieferten HTTP 404 (stale/umgezogen). Ersetzt durch die
  // per WebSearch (2026-07-24) als LIVE bestaetigten aktuellen Adressen -> Runner-Reverifikation.
  {
    ...
    method: "html", retrieval_type: "html_liste", update_character: "laufend",
    fachliche_begruendung: "Zentrale Ressortforschung Wohnungsmarkt/Stadtentwicklung/Raumbeobachtung — Frühindikator. Vom Auditor ausdrücklich vermisst (Fachreview §2).",
  ...
  },
  {
    ...
    method: "html", retrieval_type: "html_liste", update_character: "laufend",
    fachliche_begruendung: "Amtliche Presseinformationen der Ressortforschung (Wohnungs-/Immobilienmärkte, Städtebau).",
    dubletten_hinweis: "Überschneidung mit bbsr-aktuell möglich",
    aufnahme_absicht: "referenz"
  },
  {
    ...
    method: "html", retrieval_type: "html_liste", update_character: "laufend",
    fachliche_begruendung: "Laufende Publikationsliste des BBSR (Wohnungs- und Immobilienmärkte, Städtebau, Raumbeobachtung).",
    dubletten_hinweis: "Überschneidung mit bbsr-aktuell möglich",
    aufnahme_absicht: "referenz"
  },
  {
    ...
    method: "rss", retrieval_type: "rss", update_character: "laufend",
    fachliche_begruendung: "Bevorzugte Zielmethode (RSS). GSB-Sites exponieren echte Feeds unter /SiteGlobals/Functions/RSSFeed/…; die Hub-Seite listet die abonnierbaren .xml-Feeds.",
    dubletten_hinweis: "liefert i. d. R. HTML-Hub statt Feed — Runner muss die konkrete Feed-URL extrahieren (Alternative: IDW-Feed idw-online.de/de/institution957)",
    aufnahme_absicht: "referenz"
  },
  ...
  },
  {
    ...
    fachliche_begruendung: "Strukturierter Superset der amtlichen Bau-/Wohnungs-Statistiken (Bautätigkeit, Baupreisindex, Mieten, Wohngeld) — ersetzt fragile HTML-Tabellen-Scrapes. REST-Basis dokumentiert (GENESIS-Webservice-Doku v5.1).",
    dubletten_hinweis: "Superset von destatis-bautaetigkeit/-baupreisindex/-wohnen-mieten/-wohngeld; POST + Zugangsdaten (kostenfreier Token) = Aktivierungsschritt",
    aufnahme_absicht: "pflichtkern"
  },
  {
    ...
    fachliche_begruendung: "Dokumentierte, exakte XML-URL (DTD /dtd/1.0/gii-toc.dtd) — strukturierte Änderungserkennung an BauGB/BauNVO/GEG/WoFG ohne Einzelgesetz-Polling. Ersetzt die im Vorsprint (Timeout) verworfenen gesetze-im-internet-HTML-Kandidaten.",
    dubletten_hinweis: "thematische Nähe zu bgbl-teil1-liste (recht.bund.de = neue Verkündungen; gii-toc = konsolidierter Normbestand)",
    aufnahme_absicht: "pflichtkern"
  },
