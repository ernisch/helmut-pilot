...
//
// SPRINT 9B: Der geforderte echte Abruf+Parser-Test konnte NICHT durchgeführt werden — der
// Egress der Ausführungsumgebung ist gesperrt (curl UND WebFetch liefern 403 selbst für
// example.com; Kontroll-Abruf in scripts/sprint9b-verify-abrufwege.js bestätigt das). Daher
// bleibt bewusst KEIN Weg "repariert": bewertung ist maximal "reparierbar" (= reparierbar,
// Adresse recherchiert), niemals "repariert". Der echte Nachweis erfolgt durch Ausführen des
// 9B-Harness in einer Umgebung mit Egress.
//
...
const BUNDESWEG_REPARATUREN = [
  {
    ...
    note: "Exakten Feed-Dateinamen (pressemitteilungen.rss vs. presse.rss) + hib-Feed byte-genau bestätigen. Sprint 9B: /services/rss/ als RSS-Hub bestätigt (WebSearch), exakter XML-Dateiname NICHT byte-bestätigt; echter Abruf Egress-blockiert -> weiterhin 'reparierbar', NICHT 'repariert'."
  },
  {
    ...
    bewertung: "reparierbar", verifyBeforeActivation: true,
    note: "GSB-Feed-Pfad variiert je Rubrik — RSS_Breg_aktuell-Pfad bzw. separaten Pressemitteilungen-Feed verifizieren. Sprint 9B: exakte Adresse (…/RSS_Breg_aktuell/RSSNewsfeed.xml) per WebSearch bestätigt; echter Abruf+Parser Egress-blockiert -> weiterhin 'reparierbar', NICHT 'repariert'."
  },
  {
    ...
    bewertung: "reparierbar", verifyBeforeActivation: true,
    note: "feed.rss-Muster durch Relaunch bestätigt; Item-Aktualität prüfen. Sprint 9B: /start/presse/feed.rss per WebSearch NICHT bestätigt — realer Feed-Kandidat /themen/nachrichten/feed.rss (byte-genau gegen /start/presse/feed.rss klären); weiterhin 'reparierbar', NICHT 'repariert'."
  },
  {
    ...
    ergaenzung: null,
    bewertung: "reparierbar", verifyBeforeActivation: true,
    note: "Feed-Titel im Index zeigte noch Gruppe (statt Fraktion) — Inhalt/Aktualität nach Fraktions-Rückkehr prüfen. Sprint 9B: dielinkebt.de/presse/pressemitteilungen/feed.rss per WebSearch als aktueller Feed bestätigt (2026-Items); echter Abruf Egress-blockiert -> weiterhin 'reparierbar', NICHT 'repariert'."
  },
  {
