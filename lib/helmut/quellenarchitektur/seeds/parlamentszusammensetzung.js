...
const BUNDESTAG_21 = Object.freeze({
  ...
  pruefart: "amtliche_sekundaerbelege",
  pruefvorbehalt: "Der Volltext der Drucksache 21/150 war aus der pruefenden Sitzung nicht abrufbar (403 auf CONNECT). Grundlage sind die oben genannten amtlichen Bundestagsseiten."
});
...
const LANDTAG_BRANDENBURG_8 = Object.freeze({
  ...
  geprueft_am: "2026-07-26",
  pruefart: "dokumentenlage",
  pruefvorbehalt: "Phase-1-Punkt 15: eine LIVE-Gegenprobe war nicht moeglich — der Egress dieser Sitzung erreicht landtag.brandenburg.de, wahlen.brandenburg.de und auch de.wikipedia.org nicht (curl-Code 000, Verbindungsaufbau blockiert). Grundlage bleibt die oben belegte Dokumentenlage aus Punkt 13. Eine echte Neupruefung gegen die Primaerquelle muss aus einer Umgebung mit offenem Egress erfolgen (wie Sprint 9B auf dem GitHub-Actions-Runner)."
});
...
// --- Nachpruefpflicht (Phase-1-Punkt 15) ------------------------------------
// Der HARTE Driftschutz existiert bereits: aendert sich diese Datei (z. B. Die Linke kehrt in
// den Landtag zurueck), wird jede darauf gestuetzte "nicht anwendbar"-Ausnahme unbestaetigt und
// `paketvollstaendigkeit-test.js` faellt auf "teilweise" -> rot.
//
// Was fehlte, ist die ZEITACHSE: niemand war verpflichtet nachzusehen, ob dieses Bild der
// Wirklichkeit noch stimmt. Eine politische Aenderung wirkt sich in dieser Datei erst aus,
// wenn ein Mensch sie eintraegt. `geprueft_am` + `pruefstand()` machen das sichtbar.
//
// BEWUSST KEIN zeitbasierter Testfehler: ein Test, der an einem Stichtag von allein rot wird,
// blockiert CI ohne fachlichen Anlass. Die Faelligkeit wird BERICHTET (Admin-/Vollstaendigkeits-
// bericht, Aktivierungs-Runbook), nicht erzwungen.
const NACHPRUEFUNG_HORIZONT_TAGE = 180;

function istIsoDatum(v) { return typeof v === "string" && /^\d{4}-\d{2}-\d{2}$/.test(v) && !Number.isNaN(Date.parse(`${v}T00:00:00Z`)); }

// Pruefstand eines Parlaments zu einem Stichtag. `heute` wird INJIZIERT (Tests bleiben
// deterministisch); ohne Angabe gilt der reale Tag.
function pruefstand(parlamentKey, heute = new Date()) {
  const p = parlament(parlamentKey);
  if (!p) return { bekannt: false, grund: `unbekanntes Parlament '${parlamentKey}'` };
  const stichtag = heute instanceof Date ? heute : new Date(String(heute));
  const gueltig = istIsoDatum(p.geprueft_am);
  const alterTage = gueltig
    ? Math.floor((Date.UTC(stichtag.getUTCFullYear(), stichtag.getUTCMonth(), stichtag.getUTCDate()) - Date.parse(`${p.geprueft_am}T00:00:00Z`)) / 86400000)
    : null;
  return {
    bekannt: true,
    parlament: p.parlament,
    parlamentName: p.name,
    wahlperiode: p.wahlperiode,
    geprueft_am: gueltig ? p.geprueft_am : null,
    datumGueltig: gueltig,
    pruefart: p.pruefart || null,
    pruefvorbehalt: p.pruefvorbehalt || null,
    belege: p.belege || [],
    alterTage,
    horizontTage: NACHPRUEFUNG_HORIZONT_TAGE,
    // Fehlendes/kaputtes Datum gilt als faellig — nicht als "frisch".
    nachpruefungFaellig: !gueltig || alterTage > NACHPRUEFUNG_HORIZONT_TAGE || alterTage < 0
  };
}
