...
// =============================================================================================
// AUSFUEHRUNGSSCHRITTE (Befund V-1, 2026-07-26)
// =============================================================================================
// Vorher lagen Block A, B1, Stufe 1 und Stufe 2 in EINER Datei, Block B zusaetzlich in EINER
// Transaktion (ein `begin` … `commit` um B1 + B2.1 + B2.2). Die Staffelung bestand damit nur
// aus einer Kommentarzeile: wer die Datei am Stueck ausfuehrte, schaltete alle vier Wege auf
// einmal scharf — inklusive der zwei Google-Wege, deren ganzer Zweck es war, einen vollen
// Crawl-Zyklus SPAETER zu kommen.
//
// Jetzt gilt strukturell:
//   - je Schritt EINE Datei und EINE Transaktion,
//   - jeder Schritt traegt ausfuehrbare VOR- und NACHbedingungen; jede verletzte Bedingung
//     endet in `raise exception`, also fail-closed mit Rollback der eigenen Transaktion
//     (nicht in einem stillen „0 Zeilen betroffen", das wie Erfolg aussieht),
//   - Stufe 2 verlangt zusaetzlich den TELEMETRIE-BELEG, dass Stufe 1 real gelaufen ist.
//     Reihenfolge allein genuegt nicht: zwei Dateien direkt hintereinander auszufuehren waere
//     formal „Stufe 1 zuerst" und trotzdem genau der Lastfall, den die Staffelung verhindert.
//
// Die Bedingungen sind DEKLARATIV. Generator (SQL-Text) und Test (Ausfuehrung gegen eine
// In-Memory-Datenbank) lesen dieselbe Liste — Staffelung im SQL und Staffelung im Test koennen
// deshalb nicht auseinanderlaufen.

const DATEI_STOP = "20260726_berlin_aktivierung.sql";
const DATEI_A = "20260726_berlin_aktivierung_a_neutralisierung.sql";
const DATEI_B1 = "20260726_berlin_aktivierung_b1_paketstatus.sql";
const DATEI_S1 = "20260726_berlin_aktivierung_b2_stufe1.sql";
const DATEI_S2 = "20260726_berlin_aktivierung_b2_stufe2.sql";
const DATEI_S1_RB = "20260726_berlin_aktivierung_b2_stufe1_rollback.sql";
const DATEI_S2_RB = "20260726_berlin_aktivierung_b2_stufe2_rollback.sql";
const DATEI_RB_ALLE = "20260726_berlin_aktivierung_rollback.sql";
const DATEI_RB_VOLL = "20260726_berlin_aktivierung_rollback_vollstaendig.sql";

const GESPERRT = Object.freeze({ status: "needs_review", activation_mode: "manual" });
const AKTIV = Object.freeze({ status: "healthy", activation_mode: "auto" });

function stufenWege(nummer) {
  const stufe = AKTIVIERUNGSSTUFEN.find((s) => s.stufe === nummer);
  return stufe ? stufe.wege.slice().sort() : [];
}

// Abrufweg-Id -> Quellenkennung der Telemetrie (source_crawl_telemetry.source_id ist die
// Crawler-Quellen-Id, also legacy_source_id — NICHT die Abrufweg-Id).
function telemetrieQuellen(wegIds) {
  const nachId = new Map(berlinWegeAusSeed().map((w) => [w.id, w.legacy_source_id]));
  return wegIds.map((id) => nachId.get(id) || id).slice().sort();
}

// --- Bedingungen: eine Form, drei Verwender (SQL, Test, Dry Run) ------------------------------
function bedZuordnung(paket, wege, erlaubt, meldung) {
  return Object.freeze({ art: "zuordnung", paket, wege: Object.freeze(wege.slice().sort()), erlaubt: Object.freeze(erlaubt.slice()), meldung });
}
function bedPaketstatus(key, erlaubt, meldung) {
  return Object.freeze({ art: "paketstatus", key, erlaubt: Object.freeze(erlaubt.slice()), meldung });
}
function bedWegzustand(wege, zustand, erlaubt, meldung) {
  return Object.freeze({ art: "wegzustand", wege: Object.freeze(wege.slice().sort()), status: zustand.status, activation_mode: zustand.activation_mode, erlaubt: Object.freeze(erlaubt.slice()), meldung });
}
function bedTelemetriebeleg(quellen, minLaeufeJeQuelle, meldung) {
  return Object.freeze({ art: "telemetriebeleg", quellen: Object.freeze(quellen.slice().sort()), status: "ok", minLaeufeJeQuelle, meldung });
}

// Auswertung gegen eine Datenbanksicht. Rein, ohne IO. `db` traegt die vier Tabellen als
// Arrays; fehlende Tabelle = leer (und damit fail-closed, nicht „ok").
function pruefeBedingung(bedingung, db = {}) {
  const rp = db.retrieval_paths || [];
  const sp = db.source_packages || [];
  const pp = db.package_paths || [];
  const tel = db.source_crawl_telemetry || [];
  if (bedingung.art === "zuordnung") {
    const ist = pp.filter((r) => r.package_id === bedingung.paket && bedingung.wege.includes(r.retrieval_path_id)).length;
    return { ok: bedingung.erlaubt.includes(ist), ist, erlaubt: bedingung.erlaubt };
  }
  if (bedingung.art === "paketstatus") {
    const zeile = sp.find((r) => r.key === bedingung.key);
    const ist = zeile ? zeile.status : null;
    return { ok: bedingung.erlaubt.includes(ist), ist, erlaubt: bedingung.erlaubt };
  }
  if (bedingung.art === "wegzustand") {
    const ist = rp.filter((r) => bedingung.wege.includes(r.id) && r.status === bedingung.status
      && r.activation_mode === bedingung.activation_mode).length;
    return { ok: bedingung.erlaubt.includes(ist), ist, erlaubt: bedingung.erlaubt };
  }
  if (bedingung.art === "telemetriebeleg") {
    const zuWenig = bedingung.quellen.filter((sid) => {
      const laeufe = new Set(tel.filter((t) => t.source_id === sid && t.status === bedingung.status).map((t) => t.run_id));
      return laeufe.size < bedingung.minLaeufeJeQuelle;
    });
    return { ok: zuWenig.length === 0, ist: zuWenig, erlaubt: [] };
  }
  throw new Error("Unbekannte Bedingungsart: " + bedingung.art);
}

// Reihenfolge der Schritte. `wegeBeruehrt` ist verbindlich: kein ausfuehrbares Statement einer
// Datei darf einen Abrufweg nennen, der hier nicht steht (testgesichert). Genau das haelt
// Stufe 1 und Stufe 2 getrennt — und haelt Brandenburg, Bund und die Partei-/Fraktions-/
// Personenwege aus den Aktivierungsdateien heraus.
function ausfuehrungsschritte() {
  const s1 = stufenWege(1);
  const s2 = stufenWege(2);
  const alleBasisWege = berlinWegeAusSeed().map((w) => w.id).filter((id) => !UMHAENGUNG.wege.includes(id)).sort();
  return Object.freeze([
    Object.freeze({
      id: "A", nummer: 1, art: "aktivierung", stufe: null, datei: DATEI_A,
      titel: "Block A · Neutralisierung von " + BASISPAKET_KEY,
      zweck: "Die 3 Partei-/Fraktions-/Personenwege vom is_base-Pflichtpaket an das optionale "
        + "Parteipaket umhaengen (P0-2 / Befund A-3). Aktiviert NICHTS.",
      wegeBeruehrt: Object.freeze([]),        // nur Zuordnungen, kein Wegzustand
      zuordnungBeruehrt: Object.freeze([BASISPAKET_ID, PARTEIPAKET_ID]),
      paketBeruehrt: Object.freeze([]),
      vorbedingungen: Object.freeze([
        bedZuordnung(BASISPAKET_ID, UMHAENGUNG.wege, [0, 3],
          "Teilzuordnung am Pflicht-Basispaket: erwartet 3 (Ausgangszustand) oder 0 (bereits umgehaengt)"),
        bedZuordnung(PARTEIPAKET_ID, UMHAENGUNG.wege, [0, 3],
          "Teilzuordnung am Parteipaket: erwartet 0 (Ausgangszustand) oder 3 (bereits umgehaengt)")
      ]),
      nachbedingungen: Object.freeze([
        bedZuordnung(BASISPAKET_ID, UMHAENGUNG.wege, [0], "Pflicht-Basispaket traegt weiterhin Partei-/Fraktions-/Personenwege"),
        bedZuordnung(PARTEIPAKET_ID, UMHAENGUNG.wege, [3], "Parteipaket hat nicht alle 3 Wege uebernommen")
      ]),
      weiterMit: DATEI_B1, rollbackDatei: DATEI_RB_VOLL
    }),
    Object.freeze({
      id: "B1", nummer: 2, art: "aktivierung", stufe: null, datei: DATEI_B1,
      titel: "Block B1 · Paketstatus " + BASISPAKET_KEY + " prepared -> active",
      zweck: "Macht die Referenzzaehlung eines Berliner Landtagsprofils erst wirksam "
        + "(computeGlobalActivation). Schaltet KEINEN Abrufweg scharf.",
      wegeBeruehrt: Object.freeze([]),
      zuordnungBeruehrt: Object.freeze([]),
      paketBeruehrt: Object.freeze([BASISPAKET_KEY]),
      vorbedingungen: Object.freeze([
        bedZuordnung(BASISPAKET_ID, UMHAENGUNG.wege, [0], "Block A ist nicht ausgefuehrt — das Pflichtpaket ist nicht neutral"),
        bedPaketstatus(BASISPAKET_KEY, ["prepared", "active"], "Unerwarteter Paketstatus (erwartet prepared oder bereits active)")
      ]),
      nachbedingungen: Object.freeze([
        bedPaketstatus(BASISPAKET_KEY, ["active"], "Paketstatus ist nicht 'active'")
      ]),
      weiterMit: DATEI_S1, rollbackDatei: DATEI_RB_ALLE
    }),
    Object.freeze({
      id: "S1", nummer: 3, art: "aktivierung", stufe: 1, datei: DATEI_S1,
      titel: "Stufe 1 · " + s1.length + " Direktfeeds scharfschalten",
      zweck: AKTIVIERUNGSSTUFEN[0].grund,
      wegeBeruehrt: Object.freeze(s1),
      zuordnungBeruehrt: Object.freeze([]),
      paketBeruehrt: Object.freeze([]),
      vorbedingungen: Object.freeze([
        bedZuordnung(BASISPAKET_ID, UMHAENGUNG.wege, [0], "Block A ist nicht ausgefuehrt"),
        bedPaketstatus(BASISPAKET_KEY, ["active"], "Block B1 ist nicht ausgefuehrt (Paket nicht 'active')"),
        bedWegzustand(s2, GESPERRT, [s2.length],
          "Stufe 2 ist nicht mehr vollstaendig gesperrt — die Reihenfolge waere vertauscht")
      ]),
      nachbedingungen: Object.freeze([
        bedWegzustand(s1, AKTIV, [s1.length], "Stufe 1 ist nur teilweise aktiv (Teilausfuehrung)"),
        bedWegzustand(s2, GESPERRT, [s2.length], "Stufe 2 wurde mitaktiviert — genau das darf nie passieren")
      ]),
      weiterMit: DATEI_S2, weiterErst: AKTIVIERUNGSSTUFEN[0].weiterErst, rollbackDatei: DATEI_S1_RB
    }),
    Object.freeze({
      id: "S2", nummer: 4, art: "aktivierung", stufe: 2, datei: DATEI_S2,
      titel: "Stufe 2 · " + s2.length + " Google-News-Wege scharfschalten",
      zweck: AKTIVIERUNGSSTUFEN[1].grund,
      wegeBeruehrt: Object.freeze(s2),
      zuordnungBeruehrt: Object.freeze([]),
      paketBeruehrt: Object.freeze([]),
      vorbedingungen: Object.freeze([
        bedPaketstatus(BASISPAKET_KEY, ["active"], "Paket ist nicht 'active'"),
        bedWegzustand(s1, AKTIV, [s1.length],
          "Stufe 1 ist nicht vollstaendig aktiv — Stufe 2 darf nur nach erfolgreicher Stufe 1 laufen"),
        bedTelemetriebeleg(telemetrieQuellen(s1), 2,
          "Stufe 1 ist nicht belegt gelaufen: je Weg mindestens 2 erfolgreiche Crawl-Laeufe "
          + "(source_crawl_telemetry, status='ok') sind Voraussetzung — " + AKTIVIERUNGSSTUFEN[0].weiterErst)
      ]),
      nachbedingungen: Object.freeze([
        bedWegzustand(s2, AKTIV, [s2.length], "Stufe 2 ist nur teilweise aktiv (Teilausfuehrung)"),
        bedWegzustand(s1, AKTIV, [s1.length], "Stufe 1 wurde durch Stufe 2 veraendert")
      ]),
      weiterMit: null, rollbackDatei: DATEI_S2_RB
    }),
    Object.freeze({
      id: "S1-RB", nummer: 5, art: "rollback", stufe: 1, datei: DATEI_S1_RB,
      titel: "Rollback Stufe 1 · nur die " + s1.length + " Direktfeeds",
      zweck: "Nimmt AUSSCHLIESSLICH Stufe 1 zurueck. Stufe 2 bleibt unveraendert gesperrt; "
        + "Block A und der Paketstatus bleiben bestehen.",
      wegeBeruehrt: Object.freeze(s1),
      zuordnungBeruehrt: Object.freeze([]),
      paketBeruehrt: Object.freeze([]),
      vorbedingungen: Object.freeze([
        bedWegzustand(s2, GESPERRT, [s2.length],
          "Stufe 2 ist aktiv: zuerst " + DATEI_S2_RB + " ausfuehren, oder " + DATEI_RB_ALLE + " fuer alle Stufen")
      ]),
      nachbedingungen: Object.freeze([
        bedWegzustand(s1, GESPERRT, [s1.length], "Stufe 1 ist nicht vollstaendig zurueckgesetzt"),
        bedWegzustand(s2, GESPERRT, [s2.length], "Stufe 2 ist nicht mehr gesperrt")
      ]),
      weiterMit: null, rollbackDatei: null
    }),
    Object.freeze({
      id: "S2-RB", nummer: 6, art: "rollback", stufe: 2, datei: DATEI_S2_RB,
      titel: "Rollback Stufe 2 · nur die " + s2.length + " Google-News-Wege",
      zweck: "Nimmt AUSSCHLIESSLICH Stufe 2 zurueck und laesst Stufe 1 ausdruecklich laufen "
        + "(Rueckfall auf den bewiesenen Zustand nach Stufe 1). Jederzeit ausfuehrbar.",
      wegeBeruehrt: Object.freeze(s2),
      zuordnungBeruehrt: Object.freeze([]),
      paketBeruehrt: Object.freeze([]),
      vorbedingungen: Object.freeze([]),
      nachbedingungen: Object.freeze([
        bedWegzustand(s2, GESPERRT, [s2.length], "Stufe 2 ist nicht vollstaendig zurueckgesetzt")
      ]),
      weiterMit: null, rollbackDatei: null
    }),
    Object.freeze({
      id: "RB-ALLE", nummer: 7, art: "rollback", stufe: null, datei: DATEI_RB_ALLE,
      titel: "Rollback alle Stufen · Block B zurueck (Betriebs-Abbruch)",
      zweck: "Stoppt die Berliner Versorgung vollstaendig: alle Wege des Basispakets zurueck "
        + "auf needs_review/manual, Paket zurueck auf 'prepared'. Block A bleibt bestehen.",
      wegeBeruehrt: Object.freeze(alleBasisWege),
      zuordnungBeruehrt: Object.freeze([]),
      paketBeruehrt: Object.freeze([BASISPAKET_KEY]),
      vorbedingungen: Object.freeze([]),
      nachbedingungen: Object.freeze([
        bedWegzustand(alleBasisWege, GESPERRT, [alleBasisWege.length], "Es ist noch ein Weg des Basispakets aktiv"),
        bedPaketstatus(BASISPAKET_KEY, ["prepared"], "Paketstatus ist nicht zurueck auf 'prepared'")
      ]),
      weiterMit: null, rollbackDatei: null
    }),
    Object.freeze({
      id: "RB-VOLL", nummer: 8, art: "rollback", stufe: null, datei: DATEI_RB_VOLL,
      titel: "Rollback vollstaendig · Block B UND Block A zurueck",
      zweck: "Stellt den gemessenen Ausgangszustand vom 2026-07-26 wieder her — inklusive des "
        + "Befunds A-3. Nur wenn die Umhaengung selbst Schaden angerichtet hat.",
      wegeBeruehrt: Object.freeze(alleBasisWege),
      zuordnungBeruehrt: Object.freeze([BASISPAKET_ID, PARTEIPAKET_ID]),
      paketBeruehrt: Object.freeze([BASISPAKET_KEY]),
      vorbedingungen: Object.freeze([]),
      nachbedingungen: Object.freeze([
        bedWegzustand(alleBasisWege, GESPERRT, [alleBasisWege.length], "Es ist noch ein Weg des Basispakets aktiv"),
        bedPaketstatus(BASISPAKET_KEY, ["prepared"], "Paketstatus ist nicht zurueck auf 'prepared'"),
        bedZuordnung(BASISPAKET_ID, UMHAENGUNG.wege, [3], "Die 3 Wege haengen nicht wieder am Basispaket"),
        bedZuordnung(PARTEIPAKET_ID, UMHAENGUNG.wege, [0], "Das Parteipaket traegt die 3 Wege noch")
      ]),
      weiterMit: null, rollbackDatei: null
    })
  ]);
...
// Selbstpruefung der Staffelung: die beiden Stufen muessen disjunkt sein, zusammen genau das
// Aktivierungsset ergeben, und keine Aktivierungsdatei darf einen Weg der anderen Stufe oder
// einen Partei-/Fraktions-/Personenweg nennen. Ohne diese Pruefung waere die Trennung wieder
// nur eine Absicht.
function staffelungspruefung() {
  const schritte = ausfuehrungsschritte();
  const s1 = stufenWege(1);
  const s2 = stufenWege(2);
  const ueberschneidung = s1.filter((id) => s2.includes(id));
  const s1Schritt = schritte.find((s) => s.id === "S1");
  const s2Schritt = schritte.find((s) => s.id === "S2");
  const parteiInAktivierung = schritte
    .filter((s) => s.art === "aktivierung")
    .filter((s) => s.wegeBeruehrt.some((id) => UMHAENGUNG.wege.includes(id)))
    .map((s) => s.id);
  return {
    ok: ueberschneidung.length === 0
      && s1.concat(s2).slice().sort().join(",") === AKTIVIERUNGSSET.slice().sort().join(",")
      && !s1Schritt.wegeBeruehrt.some((id) => s2.includes(id))
      && !s2Schritt.wegeBeruehrt.some((id) => s1.includes(id))
      && parteiInAktivierung.length === 0,
    ueberschneidung, parteiInAktivierung,
    stufe1: s1, stufe2: s2
  };
}
...
module.exports = {
  ...
  DATEI_STOP, DATEI_A, DATEI_B1, DATEI_S1, DATEI_S2, DATEI_S1_RB, DATEI_S2_RB, DATEI_RB_ALLE, DATEI_RB_VOLL,
  GESPERRT, AKTIV, stufenWege, telemetrieQuellen, pruefeBedingung,
  ausfuehrungsschritte, staffelungspruefung
};
