...
// GATE-VERSION (Arm-Schritt, Kapazitätssprint 2026-08-31): kennzeichnet den Stand der
// Entscheidungsregeln. Jede Parkungs-Belegzeile traegt diese Version (understanding_result
// = 'gate-geparkt@<version>'); die Wiedervorlage prueft geparkte Vorgaenge mit der jeweils
// AKTUELLEN Version erneut (KI-frei). Bei jeder fachlichen Aenderung an Lexika, Tiers oder
// Entscheidungsregeln MUSS diese Version erhoeht werden — sonst sehen bereits geparkte
// Vorgaenge die neue Regel nur ueber die laufende Wiedervorlage-Rotation, nicht bevorzugt.
const GATE_VERSION = "g2026-08-31.1";

// Feature-Guard-Modus. Default 'off' (byte-identisches Altverhalten). 'shadow' = Entscheidung
// berechnen + protokollieren, aber NICHT blockieren. 'on' = echter Vorfilter (Arm-Schritt,
// Kapazitätssprint 2026-08-31): die kanonische Pruefung sitzt in understandOneCluster —
// VOR Restzeitwache, CAS-Reservierung und Budget — und parkt nicht befuerwortete
// ERSTVERSTEHEN fail-geschlossen (erst Belegzeile, dann Zustand; jeder Fehler dabei
// fuehrt zur normalen Verarbeitung, nie zu Arbeitsverlust). Bestehende complete-Vorgaenge
// (Aktualisierungen) und ausdrueckliche Betreiberfreigaben werden NIE geparkt.
...
module.exports = {
  assessDocument, assessCluster, gateDocuments, sourceTier, gateMode, GATE_VERSION,
