...
async function ausfuehren({ c, get, storage, profile, now, beforeReview, beforeSave, generate }) {
  ...
  fordere(lageRead && standHash(lageRead) === standHash(lage));
  ...
  fordere(Array.isArray(rows) && rows.length === 1 && standHash(rows[0]) === standHash(entry));
  const rueck = await B.lese({ userId: c.userId, day: c.tag, profile, storage });
  fordere(rueck && standHash(rueck) === standHash(entry));
