...
async function handleSourceFetch(auftrag, deps) {
  ...
  const bilanz = await deps.crawlAllSources([quelle], {
    ...
    // Im Queuepfad ist die dauerhafte Source-Demand-/Idempotenzdefinition die
    // Deduplizierungswahrheit. Ein prozesslokales 15-Minuten-Ledger koennte einen
    // Retry als `skipped-shared/ok` abschliessen und Arbeit verlieren; deshalb
    // bleibt es hier fail-closed aus. Der Bestands-Cron darf es weiter nutzen.
    sharedLedger: null,
    // Nur fuer vollstaendige Offline-Regressionstests gesetzt. Production hat
    // keinen Adapter und benutzt unveraendert die echten Transportfunktionen.
    requestDeps: {
      // Der echte Queuepfad reicht dieselbe Umgebung und denselben gehaltenen
      // Klassenslot bis an die Transportnaht durch; sonst waere die Lease-Wache
      // im source_fetch trotz belegtem Slot wirkungslos.
      env: deps.env || process.env,
      klassen: deps.klassen || null,
      klassenSlot,
      ...(deps.crawlerRequestDeps || {})
    }
  });
  ...
  if (ergebnis.ok === false) {
    ...
    const fehler = new Error(ergebnis.error || "abruf-fehlgeschlagen");
    // Die Providergrenze wurde VOR jedem Transportaufruf bestimmt. Diese
    // Metadaten duerfen auf dem Weg vom Crawler zum Queue-Kern nicht auf einen
    // neuen nackten Error reduziert werden, sonst wird aus Warten ein Fehlversuch.
    if (ergebnis.anbieterVertagung) fehler.anbieterVertagung = ergebnis.anbieterVertagung;
    throw fehler;
  }
...
async function fuehreAuftragAus({
  ...
  if (!ergebnis && fehlerRoh && fehlerRoh.anbieterVertagung) {
    ...
    ergebnis = {
      ...
      // Unveraendert durchreichen. Die Queue vertagt auf den von der atomaren
      // Reservierung gelieferten fruehesten Zeitpunkt, nicht auf einen Pauschalwert.
      wartenMs: v.wartenMs
    };
    fehler = null;
  }
  ...
  if (ergebnis && ergebnis.zurueckgestellt === true) {
    ...
    if (typeof d.zurueckstellen === "function") {
      const z = await d.zurueckstellen({
        ...
        // Anbietergrenzen liefern den exakten fruehesten Zeitpunkt. Andere
        // Zurueckstellungsarten behalten ihre bewiesenen Pauschalwerte.
        delayMs: Number.isFinite(Number(ergebnis.wartenMs)) && Number(ergebnis.wartenMs) > 0
          ? Math.max(1000, Math.floor(Number(ergebnis.wartenMs)))
          : (ergebnis.langeWarten ? BUDGET_WARTE_MS : VORBEDINGUNG_WARTE_MS),
