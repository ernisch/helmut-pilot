...
async function handleSourceFetch(auftrag, deps) {
  ...
  if (einreihen && gespeichert.length) {
    const kennungen = rohdokumentKennungen(gespeichert, deps);
    for (let i = 0; i < kennungen.length; i += UNDERSTANDING_BUENDEL) {
      ...
      // wiederholt worden — und `saveRawItems` liefert beim naechsten Fenster nur NEUE
      // Items zurueck, die Dokumente waeren also nie verstanden worden.
...
// Die Kennungen, unter denen die eben gespeicherten Dokumente in `raw_documents` LIEGEN.
//
...
async function handleDocumentUnderstanding(auftrag, deps) {
  ...
  //
  // BELEGTER FEHLER (Abschlussreview 2026-08-08): `runUnderstandingShadow` liefert in
  // mehreren Faellen `{ skipped: true, reason }` — darunter `understanding-locked`
  // (ein anderer Lauf haelt die Sperre) und `ai-disabled`. Der Handler meldete darauf
  // `ok:true, verstanden:0` und der Auftrag galt als ERLEDIGT. Weil der Idempotenz-
  // schluessel des Verstehens bewusst KEIN Aktualitaetsfenster traegt, waere derselbe
  // Auftrag nie wieder eingereiht worden: eine voruebergehende Sperrkollision haette die
  // Dokumente DAUERHAFT unverstanden gelassen — gemeldet als Erfolg (CLAUDE.md §4.10).
  //
  // Jetzt wird zurueckgestellt statt abgeschlossen. Das verbraucht keinen Versuch
  // (`helmut_defer_job` nimmt ihn zurueck) und der Auftrag kommt wieder.
  if (ergebnis && ergebnis.skipped === true) {
    return {
      ok: false,
      zurueckgestellt: true,
      grund: `verstehen-uebersprungen: ${(ergebnis && ergebnis.reason) || "unbekannt"}`,
...
async function handleMandateProjection(auftrag, deps) {
  ...
  // Gleiche Lehre wie beim Verstehen (Abschlussreview 2026-08-08): `runMatchingShadow` und
  // `runDecisionShadow` liefern `{ skipped: true, reason }` — u. a. bei `matching-locked`,
  // also einer VORUEBERGEHENDEN Sperrkollision. Ein solcher Lauf wurde als erfolgreiche
  // Projektion verbucht; weil der Idempotenzschluessel der Projektion das Aktualitaetsfenster
  // traegt, haette das Mandat seine Projektion fuer das GANZE Fenster (Standard 24 h)
  // verloren — und der Lauf haette es als Erfolg gemeldet.
  const uebersprungen = [
    matching && matching.skipped ? `matching:${matching.reason || "unbekannt"}` : null,
    decisions && decisions.skipped ? `entscheidungen:${decisions.reason || "unbekannt"}` : null
  ].filter(Boolean);
  if (uebersprungen.length) {
    return { ok: false, zurueckgestellt: true, grund: `projektion-uebersprungen: ${uebersprungen.join(", ")}` };
  }
  return {
    ok: true,
    ...
    for (let index = 0; index < auftraege.length; index += 1) {
      ...
      if (typeof handler !== "function") {
        ...
        try {
          ...
          // halben Lease als Obergrenze ist das strukturell ausgeschlossen. Im verdrahteten
          // Pfad (server.js: leaseMs 300 000, HELMUT_JOB_TIMEOUT_MS 120 000) aendert die
          // Grenze nichts; sie greift nur bei knapp konfigurierter Lease.
          const auftragsBudget = Math.max(1000, Math.min(
            AUFTRAG_MAX_MS,
            verbleibend() - ABSCHLUSS_RESERVE_MS,
      ...
      const abschluss = await d.finish({
        ...
        // KOMMENTAR PRAEZISIERT (Abschlussreview 2026-08-08) — Verhalten UNVERAENDERT.
        // Hier stand „Endgueltige Fehler brauchen keinen Backoff — sie kommen nicht wieder."
        // Das ist so nicht richtig und war beim Lesen irrefuehrend: ueber den Zielstatus
        // entscheidet allein `helmut_finish_job`, und die Funktion setzt einen Auftrag nur
        // dann auf `fehlgeschlagen`, wenn `attempts >= max_attempts`. Ein als endgueltig
        // EINGESTUFTER Fehler vor dieser Grenze kommt also sehr wohl wieder — nur ohne
        // Wartezeit, und damit noch in derselben Worker-Runde.
        // Das ist ABSICHT und bleibt so: ein Auftrag, dessen Fehler sich nicht von selbst
        // erledigt (unbekannter Aufgabentyp, ungueltige Nutzdaten, nicht implementiert),
        // soll seine Versuche schnell aufbrauchen statt tagelang Kapazitaet zu binden.
        // Die Schleife ist durch `max_attempts` hart begrenzt (Nachweis:
        // jobqueue-vertrag-test.js §6.5). Bei erschoepften Versuchen ist der Wert ohnehin
        // wirkungslos — die SQL-Funktion liest `due_at` im Fehlgeschlagen-Zweig nicht.
