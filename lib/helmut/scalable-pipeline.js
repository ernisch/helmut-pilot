...
function istEndgueltig(fehlertext) {
  ...
  return /unbekannter-aufgabentyp|payload-ungueltig|nicht-implementiert|profil-nicht-gefunden|budget-dauerhaft-erschoepft|narrativ-nicht-moeglich/i
...
async function planeArbeit({
  ...
  return {
    ...
    tagesplan: {
      tag: tagesplan.tag,
      mandate: tagesplan.mandate,
      deckel: tagesplan.deckel,
      plaetze: tagesplan.reihenfolge.length,
      reihenfolge: tagesplan.reihenfolge,
      reichtFuerAlleNotwendigen: tagesplan.reichtFuerAlleNotwendigen,
      notwendigOffen: tagesplan.notwendigOffen,
      zuteilung: tagesplan.zuteilung,
      global: fair.globalerTopf({
        deckel: tagesplan.deckel, mandatsBedarf: mandatsIds.length, env
      })
    }
  };
}
...
async function betriebsstatus({ deps = {}, seitMinuten = 1440, env = process.env } = {}) {
  ...
  //
  // Fehlt die Migration 20260809, ist das ein UNBEKANNTER Zustand, kein guter: er wird als
  // solcher gemeldet und macht den Gesamtzustand nicht gruener, als er belegt ist.
  const blockiert = await blockierteFn({ maxWiedervorlagen: WIEDERVORLAGE_MAX })
    .catch((error) => ({ verfuegbar: false, grund: bereinigeFehler(error) }));
  if (blockiert && blockiert.verfuegbar && Number(blockiert.blockiert) > 0) {
    zustand = "kritisch";
    befunde.push(`dauerhaft-blockiert:${Number(blockiert.blockiert)}`);
  } else if (!blockiert || blockiert.verfuegbar === false) {
