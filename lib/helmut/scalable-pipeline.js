...
async function planeArbeit({
  ...
  // Mandatsarbeit bleibt vor geteilter Arbeit, aber alle Profile erhalten
  // dieselbe Einreiheregel. Die zuvor ermittelte Rotation bleibt innerhalb
  // der Mandatsgruppe erhalten; keine getrennte Real-/Testgruppe.
  const roheAuftraege = [...bedarf.auftraege, ...mandatsarbeit.auftraege];
  const profilArbeit = [], geteilteArbeit = [];
  for (const auftrag of roheAuftraege) {
    if (auftrag && auftrag.tenantId) profilArbeit.push(auftrag);
    else geteilteArbeit.push(auftrag);
  }
  const alle = [...profilArbeit, ...geteilteArbeit];
  ...
  // bleiben getrennt: erst alle Mandate, dann geteilte Arbeit.
  const einreihParallelitaet = profile.length >= 100 ? 4 : 1;
  planen: for (const gruppe of [profilArbeit, geteilteArbeit]) {
