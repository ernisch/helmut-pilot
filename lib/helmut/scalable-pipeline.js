...
async function planeArbeit({
  ...
  jetztMs = Date.now(),
  env = process.env,
  // ABSOLUTES Ende der Planungsphase. Die Grenze sitzt bewusst IN der begrenzten
  ...
  // LAUFWEITER PLAN-ZWISCHENSPEICHER (Sprint 02.09.): der relationale Crawlplan
  // ist profilunabhaengig und wurde bisher je Profil neu geladen. Genau EIN
  // Objekt je Planungslauf — kein Modulzustand, keine TTL, keine Wiederverwendung
  // ueber Laeufe hinweg.
  const planCache = {};
  const quellenFuerProfil = deps.quellenFuerProfil
    || ((p) => require("./scheduler").getSourcesForProfile(p, { planCache }));
  ...
  // ── WER ÜBERLEBT EINEN ABGESCHNITTENEN PLANUNGSLAUF? (Sprint 02.09.) ──────
  // BELEGTER BEFUND (adversarialer Review): die Einreiheschleife unten bricht am
  // Planungsbudget ab (`planungsDeadlineMs`). Die Reihenfolge dieser Liste
  // entscheidet damit, WAS bei einem abgeschnittenen Lauf noch eingereiht wird —
  // und die mandatsgebundene Arbeit (Projektion, Briefing, Narrativ) stand
  // GESCHLOSSEN AM ENDE. Bei 5 Mandaten fällt das nicht auf; bei 500 wäre der
  // sichtbare Teil des Produkts der erste, der wegfällt.
  //
  // Neue Reihenfolge, stabil innerhalb jeder Gruppe:
  //   1. mandatsgebundene Arbeit REALER Mandate — sie ist der Produktkern,
  //   2. geteilte Arbeit (kein Mandatsbezug) — sie nützt allen,
  //   3. mandatsgebundene Arbeit synthetischer Profile.
  // Die AUSFÜHRUNG steuert weiterhin `dueAt` und die Anspruchsordnung der
  // Warteschlange; hier geht es allein um die Reihenfolge des EINREIHENS.
  // Bei 0 synthetischen Profilen ist Gruppe 3 leer.
  const roheAuftraege = [...bedarf.auftraege, ...mandatsarbeit.auftraege];
  const realeArbeit = [];
  const geteilteArbeit = [];
  const synthetischeArbeit = [];
  for (const auftrag of roheAuftraege) {
    const tenant = auftrag && auftrag.tenantId ? String(auftrag.tenantId) : "";
    if (!tenant) geteilteArbeit.push(auftrag);
    else if (mandatsklasse.istSynthetischeKennung(tenant)) synthetischeArbeit.push(auftrag);
    else realeArbeit.push(auftrag);
  }
  const alle = [...realeArbeit, ...geteilteArbeit, ...synthetischeArbeit];
  ...
  // Production 08.09.: Bei 500 Profilen passten 1.678 Auftraege seriell nicht
  // in die bestehenden 60 s. Ab 100 Profilen hoechstens vier unabhaengige,
  // atomare Enqueues zugleich; kleine Bestaende behalten ihren seriellen Pfad.
  // Keine Erhoehung von Worker-, KI-, Abruf- oder Zeitbudgets. Die Vorranggruppen
  // bleiben getrennt: erst alle realen, dann geteilte, dann synthetische Arbeit.
  const einreihParallelitaet = profile.length >= 100 ? 4 : 1;
  planen: for (const gruppe of [realeArbeit, geteilteArbeit, synthetischeArbeit]) {
    for (let start = 0; start < gruppe.length; start += einreihParallelitaet) {
      if (hatPlanungsDeadline && uhr() >= planungsEnde) break planen;
      const antworten = await Promise.allSettled(gruppe.slice(start, start + einreihParallelitaet)
        .map(async (auftrag) => {
          // Vor JEDEM wirklichen Schreibbeginn pruefen, auch innerhalb einer Gruppe.
          if (hatPlanungsDeadline && uhr() >= planungsEnde) return { begonnen: false };
          return { begonnen: true, ergebnis: await enqueue(auftrag) };
        }));
      // Auch bei einem Fehler erst alle schon gestarteten RPCs beobachten. Ein
      // frueher Promise.all-Abbruch liesse sonst Hintergrundschreiber zurueck.
      const fehler = antworten.find((a) => a.status === "rejected");
      if (fehler) throw fehler.reason;
      for (const antwort of antworten) {
        if (!antwort.value.begonnen) continue;
        const ergebnis = antwort.value.ergebnis;
        versucht += 1;
...
async function handleDocumentUnderstanding(auftrag, deps, kontext = {}) {
  ...
  if (budget && reservierung && typeof budget.melde === "function") {
    ...
    const kontextOffen = Number(ergebnis?.counts?.["skipped-artikelkontext"]) || 0;
    const kontextZeitOffen = Number(ergebnis?.artikelkontextZeitVertagt) || 0;
    const zeitOhneModell = kontextZeitOffen > 0 ? Number(ergebnis?.counts?.["skipped-zeitbudget"]) || 0 : 0;
    // `processed` zaehlt hier auch besuchte Cluster. Ausschliesslich vor dem
    // Modell vertagte Kontextfaelle duerfen kein ungenutztes Jobbudget verbrauchen.
    // Mischungen mit anderen Ausgaengen geben dagegen nichts pauschal zurueck.
    const nurKontextVertagt = kontextOffen + zeitOhneModell > 0
      && kontextOffen + zeitOhneModell === Number(ergebnis.processed)
      && Object.entries(ergebnis.counts).every(([k, n]) => k === "skipped-artikelkontext"
        || (kontextZeitOffen > 0 && k === "skipped-zeitbudget") || Number(n) === 0);
    const nichtsGetan = Number((ergebnis && ergebnis.processed) || 0) === 0 || nurKontextVertagt;
  ...
  if (ergebnis && (ergebnis.skipped === true || Number(ergebnis.counts?.["skipped-artikelkontext"]) > 0
    || Number(ergebnis.artikelkontextZeitVertagt) > 0)) {
    const grund = Number(ergebnis.counts?.["skipped-artikelkontext"]) > 0
      ? "artikelkontext-offen" : Number(ergebnis.artikelkontextZeitVertagt) > 0
        ? "artikelkontext-zeitbudget" : String(ergebnis.reason || "unbekannt");
...
async function handleBriefingMaterialization(auftrag, deps) {
  ...
  const materialisiere = deps.materialisiereBriefing || require("./briefing-speicher").materialisiere;
  const gespeichert = await materialisiere({ profile: profil, userId: mandatsId, briefing });
  if (gespeichert?.ok !== true || gespeichert.gespeichert !== true)
    throw new Error("briefing-speicherung-nicht-bestaetigt");
  return {
    ok: true,
    gespeichert: true,
    briefingId: gespeichert.id,
    qualitaetBestanden: gespeichert.qualitaetBestanden === true,
...
async function handleTenantNarrative(auftrag, deps) {
  ...
  if (grund === "no-vorgaenge" || grund === "no-current-sources") {
    ...
    return { ok: true, veroeffentlicht: false, grund };
  }
...
function workerDeps(overrides = {}) {
  ...
  return {
    ...
    eagerUnderstanding: lazy("./artikelkontext-lauf", "runUnderstandingShadow"),
...
async function betriebsstatus({ deps = {}, seitMinuten = 1440, env = process.env } = {}) {
  ...
  // `jobQueueZurueckgestellteGruende` liest hoechstens 1.000 Zeilen. Erreicht die Antwort
  // dieses Limit, ist sie eine UNGEORDNETE STICHPROBE — nicht die Verteilung des gesamten
  // Rueckstands. Der alte Code teilte bis zu 1.000 gelesene Gruende durch `wartend` und
  // traf daraus eine globale Ursachenentscheidung. Bei groesserem Rueckstand war der Wert
  // weder ein Stichprobenanteil noch ein Gesamtanteil. Nur eine nachweislich vollstaendige
  // Lesung darf deshalb die Zustandsklasse bestimmen; die Stichprobe bleibt separat sichtbar.
  const gruendeVollstaendig = gruendeVerfuegbar && (!gruendeRoh || gruendeRoh.vollstaendig !== false);
  const gruendeGelesen = gruendeVerfuegbar
    ? (Number.isFinite(Number(gruendeRoh.gelesen)) ? Number(gruendeRoh.gelesen) : (gruendeRoh.texte || []).length)
    : null;
  const anteilBlockierend = gruendeVollstaendig
    ? (wartend > 0 ? blockierendeGruende / wartend : 0)
    : null;
  // Auch innerhalb der Stichprobe ist der Nenner die Zahl ALLER gelesenen Zeilen.
  // `texte` enthaelt nur nicht-leere Werte; ein leerer technischer Fehlerwert darf den Anteil
  // blockierender Gruende nicht kuenstlich erhoehen.
  const anteilBlockierendStichprobe = gruendeVerfuegbar
    ? (gruendeGelesen > 0 ? blockierendeGruende / gruendeGelesen : 0)
    : null;
  const blockierendeGruendeDominieren = gruendeVollstaendig && anteilBlockierend >= 0.5;
  ...
  if (!flagAn) {
    ...
    } else if (!gruendeVollstaendig && wartend > 0) {
      befunde.push(`zurueckstellgruende-stichprobe:${gruendeGelesen}-von-${wartend}`);
    }
    ...
    if (dauerhaftBlockiert != null && dauerhaftBlockiert > 0) {
      ...
      if (blockierendeGruendeDominieren) {
        zustandsklasse = ZUSTANDSKLASSEN.ABHAENGIGKEIT;                      // Klasse 8
      ...
      zustandsklasse = blockierendeGruendeDominieren
  ...
  return {
    ...
    zurueckgestellt: gruendeVerfuegbar
      ? {
        verfuegbar: true,
        nachGrundklasse,
        // Gesamtanteil nur bei vollstaendiger Lesung; `null` ist eine Messgrenze, nicht 0.
        anteilBlockierend: anteilBlockierend == null ? null : Math.round(anteilBlockierend * 100) / 100,
        // Die Verteilung INNERHALB der Stichprobe ist weiterhin nuetzliche Diagnose, aber
        // der Feldname verhindert, dass sie als Anteil des ganzen Rueckstands gelesen wird.
        anteilBlockierendStichprobe: anteilBlockierendStichprobe == null
          ? null : Math.round(anteilBlockierendStichprobe * 100) / 100,
        messumfang: {
          gelesen: gruendeGelesen,
          limit: Number.isFinite(Number(gruendeRoh.limit)) ? Number(gruendeRoh.limit) : null,
          rueckstand: wartend,
          vollstaendig: gruendeVollstaendig
        }
      }
