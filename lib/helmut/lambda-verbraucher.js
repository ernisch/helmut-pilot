...
function erstelleRelayAusloeser(env, deps) {
  if (deps && typeof deps.loeseRelayAus === "function") return deps.loeseRelayAus;
  const funktion = String((env && env.HELMUT_RELAY_FUNKTION) || "").trim();
  if (!funktion) return null;                    // lokal/Route: der Verbraucher relayt selbst
  ...
  try {
    ...
    return null;                                  // SDK fehlt -> Zeitgeber traegt allein
  }
}
...
async function handler(event, kontext = {}, deps = {}) {
  ...
  for (const record of records) {
    ...
    let ergebnis;
    try {
      const umgebung = deps.env || process.env;
      const relayAusloeser = erstelleRelayAusloeser(umgebung, deps);
      ergebnis = await (deps.verarbeite || queueVerbraucher.verarbeiteWecksignal)({
        ...
        deps: {
          ...
          relayDeps: {
            ...
            ...(relayAusloeser ? { loeseAus: relayAusloeser } : {})
          }
        }
      });
    } catch (fehler) {
