...
function baueEingabe(kos, quellen, jetzt = new Date(), env = process.env) {
  ...
  for (const ko of kos || []) {
    ...
    const belege = (quellen[ko.vorgang_id] || []).map((d) => {
      const datum = zeitvertrag.publikationsdatum(d.published_at);
      const metatext = require("./quellen-auszug").institutionellerMetatext(d.summary);
      const auszug = metatext ? "" : text(d.summary, 1400);
      ...
      if ((!metatext && text(d.summary, Infinity) && !auszug)
        || (text(d.source_name, Infinity) && !quelle)) return null;
      return {
        ...
        auszug, quelle, url: artikelUrl(d.canonical_url) || artikelUrl(d.url),
        veroeffentlichtAm: datum?.iso || null,
      ...
      if (!d || !d.url || !d.titel || !d.veroeffentlichtAm) return false;
...
function cacheGueltig(payload, koSetHash, quellenHash, vorgaenge) {
  return Boolean(payload && payload.quellenVersion === VERSION && payload.koSetHash === koSetHash
    ...
    && require("./lage-textqualitaet").sachdetailsGebunden(payload.paragraphs, vorgaenge));
}
