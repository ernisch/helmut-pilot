...
// gesetzt sein: ein inaktives Profil erzeugt keinen Auftrag, keine Last, keinen
// Modellaufruf und keine Außenkommunikation (verhaltensbelegt:
// `scripts/testkohorte-provisionierung-inaktiv-test.js`). Sie müssen aber
...
function stufenSchritte(gruppe, vorherigeKontrolle) {
  ...
  return [
    Object.freeze({
      ...
      vorbedingungen: Object.freeze([V.GRUNDLINIE, V.SICHERUNG, V.FENSTER, ...vorstufe]),
      liefert: vb.angelegt,
      ...
      zweck: `Legt ausschließlich die ${gruppe.groesse} Kennungen der Stufe ${S} INAKTIV an — `
        + "inaktive Profile erzeugen keinen Auftrag, keine Last, keinen Modellaufruf und keine "
        + "Außenkommunikation (verhaltensbelegt). Das CLI verlangt `--stufe=` zwingend und fällt "
        ...
        + "DIESER Stufe). Die acht Betreiberwerte sind hier KEINE Vorbedingung."
    }),
    Object.freeze({
      ...
      vorbedingungen: Object.freeze([vb.angelegt]),
    ...
    Object.freeze({
      ...
      vorbedingungen: Object.freeze([vb.aktiv]),
      liefert: vb.zyklus,
    ...
    Object.freeze({
      ...
      vorbedingungen: Object.freeze([vb.zyklus]),
  ...
  const roh = [
    ...
    Object.freeze({
      ...
      zweck: `Der eingefrorene Vertrag, gegen den alles Weitere gemessen wird: `
        ...
        + "Vor JEDER Provisionierung aktuell erheben — eine ältere Grundlinie ist kein Vertrag."
    }),
    Object.freeze({
    ...
    Object.freeze({
      ...
      art: ART_LESEND,
      befehl: `${CLI_ABLAUF} werte (Sollwerte)  →  Betreibernachweis der gesetzten Vercel-Env`
        + "  →  /api/admin/overview (rein lesend, zeigt HELMUT_MAX_LLM_CALLS_PER_DAY gesetzt/Wert)"
        + `  →  ${LOKAL} ${CLI_ZYKLUS} --stufe=a --konfiguration=<json> … (Startbereitschaft, Hürden `
        + "„Tagesdeckel und beide Reserven passen in der LAUFENDEN Umgebung“, "
        + "„Vorrangreserve der realen Mandate ist LAUFZEITWIRKSAM“, „Kommunikationsriegel scharf“)",
      vorbedingungen: Object.freeze([V.WERTE, V.RIEGEL]),
      liefert: V.WERTE_GEPRUEFT,
      freigabe: null,
      abbruchkontrolle: "A10, A14",
      zweck: "Gesetzt heißt nicht wirksam. Vercel-Env ist aus keiner Sitzung lesbar; wirksam "
        + "belegt sind nur Werte, die ein Laufzeitpfad tatsächlich liest: der Tagesdeckel und die "
        + "Vorrangreserve über die Startbereitschaftshürden, der Riegel über seinen Test. RPM/TPM "
        + "liest KEIN Ausführungspfad (§23.4) — sie bleiben Betreiberangabe. Ohne diesen Beleg "
        + "darf kein synthetisches Profil aktiviert werden: mit Vorrangreserve 0 ist der "
        + "Verdrängungsschutz der fünf realen Mandate NICHT wirksam (§25.2)."
    }),
...
function vorbereitung() {
  return Object.freeze({
    ...
    zeitpunkt: Object.freeze({
      ...
      begruendung: "Ein inaktives Profil erzeugt keinen Auftrag, keine Last, keinen Modellaufruf "
        + "und keine Außenkommunikation. Last entsteht erst mit der Aktivierung — und davor "
