...
const V = Object.freeze({
  ...
  // die Kennung war eine offene BETREIBERENTSCHEIDUNG. Der Betreiber entschied
  // noch am selben Tag auf Variante A (Sprintauftrag 03.09.); umgesetzt wurde: die Bundestagsprofile der Kohorte tragen die amtlichen
  ...
  // BELEGBAR — es gibt einen rein lesenden Lauf, der sie liefert, statt einer
  // Entscheidung, die nur behauptet werden konnte. Genauer, damit hier nichts
  // Stärkeres steht, als der Plan trägt: dieser Plan FUEHRT NICHTS AUS und
  // PRUEFT NICHTS NACH (siehe `hinweis` am Ende) — `belegt` kommt wie jede
  // andere Kennung aus der Eingabe `belegt`, ist also eine Betreiberangabe.
  // Der Unterschied liegt nicht im Mechanismus, sondern darin, dass die Angabe
  // jetzt durch einen Lauf einloesbar ist. Sie bleibt Vorbedingung jeder
  // Anlage: kein Anlegen ohne frischen Reifebeleg.
...
function schritte() {
  ...
  const roh = [
    ...
    Object.freeze({
      ...
      freigabe: null,
      abbruchkontrolle: null,
      zweck: "Sicherheitsrahmen §34.7 (Variante A, vom Betreiber am 03.09. entschieden und noch am "
        + "selben Tag umgesetzt): die Bundestagsprofile der "
    ...
    Object.freeze({
      ...
      abbruchkontrolle: null,
      immerErlaubt: true,
      zweck: "Kennt KEINEN Löschpfad (im Quelltext geprüft) und wirkt ausschließlich auf die "
...
function ablaufplan({ belegt = [] } = {}) {
  ...
  return Object.freeze({
    ...
    // EHRLICH ZU `offen`: das ist ein festgeschriebener Wert, kein Messergebnis.
    // Er beschreibt den dokumentierten Stand von §34.7, und ein Test, der nur
    // ihn liest, prueft nichts (Reviewbefund 03.09.). Die inhaltliche Deckung
    // leistet `funktionstest-ablaufplan-test.js` A4c: dort wird die ECHTE
    // Reifepruefung ueber die echte Kohorte gerufen — nimmt sie die 434
    // Bundestagsprofile nicht mehr an, oder weist sie einen schlechten
    // Ausschuss nicht mehr ab, wird dieser Eintrag dort rot.
...
function vorbereitung() {
  return Object.freeze({
    ...
    vorrangreserve: Object.freeze({
      ...
      // Historischer Variablenname; inzwischen Reserve fuer ALLE Mandate.
      wirkung: "zieht die Reserve fuer nicht zuordenbare Aufrufe und GETEILTE Arbeit "
        + "(Verstehen, Backfills) vom Tagesmaximum ab. AUSGENOMMEN ist die "
        + "mandatsgebundene Arbeit ALLER Profile. Dem geteilten/priorisierten Pfad "
        + "bleibt mindestens die Verstehens-Reserve; beide Reserven liegen IM Deckel.",
      warnung: "Der Wert muss KLEINER sein als `HELMUT_MAX_LLM_CALLS_PER_DAY` minus "
        + "`HELMUT_LLM_RESERVE_UNDERSTANDING`. Vor dem Start die tatsaechliche "
        + "Laufzeitkonfiguration pruefen; daraus folgt keine Freigabe fuer Budgetaenderungen."
    }),
