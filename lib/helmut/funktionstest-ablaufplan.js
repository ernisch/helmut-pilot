  ...
  return Object.freeze([
    ...
    Object.freeze({
      ...
      befehl: `${CLI} plan --grundlinie=grundlinie.json --bestand=bestand.json`
        + "  →  Ausführung: node scripts/testkohorte-vorwaerts.js provisionierung"
        + " --start=<HH:MM> --dauer=<min> --scharf",
      vorbedingungen: Object.freeze([V.GRUNDLINIE, V.SICHERUNG, V.FENSTER]),
    ...
    ...[
      ...
      Object.freeze({
        ...
        befehl: `${CLI} aktivierung --gruppe=${g.kennung} --grundlinie=grundlinie.json `
          ...
          + `  →  Ausführung: node scripts/testkohorte-vorwaerts.js aktivierung --gruppe=${g.kennung}`
          + " --start=<HH:MM> --dauer=<min> --vorstufen-vollstaendig=ja --scharf",
    ...
    Object.freeze({
      ...
      art: ART_PRODUCTION,
      // KORRIGIERT 02.09. (zweiter Reviewbefund): Hier stand „bestehender Motor
      // (Cron-Antrieb) — kein eigener Befehl". Das war eine STILLE ANNAHME und
      // sie ist falsch: der Pipeline-Cron läuft 16:00 UTC, das sichere Fenster
      // endet 15:59. Im Fenster läuft der Motor NIE — es ist ja gerade so
      // gewählt, dass kein Bestandscron hineinfällt. Der Zyklus hatte damit
      // überhaupt keinen Auslöser.
      befehl: "node scripts/funktionstest-500-zyklus.js --start=<HH:MM> --dauer=<min>"
        + "  →  Ausführung: dieselbe Zeile mit --scharf",
      ...
      freigabe: Object.freeze({
        ...
        flag: kohorte.EXECUTE_FLAG,
        variable: kohorte.CONFIRM_VARIABLE,
        wort: kohorte.FREIGABEWORTE.fachzyklus
      }),
      abbruchkontrolle: "A01–A15",
      zweck: "Die einzige Ebene, die weder durch Offline-Tests noch durch diesen Sprint "
        + "ersetzbar ist (Beweisstand §2, Ebene 3). Der Startweg ruft ausschließlich die "
        + "bestehende Route /api/cron/pipeline in begrenzten Scheiben auf — kein Cron wird "
        + "verändert, kein Motor angefasst. OFFENER BLOCKER: die Briefingmaterialisierung "
        + "ist erst ab 18:00 UTC fällig und entsteht im sicheren Fenster GAR NICHT."
    }),
    Object.freeze({
...
function ablaufplan({ belegt = [] } = {}) {
  ...
  return Object.freeze({
    ...
    // ist keine Lücke. Was sich am 02.09. geändert hat: für jeden
    // Production-Schritt existiert jetzt ein WERKZEUG, das ihn tatsächlich
    // ausführen kann. Vorher gab es nur den Rückweg; Provisionierung,
    // Aktivierung und Fachzyklus waren beschrieben, aber unausführbar.
    ausfuehrbar: false,
    // EHRLICHE AUSKUNFT: welcher Schritt hat einen Ausführer, und welcher nicht?
    // Ein Schritt ohne Werkzeug ist ein Schritt, den niemand gehen kann.
    ausfuehrer: Object.freeze({
      provisionierung: "scripts/testkohorte-vorwaerts.js provisionierung",
      "aktivierung-a": "scripts/testkohorte-vorwaerts.js aktivierung --gruppe=a",
      "aktivierung-b": "scripts/testkohorte-vorwaerts.js aktivierung --gruppe=b",
      "aktivierung-c": "scripts/testkohorte-vorwaerts.js aktivierung --gruppe=c",
      fachzyklus: "scripts/funktionstest-500-zyklus.js",
      deaktivierung: "scripts/testkohorte-rueckbau.js",
      "scheduler-spur": "scripts/testkohorte-rueckbau.js --spur",
      "migration-llm-usage": null,
      "flag-llm-usage": null
    }),
    ohneAusfuehrer: Object.freeze(["migration-llm-usage", "flag-llm-usage"]),
