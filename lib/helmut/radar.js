...
function profileRadarTerms(profile = {}) {
  ...
  const fullName = slug(fullNameRaw);
  const tokens = fullName.split(" ").filter(Boolean);
  // Nachname als distinktiver Einzelbegriff (ab 4 Zeichen), z. B. "özdemir", "ince".
...
// Voller Name als eigenständige Wortfolge im (gesluggten) Text? Vermeidet
// Teilwort-Treffer wie "ince" in "provinces".
function fullNameInText(text, fullName) {
  if (!text || !fullName) return false;
  return new RegExp(`(^| )${fullName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}( |$)`).test(text);
}
...
function mentionReason(ko, terms) {
  if (terms.fullName) {
    // 1. Strukturierte, AI-extrahierte Erwähnungsfelder (deckt auch "Özdemir" allein).
    const people = cleanList([].concat(ko.mentioned_people || [], ko.mentioned_mps || [])).map(slug);
    if (people.some((p) => p === terms.fullName || p.includes(terms.fullName) || terms.fullName.includes(p))) {
      return "person";
    }
    // 2. Fallback: der volle Name steht im öffentlichen KO-Analysetext, wurde aber
    //    NICHT als strukturierte Erwähnung extrahiert -> Eigenerwähnung trotzdem
    //    zuverlässig erkennen (deterministisch, kein KI-Call, kein Volltext-Scraping).
    const text = slug([ko.display_title, ko.headline, ko.display_summary, ko.was_ist_passiert].filter(Boolean).join(" "));
    if (fullNameInText(text, terms.fullName)) return "person";
    // 3. Distinktiver Nachname als eigenständiges Wort in Erwähnungsfeldern ODER Text.
    if (terms.lastName && (people.some((p) => fullNameInText(p, terms.lastName)) || fullNameInText(text, terms.lastName))) {
      return "person";
    }
  }
...
function buildRadarSignals(profile = {}, knowledgeObjects = [], opts = {}) {
  ...
  const capped = signals.slice(0, Math.max(0, cap));

  const buckets = { risk: [], demand: [], chance: [], warning: [], mention: [] };
  for (const s of capped) buckets[s.signalType].push(s);
...
async function buildRadarForUser(input = {}, overrides = {}) {
  ...
  try {
    ...
    const kos = (await deps.listKnowledgeObjects({ limit: input.limit || 200 })) || [];
