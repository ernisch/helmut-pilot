...
// und türkisches İ/ı. "Deniz İnal", "Deniz Inal", "İnal", "Inal" -> alle "deniz inal"/"inal".
...
function profileRadarTerms(profile = {}) {
  ...
  // Nachname als distinktiver Einzelbegriff (ab 4 Zeichen), z. B. "müller"->"muller", "inal".
...
// Teilwort-Treffer wie "inal" in "final" oder "ince" in "provinces".
...
function mentionReason(ko, terms) {
  if (terms.fullName) {
    // 1. Strukturierte, AI-extrahierte Erwähnungsfelder (deckt auch "İnal" allein).
...
function buildRadarSignals(profile = {}, knowledgeObjects = [], opts = {}) {
  ...
  for (const ko of understood) {
    ...
    // Hinweis (radar.md §5): eine Spalte `best_source_type` existiert NICHT — der
    // frühere Zugriff `ko.best_source_type` war immer undefined (toter Feldzugriff).
    // System B kennt den Quellentyp nicht auf KO-Ebene; der Guard läuft ohne ihn.
    const docs = ko.best_source_url ? [{ url: ko.best_source_url, source_trust: ko.source_trust }] : [];
...
async function buildRadarForUser(input = {}, overrides = {}) {
  ...
  try {
    ...
    // STÖRUNGSWAHRHEIT (P1-8): wie Lage/Briefing das __storeError-Sentinel nutzen —
    // ein harter DB-/Netzfehler lieferte im Radar bisher still [] und war von einem
    // ruhigen Tag ("keine Erwähnung") NICHT unterscheidbar (Audit R Radar). Jetzt
    // wird der Store-Ausfall als eigener Grund 'store-error' nach oben gereicht, den
    // der Client als STÖRUNG (nicht als Datenlücke/ruhig) rendert.
    const kos = await deps.listKnowledgeObjects({ limit: scanLimit, _signalError: true });
    if (kos && kos.__storeError) {
      return { skipped: true, reason: "store-error", error: true, buckets: emptyBuckets(), signals: [] };
    }
