...
// DIE PHASENFENSTER DER MANDATSGEBUNDENEN ARBEIT — eine einzige Quelle.
//
// Herausgezogen am 02.09. (zweiter Reviewbefund): Diese Zahlen entscheiden, ob
// eine Arbeitsklasse zu einem gegebenen Zeitpunkt ueberhaupt FAELLIG ist. Der
// Sicherheitsrahmen des 500er-Funktionstests muss genau dagegen rechnen koennen —
// stuenden die Zahlen dort ein zweites Mal, koennten die beiden auseinanderlaufen
// und das Tor wuerde etwas anderes pruefen als der Motor tut.
//
// Bei der Standardbreite von 24 h (HELMUT_DEMAND_TENANT_MAX_AGE_H) bedeutet das
// in UTC: Projektion 12:00-18:00, Briefingmaterialisierung 18:00-21:36.
const MANDATSPHASEN = Object.freeze([
  Object.freeze(["mandate_projection", 200, 0.50, 0.75]),
  Object.freeze(["briefing_materialization", 250, 0.75, 0.90])
]);
      ...
      if (persoenlich) {
        ...
        for (const weg of wege) {
          ...
          auftraege.set(schluessel, {
            ...
            tenantId: mandatsId,
            // Archiv ist Hintergrundarbeit, Aktuell ist wichtig. Der +1-Aufschlag
            // fuer synthetische Mandate stellt reale strukturell davor.
            priority: mandatsPrioritaet(archiv ? 300 : 60, mandatsId),
            maxAttempts: 5,
...
// wandert taeglich.
// ── VORRANG DER REALEN MANDATE IN DER ANSPRUCHSORDNUNG (Sprint 02.09.) ──────
// Die Warteschlange zieht `order by priority asc, due_at asc, created_at asc`
// (Migration 20260808_scalable_job_queue.sql:323). `priority` ist damit die
// STÄRKSTE Ordnung — stärker als jede Fälligkeitsstreuung. Bei 5 realen und 495
// synthetischen Mandaten tragen heute ALLE mandatsgebundenen Aufträge dieselbe
// Zahl; wer zuerst drankommt, entscheidet dann allein `due_at`.
//
// Die Rotation (llm-budget-fair) stellt reale Mandate seit diesem Sprint nach
// vorn und gibt ihnen damit bereits die frühesten Fälligkeiten. Diese Funktion
// ist die zweite, unabhängige Lage: ein mandatsgebundener Auftrag eines
// SYNTHETISCHEN Profils bekommt +1 auf die Prioritätszahl und steht damit
// STRUKTURELL hinter dem gleichartigen Auftrag eines realen Mandats — auch dann,
// wenn seine Fälligkeit früher wäre.
//
// STRUKTURELL WIRKUNGSLOS, SOLANGE ES KEINE KOHORTE GIBT: ohne synthetische
// Mandate ist der Aufschlag nie ungleich 0, die Prioritätszahlen sind
// byte-identisch zu vorher. Geteilte Aufträge (tenantId === null) bleiben
// unberührt — sie gehören keinem Mandat und nützen allen.
function mandatsPrioritaet(basis, mandatsId) {
  // KEINE KOERZIERUNG (CLAUDE.md §4.4): `Number(null)` und `Number("")` sind 0.
  // Eine fehlende Prioritaet haette so eine gueltige 1 ergeben und den Auftrag
  // ganz nach VORN gestellt — das Gegenteil der Absicht. Nur eine echte Zahl
  // zaehlt; alles andere bleibt unveraendert und faellt dem Aufrufer auf.
  if (typeof basis !== "number" || !Number.isFinite(basis)) return basis;
  return mandatsklasse.klassifiziereMandat(mandatsId) === mandatsklasse.KLASSE_SYNTHETISCH ? basis + 1 : basis;
}
...
function planeMandatsarbeit({ profile = [], jetztMs = Date.now(), env = process.env, rotation = null,
  ...
  //
  // BELEGTER FEHLER (Abschlussreview PR #236, 2026-08-09 — Befund R1, hier behoben).
  // Die Faelligkeit wurde ueber das GANZE Phasenfenster gestreut: 33,3 % - 24 % = 134,4
  // Minuten. Abgearbeitet wird sie aber ausschliesslich vom Cron `lage-briefing` — EIN
  // Slot mit 230 s Budget (`server.js`). Der Worker beendet einen Slot sofort, wenn
  // nichts FAELLIG ist (`scalable-pipeline.arbeite`: `if (!auftraege.length) break`).
  // Mit dem echten Planer nachgemessen (scripts/narrativ-slotvertrag-test.js):
  //   5 Mandate -> 1 Auftrag im Slot faellig · 200 Mandate -> 5 · 250 Mandate -> 7.
  // Der Slot lief also nach wenigen Sekunden leer, >97 % seines Budgets blieben
  // ungenutzt, und die uebrigen Narrative wurden erst im 16:00-Slot faellig — die
  // Morgenlage haette am Nachmittag gestanden, bei den heutigen 5 Mandaten genauso wie
  // bei 200. Die Bestandssimulation konnte das nicht sehen: sie nimmt einen Worker alle
  // 2 Minuten an (`scripts/fixtures/skalierung-lauf.js`), Production hat Cron-Slots.
  //
  // Ein Streubereich darf nie groesser sein als das Fenster, in dem er bedient wird.
  // Deshalb: die Streuung des Narrativs ist auf `NARRATIV_STREU_MS` begrenzt. Der
  // Phasenbeginn (24 % = 05:45 UTC) und die Obergrenze 33,3 % bleiben unveraendert —
  // die Faelligkeit liegt weiterhin im Morgenkorridor, nur eben ganz an dessen Anfang.
  // Die REIHENFOLGE bleibt die des Rotationsrangs (Befund O1): 1 ms Abstand je Rang
  // haelt die Rotation aufloesbar, ohne den Slot zu verlassen.
  const NARRATIV_STREU_MS = 1000;
  const NARRATIV_PHASE = ["tenant_narrative", 240, 0.24, 1 / 3, NARRATIV_STREU_MS];
  for (const profil of Array.isArray(profile) ? profile : []) {
    const mandatsId = String((profil && profil.id) || "").trim();
    ...
    for (const [typ, prio, ab, bis, streuMs] of [
      ...(narrativAktiv ? [NARRATIV_PHASE] : []),
      ...MANDATSPHASEN
    ]) {
      const abMs = Math.floor(breiteMs * ab);
      // Der Streubereich ist standardmaessig das ganze Phasenfenster. Ein Typ darf ihn
      // ausdruecklich VERKLEINERN (nie vergroessern), wenn er von einem einzelnen kurzen
      // Slot bedient wird — siehe NARRATIV_STREU_MS oben.
      const phasenBreite = Math.max(1, Math.floor(breiteMs * (bis - ab)));
      const spanne = streuMs == null ? phasenBreite : Math.max(1, Math.min(Math.floor(streuMs), phasenBreite));
      ...
      auftraege.push({
        ...
        freshnessWindow: fenster,
        tenantId: mandatsId,
        priority: mandatsPrioritaet(prio, mandatsId),
        maxAttempts: 5,
