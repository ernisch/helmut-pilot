...
async function kompiliereQuellenbedarf({
  ...
  for (const auftrag of liste) {
    ...
    const versatzMs = (streuwert(auftrag._streuSchluessel) % Math.max(1, Math.floor(breiteMs * 0.9)));
...
function planeMandatsarbeit({ profile = [], jetztMs = Date.now(), env = process.env } = {}) {
  const konfig = fensterKonfig(env);
  ...
  for (const profil of Array.isArray(profile) ? profile : []) {
    ...
    for (const [typ, prio, ab, bis] of [
      ...
      const versatz = abMs + (streuwert(`${typ}|${mandatsId}`) % spanne);
      auftraege.push({
        jobType: typ,
