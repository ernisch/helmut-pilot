...
function buildEvidenceRelevance(item, profile) {
  const text = `${item.title} ${item.content}`.toLowerCase();
  if (item.sourceId === "source-person") return `Direktes Signal zu deiner Person mit Bezug zu deinem Ausschuss ${profile.committee || profile.committees.join(", ")}.`;
...
function buildDecisionDetails(signal, rawItem, score, profile) {
  ...
  const committeeLabel = profile.committee || (profile.committees || [])[0] || "Fachausschuss";
  ...
  const isCommittee = signal.sourceType === "committee" || (profile.committee && text.includes(String(profile.committee).toLowerCase()));
  ...
  if (isAttack) {
    const attackTopicLabel = signal.topic && signal.topic !== "Nicht mandatsrelevant" ? signal.topic : "deine Position";
    return {
      whyNow: `Der Frame entsteht jetzt. In den nächsten Stunden entscheidet sich, ob du zu ${attackTopicLabel} fachlich sprichst oder auf einen Vorwurf reagieren musst.`,
      estimatedTimeMinutes: 15,
      politicalBenefit: "Hoch",
      targetGroup: "Presse, Social Media, fachlich interessierte Öffentlichkeit",
      recommendedChannel: "Kurzstatement und Social Media",
      riskLevel: "Hoch",
      recommendedPreparation: "Drei Kernargumente gegen den Angriffsframe vorbereiten und den Vorwurf nicht wiederholen."
    };
  }
  ...
  if (isParty) {
    const partyTopicLabel = (profile.focusTopics || [])[0] || "deinem Fachgebiet";
    return {
      whyNow: `Deine Fraktion sucht heute eine klare Stimme zu ${partyTopicLabel}. Wenn du früh lieferst, wird deine Linie anschlussfähig.`,
      estimatedTimeMinutes: 10,
      politicalBenefit: "Mittel",
      targetGroup: "Fraktion, Parteiumfeld und fachlich interessierte Community",
...
function buildContext(signal, rawItem, profile) {
  const text = `${rawItem.title} ${rawItem.content}`.toLowerCase();
  const committeeLabel = profile.committee || (profile.committees || [])[0] || "Fachausschuss";
  const focusTopicList = (profile.focusTopics || []).slice(0, 4).join(", ") || "die Schwerpunktthemen deines Mandats";
  const committeeLine = `Das betrifft dich direkt: Es gehört in deinen Ausschuss ${committeeLabel}, weil dort ${focusTopicList} politisch entschieden werden.`;
  if (signal.sourceType === "person" || signal.entities.includes(profile.fullName)) {
    const personTopicLabel = signal.topic && signal.topic !== "Nicht mandatsrelevant" ? signal.topic : "ein Thema aus deinem Mandat";
    return context(`Das betrifft dich direkt: Du wirst persönlich zu ${personTopicLabel} genannt, und das liegt mitten in deinem Ausschuss ${committeeLabel}. Du kannst daraus eine klare Position gegenüber der Bundesregierung machen.`, `Lege kurz öffentlich nach: Miss die Bundesregierung an ${personTopicLabel} und gib eine Social-Media-Formulierung frei.`, profile.publicPositions?.[0] || `${personTopicLabel} entscheidet sich nicht in Ankündigungen, sondern in der Umsetzung. Die Bundesregierung muss liefern, nicht vertrösten.`, "Ohne deine Einordnung bleibt die Nachfrage bei einer Einzeläußerung und verliert den Ausschussbezug.", `Du kannst dein Profil als Stimme für ${(profile.focusTopics || [])[0] || "dein Fachgebiet"} schärfen.`, `Wenn du nicht reagierst, besetzen andere ${personTopicLabel}, obwohl es direkt in deinen fachpolitischen Raum fällt.`);
  }
  if (text.includes("afd") || text.includes("persönlich") || text.includes("leistungsverweigerung")) return context(`Das trifft deine politische Position und damit ein Kernfeld deines Ausschusses ${committeeLabel}. Der Angriff versucht, deine fachliche Linie in einen persönlichen Frame zu ziehen.`, "Reagiere sofort, aber wiederhole den Vorwurf nicht. Veröffentliche eine klare Gegenformulierung, die sachlich bei der Sache bleibt.", "Politik darf Menschen nicht unter Generalverdacht stellen. Wer Probleme wirklich lösen will, braucht Fakten, Beratung und Respekt statt populistischer Schuldzuweisungen.", "Eine falsche Antwort verstärkt den Angriff; Schweigen lässt den Frame an deiner Position kleben.", `Mit einer ruhigen Antwort zeigst du fachliche Autorität im Ausschuss ${committeeLabel} und grenzt dich klar gegen Hetze ab.`, "Wenn du nicht reagierst, kann der Frame spätere Pressefragen vorprägen.");
  if (text.includes("bürgergeld") || text.includes("sanktion")) return context(`Das betrifft dich, weil Bürgergeld direkt in deinen Ausschuss ${committeeLabel} gehört. Die Bundesregierung plant Änderungen, auf die du politisch reagieren solltest.`, "Bereite eine Gegenlinie vor: Steig nicht in Sanktionssprache ein, sondern stell Beratung, gute Arbeit und Armutsvermeidung in den Mittelpunkt.", "Wer Bürgergeld nur als Misstrauensdebatte führt, macht Menschen kleiner, aber löst keine Armut.", "Die Debatte kann dich in eine Verteidigungsecke ziehen, wenn du nur auf Vorwürfe antwortest.", "Du kannst die Kernfrage stellen: Welche Pläne der Bundesregierung helfen Menschen wirklich in gute Arbeit?", "Wenn du nicht reagierst, dominiert die Sanktionssprache den Tag.");
...
function selectThemeOfDay(items) {
  // Rangfolge: priority zuerst (identisch zur Listensortierung), themeRank nur als
  // Tie-Breaker unter nahezu gleichrangigen Themen. So kann "Thema des Tages" nie
  // einen Vorgang zeigen, der in der Liste niedriger steht als ein anderer.
  const theme = items
    .filter((item) => item.decision !== "Ignorieren")
    .slice()
    .sort((a, b) => (b.priority - a.priority) || (themeRank(b) - themeRank(a)))[0] || items[0];
