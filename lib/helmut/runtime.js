...
function buildContext(signal, rawItem, profile) {
  ...
  if ((text.includes("ausschuss") || text.includes("pflege")) && !text.includes("tariftreuegesetz") && !text.includes("bundestariftreuegesetz")) return context(committeeLine, "Bereite für die Ausschusssitzung zwei Fragen vor: Was plant die Bundesregierung konkret, und wie werden Arbeitsbedingungen statt nur Abläufe verbessert?", "Gute Pflege braucht gute Arbeit: verlässliche Dienstpläne, genug Personal und Tarifbindung. Genau daran muss sich die Bundesregierung messen lassen.", "Ohne Vorbereitung bleibst du im Ausschuss reaktiv und bekommst keine eigene Prüfspur gegenüber der Bundesregierung.", "Das Thema verbindet deine Ausschussarbeit mit konkreter Lebensrealität von Beschäftigten.", "Wenn du nicht reagierst, läuft die Ausschusssitzung ohne deine sichtbare sozialpolitische Linie.");
