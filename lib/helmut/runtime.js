...
function generateBriefing(profile, rawItems, sources = demoSources) {
  ...
  const signals = topics.map((topic) => {
    const signal = topicToSignal(topic, rawItems, sources, profile);
  ...
  const themeOfDay = selectThemeOfDay(curatedItems, profile);
...
function topicToSignal(topic, rawItems, sources, profile = null) {
  ...
  return {
    ...
    entities: Array.from(new Set(topic.rawItemIds.flatMap((id) => inferEntities(rawItems.find((item) => item.id === id), profile)))),
...
function scorePoliticalTopic(topic, rawItems, profile) {
  ...
  const governmentScore = hasAnyTerm(text, ["bundesregierung", "bmas", "bundesministerium", "bundesarbeitsministerium", "bundesarbeitsministeriums", "gesetzentwurf", "eckpunkte", "reform", "initiative", "paket", "koalitionsausschuss", "bundeskabinett", "kabinett", "regierungskoalition"]) ? 92 : 20;
...
function relevanceToScore(signal, profile, relevance) {
  const governmentTimeSensitive = relevance.governmentScore >= 80 && relevance.timePressureScore >= 80;
  const decision = relevance.level === "Kritisch" || relevance.level === "Hoch" ? "Sofort reagieren"
    : relevance.level === "Mittel" ? "Beobachten"
    : (relevance.level === "Niedrig" && governmentTimeSensitive) ? "Beobachten"
    : "Ignorieren";
  return {
    id: `score-${signal.id}`,
    signalId: signal.id,
...
function hasTerm(text, term) {
  ...
  // Linke Wortgrenze ist zwingend (kein Treffer mitten in einem anderen Wort,
  // z. B. "rente" in "Referenten"). Rechts wird bewusst keine Grenze verlangt,
  // damit deutsche Komposita mit dem Begriff als Wortstamm erkannt werden
  // ("Pflegereform", "Rentenpaket", "Tariftreuegesetz").
  return new RegExp(`(^|[^a-zäöüß])${escaped}`, "i").test(text);
}
...
function rawItemToSignal(item, sources, profile = null) {
  ...
  return {
    ...
    entities: inferEntities(item, profile),
...
function toBriefingItem(briefingId, signal, score, relevance, topic, rawItems, sources, profile) {
  ...
  const context = buildContext(signal, rawItem, profile, score.decision);
...
function buildEvidenceRelevance(item, profile) {
  const text = `${item.title} ${item.content}`.toLowerCase();
  const committeeLabel = profile.committee || (profile.committees || []).join(", ") || "dein Fachausschuss";
  if (item.sourceId === "source-person") return `Direktes Signal zu deiner Person im Ausschuss ${committeeLabel}.`;
  if (item.sourceType === "ministry") return `Direktes Vorhaben der Bundesregierung im Ausschussbereich ${committeeLabel}.`;
  if (item.sourceType === "bundestag" || item.sourceType === "committee") return `Offizielle parlamentarische Quelle mit unmittelbarem Bezug zu deinem Ausschuss ${committeeLabel}.`;
  ...
  if (hasAnyTerm(text, ["pflege", "pflegereform"])) return `Fachlicher Bezug zu Pflege, sozialer Sicherung und damit zu deinem Ausschuss ${committeeLabel}.`;
...
function buildTaskTitle(item) {
  ...
  // Gleiche Regeln wie in buildContext/buildDecisionDetails: "rente" darf nicht
  // in "Referenten"/"Referentenentwurf" greifen (linke Wortgrenze via hasTerm),
  // und "sanktion" ist zwischen Buergergeld- und aussenpolitischen Sanktionen
  // mehrdeutig - nur mit Sozialleistungsbezug werten.
  const isWelfareSanction = hasAnyTerm(text, ["sanktion", "sanktionen"]) && hasAnyTerm(text, ["bürgergeld", "buergergeld", "grundsicherung", "jobcenter", "arbeitslosengeld", "erwerbslos", "mitwirkungspflicht"]);
  if (hasTerm(text, "tariftreue")) return "BMAS-Eckpunkte analysieren";
  if (hasTerm(text, "arbeitsmarkt")) return "Arbeitsmarktinitiative bewerten";
  if (hasTerm(text, "bürgergeld") || isWelfareSanction) return "Bürgergeld-Gegenlinie vorbereiten";
  if (hasTerm(text, "mindestlohn")) return "Mindestlohn-Positionierung freigeben";
  if (hasAnyTerm(text, ["pflege", "pflegereform"])) return "Ausschussfragen Pflege vorbereiten";
  if (hasTerm(text, "rente")) return "Rentenpaket fachlich prüfen";
...
function buildDecisionDetails(signal, rawItem, score, profile) {
  ...
  const committeeLabel = profile.committee || (profile.committees || []).join(", ") || "dein Fachausschuss";
  const ownCommittees = [profile.committee, ...(profile.committees || [])].filter(Boolean);
  ...
  // "persönlich" allein ist zu generisch (siehe buildContext) - nur mit einem
  // echten Angriffssignal werten.
  const isAttack = hasAnyTerm(text, ["angriff", "leistungsverweigerung", "afd", "vorwurf"]) || (hasTerm(text, "persönlich") && hasAnyTerm(text, ["angriff", "vorwurf", "attackiert", "hetze"]));
  const isCommittee = signal.sourceType === "committee" || ownCommittees.some((name) => hasTerm(text, name));
  const isParty = signal.sourceType === "party";
  const isIgnore = score.decision === "Ignorieren";

  if (isIgnore) {
    return {
      whyNow: `Dieses Thema erzeugt heute keinen politischen Entscheidungsbedarf für deinen Ausschuss ${committeeLabel}.`,
  ...
  if (hasAnyTerm(text, ["pflege", "pflegereform"])) {
    return {
  ...
  if (isGovernmentPlan) {
    return {
      ...
      politicalBenefit: "Hoch",
      targetGroup: hasTerm(text, "tarif") ? "Arbeitnehmer, Gewerkschaften und Betriebsräte" : "Beschäftigte, Sozialverbände und Fachpresse",
      recommendedChannel: hasTerm(text, "tarif") ? "LinkedIn und Presse" : "Presse-Statement und Ausschussfragen",
  ...
  if (isParty) {
    return {
      whyNow: `Deine Fraktion sucht heute eine klare Stimme aus ${committeeLabel}. Wenn du früh lieferst, wird deine Linie anschlussfähig.`,
      estimatedTimeMinutes: 10,
      politicalBenefit: "Mittel",
  ...
  return {
    whyNow: `Das Thema berührt deinen Ausschuss ${committeeLabel} heute indirekt und sollte für Anschlussfähigkeit beobachtet werden.`,
...
function buildContext(signal, rawItem, profile, decision) {
  const text = `${rawItem.title} ${rawItem.content}`.toLowerCase();
  const committeeLabel = profile.committee || (profile.committees || []).join(", ") || "dein Fachausschuss";
  const ownCommittees = [profile.committee, ...(profile.committees || [])].filter(Boolean);
  const mentionsOwnCommittee = ownCommittees.some((name) => hasTerm(text, name));
  const isAttackContext = hasAnyTerm(text, ["angriff", "vorwurf", "attackiert", "hetze", "diffamiert"]);
  const isWelfareSanction = hasAnyTerm(text, ["sanktion", "sanktionen"]) && hasAnyTerm(text, ["bürgergeld", "buergergeld", "grundsicherung", "jobcenter", "arbeitslosengeld", "erwerbslos", "mitwirkungspflicht"]);
  const committeeLine = `Das betrifft dich direkt: Es gehört in deinen Ausschuss ${committeeLabel}, weil dort Arbeit, soziale Sicherung, Tarifbindung, Rente und Pflege politisch entschieden werden.`;
  // Reine Personen-Erwaehnung (auch ueber die Person-Quelle) sagt noch nichts
  // ueber das Thema aus - ohne Mindestlohnbezug waere die feste Mindestlohn-
  // Erzaehlung fuer jede beliebige Erwaehnung der eigenen Person falsch.
  const mentionsPersonWithoutTopicCheck = signal.sourceType === "person" || signal.entities.includes(profile.fullName);
  if (mentionsPersonWithoutTopicCheck && hasAnyTerm(text, ["mindestlohn", "lohnforderung"])) return context(`Das betrifft dich direkt: Deine Mindestlohnforderung wird aufgegriffen und liegt mitten in deinem Ausschuss ${committeeLabel}. Du kannst daraus eine klare Frage an die Bundesregierung machen.`, "Lege kurz öffentlich nach: Miss die Bundesregierung am höheren Mindestlohn und gib eine Social-Media-Formulierung frei.", "Wer Vollzeit arbeitet, muss von seinem Lohn leben können. Die Bundesregierung muss beim Mindestlohn liefern, nicht weiter vertrösten.", "Ohne deine Einordnung bleibt die Nachfrage bei einer Einzeläußerung und verliert den Ausschussbezug.", "Du kannst dein Profil als Stimme für Arbeit, Lohn und soziale Sicherheit schärfen.", "Wenn du nicht reagierst, besetzen andere die Mindestlohnfrage, obwohl sie direkt in deinen fachpolitischen Raum fällt.");
  // "persönlich" ist als Einzelwort zu generisch (z. B. eine "persönliche Erklärung"
  // nach GOBT hat nichts mit einem persönlichen Angriff zu tun) - nur mit einem
  // erkennbaren Angriffssignal werten. "afd" allein sagt noch nichts ueber das
  // Thema aus (AfD kommentiert praktisch jedes Politikfeld) - der Bürgergeld-
  // Sanktionsframe wird hier nur behauptet, wenn auch ein Sozialleistungsbezug
  // erkennbar ist. "leistungsverweigerung" bleibt als eigenstaendiger, bereits
  // sozialleistungsspezifischer Begriff ausreichend.
  const isWelfareAttack = hasTerm(text, "leistungsverweigerung")
    || ((hasTerm(text, "afd") || (hasTerm(text, "persönlich") && isAttackContext)) && hasAnyTerm(text, ["bürgergeld", "buergergeld", "sanktion", "sanktionen", "grundsicherung", "sozialleistung"]));
  if (isWelfareAttack) return context(`Das trifft deine Bürgergeldposition und damit ein Kernfeld deines Ausschusses ${committeeLabel}. Der Angriff versucht, deine fachliche Linie in einen persönlichen Sanktionsframe zu ziehen.`, "Reagiere sofort, aber wiederhole den Vorwurf nicht. Veröffentliche eine klare Gegenformulierung zu Würde, Beratung und armutsfester sozialer Sicherung.", "Bürgergeldpolitik darf Menschen nicht unter Generalverdacht stellen. Wer Armut bekämpfen will, braucht Beratung, gute Arbeit und Respekt statt populistischer Schuldzuweisungen.", "Eine falsche Antwort verstärkt den Angriff; Schweigen lässt den Sanktionsframe an deiner Position kleben.", "Mit einer ruhigen Antwort zeigst du fachliche Autorität im Ausschuss und grenzt soziale Politik klar gegen Hetze ab.", "Wenn du nicht reagierst, kann der Frame spätere Pressefragen zur Bürgergelddebatte vorprägen.");
  // "Sanktion" allein ist zwischen Buergergeld-Sanktionen und aussenpolitischen
  // Sanktionen (EU/Russland etc.) mehrdeutig - nur mit Sozialleistungsbezug werten.
  if (hasTerm(text, "bürgergeld") || isWelfareSanction) return context(`Das betrifft dich, weil Bürgergeld direkt in deinen Ausschuss ${committeeLabel} gehört. Die Bundesregierung plant Änderungen, auf die du politisch reagieren solltest.`, "Bereite eine Gegenlinie vor: Steig nicht in Sanktionssprache ein, sondern stell Beratung, gute Arbeit und Armutsvermeidung in den Mittelpunkt.", "Wer Bürgergeld nur als Misstrauensdebatte führt, macht Menschen kleiner, aber löst keine Armut.", "Die Debatte kann dich in eine Verteidigungsecke ziehen, wenn du nur auf Vorwürfe antwortest.", "Du kannst die Kernfrage stellen: Welche Pläne der Bundesregierung helfen Menschen wirklich in gute Arbeit?", "Wenn du nicht reagierst, dominiert die Sanktionssprache den Tag.");
  // Pflegereform-Erzaehlung nur mit echtem Pflegebezug - bare "Ausschuss"-Treffer
  // (mentionsOwnCommittee) landen jetzt separat weiter unten, statt hier
  // faelschlich als Pflegereform behandelt zu werden.
  if (hasAnyTerm(text, ["pflege", "pflegereform"]) && !hasAnyTerm(text, ["tariftreuegesetz", "bundestariftreuegesetz"])) return context(committeeLine, "Bereite eine kurze Linie zur Pflegereform vor: Keine Entlastung auf Kosten pflegender Angehöriger, Finanzierung offenlegen, soziale Absicherung sichern.", "Pflegereform darf nicht heißen, dass pflegende Angehörige am Ende die Rechnung zahlen. Die Bundesregierung muss Finanzierung, Entlastung und soziale Absicherung zusammen denken.", "Ohne Einordnung läuft die Debatte als reine Sparlogik, obwohl sie direkt deinen Ausschuss betrifft.", "Du kannst Pflege als soziale Frage sichtbar machen und dich an die Seite pflegender Angehöriger stellen.", "Wenn du nicht reagierst, prägen Regierung und Fachkommentare die Debatte ohne deine soziale Linie.");
  if (hasAnyTerm(text, ["tariftreue", "tarifbindung", "gewerkschaft"])) return context(`Das betrifft dich direkt: Tariftreue ist ein zentrales Thema in deinem Ausschuss ${committeeLabel}. Du kannst die Bundesregierung heute an Kontrollen, Sanktionen und echter Tarifbindung messen.`, "Bereite bis 11:00 Uhr eine zustimmend-kritische Linie vor: Unterstützung für Tariftreue, aber klare Bedingungen bei Kontrolle, Sanktionen und Schlupflöchern.", "Tariftreue darf kein schönes Wort bleiben. Wer öffentliche Aufträge bekommt, muss gute Arbeit und Tarifbindung garantieren.", "Wenn du nur zustimmst, übernimmt die Bundesregierung den Erfolg ohne Druck auf Schlupflöcher.", "Mit Gewerkschaften im Rücken kannst du eine konstruktive, aber klare linke Arbeitsmarktposition setzen.", "Wenn du nicht reagierst, prägt die Bundesregierung das Thema allein und deine Ausschusslinie bleibt unsichtbar.");
  if (hasTerm(text, "rente")) return context(`Das betrifft dich, weil Rente ein Kernfeld deines Ausschusses ${committeeLabel} ist. Das Rentenpaket der Bundesregierung trifft genau deine Frage nach politischen Vorhaben der Regierung.`, "Prüfe das Paket auf niedrige Einkommen und bereite eine Presselinie vor, die Finanzierung und Armutsfestigkeit abfragt.", "Eine Rentenreform muss zuerst Menschen mit niedrigen Einkommen Sicherheit geben. Alles andere ist sozialpolitisch zu wenig.", "Ohne schnelle Einordnung wirkst du bei einem zentralen Ausschussthema nicht sprechfähig.", "Du kannst die Bundesregierung an sozialer Sicherheit im Alter messen.", "Wenn du nicht reagierst, bleibt die Rentendebatte ohne deine linke, ausschussbezogene Zuspitzung.");
  if (hasAnyTerm(text, ["arbeitszeit", "acht-stunden-tag", "arbeitszeitgesetz"])) return context(`Das betrifft dich direkt: Arbeitszeitrecht liegt im Kern deines Ausschusses ${committeeLabel}. Wenn das BMAS oder Arbeitgeberverbände den Acht-Stunden-Tag zur Debatte stellen, geht es um Schutzrechte von Beschäftigten.`, "Bereite eine kurze Linie vor: Flexibilität darf nicht bedeuten, dass Schutzrechte ausgehöhlt werden. Frage konkret nach Kontrolle, Gesundheitsschutz und Mitbestimmung.", "Arbeitszeit darf nicht allein aus Sicht der Unternehmen diskutiert werden. Wer am Acht-Stunden-Tag rüttelt, muss Beschäftigte, Gesundheitsschutz und Mitbestimmung zuerst schützen.", "Wenn du nicht reagierst, prägen Arbeitgeber und Bundesregierung die Reformdebatte ohne klare linke Schutzperspektive.", "Du kannst dich als Stimme für gute Arbeit, Gesundheitsschutz und Mitbestimmung positionieren.", "Wenn du nicht reagierst, kann die Debatte als reine Flexibilisierungsfrage laufen, obwohl sie direkt deinen Ausschuss betrifft.");
  if (hasAnyTerm(text, ["arbeitsmarkt", "qualifizierung", "mitwirkungspflichten"])) return context(`Das betrifft dich, weil die Arbeitsmarktinitiative ein Bundesregierungsvorhaben im Kernbereich ${committeeLabel} ist. Darin stecken Chancen durch Qualifizierung, aber auch Risiken durch strengere Pflichten.`, "Lehne nicht pauschal ab. Benenne die Chance bei Qualifizierung und formuliere zugleich rote Linien gegen Druck und Sanktionen.", "Qualifizierung ist richtig, Druck ersetzt aber keine gute Arbeit. Die Bundesregierung muss fördern statt Menschen nur schneller in schlechte Jobs zu schieben.", "Wenn du nur gegen Pflichten argumentierst, wird die Chance auf Qualifizierung übersehen.", "Du kannst zeigen, dass linke Arbeitsmarktpolitik konkrete Weiterbildung und gute Arbeit verbindet.", "Wenn du nicht reagierst, definiert die Bundesregierung allein, was Arbeitsmarktintegration bedeutet.");
  // Eigener Ausschuss wird namentlich genannt, aber keines der spezifischeren
  // Themenmuster oben greift: eigene, ehrliche Zwischenstufe statt der
  // Pflegereform-Erzaehlung faelschlich fuer JEDES Ausschussthema zu verwenden.
  if (mentionsOwnCommittee) {
    const title = rawItem.title || "dieser Vorgang";
    return context(
      `Das betrifft dich direkt: „${title}" wird deinem Ausschuss ${committeeLabel} zugeordnet.`,
      `Prüfe kurz, was „${title}" konkret für deinen Ausschuss ${committeeLabel} bedeutet, und bereite bei Bedarf eine kurze fachliche Linie vor.`,
      "",
      `Ohne Einordnung bleibt unklar, ob aus „${title}" ein Risiko oder eine Chance für deine Ausschussarbeit entsteht.`,
      "",
      `Wenn du nicht reagierst, bleibt „${title}" ohne deine fachliche Einordnung aus dem Ausschuss ${committeeLabel}.`
    );
  }
  // Kein spezifisches Muster erkannt. Fuer echte "Ignorieren"-Signale bleibt die
  // bisherige, ehrliche Absage. Fuer alles andere waere eine pauschale
  // Irrelevanz-Behauptung selbst falsch: das Item hat bereits den profilbasierten
  // Relevanzfilter der Pipeline durchlaufen. Lieber neutral und konkret auf den
  // echten Titel bezogen als praezise, aber falsch.
  if (decision === "Ignorieren") {
    return context("Das betrifft dich heute nicht ausreichend: Es gibt keinen belastbaren Bezug zu deinem Ausschuss, deiner Partei, deiner Person oder einer anstehenden politischen Entscheidung.", "Ignoriere das Thema und halte es aus deinem Morgenbriefing heraus.", "", "Keine relevante politische Gefahr erkennbar.", "", "Wenn du nicht reagierst, entsteht kein Schaden; deine Zeit bleibt für relevante Vorgänge frei.");
  }
  const title = rawItem.title || "dieser Vorgang";
  return context(
    `Das könnte deinen Ausschuss ${committeeLabel} betreffen: „${title}" wurde als möglicherweise mandatsrelevant eingestuft, auch wenn Helmut noch kein spezifisches Muster dazu erkannt hat.`,
    `Prüfe kurz, ob „${title}" tatsächlich in deinen Ausschuss ${committeeLabel} fällt, bevor du reagierst.`,
    "",
    `Ohne kurze Prüfung bleibt unklar, ob sich aus „${title}" ein Risiko für deine Position ergibt.`,
    "",
    `Wenn du „${title}" nicht prüfst, weißt du nicht, ob eine Reaktion nötig gewesen wäre.`
  );
}
...
function selectThemeOfDay(items, profile) {
  const theme = items
    .filter((item) => item.decision !== "Ignorieren")
    .sort((a, b) => themeRank(b, profile) - themeRank(a, profile))[0] || items[0];
...
function themeRank(item, profile = {}) {
  const text = `${item.title} ${item.summary} ${item.recommendedAction}`.toLowerCase();
  const governmentBoost = hasAnyTerm(text, ["bmas", "bundesregierung", "bundesministerium", "gesetzentwurf", "eckpunkte", "rentenpaket"]) ? 10 : 0;
  // Bare "Arbeit und Soziales" wuerde fuer jedes andere Mandat nie greifen bzw.
  // fuer dieses eine Mandat jeden Ausschuss-Treffer erzwingen - der eigene
  // Ausschuss kommt jetzt aus dem Profil statt hartcodiert zu sein.
  const ownCommittees = [profile.committee, ...(profile.committees || [])].filter(Boolean);
  const committeeBoost = (ownCommittees.some((name) => hasTerm(text, name)) || hasAnyTerm(text, ["tariftreue", "mindestlohn", "rente", "bürgergeld"])) ? 5 : 0;
...
function buildExecutiveSummary(items, themeOfDay) {
  ...
  if (!themeOfDay) return `${greeting} Heute solltest du auf ${react} Themen reagieren, ${watch} beobachten und ${ignore} ignorieren.`;
  return `${greeting} Heute solltest du auf ${react} Themen reagieren, ${watch} beobachten und ${ignore} ignorieren. Dein wichtigstes Thema ist: ${themeOfDay.title}. ${themeOfDay.recommendedAction}`;
}
...
// Universelle Institutionen (mandatsunabhaengig). Mandatsspezifische Entitaeten
// (Name, Partei, Fraktion, Ausschuesse, Ministerien, Gegner) kommen aus dem
// Profil — kein hardcodierter Personen-/Parteiname mehr (Datenmotor V2, Commit 3).
...
module.exports = { cemInceProfile, demoSources, demoRawItems, generateBriefing, inferEntities };
