...
// "rente" als loser Teilstring (bewusst KEINE Wortgrenzen-Pruefung wie hasTerm(), weil "Rente" fast nur
// in Komposita vorkommt: Rentenpaket, Rentenreform, Altersrente, Fruehrente - eine Wortgrenzen-Pruefung
// wuerde genau diese haeufigsten echten Treffer verlieren) kollidiert mit Formen von "Referent(in/en)":
// "Referentenentwurf" (= Gesetzentwurf eines Ministeriums, unabhaengig vom Thema) ist eines der
// haeufigsten Woerter in der Bundestags-Berichterstattung ueberhaupt. Deshalb: die bekannte Kollision
// explizit herausfiltern, bevor auf "rente" geprueft wird.
function mentionsRentenThema(text) {
  return text.replace(/referent(en|in|innen)?/g, "").includes("rente");
}
...
function buildTaskTitle(item) {
  ...
  if (mentionsRentenThema(text)) return "Rentenpaket fachlich prüfen";
...
function buildContext(signal, rawItem, profile) {
  ...
  // "persönlich" allein wurde entfernt: das Wort ist in beliebigen neutralen Meldungen ueblich (z. B.
  // "der Minister nahm persoenlich teil") und loeste den AfD-Angriffs-Text auch fuer voellig unbeteiligte
  // Artikel aus. "afd" und "leistungsverweigerung" sind spezifisch genug, um den echten Angriffsfall
  // weiterhin zu erkennen (siehe Regressionstest mit echtem AfD-Angriffsartikel).
  if (text.includes("afd") || text.includes("leistungsverweigerung")) return context("Das trifft deine Bürgergeldposition und damit ein Kernfeld deines Ausschusses Arbeit und Soziales. Der Angriff versucht, deine fachliche Linie in einen persönlichen Sanktionsframe zu ziehen.", "Reagiere sofort, aber wiederhole den Vorwurf nicht. Veröffentliche eine klare Gegenformulierung zu Würde, Beratung und armutsfester sozialer Sicherung.", "Bürgergeldpolitik darf Menschen nicht unter Generalverdacht stellen. Wer Armut bekämpfen will, braucht Beratung, gute Arbeit und Respekt statt populistischer Schuldzuweisungen.", "Eine falsche Antwort verstärkt den Angriff; Schweigen lässt den Sanktionsframe an deiner Position kleben.", "Mit einer ruhigen Antwort zeigst du fachliche Autorität im Ausschuss und grenzt soziale Politik klar gegen Hetze ab.", "Wenn du nicht reagierst, kann der Frame spätere Pressefragen zur Bürgergelddebatte vorprägen.");
  // "sanktion" allein wurde entfernt: das Wort trifft genauso auf aussenpolitische Sanktionen (EU/Russland
  // etc.) und andere themenfremde Meldungen. Eine echte Buergergeld-Sanktionsdebatte nennt in der
  // Berichterstattung praktisch immer auch "buergergeld" selbst, das als Trigger bereits ausreicht.
  if (text.includes("bürgergeld")) return context("Das betrifft dich, weil Bürgergeld direkt in deinen Ausschuss Arbeit und Soziales gehört. Die Bundesregierung plant Änderungen, auf die du politisch reagieren solltest.", "Bereite eine Gegenlinie vor: Steig nicht in Sanktionssprache ein, sondern stell Beratung, gute Arbeit und Armutsvermeidung in den Mittelpunkt.", "Wer Bürgergeld nur als Misstrauensdebatte führt, macht Menschen kleiner, aber löst keine Armut.", "Die Debatte kann dich in eine Verteidigungsecke ziehen, wenn du nur auf Vorwürfe antwortest.", "Du kannst die Kernfrage stellen: Welche Pläne der Bundesregierung helfen Menschen wirklich in gute Arbeit?", "Wenn du nicht reagierst, dominiert die Sanktionssprache den Tag.");
  // "ausschuss" allein war bisher Teil dieser Bedingung — das Wort steht in praktisch JEDER
  // Bundestags-Ausschuss-Meldung, unabhängig vom Thema (z. B. ein Artikel über den Digitalausschuss),
  // und ordnete dann faelschlich die feste Pflegereform-Antwort zu. Nur noch echte Pflege-Treffer
  // loesen diesen Zweig aus (verifiziert am produktiven Pfad scheduler.js -> generateBriefing ->
  // buildContext, nicht nur am Demo-Datensatz).
  if (text.includes("pflege") && !text.includes("tariftreuegesetz") && !text.includes("bundestariftreuegesetz")) return context(committeeLine, "Bereite eine kurze Linie zur Pflegereform vor: Keine Entlastung auf Kosten pflegender Angehöriger, Finanzierung offenlegen, soziale Absicherung sichern.", "Pflegereform darf nicht heißen, dass pflegende Angehörige am Ende die Rechnung zahlen. Die Bundesregierung muss Finanzierung, Entlastung und soziale Absicherung zusammen denken.", "Ohne Einordnung läuft die Debatte als reine Sparlogik, obwohl sie direkt deinen Ausschuss betrifft.", "Du kannst Pflege als soziale Frage sichtbar machen und dich an die Seite pflegender Angehöriger stellen.", "Wenn du nicht reagierst, prägen Regierung und Fachkommentare die Debatte ohne deine soziale Linie.");
...
function inferTopic(item) {
  ...
  if (mentionsRentenThema(text)) return "Rente";
