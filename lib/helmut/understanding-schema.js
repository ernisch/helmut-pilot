...
const KNOWLEDGE_OBJECT_SCHEMA = {
  ...
  properties: {
    ...
    // OPTIONALE Referenz auf public.geographies: AUSDRUECKLICH string ODER null
    // (nullable) bei unaufgeloestem Ort -> "erhalten, aber unaufgeloest".
    affected_geographies: {
      ...
      items: {
        ...
        properties: {
          ...
          geography_id: { type: "string", maxLength: 80, nullable: true }
        }
      }
    },
    ...
    // entity_id ist eine OPTIONALE Referenz auf public.political_entities: AUSDRUECKLICH
    // string ODER null (nullable) bei unaufgeloester Entitaet.
    decision_entities: {
      type: "array",
      items: {
        type: "object",
        properties: {
          ...
          entity_id: { type: "string", maxLength: 80, nullable: true }
        }
      }
    },
    related_entities: {
...
function checkValue(value, spec, path, errors) {
  // Explizit nullable (z. B. optionale Referenzen geography_id/entity_id: string ODER null):
  // null ist dann ein gueltiger Wert; sonst bleibt null ein Fehler (statt es aus der
  // Validierung herauszunehmen -> der Vertrag deklariert die Nullbarkeit ausdruecklich).
  if (value === null) {
    if (spec.nullable) return;
    errors.push(`${path}: null nicht erlaubt (nicht als nullable deklariert)`);
    return;
  }
  if (spec.type === "array") {
...
// ── WERTFREIE, SICHERE DIAGNOSE DER SCHEMA-/DSGVO-FEHLER ───────────────────────────────────
// Anlass (Production-Befund 2026-09-24, 169er Run 35987448290, Vorgang
// vg-verzögerung-20230613-95c80f): der Cluster endete als `skipped-invalid`
// (`reason = validierung-fehlgeschlagen`) mit `validierungsfehler = []`. Die leere Liste ist
// DETERMINISTISCH erklaerbar: `validateUnderstandingResult` liefert nur dann keinen sichtbaren
// Code, wenn AUSSCHLIESSLICH Schema-/DSGVO-Meldungen den Ausschlag gaben — die frei formulierten
// Meldungen koennen Rohwerte der Modellantwort enthalten (z. B. "zeitdruck: '<wert>' nicht in
// {...}") und werden deshalb vom Bedienweg verworfen. Die URSACHE wurde damit unsichtbar.
//
// Diese Funktion uebersetzt dieselben Meldungen in WERTFREIE, feste Codes. Uebernommen wird
// ausschliesslich der FUEHRENDE FELDPFAD — ein Name aus UNSEREM eigenen Schema, niemals ein
// Modellwert. Beispiele:
//   "warum_wichtig: leer/zu kurz"                      -> "schema-leer:warum_wichtig"
//   "zeitdruck: 'x' nicht in {hoch, mittel, ...}"       -> "schema-enum:zeitdruck"
//   "action_items_struct[0].priority: erwartet string" -> "schema-typ:action_items_struct.priority"
...
// Es wird KEIN Rohwert, kein Satz und keine Antwort ausgegeben. Reihenfolge bleibt stabil,
// Duplikate werden entfernt.
function sichererFeldpfad(meldung) {
  // Nur der fuehrende Pfad: Buchstaben/Ziffern/Unterstrich, optionale [index]-Teile,
  // durch Punkte getrennt. Alles weitere (Wert, Vergleich) wird verworfen.
  const treffer = /^[A-Za-z_][A-Za-z0-9_]*(?:\[\d+\])?(?:\.[A-Za-z_][A-Za-z0-9_]*(?:\[\d+\])?)*/
    .exec(String(meldung || ""));
  return treffer ? treffer[0].replace(/\[\d+\]/g, "") : null;
}

function sichereSchemaFehler(errors = []) {
  const codes = [];
  for (const eintrag of Array.isArray(errors) ? errors : []) {
    const text = String(eintrag == null ? "" : eintrag);
    let code = null;
    if (/^knowledge_object ist kein Objekt/.test(text)) {
      code = "schema-objekt";
    } else if (/^Pflichtfeld fehlt: /.test(text)) {
      code = "schema-pflichtfeld:" + (sichererFeldpfad(text.slice("Pflichtfeld fehlt: ".length)) || "unbekannt");
    } else if (/^DSGVO: E-Mail-Muster/.test(text)) {
      code = "dsgvo-emailmuster:" + (sichererFeldpfad(text.slice("DSGVO: E-Mail-Muster in ".length)) || "unbekannt");
    } else if (/^DSGVO: verbotenes PII-Feld/.test(text)) {
      // Der Feldname selbst ist ein Modell-/Eingabeschluessel und wird bewusst NICHT uebernommen.
      code = "dsgvo-pii-feld";
    } else if (/^DSGVO: /.test(text)) {
      // `DSGVO: <feld>-Eintrag zu lang (n)` — der Feldname stammt aus UNSERER festen
      // Erwaeehnungsliste (MENTION_FIELDS), ist also kein Modellwert und darf im Code stehen.
      const feld = /^DSGVO:\s*([A-Za-z_][A-Za-z0-9_]*)-Eintrag zu lang/.exec(text);
      code = feld ? `dsgvo-eintrag-zu-lang:${feld[1]}` : "dsgvo-eintrag-zu-lang";
    } else {
      const pfad = sichererFeldpfad(text) || "unbekannt";
      if (/leer\/zu kurz/.test(text)) code = "schema-leer:" + pfad;
      else if (/zu lang \(/.test(text)) code = "schema-zu-lang:" + pfad;
      else if (/nicht in \{/.test(text)) code = "schema-enum:" + pfad;
      else if (/null nicht erlaubt/.test(text)) code = "schema-null:" + pfad;
      else if (/erwartet /.test(text)) code = "schema-typ:" + pfad;
      else code = "schema-fehler:" + pfad;
    }
