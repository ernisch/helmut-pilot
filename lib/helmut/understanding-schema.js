...
const KNOWLEDGE_OBJECT_SCHEMA = {
  ...
  properties: {
    ...
    // UI-Zeigefelder (additiv, gleicher KI-Call, NICHT required): Lage/Radar/
    // Helmut/Detailansicht lesen diese fertigen Felder statt selbst zu kuerzen.
    // Bewusst wie "headline" NICHT required/minLength-geprueft — ein einzelnes
    // schwaches UI-Feld darf nie die gesamte Analyse (was_ist_passiert etc.)
    // zu Fall bringen. display_title/display_category werden bei Ueberlaenge
    // in assembleKnowledgeObject verworfen (nie abgeschnitten) statt hier
    // hart zu scheitern — der Client faellt dann sauber auf den vollstaendigen
    // Originaltitel zurueck.
    display_title: { type: "string", maxLength: 70 },
