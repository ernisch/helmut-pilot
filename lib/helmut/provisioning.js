...
async function provisionBatch(specs, deps = {}, optionen = {}) {
  ...
  if (!ausfuehren) {
    ...
    for (const spec of liste) {
      let vorhaben = "anlegen";
      if (await isProtectedTenant(spec.id, deps)) vorhaben = "abbruch:geschuetztes-mandat";
      else {
        let bestand = null;
        try { bestand = await storage.getProfile(spec.id); } catch { bestand = null; }
        if (bestand) vorhaben = bestand.profileActive === false ? "aktualisieren:bleibt-deaktiviert" : "aktualisieren";
      }
      ergebnisse.push({ tenantId: spec.id, ok: true, trockenlauf: true, vorhaben });
    }
    return { ok: true, trockenlauf: true, abgebrochen: false, vorbefunde: [], ergebnisse,
...
function leereBilanz() {
  return { gesamt: 0, angelegt: 0, aktualisiert: 0, fehlgeschlagen: 0, geplant: 0 };
}
...
function bilanzAus(ergebnisse, trockenlauf) {
  ...
  for (const r of ergebnisse) {
    if (trockenlauf) { b.geplant += 1; continue; }
...
async function provisionTenant(spec = {}, deps = {}) {
  ...
  const { profil: profile, reaktivierung } = mergeMitBestand(
    buildProfile(spec), bestandsprofil, { reaktivieren: spec.reaktivieren === true }
  );
  if (bestandsprofil) {
    const behalten = Object.keys(bestandsprofil).filter((k) => !(k in buildProfile(spec)));
  ...
  try {
    if (byEmail) {
      ...
      if (!persistiert) {
        ...
        return { ok: false, aborted: true, reason: "auth-write-not-persisted", tenantId: spec.id, log: entries };
      }
