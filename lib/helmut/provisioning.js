...
// geklappt hat. Im ausdruecklich inaktiven Kohortenlauf ist Loeschen gesperrt:
// ein moeglicher Teilbestand wird gemeldet und bleibt zur Klaerung erhalten.
// Rueckgabe: "ok" | "fehlgeschlagen" | "gesperrt".
async function rolleNeuesKontoZurueck(accounts, entries, userId, kontoBeiFehlerBehalten = false) {
  if (kontoBeiFehlerBehalten) {
    log(entries, "rollback", "gesperrt",
      `Automatische Loeschung von Konto ${userId} ist fuer diesen inaktiven Lauf gesperrt.`
      + " Ein moeglicher Teilbestand bleibt erhalten und muss rein lesend geklaert werden.");
    return "gesperrt";
  }
  try {
...
async function provisionTenant(spec = {}, deps = {}, optionen = {}) {
  ...
  const kontoBeiFehlerBehalten = optionen.kontoBeiFehlerBehalten === true;
  if (optionen.kontoBeiFehlerBehalten !== undefined
      && typeof optionen.kontoBeiFehlerBehalten !== "boolean") {
    throw new TypeError("kontoBeiFehlerBehalten muss ein ausdruecklicher boolescher Wert sein.");
  }
  if (kontoBeiFehlerBehalten && neuAktiv !== false) {
    throw new TypeError("kontoBeiFehlerBehalten ist ausschliesslich fuer neuAktiv:false zulaessig.");
  }
  const accounts = deps.accounts || require("./accounts");
  const storage = deps.storage || require("./storage");
  const { validateProfile } = deps.validation || require("./profile-validation");
  ...
  // FAIL CLOSED bei einem Reaktivierungswunsch in einem AUSDRUECKLICH INAKTIVEN Lauf
  // (Reviewbefund 03.09.). Der STAPELpfad weist `reaktivieren` ueber
  // `aktivierungswunschBefund` seit jeher ab; dieser EINZELpfad tat es nicht — ein Lauf
  // mit `neuAktiv:false` haette ein deaktiviertes Bestandsprofil trotzdem reaktiviert und
  // `ok:true` gemeldet. Ein Lauf, der sagt "ich aktiviere nichts", darf nichts aktivieren:
  // das ist die Zusage, auf der die stufenweise Provisionierung der Testkohorte steht.
  // Ein Reaktivierungslauf bleibt moeglich — er muss sich nur als solcher ausweisen
  // (`neuAktiv:true`), und die Aktivierung bleibt eine eigene Freigabe (CLAUDE.md §5).
  if (neuAktiv === false && spec && spec.reaktivieren !== undefined
      && spec.reaktivieren !== null && spec.reaktivieren !== false) {
    log(entries, "aktivierungswunsch", "abbruch",
      `"reaktivieren" ist gesetzt (${JSON.stringify(spec.reaktivieren)}), der Lauf ist aber`
      + " ausdruecklich inaktiv (neuAktiv=false). Eine Reaktivierung ist eine getrennte"
      + " Freigabeentscheidung und wird hier nicht still mitgemacht — es wird nichts geschrieben.");
    return { ok: false, aborted: true, reason: "reaktivierung-in-inaktivem-lauf", tenantId: spec.id, log: entries };
  }
  ...
  try {
    if (byEmail) {
      ...
      if (user.politicianId !== spec.id) {
        ...
        const rueckweg = await rolleNeuesKontoZurueck(accounts, entries, user.id, kontoBeiFehlerBehalten);
        return { ok: false, aborted: true, reason: "politician-id-collision",
          rueckweg, teilbestandMoeglich: true, kontoId: user.id, tenantId: spec.id, log: entries };
      }
    ...
    return { ok: false, aborted: true, reason: "auth-write-failed",
      teilbestandMoeglich: true, kontoId: user && user.id || null, tenantId: spec.id, log: entries };
  }

  // 5) Profil schreiben. Der Kohortenpfad behaelt einen moeglichen Teilbestand;
  // er darf auch im Fehlerfall keine Daten loeschen.
  try {
    ...
    let rueckweg;
    if (createdUserThisRun) rueckweg = await rolleNeuesKontoZurueck(accounts, entries, user.id, kontoBeiFehlerBehalten);
...
async function setTestProfileParticipation(id, active, deps = {}) {
  if (!require("./testkohorte-betrieb").istKohortenKennung(id) || await isProtectedTenant(id, deps))
    return { ok: false, reason: "protected-tenant" };
  return (deps.storage || require("./storage")).setTestProfileActive(id, active);
}

module.exports = {
  setTestProfileParticipation,
  PROVISIONING_MARKER,
  isProtectedTenant,
  validateSpec,
  buildProfile,
