...
async function provisionTenant(spec = {}, deps = {}) {
  ...
  if (byEmail && byEmail.politicianId !== spec.id) {
    log(entries, "konflikt", "abbruch", `E-Mail ${spec.email} gehört einem anderen Konto (politicianId=${byEmail.politicianId ?? "—"}, Rolle ${byEmail.role})`);
...
// würde beim Entfernen EINES Mandanten auch Personen-/News-Rohdaten ANDERER Mandanten
// (inkl. cem-ince) mitlöschen.
