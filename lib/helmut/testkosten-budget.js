...
function konfiguration(env = process.env) {
  return { version: VERSION, aktiv: aktiv(env), limitUsd: LIMIT_MICRO_USD / 1e6,
    maxManualCalls: MAX_MANUAL_CALLS, maxWindowMs: MAX_WINDOW_MS, tarif: RATE_ID,
...
function pruefeTag(t, day) {
  fordere(t && t.version === VERSION && t.day === day && t.tarif === RATE_ID
    ...
    && Number.isSafeInteger(t.manualCalls) && t.manualCalls >= 0 && t.manualCalls <= MAX_MANUAL_CALLS
  ...
  fordere(Object.keys(t.calls).length <= 5000 && Object.values(t.calls).every(c => c
    && ["reserviert", "abgerechnet", "nicht-gesendet"].includes(c.status)
    ...
    && (c.status === "reserviert" || (Number.isSafeInteger(c.cost) && c.cost >= 0 && c.cost <= c.reserved))),
...
function belegt(t) {
  return t.spent + Object.values(t.calls).filter(c => c.status === "reserviert").reduce((n, c) => n + c.reserved, 0);
}
async function reserviere({ model, maxOutputTokens, runId = null }, deps = {}) {
    ...
    const result = await storage.mutateAuthStore(async auth => {
      ...
      if (!auth[KEY][day]) {
        ...
        auth[KEY][day] = { version: VERSION, day, tarif: RATE_ID, limit: LIMIT_MICRO_USD,
          spent: baseline, baseline, manualCalls: 0, manualUntil: null, calls: {}, frozen: null };
      }
      const t = pruefeTag(auth[KEY][day], day);
      if (Object.values(t.calls).some(c => c.status === "reserviert"
        && Date.parse(c.createdAt) + MAX_PENDING_MS <= now().getTime())) t.frozen = "test-usd-ausgang-unklar";
      if (t.frozen) return { error: t.frozen };
      fordere(!t.calls[id], "test-usd-doppelte-reservierung");
      fordere(Object.keys(t.calls).length < 5000, "test-usd-buch-voll");
      if (manual && (t.manualCalls >= MAX_MANUAL_CALLS
        || (t.manualUntil && now().getTime() >= Date.parse(t.manualUntil)))) return { error: "test-usd-fenster-geschlossen" };
      ...
      if (manual) {
        t.manualCalls++;
        t.manualUntil ||= new Date(Math.min(start.getTime() + MAX_WINDOW_MS,
          Date.parse(day + "T00:00:00Z") + 86400000)).toISOString();
      }
      return { ticket };
    });
...
async function abschliessen(ticket, receipt, deps = {}) {
  ...
  try {
    const known = await storage.mutateAuthStore(auth => {
      ...
      if (c.status !== "reserviert") return true;
      let cost;
      try {
        ...
        // Timeout, fehlende Usage oder fehlender Speicherbeleg: volle Reserve
        // bleibt bestehen. Keine vermeintliche Rueckerstattung oder Wiederholung.
        t.frozen = "test-usd-ausgang-unklar";
        return false;
      }
...
module.exports = { aktiv, konfiguration, reserviere, abschliessen, nichtGesendet,
  tokenKosten, pruefeTag, belegt, KEY, VERSION, LIMIT_MICRO_USD, MAX_MANUAL_CALLS, MAX_WINDOW_MS };
