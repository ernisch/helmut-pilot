...
const LEGACY_LIMIT_MICRO_USD = 4000000;
const LIMIT_MICRO_USD = 6000000;
// Ausdrueckliche Betreiberfreigabe26.09.: Tages- und Auftragsgrenze je6USD.
// Alte Tagesbuecher und Version1-Auftraege behalten ihren4USD-Vertrag.
const AUFTRAG_V2_LIMIT_MICRO_USD = 6000000;
const MAX_INPUT_TOKENS = 400000;
// 8000 statt 3000: der einmalige isolierte 8-Fall-Modellvergleich (genau EIN
// Aufruf) braucht eine groessere Ausgabegrenze. Volle Reserve =
// tokenKosten(400000, 8000) = 232000 Mikro-USD (0,232 USD), weiterhin deutlich
// unter dem freigegebenen 6-USD-Tagesriegel. Rueckwaertsvertraeglich: bereits
// gebuchte Alttickets mit 3000 Tokens / 212000 bleiben unveraendert gueltig.
const MAX_OUTPUT_TOKENS = 8000;
const MAX_PENDING_MS = 5 * 60 * 1000;
const KEY = "testKostenTage";
const AUFTRAG_KEY = "testKostenAuftrag";
const RATE_ID = "azure-gpt5-mini-obergrenze-20260909";
// Manuelle Laufkennungen: genau diese Formen erhalten einen laufgebundenen Bezug
// (bezug.runId) und sind damit in der Kostenablage einzeln zuordenbar. `nachlauf500-…` ist die
// bestehende Konvention des 500er-Nachlaufs; `verstehen169-…` die Kennung des einmaligen
// 169er Verstehenslaufs; verstehen4 ist dessen isolierte Vierernacharbeit; verstehen30
// sowie verstehen-bund7, verstehen16 und verstehen18 binden die jeweiligen Quellenauftraege.
// CAS und die Einzelauftragsgrenzen bleiben unveraendert.
const MANUELLE_RUN_ID = /^(nachlauf500|verstehen169|verstehen4|verstehen30|verstehen-bund7|verstehen16|verstehen18)-[0-9]{5,20}$/;
...
// Nur bekannte Paare akzeptieren; kein freier Budgetwert aus einem Laufzeitbericht.
function tagespolitikGueltig(c) {
  return Boolean(c && ((c.version === 2 && c.limitUsd === 4)
    || (c.version === POLICY_VERSION && c.limitUsd === LIMIT_MICRO_USD / 1e6)));
}
function tageslimitGueltig(limitUsd) { return limitUsd === 4 || limitUsd === 6; }
...
function bezugGueltig(b) {
  return b && b.version === 1 && MANUELLE_RUN_ID.test(b.runId || "")
...
function pruefeTag(t, day) {
  fordere(t && ((t.version === 1 && t.limit === LEGACY_LIMIT_MICRO_USD)
    || (t.version === VERSION && t.limit === LIMIT_MICRO_USD)) && t.day === day && t.tarif === RATE_ID && Number.isSafeInteger(t.spent) && t.spent >= 0
  ...
  fordere(Object.keys(t.calls).length <= 5000 && Object.values(t.calls).every(c => c
    ...
    // Obergrenze aus derselben Formel wie die Buchung, damit Deckel und
    // tokenKosten(MAX_INPUT_TOKENS, MAX_OUTPUT_TOKENS) nie auseinanderlaufen.
    && Number.isSafeInteger(c.reserved) && c.reserved > 0
    && c.reserved <= tokenKosten(MAX_INPUT_TOKENS, MAX_OUTPUT_TOKENS)
...
// Optionaler Betreiberauftrag: dieselben Tagesbuecher PLUS extern gebundene
// Kosten. Kein zweites Produktionskonto, kein Reset bei UTC-Tageswechsel.
// Die Konfiguration wird ausschliesslich administrativ gesetzt. Fehlt sie,
// bleibt der bestehende Tagesvertrag unveraendert; unlesbar bedeutet gesperrt.
function auftragsStand(auth, day) {
  if (!Object.hasOwn(auth, AUFTRAG_KEY)) return null;
  const a = auth[AUFTRAG_KEY];
  const tagGueltig = s => typeof s === "string" && /^\d{4}-\d{2}-\d{2}$/.test(s)
    && Number.isFinite(Date.parse(s)) && new Date(s).toISOString().slice(0, 10) === s;
  fordere(a && ((a.version === 1 && a.limit === LEGACY_LIMIT_MICRO_USD)
    || (a.version === 2 && a.limit === AUFTRAG_V2_LIMIT_MICRO_USD))
    && /^[a-z0-9-]{1,100}$/.test(a.id || "")
    && Object.keys(a).every(k => ["version", "id", "abTag", "limit", "externGebunden"].includes(k))
    && tagGueltig(a.abTag) && tagGueltig(day) && a.abTag <= day
    && Number.isSafeInteger(a.externGebunden)
    && a.externGebunden >= 0, "test-usd-auftrag-unlesbar");
  const tage = (Date.parse(day) - Date.parse(a.abTag)) / 86400000 + 1;
  fordere(Number.isInteger(tage) && tage > 0 && tage <= 31, "test-usd-auftrag-unlesbar");
  let gebunden = a.externGebunden;
  for (let i = 0; i < tage; i++) {
    const d = new Date(Date.parse(a.abTag) + i * 86400000).toISOString().slice(0, 10);
    // Fehlende Tage niemals kostenlos annehmen. Der heutige Tag wird vor
    // Reservierung aus Historie und atomarem Zaehler bestaetigt initialisiert.
    const t = pruefeTag(auth[KEY]?.[d], d);
    kontrolliere(auth, d);
    gebunden += belegt(t);
  }
  fordere(Number.isSafeInteger(gebunden), "test-usd-auftrag-unlesbar");
  return { id: a.id, gebundenMicroUsd: gebunden, limitMicroUsd: a.limit,
    grenzeInklusive: a.version === 2 };
}
function pruefeAuftrag(auth, day, reserve = 0) {
  const stand = auftragsStand(auth, day);
  fordere(!stand || (stand.grenzeInklusive
    ? stand.gebundenMicroUsd + reserve <= stand.limitMicroUsd
    : stand.gebundenMicroUsd + reserve < stand.limitMicroUsd),
    "test-usd-auftragsgrenze-erreicht");
  return stand;
}
...
// Rein lesende Startpruefung, auch vor der ersten atomaren Tagesreservierung.
// Ein fehlendes Tagesbuch gilt nur nach vollstaendigem Historien-/Zaehlerabgleich
// als neuer Tag. Die eigentliche Reservierung prueft und initialisiert erneut.
function pruefeStart(auth, day, counter) {
  fordere(/^\d{4}-\d{2}-\d{2}$/.test(day || "") && Number.isFinite(Date.parse(day))
    && new Date(day).toISOString().slice(0, 10) === day, "test-usd-tag-ungueltig");
  fordere(counter?.ok === true && Number.isSafeInteger(counter.used) && counter.used >= 0,
    "test-usd-zaehler-unlesbar");
  const buch = auth[KEY];
  fordere(buch === undefined || (buch && typeof buch === "object" && !Array.isArray(buch)
    && Object.keys(buch).length <= 31), "test-usd-buch-unlesbar");
  if (buch && Object.hasOwn(buch, day)) {
    const stand = kontrolliere(auth, day, 0, counter.used);
    pruefeAuftrag(auth, day);
    fordere(buch[day].frozen === null && stand.offeneReservierungen === 0,
      "test-usd-start-nicht-frei");
    return { ...stand, startklar: true, tagesbuchVorhanden: true };
  }
  fordere(!buch || Object.keys(buch).length < 31, "test-usd-buch-voll");
  const baseline = historie(auth, day, counter.used);
  fordere(baseline <= LIMIT_MICRO_USD, "test-usd-grenze-erreicht");
  // Nur eine Leseprojektion des noch nicht angelegten Tages, keine Mutation.
  pruefeAuftrag({ ...auth, [KEY]: { ...buch, [day]: { version: VERSION, day,
    tarif: RATE_ID, limit: LIMIT_MICRO_USD, spent: baseline, baseline,
    baselineCalls: counter.used, manualCalls: 0, manualUntil: null, calls: {}, frozen: null } } }, day);
  return { startklar: true, tagesbuchVorhanden: false, atomarerUsdRiegel: true,
    limitUsd: LIMIT_MICRO_USD / 1e6, gebundenUsd: baseline / 1e6,
    offeneReserveUsd: 0, offeneReservierungen: 0, anbieterrechnung: false };
}
async function reserviere({ model, maxOutputTokens, runId = null, politicianId = null, phase = null }, deps = {}) {
  ...
  const manual = MANUELLE_RUN_ID.test(runId || "");
  ...
  try {
    const result = await storage.mutateAuthStore(async auth => {
      ...
      // Innerhalb DESSSELBEN CAS wie die neue Reserve: auch konkurrierende
      // Crons und manuelle Aufrufe koennen die Auftragsgrenze nicht ueberholen.
      pruefeAuftrag(auth, day, reserved);
...
// Die VOLLE Reservierung eines Aufrufs in USD (dieselbe Formel wie die Buchung selbst —
// `tokenKosten(MAX_INPUT_TOKENS, maxOutputTokens)`); der Default 3000 ist die
// requestStructuredJson-Ausgabegrenze des Understanding-Pfads.
function reservierungHoeheUsd(maxOutputTokens = 3000) {
  fordere(Number.isSafeInteger(maxOutputTokens) && maxOutputTokens > 0
    && maxOutputTokens <= MAX_OUTPUT_TOKENS, "test-usd-ausgabegrenze-ungueltig");
  return tokenKosten(MAX_INPUT_TOKENS, maxOutputTokens) / 1e6;
}
// REIN LESENDER Stand der einem manuellen Lauf zurechenbaren Kosten: abgerechnete echte
// Tokenkosten plus offene (voll reservierte) Buchungen. Fail closed: eine unlesbare oder
// ungueltige Kostenablage wirft — niemals „0 USD". Kein Write, keine zweite Buchhaltung.
async function laufGebundenUsd(runId, deps = {}) {
  fordere(MANUELLE_RUN_ID.test(String(runId || "")), "test-usd-laufkennung-ungueltig");
  const env = deps.env || process.env;
  fordere(aktiv(env), "test-usd-reservierung-aus");
  const storage = deps.storage || require("./storage");
  const auth = await storage.readAuthStore();
  const tage = (auth && auth[KEY]) || {};
  let abgerechnet = 0, offenReserviert = 0;
  for (const day of Object.keys(tage)) {
    const t = pruefeTag(tage[day], day);
    for (const c of Object.values(t.calls)) {
      if (c.bezug && c.bezug.runId === runId) {
        if (typeof c.cost === "number") abgerechnet += c.cost;
        if (c.status === "reserviert" || c.status === "ungeklaert") offenReserviert += c.reserved;
      }
    }
  }
  return (abgerechnet + offenReserviert) / 1e6;
}
module.exports = { aktiv, konfiguration, reserviere, abschliessen, nichtGesendet,
  AUFTRAG_KEY, auftragsStand, tagespolitikGueltig, tageslimitGueltig,
  tokenKosten, pruefeTag, belegt, kontrolliere, pruefeStart, KEY, VERSION, POLICY_VERSION, LIMIT_MICRO_USD,
  reservierungHoeheUsd, laufGebundenUsd, MANUELLE_RUN_ID };
