...
function azureEmbeddingProvider(env = process.env, fetchImpl = fetch) {
  ...
  // ENDPUNKTGUARD (Haertung 2026-09-02): die Adresse wurde hier roh aus dem
  // Umgebungswert interpoliert und mit dem echten `api-key`-Kopf plus dem
  // Wissensobjekt-Text abgesendet. `ai.js` war nach der Haertung dicht, dieser
  // Weg nicht. Ein ungueltiger Endpunkt liefert jetzt KEINEN Provider — der
  // Backfill meldet dann "kein Provider konfiguriert" und sendet nichts.
  const geprueft = azureEndpunkt.pruefeEndpunkt(endpoint, {
    erlaubeLoopback: env.HELMUT_KI_LOOPBACK_ERLAUBT === "1"
  });
  if (!geprueft.gueltig) return null;
  const url = `${geprueft.basis}/openai/deployments/${deployment}/embeddings?api-version=${AZURE_API_VERSION}`;
  return {
    name: "azure-openai",
    deployment,
