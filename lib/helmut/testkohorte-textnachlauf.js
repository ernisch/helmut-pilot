...
const W = require("./testnachweis-arbeitsfenster");
const B = require("./testkosten-budget");
...
const SICHERE_LAUFGRUENDE = new Set([
  ...
  "nachlauf-briefing-aussagenpruefung-fehlt",
  "nachlauf-testfenster-nicht-aktiv", "nachlauf-testfenster-commit-abweichend",
  "nachlauf-testfenster-geschlossen", "nachlauf-testfenster-veraendert",
  "nachlauf-testfenster-ziel-abweichend", "nachlauf-testfenster-id-fehlt",
  "nachlauf-testfenster-unlesbar"
]);
...
function pruefeKosten(auth, counter, tag) {
  ...
  if (auth.testKostenTage?.[tag]) {
    // Der Aufrufer hat die aktive Tagesregel bereits geprueft. Fehlende
    // Nutzungszeilen bleiben unbekannt und brauchen eigene volle Reserven.
    try { Object.assign(k, B.kontrolliere(auth, tag,
...
function pruefeConfig(c, commit, arbeitsbeginn = 1) {
  fordere(c && c.production === true && c.commit === commit && /^[a-f0-9]{40}$/.test(commit || "")
    ...
    && B.tagespolitikGueltig(c.testKosten) && c.testKosten.aktiv === true
...
async function ausfuehren({ commit, runId, confirmation, config, arbeitsbeginn = 1,
  testfensterId, env = process.env, fetchFn = global.fetch, deps = {} } = {}) {
  const now = deps.now || (() => new Date());
  const start = now();
  const tag = start.toISOString().slice(0, 10);
  const day = berlinTagKey(start);
  let end = start.getTime() + BUDGET_MS, fenster = null;
  const results = [];
  ...
  const bericht = () => ({ ok: grund === null, schemaVersion: 1, runId,
    ...
    ...(fenster ? { testfenster: W.beleg(fenster) } : {}) });
  try {
    ...
    const startfenster = F.pruefeStartfenster({ startUtc: start.toISOString(), dauerMinuten: 6,
      crons, maxLaufzeitMs: 8 * 60000 });
    fordere(startfenster.startErlaubt, "nachlauf-cronkonkurrenz");
    storage = deps.storage || require("./storage");
    const build = deps.build || require("./lage").buildLageBriefing;
    const pruefeBriefing = async (profile, id) => {
      fordere(typeof deps.pruefeBriefing === "function", "nachlauf-briefing-aussagenpruefung-fehlt");
      const check = await deps.pruefeBriefing(profile, id);
      fordere(check?.bereit === true && /^[a-f0-9]{64}$/.test(check.eingabeHash || "")
        && check.lageEingabe, "nachlauf-briefing-aussagenpruefung-fehlt");
      return check;
    };
    ...
    const snapshot = async () => {
      ...
      const bereinigt = fenster?.manifest.version === 2;
      const b = D.pruefeSnapshot(s, bereinigt ? "500-bereinigt-bestand" : "500-bestand");
      fordere(b.gesamt === (bereinigt ? 500 : 504) && b.aktiv === 500 && b.aktive.length === 495,
    ...
    const guard = async (ownTenant = null) => {
      ...
      const kosten = pruefeKosten(auth, counters[0].used, tag);
      fenster = await W.lese({ laufId: testfensterId, commit, get, jetzt: now, vorher: fenster });
      if (fenster) end = Math.min(end, Date.parse(fenster.manifest.endeAm));
      return kosten;
    };
    kostenVorher = await guard();
    const s = await snapshot();
    if (fenster) W.pruefe(fenster, { commit, jetzt: now(), profile: s.mandate });
    grundlinie = profilHash(s);
    // Betreiber 09.09.: saemtliche 500 Profile sind Testprofile, kein Sondervorrang.
    ids = fenster ? [...fenster.manifest.ids] : s.mandate.filter(m => m.aktiv).map(m => m.user_id).sort();
    ...
    for (const id of arbeitsIds) {
      ...
      const p = storage.fromMandateProfileRow(profileMap.get(id), mandateMap.get(id));
      // Ungepruefte Briefingaussagen werden durch einen neuen Lageabsatz nicht
      // richtig. Fehlendes/abgelehntes Urteil stoppt VOR Generator und Kosten.
      const briefingPruefung = await pruefeBriefing(p, id);
      const pruefeGleicheEingabe = async () => {
        const [mandate, identitaeten] = await Promise.all([
        ...
        fordere(mandate.length === 1 && D.hash(mandate[0]) === D.hash(mandateMap.get(id))
          && identitaeten.length === 1 && D.hash(identitaeten[0]) === D.hash(profileMap.get(id)),
        "nachlauf-zielprofil-veraendert");
        const frisch = await pruefeBriefing(p, id);
        fordere(frisch.eingabeHash === briefingPruefung.eingabeHash,
          "nachlauf-briefing-aussagenpruefung-fehlt");
      };
      const r = await build(p, { politicianId: id, missingOnly: true, repairIncomplete: true, costRunId: runId,
        briefingEingabe: briefingPruefung.lageEingabe,
        ...
        beforeGenerate: async (owner) => {
          ...
          await pruefeGleicheEingabe();
          fordere(now().getTime() + RESERVE_MS < end, "nachlauf-zeitbudget");
          freigegebeneModelle++;
        },
        beforeSave: async owner => {
          fordere(owner === id, "nachlauf-zielprofil-veraendert");
          await guard(id); await pruefeGleicheEingabe();
          if (fenster) fordere(now().getTime() < Date.parse(fenster.manifest.endeAm),
            "nachlauf-testfenster-geschlossen");
        }
      });
...
module.exports = { ausfuehren, pruefeKosten, pruefeConfig, pruefeArbeitsbeginn, leseTexte, getReader,
