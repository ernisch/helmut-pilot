...
const istQualitaetsfehler = grund => /^(?:nachlauf-textfehler-)?ai-text-(?:paragraph-count|empty-or-type|source-reference|visible-id|word-limit|quality-incomplete|source-support|evidence-quote|repetition)$/.test(grund || "");
...
function pruefeKosten(auth, counter, tag) {
  ...
  fordere(k.reservierungsluecke === 0 && k.aufrufbelege === k.reservierungen, "nachlauf-kosten-unklar");
  if (auth.testKostenTage?.[tag]) {
    try { Object.assign(k, require("./testkosten-budget").kontrolliere(auth, tag, k.unbekannteKosten, k.aufrufbelege)); }
...
async function ausfuehren({ commit, runId, confirmation, config,
  ...
  const bericht = () => ({ ok: grund === null, schemaVersion: 1, runId,
    ...
    freigegebeneModelle, results, grund, qualitaetsfehler, automatischeWiederholung: false,
  ...
  try {
    ...
    const arbeitsIds = [...ids].sort((a, b) => Number(vorhanden.has(a)) - Number(vorhanden.has(b)));
    for (const id of arbeitsIds) {
      ...
      const r = await build(p, { politicianId: id, missingOnly: true, repairIncomplete: true, costRunId: runId,
        ...
        beforeGenerate: async (owner) => {
          fordere(owner === id && freigegebeneModelle < 1000, "nachlauf-ziel-abweichend");
          await guard(id);
    ...
    if (begonnen) {
      const q = await storage.schreibeWarteschlangenLaufquittung({ process: PROCESS, runId, mode: "manual",
        ...
        gespeichert: results.filter(r => r.gespeichert).length,
        mandatsErgebnisse: ids.map(id => {
          const r = results.find(x => x.userId === id);
          return { mandatHash: D.hash(id), gestartet: r?.gestartet === true,
...
module.exports = { ausfuehren, pruefeKosten, pruefeConfig, leseTexte, istQualitaetsfehler, CONFIRM, PROCESS, BUDGET_MS, RESERVE_MS };
