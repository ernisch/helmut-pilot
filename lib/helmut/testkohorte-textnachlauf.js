...
async function ausfuehren({ commit, runId, confirmation, config,
  ...
  const bericht = () => ({ ok: grund === null, schemaVersion: 1, runId,
    ...
    funktionsnachweis500: false, kostenVorher, projektionsbeleg, atomarerUsdRiegel: false });
  try {
    ...
    for (const id of ids) {
      ...
      if (r.available && r.fromCache === false && r.paragraphs?.length) {
        ...
        const result = { userId: id, gestartet: true, gespeichert: true, generatedAt: saved.generated_at };
        results.push(result);
        ...
        results.push({ userId: id, gestartet: true,
