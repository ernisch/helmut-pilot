...
// --- Identitaet -------------------------------------------------------------
// Akademische/parlamentarische Titel, die aus dem Namen fuer den kanonischen
// Schluessel entfernt werden (Dubletten-Vermeidung: "Dr. Anna Muster" == "Anna Muster").
const TITLE_TOKENS = new Set([
  "dr", "prof", "professor", "professorin", "med", "jur", "rer", "nat", "phil",
  "pol", "ing", "dipl", "mba", "llm", "mult", "hc", "mdb", "mda", "freiherr",
  "freifrau", "graf", "graefin", "baron", "baronin", "von", "van", "zu", "der", "den"
]);
...
// Kanonischer Namensslug: Titel entfernt, Umlaute gefaltet, nur [a-z0-9-].
// "Prof. Dr. Änna von Müster-Schmidt (MdB)" -> "aenna-muster-schmidt"
// Hinweis: Adelspartikel/"von" werden als Titel-Token entfernt; das ist fuer die
// DUBLETTEN-Erkennung gewollt (mehr Zusammenfaelle), fuer die Anzeige gilt der Klarname.
...
// Bekannte stabile externe ID-Quellen (Reihenfolge = Vertrauensrang).
const EXTERNAL_ID_FIELDS = [
  ...
  { source: "wahlkreis_nr", fields: ["wahlkreisNr", "wahlkreis_nr", "constituencyNumber"] }
];
...
function computeSupplyOutlook(parts) {
  const { party, committees, packages } = parts;
  ...
  // laufzeitgebundene persoenliche Paket.
  const libraryNeeds = needed.filter((n) => n.category !== "persoenlich");
  const allNeededActive = libraryNeeds.length > 0 && libraryNeeds.every((n) => n.status === "aktiv");
...
function computeVersorgungsstatus(parts, opts = {}) {
  ...
  // Eine faktions-abgeleitete uneindeutige Partei (z. B. Fraktion "CDU/CSU" ohne Partei-Feld,
  // oder "fraktionslos") ist KEIN Datenfehler — die Fraktion ist gueltig.
  if (party.fromField && hasStr(party.raw) && !party.known) reviewFlags.push({ code: "partei-unbekannt", detail: party.raw });
...
function resolveMandate(profile = {}, opts = {}) {
  ...
  const supplyOutlook = computeSupplyOutlook({ party, committees, packages });
...
// echte Doppelanlagen (gleiche Person mehrfach) dagegen schon. Titel/Umlaute sind im Namensslug
// bereits gefaltet ("Dr. Anna Müster" == "Anna Muster").
