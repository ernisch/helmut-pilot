...
function buildClassificationPatch(ko = {}, deps = {}) {
  // SPRINT 19: das KO ist hier zugleich sein eigener Bestand. Traegt es bereits eine
  // ermittelte Ebene, wird sie wiederverwendet — ein Backfill kann eine gespeicherte
  // Ebene damit selbst dann nicht mehr auf 'unknown' herabstufen, wenn er
  // versehentlich auf schon klassifizierte Datensaetze angesetzt wird (fail closed).
  const cls = classifyKnowledgeObject(ko, {}, { bestand: ko });
  const patch = {
...
// Technischer Feature-/Merkmalsvektor (Token-Hash), KEIN semantisches Embedding.
// Wird in die (Legacy-benannte) Spalte "embedding" geschrieben.
function defaultEmbed(ko) {
  const { computeFeatureVectorForKnowledgeObject } = require("./matching");
  return computeFeatureVectorForKnowledgeObject(ko);
}
