...
// Befund 3 des Korrektursprints am selben Tag: Quelle ist der ATOMARE ZÄHLER) ────────
...
//
// QUELLE (Befund 3): NICHT canSpendLlm — das liest das verlustbehaftete
// llmUsage-Log (~16 % Verlust dokumentiert) und sieht zu wenig Verbrauch, die
// Prüfung würde zu spät blockieren. Maßgeblich ist der atomare Tageszähler
// `llm_budget_counters` (UTC-Tag, Scope 'global'), rein lesend über
// `leseTageszaehler` (storage.leseLlmTageszaehler: { ok, used, limit, remaining }).
// FAIL CLOSED: ein unlesbarer Zähler oder ein nicht bezifferbarer Reststand
// blockiert VOR der teuren Lesearbeit — Rückstandsabbau ist immer aufschiebbar.
// Der Lauf-Wächter (baueRueckstandsWaechter) behält bewusst seinen bisherigen
// Leser als ZUSATZ-Sicherung; die harte Garantie bleibt in jedem Fall die
// atomare Reservierung am Choke-Point (helmut_reserve_llm_call).
async function vorabBodenPruefung({ leseTageszaehler, budgetBoden } = {}) {
  if (typeof leseTageszaehler !== "function") {
    return { erlaubt: false, grund: "vorab-kein-zaehlerleser" };
  }
  const boden = Number.isFinite(Number(budgetBoden)) && Number(budgetBoden) >= 0
    ? Math.floor(Number(budgetBoden))
    : rueckstandBudgetBoden();
  let zaehler = null;
  try { zaehler = await leseTageszaehler(); } catch (_) { zaehler = null; }
  if (!zaehler || zaehler.ok !== true) {
    return {
      erlaubt: false,
      grund: `vorab-zaehler-nicht-lesbar:${String((zaehler && zaehler.fehler) || "unbekannt").slice(0, 120)}`,
      used: (zaehler && zaehler.used) ?? null, limit: (zaehler && zaehler.limit) ?? null, remaining: null, boden
    };
  }
  // Dieselbe Falle wie PR #283 §6.1: Number(null) === 0 — unbekannt bleibt unbekannt.
  const rest = (zaehler.remaining === null || zaehler.remaining === undefined || zaehler.remaining === "")
    ? NaN
    : Number(zaehler.remaining);
  if (!Number.isFinite(rest)) {
    return { erlaubt: false, grund: "rueckstand-budget-unbekannt", used: zaehler.used ?? null, limit: zaehler.limit ?? null, remaining: null, boden };
  }
  if (rest <= boden) {
    return { erlaubt: false, grund: "rueckstand-budget-boden-erreicht", used: zaehler.used ?? null, limit: zaehler.limit ?? null, remaining: rest, boden };
  }
  return { erlaubt: true, used: zaehler.used ?? null, limit: zaehler.limit ?? null, remaining: rest, boden };
}
