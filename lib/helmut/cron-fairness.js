...
// `zustandGeladen:false` / `zustandFehler`. Ausnahme: persistenzPflicht im
// direkten Lagepfad beendet die Facharbeit ohne bestaetigte Fortsetzung.
...
// Version 3 schuetzt 500 Laufplaetze und den neuen Nichtbeginn-Ausgang vor
// aelteren Schreibern, die diese Daten sonst auf 200 kuerzen bzw. entfernen.
const FAIRNESS_VERSION = 3;
...
// OBERGRENZE JE MANDAT (Befund 05.09.2026, `/api/cron/lage-check` 10:04 UTC).
// ============================================================================================
// Bis hierher prüfte die Schleife vor jedem Mandat NUR, ob überhaupt noch `reserveMs` Restzeit
// da ist — sie gab dem Mandat danach aber das GESAMTE verbleibende Budget. Ein einziges langsames
// Mandat konnte damit alle übrigen aushungern: belegt am 05.09., als das erste von fünf Mandaten
// ~225 s verbrauchte und vier mit `zeitbudget` übersprungen wurden (1 von 5 verarbeitet).
// Die Fairnessrotation verteilte diesen Rückstand danach gerecht — sie verhinderte ihn nicht.
//
// Jetzt bekommt jedes Mandat eine SCHEIBE: den fairen Anteil an der Restzeit, nach unten durch
// `mindestMs` und nach oben durch die Restzeit begrenzt. Die Scheibe ist eine OBERGRENZE, keine
// Reservierung — ein Mandat, das früher fertig ist, gibt seine Restzeit sofort an das nächste
// zurück, weil die Scheibe in JEDEM Schleifendurchlauf aus der TATSÄCHLICHEN Restzeit neu
// berechnet wird. Sie erhöht kein Budget und ändert keine Cron-Zeit.
//
// Untergrenze der Scheibe ist die BESTEHENDE `reserveMs` — es kommt kein zweiter Zeitknopf
// dazu, und die Bedingung, ab wann ein Mandat gar nicht mehr begonnen wird, bleibt WÖRTLICH
// dieselbe wie vorher (`Restzeit >= reserveMs`). Aus dieser Untergrenze folgt zugleich die
// Zahl der Mandate je Lauf, k ≈ Restzeit/reserveMs, und damit die nachrechenbare Obergrenze
// ceil(n/k) der Fairnessrotation.
...
const AUSGANG_FORTSETZUNG_OFFEN = "fortsetzung-nicht-bestaetigt";
...
// Vollstaendige Planung und Ausgaenge fuer die freizugebende 500er Zielmenge.
const MAX_LAUF_MANDATE = 500;
...
// REINE Berechnung der Mandatsscheibe — kein IO, kein Zustand, ohne Netz testbar.
//
//   rest      = deadline − jetzt
//   fairMs    = floor(rest / offen)                 (offen = dieses Mandat eingeschlossen)
//   scheibeMs = min(rest, max(mindestMs, fairMs))
//
// `startbar` ist genau dann falsch, wenn weniger als `mindestMs` Restzeit bleibt — das ist
// WÖRTLICH die bisherige Bedingung `now() + reserveMs > deadline`, nur ohne Vorzeichenwechsel
// hingeschrieben. Ein nicht begonnenes Mandat wird NICHT als versucht vermerkt und steht im
// nächsten Lauf vorn; die Bedeutung von `reason: "zeitbudget"` bleibt unverändert.
//
// `offen <= 0` (defensiv) wird wie `offen === 1` behandelt: die volle Restzeit.
function mandatsScheibeMs({
  jetztMs = 0,
  deadlineMs = 0,
  offen = 1,
  mindestMs = DEFAULT_TENANT_RESERVE_MS
} = {}) {
  // ROH, NICHT geklemmt: nur so bleibt „die Frist ist bereits verstrichen" (negativ) von
  // „genau erreicht" (0) unterscheidbar. `startbar` prueft gegen den ROHwert und ist damit
  // WÖRTLICH die bisherige Bedingung `!(jetzt + mindestMs > deadline)`. Gerechnet wird
  // danach mit dem geklemmten Wert, damit keine negative Scheibe entsteht.
  const restRoh = Math.floor(Number(deadlineMs) - Number(jetztMs));
  const rest = Math.max(0, restRoh);
  const anzahl = Math.max(1, Math.floor(Number(offen) || 0));
  // Untergrenze 0 ist zulaessig und bedeutet „kein Mindestbedarf" — nur so bleibt die
  // Startbedingung bei `reserveMs = 0` byte-identisch zum bisherigen Verhalten (dort wurde
  // auch bei Restzeit 0 noch begonnen).
  const untergrenze = Math.max(0, Math.floor(Number(mindestMs) || 0));
  const fairMs = Math.floor(rest / anzahl);
  const scheibeMs = Math.min(rest, Math.max(untergrenze, fairMs));
  return {
    restMs: rest,
    fairMs,
    scheibeMs,
    offen: anzahl,
    startbar: restRoh >= untergrenze,
    // Absolute Frist, die der Aufrufer an seine eigene Zeitbudgetierung durchreichen kann.
    fristMs: Number(jetztMs) + scheibeMs,
    // true = die Untergrenze hat den fairen Anteil angehoben; dann passen nicht alle offenen
    // Mandate in diesen Lauf und der Rest wird über die Rotation nachgeholt.
    gekuerzt: fairMs < untergrenze
  };
}
...
function planTenantOrder({
  ...
  planbar.sort((a, b) => {
    ...
    // bisherige Ordnung entscheidet unveraendert weiter. Der Lagepfad setzt
    // gleichberechtigt und laesst ausschliesslich die Versuchshistorie entscheiden.
    ...
    if (!gleichberechtigt && aReal !== bReal) return aReal ? -1 : 1;
...
function laufAbschlussPatch({
  cronName, laufId, startAt, nowMs = Date.now(), zeitbudget = [], fortsetzungOffen = [], kapazitaet = 0,
  obergrenzeLaeufe = null, naechstesMandat = null, zustandGeladen = true, zustandFehler = null
} = {}) {
  const offen = kennungsListe(zeitbudget);
  const unbestaetigt = kennungsListe(fortsetzungOffen);
  const ausgaenge = {};
  for (const tenantId of unbestaetigt) ausgaenge[tenantId] = AUSGANG_FORTSETZUNG_OFFEN;
  for (const tenantId of offen) ausgaenge[tenantId] = AUSGANG_ZEITBUDGET;
  return laufPatch(cronName, {
    laufId,
    status: offen.length || unbestaetigt.length ? LAUF_TEILWEISE : LAUF_ABGESCHLOSSEN,
    startAt: new Date(startAt).toISOString(),
    standAt: new Date(nowMs).toISOString(),
...
function rekonstruiereLauf(state, cronName, { nowMs = Date.now(), staleMs = DEFAULT_STALE_CLAIM_MS } = {}) {
  ...
  return {
    ...
    zeitbudget,
    fortsetzungOffen: mitAusgang(AUSGANG_FORTSETZUNG_OFFEN),
    nichtBegonnen: ohneAusgang.concat(zeitbudget, mitAusgang(AUSGANG_FORTSETZUNG_OFFEN)).filter((id, i, alle) => alle.indexOf(id) === i),
    kapazitaet,
...
async function runTenantsFairly({
  ...
  // Der Lagepfad darf ohne bestaetigte Fortsetzung keine Facharbeit beginnen.
  if (persistenzPflicht && !fair) {
    zustandGeladen = false;
    zustandFehler = "fairness-deaktiviert";
  }
  ...
  const planung = fair
    ? planTenantOrder({
      ...
      nowMs: now(),
      staleMs,
      tiebreakBucketMs, gleichberechtigt
    })
    : (() => {
  ...
  const speichern = async (patch, { pruefen = false } = {}) => {
    if (persistenzPflicht && !zustandGeladen) return { ok: false, remote: null };
  ...
  // Zahl der ab hier noch NICHT begonnenen Kandidaten — Nenner der Mandatsscheibe.
  const nochOffen = (abIndex) => {
    let offen = 0;
    for (let j = abIndex; j < planung.plan.length; j += 1) {
      if (!begonnen.has(planung.plan[j].politicianId)) offen += 1;
    }
    return offen;
  };

  for (let index = 0; index < planung.plan.length; index += 1) {
    const kandidat = planung.plan[index];
    const tenantId = kandidat.politicianId;
    if (begonnen.has(tenantId)) continue;
    if (persistenzPflicht && (!zustandGeladen || zustandFehler)) {
      results.push({ politicianId: tenantId, failed: true, skipped: true, reason: "fortsetzung-nicht-bestaetigt" });
      continue;
    }
    ...
    // Die Scheibe wird in JEDEM Durchlauf aus der tatsächlichen Restzeit neu berechnet: die
    // ungenutzte Zeit eines schnellen Mandats fließt damit sofort an das nächste zurück.
    const scheibe = mandatsScheibeMs({
      jetztMs: now(), deadlineMs: deadline, offen: nochOffen(index), mindestMs: reserveMs
    });
    if (!scheibe.startbar) {
    ...
    const fernClaim = registrierung.remote && entryOf(registrierung.remote, cronName, tenantId);
    if (persistenzPflicht && (!registriert || !fernClaim
      || fernClaim.letzteLaufkennung !== runId || fernClaim.status !== STATUS_LAUFEND)) {
      begonnen.delete(tenantId);
      zustandFehler = zustandFehler || "fortsetzung-nicht-bestaetigt";
      results.push({ politicianId: tenantId, failed: true, skipped: true, reason: "fortsetzung-nicht-bestaetigt" });
      continue;
    }
    let erfolg = false;
    ...
    let fehler = null;
    try {
      // Die Scheibe geht als ZWEITES Argument an den Aufrufer. Bestehende Aufrufer, die nur
      // `(tenantId)` entgegennehmen, bleiben unveraendert — JavaScript ignoriert das
      // zusaetzliche Argument. Wer sie nutzt, budgetiert seine eigene Arbeit damit.
      ergebnis = await perTenant(tenantId, {
        scheibeMs: scheibe.scheibeMs,
        fristMs: scheibe.fristMs,
        offen: scheibe.offen,
        restMs: scheibe.restMs,
        gekuerzt: scheibe.gekuerzt
      });
      erfolg = true;
    } catch (error) {
  ...
  const nachher = fair
    ? planTenantOrder({ cronName, tenantIds, state: zustand, nowMs: now(), staleMs, tiebreakBucketMs, gleichberechtigt })
    : planung;
  ...
  const abschluss = await speichern(laufAbschlussPatch({
    ...
    fortsetzungOffen: results.filter(r => r.reason === AUSGANG_FORTSETZUNG_OFFEN).map(r => r.politicianId),
    kapazitaet,
    obergrenzeLaeufe: Number.isFinite(grenze) ? grenze : null,
  ...
  return {
    ...
    fairness: {
      ...
      fortschrittsgarantie: Number.isFinite(grenze) && kapazitaet > 0
        && (!persistenzPflicht || (zustandGeladen && !zustandFehler)),
      ohneFortschritt,
      obergrenzeLaeufe: Number.isFinite(grenze) && (!persistenzPflicht || (zustandGeladen && !zustandFehler)) ? grenze : null,
      zustandGeladen,
      zustandFehler,
