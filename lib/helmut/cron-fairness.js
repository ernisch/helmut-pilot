...
function normalizeState(raw = {}) {
  const out = { version: FAIRNESS_VERSION, crons: {} };
...
async function runTenantsFairly({
  ...
  for (const kandidat of planung.plan) {
    ...
    const registriert = await speichern(claimPatch({ cronName, tenantId, runId, nowMs: versuchMs, vorher }));
    let erfolg = false;
    let ergebnis = null;
    let fehler = null;
    try {
  ...
  return {
    ...
    fairness: {
      ...
      obergrenzeLaeufe: fairnessBound(planung.planbar, begonnen.size),
      zustandGeladen,
      zustandFehler
    }
  };
