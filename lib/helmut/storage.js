...
async function listKoDocuments(knowledgeObjectId, limit = 40) {
  ...
  try {
    ...
    const cols = "id,title,summary,quellenauszug_beleg:raw->helmutQuellenkontext,published_at,url,canonical_url,content_hash,source_id,source_name,document_type,link_type,confidence";
    const rows = await supabaseRequest(
...
async function listRecentRawDocuments(limit = 500, days = 30) {
  ...
  try {
    ...
    return await fetchRawDocumentPages(
      `/rest/v1/raw_documents?select=id,content_hash,canonical_url,title,summary,quellenauszug_beleg:raw->helmutQuellenkontext,url,source_name,source_id,published_at,created_at&created_at=gte.${encodeURIComponent(cutoffIso)}&order=created_at.desc`,
      limit
    );
  } catch (error) {
...
async function listRawDocuments({ limit = 800, days = 45 } = {}) {
  ...
  try {
    ...
    const cols = "id,content_hash,canonical_url,title,summary,quellenauszug_beleg:raw->helmutQuellenkontext,url,source_name,source_id,source_type,confidence,link_type,published_at,retrieved_at,created_at,document_type,wahlperiode";
    return await fetchRawDocumentPages(
...
async function getSourcesForVorgang(vorgangId, { limit = 40, lageQuellen = null, lageKoId = null } = {}) {
  ...
  try {
    ...
    const select = "raw_documents(id,title,url,canonical_url,source_name,source_type,published_at,document_type,link_type,confidence,summary,quellenauszug_beleg:raw->helmutQuellenkontext)";
    const rows = await supabaseRequest(
...
async function getGebundeneLageQuellen(vorgangId, koId, erwartet) {
  try {
    ...
    const select = "knowledge_object_id,raw_document_id,raw_documents!inner(id,title,url,canonical_url,source_name,source_type,published_at,document_type,link_type,confidence,summary,quellenauszug_beleg:raw->helmutQuellenkontext)";
...
async function getRawDocumentsByIds(ids = []) {
  ...
  if (!liste.length) return [];
  const cols = "id,content_hash,canonical_url,title,summary,quellenauszug_beleg:raw->helmutQuellenkontext,url,source_name,source_id,source_type,"
