...
// Markiert Ergebniszeilen als abgeloest, die NICHT mehr in der aktuellen
// Trefferliste stehen. Es wird NIE geloescht — die Zeile bleibt mit allen
// alten Werten erhalten und traegt nur zusaetzlich aktuell=false.
// Leere Trefferliste = bewusster No-Op: ein Lauf ohne Treffer soll den letzten
// belegten Stand nicht pauschal entwerten.
async function markMatchingResultsSuperseded({ userId = null, keepKnowledgeObjectIds = [], abgeloestAm = null } = {}) {
  const uid = assertTenant(userId, "markMatchingResultsSuperseded");
  if (!v3StoreReady()) throw new Error("matching_results: V3-Store nicht bereit");
  const keep = [...new Set((Array.isArray(keepKnowledgeObjectIds) ? keepKnowledgeObjectIds : [])
    .map((v) => String(v == null ? "" : v)).filter(Boolean))];
  if (!keep.length) return { abgeloest: 0, skipped: "leere-trefferliste" };
  const liste = keep.map((v) => `"${v.replace(/"/g, '""')}"`).join(",");
  const endpoint = `/rest/v1/matching_results?user_id=eq.${encodeURIComponent(uid)}`
    + `&aktuell=is.true`
    + `&knowledge_object_id=not.in.(${encodeURIComponent(liste)})`;
  const rows = await supabaseRequest(endpoint, {
    method: "PATCH",
    headers: { Prefer: "return=representation" },
    body: JSON.stringify({ aktuell: false, abgeloest_am: abgeloestAm || new Date().toISOString() })
  });
  return { abgeloest: Array.isArray(rows) ? rows.length : 0 };
}
