...
// P0-Fix (Nutzer & Kunden"-Timeout, Teil 2): der Haupt-Store retryt transiente
// Supabase-Fehler (withStoreRetry, siehe readSupabaseStore) — der Auth-Store tat
// das bisher NICHT, obwohl er ueber dieselbe supabaseRequest-Anbindung laeuft und
// genau denselben Fehlerklassen (Timeout/5xx/Netz) ausgesetzt ist. Ein einzelner
// transienter Blip liess dadurch JEDEN auth-abhaengigen Aufruf (Login, Admin-
// Uebersicht, Nutzer & Kunden) sofort und ungebremst scheitern. Gleiche Garantie
// wie beim Haupt-Store: 4xx (permanent) wird sofort durchgereicht, nur transiente
// Fehler bekommen einen begrenzten Backoff-Retry.
async function readAuthStore() {
  if (useSupabase()) {
    const rows = await withStoreRetry(
      () => supabaseRequest(`/rest/v1/helmut_store?id=eq.${encodeURIComponent(authStoreId)}&select=data`),
      { label: "auth-store-read", storeKey: "auth" }
    );
    const row = Array.isArray(rows) ? rows[0] : null;
...
async function writeAuthStore(store) {
  ...
  if (useSupabase()) {
    await withStoreRetry(
      ...
      { label: "auth-store-write", storeKey: "auth" }
    );
    return normalized;
  }
  fs.mkdirSync(dataDir, { recursive: true });
...
// deps.store: optionaler, bereits gelesener Auth-Store-Snapshot (P0-Fix "Nutzer &
// Kunden"-Timeout — siehe buildAdminOverview/buildSourceArchitectureReport).
async function getLlmUsage(politicianId = null, limit = 200, deps = {}) {
  const store = deps.store || await readAuthStore();
...
// store: optionaler, bereits gelesener Auth-Store-Snapshot (P0-Fix "Nutzer &
// Kunden"-Timeout — siehe getAdminPeriodStats/buildAdminOverview). Ohne Angabe
// unveraendertes Verhalten (eigener frischer readAuthStore()-Aufruf).
async function getAdminStatsCosts({ days = 30, referenceIso = null, store: preStore = null } = {}) {
  const store = preStore || await readAuthStore();
...
// store: siehe getAdminStatsCosts oben — optionaler geteilter Snapshot.
async function getAdminCostsPerUser({ days = 30, store: preStore = null } = {}) {
  const store = preStore || await readAuthStore();
  const allUsage = Array.isArray(store.llmUsage) ? store.llmUsage : [];
...
// deps.authStore: optionaler geteilter Auth-Store-Snapshot, an beide Unterrufe
// weitergereicht (P0-Fix "Nutzer & Kunden"-Timeout, siehe buildAdminOverview).
async function getAdminPeriodStats(days = 30, deps = {}) {
  const cutoffMs = Date.now() - days * 24 * 60 * 60 * 1000;
  const cutoffIso = new Date(cutoffMs).toISOString();

  const [perUserData, aiData] = await Promise.all([
    getAdminCostsPerUser({ days, store: deps.authStore }),
    getAdminStatsCosts({ days, store: deps.authStore })
  ]);
