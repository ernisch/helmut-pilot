  ...
  const run = async () => {
    for (let attempt = 0; attempt < 8; attempt += 1) {
      ...
      try {
        ...
        if (error?.code !== "AUTH_STORE_CONFLICT" || attempt === 7) throw error;
        await new Promise((resolve) => setTimeout(resolve, Math.min(250, 10 * 2 ** attempt) + Math.floor(Math.random() * 10)));
      }
    }
  };
