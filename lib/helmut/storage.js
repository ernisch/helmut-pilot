...
async function listFullProfilesFromDb(deps = {}) {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      if (mandateRow && mandateRow.geloescht_at) continue;
...
// DB-Modus: relational zuerst; SQL-Fehler -> Blob-Fallback (Stufe D) bzw. []
// (Stufe E, Exklusivmodus — kein Blob mehr).
async function listFullProfiles(deps = {}) {
  if (profileDbModeEnabled()) {
    const fromDb = await listFullProfilesFromDb(deps);
    if (fromDb) return fromDb;
    if (profileDbExclusiveEnabled()) return [];
  }
  const store = await readStore();
...
async function listProfiles(deps = {}) {
  if (profileDbModeEnabled()) {
    const fromDb = await listFullProfilesFromDb(deps);
    if (fromDb) return fromDb.map(toProfileSummary);
    if (profileDbExclusiveEnabled()) return [];
  }
  const store = await readStore();
  return Object.values(store.profiles || {}).map(toProfileSummary);
}
...
async function exportProfileData(politicianId) {
  ...
  // SQL-aware (Exklusivmodus): im relational-only Modus liegt das Profil NICHT im
  // Blob -> ueber getProfile lesen (SQL-first, Blob-Fallback). Flag-off/Dual-Write
  // ergibt denselben Blob-Wert wie zuvor (byte-kompatibel).
  const profile = (await getProfile(politicianId)) || store.profiles?.[politicianId] || null;
...
async function deleteProfileData(politicianId) {
  const [store, pStore] = await Promise.all([readStore("main"), readStore(pKey(politicianId))]);
  // SQL-aware (Exklusivmodus): das Profil zur rawItem-Zuordnung ueber getProfile
  // beziehen, damit die Loeschung auch relational-only vollstaendig bleibt. Die
  // relationalen Profilzeilen selbst loescht deleteProfileDataV3 (Cascade) weiter unten.
  const profile = (await getProfile(politicianId)) || store.profiles?.[politicianId] || null;
