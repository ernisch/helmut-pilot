...
// bisher (nur globaler Scope) — der laufende Pilot (cem-ince) bleibt unberuehrt,
...
// HELMUT_TENANT_LLM_LIMITS='{"cem-ince":150,"tenant-x":30}'. Ungueltiges JSON ->
...
function reserveTenantLocally(day, tenantId, tenantMax, usageTodayFn) {
  const work = async () => {
    ...
    if (!counter) {
      ...
      if (localTenantLlmCounters.size > 8) {
        const oldest = [...localTenantLlmCounters.keys()].sort()[0];
        if (oldest !== key) localTenantLlmCounters.delete(oldest);
      }
    }
