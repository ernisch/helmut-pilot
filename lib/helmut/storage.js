...
async function savePushSubscription(politicianId, subscription, meta = {}) {
  ...
  if (!endpoint) {
    // Ohne .statusCode faengt respondError() das als 500 "Interner Serverfehler" ab und
    // schreibt es ins Systemfehler-Log - fuer eine simple Client-Eingabevalidierung (fehlendes
    // Feld) sowohl der falsche Statuscode als auch unnoetiges Log-Rauschen.
    const error = new Error("Push subscription endpoint missing");
    error.statusCode = 400;
    error.publicMessage = "Push-Abo ohne endpoint ist ungültig.";
    throw error;
  }
  const entry = {
...
async function exportProfileData(politicianId) {
  ...
  return {
    ...
    feedback: profileRows(store.feedback, politicianId)
  };
}
...
async function deleteProfileData(politicianId) {
  ...
  // Feedback liegt im geteilten "main"-Store (saveFeedback/listFeedback), nicht im Politiker-Blob
  // unten — ohne diesen Filter ueberlebte eingereichtes Feedback eine "Daten loeschen"-Anfrage.
  store.feedback = withoutProfileRows(store.feedback, politicianId);
  await writeStore(store, "main");
