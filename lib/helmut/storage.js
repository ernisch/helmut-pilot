...
// Thundering-Herd-Schutz: waehrend ein Cache-Miss-Fetch fuer einen storeKey laeuft,
// docken weitere gleichzeitige readStore()-Aufrufe fuer DENSELBEN Key an dieselbe
// Promise an, statt je einen eigenen parallelen Supabase-Request auszuloesen (z.B.
// mehrere App-Starts im selben Moment beim Morgen-Peak, auf derselben warmen
// Instanz).
const storeFetchInFlight = new Map(); // storeKey → Promise<data>
...
async function readStore(storeKey = "main") {
  ...
  const inFlight = storeFetchInFlight.get(storeKey);
  if (inFlight) return inFlight;
  const fetchPromise = Promise.resolve(useSupabase() ? readSupabaseStore(storeKey) : readLocalStore(storeKey))
    .then((store) => {
      storeCacheMap.set(storeKey, { data: store, at: Date.now() });
      return store;
    })
    .finally(() => {
      storeFetchInFlight.delete(storeKey);
    });
  storeFetchInFlight.set(storeKey, fetchPromise);
  return fetchPromise;
}
...
// Perf-Fix: der Auth-Store (Sessions/Assignments/Audit/LLM-Usage/Pipeline-Locks)
// wurde bisher bei JEDEM Aufruf frisch aus Supabase gelesen — anders als der
// Content-Blob (s.o.) ohne jede Cache-Stufe. Da resolveSession() ihn bei jedem
// authentifizierten Request liest, war das der Supabase-Call, der garantiert auf
// jedem Request lag. Gleiche kurze TTL-Cache-Strategie wie beim Content-Blob,
// plus In-Flight-Sharing (Thundering-Herd-Schutz, s. readStore). TTL bewusst
// kuerzer (Default 3s statt 10s), weil Session-/Audit-Daten schneller aktuell
// sein sollen. Pipeline-Locks sind korrektheitskritisch (Doppel-Crawl/-Briefing
// vermeiden) und lesen deshalb ueber bypassCache IMMER frisch, nie aus dem Cache.
const authStoreCacheTtlMs = Number(process.env.HELMUT_AUTH_STORE_CACHE_MS || 3000);
let authStoreCache = null; // { data, at }
let authStoreFetchInFlight = null;

async function readAuthStore({ bypassCache = false } = {}) {
  if (!bypassCache && authStoreCacheTtlMs > 0 && authStoreCache && Date.now() - authStoreCache.at < authStoreCacheTtlMs) {
    return authStoreCache.data;
  }
  if (!bypassCache && authStoreFetchInFlight) return authStoreFetchInFlight;
  const fetchPromise = (async () => {
    if (useSupabase()) {
      const rows = await supabaseRequest(`/rest/v1/helmut_store?id=eq.${encodeURIComponent(authStoreId)}&select=data`);
      const row = Array.isArray(rows) ? rows[0] : null;
      return row?.data ? { ...defaultAuthStore(), ...row.data } : defaultAuthStore();
    }
  ...
  if (!bypassCache) authStoreFetchInFlight = fetchPromise.finally(() => { authStoreFetchInFlight = null; });
  const data = await fetchPromise;
  if (!bypassCache) authStoreCache = { data, at: Date.now() };
  return data;
}
...
async function writeAuthStore(store) {
  ...
  if (useSupabase()) {
    ...
    authStoreCache = { data: normalized, at: Date.now() };
    return normalized;
  }
  fs.mkdirSync(dataDir, { recursive: true });
...
async function acquirePipelineLock(jobName, ttlMs = 10 * 60 * 1000) {
  try {
    // bypassCache: Locks sind korrektheitskritisch (Doppel-Crawl/-Briefing =
    // doppelte Kosten) — hier zaehlt Aktualitaet mehr als der Perf-Gewinn des
    // Auth-Store-Caches.
    const store = await readAuthStore({ bypassCache: true });
...
async function getRenderedBriefingV3(userId, slot, day) {
  ...
  try {
    ...
    const rows = await supabaseRequest(`/rest/v1/briefings?id=eq.${encodeURIComponent(id)}&select=*&limit=1`);
    return Array.isArray(rows) && rows.length ? rows[0] : null;
  } catch (error) {
