...
async function leseLlmUsageRelational({ politicianId = null, seitIso = null, limit = 200 } = {}, deps = {}) {
  ...
  if (politicianId) teile.push(`or=(politician_id.eq.${encodeURIComponent(politicianId)},user_id.eq.${encodeURIComponent(politicianId)})`);
...
// Tagesmaximum abgezogen — ABER NUR fuer mandatsgebundene Aufrufe, die einem
// SYNTHETISCHEN Profil zuzuordnen sind oder deren Zuordnung fehlt (fail closed).
// Reale Mandate und GETEILTE Arbeit (Verstehen, Backfills) sehen unveraendert
// dasselbe Maximum wie bisher. Die Reserve liegt IM Deckel und wird NIE addiert.
//
...
// Zusaetzlich ist er in der heutigen Production strukturell wirkungslos, weil
// dort 0 synthetische Mandatszeilen existieren.
...
async function reserveLlmCall(opts = {}) {
  ...
  // Vorrangreserve der realen Mandate: gilt NUR fuer synthetische bzw. nicht
  // zuordenbare mandatsgebundene Aufrufe (siehe llmVorrangAbzug). Fuer reale
  // Mandate und geteilte Arbeit ist der Abzug 0 — deren Maximum ist unveraendert.
  const vorrang = llmVorrangAbzug(callType, opts.politicianId ?? null, opts.env || process.env);
  const effectiveMax = Math.max(0, (priority ? limit : limit - reserve) - vorrang.abzug);
