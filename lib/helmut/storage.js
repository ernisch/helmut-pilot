...
async function ladeDedupBestandsfenster() {
  // `order=created_at.desc` macht die Offset-Paginierung deterministisch (ohne Ordnung
  // waere die Seitenfolge undefiniert und Zeilen koennten doppelt/gar nicht erscheinen).
  ...
  for (; seiten < maxSeiten; seiten += 1) {
    const rows = await supabaseRequest(
      ...
      `&created_at=gte.${stichtag}&order=created_at.desc` +
...
async function markUnderstandingGateGeparkt(vorgangId) {
  ...
  try {
    await supabaseRequest(
      ...
      { method: "PATCH", headers: { Prefer: "return=minimal" }, body: JSON.stringify({ understanding_status: "gate-geparkt", updated_at: new Date().toISOString() }) }
    );
    return { ok: true };
  } catch (error) {
...
// ── DRAIN-BILANZ (PR-C, Kapazitätssprint 2026-08-31): drei fail-safe Lesegroessen ────
...
// statt behauptet. Alle drei liefern bei Nichtmessbarkeit null (nie eine erfundene 0,
// CLAUDE.md §4.4); der Bericht zeigt dann "?". Rein lesend, keine neue Tabelle.

// Gate-wuerdige ANKUNFT der letzten `stunden`: verschiedene Rohdokumente, deren
// (Schatten- oder Arm-)Gate-Entscheidung verstehen/zurueckstellen war. Zaehlt
// distinct raw_document_id — Mehrfachbewertungen desselben Dokuments (Schattenlaeufe
// bewerten je Lauf) zaehlen nicht doppelt.
async function zaehleGateWuerdigeAnkunft(stunden = 24) {
  ...
  try {
    const rows = await supabaseRequest(
      "/rest/v1/gate_shadow_events?select=raw_document_id"
      + `&gate_decision=in.(verstehen,zurueckstellen)&created_at=gte.${encodeURIComponent(seit)}`
      + "&limit=10000"
    );
    if (!Array.isArray(rows)) return null;
    return new Set(rows.map((r) => r && r.raw_document_id).filter(Boolean)).size;
  } catch {
    return null;
  }
}

// ABFLUSS der letzten `stunden`: Vorgaenge, deren Verstehen abgeschlossen wurde.
// Ehrliche Benennung: das sind ALLE Abschluesse (bei scharfem Gate per Konstruktion
// nur gate-wuerdige; im Schattenbetrieb eine Obergrenze der gate-wuerdigen Menge).
...
module.exports = {
  ...
  countGateGeparkt,
  // PR-C Drain-Bilanz (2026-08-31): drei fail-safe Lesegroessen (null = nicht messbar)
