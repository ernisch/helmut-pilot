...
async function persistRawDocumentsDeduped(items = []) {
  ...
  const zuSchreiben = plan.persists.map((doc) => {
    const stand = require("./bundestag-artikelstand").leseArtikelstand(doc);
    const row = {
      ...doc,
      // Ein versionierter Artikelstand ist nie das kanonische URL-Ziel: die Adresse bleibt
      // der unversionierten Quelle zugeordnet, sonst wuerde sie den Stand verschlucken.
...
// Gebuendelte Neuanlage der vom globalen Dedup-Plan als neu erkannten Dokumente.
// Der gelesene Bestand ist begrenzt und kann mit einem anderen Lauf konkurrieren.
// Deshalb auch bei gleicher ID atomar NICHT ueberschreiben: sonst koennte der neue
// Quellenkontext einen bereits bestaetigten Auszug von seiner Herkunft trennen.
//
...
// aufzufuellen. Der Einzelrueckfall behaelt denselben atomaren Anlageschutz.
//
...
async function saveRawDocumentsGebuendelt(rows = []) {
  ...
  const einfuegen = async (spalten) => {
    const inserted = await supabaseRequest("/rest/v1/raw_documents?on_conflict=id&select=id", {
      method: "POST",
      headers: { Prefer: "resolution=ignore-duplicates,return=representation" },
      body: JSON.stringify(spalten)
    });
    if (!Array.isArray(inserted)) throw new Error("Rohdokument-Neuanlage ohne bestaetigte Rueckgabe");
    const expected = new Set(spalten.map(row => row.id));
    const confirmed = new Set(inserted.map(row => row && row.id));
    if (confirmed.size !== inserted.length || [...confirmed].some(id => !expected.has(id))) {
      throw new Error("Rohdokument-Neuanlage mit widerspruechlichen Kennungen");
    }
  ...
  for (const gruppe of gruppen.values()) {
    for (let i = 0; i < gruppe.length; i += RAW_DOCUMENT_BULK_CHUNK) {
      ...
      try {
        anfragen += 1;
        persisted += await einfuegen(block.map((b) => b.spalten));
      } catch (error) {
        ...
        for (const b of block) {
          ...
          einzeln += 1;
          try {
            persisted += await einfuegen([b.spalten]);
          } catch (singleError) {
            console.error(`[v3Store] raw_documents Einzelanlage fehlgeschlagen: ${singleError.message}`);
          }
        }
      }
    }
...
//
// Die historische Vorrangreserve schuetzt jetzt ALLE mandatsgebundenen
// Aufrufe gleich. Die GETEILTE
// Arbeit (Verstehen, Backfills) IST betroffen und behaelt mindestens die
// Verstehens-Reserve (llmVorrangUntergrenze). Beide Reserven liegen IM Deckel;
// der Geldriegel ist davon unabhaengig. Fehlende Kennungen bleiben konservativ.
// Ohne Umgebungswert ist der Abzug 0. Kein Sondermaximum fuer reale Profile.
...
function buildReservationResult(allowed, used, limit, effectiveMax, priority, atomic, vorrangAbzug = 0) {
  ...
  // Greift der Mandatsvorbehalt fuer geteilte/nicht zuordenbare Arbeit, ist
  // er die aeusserste Bindung. Der historische Fehlercode bleibt kompatibel.
...
async function getKnowledgeObjectByVorgang(vorgangId, { throwOnError = false } = {}) {
  ...
  try {
    ...
    // W-1: `getExistingStreng` verlässt sich auf `throwOnError` — der Fehler muss TYPIERT
    // durchschlagen (fehlerklasse/quelle), nicht als roher Netz-/DB-Fehler, damit Aufrufer
    // ihn sichtbar und unterscheidbar von „existiert nicht" behandeln koennen.
    if (throwOnError) throw new StorageReadError("knowledge_objects", error);
    return null;
  }
}
...
async function listKnowledgeObjectsByVorgangPrefix(prefixes = [], limit = 8) {
  ...
  try {
    ...
    // W-1: ein Lesefehler darf NIE wie „keine Kandidaten" aussehen — sonst stuft der
    // Resolver den Cluster als `neu` ein (Modellaufruf + Write auf moeglicherweise vergebener
    // Kennung). Typisierter Fehler statt stillem [], wie bei den Geschwister-Lesern.
...
async function listKoDocuments(knowledgeObjectId, limit = 40) {
  ...
  try {
    ...
    const cols = "id,title,summary,quellenauszug_beleg:raw->helmutQuellenkontext,dip_quellfelder:raw->helmutDipQuellfelder,bundestag_artikelstand:raw->helmutBundestagArtikelstand,bundestag_abgerufen_at:retrieved_at,published_at,url,canonical_url,content_hash,source_id,source_name,document_type,link_type,confidence";
    const rows = await supabaseRequest(
    ...
    return (Array.isArray(rows) ? rows : []).map((r) => r && r.raw_documents).filter(Boolean)
      .map(require("./quellen-auszug").geleseneQuelle);
  } catch (error) {
    // W-1: ein Bestandslesefehler darf NIE wie "keine verknuepften Dokumente" aussehen —
    // sonst trifft `sameVorgang` ein Urteil auf unvollstaendiger Grundlage und ein Vorgang
    // wuerde als `neu` klassifiziert (spaeter Modellaufruf + Write auf moeglicherweise
    // vergebener Kennung). Typisierter Fehler statt stillem [], wie bei den Geschwister-
    // Lesern `listKoDocumentLinks` und `listRawDocuments`.
...
function klassifiziereLesefehler(error) {
  ...
  // Statuszahlen nur als strukturierte Werte oder ausdrueckliche HTTP-Meldung.
  // Ein Port, eine Objektkennung oder .401Z/.403Z im Timeout-Endpunkt ist kein
  // Authfehler (OP-28; reproduzierter CI-Befund 34967754746).
  let authStatus = false;
  for (let e = error, tiefe = 0; e && tiefe < 4; e = e.cause, tiefe += 1) {
    if ([e.status, e.statusCode, e.code].some(v => v === 401 || v === 403 || v === "401" || v === "403")) authStatus = true;
  }
  const httpAuth = /\b(?:supabase storage failed\s*\(|http(?:\/[\d.]+)?(?:\s+status)?\s+|status(?:code| code)?\s*[:=]\s*)(?:401|403)\b/.test(m);
  if (authStatus || httpAuth || m.includes("unauthorized")
...
async function fetchRawDocumentPages(basePath, limit) {
  ...
  for (let offset = 0; offset < ziel; offset += RAW_DOC_SEITE) {
    ...
    if (!Array.isArray(rows) || !rows.length) break;
    out.push(...rows.map(require("./quellen-auszug").geleseneQuelle));
    if (rows.length < seite) break;
  }
  return out;
}
...
async function listRecentRawDocuments(limit = 500, days = 30) {
  ...
  try {
    ...
    return await fetchRawDocumentPages(
      `/rest/v1/raw_documents?select=id,content_hash,canonical_url,title,summary,quellenauszug_beleg:raw->helmutQuellenkontext,dip_quellfelder:raw->helmutDipQuellfelder,bundestag_artikelstand:raw->helmutBundestagArtikelstand,bundestag_abgerufen_at:retrieved_at,url,source_name,source_id,document_type,published_at,created_at&created_at=gte.${encodeURIComponent(cutoffIso)}&order=created_at.desc`,
      limit
    );
  } catch (error) {
...
async function listRawDocuments({ limit = 800, days = 45 } = {}) {
  ...
  try {
    ...
    const cols = "id,content_hash,canonical_url,title,summary,quellenauszug_beleg:raw->helmutQuellenkontext,dip_quellfelder:raw->helmutDipQuellfelder,bundestag_artikelstand:raw->helmutBundestagArtikelstand,bundestag_abgerufen_at:retrieved_at,url,source_name,source_id,source_type,confidence,link_type,published_at,retrieved_at,created_at,document_type,wahlperiode";
    return await fetchRawDocumentPages(
...
// Ein explizit validierter Bundestags-Artikelstand traegt seinen Tag dagegen in
// den geschlossenen Stand-Metadaten (published_at bleibt absichtlich NULL). Er
// wird ueber eine eigene, ebenso begrenzte Projektion gelesen, im JS vollstaendig
// validiert und nur akzeptiert, wenn sein GANZER Berliner Publikationstag im
// Lagefenster liegt. Kein Ersatzdatum aus Abruf/created_at, kein voller Raw-Payload.
...
async function listAktuelleLageQuellen(ids = [], jetzt = new Date(), deps = {}) {
  ...
  try {
    ...
    const beginnMs = endeMs - frische.relevanzTage() * 86400000;
    const beginn = new Date(beginnMs).toISOString();
    const select = "knowledge_object_id,raw_documents!inner(id,title,url,canonical_url,published_at)";
    const standSelect = "knowledge_object_id,raw_documents!inner(id,title,url,canonical_url,published_at,summary,"
      + "bundestag_artikelstand:raw->helmutBundestagArtikelstand,bundestag_abgerufen_at:retrieved_at)";
    const out = [], gesehen = new Set();
    for (let i = 0; i < liste.length; i += KO_ID_STAPEL) {
      const teil = liste.slice(i, i + KO_ID_STAPEL);
      const inList = teil.map(id => `"${encodeURIComponent(id)}"`).join(",");
      // 1) Unveraenderter Zeitstempelpfad fuer gewoehnliche Quellen.
      let gelesenImStapel = 0;
      ...
      for (let offset = 0; ; offset += RAW_DOC_SEITE) {
        ...
        gelesenImStapel += rows.length;
        if (gelesenImStapel > KO_ID_STAPEL * 40) throw new Error("lage-quellen-lesegrenze");
        for (const r of rows) {
      ...
      // 2) Artikelstaende: published_at ist absichtlich NULL. Gezielte Projektion
      // (geschlossene Stand-Metadaten, summary, bekannte Abrufzeit) ohne vollen
      // raw-Payload. Zeilen ohne Stand-Metadaten werden im JS NICHT akzeptiert und
      // erhalten niemals ein Ersatzdatum aus Abruf/created_at. Beide Pfade teilen
      // dieselbe unveraenderte Obergrenze pro Stapel.
      const standBase = `/rest/v1/ko_document_links?select=${standSelect}&knowledge_object_id=in.(${inList})`
        + `&raw_documents.published_at=is.null`
        + `&raw_documents.raw->helmutBundestagArtikelstand=not.is.null`
        + `&order=knowledge_object_id.asc,raw_document_id.asc`;
      for (let offset = 0; ; offset += RAW_DOC_SEITE) {
        const rows = await request(`${standBase}&limit=${RAW_DOC_SEITE}&offset=${offset}`);
        if (!Array.isArray(rows) || rows.length > RAW_DOC_SEITE) throw new Error("ungueltige-quellenantwort");
        gelesenImStapel += rows.length;
        if (gelesenImStapel > KO_ID_STAPEL * 40) throw new Error("lage-quellen-lesegrenze");
        for (const r of rows) {
          ...
          if (!teil.includes(r?.knowledge_object_id) || typeof r?.raw_documents?.id !== "string"
            || gesehen.has(key)) throw new Error("ungueltige-quellenantwort");
          gesehen.add(key);
          // Vollstaendige Stand-Validierung VOR der Akzeptanz. Widerspruechliche
          // Metadaten sind ein lauter Fehler, kein stiller URL-Rueckfall.
          const stand = require("./bundestag-artikelstand").leseArtikelstand(r.raw_documents);
          if (!stand) continue; // keine Stand-Metadaten: nicht akzeptiert, kein Ersatzdatum
          if (!frische.berlinTagVollImFenster(stand.publikationstag, beginnMs, endeMs)) continue;
          out.push(r);
        }
        if (rows.length < RAW_DOC_SEITE) break;
      }
...
async function getSourcesForVorgang(vorgangId, { limit = 40, lageQuellen = null, lageKoId = null } = {}) {
  ...
  try {
    ...
    const select = "raw_documents(id,title,url,canonical_url,source_name,source_id,source_type,published_at,document_type,link_type,confidence,summary,quellenauszug_beleg:raw->helmutQuellenkontext,dip_quellfelder:raw->helmutDipQuellfelder,bundestag_artikelstand:raw->helmutBundestagArtikelstand,bundestag_abgerufen_at:retrieved_at)";
    const rows = await supabaseRequest(
...
async function getGebundeneLageQuellen(vorgangId, koId, erwartet) {
  try {
    ...
    const select = "knowledge_object_id,raw_document_id,raw_documents!inner(id,title,url,canonical_url,source_name,source_id,source_type,published_at,document_type,link_type,confidence,summary,quellenauszug_beleg:raw->helmutQuellenkontext,dip_quellfelder:raw->helmutDipQuellfelder,bundestag_artikelstand:raw->helmutBundestagArtikelstand,bundestag_abgerufen_at:retrieved_at)";
    ...
    for (const r of rows) {
      ...
      const vorStand = artikelstand.leseArtikelstand(vor), nachStand = artikelstand.leseArtikelstand(d);
      if ((vorStand?.standHash || null) !== (nachStand?.standHash || null)
        || (vorStand && Object.hasOwn(vor, "summary") && (vor.summary || null) !== (d.summary || null)))
        throw new Error("lage-quellenbindung-abweichend");
      gelesen.set(d.id, require("./quellen-auszug").geleseneQuelle(d));
    }
...
async function getRawDocumentsByIds(ids = []) {
  ...
  if (!liste.length) return [];
  const cols = "id,content_hash,canonical_url,title,summary,quellenauszug_beleg:raw->helmutQuellenkontext,dip_quellfelder:raw->helmutDipQuellfelder,bundestag_artikelstand:raw->helmutBundestagArtikelstand,bundestag_abgerufen_at:retrieved_at,url,source_name,source_id,source_type,"
  ...
  for (let i = 0; i < liste.length; i += RAW_DOC_ID_CHUNK) {
    ...
    try {
      ...
      if (Array.isArray(rows)) out.push(...rows.map(require("./quellen-auszug").geleseneQuelle));
    } catch (error) {
...
// Vorbereitete globale Belegablage, bewusst ohne regulaeren Aufrufer. Kein
// Tenant Blob und kein lokaler Fallback bei fehlendem Supabase Backend.
function artikelkontextSpeicher() {
  if (!useSupabase()) throw new Error("artikelkontext-speicher-nicht-verfuegbar");
  return require("./artikelkontext-belegspeicher").erzeugeBelegspeicher({ request: supabaseRequest, storeId: supabaseStoreId });
}

module.exports = {
  artikelkontextSpeicher,
