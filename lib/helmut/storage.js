...
//
// `readStore` ist im Supabase-Pfad NICHT nebenwirkungsfrei: `readSupabaseStore`
// legt eine fehlende Zeile beim Lesen SELBST an (`writeSupabaseStore(seeded)`).
// Ein Teardown, der zum Pruefen liest, erzeugt damit genau die Zeile, die er
// verhindern soll — der erste Korrekturversuch verschob das Anlegen nur vom
// Schreib- auf den Lesevorgang. Diese Funktion fragt stattdessen direkt nach der
// Zeile und legt NICHTS an. Sie umgeht auch den Zwischenspeicher, weil eine
...
const AUTH_MUTATION_MAX_ATTEMPTS = 16;
  ...
  const run = async () => {
    for (let attempt = 0; attempt < AUTH_MUTATION_MAX_ATTEMPTS; attempt += 1) {
      ...
      try {
        ...
        // gleichen Takt kollidieren. Der fruehere kurze Achtversuchsrahmen gab
        // im echten Mehrprozessnachweis zu frueh auf (99 von 100 Erfolgen).
        const pause = 10 + Math.floor(Math.random() * Math.min(500, 25 * 2 ** attempt));
...
async function deleteTenantScopedData(politicianId) {
  ...
  // mit `readStore`. Das ist im Supabase-Pfad KEINE Pruefung — `readSupabaseStore`
