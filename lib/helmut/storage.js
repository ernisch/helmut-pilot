...
async function deleteTenantScopedData(politicianId) {
  ...
  //
  // Die Schwesterfunktion `deleteProfileData` macht es zwei Funktionen weiter
  // oben bereits richtig (`if (pStoreHadData) …`, Kommentar dort: "Politiker-Store
  // nur leeren, wenn er ueberhaupt Daten traegt"). Dieselbe Bedingung gilt jetzt
  // auch hier: eine nicht vorhandene Zeile liefert den Standard-Store ohne
  // Inhalte, und der Schreibvorgang entfaellt.
  const pStoreVorher = await readStore(pKey(uid));
  const pStoreHatDaten = Object.values(pStoreVorher || {}).some((v) => Array.isArray(v) && v.length > 0);
