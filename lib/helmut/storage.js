...
async function readCronFairnessState() {
  const { normalizeState } = require("./cron-fairness");
  try {
    if (useSupabase()) {
      ...
      return { ok: true, state: normalizeState((row && row.data) || {}) };
    }
    const file = cronFairnessLocalFile();
    if (!fs.existsSync(file)) return { ok: true, state: normalizeState({}) };
    return { ok: true, state: normalizeState(JSON.parse(fs.readFileSync(file, "utf8"))) };
  } catch (error) {
    return { ok: false, state: normalizeState({}), fehler: String((error && error.message) || "unbekannt").slice(0, 200) };
  }
}

// Verschmilzt einen Patch monoton in den FRISCH gelesenen Zustand und schreibt ihn
// zurueck. Read-modify-write mit sehr kurzem Fenster (ein Lesen unmittelbar vor dem
// Schreiben); die Verschmelzung ist monoton, ein verspaeteter aelterer Stand kann
// einen juengeren also nicht zurueckdrehen. Wirft NIE.
async function saveCronFairnessState(patch, { nowMs = Date.now() } = {}) {
  const { mergeState } = require("./cron-fairness");
  try {
    const aktuell = await readCronFairnessState();
    // KEIN Schreiben auf einen nicht gelesenen Stand: der Patch traegt nur EIN Mandat,
    // ein leerer Lesestand wuerde die Eintraege ALLER anderen Mandate ueberschreiben.
    // Ein verlorener Versuchsvermerk ist harmlos (das Mandat bleibt vorn), ein
    // geloeschter Fairnesszustand nimmt der Rotation ihr Gedaechtnis.
    ...
    if (useSupabase()) {
      await supabaseRequest("/rest/v1/helmut_store", {
        ...
        body: JSON.stringify({ id: cronFairnessRowId(), data: merged })
      });
    } else {
      fs.mkdirSync(dataDir, { recursive: true });
      fs.writeFileSync(cronFairnessLocalFile(), JSON.stringify(merged, null, 2));
    }
    return { ok: true, state: merged, gelesen: true };
  } catch (error) {
    return { ok: false, fehler: String((error && error.message) || "unbekannt").slice(0, 200) };
  }
}

// DSGVO-Loeschung/Teardown: Scheduler-Spuren eines Mandats entfernen. Wirft NIE.
async function deleteCronFairnessTenant(politicianId) {
  ...
  try {
    ...
    if (useSupabase()) {
      await supabaseRequest("/rest/v1/helmut_store", {
        method: "POST",
        headers: { Prefer: "resolution=merge-duplicates,return=minimal" },
        body: JSON.stringify({ id: cronFairnessRowId(), data: bereinigt })
      });
    } else {
      fs.mkdirSync(dataDir, { recursive: true });
      fs.writeFileSync(cronFairnessLocalFile(), JSON.stringify(bereinigt, null, 2));
    }
