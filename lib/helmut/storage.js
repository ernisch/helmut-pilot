...
async function getRunCostReport(opts = {}) {
  ...
  const runs = (Array.isArray(store.processRuns) ? store.processRuns : []).slice(0, limitRuns);
...
function aggregateVorgangsbildung(runs = [], { tage = 7 } = {}) {
  const relevant = (Array.isArray(runs) ? runs : []).filter((r) =>
    r && (r.process === "understanding-eager" || r.process === "understanding-cron" || r.process === "understanding-lagecheck"));
...
async function listKnowledgeObjectsByVorgangPrefix(prefixes = [], limit = 8) {
  ...
  try {
    ...
    const cols = "id,vorgang_id,ko_version,status,understanding_status,headline,display_title,created_at,updated_at,source_document_count";
