...
// abgeloeste Zeilen koennen also keinen Platz im Nutzerlimit belegen. Der
// 23B-1-Hotfix liess die damalige Sortierung (`created_at.desc`) unveraendert;
// seit Befund F-E2E (2026-08-04) sortiert der Lesevertrag nach dem BERECHNETEN
// Rang (Begruendung am Endpoint unten). Geloescht wird weiterhin nichts.
...
async function listMatchingResults({ userId = null, limit = 50, includeAbgeloest = false } = {}, deps = {}) {
  ...
  try {
    ...
    // Sortierung: der BERECHNETE Rang, danach die Objektkennung als eindeutiger
    // Tiebreak (Befund F-E2E, 2026-08-04). Vorher `created_at.desc` — das war
    // sowohl unbestimmt (alle Zeilen eines Laufs teilen sich ein `now()`, die
    // Reihenfolge gleicher Werte ist in PostgreSQL nicht zugesichert) als auch
    // fachlich falsch: `created_at` friert beim ERSTEN Auftreten einer Zeile ein
    // (helmut_publish_matching_run), die Liste haette also nach „zuerst gesehen"
    // sortiert und `limit` haette die juengsten statt der relevantesten Zeilen
    // abgeschnitten. Zeilen ohne Rang (Altbestand) stehen hinten.
    const endpoint = `/rest/v1/matching_results?select=*&user_id=eq.${encodeURIComponent(uid)}${aktuellFilter}&order=rank.asc.nullslast,knowledge_object_id.asc&limit=${safeLimit}`;
