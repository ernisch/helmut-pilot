...
// ── Vollstaendige, seitenweise Lesung (Pagination) ──────────────────────────
// HINTERGRUND (Bugfix 2026-07-17): listRawDocuments verlaesst sich auf ein
// einzelnes ?limit=-Argument. PostgREST erzwingt jedoch server-seitig eine
// Obergrenze (db-max-rows, Supabase-Default 1000) — ein hoeheres limit wird still
// gekappt. Bei order=created_at.desc liefert das NUR die neuesten ~1000 Zeilen;
// aeltere Seed-Dokumente (Anfang Juli) fehlen komplett. Dadurch wurden im
// Recovery-Plan Faelle faelschlich als "keine-quelldokumente" bewertet.
//
// collectAllPages ist die REINE (netzwerkfreie, damit offline testbare)
// Pagination-Schleife: sie ruft fetchPage(offset, pageSize) seitenweise auf,
// dedupliziert ueber keyField und terminiert AUSSCHLIESSLICH an einer leeren
// Seite. Der Fortschritt zaehlt die tatsaechlich zurueckgegebene Zeilenzahl
// (NICHT das angeforderte limit) — dadurch ist die Schleife robust gegen einen
// server-seitigen Cap, der kleiner als pageSize ist. maxPages ist nur ein harter
// Endlosschleifen-Schutz. Wirft fetchPage (fehlerhafte Seite), propagiert der
// Fehler bewusst nach oben (Abbruch statt stiller Teil-Pool).
async function collectAllPages(fetchPage, opts = {}) {
  const pageSize = Number(opts.pageSize) > 0 ? Number(opts.pageSize) : 1000;
  const maxPages = Number(opts.maxPages) > 0 ? Number(opts.maxPages) : 1000;
  ...
  while (pages < maxPages) {
    const rows = await fetchPage(offset, pageSize); // darf werfen -> Abbruch (propagiert)
    pages += 1;
    if (!Array.isArray(rows) || rows.length === 0) { done = true; break; }
    for (const row of rows) {
      const key = row && row[keyField];
      if (key == null) continue;          // Zeile ohne Schluessel: konservativ ueberspringen
      if (!byKey.has(key)) byKey.set(key, row); // Dedup: erste Sichtung gewinnt
    }
    offset += rows.length;                // Fortschritt via echter Zeilenzahl (cap-robust)
  }
  return { rows: [...byKey.values()], pages, truncated: !done };
}

// Laedt ALLE raw_documents im Zeitfenster (Default 120 Tage) via Pagination.
// Anders als listRawDocuments schlucken wir hier einen Seitenfehler NICHT: ein
// fehlerhafter Abruf bricht ab (throw), damit niemals ein unvollstaendiger Pool
// eine irrefuehrende Recovery-Bewertung erzeugt ("Abbruch bei fehlerhafter Seite").
// Stabile Sortierung (created_at.desc + id.asc als Tiebreaker) plus Dedup-nach-id
// verhindern Luecken/Duplikate an Seitengrenzen. Rein lesend.
async function listAllRawDocuments({ days = 120, pageSize = 1000, maxPages = 200 } = {}) {
  if (!v3StoreReady()) return [];
  const cutoffIso = new Date(Date.now() - days * 24 * 60 * 60 * 1000).toISOString();
  const cols = "id,content_hash,canonical_url,title,summary,url,source_name,source_id,source_type,confidence,link_type,published_at,retrieved_at,document_type,wahlperiode";
  const fetchPage = (offset, limit) => supabaseRequest(
    `/rest/v1/raw_documents?select=${cols}&created_at=gte.${encodeURIComponent(cutoffIso)}` +
    `&order=created_at.desc,id.asc&offset=${offset}&limit=${limit}`
  );
  const { rows, pages, truncated } = await collectAllPages(fetchPage, { pageSize, maxPages, keyField: "id" });
  if (truncated) {
    console.error(`[v3Store] listAllRawDocuments: maxPages(${maxPages}) erreicht nach ${pages} Seiten — Pool evtl. unvollstaendig.`);
  }
  return rows;
}

// EXAKTE Einzel-Lesung EINES Rohdokuments per id (keine Anker-/Teilstring-/Themen-Suche).
// Rein lesend; gibt die vollstaendige Zeile oder null zurueck.
async function getRawDocumentById(rawDocumentId) {
  if (!v3StoreReady()) return null;
  if (!rawDocumentId) return null;
  try {
    const cols = "id,content_hash,canonical_url,title,summary,url,source_name,source_id,source_type,confidence,link_type,published_at,retrieved_at,document_type,wahlperiode";
    const rows = await supabaseRequest(
      `/rest/v1/raw_documents?id=eq.${encodeURIComponent(rawDocumentId)}&select=${cols}&limit=1`
    );
    return Array.isArray(rows) && rows.length ? rows[0] : null;
  } catch (error) {
    console.error("[v3Store] getRawDocumentById fehlgeschlagen:", error.message);
    return null;
  }
}

// Rein lesend: welche Wissensobjekte verlinken ein bestimmtes Rohdokument (exakt per
// raw_document_id)? Fuer die "bereits verarbeitet / semantisches Duplikat"-Pruefung im
// Einzel-Dokument-Recovery-Pfad — id-basiert, keine unscharfe Themen-Suche.
async function listKnowledgeObjectLinksForDoc(rawDocumentId) {
  if (!v3StoreReady()) return [];
  if (!rawDocumentId) return [];
  try {
    const rows = await supabaseRequest(
      `/rest/v1/ko_document_links?raw_document_id=eq.${encodeURIComponent(rawDocumentId)}&select=knowledge_object_id,raw_document_id&limit=1000`
    );
    return Array.isArray(rows) ? rows : [];
  } catch (error) {
    console.error("[v3Store] listKnowledgeObjectLinksForDoc fehlgeschlagen:", error.message);
    return [];
  }
}
...
async function saveProfile(profile, deps = {}) {
  ...
  await writeStore(store);
  // Blob bleibt IMMER die erste, garantierte Schreiboperation (Fallback-Treue).
  // Der SQL-Schreibpfad ist rein additiv und darf den Blob-Erfolg nie verhindern
  // (daher kein throw, kein await-Block um den Rueckgabewert herum).
...
async function getProfileFromDb(userId, deps = {}) {
  ...
  try {
    ...
    if (mandateRow && mandateRow.geloescht_at) return null; // Soft-geloescht: wie "nicht vorhanden" behandeln.
...
async function saveProfileToDb(profile = {}, deps = {}) {
  if (!profile.id) return { skipped: true, reason: "missing-id" };
  if (!profileDbModeEnabled()) return { skipped: true, reason: v3SkipReason() };
...
module.exports = {
  ...
  collectAllPages,
  getRawDocumentById,
  listKnowledgeObjectLinksForDoc,
