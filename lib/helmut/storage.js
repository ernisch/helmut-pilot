...
async function listKnowledgeObjectsByVorgangPrefix(prefixes = [], limit = 8) {
  ...
  try {
    ...
    const cols = "id,vorgang_id,ko_version,status,understanding_status,headline,display_title,created_at,updated_at,source_document_count,decision_level,political_level,classification_confidence";
    const rows = await supabaseRequest(
...
async function getClassificationCoverage() {
  ...
  try {
    ...
    const [total, withLevel, withEstablishedLevel, withEmbedding] = await Promise.all([
    ...
    return {
      total,
      withLevel, withEstablishedLevel, withEmbedding,
