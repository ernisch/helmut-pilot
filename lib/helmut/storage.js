...
async function listMatchingResults({ userId = null, limit = 50, includeAbgeloest = false } = {}, deps = {}) {
  ...
  try {
    ...
    // REIHENFOLGE — belegter Befund (2026-08-08), BEWUSST NICHT hier geaendert.
    // `order=created_at.desc` laesst die Reihenfolge undefiniert, sobald mehrere Zeilen
    // denselben Zeitstempel tragen — und das ist der Normalfall: `now()` ist in Postgres
    // innerhalb einer Transaktion konstant, ein Stapel-Upsert vergibt also fuer ALLE Zeilen
    // eines Matchinglaufs denselben Wert. Die Lage haengt damit an einer Reihenfolge, die
    // nichts mit Relevanz zu tun hat.
    // NACHGEWIESEN in `berlin-e2e-vertrag-test.js` (J8) unter CPU-Knappheit: 1 Fehlschlag
    // in 25 Laeufen, ohne Fremdlast in 78 Laeufen keiner. Vollstaendige Herleitung und
    // Empfehlung: docs/betrieb/skalierungsgrundlage-1000.md §15.
    // NICHT GEAENDERT, weil jede Korrektur hier entscheidet, WELCHER Vorgang dem
    // Mandatstraeger zuerst angezeigt wird — das ist eine Produktentscheidung, keine
    // Aufraeumarbeit, und dieser Sprint darf bestehende V3-Logik nicht ersetzen.
...
async function savePendingKnowledgeObjectsBulk(eintraege = [], { deadlineMs = 0, now = Date.now } = {}) {
  ...
  for (let i = 0; i < gueltige.length; i += PENDING_BULK_CHUNK) {
    ...
    try {
      ...
      if (docsById.size) {
        await v3Upsert("raw_documents", [...docsById.values()], "id");
        bilanz.anfragen += 1;
      }
      if (links.length) {
...
function providerInteger(value, minimum) {
  return typeof value === "number" && Number.isSafeInteger(value)
    && value >= minimum && value <= POSTGRES_INTEGER_MAX
    ? value : null;
}

async function anbieterReserviere({ schluessel, menge = 1, grenzeMinute = 0, grenzeTag = 0 } = {}) {
  // Letzte Grenze vor dem RPC: PostgreSQL erwartet fuer alle drei Werte
  // `integer`. Nie runden, koerzieren oder einen Overflow erst der Datenbank
  // ueberlassen; bei ungueltigem Adapterwert wird kein Request erzeugt.
  const mengeInt = providerInteger(menge, 1);
  const minuteInt = providerInteger(grenzeMinute, 0);
  const tagInt = providerInteger(grenzeTag, 0);
  if (mengeInt == null || minuteInt == null || tagInt == null) {
    return { verfuegbar: false, grund: "anbieterparameter-ausserhalb-postgresql-integer" };
  }
  if (!jobQueueBereit()) return { verfuegbar: false, grund: "supabase-nicht-konfiguriert" };
  try {
...
async function verstehenKennzahlen() {
  ...
  try {
    ...
    return { verfuegbar: true, zustaende: rows || [] };
  } catch (error) {
    if (isMissingReservationRpcError(error)) return { verfuegbar: false, grund: "migration-fehlt" };
    return { verfuegbar: false, grund: String((error && error.message) || "unbekannt").slice(0, 200) };
  }
...
// PLANUNG — KEIN Volltext. Eine Tabelle gilt erst nach einer terminalen (kurzen)
// Seite als vollständig. Ein Fehler in IRGENDEINER Tabelle verwirft den gesamten
// Abzug; insbesondere werden fehlende Links nie als leere Linkmenge ausgegeben.
// Trotzdem ist dies nur ein SEQUENZIELLER, NICHT TRANSAKTIONALER REST-Abzug:
// Inserts/Deletes koennen Offset-Seiten verschieben, und zwischen Abzug und einer
// spaeteren Mutation kann ein neuer Link entstehen. Der Abzug ist daher nur fuer
// einen Trockenlauf geeignet und niemals ein Ausfuehrungsbeleg.
async function loadRetentionCollection(request, { name, table, select, order, pageSize, maxPages }) {
  const rows = [];
  for (let pageIndex = 0; pageIndex < maxPages; pageIndex += 1) {
    const offset = pageIndex * pageSize;
    const endpoint = `/rest/v1/${table}?select=${select}&order=${order}&limit=${pageSize}&offset=${offset}`;
    const page = await request(endpoint);
    if (!Array.isArray(page)) throw new Error(`${name}-seite-ist-kein-array`);
    if (page.length > pageSize) throw new Error(`${name}-seite-ueberschreitet-seitengroesse`);
    rows.push(...page);
    if (page.length < pageSize) {
      return {
        ...
        proof: {
          ...
          terminalPage: true,
          truncated: false,
          pagination: "offset-until-terminal-page",
          rows: rows.length,
          pages: pageIndex + 1,
          pageSize,
          terminalPageRows: page.length
        }
      };
    }
  }
  // Das Sicherheitslimit beendet nur den LESEVERSUCH. Es darf niemals einen
  // scheinbar vollständigen Löschplan freigeben.
  throw new Error(`${name}-pagination-unvollstaendig-seitenlimit`);
}

async function loadRetentionMetadata(deps = {}) {
  const { RETENTION_REST_CONSISTENCY } = require("./retention");
  const ready = Object.prototype.hasOwnProperty.call(deps, "ready") ? deps.ready === true : v3StoreReady();
  if (!ready) return {
    available: false,
    complete: false,
    ...
    integrityOk: false,
    executable: false,
    reason: "relational-store-not-ready"
  };
  try {
    const request = Object.prototype.hasOwnProperty.call(deps, "request") ? deps.request : supabaseRequest;
    if (typeof request !== "function") throw new Error("retention-request-fehlt");
    const pageSize = Object.prototype.hasOwnProperty.call(deps, "pageSize") ? deps.pageSize : 1000;
    const maxPages = Object.prototype.hasOwnProperty.call(deps, "maxPages") ? deps.maxPages : 10000;
    if (!Number.isSafeInteger(pageSize) || pageSize < 1 || pageSize > 1000) throw new Error("retention-pageSize-ungueltig");
    if (!Number.isSafeInteger(maxPages) || maxPages < 1) throw new Error("retention-maxPages-ungueltig");

    // Bewusst sequenziell: kein falscher Snapshot-Anspruch durch drei parallele
    // Abzuege. Auch sequenziell bleibt REST ohne DB-Transaktion nur Dry-run-tauglich.
    const raw = await loadRetentionCollection(request, {
      name: "rawDocuments", table: "raw_documents", select: "id,created_at", order: "id.asc", pageSize, maxPages
    });
    const kos = await loadRetentionCollection(request, {
      name: "knowledgeObjects", table: "knowledge_objects", select: "id,created_at", order: "id.asc", pageSize, maxPages
    });
    const links = await loadRetentionCollection(request, {
      name: "koDocumentLinks", table: "ko_document_links", select: "knowledge_object_id,raw_document_id",
      order: "knowledge_object_id.asc,raw_document_id.asc", pageSize, maxPages
    });
    ...
    const result = {
      ...
      metadataCompleteness: {
        rawDocuments: raw.proof,
        knowledgeObjects: kos.proof,
        koDocumentLinks: links.proof
      },
      rawDocuments: raw.rows,
      knowledgeObjects: kos.rows,
      koDocumentLinks: links.rows
    };
    const validation = validateRetentionMetadata(result);
    if (!validation.ok) throw new Error(`retention-metadaten-ungueltig:${validation.errors.join(",")}`);
    return result;
  } catch (error) {
    return {
      available: false,
      ...
      integrityOk: false,
      executable: false,
      reason: (error && error.message) || "retention-metadaten-lesefehler"
    };
  }
}

// Der bisherige clientseitige DELETE-Executor ist bewusst stillgelegt. Kein
// selbst gesetztes Flag und kein Plan-Boolean kann den nicht-transaktionalen
// REST-Abzug in eine Löschfreigabe verwandeln. Eine künftige Ausführung braucht
// entweder einen DB-seitig transaktionalen/sperrenden RPC oder einen belegten
// Schreibstopp plus erneute atomare Referenzprüfung unmittelbar beim Löschen.
async function deleteRetention(plan = {}, deps = {}) {
  const { retentionExecuteEnabled } = require("./retention");
  const env = Object.prototype.hasOwnProperty.call(deps, "env") ? deps.env : process.env;
  if (!retentionExecuteEnabled(env)) return { skipped: true, reason: "disabled" };
  const ready = Object.prototype.hasOwnProperty.call(deps, "ready") ? deps.ready === true : v3StoreReady();
  if (!ready) return { skipped: true, reason: "relational-store-not-ready" };
  return {
    skipped: true,
    reason: "transaktionaler-retention-executor-nicht-implementiert",
    konsistenz: plan && plan.konsistenz || null,
    erforderlich: "db-transaktionaler-sperrender-rpc-oder-schreibstopp-mit-atomarer-referenzpruefung"
  };
}
