...
async function readCronFairnessState(deps = {}) {
  ...
  try {
    if (useSupabase()) {
      ...
      const rohdaten = (row && row.data) || {};
      return { ok: true, state: normalizeState(rohdaten), existiert: Boolean(row), version: stateVersion(rohdaten) };
    }
    const file = cronFairnessLocalFile();
    if (!fs.existsSync(file)) return { ok: true, state: normalizeState({}), existiert: false, version: null };
    const rohdaten = JSON.parse(fs.readFileSync(file, "utf8"));
    return { ok: true, state: normalizeState(rohdaten), existiert: true, version: stateVersion(rohdaten) };
  } catch (error) {
...
//      entfaellt die Gegenpruefung bewusst — ein verlorener Abschluss laesst den
//      Eintrag `laufend`, und der laeuft nach der Frist kontrolliert ab.
async function saveCronFairnessState(patch, { nowMs = Date.now(), pruefen = false, maxVersuche = 3, deps = {} } = {}) {
  ...
  return { ok: false, fehler: letzterFehler || "unbekannt", konflikt: /ueberschrieben/.test(String(letzterFehler)) };
}
