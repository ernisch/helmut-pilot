...
const STORE_SNAPSHOT = Symbol("store-local-snapshot");

function storeSnapshot(store, storeKey, version) {
  Object.defineProperty(store, STORE_SNAPSHOT, {
    value: Object.freeze({ storeKey, version }), enumerable: true
  });
  return store;
}
...
async function readStore(storeKey = "main") {
  ...
  if (storeCacheTtlMs > 0 && cached && Date.now() - cached.at < storeCacheTtlMs) {
    return storeSnapshot(structuredClone(cached.data), storeKey, version);
  }
  ...
  if (version === (storeVersions.get(storeKey) || 0)) {
    storeCacheMap.set(storeKey, { data: structuredClone(store), at: Date.now() });
  }
  return storeSnapshot(store, storeKey, version);
}
...
async function writeStore(store, storeKey = "main") {
  ...
  const run = async () => {
    ...
    // legitimieren. Gegen ANDERE Instanzen braucht es weiterhin Datenbank-CAS.
    ...
    try {
      ...
      storeCacheMap.set(storeKey, { data: structuredClone(normalized), at: Date.now() });
      ...
      return storeSnapshot(normalized, storeKey, version);
    } catch (error) {
...
async function readSupabaseStore(storeKey = "main") {
  ...
  if (rows.length === 1) {
    return storeKey === "main" ? normalizeStore(rows[0].data) : normalizePoliticianStore(rows[0].data);
  }
...
// ── EXISTENZPRUEFUNG OHNE NEBENWIRKUNG (02.09., zweiter adversarialer Review) ──
//
// `readSupabaseStore` legte eine fehlende Zeile frueher beim Lesen selbst an.
// Dieser Fehler ist behoben. Der direkte Existenzpfad bleibt sinnvoll: Er
// umgeht den Zwischenspeicher, weil eine
// Loeschentscheidung nicht auf einer bis zu 10 s alten Kopie beruhen darf.
async function pStoreHatDatenOhneAnlegen(storeKey) {
  if (!useSupabase()) {
    const lokal = readLocalStore(storeKey);
    return Object.values(lokal || {}).some((v) => Array.isArray(v) && v.length > 0);
  }
  const rowId = `${supabaseStoreId}-${storeKey}`;
  const rows = await withStoreRetry(
    () => supabaseRequest(`/rest/v1/helmut_store?id=eq.${encodeURIComponent(rowId)}&select=data`),
    { label: "blob-exists", storeKey }
  );
  const row = Array.isArray(rows) ? rows[0] : null;
  if (!row || !row.data) return false;   // keine Zeile = nichts zu leeren, nichts anzulegen
  return Object.values(normalizePoliticianStore(row.data) || {})
...
async function deleteTenantScopedData(politicianId) {
  ...
  const pStoreHatDaten = await pStoreHatDatenOhneAnlegen(pKey(uid));
