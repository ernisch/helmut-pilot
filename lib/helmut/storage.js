...
async function getLlmUsageToday(politicianId = null, referenceIso = null) {
  ...
  const cost = todays.reduce((sum, e) => sum + (typeof e.estimatedCost === "number" ? e.estimatedCost : 0), 0);
  return {
    day: today,
    calls: todays.length,
    successfulCalls: todays.filter((e) => e.success !== false).length,
...
// bei allowed=false auf den Regelmotor zurueckfaellt. Fehlerfall: fail-OPEN
// (allowed=true), damit ein Storage-Problem den Briefing-Motor nicht lahmlegt.
async function canSpendLlm(politicianId = null, referenceIso = null) {
  ...
  try {
    ...
    return { allowed: true, used: null, limit: Number.isFinite(limit) ? limit : null, remaining: null, reason: "budget-check-failed-open" };
  }
}
