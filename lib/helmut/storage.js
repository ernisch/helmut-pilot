...
async function listKnowledgeObjects({ limit = 50, status = null, reihenfolge = "neueste", _signalError = false } = {}) {
...
async function listPendingKnowledgeObjects({ limit = 50, reihenfolge = "neueste" } = {}) {
  const rows = await listKnowledgeObjects({ status: "pending", limit, reihenfolge });
