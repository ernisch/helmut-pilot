...
// DOKUMENTIERT (.env.example): leer oder 0 = bewusst kein Limit.
// AUDIT-FIX 2026-07 (fail-closed): ein GESETZTER, aber UNGUELTIGER Wert (Tippfehler
// wie "2Oo", negatives Vorzeichen, Einheiten) fiel frueher STILL auf Infinity
// zurueck — ein einziger Vertipper hob also unbemerkt den Kostendeckel auf.
// Jetzt greift dann ein konservatives Schutzlimit (einmalige Warnung im Log).
const LLM_LIMIT_FALLBACK_ON_INVALID = 50;
let warnedInvalidLlmLimit = false;
function llmDailyCallLimit() {
  const rawStr = String(process.env.HELMUT_MAX_LLM_CALLS_PER_DAY ?? "").trim();
  if (rawStr === "" || rawStr === "0") return Infinity; // dokumentiert: bewusst kein Limit
  const raw = Number(rawStr);
  if (Number.isFinite(raw) && raw > 0) return Math.floor(raw);
  if (!warnedInvalidLlmLimit) {
    warnedInvalidLlmLimit = true;
    console.warn(`[llm-budget] HELMUT_MAX_LLM_CALLS_PER_DAY ist gesetzt, aber ungueltig — Schutzlimit ${LLM_LIMIT_FALLBACK_ON_INVALID} Calls/Tag aktiv (fail-closed statt unbegrenzt).`);
  }
  return LLM_LIMIT_FALLBACK_ON_INVALID;
}
...
async function canSpendLlmForTenant(politicianId = null, budgets = {}, referenceIso = null, deps = {}) {
  ...
  const global = await canSpendFn(politicianId, referenceIso);
...
function assertTenantRows(rows, fn = "write") {
...
async function saveMatchingResults(rows = []) {
  assertTenantRows(rows, "saveMatchingResults(rows)");
...
async function saveDecisions(rows = []) {
  assertTenantRows(rows, "saveDecisions(rows)");
  if (!v3StoreReady()) return { skipped: true, reason: v3SkipReason(), saved: 0 };
  const list = (Array.isArray(rows) ? rows : []).filter((r) => r && r.id && r.user_id);
  if (!list.length) return { skipped: true, reason: "no-rows", saved: 0 };
  try {
...
async function saveKnowledgeObjectEnrichment(id, patch = {}) {
  ...
  if (typeof patch.decision_level === "string") body.decision_level = patch.decision_level;
  if (typeof patch.political_level === "string") body.political_level = patch.political_level;
  if (typeof patch.event_type === "string") body.event_type = patch.event_type;
  if (Array.isArray(patch.related_levels)) body.related_levels = patch.related_levels;
...
async function exportProfileDataV3(politicianId, deps = {}) {
  ...
  if (!ready) return { skipped: true, reason: v3SkipReason() };
  ...
  for (const t of tables) {
    try {
      const rows = await request(`/rest/v1/${t.table}?${v3PrivacyFilter(t, politicianId)}&limit=2000`, { method: "GET" });
      out.tabellen[t.table] = Array.isArray(rows) ? rows : [];
    } catch (error) {
...
async function deleteProfileDataV3(politicianId, deps = {}) {
  const ready = deps.ready !== undefined ? deps.ready : v3StoreReady();
  if (!ready) return { skipped: true, reason: v3SkipReason(), ok: true };
  const request = deps.request || supabaseRequest;
...
async function exportProfileData(politicianId) {
  ...
  return {
    ...
    politicianId,
    vollstaendig: (v3.skipped || v3.vollstaendig !== false) && authDaten.vollstaendig !== false,
    v3Tabellen: v3,
    authDaten,
    profile,
...
async function deleteProfileData(politicianId) {
  ...
  return {
    ok: Boolean((v3.skipped || v3.ok) && auth.ok),
...
// Internes Profil-Objekt (camelCase, Form wie cemInceProfile/config.js) -> Zeile
