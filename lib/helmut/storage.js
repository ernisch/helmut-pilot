...
function normalizePoliticianStore(store = {}) {
  ...
  return {
    ...
    // BUGFIX (Audit-Folgebranch): watchdogStates fehlte in dieser harten
    // Whitelist — saveWatchdogState schrieb den Zustand, der naechste
    // writeStore-Durchlauf verwarf ihn wieder. Damit war die P1-4-Hysterese
    // ("Erholt"-Erkennung im Health-Report) in ALLEN Modi wirkungslos.
    watchdogStates: Array.isArray(parsed.watchdogStates) ? parsed.watchdogStates.slice(0, 20) : []
  };
}
...
function compactStore(store) {
  ...
  return {
    ...
    crawlRuns: sortByDate(store.crawlRuns, "createdAt").slice(0, 30).map((run) => ({
      ...
      // BUGFIX (Audit-Folgebranch): diese Whitelist strippte beim Persistieren
      // die von Health-Report/Watchdog-Statuspfad gelesenen Telemetrie-Felder —
      // die Google-News-Quote und die Pipeline-Laufzeit waeren live nie
      // angekommen, obwohl saveCrawlRun sie schrieb. Beide sind kleine Zaehler
      // (keine Inhalte) und bleiben jetzt erhalten.
      googleUrlResolution: run.googleUrlResolution || null,
