...
function sanitizeStoredRawItem(item) {
  if (!isBlockedStoredArticleUrl(item?.url)) {
    return {
      ...item,
      linkType: item.linkType || articleUrlQuality(item.url),
      linkResolutionNote: item.linkResolutionNote || linkResolutionNote(item.linkType || articleUrlQuality(item.url))
    };
  }
  ...
  return {
    ...
    linkType: fallbackUrl ? "publisher" : "missing",
    linkResolutionNote: fallbackUrl ? linkResolutionNote("publisher") : linkResolutionNote("missing")
  };
}
