...
async function zaehleGateWuerdigerAbfluss(stunden = 24) {
  ...
  const seit = new Date(Date.now() - Math.max(1, Number(stunden) || 24) * 3600e3).toISOString();
  try {
    // 1) Abschluesse im Fenster — deterministisch paginiert (updated_at + id).
    const koIds = [];
    const maxSeiten = abflussMaxSeiten();
    let seiten = 0;
    for (; seiten < maxSeiten; seiten += 1) {
      const rows = await supabaseRequest(
        "/rest/v1/knowledge_objects?select=id&understanding_status=eq.complete"
        + `&updated_at=gte.${encodeURIComponent(seit)}`
    ...
    // 2) Dokumentverknuepfungen der Abschluesse (Blockweise; ein voller Block
    //    waere eine moegliche Kappung -> nicht messbar statt falsch).
    ...
    for (let i = 0; i < koIds.length; i += 50) {
      ...
      const rows = await supabaseRequest(
        ...
        + `&order=knowledge_object_id.asc,raw_document_id.asc&limit=${ABFLUSS_SEITE}`
      );
      if (!Array.isArray(rows)) return null;
      if (rows.length >= ABFLUSS_SEITE) {
        console.error("[drain-bilanz] Verknuepfungs-Block voll (moegliche Kappung) — Abfluss gilt als nicht messbar.");
        return null;
      }
      for (const r of rows) {
    ...
    for (let i = 0; i < docListe.length; i += 50) {
      ...
      const rows = await supabaseRequest(
        ...
        + `&order=raw_document_id.asc,id.asc&limit=${ABFLUSS_SEITE}`
      );
      if (!Array.isArray(rows)) return null;
      if (rows.length >= ABFLUSS_SEITE) {
        console.error("[drain-bilanz] Entscheidungs-Block voll (moegliche Kappung) — Abfluss gilt als nicht messbar.");
        return null;
      }
      for (const r of rows) {
...
// Ein Trend braucht ZWEI Messpunkte. Der Gesundheitslauf persistiert deshalb je
// UTC-Tag GENAU EINEN Schnappschuss des verarbeitbaren Rueckstands in einer
// EIGENEN helmut_store-Zeile — exakt nach dem belegten F-CAS-Muster der
// Cron-Fairness (eigene Zeile, ein Schreiber, Compare-and-Set ueber `data.rev`,
// CLAUDE.md §4.10; keine neue Tabelle, keine Migration). Der ERSTE Eintrag eines
// Tages gewinnt (deterministischer Tagesanker, kein Nachmittags-Drift); ohne
// gestrigen Vergleichswert bleibt der Trend ehrlich `unvollstaendig` — der
// Mechanismus verankert sich selbst wie die Rueckstandsslot-Ueberwachung.
...
// nie verschluckt, und verhindert den Bericht nicht (der Trend bleibt dann
// `unvollstaendig`).
...
async function listMatchingResults({ userId = null, limit = 50, includeAbgeloest = false } = {}, deps = {}) {
  ...
  try {
    ...
    // REIHENFOLGE — Befund F-E2E (2026-08-08), KORRIGIERT im 500-Mandate-
    // Reifesprint (2026-09-01, Betreiberauftrag: "nachvollziehbare, mandats-
    // neutrale und deterministische Zweitsortierung"):
    // `order=created_at.desc` allein laesst die Reihenfolge undefiniert, sobald
    // mehrere Zeilen denselben Zeitstempel tragen — der NORMALFALL, denn `now()`
    // ist in Postgres je Transaktion konstant (rein lesend belegt 31.08.: 24
    // Gleichstandsgruppen in den aktuellen Production-Zeilen). Die Zweitsortierung
    // ist der VOM MATCHER BERECHNETE RANG (`rank.asc`): innerhalb eines Stapels
    // zeigt die Lage damit exakt die Relevanzordnung des Laufs — nachvollziehbar
    // (der Rang steht in der Zeile), mandatsneutral (jeder Mandant traegt seine
    // eigenen Raenge) und fachlich richtig statt willkuerlich (der 2026-08-08
    // verworfene Versuch `id.desc` hatte den falschen Vorgang nach vorn gestellt).
    // `id.asc` als letzter Schluessel macht die Ordnung TOTAL (deterministisch
    // auch bei theoretisch gleichem Rang, z. B. Altzeilen ueber includeAbgeloest).
    // NACHGEWIESEN instabil in `berlin-e2e-vertrag-test.js` (J8) unter CPU-
    // Knappheit: 1 Fehlschlag in 25 Laeufen; Herleitung
    // docs/betrieb/skalierungsgrundlage-1000.md §15.
    const endpoint = `/rest/v1/matching_results?select=*&user_id=eq.${encodeURIComponent(uid)}${aktuellFilter}&order=created_at.desc,rank.asc.nullslast,id.asc&limit=${safeLimit}`;
...
async function jobQueueZurueckgestellteGruende({ limit = 1000 } = {}) {
  ...
  try {
    ...
    const texte = (Array.isArray(rows) ? rows : [])
      .map((r) => String((r && r.last_error) || "")).filter(Boolean);
    return { verfuegbar: true, anzahl: texte.length, texte };
  } catch (error) {
    return { verfuegbar: false, grund: String((error && error.message) || "unbekannt").slice(0, 200) };
  }
}
