...
function understandingQuelle(d = {}, index = 0) {
  ...
  // Ein geprueffter Bundestags-Artikelstand kennt ausschliesslich den Publikationstag.
  // Er erreicht den Prompt als Tag aus den validierten Metadaten; die timestamptz-Spalte
  // published_at bleibt dafuer leer. Keine erfundene Uhrzeit, keine scheinbare Frische.
  const stand = require("./bundestag-artikelstand").leseArtikelstand(d);
  const quellenDatum = stand ? stand.publikationstag : d.published_at;
  const abruf = publikationsdatum(d.retrieved_at ?? (stand ? d.bundestag_abgerufen_at : null) ?? d.fetched_at);
  return { quelle_id: d.id || `quelle-${index + 1}`, titel: d.title || "", auszug: d.summary || "",
    ...
    veroeffentlichtAm: publikationsdatum(quellenDatum)?.original || null,
    // Nur der neue Standvertrag normalisiert dieselbe Abrufzeit aus PostgREST/JS.
    // So bleiben Quellenhash und separater Beleg nach timestamptz-Roundtrip identisch.
    abgerufenAm: (stand ? abruf?.iso : abruf?.original) || null,
    zeitbezug: zeitbezug(quellenDatum),
