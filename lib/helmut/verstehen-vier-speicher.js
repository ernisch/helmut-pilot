...
function adapter({ env = process.env, request: injected, storage: storageDeps, ai: aiDeps, budget: budgetDeps, motorBasis } = {}) {
  ...
  return {
    async profile() {
      ...
      // Production: 504 Mandatsprofile und 505 Kontoprofile (eins ohne Mandat).
      // Je eine zusaetzliche Zeile lesen: auch ein groesserer Bestand sperrt.
      const profile = await rows("profiles?select=*&order=id.asc&limit=506");
