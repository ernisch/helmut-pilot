...
// ── DRAIN-BILANZ (PR-C, Kapazitätssprint 2026-08-31) — reine Darstellungs-/Ableitungslogik ──
// Die Abnahmegroesse "gate-wuerdiger Abfluss >= gate-wuerdige Ankunft, verarbeitbarer
// Rueckstand waechst nicht" als EINE ehrliche Berichtszeile. Jede Groesse einzeln:
// null/undefined => "?" (nicht messbar), nie eine erfundene 0. Rein, testbar, ohne I/O.
function drainBilanzZeile({ ankunftWuerdig = null, abfluss = null, rueckstand = null, gateGeparkt = null, fehlversuchsQuote = null } = {}) {
  const z = (v) => (v == null || !Number.isFinite(Number(v)) ? "?" : String(Number(v)));
  // Richtungsurteil NUR, wenn beide Seiten messbar sind — sonst keine Behauptung.
  const beide = ankunftWuerdig != null && abfluss != null
    && Number.isFinite(Number(ankunftWuerdig)) && Number.isFinite(Number(abfluss));
  const richtung = beide ? (Number(abfluss) >= Number(ankunftWuerdig) ? " ✓" : " ⚠️ Abfluss < Ankunft") : "";
  const quote = fehlversuchsQuote == null ? "?" : `${Math.round(Number(fehlversuchsQuote) * 100)}%`;
  return `Verstehen (24h): Ankunft würdig ${z(ankunftWuerdig)} · Abfluss ${z(abfluss)}${richtung}`
    + ` · Rückstand ${z(rueckstand)} · Gate geparkt ${z(gateGeparkt)} · Fehlversuchsquote Rückstand ${quote}`;
}
