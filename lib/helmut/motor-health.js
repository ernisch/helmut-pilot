...
function bewerteBriefingStufen({ einzel = [], jetztMs = Date.now(), erwartetAbMs = null } = {}) {
  ...
  const zugestellt = mitEmpfaenger.filter((e) => Number(e.zugestellt) > 0).length;
  // Ein Zustellfehler zählt als Störung nur bei vorhandenem Empfänger. Ein Fehler ohne
  ...
  // (2) Ein Zustellfehler bei vorhandenem Empfänger ist eine Störung.
  if (zustellfehler > 0) gruende.push("briefing-zustellfehler");
  ...
  const text = [
    ...
    mitEmpfaenger.length
      ? `Zugestellt: ${zugestellt} von ${mitEmpfaenger.length} registrierten Empfängern.`
      : "Zugestellt: kein registrierter Empfänger.",
    ...(ohneEmpfaenger > 0 ? [`Ohne registrierten Push Empfänger: ${ohneEmpfaenger} Mandate.`] : []),
    ...(zustellfehler > 0 ? [`Zustellung fehlgeschlagen: ${zustellfehler} Mandate.`] : []),
    ...(zustellfehlerOhneEmpfaenger > 0 ? [`Zustellfehler ohne registriertes Abo: ${zustellfehlerOhneEmpfaenger} Mandate.`] : []),
  ...
  return {
    ...
    empfaengerUnbekannt,
    zugestellt, zustellfehler, zustellfehlerOhneEmpfaenger, ohneEmpfaenger,
...
function klassifiziereMotorZustand({
  ...
  const auswirkungOffen = (num(k.endgueltigFehler) > 0)
