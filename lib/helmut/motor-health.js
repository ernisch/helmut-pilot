...
// `selbstAnker` (Kapazitätssprint 2026-08-31): der Slot wird erst ab der ERSTEN
// eigenen Quittung erzwungen. Anlass: die Rückstandsslots (11:30/17:30 UTC)
// entstehen erst mit ihrem Deployment — am Deploy-Tag existiert für bereits
// verstrichene Slotzeiten strukturell keine Quittung, und ohne Anker meldete der
// Bericht bis zu 24 h lang „Slot fehlt" (dasselbe Fehlalarm-Muster wie der
// belegte 26.08.-Fall, nur zeitlich gespiegelt). Nach der ersten Quittung gilt
// die volle Slot-Erwartung.
const SLOT_PLAN = Object.freeze([
  ...
  { process: "understanding-rueckstand", stundenUtc: [11.5, 17.5], selbstAnker: true },
...
function pruefeSlotQuittungen({
  ...
  for (const eintrag of plan) {
    ...
    for (const stunde of eintrag.stundenUtc) {
      ...
      if (eintrag.selbstAnker) {
        // Selbstverankerte Slots: vor der ersten eigenen Quittung nicht erwartbar
        // (Deploy-Tag), danach volle Erwartung ab dem ersten belegten Lauf.
        const erste = des.map(zeit).filter((t) => t != null).sort((a, b) => a - b)[0] ?? null;
        if (erste == null || slotMs < erste - SLOT_VORLAUF_MS) continue;
      }
...
// `messbar: false` heisst „der Mandantenspeicher war nicht lesbar" — dann ist das
// Alter UNBEKANNT. „nie" waere an dieser Stelle eine Behauptung ohne Beleg (der
// Lage-Check kann sehr wohl gelaufen sein), und ein Rueckstand ist nicht ableitbar.
function lageRotationsHinweis({ lageAlterMs = null, mandate = null, messbar = true } = {}) {
  ...
  const alterText = messbar ? fmtAlter(lageAlterMs) : "nicht lesbar";
  const rueckstand = messbar && lageAlterMs != null && lageAlterMs > 24 * 3600e3;
...
function leiteMotorHinweise({ ingest = null, coverage = null, errors24 = 0, lageHinweis = null } = {}) {
  ...
  if (ingest && ingest.status === "success") {
    const grund = String(ingest.reason || "");
    if (grund === "blob-spiegel-fehlgeschlagen") hinweise.push("spiegel-write-fehlgeschlagen-wert-unbekannt");
    // Der relationale Rohdokumentbestand ist bei diesem Befund vollstaendig; nur der
    // abgeleitete Blob-Lesespiegel wurde an seiner festen Speichergrenze gekappt. Die
    // Zahl bleibt im Berichtstext, der Slug absichtlich stabil fuer Webhook-Dedupe.
    else if (grund.startsWith("blob-spiegel-ueberlauf:")) hinweise.push("spiegel-ueberlauf");
...
function klassifiziereMotorZustand({
  ...
  // Messlücken, die der Aufrufer schon kennt (z. B. „Mandantenspeicher nicht lesbar").
  // Sie machen den Zustand „nicht bestimmbar" — nie Grün, nie Rot.
  zusatzUnbestimmt = []
} = {}) {
  const gruende = [];
  const unbestimmt = [...(Array.isArray(zusatzUnbestimmt) ? zusatzUnbestimmt : [])];
...
// ── DRAIN-BILANZ (PR-C, Kapazitätssprint 2026-08-31; Blocker-3-Korrektur
// 500-Mandate-Reife 2026-09-01) — reine Darstellungs-/Ableitungslogik ─────────────
// Die Abnahmegroesse "ECHTER gate-wuerdiger Abfluss >= gate-wuerdige Ankunft UND
// der verarbeitbare Rueckstand waechst nicht" als EINE ehrliche Berichtszeile.
// Jede Groesse einzeln: null/undefined => "?" (nicht messbar), nie eine erfundene 0.
// Rein, testbar, ohne I/O.

// Trendurteil aus Anfangs- und Endwert des verarbeitbaren Rueckstands. Fehlt
// eine Seite, ist das Urteil `unvollstaendig` — NIE gruen.
function bewerteRueckstandsTrend({ anfang = null, ende = null } = {}) {
  const messbar = anfang != null && ende != null
    && Number.isFinite(Number(anfang)) && Number.isFinite(Number(ende));
  if (!messbar) return { messbar: false, trend: null, urteil: "unvollstaendig" };
  const trend = Number(ende) - Number(anfang);
  return { messbar: true, trend, urteil: trend > 0 ? "wachsend" : (trend < 0 ? "sinkend" : "stabil") };
}

// Waehlt aus den persistierten Tages-Schnappschuessen den VERGLEICHSANKER: den
// juengsten Eintrag, dessen Alter im Fenster [minAlter, maxAlter] liegt (Default
// 20-40 h — der gestrige Tagesanker beim taeglichen 06:00-Lauf). Kein passender
// Eintrag => null (Trend bleibt `unvollstaendig`; erster Tag nach Deploy).
function waehleTrendAnker(eintraege = [], { nowMs = Date.now(), minAlterMs = 20 * 3600e3, maxAlterMs = 40 * 3600e3 } = {}) {
  let bester = null;
  for (const e of Array.isArray(eintraege) ? eintraege : []) {
    if (!e || !Number.isFinite(Number(e.wert))) continue;
    const t = Date.parse(e.um || "");
    if (!Number.isFinite(t)) continue;
    const alter = nowMs - t;
    if (alter < minAlterMs || alter > maxAlterMs) continue;
    if (!bester || t > Date.parse(bester.um)) bester = e;
  }
  return bester;
}

// Blocker 3: `abfluss` ist jetzt der ECHTE gate-wuerdige Abfluss (Vorgaenge mit
// mindestens einer persistierten verstehen/zurueckstellen-Entscheidung), nie mehr
// die Bruttomenge aller complete-Beruehrungen. Ein GRUENES ✓ verlangt ALLE drei
// Bedingungen: Abfluss >= Ankunft, Rueckstandstrend nicht wachsend, alle
// Groessen messbar. Waechst der Rueckstand oder fehlt eine Messgroesse, gibt es
// KEIN Gruen (unvollstaendige Daten werden ausdruecklich benannt).
function drainBilanzZeile({
  ankunftWuerdig = null, abfluss = null, abflussUnbewertet = null,
  rueckstandAnfang = null, rueckstandEnde = null,
  gateGeparkt = null, fehlversuchsQuote = null
} = {}) {
  const z = (v) => (v == null || !Number.isFinite(Number(v)) ? "?" : String(Number(v)));
  const beide = ankunftWuerdig != null && abfluss != null
    && Number.isFinite(Number(ankunftWuerdig)) && Number.isFinite(Number(abfluss));
  const trendUrteil = bewerteRueckstandsTrend({ anfang: rueckstandAnfang, ende: rueckstandEnde });
  let richtung = "";
  if (!beide || !trendUrteil.messbar) {
    richtung = " · Daten unvollständig (kein Urteil)";
  } else if (Number(abfluss) < Number(ankunftWuerdig)) {
    richtung = " ⚠️ Abfluss < Ankunft";
  } else if (trendUrteil.urteil === "wachsend") {
    richtung = " ⚠️ Rückstand wächst";
  } else {
    richtung = " ✓";
  }
  const trendText = trendUrteil.messbar
    ? `${z(rueckstandAnfang)}→${z(rueckstandEnde)} (${trendUrteil.trend > 0 ? "+" : ""}${trendUrteil.trend}, ${trendUrteil.urteil})`
    : `${z(rueckstandAnfang)}→${z(rueckstandEnde)} (unvollständig)`;
  const quote = fehlversuchsQuote == null ? "?" : `${Math.round(Number(fehlversuchsQuote) * 100)}%`;
  const unbewertet = abflussUnbewertet != null && Number.isFinite(Number(abflussUnbewertet)) && Number(abflussUnbewertet) > 0
    ? ` (+${Number(abflussUnbewertet)} unbewertet)` : "";
  return `Verstehen (24h): Ankunft würdig ${z(ankunftWuerdig)} · Abfluss würdig ${z(abfluss)}${unbewertet}${richtung}`
    + ` · Rückstand ${trendText} · Gate geparkt ${z(gateGeparkt)} · Fehlversuchsquote Rückstand ${quote}`;
}

// Fehlversuchsquote der Rueckstandslaeufe aus den BEREITS geladenen Quittungen
// (kein neuer Fetch): fehlgeschlagen / (gespeichert + fehlgeschlagen) der letzten
// `fensterMs`. null, wenn kein Lauf im Fenster oder Zaehler nicht lesbar —
// 0 Laeufe sind keine Quote von 0.
function rueckstandFehlversuchsQuote(quittungen = [], { nowMs = Date.now(), fensterMs = 24 * 3600e3 } = {}) {
  const seit = nowMs - fensterMs;
  let gespeichert = 0, fehlgeschlagen = 0, laeufe = 0;
  for (const q of Array.isArray(quittungen) ? quittungen : []) {
    if (!q || q.process !== "understanding-rueckstand") continue;
    const t = Date.parse(q.startedAt || q.createdAt || "");
    if (!Number.isFinite(t) || t < seit) continue;
    const g = Number(q.gespeichert), f = Number(q.fehlgeschlagen);
    if (!Number.isFinite(g) || !Number.isFinite(f)) continue; // nicht abrechenbarer Lauf zaehlt nicht als 0
    gespeichert += g; fehlgeschlagen += f; laeufe += 1;
  }
  if (!laeufe || (gespeichert + fehlgeschlagen) === 0) return null;
  return fehlgeschlagen / (gespeichert + fehlgeschlagen);
}
...
module.exports = {
  ...
  slotDarstellung,
  drainBilanzZeile,
  bewerteRueckstandsTrend,
  waehleTrendAnker,
  rueckstandFehlversuchsQuote,
  OEFFNUNG_MESSBAR,
