...
function plan(vorgang) {
  ...
  if (vorgang === "quellenkontext") return { ziel: ZIEL, vorgang, modellaufrufe: 0,
...
function pruefeSnapshot(s, vorgang) {
  const voll = ["reaktivierung", "500-bestand"].includes(vorgang);
  ...
  fordere(geschuetzt.length === (voll ? 9 : 29) && geschuetzt.filter((m) => m.aktiv).length === (voll ? 5 : 25)
