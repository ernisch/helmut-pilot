...
const ALLE_KENNUNGEN = Object.freeze([...GRUPPEN_KENNUNGEN.a, ...KENNUNGEN]);
const ZIELMENGE = new Set(KENNUNGEN);
const A_MENGE = new Set(GRUPPEN_KENNUNGEN.a);
const SPECS = new Map(baueKohorte().map((s) => [s.id, s]));
const BUND = require("./testkohorte-bundestag");
const BUND_SPECS = new Map(BUND.baueBundestagsKohorte().map(s => [s.id, s]));
const WORTE = Object.freeze({
  ...
  reaktivierung: "TESTKOHORTE_ZIEL_500_495_REAKTIVIEREN_BESTAETIGT",
  ...
  quellenkontext: "TESTKOHORTE_500_FEHLENDE_QUELLENAUSZUEGE_BESTAETIGT",
  "quellenkontext-ruhe": "TESTKOHORTE_500_QUELLENAUSZUEGE_BEI_NULL_AKTIVEN_BESTAETIGT",
  quellenvorlauf: "TESTKOHORTE_500_QUELLENVORLAUF_BEI_NULL_AKTIVEN_BESTAETIGT"
});
const QUELLENVORLAUF_MAX_SOURCE_FETCH = 200;
const QUELLENVORLAUF_MAX_MS = 20 * 60 * 1000;
...
function plan(vorgang) {
  ...
  if (vorgang === "quellenvorlauf") return { ziel: ZIEL, vorgang, modellaufrufe: 0,
    maxSourceFetch: QUELLENVORLAUF_MAX_SOURCE_FETCH, maxArbeitszeitMs: QUELLENVORLAUF_MAX_MS,
    profileSchreiben: false, aktiveProfileVorher: 0, aktiveProfileNachher: 0,
    bestandsauswahlErforderlich: true, bestaetigung: WORTE[vorgang],
    automatischeWiederholung: false, funktionsnachweis500: false, quellenversorgung500: false };
  if (["quellenkontext", "quellenkontext-ruhe"].includes(vorgang)) return { ziel: ZIEL, vorgang, modellaufrufe: 0,
    maxArtikelAbrufe: 40, maxQuellzeilen: 40, maxArbeitszeitMs: 480000, profileSchreiben: false,
    ...(vorgang === "quellenkontext-ruhe" ? { aktiveProfileVorher: 0, aktiveProfileNachher: 0, bestandsauswahlErforderlich: true } : {}),
    bestaetigung: WORTE[vorgang], automatischeWiederholung: false, funktionsnachweis500: false };
  if (vorgang === "reaktivierung") return { ziel: ZIEL, vorgang, vorhandeneAktive: 5,
    vorhandeneTestprofile: 495, neuAnlegen: false, kontenAktivieren: false,
    bestaetigung: WORTE[vorgang], automatischeWiederholung: false, funktionsnachweis500: false };
...
function pruefeSnapshot(s, vorgang) {
  const bereinigt = ["500-bereinigt-ruhend", "500-bereinigt-bestand"].includes(vorgang);
  const ruhend = ["500-bereinigt-ruhend", "500-ruhend"].includes(vorgang);
  const voll = bereinigt || ruhend || ["reaktivierung", "500-bestand"].includes(vorgang);
  const kennungen = voll ? ALLE_KENNUNGEN : KENNUNGEN;
  const zielmenge = new Set(kennungen);
  ...
  fordere(geschuetzt.length === (bereinigt ? 5 : voll ? 9 : 29) && geschuetzt.filter((m) => m.aktiv).length === (ruhend ? 0 : voll ? 5 : 25)
    && (voll || [...A_MENGE].every((id) => mandate.get(id)?.aktiv === true)), "geschuetzter-profilbestand-abweichend");
  ...
  fordere(geschuetzteIdentitaeten.length === (bereinigt ? 6 : voll ? 10 : 30) && geschuetzteKonten.length === (bereinigt ? 3 : voll ? 5 : 25)
    && geschuetzteKonten.filter((u) => u.active === true).length === 3,
    "geschuetzte-identitaeten-oder-konten-abweichend");
  const vorhandene = [];
  const aktive = [];
  // Nur eine vollstaendig umgestellte 495er Kohorte darf die zweite exakte
  // Sollmenge verwenden. Teilumstellung und falsche Ausschuesse bleiben Fehler.
  const bundestagsKohorte = voll && BUND.UMSTELL_IDS.every(id => mandate.get(id)?.politische_ebene === "bundestag");
  const specs = bundestagsKohorte ? BUND_SPECS : SPECS;
  ...
  if (voll) fordere(vorhandene.length === 495, "reaktivierung-braucht-495-vollstaendige-profile");
  if (ruhend) fordere(aktive.length === 0, "ruhender-bestand-braucht-null-aktive-profile");
...
// ── SCHUTZPROJEKTION DES QUELLEN-VORLAUFS ────────────────────────────────────────────────────
// EIGENER, bewusst ENGER Vertrag. `pruefeSnapshot` (oben) bleibt dafuer UNVERAENDERT und
// laeuft im Quellen-Vorlauf weiterhin VORGESCHALTET: Zielmenge, Kohorteninhalt und der
// 504/0-Zustand bleiben damit fail closed geschuetzt. Diese Funktion liefert nur die
// Vergleichsgrundlage der abschliessenden Integritaetspruefung.
//
// WARUM NICHT DER GANZE SNAPSHOT: `main-auth` ist EIN Blob, den JEDER Auth-Schreiber als
// GANZE `data`-Spalte ersetzt (Sessions, Nutzungs-Tracking, Fehlertelemetrie, Kostenring) und
// der mit `_authStoreRevision` einen Wert traegt, der sich bei JEDEM Schreiben aendert
// (lib/helmut/auth-store-cas.js). Ein Hash ueber den gesamten Blob kann in einem bis zu
// 20-minuetigen Lauf bei lebendigem Auth-Store strukturell NICHT stabil bleiben und meldet
// legitime Kontonutzung als Schutzverletzung. Belegter Anlass: Quellen-Vorlauf-Lauf
// 35732896378 (38/38 Abrufe erfolgreich, 137 neue Rohdokumente, Abschluss nur wegen des zu
// breiten Ganz-Blob-Hashes rot; `main`, `profiles` und `mandate_profiles` nachweislich
// unveraendert, `main-auth` durch einen fremden Auth-Schreiber veraendert).
//
// Der Vertrag schuetzt deshalb NUR Identitaet, Zugriff und Fachinhalt:
//   * mandate_profiles  — kompletter Fachinhalt; `updated_at` ist reines Laufzeitfeld und
//                         wird ignoriert. `aktiv` bleibt AUSDRUECKLICH enthalten (die
//                         504/0-Invariante erzwingt zusaetzlich `pruefeSnapshot`).
//   * profiles          — Identitaet ALLER Profile, ungekuerzt.
//   * auth.users        — je Konto nur die stabile Sicherheitsprojektion (siehe kontenFeld).
//   * auth.assignments  — vollstaendige Zuordnungsstruktur.
//   * main.crawlRuns    — wie im bestehenden Schutzvertrag.
// AUSDRUECKLICH NICHT enthalten sind reine Laufzeit-/Nutzungsmetadaten: `_authStoreRevision`,
// sessions, passwordTokens, dailyInputs, auditEvents, systemErrors, llmUsage, pipelineLocks,
// adminRecoveryLastRun, processRuns, understandingRetries, updateRetries,
// monitoringWebhookDelivery, sourceModeShadowLastRun, testKostenTage sowie je Konto
// lastSeenAt/lastLoginAt/loginCount/openCount/updatedAt. Keines davon steuert Identitaet oder
// Zugriff; alle duerfen sich waehrend eines laufenden Quellen-Vorlaufs legitim aendern.
//
// `paidUntil`, `customer` und `notificationSettings` bleiben ebenfalls draussen: sie werden
// im Code an keiner Stelle zur Zugriffs- oder Identitaetsentscheidung ausgewertet (nur
// Anzeige/Admin-Patch), sind also kein Schutzgut dieses Nachweises.
function quellenvorlaufSchutzprojektion(s = {}) {
  const quelle = s && typeof s === "object" ? s : {};
  const auth = quelle.auth && typeof quelle.auth === "object" ? quelle.auth : {};
  const kontenFeld = u => ({
    id: u?.id ?? null,
    name: u?.name ?? null,
    email: u?.email ?? null,
  ...
  return {
    mandate: sortiert((quelle.mandate || []).map(({ updated_at, ...rest }) => rest), "user_id"),
    identitaeten: sortiert(quelle.identitaeten || [], "id"),
    konten: sortiert((auth.users || []).map(kontenFeld), "id"),
    assignments: sortiert(auth.assignments || [], "id"),
    crawlRuns: quelle.main && Array.isArray(quelle.main.crawlRuns) ? quelle.main.crawlRuns : null
  };
}
...
async function fuehreAus({ vorgang, env = process.env, grundlinieHash, deps = {} } = {}) {
  ...
  try {
    fordere(["provisionierung", "aktivierung", "reaktivierung"].includes(vorgang), "fachzyklus-braucht-eigenen-ausfuehrer");
    ...
    const zielKennungen = vorgang === "reaktivierung" ? ALLE_KENNUNGEN : KENNUNGEN;
    for (const [index, id] of zielKennungen.entries()) {
      ...
      fordere(row && row.user_id === id && row.geloescht_at === null
        && row.aktiv === (vorgang !== "provisionierung"), "zielzustand-nicht-persistiert");
      bestaetigt += 1;
      if (bestaetigt % 10 === 0) {
        await kontrolliereBestand();
        if (deps.fortschritt) deps.fortschritt({ ziel: 500, vorgang, bestaetigt,
          bereitsVorhanden: bereits.size, offen: zielKennungen.length - bereits.size - bestaetigt });
      }
    }
    await kontrolliereBestand();
    fordere(nachher.gesamt === 504 && nachher.aktiv === (vorgang !== "provisionierung" ? 500 : 25),
...
module.exports = { ZIEL, KENNUNGEN, ALLE_KENNUNGEN, WORTE, KLASSEN,
  QUELLENVORLAUF_MAX_SOURCE_FETCH, QUELLENVORLAUF_MAX_MS,
  DirektAbbruch, fordere, hash,
  plan, pruefeAbnahmeA, pruefeSnapshot, pruefeZeit, fuehreAus, quellenvorlaufSchutzprojektion };
