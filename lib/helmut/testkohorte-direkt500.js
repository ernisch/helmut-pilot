...
// Expliziter Betreiberweg 25 -> 500 (SR §55). Die alten A/B/C Vertraege
...
function plan(vorgang) {
  ...
  if (vorgang === "fachzyklus") return { ziel: ZIEL, vorgang, pipelineRunden: 1,
    profileSchreiben: false, bestaetigung: WORTE[vorgang], aAbnahmeErforderlich: true,
  ...
  return { ziel: ZIEL, vorgang, zusaetzlicheProfile: KENNUNGEN.length,
    ...
    bestaetigung: WORTE[vorgang], aAbnahmeErforderlich: true,
...
function pruefeZeit(zeit) {
  ...
  // SR §55: 21:36 bis 03:59. Eine Minute verbleibt fuer die letzte Zeile.
  fordere(minute >= 21 * 60 + 36 || minute < 3 * 60 + 58, "direktausbau-ausserhalb-des-nachtfensters");
}

async function fuehreAus({ vorgang, env = process.env, abnahmeA, deps = {} } = {}) {
  ...
  try {
    fordere(vorgang !== "fachzyklus", "fachzyklus-braucht-eigenen-ausfuehrer");
    ...
    pruefeZeit(start);
    const abnahme = pruefeAbnahmeA(abnahmeA, start);
    await deps.pruefeBetrieb();
    vorher = pruefeSnapshot(await deps.leseSnapshot(), vorgang);
    fordere(vorher.geschuetzterBestandHash === abnahmeA.geschuetzterBestandHash
      && [...vorher.aktiveKennungen].every((id) => abnahme.aktiveKennungen.has(id)),
