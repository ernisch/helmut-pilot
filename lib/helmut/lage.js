...
//
// UI-Zeigefelder (displayTitle/displaySummary/whyRelevant/recommendation/
...
// KI-Aufruf hier). Aeltere Vorgaenge ohne diese Felder liefern leere Strings —
// der Client faellt dafuer auf die bestehenden Felder zurueck, OHNE Titel
// abzuschneiden (siehe renderVorgangCard in client.js).
//
...
async function loadRankedVorgaenge(storage, matchingFn, profile, userId) {
  const all = await storage.listKnowledgeObjects({ limit: 200 });
...
async function buildLageBriefing(profile = {}, opts = {}) {
  ...
  const ranked = await loadRankedVorgaenge(storage, matchingFn, profile, userId);
  if (!ranked.length) return unavailable("no-vorgaenge");
  const sourcesRaw = await loadSourcesForVorgaenge(storage, ranked);
  const cards = ranked.map((ko) => koToVorgangCard(ko, sourcesRaw[ko.vorgang_id]));
  const sourcesByVorgang = {};
  cards.forEach((c) => { sourcesByVorgang[c.vorgangId] = c.sources; });
  ...
  try {
    // 4) Budget-Gate (global, wie Understanding) — bei Erschoepfung KEIN Fake-Briefing.
    let budget = { allowed: true };
    try { budget = await storage.canSpendLlm(null); } catch (_) { budget = { allowed: true }; }
