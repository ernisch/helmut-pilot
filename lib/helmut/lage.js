...
// "Modern" = alle fünf Anzeigefelder sind gefüllt (die Understanding-Engine
// erzeugt sie gemeinsam in EINEM Lauf, daher ganz-oder-gar-nicht).
function isModernVorgang(card) {
  return card != null && LAGE_PRESENTATION_FIELDS.every((f) => String(card[f] == null ? "" : card[f]).trim() !== "");
}
function selectLageVorgaenge(vorgaenge = []) {
  ...
  // Nie mischen: entweder-oder, nie beide Mengen zusammen.
