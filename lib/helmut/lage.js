...
// Nur fuer die byte-stabile Rangfolge gespeicherter Matching-Zeilen
// (compareStoredMatchingRows) — bewusst kein localeCompare.
...
async function loadRankedVorgaenge(storage, matchingFn, profile, userId) {
  ...
  try {
    ...
    if (Array.isArray(stored) && stored.length) {
      ...
      // Rangfolge: der BERECHNETE Rang entscheidet, nicht der Schreibzeitpunkt
      // (Befund F-E2E; Begruendung bei matching-contract.compareStoredMatchingRows).
      ranked = stored.slice().sort(contract.compareStoredMatchingRows)
