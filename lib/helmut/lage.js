...
async function loadRankedVorgaenge(storage, matchingFn, profile, userId) {
  ...
  const understood = (all || []).filter(istVerstanden);
  ...
  if (ranked.length < MAX_VORGAENGE && typeof matchingFn === "function") {
    try {
      ...
      for (const match of m || []) {
        ...
        if (ko && !gesehen.has(ko.id)) { ranked.push(ko); gesehen.add(ko.id); }
      }
    } catch (_) { /* ignore */ }
  }
...
async function buildLageBriefing(profile = {}, opts = {}) {
  ...
  const sourcesRaw = gebunden ? gebunden.sources : await loadSourcesForVorgaenge(storage, rankedAll);
