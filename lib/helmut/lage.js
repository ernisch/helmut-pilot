...
async function buildLageBriefing(profile = {}, opts = {}) {
  ...
  // Jeder vorhandene Tagessatz bleibt erhalten, auch bei inzwischen anderem
  // Quellenhash. Lesefehler sind keine Erlaubnis zur Neuerzeugung.
  if (opts.missingOnly) {
    if (opts.force || opts.cacheOnly || opts.countOnly) throw new Error("nachlauf-optionen-widerspruechlich");
    if (await storage.getRenderedBriefingV3(userId, CACHE_SLOT, berlinDayKey(), { strict: true })) {
  ...
  if (!opts.force) {
    try {
      ...
      if (cached && quellenbeleg.cacheGueltig(cached.payload, koSetHash, quellenHash, koLite)) {
