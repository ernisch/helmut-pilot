...
// Nur fuer den byte-stabilen Vergleich (compareIds) — bewusst kein localeCompare.
...
// Lesefenster fuer die gespeicherten Matching-Zeilen — BEWUSST groesser als
// MAX_VORGAENGE (Reviewbefund F-E2E-2, 2026-08-04).
//
// Die Ablage schneidet jetzt nach RANG ab. Direkt danach verwirft
// `loadRankedVorgaenge` aber jede Zeile, deren Wissensobjekt NICHT im
// KO-Scanfenster liegt (`HELMUT_KO_SCAN_LIMIT`, Default 500 der zuletzt
// GEAENDERTEN von aktuell 2 422). Wird schon in der Ablage auf MAX_VORGAENGE
// gekappt, ist der Rest der Rangliste unerreichbar: die bestplatzierten Zeilen
// zeigen oft auf aeltere Wissensobjekte, und die Lage bleibt leerer, als der
// Bestand hergibt.
//
// Am Production-Bestand vom 2026-08-04 gemessen (rein lesend, 7 Mandate mit je
// 20 aktuellen Zeilen): sichtbare Vorgaenge mit Lesefenster 12 = 15, mit dem
// vollen Bestand = 31 (der alte Lesevertrag `created_at.desc` lag bei 30). Ein
// Mandat fiel mit Fenster 12 auf 0 und damit still in den unpersonalisierten
// Rueckfall. Das Fenster bleibt hart begrenzt (Kosten/Antwortgroesse); der
// sichtbare Schnitt auf MAX_VORGAENGE passiert unveraendert in dieser Datei,
// jetzt aber NACH dem KO-Fenster statt davor.
const MATCHING_LESEFENSTER = Math.min(200, Math.max(60, MAX_VORGAENGE * 5));
...
// Rangfolge der gespeicherten Matching-Zeilen: der BERECHNETE Rang entscheidet,
// nicht der Schreibzeitpunkt (Befund F-E2E, 2026-08-04). `created_at` taugt dafuer
// nicht: alle Zeilen eines Laufs bekommen denselben Wert (ein Statement, ein
// `now()`) und bereits vorhandene Zeilen behalten beim Upsert ihren ALTEN Wert
// (helmut_publish_matching_run, Schritt 2) — die Reihenfolge waere damit
// „zuerst gesehen" statt „am relevantesten", und bei Gleichstand unbestimmt.
// Zeilen ohne Rang (Altbestand) stehen hinten, nie vor einem belegten Rang.
function vergleicheGespeicherteRaenge(a, b) {
  const r = (row) => {
    const roh = row ? row.rank : null;
    // Nur eine echte Zahl zaehlt. `Number(null)`/`Number("")`/`Number(false)`
    // ergaeben 0 und wuerden eine Zeile OHNE Rang an die Spitze heben.
    if (roh == null || roh === "" || typeof roh === "boolean") return Number.POSITIVE_INFINITY;
    const n = Number(roh);
    return Number.isFinite(n) ? n : Number.POSITIVE_INFINITY;
  };
  return (r(a) - r(b)) || contract.compareIds(
    (a && a.knowledge_object_id) || "", (b && b.knowledge_object_id) || ""
  );
}
...
async function loadRankedVorgaenge(storage, matchingFn, profile, userId) {
  ...
  if (scoring.scoringActive()) {
    return scoring.rankForLage(understood, { limit: MAX_VORGAENGE }).map((x) => x.ko);
  }
  ...
  try {
    const stored = await storage.listMatchingResults({ userId, limit: MATCHING_LESEFENSTER });
    if (Array.isArray(stored) && stored.length) {
      // Die Reihenfolge wird hier NOCHMALS festgelegt (nicht von der Ablage
      // uebernommen): die Ausgabe darf nicht davon abhaengen, in welcher
      // Reihenfolge PostgREST gleichwertige Zeilen zurueckgibt. Mitgefuehrt wird
      // zusaetzlich die bereits gespeicherte Relevanzerklaerung (Sprint 23C).
      // Gibt es fuer eine Zeile keinen Beleg, bleibt das KO unveraendert und die
      // Oberflaeche zeigt den Abschnitt schlicht nicht.
      //
      // REIHENFOLGE DER SCHRITTE IST FACHLICH: sortieren -> Zeilen ohne
      // Wissensobjekt im Scanfenster verwerfen -> ERST DANN auf MAX_VORGAENGE
      // kappen (Reviewbefund F-E2E-2). Wird vorher gekappt, verbrauchen Zeilen
      // ohne ladbares Wissensobjekt die sichtbaren Plaetze.
      ranked = stored.slice().sort(vergleicheGespeicherteRaenge)
        .filter((m) => m && byId.has(m.knowledge_object_id))
...
async function buildLageBriefing(profile = {}, opts = {}) {
  ...
  try {
    ...
    if (!budget || !budget.allowed) {
      ...
      try { storage.recordLlmUsage({ callType: "skipped-lage-narrativ", politicianId: userId, success: false, error: budget && budget.reason ? String(budget.reason) : "budget" }); } catch (_) { /* Telemetrie darf Lage nie brechen */ }
