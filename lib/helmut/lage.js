...
async function buildLageBriefing(profile = {}, opts = {}) {
  ...
  const absatzQuellen = Object.fromEntries(koLite.map(v => [v.vorgang_id,
    v.quellenbelege.map(q => {
      ...
      return mapSource({ ...original, url: q.url, source_name: q.quelle,
        title: q.titel, published_at: q.veroeffentlichtAm });
    })]));
  ...
  try {
    ...
    if (opts.missingOnly && await storage.getRenderedBriefingV3(userId, CACHE_SLOT, day, { strict: true })) {
    ...
    if (opts.missingOnly) {
      ...
      try { await storage.saveRenderedBriefingV3(entry); }
      catch (_) { /* Historischer interaktiver Pfad unveraendert. */ }
    }
