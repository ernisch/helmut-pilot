...
function zeilenkennungen(env = process.env) {
  const e = env || {};
  const blob = e.HELMUT_SUPABASE_STORE_ID || "main";
  return Object.freeze({
    blob,
    auth: e.HELMUT_SUPABASE_AUTH_STORE_ID || `${blob}-auth`,
    verschoben: Boolean(e.HELMUT_SUPABASE_STORE_ID) || Boolean(e.HELMUT_SUPABASE_AUTH_STORE_ID),
    vorgabe: { blob: "main", auth: "main-auth" }
  });
}
...
function pruefeSpeicherpfad({
  ...
  befunde.push(Object.freeze({
    ...
    detail: ziel.zeilenkennungen.verschoben
      ? `HELMUT_SUPABASE_STORE_ID/HELMUT_SUPABASE_AUTH_STORE_ID verschieben das Ziel auf `
        + `"${ziel.zeilenkennungen.blob}" / "${ziel.zeilenkennungen.auth}". Die Erhebungs-SQL der `
        + "Kohortenwerkzeuge liest weiterhin main/main-auth — geschrieben und kontrolliert wuerde "
        + "dann NICHT dieselbe Zeile."
  ...
  zeilen.push(`  Geteilte Zeile          : ${ziel.zeilenkennungen.blob}`
    ...
    + `${ziel.zeilenkennungen.verschoben ? "  ← VERSCHOBEN gegenüber der Vorgabe" : ""}`);
  ...
  if (!sicher) {
    ...
    zeilen.push("  HELMUT_CRAWL_RUN_RETENTION (Production: 36) · HELMUT_PROFILE_DB_MODE");
