...
async function runMorningBriefing(politicianId = cemInceProfile.id, { skipAi = false } = {}) {
  ...
  const linkStrictLiveBriefing = liveBriefing ? requirePreciseBriefingLinks(skipAi ? liveBriefing : await enrichBriefing(liveBriefing, profile)) : null;
  ...
  const fallbackLiveBriefing = !hasDecisionValue(linkStrictLiveBriefing) && fallbackBriefingInputItems.length
    ...
    : null;
  const linkStrictFallbackBriefing = fallbackLiveBriefing ? requirePreciseBriefingLinks(skipAi ? fallbackLiveBriefing : await enrichBriefing(fallbackLiveBriefing, profile)) : null;
