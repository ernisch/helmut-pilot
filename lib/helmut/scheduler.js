...
async function runMorningBriefing(politicianId = cemInceProfile.id, { skipAi = false } = {}) {
  ...
  const linkStrictLiveBriefing = liveBriefing ? requirePreciseBriefingLinks(skipAi ? liveBriefing : await enrichBriefingWithAI(liveBriefing, profile)) : null;
  ...
  const fallbackLiveBriefing = !hasDecisionValue(linkStrictLiveBriefing) && fallbackBriefingInputItems.length
    ...
    : null;
  const linkStrictFallbackBriefing = fallbackLiveBriefing ? requirePreciseBriefingLinks(skipAi ? fallbackLiveBriefing : await enrichBriefingWithAI(fallbackLiveBriefing, profile)) : null;
  ...
  const helmutAssessment = skipAi ? "" : await generateHelmutAssessment({ briefing: qualityCheckedBriefing, profile });
  const finalBriefing = {
    ...qualityCheckedBriefing,
    helmutAssessment
  };
  await savePoliticalItems(finalBriefing.politicalItems || []);
  await savePersonalizedRecommendations(profile.id, finalBriefing.personalizedRecommendations || []);
  await savePriorityChanges(finalBriefing.priorityChanges || []);
...
function mergeProfileDefaults(profile) {
  return {
    ...
    function: profileValue(profile.function, cemInceProfile.function),
    constituency: profileValue(profile.constituency, cemInceProfile.constituency),
    state: profileValue(profile.state, cemInceProfile.state),
    location: profileValue(profile.location, cemInceProfile.location),
    mainQuestion: profileValue(profile.mainQuestion, cemInceProfile.mainQuestion),
    communicationStyle: profileValue(profile.communicationStyle, cemInceProfile.communicationStyle),
    committees: mergeArrayValue(profile.committees, profile.committee ? [profile.committee] : cemInceProfile.committees),
    focusTopics: mergeArrayValue(profile.focusTopics, cemInceProfile.focusTopics),
    topicPriorities: topicPriorityValue(profile.topicPriorities, cemInceProfile.topicPriorities),
    regionalInterests: mergeArrayValue(profile.regionalInterests, cemInceProfile.regionalInterests),
    relevantMinistries: mergeArrayValue(profile.relevantMinistries, cemInceProfile.relevantMinistries),
    monitoringTargets: mergeArrayValue(profile.monitoringTargets, cemInceProfile.monitoringTargets),
    outputNeeds: mergeArrayValue(profile.outputNeeds, cemInceProfile.outputNeeds),
    opponents: mergeArrayValue(profile.opponents, cemInceProfile.opponents),
    localMedia: mergeArrayValue(profile.localMedia, cemInceProfile.localMedia),
    noGoTopics: mergeArrayValue(profile.noGoTopics, cemInceProfile.noGoTopics),
    reportingTopics: mergeArrayValue(profile.reportingTopics, cemInceProfile.reportingTopics),
    currentCampaigns: mergeArrayValue(profile.currentCampaigns, cemInceProfile.currentCampaigns),
    publicPositions: mergeArrayValue(profile.publicPositions, cemInceProfile.publicPositions),
    keyAudiences: mergeArrayValue(profile.keyAudiences, cemInceProfile.keyAudiences),
    riskTopics: mergeArrayValue(profile.riskTopics, cemInceProfile.riskTopics),
    opportunityTopics: mergeArrayValue(profile.opportunityTopics, cemInceProfile.opportunityTopics),
    preferredChannels: mergeArrayValue(profile.preferredChannels, cemInceProfile.preferredChannels),
    upcomingAppointments: appointmentValue(profile.upcomingAppointments, cemInceProfile.upcomingAppointments)
  };
}
