...
const { enrichBriefingWithAI, generateHelmutAssessment } = require("./ai");

// Textveredelung des regelbasiert priorisierten Briefings (kein Ranking durch die
// KI). Der frühere HELMUT_ENGINE_V2-Umschaltpunkt (hybrides KI-Scoring) ist
// entfernt — es gibt genau einen Enrich-Pfad.
function enrichBriefing(briefing, profile) {
  return enrichBriefingWithAI(briefing, profile);
}
...
async function runSourceCrawl(politicianId = cemInceProfile.id, options = {}) {
  ...
  try {
    ...
    await runDecisionShadow({ profile }).catch(() => {});
...
async function runLageCheck(politicianId = cemInceProfile.id) {
  ...
  const updatedCheck = await saveLageCheck({
    ...check,
    // Feld bleibt für Ops-/Client-Kompatibilität; bedeutet jetzt „V3 neu bewertet".
