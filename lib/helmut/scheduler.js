...
async function runSourceCrawl(politicianId = cemInceProfile.id, options = {}) {
  ...
  // TTL bewusst UNTER der Vercel-maxDuration (300s), analog zu runMorningBriefing:
  // ein durch Function-Timeout abgebrochener Crawl gibt den Lock nie via finally
  // frei, ein TTL ueber der maxDuration wuerde jeden Retry unnoetig lange blockieren.
...
// Profilbasierter Mentions-Check (ersetzt einen frueher hartkodierten "cem ince"-
// String-Vergleich, der fuer jedes andere Mandat nie gegriffen haette).
function mentionsProfileName(text, profile) {
  const fullName = String(profile?.fullName || "").trim();
  if (!fullName) return false;
  return String(text || "").toLowerCase().includes(fullName.toLowerCase());
}

function temperSingleSourceMediaDecisions(briefing, profile) {
  ...
  const items = briefing.items.map((item) => {
    ...
    const hasPersonMention = mentionsProfileName(`${item.title} ${item.summary}`, profile);
