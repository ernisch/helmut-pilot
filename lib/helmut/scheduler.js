...
function mandateNewsSources(profile) {
  ...
  if (ministryQuery) {
    sources.push(newsSearchSource({
      ...
      query: `(${ministryQuery}) ${topics.slice(0, 3).join(" OR ")}`,
      priority: 88,
...
function hasGovernmentSignal(item = {}) {
  ...
  return hasAnyTerm(text, ["bundesregierung", "bundeskabinett", "bmas", "bundesministerium", "arbeitsminister"]);
}

function hasCommitteeSignal(item = {}) {
  const text = `${item.title || ""} ${item.summary || ""} ${item.whyItMatters || ""} ${item.content || ""}`.toLowerCase();
  return hasAnyTerm(text, ["ausschuss", "arbeit und soziales", "bmas", "bundestag"]);
}
