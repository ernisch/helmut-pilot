...
async function runMorningBriefing(politicianId = cemInceProfile.id) {
  ...
  const briefingWithMemory = attachPoliticalMemory(briefingWithStatus, await getTopicMemory(profile.id));
  const savedBriefing = await saveBriefing(briefingWithMemory);
