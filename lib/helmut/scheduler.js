...
async function runMorningBriefing(politicianId = cemInceProfile.id) {
  ...
  const fallbackLiveBriefing = !hasDecisionValue(linkStrictLiveBriefing) && fallbackBriefingInputItems.length
    ...
    : null;
  const linkStrictFallbackBriefing = fallbackLiveBriefing ? requirePreciseBriefingLinks(await enrichBriefingWithAI(fallbackLiveBriefing, profile)) : null;
  ...
  const helmutAssessment = await generateHelmutAssessment({ briefing: qualityCheckedBriefing, profile });
  const finalBriefing = {
