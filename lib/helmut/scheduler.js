...
// Landtag-Sprint (P2-3/P2-5/P2-6): zentrale Parlaments-Registry. Bundestag-Werte sind
// byte-identisch zu den bisherigen Hardcodes; Landtagsprofile erhalten Landes-Pendants.
const { resolveParlament, usesDip } = require("./parlamente");
// Quellenmodus (off/shadow/on) + relationaler Crawl-Plan (Cutover-Vorbereitung, Default off).
const { sourceMode, buildRelationalCrawlPlan, mergeProfileAndPlanSources, buildShadowRunReport } = require("./quellenarchitektur/source-mode");
...
async function runSourceCrawl(politicianId = cemInceProfile.id, options = {}) {
  ...
  try {
    ...
    // Landtag-Sprint (P2-6): DIP ist Bundestag-only — Landtagsmandate erhalten KEINEN
    // automatischen DIP-Abruf mehr (ihre amtliche Primärquelle ist der separat
    // gegatete PARDOK-Weg). Bundestag/unbekannte Ebene: unverändert wie bisher.
    const dipRawItems = isDipEnabled() && usesDip(profile) ? await fetchDipAsRawItems(profile) : [];
    ...
    await recordProcessRun({
      ...
      status: eagerResult && eagerResult.skipped ? "skipped" : "ok"
    }).catch(() => {});
    ...
    const matchingResult = await runMatchingShadow({ profile }).catch(async (error) => {
...
async function foldLageItemsIntoV3(savedItems, profile, runId = null) {
  ...
  await recordProcessRun({
    ...
    status: understanding && understanding.skipped ? "skipped" : "ok"
  }).catch(() => {});
  ...
  await runMatchingShadow({ profile }).catch((error) => {
...
async function runLageCheck(politicianId = cemInceProfile.id) {
  ...
  try {
    ...
    const telemetryRows = buildSourceTelemetryRows({
      ...
      defaultLevel: profile && profile.politicalLevel, newBySource
    });
    await persistSourceCrawlTelemetry(telemetryRows, { insert: insertSourceCrawlTelemetry }).catch(() => {});
  } catch (error) {
...
function selectLageCheckSources(sources = [], limit = lageCheckSourceLimit, profile = null) {
  ...
  return [...sources]
    ...
    .sort((a, b) => lageCheckSourceWeight(b, parlament) - lageCheckSourceWeight(a, parlament))
    .slice(0, Math.max(20, limit));
}

// Landtag-Sprint (P2-5): Strategie-Bonus je Parlament konfigurierbar (Bundestag-Muster
// unverändert = bisheriges Verhalten). 'landesparlament'/'parliament' zählen wie die
// übrigen amtlichen Typen (heute im Katalog nicht vorhanden — additiv, inert für Bund).
function lageCheckSourceWeight(source = {}, parlament = resolveParlament({})) {
  const type = String(source.type || "").toLowerCase();
  const name = `${source.name || ""} ${source.id || ""}`.toLowerCase();
  const base = Number(source.priority || 0);
  const typeWeight = type === "person" ? 1000
    : ["ministry", "bundestag", "committee", "government", "landesparlament", "parliament"].includes(type) ? 900
      : ["party", "faction", "parliamentary_group"].includes(type) ? 850
        : type === "association" ? 720
          : type === "media" ? 650
            : type === "local" ? 600
              : type === "social" ? 250
                : 400;
  const strategicBonus = new RegExp(parlament.relevanz.strategischeQuellenMuster, "i").test(name) ? 120 : 0;
...
async function loadRelationalSharedSources() {
  const rows = await storageListSourceArchitectureRows();
  ...
  return plan.aktiv.map((p) => p.source);
}
...
async function getSourcesForProfile(profile) {
  ...
  if (sourceMode() === "on") {
    try {
      const planSources = await loadRelationalSharedSources();
      if (planSources) {
        const completeness = profileCompleteness(profile);
...
function isDecisionRelevantForProfile(item, profile = cemInceProfile) {
  ...
  const hasMandate = hasAnyTerm(text, buildMandateTerms(profile));
  const hasTrigger = hasAnyTerm(text, decisionTriggerTerms);
  // Landtag-Sprint (P2-5): "Regierungsarbeit" je Parlament — Bundestag-Begriffe unverändert;
  // Landtagsprofile erkennen Senat/Staatskanzlei/Landesregierung als Regierungsarbeit.
  const hasGovernmentWork = hasAnyTerm(text, parlament.regierung.arbeitsBegriffe) && hasMandate;
  const isWeakProtocol = hasAnyTerm(text, weakProtocolTerms) && !hasAnyTerm(text, ["gesetzentwurf", "gesetz", "reform", "verordnung", "beschluss", "kritik", "fordert", "warnt"]);
  ...
  if (parlament.relevanz.amtlicheQuelltypen.includes(item.sourceType)) return hasMandate && (hasTrigger || hasGovernmentWork);
...
function itemPoliticalWeight(item, profile = cemInceProfile) {
  ...
  const mandateTerms = buildMandateTerms(profile);
  let weight = 0;
  // Landtag-Sprint (P2-5): amtliche Quelltypen + politische Schlüsselbegriffe je Parlament
  // (Bundestag-Kataloge byte-identisch zum bisherigen Hardcode).
  if (parlament.relevanz.amtlicheQuelltypen.includes(item.sourceType)) weight += 35;
  if (["party", "association"].includes(item.sourceType)) weight += 22;
  if (item.sourceName && String(item.sourceId || "").includes("-news")) weight += 8;
  if (hasAny(text, parlament.relevanz.politischeBegriffe)) weight += 35;
...
// Landtag-Sprint (P2-3, Ebenen-Default-Fix): Die neutralen Defaults duerfen ein
// Landtagsmandat nicht mehr stillschweigend als Bundestag ausstatten. Erkennt das
// Profil selbst eine Landtags-Ebene (politische_ebene/parliamentType/politicalLevel),
// kommen Funktion/Rolle/Ebene/Ministerien/Monitoring aus der Parlaments-Registry.
// Profile OHNE erkennbare Ebene erhalten unveraendert die Bundestag-Defaults
// (bisheriges Verhalten, Regressionsschutz).
function neutralDefaultsFor(profile = {}) {
  const parlament = resolveParlament(profile);
  if (parlament.key === "bundestag") return neutralProfileDefaults;
  return {
    ...neutralProfileDefaults,
    function: parlament.mitglied.funktion,
    role: parlament.mitglied.funktion,
    parliamentType: "Landtag",
    politicalLevel: parlament.defaults.politicalLevel,
    monitoringTargets: [...parlament.defaults.monitoringTargets],
    relevantMinistries: [...parlament.defaults.relevantMinistries]
  };
}
...
function mergeProfileDefaults(profile = {}) {
  ...
  // Defaults — keine fremden Themen/Ausschuesse/Personen (Ebene siehe neutralDefaultsFor).
  const isDemo = !authModeOn() && String(profile.id || "").trim() === cemInceProfile.id;
  const base = isDemo ? cemInceProfile : neutralDefaultsFor(profile);
  return {
    ...base,
    ...profile,
...
function buildRelevantTerms(profile) {
  // Landtag-Sprint (P2-5): fester Regierungs-Zusatzterm je Parlament (Bundestag:
  // unverändert "Bundesregierung"; Landtag: z. B. "Senat von Berlin" + Parlamentsname).
  const parlament = resolveParlament(profile || {});
  return uniqueTerms([
...
module.exports = {
  ...
  // Landtag-Sprint: ebenen-bewusste Neutral-Defaults + Relevanzfunktionen (Testbarkeit).
  neutralDefaultsFor,
  isDecisionRelevantForProfile,
  itemPoliticalWeight,
  buildRelevantTerms,
  selectLageCheckSources,
  lageCheckSourceWeight,
