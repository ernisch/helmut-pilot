...
async function runGlobaleErfassung(options = {}) {
  ...
  try {
    ...
    //
    // BEFUND (Production, 2026-08-03, OP-25 Kapazitaetsnachweis von HELMUT_CRON_GLOBALABRUF):
    // die Restzeitpruefung oben ist ein START-GATTER zwischen Stufen, kein STOPP-Gatter
    // waehrend einer Stufe — strukturell derselbe Fehler wie R-6
    // (docs/betrieb/cron-fairness.md §11.1), nur an dieser Stelle statt in der
    // Fairnessschleife. Eine begonnene Stufe von `HELMUT_GLOBALPHASE_ABRUF_STUFE` Quellen
    // laeuft ungebremst zu Ende: bei Google-News-Quellen (Gate-Nebenlaeufigkeit
    // `HELMUT_GOOGLE_CONCURRENCY`, Default 5, `google-news-hardening.js`) und dem
    // Quellen-Timeout `CRAWLER_TIMEOUT_MS` (Default 7000 ms) betraegt die schlechtestmoegliche
    // Dauer EINER Stufe `ceil(stufenGroesse / concurrency) * timeoutMs`. Beim frueheren
    // Default 20 sind das bis zu 4 * 7000 ms = 28 000 ms — belegt: der reguläre
    // `cron/pipeline`-Lauf 2026-08-03 16:00 UTC ueberzog sein zugeteiltes Budget
    // (`budgetGlobalMs=221674`) um 45 448 ms auf 267 122 ms, ein spaeterer `cron/crawl`-Lauf
    // 20:00 UTC riss sogar das AEUSSERE 280 000-ms-Limit (`bounded=true`, Lauf als
    // `abgebrochen` vermerkt). Der Default sinkt hier auf die Google-Gate-Nebenlaeufigkeit
    // (5): das bindet die schlechtestmoegliche Ueberziehung je Stufe an EINE Gate-Runde
    // (~timeoutMs) statt an mehrere, OHNE den Gesamtdurchsatz zu senken — das Gate selbst
    // laesst ohnehin nie mehr als `concurrency` Google-Quellen gleichzeitig laufen, unabhaengig
    // davon, wie viele Quellen ihm auf einmal uebergeben werden; direkte/amtliche Quellen
    // (eigener Pool, `CRAWLER_CONCURRENCY`, Default 20) bleiben von der kleineren Stufe
    // unberuehrt, weil das Gate nur die Google-Haelfte einer Stufe seriell zaehlt. Behebt NICHT
    // die zugrundeliegende Kapazitaetsgrenze (der Gesamtabruf braucht bei ~140+ gemeinsamen
    // Google-Wegen weiterhin laenger als das globale Budget zulaesst, siehe
    // `scripts/globalabruf-kapazitaet-test.js` und `docs/betrieb/vorgangskontext.md`) — nur
    // die UNKONTROLLIERTE Ueberziehung darueber hinaus, bis zum Anschlag ans aeussere
    // Zeitlimit.
    const stufenGroesse = Math.max(1, Number(process.env.HELMUT_GLOBALPHASE_ABRUF_STUFE || 5));
    ...
    try {
      if (savedItems.length) {
        ...
        for (const [index, teil] of stapel.entries()) {
          ...
          const teilStichtag = lazyStart + (vorgangskontext.verstehensScheiben({ gesamtMs: lazyBudgetMs, anzahl: stapel.length })[index] || 0);
          for (const cluster of clusters) {
            if (now() > teilStichtag) break;
            lazyRan += 1;
        ...
        console.log(`[globalphase] lazy-understanding ${now() - lazyStart}ms stapel=${stapel.length} clusters=${lazyCluster} processed=${lazyRan} deferred=${lazyCluster - lazyRan}`);
      }
    } catch (error) {
    ...
    for (const [index, teil] of stapel.entries()) {
      ...
      const teilErgebnis = await deps.eagerUnderstanding(teil.dokumente, {
        ...
        vormerkDeadlineMs: startedMs + budgetMs - 5000,
        runId: laufId
      }).catch((error) => ({ skipped: true, reason: "eager-error", error: error && error.message }));
