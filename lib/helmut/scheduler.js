...
async function runSourceCrawl(politicianId = cemInceProfile.id, options = {}) {
  ...
  try {
    ...
    await (async () => {
      ...
      try {
        ...
        for (const cluster of clusters) {
          ...
          await runLazyUnderstandingShadow({ cluster, vorgangId, profiles }).catch((error) => {
            recordPipelineError({ process: "lazy-understanding", runId, error }).catch(() => {});
          });
        }
    ...
    const matchingResult = await runMatchingShadow({ profile }).catch((error) => {
      recordPipelineError({ process: "matching-shadow", runId, error }).catch(() => {});
      return null;
    });
    ...
    const decisionResult = await runDecisionShadow({ profile }).catch((error) => {
      recordPipelineError({ process: "decision-shadow", runId, error }).catch(() => {});
      return null;
    });
    ...
    // NICHT der blob-cap-verzerrte savedItems-Zaehler. null, wenn der relationale
    // Pfad in diesem Lauf nicht lief (z. B. v3-store aus) — dann ehrlich unbekannt.
    const newRawDocuments = persistResult && Number.isFinite(persistResult.persisted) ? persistResult.persisted : null;
