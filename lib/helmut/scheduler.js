...
const { runBriefingShadow } = require("./briefing");
...
// Situational-Briefing ("Beobachten"-Karten): wie viele Items maximal erzeugt
// werden und ab welchem politischen Gewicht ein Item auch ohne exakten
// Profil-Keyword-Treffer aufgenommen wird. Der Weight-Floor haelt die Qualitaet
// (politische Substanz statt reiner Aktualitaet), waehrend der hoehere Deckel
// verhindert, dass das Karussell auf 2 Karten kollabiert, wenn nur wenige Items
// exakt auf die Profil-Terms passen.
const situationalItemLimit = Number(process.env.HELMUT_SITUATIONAL_ITEM_LIMIT || 20);
const situationalWeightFloor = Number(process.env.HELMUT_SITUATIONAL_WEIGHT_FLOOR || 40);
...
async function runSourceCrawl(politicianId = cemInceProfile.id, options = {}) {
  ...
  try {
    ...
    await runUnderstandingShadow(savedItems).catch(() => {});
    ...
    // V3-Schatten (C7b): Template-Briefing aus Matching-Ergebnissen rendern (kein KI-Call).
    // Hinter HELMUT_V3_BRIEFING; fail-safe.
    await runBriefingShadow({ profile }).catch(() => {});
...
async function runMorningBriefing(politicianId = cemInceProfile.id, { skipAi = false } = {}) {
  ...
  // TTL bewusst UNTER der Vercel-maxDuration (300s): ein durch Function-Timeout
  // abgebrochener Lauf gibt den Lock nie via finally frei, deshalb muss der TTL
  // ihn schneller ablaufen lassen, sonst blockiert er jeden Retry für volle 5 min.
  const locked = await acquirePipelineLock(lockName, 4 * 60 * 1000);
  if (!locked) {
    console.warn(`[runMorningBriefing] Job läuft bereits für ${politicianId}, übersprungen.`);
    // Lock gehalten (paralleler oder abgebrochener Lauf) -> statt null das zuletzt
    // gespeicherte Briefing zurückgeben, damit ein manuelles "Aktualisieren" das
    // Karussell nicht leert.
    return await getLatestBriefing(politicianId);
  }
  try {
  const profile = await getActiveProfile(politicianId);
  ...
  const liveBriefing = briefingInputItems.length ? temperSingleSourceMediaDecisions(generateBriefing(profile, briefingInputItems, demoSources), profile) : null;
  ...
  const fallbackLiveBriefing = !hasDecisionValue(linkStrictLiveBriefing) && fallbackBriefingInputItems.length
    ? temperSingleSourceMediaDecisions(generateBriefing(profile, fallbackBriefingInputItems, demoSources), profile)
    : null;
  ...
  const survivedSignalIds = new Set((linkStrictLiveBriefing?.items || []).map(i => i.signalId));
  const urlFailedObservations = (liveBriefing?.items || [])
    .filter(item => item.signalId && !survivedSignalIds.has(item.signalId))
    .map(briefingItemToSituational);
  const existingSituationalIds = new Set(situationalItems.map(i => i.id));
  const extendedSituationalItems = [
    ...situationalItems,
    ...urlFailedObservations.filter(i => !existingSituationalIds.has(i.id))
  ];
  const briefing = usesLiveBriefing
    ? { ...selectedLiveBriefing, situationalBriefing: extendedSituationalItems }
    : buildEmptyBriefing(profile, {
      ...
      situationalBriefing: extendedSituationalItems
    });
  ...
  const briefingWithMemory = attachPoliticalMemory(briefingWithStatus, memoryEntries, profile);
  ...
  await savePipelineDebugReport(buildPipelineDebugReport({
    ...
    situationalItems: extendedSituationalItems,
...
function selectLageCheckSources(sources = [], limit = lageCheckSourceLimit, profile = {}) {
  const seen = new Set();
  return [...sources]
    .filter((source) => source && source.active !== false && !seen.has(source.id) && seen.add(source.id))
    .sort((a, b) => lageCheckSourceWeight(b, profile) - lageCheckSourceWeight(a, profile))
    .slice(0, Math.max(20, limit));
}

function lageCheckSourceWeight(source = {}, profile = {}) {
        ...
        : type === "association" ? 720
          : type === "media" ? 650
            : type === "local" ? 600
              : type === "social" ? 250
                : 400;
  // Eigenes Ressort/eigene Partei kommen aus dem Profil statt "BMAS"/"Linke" fuer
  // jedes Mandat als strategisch wichtige Quelle vorauszusetzen.
  const ministryTerms = uniqueTerms(profile.relevantMinistries || []).filter((term) => term.toLowerCase() !== "bundesregierung");
  const partyTerms = uniqueTerms([profile.party, profile.faction]);
  const strategicBonus = hasAnyTerm(name, ["bundesregierung", "bundestag", "ausschuss", "dgb", "tagesschau", "deutschlandfunk", "person", "news-suche", ...ministryTerms, ...partyTerms]) ? 120 : 0;
...
function mandateNewsSources(profile) {
  ...
  // Kein hartcodierter Ausschuss-Fallback: ein neues, noch nicht eingerichtetes
  // Mandat hat "" als Ausschuss (siehe neutralProfileDefaults) - "Arbeit und
  // Soziales" waere hier falsch und fuer jedes andere Mandat irrefuehrend.
  const committee = profile.committee || profile.committees?.[0] || "";
  const topicQuery = topics.slice(0, 5).join(" OR ");
  // Partei/Fraktion und Ressorts kommen aus dem Profil, nicht hartcodiert -
  // sonst wuerde jedes andere Mandat weiterhin "Die Linke"/BMAS crawlen.
  const partyTerms = uniqueTerms([profile.party, profile.faction]);
  const partyQuery = partyTerms.map((term) => (term.includes(" ") ? `"${term}"` : term)).join(" OR ");
  const ministryTerms = uniqueTerms(profile.relevantMinistries || []).filter((term) => term.toLowerCase() !== "bundesregierung");
  const ministryQuery = ministryTerms.map((term) => (term.includes(" ") ? `"${term}"` : term)).join(" OR ");
  ...
  if (hasTopics) {
    sources.push(newsSearchSource({
      ...
      query: `(Bundesregierung OR Bundeskabinett${ministryQuery ? ` OR ${ministryQuery}` : ""}) (${topicQuery}) (Gesetzentwurf OR Eckpunkte OR Reform OR Initiative OR Vorhaben OR Kabinett)`,
      priority: 94,
      queryTerms: ["Bundesregierung", "Bundeskabinett", ...ministryTerms, "Gesetzentwurf", "Eckpunkte", "Reform", "Vorhaben", ...topics]
    }));
  }
  if (partyQuery) {
    sources.push(newsSearchSource({
      ...
      query: hasTopics ? `(${partyQuery}) (${topicQuery})` : partyQuery,
      priority: 90,
      queryTerms: [...partyTerms, ...topics]
    }));
  }
  if (ministryQuery) {
    sources.push(newsSearchSource({
      id: `${profile.id}-news-ministerium-vorhaben`,
      name: "Ministerium Vorhaben Radar",
      query: hasTopics ? `(${ministryQuery}) ${topics.slice(0, 3).join(" OR ")}` : ministryQuery,
      priority: 88,
      queryTerms: [...ministryTerms, ...topics]
    }));
  }
  if (committee) {
    sources.push(newsSearchSource({
      ...
      query: hasTopics ? `"${committee}" ${topics.slice(0, 4).join(" OR ")}` : `"${committee}"`,
      priority: 84,
  ...
  if (hasTopics) {
    sources.push(newsSearchSource({
      id: `${profile.id}-news-politikfeld-medien`,
      name: "Politikfeld Medienlage",
...
function topProfileTopics(profile, limit = 5) {
  const prioritized = Object.entries(profile.topicPriorities || {})
    ...
    .map(([topic]) => topic);
  // Kein hartcodierter Themen-Fallback: ein Mandat ohne eigene Themen liefert
  // hier bewusst eine leere Liste (mandateNewsSources ueberspringt dann die
  // themenabhaengigen Quellen), statt fuenf Arbeit-und-Soziales-Begriffe fuer
  // JEDES noch nicht eingerichtete Mandat vorzugeben.
  return uniqueTerms([...prioritized, ...(profile.focusTopics || [])]).slice(0, limit);
}

function attachPoliticalMemory(briefing, memoryEntries, profile = {}) {
  ...
  const items = (briefing.items || []).map((item) => {
    ...
    const lageMovement = buildLageMovement(item, memory, profile);
    return {
      ...item,
...
function buildLageMovement(item, memory, profile = {}) {
  ...
  const government = hasGovernmentSignal(item, profile);
  const committee = hasCommitteeSignal(item, profile);
  ...
  // Kein hartcodiertes "BMAS" im Begruendungstext - fuer ein anderes Mandat waere
  // das eigene Ressort ein anderes (oder gar keins).
  const ministryLabel = uniqueTerms(profile.relevantMinistries || []).filter((term) => term.toLowerCase() !== "bundesregierung")[0] || "dein Ressort";
  ...
  if (!memory) {
    ...
    reason = government
      ? `Neu, weil ein Bundesregierung- oder ${ministryLabel}-Signal zu deinem Themenfeld vorliegt.`
      : committee
  ...
  if (government && !reasonLower.includes("bundesregierung") && !reasonLower.includes(ministryLabel.toLowerCase())) reason = `${reason} Relevant ist vor allem der mögliche Bezug zu Bundesregierung oder ${ministryLabel}.`;
...
function hasGovernmentSignal(item = {}, profile = {}) {
  const text = `${item.title || ""} ${item.summary || ""} ${item.content || ""} ${item.sourceName || ""} ${item.primarySource?.sourceName || ""}`.toLowerCase();
  // Eigene Ressorts kommen aus dem Profil statt "BMAS" fuer jedes Mandat
  // vorzugeben; die Institutionsbegriffe selbst sind mandatsunabhaengig.
  const ministryTerms = uniqueTerms(profile.relevantMinistries || []).filter((term) => term.toLowerCase() !== "bundesregierung");
  return hasAnyTerm(text, ["bundesregierung", "bundeskabinett", "bundesministerium", ...ministryTerms]);
}

function hasCommitteeSignal(item = {}, profile = {}) {
  const text = `${item.title || ""} ${item.summary || ""} ${item.whyItMatters || ""} ${item.content || ""}`.toLowerCase();
  // Bare "ausschuss" wuerde auf JEDEN der 24 Bundestagsausschuesse anschlagen;
  // hier zaehlt nur der eigene Ausschuss oder das eigene Ressort namentlich.
  const ownCommittees = [profile.committee, ...(profile.committees || [])].filter(Boolean);
  const ministryTerms = uniqueTerms(profile.relevantMinistries || []).filter((term) => term.toLowerCase() !== "bundesregierung");
  return ownCommittees.some((name) => hasTerm(text, name)) || hasAnyTerm(text, [...ministryTerms, "bundestag"]);
}

function hasCitizenImpactSignal(item = {}) {
  const text = `${item.title || ""} ${item.summary || ""} ${item.content || ""}`.toLowerCase();
  return hasAnyTerm(text, ["rente", "bürgergeld", "mindestlohn", "pflege", "armut", "beschäftigte", "arbeitnehmer", "sozial"]);
}
...
function semanticTopicKeysForText(value) {
  ...
  const rules = [
    ...
    ["buergergeld", ["bürgergeld", "buergergeld", "grundsicherung"]],
  ...
  for (const [key, terms] of rules) {
    if (hasAnyTerm(text, terms)) keys.add(key);
  }
  // "Sanktion" ist zwischen Buergergeld-Sanktionen und aussenpolitischen
  // Sanktionen (EU/Russland etc.) mehrdeutig - nur mit erkennbarem
  // Sozialleistungsbezug als Buergergeld-Thema werten.
  if (hasAnyTerm(text, ["sanktion", "sanktionen"]) && hasAnyTerm(text, ["bürgergeld", "buergergeld", "grundsicherung", "jobcenter", "arbeitslosengeld", "erwerbslos", "mitwirkungspflicht"])) {
    keys.add("buergergeld");
  }
...
function analyzeItemForProfile(item, profile = cemInceProfile) {
  ...
  const matchedRelevantTerms = profileRelevantTerms.filter((term) => hasTerm(text, term));
  const matchedMandateTerms = mandateTerms.filter((term) => hasTerm(text, term));
  const matchedTriggers = decisionTriggerTerms.filter((term) => hasTerm(text, term));
  const matchedWeakProtocol = weakProtocolTerms.filter((term) => hasTerm(text, term));
  const reasons = [];
  const rejectionReasons = [];

  if (isGenericSourcePage(item, profile)) rejectionReasons.push("Generische Quellen- oder Startseite");
...
function buildSituationalBriefingItems(items, profile = cemInceProfile) {
  ...
  const byUrl = new Map();
  items
    .filter((item) => !isGenericSourcePage(item, profile))
    .filter((item) => !isLowValuePublisher(item))
    .filter((item) => !isStaleNewsSearchItem(item))
    .filter((item) => {
      const text = `${item.title || ""} ${item.content || ""} ${item.sourceName || ""}`.toLowerCase();
      return terms.some((term) => hasTerm(text, term))
        || hasAnyTerm(text, buildMandateTerms(profile))
        || itemPoliticalWeight(item, profile) >= situationalWeightFloor;
    })
    .sort((a, b) => itemPoliticalWeight(b, profile) - itemPoliticalWeight(a, profile))
    .forEach((item) => {
  ...
  return Array.from(byUrl.values()).slice(0, situationalItemLimit).map((item) => ({
...
function briefingItemToSituational(item) {
  const src = item.primarySource || item.sources?.[0] || {};
  return {
    id: `situational-${item.signalId}`,
    title: item.title,
    contextType: "observation",
    summary: twoSentenceSummary(item.summary || item.title),
    sourceName: src.sourceName || item.sourceName,
    sourceType: src.sourceType || item.sourceType,
    url: src.url || src.itemUrl || item.url,
    sourceUrl: src.sourceUrl || item.sourceUrl,
    publishedAt: src.publishedAt || item.publishedAt,
    retrievedAt: src.retrievedAt || item.retrievedAt,
    confidence: item.confidence,
    relevanceReason: item.whyNow || item.relevanceReason || "Relevant für dein Mandat.",
    action: "Beobachten"
  };
}
...
function situationalContextType(item, profile = cemInceProfile) {
  ...
  // Nur die eigene Partei/Fraktion des Profils zaehlt - "Die Linke"/"Linksfraktion"
  // sonst hartcodiert wuerden einen Artikel ueber eine ANDERE Partei faelschlich
  // als eigene Partei-/Fraktionslage einsortieren.
  const partyTerms = [profile.party, profile.faction].filter(Boolean);
  if (hasAnyTerm(text, partyTerms) || (!partyTerms.length && text.includes("fraktion"))) return "party";
  return "mandate";
}
...
function isGenericSourcePage(item, profile = {}) {
  const title = item.title.toLowerCase();
  const content = String(item.content || "").toLowerCase();
  // Eigener Ausschuss/eigenes Ressort kommen aus dem Profil statt "Arbeit und
  // Soziales"/"BMAS" fuer jedes Mandat als Startseiten-Titel vorauszusetzen.
  // "start - fraktion die linke" entfaellt: bereits durch das generische
  // title.startsWith("start -") oben abgedeckt (gilt fuer jede Fraktion).
  const ownCommittees = [profile.committee, ...(profile.committees || [])].filter(Boolean);
  const ministryTerms = uniqueTerms(profile.relevantMinistries || []).filter((term) => term.toLowerCase() !== "bundesregierung");
  return (
    title.length < 8 ||
    ...
    ownCommittees.some((name) => title === `deutscher bundestag - ausschuss für ${name.toLowerCase()}`) ||
    title === "deutscher gewerkschaftsbund | dgb" ||
    title === "bundesregierung" ||
    ministryTerms.some((term) => title === term.toLowerCase()) ||
...
function itemPoliticalWeight(item, profile = cemInceProfile) {
  ...
  if (hasAnyTerm(text, ["gesetzentwurf", "gesetz", "eckpunkte", "reform", "initiative", "bundesregierung", "bmas"])) weight += 35;
  if (hasAnyTerm(text, mandateTerms)) weight += 30;
  if (hasAnyTerm(text, ["kritik", "fordert", "warnt", "debatte", "beschluss", "ausschuss"])) weight += 18;
...
function temperSingleSourceMediaDecisions(briefing, profile = cemInceProfile) {
  ...
  const items = briefing.items.map((item) => {
    ...
    const hasPersonMention = fullName.length > 2 && hasTerm(String(`${item.title} ${item.summary}`).toLowerCase(), fullName);
