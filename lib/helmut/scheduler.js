...
async function runGlobaleErfassung(options = {}) {
  ...
  try {
    ...
    // 6. Lazy-Understanding — schon heute global (es prueft ALLE Profile je Cluster).
    let lazyRan = 0;
    let lazyCluster = 0;
    try {
      if (savedItems.length) {
        ...
        const lazyStart = now();
        for (const cluster of clusters) {
          if (now() - lazyStart > lazyBudgetMs) break;
          lazyRan += 1;
        ...
        console.log(`[globalphase] lazy-understanding ${now() - lazyStart}ms clusters=${clusters.length} processed=${lazyRan} deferred=${clusters.length - lazyRan}`);
      }
    } catch (error) {
    ...
    //    Default 90 000 ms). Es wird NICHT erhoeht, damit die Zahl der KI-Aufrufe je Lauf
    //    exakt so gedeckelt bleibt wie heute.
    ...
    const eagerResult = eagerBudgetMs > 0
      ? await deps.eagerUnderstanding(savedItems, {
        budgetMs: eagerBudgetMs,
        vormerkDeadlineMs: startedMs + budgetMs - 5000,
        runId: laufId
      }).catch((error) => ({ skipped: true, reason: "eager-error", error: error && error.message }))
      : { skipped: true, reason: "zeitbudget", processed: 0, deferred: 0 };
    const eagerDurationMs = now() - eagerStart;
    console.log(`[globalphase] eager-understanding ${eagerDurationMs}ms ${JSON.stringify({ processed: eagerResult && eagerResult.processed, deferred: eagerResult && eagerResult.deferred, reason: eagerResult && eagerResult.reason })}`);
    ...
    return { datenstand, plan, crawlRun: globalerLauf, savedItems: savedItems.length };
  } catch (error) {
