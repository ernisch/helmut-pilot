...
async function runSourceCrawl(politicianId = cemInceProfile.id, options = {}) {
  ...
  // Laufzeit persistieren (Monitoring-Haertung): bisher stand die Dauer nur im
  // Console-Log — "Pipeline läuft gegen das 300s-Limit" war für keinen Watchdog
  // erkennbar. durationMs landet unten in saveCrawlRun und im Health-Report.
  ...
  try {
    ...
    const run = await saveCrawlRun({
      mode,
      politicianId,
      // Gesamtlaufzeit dieses Crawl-Laufs (siehe startedAtMs oben). Kein eigenes
      // bounded/timeout-Flag: das 280s-Timeout liegt ausserhalb (server.js
      // withTimeout) und erzeugt gar keinen Crawl-Lauf-Eintrag.
