...
async function runGeteilteLageErfassung({ tenantIds = [], runId = null, deadlineMs = 0 } = {}) {
  ...
  try {
    ...
    // 6. GLOBALE FALTUNG — genau EINMAL je Lauf. Rohdokumente spiegeln und den frisch
    //    abgerufenen Korpus verstehen sind mandantenneutrale Arbeit (`raw_documents` und die
    //    Wissensobjekte tragen keinen Mandantenbezug); sie liefen bisher je Mandat erneut fuer
    //    denselben Korpus. Zeitbudgetiert und fail-safe: scheitert sie, faellt der Lauf auf die
    //    mandatsweise Faltung zurueck (`globalErledigt` bleibt false), es geht nichts verloren.
    ...
    if (savedItems.length) {
      ...
      try {
        ...
        const budgetMs = Number.isFinite(restMs)
          ? Math.max(1, Math.min(Number(process.env.HELMUT_LAGE_UNDERSTAND_BUDGET_MS || 60000), Math.floor(restMs / 2)))
          : Number(process.env.HELMUT_LAGE_UNDERSTAND_BUDGET_MS || 60000);
        const verstehen = await runUnderstandingShadow(savedItems, {
          budgetMs,
          deadlineMs: Number(deadlineMs) > 0 ? Number(deadlineMs) : 0,
          vormerkBudgetMs: Number(process.env.HELMUT_LAGE_VORMERK_BUDGET_MS || LAGE_VORMERK_BUDGET_MS),
          vormerkDeadlineMs: Number(deadlineMs) > 0 ? Number(deadlineMs) - LAGE_ABSCHLUSS_RESERVE_MS : 0,
          savePendingBulk: savePendingKnowledgeObjectsBulk,
          runId
        }).catch((error) => ({ skipped: true, reason: "lage-understand-error", error: error && error.message }));
        ...
        await recordProcessRun({
          ...
          reason: verstehen && verstehen.reason, status: bilanz.status,
          fehlerklasse: bilanz.fehlerklasse, telemetrie: verstehen && verstehen.telemetrie
        }).catch(() => {});
        globalErledigt = true;
