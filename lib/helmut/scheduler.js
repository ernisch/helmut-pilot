const { crawlAllSources, deduplicateRawItems, limitRawCandidates, maxCrawlCandidates } = require("./crawler");
const lageErfassung = require("./lage-erfassung");
const { publikationsdatum } = require("./quellen-zeitvertrag");
const cronGlobalphase = require("./cron-globalphase");
// Kanonische Klassifizierung real/synthetisch (kein Aussenabruf fuer Testprofile).
...
const { clusterRawDocuments, deriveVorgangId, buildOutcomeTelemetry } = require("./understanding");
const { runUnderstandingShadow } = require("./artikelkontext-lauf");
const { laufBilanz } = require("./lauf-bilanz");
...
// Deckel des Vormerk-Loops im Lage-Pfad (Befund 05.09.2026, s. `foldLageItemsIntoV3`).
// 20 s ist ein Vielfaches dessen, was der BULK-Pfad fuer einen vollen Rueckstand braucht,
// und zugleich klein gegen die Mandatsscheibe. Er wirkt fail-closed auch dann, wenn der
// Aufrufer gar keine absolute Deadline mitgibt (Handlauf, Test) — der Loop ist danach in
// KEINER Konfiguration mehr unbegrenzt.
const LAGE_VORMERK_BUDGET_MS = 20000;
// Reserve zwischen Modell-Deadline und Vormerk-Deadline, wortgleich zum Crawl-Pfad.
const LAGE_ABSCHLUSS_RESERVE_MS = 5000;
// Reserve zwischen Modellende und Mandatsfrist: Vormerkung (Bulk) + Matching + Entscheidung.
// Production gemessen: Matching ~1,05 s, Entscheidung ~1,05 s; der Bulk-Vormerk-Pfad braucht
// wenige Chunk-Anfragen. 8 s ist ein Vielfaches davon und bleibt klein gegen die Scheibe.
const LAGE_NACHLAUF_RESERVE_MS = 8000;
...
function dipDocToRawItem(doc = {}, { primary = false } = {}) {
  const base = {
    ...
    url: doc.url,
    publishedAt: publikationsdatum(doc.date)?.original || null,
  ...
  const dipQuellfelder = require("./dip-quellfelder").lese({ ...base, dipQuellfelder: doc.dipQuellfelder });
  if (dipQuellfelder) base.dipQuellfelder = dipQuellfelder;
...
async function runSourceCrawl(politicianId, options = {}) {
  ...
  try {
    ...
    // RESTZEITWACHE (§29): absolute Deadline auch fuer die MODELLAUFRUFE selbst — das
    // Minimum aus dem eigenen Lauffenster und (falls der Cron-Handler sie mitgibt)
    // dessen Funktionsfenster. Ein spaet gestartetes Mandat kann so nicht mehr in das
    // aeussere Funktionsende hineinverstehen.
    const eagerDeadlineMs = Math.min(
      runStartedMs + runBudgetMs,
      Number(options.deadlineMs) > 0 ? Number(options.deadlineMs) : Infinity
    );
    const eagerResult = await runUnderstandingShadow(savedItems, {
      budgetMs: eagerBudgetMs,
      deadlineMs: eagerDeadlineMs,
      vormerkDeadlineMs,
...
async function foldLageItemsIntoV3(savedItems, profile, runId = null, optionen = {}) {
  // GLOBALE SCHRITTE ODER MANDATSSCHRITTE? (Befund 05.09.2026)
  // Die Schritte 1 und 2 — Rohdokumente spiegeln und den Korpus VERSTEHEN — tragen keinen
  // Mandantenbezug: `raw_documents` und die Wissensobjekte sind global. Sie liefen trotzdem je
  // Mandat, also n-mal fuer denselben Korpus. Genau diese Trennung schreibt `cron-globalphase.js`
  // (OP-25 K1) fuer den Crawl-Cron bereits vor; der Lage-Check hatte sie nie.
  // Hat die geteilte Erfassung dieses Laufs sie schon erledigt (`globalErledigt`), werden sie
  // hier UEBERSPRUNGEN und es bleiben nur die echten Mandatsschritte: Matching und Entscheidung.
  // Ohne geteilte Erfassung laeuft alles unveraendert wie bisher.
  const globalErledigt = optionen.globalErledigt === true;
  if (globalErledigt) {
    const projektionsStart = Date.now();
    ...
    const nurDecision = await runDecisionShadow({ profile }).catch((error) => ({ skipped: true, reason: "lage-decision-error", error: error && error.message }));
    if (nurDecision && nurDecision.reason === "lage-decision-error") {
      await recordPipelineError({ process: "decision-lage", runId, errorType: nurDecision.error || "decision-error" }).catch(() => {});
    }
    console.log(`[runLageCheck] mandatsphase ${Date.now() - projektionsStart}ms (global bereits im Lauf erledigt) ${profile.id}`);
    return { skippedGlobal: true, decision: nurDecision };
  }
  ...
  //    RESTZEITWACHE (§29): zusätzlich die ABSOLUTE Deadline des Cron-Fensters — das
  //    relative 60-s-Budget allein hat die verwaisten `modell-laeuft`-Vorgänge vom
  //    19./20.08. nicht verhindert, weil der Loop-Start selbst schon beliebig spät im
  //    Funktionsfenster liegen konnte.
  const understandStart = Date.now();
  // VORMERK-RIEGEL (Befund 05.09.2026 — der Riegel EXISTIERT seit K4, war hier aber nie
  // verdrahtet). Gemessen am Lauf `lage-20260905100016-5bc8j`: 608 Cluster gebildet, 4
  // verstanden, 604 zurueckgestellt; `runUnderstandingShadow` brauchte 205 062 ms von
  // 241 749 ms Gesamtlaufzeit (85 %), und der groesste Einzelposten darin war NICHT der
  // Modellaufruf, sondern der SERIELLE Vormerk-Loop fuer diese 604 Cluster (zwei
  // Supabase-Rundlaeufe je Cluster, ~1,7 Cluster/s → ~120 s). `understanding.js:2258-2266`
  // beziffert genau diese Senke aus einem frueheren Vorfall (504 im Crawl-Pfad) — dort
  // wurde sie begrenzt, hier nicht: der Lage-Pfad reichte weder `vormerkBudgetMs`/
  // `vormerkDeadlineMs` (0 = KEIN Limit) noch `savePendingBulk` durch.
  //
  // Beides wird jetzt uebergeben, WORTGLEICH zum Crawl-Pfad (`runSourceCrawl`, s. o.):
  //   - `savePendingBulk` ersetzt 2×n serielle Rundlaeufe durch wenige Chunk-Anfragen,
  //   - `vormerkBudgetMs`/`vormerkDeadlineMs` deckeln den Rest FAIL-CLOSED.
  // Was danach nicht mehr vorgemerkt wurde, geht NICHT still verloren: es wird als
  // `nichtVorgemerkt` gezaehlt, in die Telemetrie gehoben und bleibt als unverarbeitetes
  // Rohdokument liegen, das der Rueckstandslauf (`/api/cron/understanding-rueckstand`)
  // erneut aufgreift. Kein Element wird dauerhaft ausgelassen.
  const vormerkBudgetMs = Number(process.env.HELMUT_LAGE_VORMERK_BUDGET_MS || LAGE_VORMERK_BUDGET_MS);
  const vormerkDeadlineMs = Number(optionen.vormerkDeadlineMs) > 0
    ? Number(optionen.vormerkDeadlineMs)
    : (Number(optionen.deadlineMs) > 0 ? Number(optionen.deadlineMs) - LAGE_ABSCHLUSS_RESERVE_MS : 0);
  // MODELLBUDGET FOLGT DER MANDATSSCHEIBE (Befund 05.09.2026). Bisher stand hier ein FESTES
  // Regelbudget von 60 s. Sobald die Fairnessschleife einem Mandat eine kleinere Scheibe gibt
  // (bei vielen Mandaten die Untergrenze), waere der Modellaufruf laenger als das Mandat selbst
  // — das Mandat liefe unweigerlich in seinen Timeout und wuerde als FEHLGESCHLAGEN gebucht,
  // obwohl der Lage-Check selbst laengst gespeichert ist. Also dieselbe Rechnung wie im
  // Crawl-Pfad (`runSourceCrawl`: `min(Regelbudget, Restzeit − Reserve)`):
  //   Modellbudget = min(Regelbudget, Restzeit bis zur Mandatsfrist − Nachlaufreserve)
  // Die Nachlaufreserve deckt, was NACH dem Verstehen noch kommt: Vormerkung (Bulk),
  // Matching und Entscheidung (Production gemessen ~1,05 s + ~1,05 s).
  // NIE 0 — `runUnderstandingShadow` deutet `budgetMs = 0` als „KEIN Limit".
  const lageRegelBudgetMs = Number(process.env.HELMUT_LAGE_UNDERSTAND_BUDGET_MS || 60000);
  const verbleibendMs = Number(optionen.deadlineMs) > 0
    ? Math.max(0, Number(optionen.deadlineMs) - Date.now())
    : Infinity;
  const modellBudgetMs = Number.isFinite(verbleibendMs)
    ? Math.max(1, Math.min(lageRegelBudgetMs, verbleibendMs - LAGE_NACHLAUF_RESERVE_MS))
    : lageRegelBudgetMs;
  const understanding = await runUnderstandingShadow(savedItems, {
    budgetMs: modellBudgetMs,
    deadlineMs: Number(optionen.deadlineMs) > 0 ? Number(optionen.deadlineMs) : 0,
    vormerkBudgetMs,
    vormerkDeadlineMs,
  ...
  // KANONISCHE LAUFBILANZ (Befund 2026-08-30, derselbe Meldefehler wie im
  // Understanding-Cron): Zaehler und Gesamtstatus stammen aus EINER Ableitung ueber die
  // Fachtelemetrie. Vorher stand hier `skipped ? "blocked" : "success"` — ein Lauf mit
  // Fehlerfall konnte gar nicht anders als `success` erscheinen.
  const lageBilanz = laufBilanz(understanding);
  const lageTelemetrie = await recordProcessRun({
    ...
    gespeichert: lageBilanz.gespeichert,
    uebersprungen: lageBilanz.uebersprungen,
    fehlgeschlagen: lageBilanz.fehlgeschlagen,
    deferred: lageBilanz.vertagt,
    ...
    status: lageBilanz.status,
    fehlerklasse: lageBilanz.fehlerklasse,
...
// GETEILTE LAGE-ERFASSUNG — hoechstens EINMAL je Cron-Lauf statt einmal je Mandat.
// =============================================================================================
// Vertrag und Begruendung: `lib/helmut/lage-erfassung.js`. Kurz:
// Der Quellenabruf des Lage-Checks ist fachlich GLOBAL (der Rohkorpus traegt keinen
// Mandantenbezug, `saveRawItems` → `getRawItemsSince` liest global), lief aber je Mandat.
// Gemessen 03.09. kostete allein die Vorphase je Mandat 21–33 s — bei 25 Mandaten also
// 525–825 s gegen ein Budget von 240 s. Hier wird sie EINMAL bezahlt.
//
// Was hier passiert (und NUR das):
//   1. je aktivem Mandat den UNVERAENDERTEN Quellenplan bilden
//      (`getSourcesForProfile` → `selectLageCheckSources`), mit laufweitem Plan-Zwischen-
//      speicher, damit der profilunabhaengige Planbau nicht n-mal laeuft,
//   2. die Plaene VEREINIGEN (`cronGlobalphase.planGlobaleQuellen` — dieselbe geprueste
//      Funktion, die die Crawl-Globalphase benutzt),
//   3. die Vereinigung EINMAL abrufen,
//   4. je Mandat die Sicht rekonstruieren, die sein Einzelabruf ergeben haette,
//   5. die Vereinigung der mandatsweisen Elemente EINMAL speichern.
//
// Es entsteht kein Abrufweg, den nicht mindestens ein aktives Mandat heute schon erhielte,
// und es verschwindet keiner. Scheitert die Erfassung, wird sie als FEHLGESCHLAGEN versiegelt
// und jedes Mandat faellt auf seinen unveraenderten Einzelabruf zurueck — ein leerer
// Lage-Check aus einer kaputten Erfassung waere ein falsches Gruen (CLAUDE.md §4.4).
// `deadlineMs` ist — wie ueberall in diesem Modul — ein ABSOLUTER Zeitpunkt (Date.now()-Basis),
// keine Dauer. Er kommt aus dem Gesamtbudget des Cronlaufs; die Erfassung TEILT dieses Budget
// mit der Mandatsphase und erhoeht es nicht.
async function runGeteilteLageErfassung({ tenantIds = [], runId = null, deadlineMs = 0 } = {}) {
  const startedMs = Date.now();
  const ids = [...new Set((Array.isArray(tenantIds) ? tenantIds : []).map((id) => String(id || "").trim()).filter(Boolean))];
  const basis = {
    version: lageErfassung.VERSION,
    runId: runId || null,
    startedAtMs: startedMs,
    mandate: ids.length,
    status: lageErfassung.ERFASSUNG_OFFEN,
    sichten: {},
    neueElemente: {},
    quellen: 0,
    gemeinsam: 0,
    fehlerhafteProfile: [],
    savedItems: 0,
    globalErledigt: false,
    globalTelemetrie: null,
    dauerMs: 0,
    grund: null
  };
  if (!ids.length) {
    return { ...basis, status: lageErfassung.ERFASSUNG_ABGESCHLOSSEN, grund: "keine-mandate", dauerMs: 0 };
  }

  const pruefeVorlaufFrist = () => {
    if (deadlineMs > 0 && Date.now() >= deadlineMs) throw new Error("lage-vorlauf-zeitbudget-erschoepft");
  };
  try {
    pruefeVorlaufFrist();
    // Ein Bestand fuer diesen Lauf statt 500 serieller Einzelabfragen. Dieselbe
    // Profilwahrheit und dieselben neutralen Defaults, ohne prozessweiten Cache.
    // Ein gescheiterter Abruf beendet den Vorlauf sichtbar, bevor Quellen starten.
    const alleProfile = await listFullProfiles();
    if (!Array.isArray(alleProfile)) throw new Error("lage-profile-nicht-ladbar");
    const profileById = new Map(alleProfile.filter((p) => p && p.id).map((p) => [String(p.id), p]));
    // 1. Quellenplaene. LAUFWEITER Zwischenspeicher: der relationale Planbau ist
    //    profilunabhaengig und lief bisher je Mandat erneut (Warnkommentar bei
    //    `loadRelationalSharedSources`: „Bei 5 Mandaten sind das 15 Lesevorgänge, bei 500
    //    sind es 1.500"). Kein Modulzustand, keine TTL — der Cache lebt nur in diesem Aufruf.
    const planCache = {};
    const profilQuellen = [];
    for (const tenantId of ids) {
      pruefeVorlaufFrist();
      try {
        const stored = profileById.get(tenantId);
        if (!stored) throw new Error("lage-profil-nicht-im-bestand");
        const profile = mergeProfileDefaults(stored);
        const alle = await getSourcesForProfile(profile, { planCache });
        profilQuellen.push({
          politicianId: tenantId,
          quellen: selectLageCheckSources(alle, lageCheckSourceLimit, profile)
        });
      } catch (error) {
        // Ein unlesbares Profil nimmt NUR sich selbst aus der Erfassung; es faellt danach
        // auf den Einzelabruf zurueck und wird dort erneut (und sichtbar) scheitern.
        profilQuellen.push({ politicianId: tenantId, fehler: String((error && error.message) || "profil-nicht-ladbar") });
      }
    }
    pruefeVorlaufFrist();

    // 2. Vereinigung — dieselbe Funktion wie in der Crawl-Globalphase (OP-25 K1).
    const plan = cronGlobalphase.planGlobaleQuellen({ profilQuellen });
    const zuordnung = lageErfassung.quellenJeMandat(plan.herkunft);
    if (!plan.quellen.length) {
      return {
        ...basis,
        status: plan.fehlerhafteProfile.length ? lageErfassung.ERFASSUNG_TEILWEISE : lageErfassung.ERFASSUNG_ABGESCHLOSSEN,
        grund: "keine-quellen",
        fehlerhafteProfile: plan.fehlerhafteProfile,
        dauerMs: Date.now() - startedMs
      };
    }

    // 3. EIN Abruf. Haertung und geteiltes Wegegedaechtnis unveraendert wie im Einzelpfad.
    const hardening = googleHardeningConfig();
    const ledger = hardening.enabled && hardening.sharedPathDedup
      ? sharedFetchLedger(hardening.sharedPathWindowMs)
      : null;
    const crawl = await crawlAllSources(plan.quellen, {
      googleGate: hardening.enabled ? createGoogleNewsGate(hardening, { startOpen: googleBreakerMemoryActive(hardening) }) : null,
      hardeningConfig: hardening,
      sharedLedger: ledger
    });
    rememberGoogleBreakerState(crawl.googleGate);

    // 4. Mandatssichten. Dedup und Kandidaten-Cap kommen aus dem Crawler — eine Wahrheit.
    const sichten = {};
    for (const tenantId of ids) {
      const quellenIds = zuordnung.get(tenantId);
      if (!quellenIds) continue;                 // Profil war nicht planbar (siehe oben)
      sichten[tenantId] = lageErfassung.mandatsSicht({
        ergebnisse: crawl.results,
        quellen: plan.quellen,
        quellenIds,
        maxKandidaten: maxCrawlCandidates,
        dedupe: deduplicateRawItems,
        cap: limitRawCandidates
      });
    }

    // 5. EIN Speichervorgang. Gespeichert wird die VEREINIGUNG der mandatsweisen Elemente —
    //    exakt die Menge, die die n heutigen Einzelabrufe zusammen speichern wuerden.
    const auftrag = lageErfassung.speicherAuftrag(Object.values(sichten));
    const savedItems = await saveRawItems(auftrag);
    const neueElemente = {};
    for (const [tenantId, sicht] of Object.entries(sichten)) {
      neueElemente[tenantId] = lageErfassung.neueElementeJeMandat(savedItems, sicht.results.map((e) => e.sourceId));
    }

    // 6. Rohdokumente einmal spiegeln; Verstehen je SICHTBARKEITSKONTEXT (OP-25 K2.1).
    // Ein globaler Korpus erlaubt keine globale lose Clusterbildung: sie verschmilzt
    // sonst fremde Personen-/Parteiquellen (belegter K1-Befund, Fallfamilie F9).
    // Geteilte Quellen bleiben ein gemeinsamer Kontext, eigene Quellen bleiben getrennt.
    // Alle Kontexte teilen EIN Modellbudget und EINE Vormerkfrist fuer diesen Lauf.
    let globalErledigt = false;
    let globalTelemetrie = null;
    if (savedItems.length) {
      const globalStart = Date.now();
      try {
        // Die Hash-Entdoppelung kann mehrere Abrufwege auf ein Item reduzieren.
        // Deshalb stammt dessen Sichtbarkeit aus ALLEN Mandatssichten, nicht nur
        // aus der zufaellig erhaltenen sourceId des gespeicherten Exemplars.
        const dokumentHerkunft = new Map();
        for (const [tenantId, sicht] of Object.entries(sichten)) {
          for (const item of sicht.rawItems) {
            for (const key of vorgangskontext.dokumentSchluessel(item)) {
              if (!dokumentHerkunft.has(key)) dokumentHerkunft.set(key, new Set());
              dokumentHerkunft.get(key).add(tenantId);
            }
          }
        }
        const herkuenfte = {
          herkunftJeQuelle: plan.herkunft,
          herkunftJeDokument: Object.fromEntries([...dokumentHerkunft].map(([key, mandate]) => [key, [...mandate]]))
        };
        const kontextPlan = vorgangskontext.planKontexte({ dokumente: savedItems, ...herkuenfte, reihenfolge: ids });
        ...
        if (!partition.ok || !grenzen.ok) throw new Error("lage-kontextvertrag-verletzt");
        await persistRawDocumentsShadow(savedItems, { runId }).catch(() => {});
        const restMs = Number(deadlineMs) > 0 ? Math.max(0, Number(deadlineMs) - Date.now()) : Infinity;
        // Der globalen Phase steht hoechstens ihr Regelbudget zu und nie mehr als die HAELFTE
        // der Restzeit: die Mandatsphase darf nicht ausgehungert werden (dieselbe Regel wie
        // `cron-globalphase.MIN_GLOBAL_ANTEIL`).
        const regelBudgetRoh = Number(process.env.HELMUT_LAGE_UNDERSTAND_BUDGET_MS || 60000);
        const regelBudgetMs = Number.isFinite(regelBudgetRoh) && regelBudgetRoh > 0 ? regelBudgetRoh : 60000;
        const budgetMs = Number.isFinite(restMs)
          ? Math.max(1, Math.min(regelBudgetMs, Math.floor(restMs / 2)))
          : regelBudgetMs;
        const modellFristMs = Date.now() + budgetMs;
        const vormerkBudgetRoh = Number(process.env.HELMUT_LAGE_VORMERK_BUDGET_MS || LAGE_VORMERK_BUDGET_MS);
        const vormerkBudgetMs = Number.isFinite(vormerkBudgetRoh) && vormerkBudgetRoh > 0 ? vormerkBudgetRoh : LAGE_VORMERK_BUDGET_MS;
        const vormerkFristMs = Math.min(
          modellFristMs + vormerkBudgetMs,
          Number(deadlineMs) > 0 ? Number(deadlineMs) - LAGE_ABSCHLUSS_RESERVE_MS : Infinity
        );
        const kontextErgebnisse = [];
        for (const teil of kontextPlan.kontexte) {
          // Vor dem Aufruf pruefen: Understanding macht bereits vor seinen eigenen
          // Zeitgates Lock-IO, Clusterbildung und Telemetrie. Nach der gemeinsamen
          // Vormerkfrist darf auch diese Vorarbeit nicht neu begonnen werden.
          if (Date.now() >= vormerkFristMs) {
            kontextErgebnisse.push({ skipped: true, reason: "zeitbudget", kontextBegonnen: false,
              dokumente: teil.dokumente.length, processed: 0 });
            continue;
          }
          // Nach der Modellfrist darf der bestehende Restzeit-Riegel keinen weiteren
          // Modellaufruf starten. Der Kontext erreicht trotzdem den Vormerkpfad; auch
          // dieser hat die gemeinsame absolute Frist, kein frisches Budget je Kontext.
          const ergebnis = await runUnderstandingShadow(teil.dokumente, {
            budgetMs: Math.max(1, modellFristMs - Date.now()),
            deadlineMs: modellFristMs,
            vormerkBudgetMs: vormerkBudgetMs,
            vormerkDeadlineMs: vormerkFristMs,
            savePendingBulk: savePendingKnowledgeObjectsBulk,
            runId
          }).catch((error) => ({ skipped: true, reason: "lage-understand-error", error: error && error.message }));
          kontextErgebnisse.push({ ...ergebnis, kontextBegonnen: true, dokumente: teil.dokumente.length });
        }
        const verstehen = kontextErgebnisse.length === 1
          ? kontextErgebnisse[0]
          : fasseEagerErgebnisseZusammen(kontextErgebnisse);
        // Die bestehende Zusammenfassung traegt die Vormerkzaehler, aber keine
        // Fachtelemetrie. Nur vollstaendig abrechenbare Teilmengen duerfen eine
        // globale Bilanz bilden; ein fehlender Teil bleibt sichtbar unabrechenbar.
        if (kontextErgebnisse.length > 1 && kontextErgebnisse.every((e) => laufBilanz(e).zaehlbar && laufBilanz(e).stimmig)) {
          verstehen.telemetrie = buildOutcomeTelemetry({
            clusterCount: kontextErgebnisse.reduce((n, e) => n + e.telemetrie.cluster, 0),
            results: kontextErgebnisse.flatMap((e) => e.results || []),
            deferred: verstehen.deferred, vorgemerkt: verstehen.vorgemerkt,
            priorityOn: kontextErgebnisse.some((e) => e.telemetrie.priorisierungAktiv)
          });
          verstehen.telemetrie.dokumente = kontextErgebnisse.reduce((n, e) => n + e.telemetrie.dokumente, 0);
          verstehen.telemetrie.dokumenteOhneEndzustand = kontextErgebnisse.reduce((n, e) => n + e.telemetrie.dokumenteOhneEndzustand, 0);
        }
        const bilanz = laufBilanz(verstehen);
        const nichtBegonnen = kontextErgebnisse.filter((e) => e.kontextBegonnen === false);
        // Nur bereits gebildete Cluster sind zaehlbar. Fuer unbegonnene Kontexte
        // kennen wir ausschliesslich die Dokumentmenge, niemals ihre Clusterzahl.
        const nichtVorgemerkteCluster = kontextErgebnisse.reduce((n, e) => n + (Number(e.nichtVorgemerkt) || 0), 0);
        const vormerkFehlgeschlagen = kontextErgebnisse.reduce((n, e) => n + (Number(e.vormerkFehlgeschlagen) || 0), 0);
        const offeneDokumente = kontextErgebnisse.reduce((n, e) => {
          const offen = e.telemetrie && e.telemetrie.dokumenteOhneEndzustand;
          // Ohne Fachtelemetrie liegt fuer keines der Eingangsdokumente ein
          // Abschlussnachweis dieses Kontexts vor. Vorgemerkt ist ebenfalls offen.
          return n + (Number.isFinite(offen) ? offen : e.dokumente);
        }, 0);
        const unvollstaendig = nichtBegonnen.length > 0 || nichtVorgemerkteCluster > 0 || vormerkFehlgeschlagen > 0;
        globalTelemetrie = {
          dauerMs: Date.now() - globalStart,
          verarbeitet: verstehen && verstehen.processed,
          vertagt: bilanz.vertagt,
          status: unvollstaendig ? "partial" : bilanz.status,
          kontexte: kontextPlan.gesamt,
          nichtBegonneneKontexte: nichtBegonnen.length,
          nichtBegonneneDokumente: nichtBegonnen.reduce((n, e) => n + e.dokumente, 0),
          nichtVorgemerkteCluster, vormerkFehlgeschlagen, offeneDokumente,
          kontextBilanzen: kontextErgebnisse.map((e) => ({ ...laufBilanz(e), grund: e && e.reason || null,
            begonnen: e.kontextBegonnen, dokumente: e.dokumente,
            nichtVorgemerkt: e.nichtVorgemerkt ?? null, vormerkFehlgeschlagen: e.vormerkFehlgeschlagen ?? null }))
        };
        await recordProcessRun({
          process: "understanding-lage", runId, mode: "lage-check-global", location: executionLocation(),
          startedAt: new Date(globalStart).toISOString(), finishedAt: new Date().toISOString(),
          durationMs: globalTelemetrie.dauerMs,
          processed: verstehen && verstehen.processed,
          gespeichert: bilanz.gespeichert, uebersprungen: bilanz.uebersprungen,
          fehlgeschlagen: bilanz.fehlgeschlagen, deferred: bilanz.vertagt,
          reason: verstehen && verstehen.reason, status: globalTelemetrie.status,
          fehlerklasse: bilanz.fehlerklasse,
          telemetrie: { ...(verstehen && verstehen.telemetrie || {}), kontexte: kontextPlan.gesamt,
            nichtBegonneneKontexte: globalTelemetrie.nichtBegonneneKontexte,
            nichtBegonneneDokumente: globalTelemetrie.nichtBegonneneDokumente,
            nichtVorgemerkteCluster, vormerkFehlgeschlagen, offeneDokumente,
            kontextBilanzen: globalTelemetrie.kontextBilanzen }
        }).catch(() => {});
        // Beendet ist die globale PHASE, nicht ihr Rueckstand. Die offenen Mengen
        // oben bleiben offen; je Mandat wird diese Arbeit nicht nochmals begonnen.
        globalErledigt = true;
        console.log(`[lage-erfassung/global] ${globalTelemetrie.dauerMs}ms verarbeitet=${globalTelemetrie.verarbeitet}`
          + ` vertagt=${globalTelemetrie.vertagt} status=${globalTelemetrie.status} lauf=${runId || "-"}`);
      } catch (error) {
        // KEIN falsches Gruen: bleibt `globalErledigt` false, faltet jedes Mandat wie bisher.
        console.error("[lage-erfassung/global] gescheitert — die Mandate falten wie bisher einzeln:", error && error.message);
      }
    }

    const teilweise = plan.fehlerhafteProfile.length > 0 || Object.keys(sichten).length < ids.length
      || Boolean(globalTelemetrie && globalTelemetrie.status !== "success");
    const erfassung = {
      ...basis,
      status: teilweise ? lageErfassung.ERFASSUNG_TEILWEISE : lageErfassung.ERFASSUNG_ABGESCHLOSSEN,
      sichten,
      neueElemente,
      quellen: plan.gesamt,
      gemeinsam: plan.gemeinsam,
      fehlerhafteProfile: plan.fehlerhafteProfile,
    ...
    console.log(`[lage-erfassung] ${erfassung.dauerMs}ms mandate=${ids.length} quellen=${plan.gesamt}`
      + ` gemeinsam=${plan.gemeinsam} abgerufen=${crawl.checkedSources} erfolgreich=${crawl.successfulSources}`
      + ` fehlgeschlagen=${crawl.failedSources} gespeichert=${savedItems.length} status=${erfassung.status}`
      + ` lauf=${runId || "-"}`);
    return erfassung;
  } catch (error) {
    // FAIL-SAFE, nicht fail-silent: die Erfassung wird als gescheitert VERSIEGELT und der
    // Grund wandert in die Logzeile. Jedes Mandat laeuft danach unveraendert einzeln.
    console.error("[lage-erfassung] GESCHEITERT — jedes Mandat faellt auf den Einzelabruf zurueck:", error && error.message);
    return {
      ...basis,
      status: lageErfassung.ERFASSUNG_FEHLGESCHLAGEN,
      grund: String((error && error.message) || "erfassung-fehlgeschlagen").slice(0, 200),
      dauerMs: Date.now() - startedMs
    };
  }
}

async function runLageCheck(politicianId, optionen = {}) {
  politicianId = requireTenantId(politicianId, "runLageCheck");
  // RESTZEITWACHE (§29): absolute Deadline des Aufrufers (Cron-Handler). Ohne sie
  // (manuelle Route, Tests) bleibt das Verhalten unverändert.
  const lageDeadlineMs = Number(optionen.deadlineMs) > 0 ? Number(optionen.deadlineMs) : 0;
  ...
  // GETEILTE ERFASSUNG (Befund 05.09.2026): liegt fuer dieses Mandat eine brauchbare
  // Laufsicht vor, entfaellt der EIGENE Abruf vollstaendig — die Sicht enthaelt exakt
  // dieselben Quellen, Elemente und Zaehlwerte, die ein Einzelabruf ergeben haette
  // (Vertrag und Begruendung: lib/helmut/lage-erfassung.js). Ohne brauchbare Sicht bleibt
  // der Pfad unten WORTGLEICH der bisherige — Handlauf, Einzelroute und alle Tests
  // laufen unveraendert.
  const geteilt = lageErfassung.sichtBrauchbar(optionen.geteilteErfassung, profile.id);

  let crawl;
  let savedItems;
  let selectedSources;
  if (geteilt.brauchbar) {
    // KEIN eigener Abruf und KEIN eigener Speichervorgang: beides ist im Lauf bereits
    // genau einmal geschehen. `sicht` traegt dieselben Felder wie `crawlAllSources` —
    // dieselben Quellen dieses Mandats, dieselbe Dedup-/Cap-Logik, dieselben Zaehlwerte.
    crawl = optionen.geteilteErfassung.sichten[profile.id];
    savedItems = (optionen.geteilteErfassung.neueElemente || {})[profile.id] || [];
    selectedSources = crawl.quellen || [];
  } else {
    const allSources = await getSourcesForProfile(profile);
    selectedSources = selectLageCheckSources(allSources, lageCheckSourceLimit, profile);
    ...
    const lageSharedLedger = lageHardening.enabled && lageHardening.sharedPathDedup
      ...
      : null;
    crawl = await crawlAllSources(selectedSources, {
    ...
    rememberGoogleBreakerState(crawl.googleGate);
    savedItems = await saveRawItems(crawl.rawItems);
  }
  ...
  const v3Refresh = await foldLageItemsIntoV3(savedItems, profile, runId, {
    deadlineMs: lageDeadlineMs,
    // Die globalen Schritte (Rohdokumente + Verstehen) hat die geteilte Erfassung dieses Laufs
    // bereits EINMAL erledigt — hier bleiben nur Matching und Entscheidung.
    globalErledigt: geteilt.brauchbar && optionen.geteilteErfassung.globalErledigt === true
  });
...
// ── LAUFWEITER PLAN-ZWISCHENSPEICHER (Sprint 02.09.) ─────────────────────────
// BELEGTER BEFUND (adversarialer Review): der Aufbau des relationalen Crawlplans
// ist PROFILUNABHÄNGIG — er liest die Abrufwege, den Altkatalog und ALLE Profile
// und filtert erst danach je Profil (`planQuellenFuerProfil`). Trotzdem lief er
// je Profil erneut. Bei 5 Mandaten sind das 15 Lesevorgänge, bei 500 sind es
// 1.500 — in einer Planungsphase, die zusätzlich ein hartes Zeitbudget hat.
// Der Planungslauf hätte an derselben Stelle abgebrochen, an der die
// mandatsgebundene Arbeit steht.
//
// Der Zwischenspeicher ist AUSDRÜCKLICH LAUFWEIT und wird vom Aufrufer
// mitgegeben (`opts.planCache`) — kein Modulzustand, keine Uhr, keine TTL, keine
// stille Wiederverwendung über Läufe hinweg. Ohne Cache-Objekt verhält sich die
// Funktion byte-identisch zu vorher.
async function loadRelationalSharedSources(profile, opts = {}) {
  const cache = opts.planCache && typeof opts.planCache === "object" ? opts.planCache : null;
  if (cache && cache.plan !== undefined) {
    return cache.plan === null ? null : planQuellenFuerProfil(cache.plan, profile);
  }
  const rows = await storageListSourceArchitectureRows();
  if (!rows || !Array.isArray(rows.retrievalPaths) || rows.retrievalPaths.length === 0) {
    if (cache) cache.plan = null;
    return null;
  }
  ...
  if (!plan.aktiv.length) {
    if (cache) cache.plan = null;
    return null; // leerer Plan -> Fallback alter Katalog
  }
  if (cache) cache.plan = plan;
...
// ── KEINE EIGENEN AUSSENABRUFE FUER SYNTHETISCHE PROFILE (Sprint 02.09.) ──────
// BELEGTER BEFUND (adversarialer Review): `personNewsSource` und
// `mandateNewsSources` bauen je Mandat Google-News-Abrufe mit dem Namen bzw. dem
// Slug des Profils. Bei 495 synthetischen Kohortenprofilen entstuenden daraus je
// Zyklus rund 1.000 zusaetzliche Abrufe an news.google.com — nach Namen wie
// „Testmandat A-001", die naturgemaess nichts finden koennen.
//
// Drei Gruende, warum das ausgeschlossen wird:
//   1. Es sind AUSSENAUFRUFE. Der Kommunikationsriegel deckt sie nicht ab (er
//      sperrt Nachrichten an Empfaenger, nicht den Crawler), und der Crawler geht
//      ueber `client.get` — nicht ueber `fetch`.
//   2. Es verschaerft das belegte Google-Klumpenrisiko (OP-15, 146 von 163 Wegen)
//      um zwei Groessenordnungen und riskiert eine Drosselung, die die FUENF
//      REALEN Mandate mittrifft.
//   3. Es fuellt das Verstehensfenster (die 500 juengsten Rohdokumente) mit
//      synthetischen Treffern und verdraengt dort reale Vorgaenge.
//
// Die geteilten Katalogquellen bleiben unveraendert — die Kohorte laeuft also
// weiterhin durch Projektion, Briefing und Verstehen. Was entfaellt, sind
// ausschliesslich ihre EIGENEN Personensuchen.
//
// STRUKTURELL WIRKUNGSLOS in der heutigen Production (0 synthetische Zeilen).
// Ausdruecklich wieder einschaltbar: `HELMUT_TESTKOHORTE_QUELLEN=aktiv`.
function profilQuellenErlaubt(profile, env = process.env) {
  const id = profile && profile.id;
  if (!mandatsklasse.istSynthetischeKennung(id)) return true;
  let wert = "";
  try { wert = String((env && env.HELMUT_TESTKOHORTE_QUELLEN) || "").trim().toLowerCase(); } catch (_) { wert = ""; }
  return wert === "aktiv";
}

async function getSourcesForProfile(profile, opts = {}) {
  // CUTOVER-Pfad (nur Modus 'on'; Default off = unverändert alter Katalog).
  if (sourceMode() === "on") {
    try {
      const planSources = await loadRelationalSharedSources(profile, opts);
      if (planSources) {
        const completeness = profileCompleteness(profile);
        const profileSources = (completeness.empty || !profilQuellenErlaubt(profile))
          ? []
  ...
  try {
    ...
    for (const [index, teil] of stapel.entries()) {
      ...
      const teilErgebnis = await deps.eagerUnderstanding(teil.dokumente, {
        ...
        // RESTZEITWACHE (§29): die MODELLAUFRUFE enden vor derselben absoluten Grenze,
        // vor der auch die Vormerkung enden muss — kein Aufruf kann mehr in die
        // Abschlussreserve oder das Funktionsende hineinlaufen.
        deadlineMs: startedMs + budgetMs - ABSCHLUSS_RESERVE_MS,
