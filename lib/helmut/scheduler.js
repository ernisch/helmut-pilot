...
async function runSourceCrawl(politicianId = cemInceProfile.id, options = {}) {
  ...
  // TTL bewusst UNTER der Vercel-maxDuration (300s), gleiche Begruendung wie beim
  // Briefing-Lock: ein durch Function-Timeout hart abgebrochener Crawl gibt den
  // Lock nie via finally frei. Vorher 15 Minuten -> ein gekillter Lauf blockierte
  // bis zu 15 Min. lang jeden weiteren Trigger (Cron 04:00/20:00, Watchdog, manueller
  // Button).
...
async function runLageCheck(politicianId = cemInceProfile.id) {
  ...
  // Perf/Kosten-Fix: dieser Crawl lief bisher OHNE Lock (anders als runSourceCrawl) —
  // zwei ueberlappende Ausloeser (Cron 10:00 UTC + manueller "Lage pruefen"-Button)
  // konnten denselben ~90-Quellen-Crawl doppelt anstossen. Gleicher Lock-Mechanismus
  // wie runSourceCrawl, eigener Job-Name (Lage-Check und Voll-Crawl duerfen sich
  // weiterhin nicht gegenseitig blockieren). TTL analog kurz gehalten (< maxDuration).
  const lockName = `lage-crawl-${politicianId}`;
  const locked = await acquirePipelineLock(lockName, 4 * 60 * 1000);
  if (!locked) {
    console.warn(`[runLageCheck] Crawl läuft bereits für ${politicianId}, übersprungen.`);
    return {
      skipped: true,
