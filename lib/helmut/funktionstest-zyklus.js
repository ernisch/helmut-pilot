...
//     zwei Stunden nach dem Fensterende.
//
// ─── PRÄZISIERUNG (vierter Reviewbefund, nachgeprüft) ───────────────────────
// Hier stand zuerst: „Die sichtbare Produktstufe entsteht im sicheren Fenster
// GAR NICHT" und „mit KEINEM Aufruf bestehender Routen zu umgehen". Das war ZU
// ABSOLUT und ist zurückgenommen. Richtig ist:
//
//   * ÜBER DIE WARTESCHLANGE entsteht im Fenster kein Briefing. Das gilt
//     uneingeschränkt und ist der Befund, den diese Funktion rechnet.
//   * ÜBER DEN DIREKTPFAD `/api/cron/lage-briefing` sehr wohl: er ruft
//     `buildLageBriefing` je Profil unmittelbar auf und kennt die Phasenfenster
//     der Warteschlange gar nicht (server.js).
//
// Der Direktpfad ist deshalb aber KEIN gleichwertiger Ersatz, und der
// Zyklus-Startweg treibt ihn bewusst nicht an:
//   * Er ist je Aufruf auf 240 s Arbeitszeit begrenzt und arbeitet die Profile
//     in fester Listenreihenfolge ab — bei 500 Profilen kommt je Aufruf nur ein
//     Ausschnitt durch, der Rest bekommt `reason: "zeitbudget"`.
//   * Er wirkt auf ALLE aktiven Profile, also auch auf die FÜNF REALEN Mandate.
//     Er erzeugte dort Briefings zu einer unüblichen Stunde. Das ist eine eigene
//     Betreiberentscheidung und gehört nicht in einen Zyklusstart.
//
// Die Hürde bleibt damit richtig — sie sagt „nicht über die Warteschlange" —,
// aber sie sagt es jetzt genau. Wer den Direktpfad will, entscheidet das
// getrennt und mit offenen Augen.
//
// Diese Funktion rechnet den Warteschlangenbefund aus, statt ihn zu behaupten.
function arbeitsklassenImFenster({
  ...
  // hier stand fest `24`, und KEIN Aufrufer übergab etwas anderes. Der MOTOR
  // liest die Breite dagegen aus `HELMUT_DEMAND_TENANT_MAX_AGE_H`
  // (`source-demand.fensterKonfig`, Default 24, Minimum 1, fail closed). Wäre die
  // Variable je gesetzt worden, hätten Tor und Motor still mit verschiedenen
  // Phasenfenstern gerechnet — genau das, was der Kommentar in
  // `funktionstest-500.js` ausdrücklich ausschließen wollte („damit Tor und Motor
  // nicht auseinanderlaufen können"). Jetzt kommt die Breite aus DERSELBEN
  // Quelle wie beim Motor; ein ausdrücklich übergebener Wert hat weiter Vorrang
  // (die Tests brauchen ihn).
  fensterBreiteStunden = null,
  env = process.env,
  phasen = null
} = {}) {
  const von = Number(fensterStartMinuteUtc);
  const bis = Number(fensterEndeMinuteUtc);
  const breite = fensterBreiteStunden === null || fensterBreiteStunden === undefined
    ? Number(require("./source-demand").fensterKonfig(env).mandatMaxAlterStunden)
    : Number(fensterBreiteStunden);
  ...
  return Object.freeze({
    ...
    // DIE Frage: entsteht die sichtbare Produktstufe im Fenster ÜBER DIE
    // WARTESCHLANGE? Der Direktpfad `/api/cron/lage-briefing` ist davon
    // unberührt — siehe die Präzisierung im Kopf dieses Blocks.
    sichtbareProduktstufeErreichbar: klassen
      .some((k) => k.jobType === "briefing_materialization" && k.imFensterFaellig),
    direktpfadHinweis: "Über die Warteschlange gerechnet. `/api/cron/lage-briefing` "
      + "erzeugt Briefings unmittelbar und kennt diese Phasenfenster nicht — er ist "
      + "aber je Aufruf auf 240 s begrenzt und wirkt auch auf die fünf REALEN Mandate.",
    meldung: ohne.length === 0
      ? "Jede mandatsgebundene Arbeitsklasse ist im Fenster mindestens teilweise fällig."
      : `NICHT im Fenster fällig: ${ohne.join(", ")}. Über die WARTESCHLANGE entsteht diese `
        + "Arbeit im gewählten Fenster nicht — unabhängig von Budget, Parallelität und "
        + "Aufrufzahl. Der Direktpfad /api/cron/lage-briefing ist davon unberührt, aber "
        + "je Aufruf auf 240 s begrenzt und wirkt auch auf die fünf realen Mandate."
  });
}
...
// EINE Freigabequelle fuer Tor UND Ausfuehrer. Ohne Stufe unveraendert das
// Pauschalwort; mit Stufe dasselbe stufengenaue Wort, das auch die Stufenhuerde
// in `funktionstest-500.startbereitschaft` prueft.
function stufenErlaubnis(stufe, env) {
  const s = stufe === null || stufe === undefined ? "" : String(stufe).trim().toLowerCase();
  if (!s) return freigabe(SCHRITT, env);
  const stufen = require("./testkohorte-stufen");
  if (!stufen.STUFEN.includes(s)) {
    // Eine unbekannte Stufe ist ein Aufruferfehler und darf NIE auf das
    // Pauschalwort zurueckfallen — sonst waere eine vertippte Stufe eine
    // Freigabe fuer alle 495.
    return Object.freeze({
      schritt: SCHRITT, erteilt: false, flagAn: false, wortStimmt: false,
      erwartetesWort: null,
      meldung: `Unbekannte Stufe "${String(stufe).slice(0, 20)}" — keine Freigabe (fail closed).`
    });
  }
  const f = stufen.startfreigabe(s, env);
  return Object.freeze({
    schritt: SCHRITT, stufe: s, erteilt: f.erteilt === true,
    flagAn: f.erteilt === true, wortStimmt: f.erteilt === true,
    erwartetesWort: f.erwartetesWort, meldung: f.meldung
  });
}
...
async function fuehreZyklusAus({
  ...
  env = process.env,
  // ERGAENZT 02.09. (Kreisschluss-Analyse, blockierender Befund): Tor und
  // Ausfuehrer verlangten ZWEI SICH AUSSCHLIESSENDE Freigabeworte in DERSELBEN
  // Variablen. Gemessen:
  //   startbereitschaft (Stufenhuerde) → TESTKOHORTE_STUFE_A_FACHZYKLUS_BESTAETIGT
  //   fuehreZyklusAus   (Ausfuehrer)   → TESTKOHORTE_FACHZYKLUS_STARTEN_BESTAETIGT
  //   beide lesen HELMUT_TESTKOHORTE_CONFIRM
  // Welches Wort auch gesetzt war, die jeweils andere Seite fiel durch — die
  // Kette blieb damit strukturell unerreichbar, genau wie vor der Korrektur der
  // Stufenhuerde. Wird eine STUFE uebergeben, gilt jetzt das stufengenaue Wort
  // fuer BEIDE Seiten; ohne Stufe bleibt es beim Pauschalwort (Bestandsverhalten
  ...
  const erlaubnis = stufenErlaubnis(stufe, env);
  ...
  return Object.freeze({
    werkzeug: "fachzyklus",
    schritt: SCHRITT,
    // EHRLICH BENANNTE GRENZE (Gegenprüfung 02.09.): Der Abbruch einer Scheibe
    // entscheidet an `antwort.ok`, und das ist der HTTP-Status (`standardAufruf`
    // gibt `ok: antwort.ok` zurück). Eine Scheibe, die HTTP 200 liefert, deren
    // Körper aber fachliche Fehlschläge meldet, zählt hier als Erfolg.
    //
    // Die schärfere Behauptung eines Prüfers — „eine fehlgeschlagene Scheibe
    // beendet den Lauf NICHT" — ist widerlegt: die Schleife bricht bei !gut
    // ausdrücklich ab (`abgebrochen = "scheibe-fehlgeschlagen"`). Sie bricht nur
    // eben auf der HTTP-Ebene ab.
    //
    // Diese Grenze wird NICHT stillschweigend erweitert: welche Felder des
    // Antwortkörpers einen fachlichen Fehlschlag bedeuten, ist ein Vertrag der
    // Route `/api/cron/pipeline`, der hier nicht belegt ist. Ihn zu erraten wäre
    // genau die Sorte Annahme, die dieses Vorhaben schon zweimal teuer bezahlt
    // hat. Die fachliche Bewertung leisten stattdessen die Abbruchregeln
    // A01–A15 an den Kontrollpunkten ZWISCHEN den Stufen — mit gemessenen
    // Zahlen statt mit einem Statuscode.
    abbruchEbene: "http",
    fachlicheBewertungDurch: "funktionstest-kontrolle (A01–A15, zwischen den Stufen)",
