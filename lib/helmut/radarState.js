...
const RELATION_LABELS = {
  party: "Bezug zu deiner Partei",
  constituency: "Aus deinem Wahlkreis",
...
// --- Profil-Terme (Name/Nachname) fuer die personenscharfe Erwaehnung -------
function profileTerms(profile = {}) {
  ...
  return { fullNameRaw, fullName, lastName };
}

// Ist dieser Vorgang eine DIREKTE persoenliche Erwaehnung? Rueckgabe traegt zugleich
// Nachweis (method) fuer confidence/evidence. Reihenfolge: strukturierte Erwaehnungs-
// felder (hoch) -> voller Name im Analysetext (mittel) -> distinktiver Nachname (mittel).
// KEINE Themen-/Partei-/Wahlkreisrelevanz -> das ist NICHT "Über dich".
function detectPersonMention(ko = {}, terms) {
  if (!terms || !terms.fullName) return null;
  ...
  if (people.some((p) => p === terms.fullName || p.includes(terms.fullName) || terms.fullName.includes(p))) {
    return { method: "structured", confidence: "high" };
  }
  const text = nameKey([ko.display_title, ko.headline, ko.display_summary, ko.was_ist_passiert, ko.why_relevant, ko.warum_wichtig].filter(Boolean).join(" "));
  if (wholeWordInText(text, terms.fullName)) return { method: "prose-fullname", confidence: "medium" };
  if (terms.lastName && (people.some((p) => wholeWordInText(p, terms.lastName)) || wholeWordInText(text, terms.lastName))) {
    return { method: "prose-lastname", confidence: "medium" };
  }
  return null;
}
