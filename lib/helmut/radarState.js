...
function radarProfileTerms(profile = {}) {
  return {
    parties: slugSet([profile.party, profile.partei, profile.faction, profile.fraktion]),
...
function radarPartyActorEvidence(ko = {}, src = null, partySlugs = new Set()) {
  const cat = slug(src && src.sourceType).replace(/\s+/g, "_");
  if (cat === "party" || cat === "faction") return true;
  const title = slug([ko && ko.display_title, ko && ko.headline].filter(Boolean).join(" "));
  if (!title) return false;
  for (const p of (partySlugs instanceof Set ? partySlugs : new Set())) {
    if (p && wholeWordInText(title, p)) return true;
  }
  return false;
}
...
function partyRelationBeleg(ko, matchedSlug, terms, src) {
  if (!(terms && terms.parties && terms.parties.has(matchedSlug))) return false;
  if (!slugSet(ko && ko.parteien).has(matchedSlug)) return false;
  return radarPartyActorEvidence(ko, src, terms.parties);
}
...
function radarRelationBeleg(seg, matchedValue, ko = {}, terms = {}, src = null) {
  const v = slug(matchedValue);
  if (!v) return false;
  if (seg === "party") return partyRelationBeleg(ko, v, terms, src);
  if (seg === "committee") return Boolean(terms.committees && terms.committees.has(v)) && slugSet(ko.ausschuesse).has(v);
  if (seg === "constituency") {
...
// src = primaere Quelle des Vorgangs (fuer den Partei-Akteurs-/Quellenbeleg).
function belegteSegments(d, ko, terms, src = null) {
  const out = new Map();
  for (const f of Array.isArray(d && d.matched_features) ? d.matched_features : []) {
    ...
    if (radarRelationBeleg(seg, cleanStr(f && f.value), ko, terms, src)) out.set(seg, cleanStr(f && f.value));
  }
  return out;
}
...
function buildEnvironment(decisions, kosById, sourcesByVorgang, terms, now) {
  ...
  for (const d of Array.isArray(decisions) ? decisions : []) {
    ...
    const src = pickPrimarySource(ko, sourcesByVorgang[ko.vorgang_id] || []);
    const byType = belegteSegments(d, ko, terms, src);
...
function buildArticles(decisions, kosById, sourcesByVorgang, mentions, terms, now) {
  ...
  for (const d of Array.isArray(decisions) ? decisions : []) {
    ...
    for (const seg of belegteSegments(d, ko, terms, src).keys()) {
...
function radarPartyDecision(d, ko, src, terms) {
  ...
  if (!slugSet(ko.parteien).has(v)) return { party: false, reason: `'${raw}' nur beilaeufig (mentioned_parties) — nicht in ko.parteien -> kein struktureller Partei-Akteur` };
  if (!radarPartyActorEvidence(ko, src, terms.parties)) return { party: false, reason: `'${raw}' zwar in ko.parteien, aber kein Akteurs-/Quellenbeleg (Quelle nicht Partei/Fraktion, Partei nicht im Titel) -> Medien-/Kommentar-Nennung, kein Partei-Umfeldsignal` };
...
function buildRadarRelationsDebug({ profile = {}, decisions = [], kosById = {}, sourcesByVorgang = {}, commit = null } = {}) {
  ...
  for (const d of Array.isArray(decisions) ? decisions : []) {
    ...
    const src = pickPrimarySource(ko, sourcesByVorgang[ko.vorgang_id] || []);
    const base = baseItemFields(ko, src);
    const dec = radarPartyDecision(d, ko, src, terms);
    items.push({
