...
function radarProfileTerms(profile = {}) {
  return {
    ...
    // Wahlkreis/Ort bewusst UNVERAENDERT (rohes slug) — kein Partei-/Ausschuss-Synonym-
    // Konzept anwendbar; ein konkreter Ort ist entweder woertlich genannt oder nicht.
    constituencies: slugSet([profile.constituency, profile.wahlkreis].concat(Array.isArray(profile.regionalInterests) ? profile.regionalInterests : []))
  };
}
...
function radarRelationBeleg(seg, matchedValue, ko = {}, terms = {}, docs = null) {
  ...
  if (seg === "committee") {
    ...
    if (!v) return false;
    // NUR ko.ausschuesse (strukturell verankert) — mentioned_committees genuegt NICHT
    // (bloße Erwaehnung), analog zur Partei-Regel (ko.parteien vs. mentioned_parties).
    return Boolean(terms.committees && terms.committees.has(v)) && committeeKeySet(ko.ausschuesse).has(v);
  }
  if (seg === "constituency") {
