...
//   party        -> eigene Partei/Fraktion gehoert zu den INVOLVIERTEN Parteien
//                   (strukturiertes Feld ko.parteien), nicht nur beilaeufig erwaehnt
//                   (mentioned_parties) in einem fremden Thema (z. B. Aussenpolitik).
...
function radarRelationBeleg(seg, matchedValue, ko = {}, terms = {}) {
  const v = slug(matchedValue);
  if (!v) return false;
  if (seg === "party") return Boolean(terms.parties && terms.parties.has(v)) && slugSet(ko.parteien).has(v);
...
function belegteSegments(d, ko, terms) {
  const out = new Map();
  for (const f of Array.isArray(d && d.matched_features) ? d.matched_features : []) {
    ...
    if (radarRelationBeleg(seg, cleanStr(f && f.value), ko, terms)) out.set(seg, cleanStr(f && f.value));
  }
  return out;
}
...
function buildEnvironment(decisions, kosById, sourcesByVorgang, terms, now) {
  ...
  for (const d of Array.isArray(decisions) ? decisions : []) {
    ...
    // Wahlkreis) — NICHT nur ein beilaeufiger Match. 'thema' ist kein Umfeld-Segment.
    const byType = belegteSegments(d, ko, terms);
...
function buildArticles(decisions, kosById, sourcesByVorgang, mentions, terms, now) {
  ...
  for (const d of Array.isArray(decisions) ? decisions : []) {
    ...
    for (const seg of belegteSegments(d, ko, terms).keys()) {
