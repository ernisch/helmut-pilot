...
// Zentrale Partei-/Fraktions-Normalisierung — DIESELBE, die das Matching zum Erzeugen der
// matched_features nutzt. Der Radar MUSS dieselbe kanonische Form vergleichen, sonst
// verwirft er einen vom Matching korrekt erzeugten Partei-Treffer wieder (Drift). Kein
// Zyklus: matching.js laedt weder radarState noch briefingContract beim Require.
const { slugParty } = require("./matching");
...
// P1-2 / zentrale Normalisierung: Partei/Fraktion VOR dem Vergleich auf die kanonische Form
// bringen, die AUCH das Matching nutzt (slugParty). Ohne das verwarf der Radar einen KORREKT
// gematchten Partei-Treffer wieder (z. B. Profil "Die Linke" vs. ko.parteien "Linke" -> rohes
// slug() ergab "die linke" != "linke"). Gilt fuer JEDE Partei/Fraktion und JEDES Profil —
// keine hartkodierte Partei, keine Sonderregel. (Ausschuss bewusst nicht — siehe unten.)
...
function radarProfileTerms(profile = {}) {
  return {
    ...
    // Ausschuss/Wahlkreis bewusst UNVERAENDERT (rohes slug) — dieser Fix ist auf die
    // Partei-/Fraktions-Normalisierung begrenzt (angefragter Scope) und aendert das
    // bestehende Verhalten fuer Bestandsnutzer NICHT. Der Ausschuss traegt dieselbe
    // latente Normalisierungsluecke; separat entscheidbar (siehe Bericht/Test-Hinweis).
    committees: slugSet([profile.committee].concat(Array.isArray(profile.committees) ? profile.committees : [])),
...
function radarRelationBeleg(seg, matchedValue, ko = {}, terms = {}, docs = null) {
  ...
  if (seg === "committee") {
    const v = slug(matchedValue);                 // Ausschuss unveraendert (Scope: Partei/Fraktion)
    if (!v) return false;
    return Boolean(terms.committees && terms.committees.has(v)) && slugSet(ko.ausschuesse).has(v);
  }
  if (seg === "constituency") {
