...
// minimierung defensiv auf CHANCE_RISK_MAX Zeichen gekürzt (kein Volltext).
...
// Kurzes, gekapptes Label aus einem KO-Freitextfeld (Datenminimierung).
function shortLabel(value) {
  const s = String(value == null ? "" : value).replace(/\s+/g, " ").trim();
  if (!s) return null;
  return s.length > CHANCE_RISK_MAX ? s.slice(0, CHANCE_RISK_MAX).replace(/\s+\S*$/, "") : s;
}
