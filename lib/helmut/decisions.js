...
// OHNE hartkodierte Mandats-/Personen-Biases (der V2-Pilot-/„Arbeit und Soziales"-
...
//
// DSGVO: es werden nur öffentliche politische Merkmale verarbeitet. matched_features
// tragen ausschließlich kurze öffentliche Identitäts-Labels ({type,value}, z. B.
// "partei:SPD"). chance/risk werden aus den KO-Analysefeldern (risiken/chancen)
// übernommen; da diese aus Modell-Freitext stammen, werden sie zur Daten-
// minimierung nur innerhalb CHANCE_RISK_MAX Zeichen ganz uebernommen.

const matching = require("./matching");

// Datenminimierung: max. Länge der aus dem KO übernommenen Freitext-Labels chance/risk.
const CHANCE_RISK_MAX = Number(process.env.HELMUT_DECISION_LABEL_MAX || 240);
...
// Kurzes ganzes Label; ein gekappter Praefix kann eine Verneinung verlieren.
function shortLabel(value) {
  const s = String(value == null ? "" : value).replace(/\s+/g, " ").trim();
  if (!s) return null;
  return s.length > CHANCE_RISK_MAX ? null : s;
}
...
function scoreKnowledgeObject(ko = {}, match = {}) {
  ...
  // "keiner" ist ein regulaerer Schemawert, aber kein Zeitdruck. Auch ein
  // unbekannter Altwert belegt keine Dringlichkeit; nur die positiven Stufen.
  const hasZeitdruck = ["hoch", "mittel", "niedrig"].includes(String(ko.zeitdruck || "").trim().toLowerCase());
...
function buildDecision(userId, ko, match = {}) {
  ...
  const risk = shortLabel(nonEmptyList(ko.risiken)[0]);
  const chance = shortLabel(nonEmptyList(ko.chancen)[0]);
  return {
    id: `dec-${userId}-${ko.id}`,
    user_id: userId,
...
async function runDecisionShadow(input = {}, overrides = {}) {
  ...
  try {
    ...
    return { userId, candidates: decisions.length, saved: (saved && saved.saved) || 0 };
  } catch (error) {
    return { skipped: true, reason: "decision-error", error: error && error.message };
  }
}
