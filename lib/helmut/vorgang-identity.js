...
const GENERISCHE_ANKER = new Set([
  // Ein Politikfeld verbindet weder Kapitalaufnahme, Ausweis-App noch
  // Gesundheitsamtssoftware zu demselben Ereignis. Konkrete gemeinsame
  // Sachanker bleiben erforderlich; das Feld allein traegt kein Beweisgewicht.
  "digitalisierung",
  "menschen", "personen", "deutschland", "deutsche", "deutschen", "millionen",
  // Geographische Gattung, kein Ereignis: "US-Bundesstaat Texas" ist kein
  // Beleg fuer "Bundesstaatsanwaltschaft" (Production 10.09.). Der volle
  // Justizbegriff bleibt spezifisch; nur die Gattungsformen sind generisch.
  "bundesstaat", "bundesstaaten", "bundesstaates",
...
const GENERISCHE_FUNKTIONSWOERTER = new Set([
  ...
  "laufende", "laufenden",
  // Production 15.09.: "heute" + "reist" verbanden zwei verschiedene
  // Ministerreisen. Zeitliche Naehe ist kein Sachbeleg fuer dasselbe Ereignis.
  // Im Quelltext und in der Promptauswahl bleibt das Zeitwort erhalten.
  "heute"
]);
...
const FAMILIEN_DEFINITION = {
  // Eine Personengruppe beschreibt weder Arbeitgeber noch konkreten Konflikt.
  // Der gleiche Belegvertrag gilt fuer alle Unternehmen und Quellen; fachliche
  // Komposita wie Hafenbeschaeftigte bleiben als ganze Begriffe erhalten.
  beschaeftigte: ["beschäftigte", "beschäftigten", "beschäftigter", "beschäftigtem",
    "beschaeftigte", "beschaeftigten", "beschaeftigter", "beschaeftigtem"],
...
function istGenerisch(anchor) {
  const a = String(anchor || "");
  // Production 25.09.: Die CO2-Preisvorlage traf einen Sammelvorgang allein
  // ueber den gemeinsamen Akteur. Eine Regierung ist kein Sachvorhaben;
  // konkrete gemeinsame Sachanker bleiben fuer dieselbe Sache erforderlich.
  if (["bundesregierung", "bundesregierungen"].includes(a)) return true;
  // Production 09.09.: "Gesetzentwurf" verband Kita und Energienetze;
  // das Ressortlabel "Politik" verband eine Wahl mit Leitungsmanipulationen.
  // Dokumentgattung und Ressort sind keine Ereignisidentitaet.
  if (["politik", "politische", "politischen", "gesetzentwurf", "gesetzentwuerfe", "gesetzentwürfe"].includes(a)) return true;
...
function anchorTokens(text) {
  // U+00AD ist eine bedingte Drucktrennung innerhalb eines Wortes. Der
  // Bundestag liefert z.B. Tank\u00adrabatt; daraus darf kein anderer Sachanker
  // werden. Echte Bindestriche/Wortgrenzen und gespeicherter Text bleiben erhalten.
  const roh = String(text || "").replace(/\u00ad/g, "").split(/[^A-Za-z0-9ÄÖÜäöüß]+/).filter(Boolean);
  const out = [];
  const seen = new Set();
...
// Hilfsverben, Verweise, unbestimmte Eigenschaften und Amtsbezeichnungen
// bestimmen keinen einzelnen Vorgang. Diese Auswahl betrifft ausschliesslich
// die Identitaetspruefung: Quelltext, Promptauswahl und alte Suchwurzeln bleiben
// erhalten. Insbesondere sind Amt + Personenname keine zwei Sachbelege.
const IDENTITAETS_ZUSATZWOERTER = new Set([
  "hatte", "hatten", "haette", "haetten", "hätte", "hätten", "wurde", "wurden",
  "zuvor", "zuletzt", "bereits", "inzwischen", "darueber", "darüber", "dabei",
  "dafuer", "dafür", "dazu", "danach", "daher", "deshalb", "damit", "diese",
  "dieser", "diesem", "diesen", "welche", "welcher", "welchen", "seinem",
  "ihrem", "seinen", "seiner", "ihrer", "neuen", "neuer", "neues", "neuem",
  "bekannt", "bekannte", "bekannten", "moeglich", "moegliche", "moeglichen",
  "möglich", "mögliche", "möglichen", "künstliche", "künstlichen", "kuenstliche",
  "kuenstlichen", "praesident", "praesidenten", "präsident", "präsidenten",
  "minister", "ministerin", "kanzler", "bundeskanzler", "vorsitzender",
  "vorsitzende", "vorsitzenden", "krieg", "krieges",
  "beschliesst", "beschließt", "beschliessen", "beschließen", "beschlossen",
  "beraet", "berät", "beraten", "erlaeutern", "erläutern",
  "besucht", "besuchte", "besuchen", "stellt", "stellen", "stellte",
  "verkuendet", "verkündet", "verkuendete", "verkündete", "verkuenden", "verkünden",
  "erlaeutert", "erläutert", "erlaeuterte", "erläuterte", "eroeffnet", "eröffnet",
  "eroeffnete", "eröffnete", "eroeffnen", "eröffnen"
]);
// Geschlossene Zusammensetzungen bekannter Amtsbezeichnungen, kein Stemmer:
// "Bundespraesident" ist ein Amt, "Bundespraesidentenwahl" bleibt ein Sachanker.
const IDENTITAETS_AMT = /^(?:(?:bundes|minister|vize|senats|parlaments)?pr(?:ä|ae)sident(?:en|in|innen)?|(?:ober)?b(?:ü|ue)rgermeister(?:in|innen|n)?)$/;
function identitaetsAnker(anchors = []) {
  return anchors.filter(a => !IDENTITAETS_ZUSATZWOERTER.has(a) && !HAEUFIGE_ABKUERZUNGEN.has(a) && !IDENTITAETS_AMT.test(a));
}
function vorgangsAnker(doc = {}) {
  return identitaetsAnker(docAnchors(doc));
}
function titelAnker(doc = {}) {
  return identitaetsAnker(anchorTokens(titelRumpf(doc)));
}
// Institutionen duerfen den Sachbeleg verstaerken, aber nicht ersetzen.
// Production 25.09.: Bundestag + Bundesregierung verbanden Fruehstartrente
// und Tankrabatt. Beide Tankrabattberichte bleiben ueber ihren Sachanker
// verbunden; alte Kennungen und Suchwurzeln bleiben unveraendert.
const TITEL_INSTITUTIONEN = new Set([
  "bundestag", "bundestages", "bundesrat", "bundesrates", "bundesregierung",
  "landtag", "landtages", "landesregierung", "parlament", "parlaments",
  "regierung", "regierungen"
]);
function titelBelegt(aTitel, bTitel, overlap) {
  const titel = anchorOverlap(aTitel.filter(a => !TITEL_INSTITUTIONEN.has(a)),
    bTitel.filter(a => !TITEL_INSTITUTIONEN.has(a)));
  const ereignisfamilie = titel.familien.some(f => GENERISCHE_FAMILIEN.has(f.schluessel)
    && f.schluessel !== "beschaeftigte");
  const sachfamilien = overlap.familien.filter(f => !f.generisch
    && !TITEL_INSTITUTIONEN.has(f.schluessel));
  return titel.spezifischeFamilien > 0 || (ereignisfamilie && sachfamilien.length >= 2);
}
...
function matchStaerke(a, b) {
  ...
  // Ein ausgeschriebener spezifischer Gesetzesname verliert im Genitiv nicht
  // seine Identitaet: Bundespolizeigesetz / Bundespolizeigesetzes. Nur die
  // beiden Genitivendungen s/es an einem langen *gesetz-Kompositum; keine
  // allgemeine Stammwortlogik, keine Ableitung und kein zweiter Beleg.
  // Jahres-/Ereigniskonflikte und die vorhandenen Beweisschwellen bleiben davor.
  if (kurz.length >= STRONG_ANCHOR_LEN && kurz.endsWith("gesetz") && !istGenerisch(kurz)
    && (lang === kurz + "s" || lang === kurz + "es")) return 2;
  ...
  // Ein generisches Teilwort belegt den spezifischen Gesamtbegriff nicht.
  // Gewicht 1 waere hier ebenfalls falsch: anchorOverlap betrachtet die
  // Familie der Ausgangsseite. Rueckwaerts wuerde aus Bundesstaat deshalb
  // ein spezifischer Treffer fuer Bundesstaatsanwaltschaft; zwei solche
  // Teilwoerter koennten wieder einen fremden Vorgang verbinden.
  if (kurz.length >= COMPOUND_MIN_LEN && lang.includes(kurz)) return istGenerisch(kurz) ? 0 : 2;
  return 0;
}
...
function docsShareEvent(a = {}, b = {}, opts = {}) {
  const aAnchors = opts.aAnchors || vorgangsAnker(a);
  const bAnchors = opts.bAnchors || vorgangsAnker(b);
  ...
  if (overlap.spezifischeFamilien >= 1 && overlap.gewichtSpezifisch >= noetig) {
    // Gemeinsamer Hintergrund in den Kurzfassungen ersetzt kein gemeinsames
    // Hauptthema. Mindestens ein spezifischer Titelbeleg muss auf beiden
    // Seiten vorhanden sein. Fehlende Titel werden nicht aus anderen Feldern
    // erfunden; das Dokument bleibt als eigener Vorgang verfuegbar.
    if (!titelBelegt(opts.aTitel || titelAnker(a), opts.bTitel || titelAnker(b), overlap)) {
      return { gleich: false, grund: "kein-gemeinsamer-titelbeleg", overlap };
    }
...
function clusterRawDocuments(rawDocuments = []) {
  ...
  const anchors = docs.map((d) => vorgangsAnker(d));
  const titel = docs.map((d) => titelAnker(d));

  // Union-Find ueber alle Paare.
  const parent = docs.map((_, i) => i);
  const find = (i) => { while (parent[i] !== i) { parent[i] = parent[parent[i]]; i = parent[i]; } return i; };
  const union = (i, j) => { const a = find(i); const b = find(j); if (a !== b) parent[Math.max(a, b)] = Math.min(a, b); };
  const kantenGewicht = new Map();
  const paar = (i, j) => Math.min(i, j) * docs.length + Math.max(i, j);
  for (let i = 0; i < docs.length; i += 1) {
    for (let j = i + 1; j < docs.length; j += 1) {
      const vor = docsShareEvent(docs[i], docs[j], { aAnchors: anchors[i], bAnchors: anchors[j], aTitel: titel[i], bTitel: titel[j] });
      if (!vor.gleich) continue;
      const zurueck = docsShareEvent(docs[j], docs[i], { aAnchors: anchors[j], bAnchors: anchors[i], aTitel: titel[j], bTitel: titel[i] });
      if (!zurueck.gleich) continue;
      kantenGewicht.set(paar(i, j), Math.min(vor.overlap.gewichtSpezifisch, zurueck.overlap.gewichtSpezifisch));
      union(i, j);
    }
  }
  ...
  const gewichtVon = (i, j) => kantenGewicht.get(paar(i, j)) || 0;
  const kante = (i, j) => gewichtVon(i, j) > 0;

  // Ein fehlender Kern ist kein bestandener Kernnachweis. Solche Komponenten
  // werden ueber direkte Belege aufgeteilt; jede neue Teilgruppe verlangt eine
  // Kante zwischen JEDEM Mitgliederpaar. Staerkere Belege kommen zuerst, ein
  // stabiler Dokumentvergleich loest Gleichstaende. Kein Dokument geht verloren.
  const ohneKernAufteilen = indizes => {
    const teile = new Map(indizes.map(i => [i, [i]]));
    const zuordnung = new Map(indizes.map(i => [i, i]));
    const schluessel = i => String(docs[i].id || docs[i].title || "");
    const kanten = [];
    for (let a = 0; a < indizes.length; a += 1) for (let b = a + 1; b < indizes.length; b += 1) {
      const i = indizes[a], j = indizes[b], gewicht = gewichtVon(i, j);
      if (gewicht) kanten.push({ i, j, gewicht, key: [schluessel(i), schluessel(j)].sort().join("\u0000") });
    }
    kanten.sort((a, b) => b.gewicht - a.gewicht || (a.key < b.key ? -1 : a.key > b.key ? 1 : 0));
    for (const { i, j, gewicht } of kanten) {
      const a = zuordnung.get(i), b = zuordnung.get(j);
      if (a === b) continue;
      const links = teile.get(a), rechts = teile.get(b);
      // Erst bei der schwächsten gemeinsamen Kante darf die Gruppe wachsen.
      if (!links.every(x => rechts.every(y => gewichtVon(x, y) >= gewicht))) continue;
      links.push(...rechts);
      for (const x of rechts) zuordnung.set(x, a);
      teile.delete(b);
    }
    return [...teile.values()];
  };
  ...
  for (const indizes of gruppen.values()) {
    ...
    if (gruppe.length >= 3) {
      ...
      const kern = coreAnchors(gruppe.map((i) => anchors[i])).filter(a => !istGenerisch(a));
      if (kern.length) {
        ...
        for (const teil of ohneKernAufteilen(gruppe)) cluster.push(...entkette(teil));
        continue;
      }
    }
...
function candidatePrefixes(cluster = {}, limit = 3, opts = {}) {
  const prefixes = topicRoots(cluster, limit).map(vorgangPrefix);
  if (opts.personengruppenAltbestand === true) {
    // Nur Suchhinweise fuer alte Kennungen, nie neue Identitaetsbelege.
    // Vor der Korrektur konnte diese lange Personengruppenbezeichnung die
    // Kennung eines sachlich richtigen Vorgangs tragen. Hoechstens zwei
    // zusaetzliche Praefixe (Umlaut/ASCII), ausschliesslich aus den Quellen.
    for (const anchor of (cluster.documents || []).flatMap(docAnchors)) {
      // Historische Kennungen bleiben Suchkandidaten; der Akteur liefert
      // dabei niemals einen Identitaetsbeleg fuer die Zusammenfuehrung.
      if (anchor === "bundesregierung" || anchor === "bundesregierungen")
        prefixes.push(vorgangPrefix(anchor));
      if (beweisfamilie(anchor) !== "beschaeftigte") continue;
      prefixes.push(vorgangPrefix(anchor.startsWith("beschä") ? "beschäftigte" : "beschaeftigte"));
    }
  }
...
function sameVorgang(cluster = {}, bestand = {}) {
  ...
  const kernBestand = altDocs.length
    ? coreAnchors(altDocs.map((d) => vorgangsAnker(d)))
    : identitaetsAnker(anchorTokens(`${bestand.headline || ""} ${bestand.display_title || ""}`));
  ...
  // Ein schon vermischter Bestand darf keine neue Quelle anziehen. Sonst
  // erweitert ein passender Teil den gesamten Digest im naechsten Modellprompt.
  // Derselbe Ereignisvertrag schuetzt bereits die Erstgruppierung und Anzeige.
  if (altDocs.length > 1 && clusterRawDocuments(altDocs).length !== 1) {
    return nein("bestand-mehrere-ereignisse", beleg);
  }

  // --- Kern des neuen Clusters --------------------------------------------
  const kernNeu = coreAnchors(neueDocs.map((d) => vorgangsAnker(d)));
  ...
  // --- Kernvergleich -------------------------------------------------------
  // Gemessen werden spezifische Beweisfamilien, nicht die Anzahl der Woerter.
  // Ein kleiner Kern benoetigt zusaetzlich direkte Ereignisbelege.
  ...
  // Auch bei starken Kerntreffern muss JEDES neue Dokument dieselbe
  // Ereignispruefung gegen einen Originalbeleg bestehen wie bei der
  // Erstgruppierung. Trump/Trumps allein genuegt nicht. Eine echte Fortsetzung
  // zum Rentenpaket bleibt dagegen ueber Rentenpaket + Kabinett gebunden,
  // auch wenn Kabinett nicht im Mehrheitskern des ganzen Vorgangs steht.
  // Production 26.09.: "Sicherheitsrat" umging als langer Anker den
  // zeitabhaengigen Paarvergleich; die Vereinigung zweier alter Frankreich-
  // Quellen lieh sich Belege aus verschiedenen Ereignissen. Kein Kerngewicht
  // darf den direkten, beidseitigen Originalbeleg ersetzen.
  if ((altDocs.length || o.gewichtSpezifisch < MIN_BEWEISGEWICHT)
    && !neueDocs.every(neu => altDocs.some(alt =>
      docsShareEvent(neu, alt).gleich && docsShareEvent(alt, neu).gleich))) {
    return nein(o.gewichtSpezifisch < MIN_BEWEISGEWICHT
      ? "schwacher-kern-ohne-ereignisbeleg" : "kern-ohne-ereignisbeleg", beleg);
  }
  spur.erfuellt.push("spezifische-kernueberdeckung");

  // Der Bestandsweg darf den Titelvertrag der ersten Gruppierung nicht umgehen.
  // JEDES neue Dokument braucht einen passenden Titel im Bestand. Eine einzige
  // passende Quelle darf keine fachfremden Begleiter in den Vorgang mitnehmen.
  const titelBestand = altDocs.length ? altDocs : [{ title: `${bestand.headline || ""} ${bestand.display_title || ""}` }];
  if (!neueDocs.every(neu => titelBestand.some(alt => titelBelegt(titelAnker(neu), titelAnker(alt),
    anchorOverlap(vorgangsAnker(neu), vorgangsAnker(alt)))))) return nein("titelbeleg-fehlt", beleg);
  spur.erfuellt.push("titelbeleg-vorhanden");
