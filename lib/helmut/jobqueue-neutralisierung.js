...
// DRITTE FASSUNG (2026-08-25): DER KANONISCHE ABGLEICH HAT DIE ABSICHT GESCHLOSSEN.
//   Zeitliche Abfolge, jeweils tuerkische Zeit / Berlin / UTC:
//     08:55:30 / 07:55:30 / 05:55:30  Production-TROCKENLAUF der zweiten Fassung: alle Riegel
//                                     bestanden, Meldung TROCKENLAUF-OK, Transaktion
//                                     VOLLSTAENDIG zurueckgerollt — nichts veraendert.
//     08:56:06 / 07:56:06 / 05:56:06  Nachpruefung: Outbox noch `bestaetigt`, alle sieben
//                                     Vorpruefungswerte 1, keine offene Transaktion, keine
//                                     zurueckgelassene Sperre.
//     08:56:14 / 07:56:14 / 05:56:14  Der REGULAERE Outbox-Abgleich setzt die Absicht auf
//                                     `verzichtet` (updated_at 05:56:14.770379Z).
//   URSACHE, BELEGT: `helmut_outbox_abgleich` schliesst Absichten TERMINALER Auftraege —
//   `where j.status in ('erledigt','fehlgeschlagen') and o.status in ('offen','versendet',
//   'aufgegeben','bestaetigt')`. Der Auftrag ist seit 04:01 UTC `fehlgeschlagen`; die Absicht
//   war `bestaetigt`. Das ist der kanonische Abgleich und KEINE Wirkung des Trockenlaufs —
//   dieser war zu diesem Zeitpunkt laengst zurueckgerollt (Nachpruefung 8 Sekunden vorher).
//
//   WARUM `updated_at` JETZT VERTRAGSBESTANDTEIL IST: `verzichtet` ist der STABILE Endzustand.
//   Der Sweep oben fasst `verzichtet` nicht mehr an (der Wert fehlt in seiner Statusliste), die
//   Wiedereroeffnung verlangt `j.status = 'wartend'`, `helmut_outbox_vergeben` waehlt nur
//   `offen`/`versendet`, `helmut_outbox_quittieren` nur `versendet`, `helmut_outbox_zuruecklegen`
//   bricht bei jedem anderen Status ab, und `helmut_outbox_erneut_vorlegen` bricht ab, sobald der
//   Auftrag nicht `wartend` ist. Zugleich setzt der Trigger `helmut_job_outbox_kappen_trg` bei
//   JEDEM Update `updated_at := now()`. Damit ist `updated_at` ein allgemeiner Aenderungsmelder:
//   jede spaetere Beruehrung der Zeile — aus welcher Quelle auch immer — verschiebt ihn und
//   stoppt den Ablauf geschlossen.
//
// ZWEITE FASSUNG (2026-08-25), historisch:
...
const EINZEILEN_VERTRAG_TESTZEILE = Object.freeze({
  beleg: "docs/betrieb/op30-aktivierung-5-mandate.md §31.9 (kanonischer Endzustand 2026-08-25, rein lesend erhoben)",
  verfahren: "einzeilen-neutralisierung-testzeile-verzichtet-2026-08-25",
  ...
  outbox: Object.freeze({
    ...
    // STABILER ENDZUSTAND: der kanonische Abgleich hat die Absicht des terminalen Auftrags
    // am 2026-08-25 um 08:56:14 Uhr tuerkischer Zeit (07:56:14 Berlin, 05:56:14 UTC)
    // geschlossen. `verzichtet` ist der Endzustand — kein Schreiber fasst ihn erneut an.
    status: "verzichtet",
    versuche: 1,
    maxVersuche: 10,
    schemaVersion: 1,
    ...
    // Aenderungsmelder: der Trigger setzt updated_at bei JEDEM Update. Jede spaetere
    // Beruehrung der Zeile verschiebt ihn und stoppt den Ablauf (Riegel E4.12).
    aktualisiertAm: "2026-08-25 05:56:14.770379+00"
  }),
...
function pruefeEinzeilenVertrag(vertrag) {
  ...
  for (const [feld, wert] of [["auftrag.faelligAb", a.faelligAb], ["auftrag.ersteFaelligkeit", a.ersteFaelligkeit],
    ...
    ["outbox.bestaetigtAm", o.bestaetigtAm], ["outbox.aktualisiertAm", o.aktualisiertAm]]) {
  ...
  if (a.status === "wartend" || o.status === "offen" || o.status === "bestaetigt") {
    throw new Error("Einzeilenvertrag veraltet: die frueher beschriebenen Zustaende (Auftrag 'wartend',"
      + " Outbox 'offen' oder 'bestaetigt') gelten seit dem 2026-08-25 nicht mehr — der Vertrag muss den"
      + " kanonischen Endzustand beschreiben (Auftrag 'fehlgeschlagen', Outbox 'verzichtet')");
  }
  ...
  if (o.status !== "verzichtet" || o.versuche < 1 || o.versuche > o.maxVersuche) {
    throw new Error("Einzeilenvertrag unzulaessig: die Outbox-Absicht muss im kanonischen Endzustand"
      + " 'verzichtet' stehen und mindestens einen Versuch tragen");
  }
...
function pruefeEinzeilenSql(sqlText, sensibleWerte = {}) {
  ...
  if (/(created_at|due_at|first_due_at|next_attempt_at|first_claimed_at|finished_at|sent_at|confirmed_at|updated_at)\s*(<|>|<=|>=)/i.test(t)) {
...
select
  ...
  (select count(*) from public.helmut_job_outbox where id = '${o.id}'::uuid
     ...
     and next_attempt_at = timestamptz '${o.naechsterVersuchAb}'
     and updated_at = timestamptz '${o.aktualisiertAm}')                                       as outbox_vertragstreu,  -- erwartet 1
...
function einzeilenNeutralisierungSql(vertrag = EINZEILEN_VERTRAG_TESTZEILE, { modus = "trockenlauf" } = {}) {
  ...
  const OUTBOX_FELDER = [
    ...
    // E4.12 ist der allgemeine AENDERUNGSMELDER: der Trigger helmut_job_outbox_kappen_trg setzt
    // updated_at bei JEDEM Update. Jede spaetere Beruehrung der Zeile — durch welchen Schreiber
    // auch immer — verschiebt ihn und stoppt den Ablauf, bevor irgendetwas geloescht wird.
    ["E4.12", "Aenderungszeitpunkt (Aenderungsmelder)", `updated_at = timestamptz '${o.aktualisiertAm}'`]
  ];
