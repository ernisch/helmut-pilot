...
//   dass genau diese Werte geprueft werden — nur `idempotency_key = 'k'` und
//   `payload = '{}'::jsonb` unterscheiden die versehentliche Testzeile von echter Arbeit.
...
// Die Werte stammen aus der rein lesenden Production-Pruefung vom 2026-08-24, 21:33 UTC
// (Runbook §31.6.1) — Eingaben, keine Selbstauskunft des Skripts.
const EINZEILEN_VERTRAG_TESTZEILE = Object.freeze({
  beleg: "docs/betrieb/op30-aktivierung-5-mandate.md §31.6 (Stoerung 2026-08-24, rein lesend erhoben)",
  verfahren: "einzeilen-neutralisierung-testzeile-2026-08-24",
  auftrag: Object.freeze({
    ...
    prioritaet: 100,
    wiedervorlagen: 0,
    faelligAb: "2026-08-24 20:32:12.512+00",
  ...
  outbox: Object.freeze({
    ...
    naechsterVersuchAb: "2026-08-24 20:32:13.047778+00"
  }),
  // Der Datenbankvertrag: genau EINE eingehende Fremdschluesselbeziehung auf `helmut_jobs`,
  // und das ist die Outbox-Kaskade. Rein lesend am 2026-08-24 aus dem Katalog erhoben.
...
function pruefeEinzeilenVertrag(vertrag) {
  ...
  for (const [feld, wert] of [["auftrag.jobType", a.jobType], ["auftrag.idempotenzschluessel", a.idempotenzschluessel],
    ["auftrag.aktualitaetsfenster", a.aktualitaetsfenster], ["auftrag.status", a.status], ["outbox.status", o.status]]) {
  ...
  for (const [feld, wert] of [["auftrag.faelligAb", a.faelligAb], ["auftrag.ersteFaelligkeit", a.ersteFaelligkeit],
    ["auftrag.erstelltAm", a.erstelltAm], ["outbox.erstelltAm", o.erstelltAm],
    ["outbox.naechsterVersuchAb", o.naechsterVersuchAb]]) {
  ...
  // Der Auftrag MUSS unberuehrt sein — ein Vertrag, der eine Beanspruchung erlaubt, waere sinnlos.
  if (a.status !== "wartend" || a.versuche !== 0 || a.wiedervorlagen !== 0) {
    throw new Error("Einzeilenvertrag unzulaessig: nur ein unberuehrter Auftrag (wartend/0 Versuche/0 Wiedervorlagen) darf Ziel sein");
  }
...
function pruefeEinzeilenSql(sqlText) {
  ...
  const vorkommen = (t.match(EINZEILEN_SPALTE) || []).length;
  const vergleiche = (t.match(EINZEILEN_VERGLEICH) || []).length;
  ...
  if (/(created_at|due_at|first_due_at|next_attempt_at)\s*(<|>|<=|>=)/i.test(t)) {
...
select
  ...
  (select count(*) from public.helmut_jobs where id = '${a.id}'::uuid
     ...
     and tenant_id is null and payload = '{}'::jsonb and last_error is null
     and lease_owner is null and lease_expires_at is null
     and first_claimed_at is null and finished_at is null
  ...
  (select count(*) from public.helmut_job_outbox where id = '${o.id}'::uuid
     ...
     and schema_version = ${o.schemaVersion} and transport is null
     and sent_at is null and confirmed_at is null and last_error is null
...
function einzeilenNeutralisierungSql(vertrag = EINZEILEN_VERTRAG_TESTZEILE, { modus = "trockenlauf" } = {}) {
  ...
  const AUFTRAG_FELDER = [
    ...
    ["E2.7", "Fehlertext (muss leer sein)", "last_error is null"],
    ...
    ["E7.1", "Status (keine Statusaenderung)", `status = '${a.status}'`],
    ["E7.2", "Versuchszahl (keine Wiederholung)", `attempts = ${a.versuche}`],
    ["E7.3", "Versuchsobergrenze", `max_attempts = ${a.maxVersuche}`],
    ["E7.4", "Wiedervorlagen (keine Wiederholung)", `wiedervorlagen = ${a.wiedervorlagen}`],
    ["E7.5", "Lease-Besitzer (keine Beanspruchung)", "lease_owner is null"],
    ["E7.6", "Lease-Ablauf (keine Beanspruchung)", "lease_expires_at is null"],
    ["E7.7", "erste Beanspruchung (nie beansprucht)", "first_claimed_at is null"],
    ["E7.8", "Abschlusszeitpunkt (nicht abgeschlossen)", "finished_at is null"]
  ];
  const OUTBOX_FELDER = [
    ...
    ["E4.5", "Transport (muss leer sein)", "transport is null"],
    ["E4.6", "Sendezeitpunkt (muss leer sein)", "sent_at is null"],
    ["E4.7", "Bestaetigungszeitpunkt (muss leer sein)", "confirmed_at is null"],
