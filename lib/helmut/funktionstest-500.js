...
function pruefeKonfiguration(konfiguration = {}, { mandate = MANDATE_GESAMT, messungen = {} } = {}) {
  ...
  return Object.freeze({
    bereit,
    mandate,
    // BEHOBEN 02.09. (Nachprüfung nach dem Merge von #295, bestätigter Befund):
    // `gelesen` war eine rein modulinterne Variable und wurde NIE zurückgegeben —
    // `startbereitschaft()` las aber genau `konfig.gelesen.maxParallel` und
    // `konfig.gelesen.maxAnfragenJeMinute`. Beide Zugriffe liefen ins Leere, die
    // Zyklushürde rechnete deshalb IMMER mit Parallelität 1 und ohne
    // Minutengrenze — unabhängig davon, was der Betreiber konfiguriert hatte.
    //
    // Die Richtung des Fehlers war die sichere (Parallelität 1 lässt WENIGER
    // Aufrufe zu als 2, die Hürde war also zu streng statt zu lax, und ein
    // falsches „startbereit" konnte daraus nicht entstehen). Falsch war er
    // trotzdem: eine Betreiberentscheidung für Parallelität 2 wäre im Tor
    // wirkungslos geblieben, ohne dass irgendwo ein Hinweis erschienen wäre —
    // und der Betreiber hätte nicht sehen können, WARUM die Hürde weiter rot ist.
    gelesen: Object.freeze({ ...gelesen }),
...
function startbereitschaft({
  ...
  stufe = null,
  bestandeneStufen = [],
  // ERGÄNZT 02.09. (Betreiberentscheidung): die konkreten Zeitpunkte des
  // Fälligkeitstores. Minuten des Tages genügen nicht — der Frischefensterschlüssel
  // ist datumsabhängig. Fehlt die Angabe, ist die Hürde fail closed nicht erfüllt.
  faelligkeitsfenster = null,
  // ERGÄNZT 02.09. (zweiter adversarialer Review): Seit `gelesen` zurückgegeben
  // wird, entscheidet der vom Betreiber ERKLÄRTE `maxParallel` über die
  // Kapazitätshürde — und eine erklärte 2 kippt beide Tagesfenster von rot auf
  // grün, ohne dass irgendwo eine erreichbare Parallelität GEMESSEN wäre
  // („Parallelität 2" ist ausdrücklich unterbestimmt: Worker × Verstehen ergibt
  // heute 2, mit dem Vorschlag 4, das Modell kennt nur EINE Ebene). Die Korrektur
  // hat die Hürde damit in die UNSICHERE Richtung geöffnet. Ohne diesen
  // ausdrücklichen Beleg rechnet die Hürde deshalb weiter mit 1.
  parallelitaetBelegt = false,
  env = process.env
} = {}) {
  ...
  const arbeitsklassen = Number.isFinite(fenster.startMinuteUtc) && Number.isFinite(fenster.endeMinuteUtc)
    ? require("./funktionstest-zyklus").arbeitsklassenImFenster({
      ...
      // ERGAENZT 02.09. (zweiter adversarialer Review): der Commit stellte die
      // Fensterbreite von fest 24 h auf `fensterKonfig(env)` um, damit Tor und
      // Motor nicht auseinanderlaufen — reichte `env` hier aber NICHT durch.
      // Die beiden Geschwisterrechnungen desselben Befunds lasen damit aus zwei
      // verschiedenen Umgebungen.
      env
    })
    : { bewertbar: false, grund: "Fenstergrenzen fehlen" };

  const parallelErklaert = konfig.gelesen && typeof konfig.gelesen.maxParallel === "number"
    ? konfig.gelesen.maxParallel : 1;
  // FAIL CLOSED: eine erklärte Parallelität über 1 gilt erst, wenn sie belegt ist.
  const parallelGenutzt = parallelitaetBelegt === true ? parallelErklaert : Math.min(parallelErklaert, 1);
  // ── STUFENGENAUE KAPAZITAET (Kreisschluss-Analyse 02.09., bestaetigter Befund) ──
  // Hier stand kein `mandate`, also galt der Vorgabewert 500 — die Huerde verlangte
  // fuer Stufe A (20 Profile) denselben vollen 500-Mandate-Zyklus wie fuer Stufe C.
  // Gemessen: „1812 noetig" fuer jede Stufe, unabhaengig von der Kohortengroesse.
  // Damit war die kleine, billige Absicherungsstufe nur in dem Fenster zulaessig,
  // das fuer 500 Mandate dimensioniert ist — die gestufte Absicherung lief ins Leere.
  // Die Zahl ist jetzt die tatsaechliche: Kohorte der Stufe PLUS die fuenf realen
  // Mandate, die im selben Fenster weiterlaufen.
  const stufenMandate = (() => {
    const st = require("./testkohorte-stufen");
    const s2 = stufe === null || stufe === undefined ? "" : String(stufe).trim().toLowerCase();
    if (!st.STUFEN.includes(s2)) return 500;      // ohne Stufe fail closed: die groesste Menge
    return st.STUFEN_AKTIV_KUMULIERT[s2] + 5;
  })();
  const zyklus = kapazitaet.zyklusPasstInsFenster({
    fensterMinuten,
    mandate: stufenMandate,
    parallel: parallelGenutzt,
  ...
  const stufenBefund = (() => {
    ...
    // KORRIGIERT 02.09. (zweiter adversarialer Review): hier stand
    // `vertrag.offeneFreigaben.length === 0` — also ALLE FÜNF schreibenden
    // Freigaben einer Stufe GLEICHZEITIG. `HELMUT_TESTKOHORTE_CONFIRM` trägt aber
    // genau EIN Wort; die Hürde konnte strukturell nie grün werden und hätte dem
    // Betreiber vier Worte für ein Feld genannt. Zum STARTEN gebraucht wird die
    // Freigabe des Fachzyklus; Anlage, Aktivierung, Deaktivierung und Entfernung
    // werden zu ihrem jeweiligen Zeitpunkt freigegeben — nicht gleichzeitig.
    const start = stufen.startfreigabe(s, env);
    const ok = start.erteilt && reihenfolge.zulaessig;
    return {
      ok,
      detail: ok
        ? `Stufe ${s.toUpperCase()} (${vertrag.umfang} Profile): Startfreigabe für den `
          + "Fachzyklus liegt vor, Reihenfolge zulässig."
        : `Stufe ${s.toUpperCase()} (${vertrag.umfang} Profile): `
          + (start.erteilt ? "" : `${start.meldung} `)
          + reihenfolge.meldung
    };
  })();

  // ── DAS FÄLLIGKEITSTOR (Betreiberentscheidung 02.09.) ────────────────────
  // Es ersetzt die Schnittmengenprüfung als START­ENTSCHEIDUNG. Es braucht
  // konkrete Zeitpunkte — Minuten des Tages genügen nicht, weil der
  // Frischefensterschlüssel datumsabhängig ist und über Mitternacht wechselt.
  // Ohne diese Angaben ist es fail closed nicht erfüllt.
  const faelligkeit = (() => {
    const modul = require("./funktionstest-faelligkeit");
    const eingabe = faelligkeitsfenster && typeof faelligkeitsfenster === "object"
      ? faelligkeitsfenster : {};
    const s = stufe === null || stufe === undefined ? null : String(stufe).trim().toLowerCase();
    const befund = modul.faelligkeitsBefund({
      stufe: s,
      kennungen: Array.isArray(eingabe.kennungen) ? eingabe.kennungen : null,
      fensterStartMs: eingabe.fensterStartMs ?? null,
      fensterEndeMs: eingabe.fensterEndeMs ?? null,
      planungsZeitpunktMs: eingabe.planungsZeitpunktMs ?? null,
      // ROH DURCHREICHEN. `?? 1` liess ausgerechnet die beiden gefaehrlichen
      // Werte durch (`0` und `""`), die das Tor auf eine Schwelle von 0 und
      // damit auf ein pauschales Gruen gestellt haetten. Das Modul entscheidet.
      mindestAbdeckung: eingabe.mindestAbdeckung,
      // BESTAND statt „offene Aufträge" (Betreiberbefund 02.09., Kreisschluss):
      // die alte Zahl verlangte, dass die volle geplante Menge noch UNBEARBEITET
      // dasteht — ein erfolgreich abgearbeiteter Zyklus wäre blockiert worden.
      bestand: eingabe.bestand ?? null,
      rotation: Array.isArray(eingabe.rotation) ? eingabe.rotation : null,
      weitereAktiveMandate: Array.isArray(eingabe.weitereAktiveMandate)
        ? eingabe.weitereAktiveMandate : null,
      env
    });
    if (befund.bewertbar !== true) {
      return { ok: false, detail: `Nicht bewertbar: ${befund.grund}`, befund };
    }
    // `vollstaendigerZyklus === null` heißt NICHT BEWERTBAR und ist ausdrücklich
    // kein Erfolg: die Fälligkeit reicht, der Status ist aber ungemessen.
    return { ok: befund.vollstaendigerZyklus === true, detail: befund.urteil, befund };
  })();
  ...
  const huerden = [
    ...
    //
    // ═══ ABGELÖST 02.09. DURCH BETREIBERENTSCHEIDUNG ═══════════════════════
    // Diese Hürde prüfte die SCHNITTMENGE des Testfensters mit dem Streuintervall
    // der Phase — also „wird hier ERSTMALS fällig". Der Motor prüft aber
    // `due_at <= now` — also „ist hier BEANSPRUCHBAR". Beides fällt auseinander,
    // sobald ein Fenster NACH einer Phase liegt.
    //
    // Die Entscheidung über den Start trifft ab jetzt `funktionstest-faelligkeit`,
    // datums- und kohortengenau über den ECHTEN Planer. Die Schnittmengenrechnung
    // bleibt als beschreibende Ausgabe erhalten (`arbeitsklassenImFenster` im
    // Rückgabeobjekt), ist aber KEINE Startentscheidung mehr.
    { name: "Die geforderte Kohortenabdeckung ist im Startfenster nach FÄLLIGKEIT erreicht",
      ok: faelligkeit.ok, detail: faelligkeit.detail },
    { name: `Ein vollständiger Zyklus passt in das geprüfte Startfenster (${stufenMandate} Mandate)`,
      ok: zyklus.bewertbar === true && zyklus.passt === true,
      detail: (zyklus.bewertbar === true
        ? zyklus.meldung
        : `Nicht bewertbar: ${zyklus.grund} — fail closed.`)
        + (parallelErklaert > 1 && parallelitaetBelegt !== true
          ? ` · HINWEIS: erklärte Parallelität ${parallelErklaert} ist NICHT belegt, `
  ...
  return Object.freeze({
    ...
    faelligkeitsBefund: faelligkeit.befund,
