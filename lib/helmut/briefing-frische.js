...
// Liegt der GANZE Berliner Kalendertag in [beginn, ende] (beide Grenzen in ms)?
// Fail closed: unklare oder ungueltige Angaben -> false. Ein angebrochener Randtag,
// ein heute noch laufender Tag und jeder kuenftige Tag werden verworfen. Die
// Berliner Zonenhelfer bilden Sommer-/Winterzeit korrekt ab (DST-fest).
function berlinTagVollImFenster(tagKey, beginnMs, endeMs) {
  if (!Number.isFinite(beginnMs) || !Number.isFinite(endeMs)) return false;
  const start = berlinZeitpunkt(tagKey, 0, 0);
  const folge = berlinTagVerschieben(tagKey, 1);
  const tagesende = folge ? berlinZeitpunkt(folge, 0, 0) : null;
  // Date.UTC rollt unmoegliche Kalendertage (z. B. 2026-02-30) weiter; der
  // zurueckgerechnete Berliner Tag muss deshalb exakt der Angabe entsprechen.
  if (!start || !tagesende || berlinTagKey(start) !== tagKey) return false;
  return start.getTime() >= beginnMs && tagesende.getTime() <= endeMs;
}
