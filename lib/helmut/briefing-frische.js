...
// dritte ist der Beweis der Stabilitaet. An der Sommerzeit-Luecke (02:00–03:00
// im Maerz existiert nicht) liefert das Verfahren den Zeitpunkt, an dem die Uhr
// weiterspringt — es erfindet keine nicht existierende Ortszeit.
...
function ordneMeldungen(items = [], fenster = {}, jetzt = new Date(), env = process.env) {
  ...
  const angereichert = liste.map((item) => {
    ...
    const stempel = item.lastUpdated || item.publishedAt || item.published_at || item.updatedAt || item.createdAt || null;
