...
async function sendeMailpit(nachricht = {}, opts = {}) {
  ...
  // ZWEITE, UNABHAENGIGE RIEGEL-LAGE — UNMITTELBAR AN DER NETZGRENZE
  // (ergaenzt 02.09., adversarialer Review). `sendeMail` ist der vorgesehene Weg
  // und prueft den Riegel; diese Funktion ist aber EXPORTIERT und damit direkt
  // aufrufbar. Die Sperre haengt sonst allein an der Disziplin kuenftiger
  // Aufrufer — dieselbe Ueberlegung, die `push.js` schon zwei Lagen gegeben hat.
  //
  // SIE SITZT BEWUSST HIER UND NICHT AM FUNKTIONSANFANG: davor stehen die
  // Absender-/Empfaengerpruefungen mit ihren eigenen, praezisen Fehlergruenden.
  // Eine Lage davor haette diese Diagnosen ueberschrieben und damit das Verhalten
  // des ECHTEN Mailwegs geaendert — genau die Nebenwirkung, die schon einmal vier
  // gepinnte Mailvertraege gebrochen hat (Beleg §14). Der Zweck dieser Lage ist
  // der NETZAUFRUF, und der steht erst hier bevor.
  const riegelNetzgrenze = kommunikationsriegel.pruefe({
    kanal: opts.kanal === "einladung" ? "einladung" : "mail",
    kennung: opts.kennung || nachricht.kennung || "",
    empfaenger: nachricht.to
  }, env);
  if (!riegelNetzgrenze.erlaubt) {
    return { sent: false, gesperrt: true, reason: riegelNetzgrenze.grund, riegel: riegelNetzgrenze };
  }
