...
function bewerteLauf({
  ...
  if (!k || typeof k !== "object") {
    ...
    if (zusammensetzung) {
      ...
      const dipDokumente = alsZahl(zusammensetzung.dipDokumente);
      const mehrfachHerkunft = alsZahl(zusammensetzung.mehrfachHerkunft);
      if (statisch === null || dokumentgetrieben === null || zUnbekannt === null
        || statischMoeglich === null || dipDokumente === null || mehrfachHerkunft === null) {
      ...
      } else if (zUnbekannt !== (Number(k.unbekannt) || 0)) {
        // Der Producer schreibt in BEIDE Felder denselben Wert — eine Abweichung ist eine
        // manipulierte oder kaputte Telemetrie (Review-Befund: Widerspruch bestand vorher).
        befunde.push(befund(AUSGANG_NICHT_BESTANDEN, "kontextzusammensetzung-inkonsistent",
          `${slotName}: zusammensetzung.unbekannt=${zUnbekannt} != kontext.unbekannt=${k.unbekannt}`));
      ...
      } else if (dokumentgetrieben > dipDokumente + mehrfachHerkunft) {
        // MECHANISMUS-GRENZE (Review-Befund: die Selbstauskunft war sonst tautologisch):
        // ein dokumentgetriebener Kontext braucht mindestens EIN Dokument mit je-Dokument-
        // Sichtbarkeit (DIP) oder Mehrfachherkunft, und jedes Dokument liegt in genau einem
        // Kontext (Partition). Mehr dokumentgetriebene Kontexte als solcher Dokumente sind
        // mechanisch unmoeglich — ein Fragmentierungs-Bug kann sich so nicht selbst erklaeren.
        befunde.push(befund(AUSGANG_NICHT_BESTANDEN, "kontextzusammensetzung-unplausibel",
          `${slotName}: ${dokumentgetrieben} dokumentgetriebene Kontexte, aber nur`
          + ` ${dipDokumente} DIP- + ${mehrfachHerkunft} Mehrfachherkunft-Dokumente —`
          + " mechanisch unmoeglich (je Kontext mindestens ein solches Dokument)."));
  ...
  if (status === "fehlgeschlagen" || status === "offen") {
    ...
    if (!lazy || !eager
      ...
      if (werte.some((v) => v === null || v < 0)) {
        ...
        // ANDERE Skips (Sperre, deaktivierter Store, Eager-Fehler): die Dokumente dieser
        // Stapel erreichen WEDER die Verstehens- noch die Abschluss-Vormerkung und fehlen
        // in jeder Bilanzzahl — ihr Rueckstand ist nachweislich nicht dauerhaft
        // (Review-Befund: die Alt-Regel prueft andereSkips, der Bilanz-Zweig tat es nicht).
        ...
        befunde.push(befund(AUSGANG_NICHT_BESTANDEN, "rueckstand-nicht-dauerhaft",
          ...
          + ` eager speicherfehler=${eager.vormerkFehlgeschlagen || 0}, eager andereSkips=${eager.andereSkips || 0},`
