...
function bewerteNachweisfenster(eingaben = {}) {
  ...
  if (!startbaseline || !Array.isArray(startbaseline.mandate) || !startbaseline.mandate.length) {
    befunde.push(befund(AUSGANG_BLOCKIERT, "startbaseline-fehlt",
  ...
  const frozen = mandatsSignatur(startbaseline.mandate);
  if (startbaseline.signatur && startbaseline.signatur !== frozen.signatur) {
    befunde.push(befund(AUSGANG_BLOCKIERT, "startbaseline-signatur-passt-nicht",
      `Die Startbaseline traegt ${startbaseline.signatur}, ihre Mandatsliste ergibt aber`
      + ` ${frozen.signatur} — die Baseline wurde nachtraeglich veraendert.`));
    return ergebnis(AUSGANG_BLOCKIERT);
  }
  const baselineAktivierungMs = alsZahl(startbaseline.aktivierungAtMs);
  if (baselineAktivierungMs !== null && baselineAktivierungMs !== Number(aktivierungAtMs)) {
    befunde.push(befund(AUSGANG_BLOCKIERT, "startbaseline-fremde-aktivierung",
      `Die Startbaseline gehoert zur Aktivierung ${new Date(baselineAktivierungMs).toISOString()},`
      + ` bewertet wird ${new Date(Number(aktivierungAtMs)).toISOString()}.`));
    return ergebnis(AUSGANG_BLOCKIERT);
  }
  const baselineErhobenMs = alsZahl(startbaseline.erhobenAtMs);
  if (baselineErhobenMs !== null && baselineErhobenMs > fenster.vonMs + SLOT_TOLERANZ_MS) {
    befunde.push(befund(AUSGANG_BLOCKIERT, "startbaseline-zu-spaet-erhoben",
      `Die Startbaseline wurde ${new Date(baselineErhobenMs).toISOString()} erhoben,`
      + ` also deutlich nach dem Fensterstart ${new Date(fenster.vonMs).toISOString()} —`
