...
// ---------------------------------------------------------------------------------------------
// Commit-Beleg (Review 4 zu PR #222) — strikt und fail closed.
//
// BEFUND, der zu dieser Fassung fuehrte: die fruehere Pruefung im CLI verglich nur Laengen
// und `startsWith`. Damit bestaetigte eine VOLLSTAENDIGE, gueltige SHA mit ANGEHAENGTEM
// UNSINN den Stand — denn `soll.startsWith(beobachtet)` war wahr, sobald der beobachtete
// Wert ein Praefix des (laengeren, kaputten) erwarteten Werts war. Reproduziert:
//   erwartet = "<40-stellige SHA>-VOELLIGER-UNSINN"  ->  bestaetigt: true
//
// Jetzt gilt: BEIDE Werte muessen nach `trim` und Kleinschreibung fuer sich genommen
// gueltige Git-SHA-Werte oder -Praefixe sein (7..40 Hexziffern). Erst danach wird
// verglichen, und eine Kurzform zaehlt nur als ECHTES Praefix der laengeren Form.
// Damit kann kein Wert bestaetigt werden, der nicht selbst eine gueltige SHA ist.
// ---------------------------------------------------------------------------------------------

// Untergrenze: 7 Hexziffern (die uebliche Git-Kurzform). Obergrenze: 40 (SHA-1 in voller
// Laenge). Beides inklusiv.
...
// Prueft einen AUSDRUECKLICH uebergebenen erwarteten Deployment-Commit gegen den zuletzt
// beobachteten Prozess-Commit. Rueckgabe:
//   { uebergeben, bestaetigt, erwartet, beobachtet, grund? }
// `uebergeben: false` heisst: der Betreiber hat keinen erwarteten Commit genannt — dann
// wird auch NICHTS behauptet (`bestaetigt: false`, kein Fehler). Jede andere Abweichung
// ist fail closed.
function pruefeCommitBeleg({ erwartet = null, beobachtet = null } = {}) {
  const erwartetRohLeer = erwartet == null
    || (typeof erwartet === "string" && erwartet.trim() === "");
  if (erwartetRohLeer) {
    return { uebergeben: false, bestaetigt: false, erwartet: null, beobachtet: normalisiereCommit(beobachtet) };
  }
  const soll = normalisiereCommit(erwartet);
  if (soll === null) {
    return {
      uebergeben: true, bestaetigt: false, erwartet: null, beobachtet: normalisiereCommit(beobachtet),
      grund: `Der erwartete Commit ist kein gueltiger Git-SHA-Wert (${COMMIT_MIN_LAENGE}–${COMMIT_MAX_LAENGE} Hexziffern).`
    };
  }
  const ist = normalisiereCommit(beobachtet);
  if (ist === null) {
    return {
      uebergeben: true, bestaetigt: false, erwartet: soll, beobachtet: null,
      grund: beobachtet == null || beobachtet === ""
        ? "Es gibt keinen gespeicherten Prozess-Commit — der erwartete Stand ist nicht belegbar."
        : "Der beobachtete Prozess-Commit ist kein gueltiger Git-SHA-Wert — nicht belegbar."
    };
  }
  const gleich = soll === ist || istEchtesPraefix(soll, ist) || istEchtesPraefix(ist, soll);
  return gleich
    ? { uebergeben: true, bestaetigt: true, erwartet: soll, beobachtet: ist }
    : {
      uebergeben: true, bestaetigt: false, erwartet: soll, beobachtet: ist,
      grund: `Zuletzt beobachteter Prozess-Commit ${ist} weicht vom erwarteten ${soll} ab.`
    };
}
...
function pruefeStartbaseline({ roh = null, aktivierungAtMs = null, jetztMs = null, toleranzMs = BASELINE_TOLERANZ_MS } = {}) {
