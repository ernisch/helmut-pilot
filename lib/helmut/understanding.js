...
function defaultDeps() {
  ...
  return {
    ...
    // Shadow-Telemetrie-Senke: Default = reines Console-Log der AGGREGAT-Kennzahlen (kein
    // Production-Write, kein Rohtext). Wird bei echter Shadow-Messung durch eine isolierte Ablage
    // ersetzt (injizierbar). Fail-safe.
...
async function runUnderstandingShadow(rawDocsOrItems = [], overrides = {}) {
  ...
  try {
    ...
    if (gateMode !== "off") {
      try {
        const perCluster = clusters.map((c) => understandingGate.assessCluster(c.documents || [], { now: Date.now() }));
        const entscheidungen = perCluster.reduce((m, r) => { m[r.decision] = (m[r.decision] || 0) + 1; return m; }, {});
