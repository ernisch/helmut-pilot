...
async function understandOneCluster(cluster, deps, opts = {}) {
  ...
  const quellenbeleg = akteurslistenQuellenbindung.pruefeAkteurslistenQuellenbindung(aiResult, modellPrompt);
  validation.valid = validation.valid && quellenbeleg.valid;
  validation.errors = [...quellenbeleg.errors, ...validation.errors];
  if (!validation.valid) {
    await markFailed();
    deps.logSkip("skipped-understanding-invalid");
