...
async function persistiereVerstehensergebnis({ vertrag, reservierung, ko, deps }) {
  ...
  if (!cas.pruefbar && !cas.gespeichert) {
    // Nur wiederholen, wenn bis zur Deadline des Aufrufers noch ein kompletter
    // Speicherversuch + Abschluss passt — sonst bleibt es beim ehrlichen ersten Befund.
    const deadline = Number(deps.deadlineMs) > 0 ? Number(deps.deadlineMs) : 0;
    const zeitReicht = deadline === 0 || (deadline - Date.now())
      >= (verstehenRestzeit.speicherTimeoutMs() + verstehenRestzeit.ABSCHLUSS_RESERVE_MS);
    if (zeitReicht) {
      try {
...
async function understandOneCluster(cluster, deps, opts = {}) {
  ...
  // ── RESTZEITWACHE, ZWEITE PRUEFUNG — unmittelbar VOR dem Modellstart (§29) ─────────
  // Zwischen der ersten Pruefung und hier liegen Reservierung, Budget-Gate und
  // Konkurrenzwache (je ein Supabase-Roundtrip, im Stoerfall bis ~10 s). Reicht die
  // Zeit JETZT nicht mehr, wird NICHT aufgerufen: kein Modellstart-Vermerk, kein
  // Budgetverbrauch; das finally gibt die Reservierung regulaer frei („kein Ergebnis
  // vor Modellstart" — sicher wiederholbar). Der bereits gezaehlte Versuch dieser
  // Reservierung ist der bewusst akzeptierte Preis dieses seltenen Pfads — die
...
async function understandUpdate(cluster, deps, ctx = {}) {
  ...
  // ── RESTZEITWACHE, ZWEITE PRUEFUNG — unmittelbar VOR dem Modellstart (§29) ─────────
  // Wie im Erstverstehen: reicht die Zeit JETZT nicht mehr, wird nicht aufgerufen;
  // das finally gibt die Reservierung regulaer frei (vor dem Modellstart sicher
...
async function runUnderstandingShadow(rawDocsOrItems = [], overrides = {}) {
  ...
  try {
    ...
    const restzeitWeg = () => deadlineMs !== 0
      && !verstehenRestzeit.restzeitEntscheidung({ deadlineMs }).erlaubt;
