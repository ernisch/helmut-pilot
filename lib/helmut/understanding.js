...
async function parkeDurchGate({ vorgangId, existing, gate, clusterDocs, deps, verknuepfe }) {
  ...
  // 3) VERKNUEPFUNG: die Dokumente haengen nachweisbar am geparkten Vorgang —
  //    Endzustand fuer den Watchdog UND Grundlage der spaeteren Neubewertung.
  await verknuepfe(koId, clusterDocs);
...
async function understandOneCluster(cluster, deps, opts = {}) {
  ...
  if (gateModusArm === "on" && !opts.wiederaufnahmeFreigabe && clusterDocs.length) {
    ...
    if (gate && Array.isArray(gate.perDoc) && gate.perDoc.length && gate.decision === "parken") {
      ...
      const parkung = await parkeDurchGate({ vorgangId, existing, gate, clusterDocs, deps, verknuepfe });
      if (parkung.ok === true) {
