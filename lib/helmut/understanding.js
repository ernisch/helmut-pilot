...
function buildUnderstandingPrompt(cluster = {}) {
  ...
  return [
    ...
    "- why_relevant: 1 Satz, AUSSCHLIESSLICH warum der Vorgang politisch relevant ist (z. B. betroffener",
    "  Ausschuss/Partei/Politikfeld aus diesem Vorgang). Nicht allgemein bleiben, nicht den Titel umformulieren.",
    "  Beispiele: 'Betrifft den Gesundheitsausschuss direkt.' / 'Kann Auswirkungen auf die Ausschussarbeit haben.'",
    "- recommendation: 1 kurzer, konkreter, handlungsorientierter naechster Schritt NUR fuer DIESEN Vorgang",
    "  (keine Tagesstrategie, keine Gesamtpriorisierung, keine Statement-Formulierung).",
    "  Schlecht (generisch): 'Monitoring durchfuehren.' / 'Thema beobachten.'",
    "  Gut (konkret): 'Heute nicht oeffentlich reagieren.' / 'Position mit Referenten abstimmen.' / 'Bis Freitag beobachten.'",
    ...
    "- risk_of_no_action: 1-2 Saetze, was politisch oder kommunikativ passiert, wenn auf DIESEN",
    "  Vorgang NICHT reagiert wird (konkret auf den Vorgang bezogen, kein Allgemeinplatz). Sonst ''.",
    "- opportunity_summary: 1-2 Saetze, die erkennbare politische oder kommunikative CHANCE dieses",
    "  Vorgangs. Ist keine erkennbar: ''.",
    "- recommended_communication: 1 Satz, die empfohlene Kommunikationslinie ODER das empfohlene",
    "  Kommunikationsformat (z. B. 'Kurzstatement im Ausschuss', 'zunaechst nicht oeffentlich",
    "  reagieren, intern abstimmen'). Nicht ableitbar: ''.",
    "- action_items: 2-5 konkrete naechste Schritte als Liste kurzer, handlungsorientierter",
    "  Stichpunkte (jeder Punkt ein eigener String). Keine sinnvollen Schritte ableitbar: [].",
    "",
    ...
    "- recommended_communication_struct: Objekt mit:",
    "    communicationLine  = kurze empfohlene Kommunikationslinie (Satz), sonst ''.",
    "    recommendedChannel = GENAU einer von {press, social, internal, parliamentary, none, unknown}.",
    "    recommendedFormat  = GENAU einer von {statement, pressRelease, qa, socialPost, internalLine, none, unknown}.",
    "    suggestedOutputs   = Liste kurzer Format-Labels (z. B. statement, qa, pressRelease, talkingPoints, socialPost), sonst [].",
    "- action_items_struct: Liste strukturierter naechster Schritte (2-5), jedes Element ein Objekt mit:",
    "    title       = kurzer Handlungstitel (Pflicht je Eintrag; ohne title -> Eintrag weglassen).",
    "    description = 1 Satz Kontext, sonst ''.",
    "    dueHint     = grober Zeit-/Faelligkeitshinweis (z. B. 'heute', 'diese Woche'), sonst ''.",
