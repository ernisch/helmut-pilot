      ...
      if (!neueDocs.length) {
        ...
        if (vormerkung.vorhanden) {
          ...
          return understandUpdate(cluster, deps, {
            ...
            retriesCtx: opts.retriesCtx, wiederaufnahme: true, vertrag, ...kontextOption
          });
        }
        // §30: AUSDRUECKLICHE BETREIBERFREIGABE. Ohne sie ist `duplicate` hier richtig —
        // dieselbe Dokumentmenge ist bereits verstanden. Hat ein Betreiber den Vorgang
        // aber per kanonischem Weg `erneut` freigegeben, IST das die Entscheidung, dass
        // ein weiterer Aufruf stattfinden soll (der Anlass sind gescheiterte, nicht
        // fehlende Aufrufe). Der Ablauf bleibt der regulaere Aktualisierungspfad: volle
        // CAS-Reservierung, Vorgangswache, Budget-Gate, Restzeitwache, Versuchszaehler
        // und Wiederaufnahmedeckel gelten unveraendert — es wird KEINE Grenze angehoben.
        if (opts.wiederaufnahmeFreigabe) {
          const bestandsDocsFreigabe = aufloesung.bestandsDokumente || [];
          return understandUpdate(cluster, deps, {
            vorgangId, existing, neueDocs: [], spur, neueAnker: [],
            alleDocs: bestandsDocsFreigabe.length ? bestandsDocsFreigabe : clusterDocs,
            retriesCtx: opts.retriesCtx, wiederaufnahme: true, vertrag,
            deadlineMs: opts.deadlineMs, ...kontextOption
          });
        }
