...
function assembleKnowledgeObject(aiResult = {}, cluster = {}, vorgangId, opts = {}) {
  ...
  // --- Embedding write-time persistieren (Sprint 2) ---------------------------
  // Deterministischer Merkmalsvektor (KEINE KI, kein Netz) -> einmal global erzeugen
  // und speichern, statt bei jeder Anfrage read-time neu zu rechnen (matching_results=0).
  try {
    const { embedKnowledgeObject } = require("./matching");
    const vec = embedKnowledgeObject(ko);
    if (Array.isArray(vec) && vec.length) ko.embedding = vec;
  } catch (_) { /* Embedding optional; Pipeline nie blockieren */ }
