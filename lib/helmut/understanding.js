...
const { clustersInPriorityOrder } = require("./quellenarchitektur/understanding-priority");
...
async function understandOneCluster(cluster, deps, opts = {}) {
  ...
  if (existing) {
    ...
    // erneut verstanden werden — auch nicht, wenn spaeter wieder ein Cluster
    // mit passender vorgang_id auftaucht ("nie wieder"-Garantie, kein KI-Call).
    if (isTerminalUnderstandingStatus(existing.understanding_status)) {
      return { vorgangId, status: "skipped-terminal" };
    }
  }
  ...
  try {
    ...
    if (error && error.code === "LLM_BUDGET_EXHAUSTED") {
      return { vorgangId, status: "skipped-budget", reason: error.reason || "daily-llm-budget-reached" };
    }
    await markFailed();
