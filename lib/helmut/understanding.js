...
async function understandOneCluster(cluster, deps, opts = {}) {
  ...
  //      Ministeriumslisten auf den woertlich belegten Teil und entfernt einen unbelegten Wert aus
  //      der Beteiligungsliste `parteien` — LETZTERES nur, wenn die Antwort nachweislich NICHT von
  //      diesem Wert abhaengt (der Wert kommt in keinem anderen Feld vor). Andernfalls bleibt
  //      `parteien` unveraendert und die Antwort wird abgewiesen.
  // Die Beteiligungsliste `ausschuesse`, das Schema und der decision_level-Konflikt laufen danach
  // unveraendert fail closed; eine weiterhin ungueltige Antwort wird abgelehnt.
...
async function understandUpdate(cluster, deps, ctx = {}) {
  ...
  // `parteien` wird nur reduziert, wenn die Antwort nachweislich nicht von dem unbelegten Wert
  // abhaengt; sonst bleibt sie unveraendert (fail closed). Alle uebrigen Pruefungen laufen danach
  // unveraendert streng.
  aiResult = akteurslistenQuellenbindung.ohneFalschTypisierteAkteure(aiResult, modellPrompt);
  aiResult = akteurslistenQuellenbindung.ohneUnbelegteAkteurswerte(aiResult, modellPrompt);
