...
async function understandOneCluster(cluster, deps, opts = {}) {
  ...
  // OPTIONALE MINISTERIUMSLISTEN (Production-Befund 2026-09-23): `ministerien` und
  // `mentioned_ministries` sind reine Erwaehnungen. Unbelegte Werte werden hier
  // deterministisch entfernt, statt die gesamte — fachlich brauchbare — Antwort zu
  // verwerfen; woertlich belegte Werte bleiben unveraendert und unbelegte werden NIE
  // gespeichert. Die Quellenbindung selbst bleibt unveraendert streng: die Reduktion benutzt
  // exakt denselben Beleg, und alle uebrigen Felder (Parteien, Personen, Ausschuesse, Schema,
  // decision_level) laufen danach unveraendert fail closed.
...
async function understandUpdate(cluster, deps, ctx = {}) {
  ...
  // Reduktion wie im Erstverstehen — unbelegte Erwaehnungen entfallen, belegte bleiben; alle
  // uebrigen Pruefungen laufen danach unveraendert streng.
  aiResult = akteurslistenQuellenbindung.ohneUnbelegteMinisterien(aiResult, modellPrompt);
