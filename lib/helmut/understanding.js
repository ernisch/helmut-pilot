...
function defaultDeps(ctx = {}) {
  ...
  return {
    ...
    logSkip: (callType) => {
      try { storage.recordLlmUsage({ callType, model: "none", politicianId: null, success: false, runId }); }
...
async function resolveVorgang(cluster, deps, opts = {}) {
  ...
  if (opts.vorgangId) {
    ...
    return { vorgangId: opts.vorgangId, existing, bestandsDokumente, resolution: "vorgegeben", begruendung: "kennung-vorgegeben" };
  }
  ...
  for (const kandidat of sortiert) {
    ...
    if (urteil.gleich) {
      return {
        ...
        resolution: "bestand", begruendung: `${urteil.beleg}:${urteil.grund}`, vorschlag
      };
    }
  }
  ...
  if (!belegt.has(vorschlag)) {
    return { vorgangId: vorschlag, existing: null, bestandsDokumente: [], resolution: "neu", begruendung: "kein-bestand", vorschlag };
  }
  for (let n = 2; n <= MAX_KONFLIKT_SUFFIX; n += 1) {
    ...
    if (!belegt.has(alternativ)) {
      return {
        ...
        resolution: "konflikt-neue-kennung", begruendung: `kennung-belegt:${vorschlag}`, vorschlag
      };
    }
  }
  return {
    vorgangId: `${vorschlag}-x`, existing: null, bestandsDokumente: [],
    resolution: "konflikt-neue-kennung", begruendung: "kennungsraum-erschoepft", vorschlag
  };
}
...
async function understandOneCluster(cluster, deps, opts = {}) {
  ...
  const spur = { resolution: aufloesung.resolution, begruendung: aufloesung.begruendung };
  ...
  if (existing) {
    ...
    if (existing.status !== "pending") {
      ...
      return understandUpdate(cluster, deps, {
        ...
        alleDocs: [...bestandsDocs, ...neueDocs]
      });
    }
...
async function understandUpdate(cluster, deps, ctx = {}) {
  const { vorgangId, existing, neueDocs, spur, neueAnker } = ctx;
  const model = typeof deps.modelName === "function" ? deps.modelName() : null;
  ...
  if (!budget || !budget.allowed) {
    ...
    return {
      ...
      documents: neueDocs.length, ...spur, begruendung: "aktualisierung-vertagt", neueAnker
    };
  }
  ...
  try {
    ...
    // Nur die Aktualisierung ist gescheitert — sie wird erneut versucht, sobald
    // erneut neue Dokumente eintreffen oder der Nachholpfad sie aufgreift.
    const errorCode = error && error.message ? String(error.message).slice(0, 120) : "unknown";
    console.error("[understanding] update-error:", vorgangId, errorCode);
    deps.logSkip("skipped-understanding-error");
    return { vorgangId, status: "skipped-error", errorCode, documents: neueDocs.length, ...spur, begruendung: "aktualisierung-fehlgeschlagen" };
  }
  ...
  const ko = assembleKnowledgeObject(aiResult, vollCluster, vorgangId, {
    understanding_status: "complete", model, koVersion: naechsteVersion
  });
  ...
  if (!validation.valid) {
    ...
    deps.logSkip("skipped-understanding-invalid");
    return { vorgangId, status: "skipped-invalid", errors: validation.errors.slice(0, 5), documents: neueDocs.length, ...spur, begruendung: "aktualisierung-ungueltig" };
  }
...
//   erneut          skipped-budget/-store + zurueckgestellte Cluster
...
async function runUnderstandingShadow(rawDocsOrItems = [], overrides = {}) {
  ...
  try {
    ...
    for (let i = 0; i < orderedClusters.length; i += 1) {
      ...
      try {
        results.push(await understandOneCluster(orderedClusters[i], deps)); // seriell: schont Budget/Rate-Limit
      } catch (error) {
    ...
    if (nichtVorgemerkt > 0) {
      try {
        console.log(`[understanding] Vormerk-Zeitbudget erschoepft: ${vorgemerkt} vorgemerkt, ${nichtVorgemerkt} bleiben unvorgemerkt zurueckgestellt`);
      } catch (_) { /* ignore */ }
    }
    return {
      clusters: clusters.length, processed: ran, deferred, vorgemerkt, nichtVorgemerkt, counts, results,
...
async function runPendingUnderstandingShadow(rawDocsOrItems = [], overrides = {}) {
  ...
  try {
    ...
    for (const ko of pending) {
      ...
      try {
        const r = await understandOneCluster(cluster, deps, { vorgangId: ko.vorgang_id, existing: ko });
