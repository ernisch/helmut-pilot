...
async function understandOneCluster(cluster, deps, opts = {}) {
  ...
  //   2) `ohneUnbelegteAkteurswerte` reduziert die REDUZIERBAREN Akteurslisten (die
  //      Erwaehnungslisten `mentioned_*` und die beiden Ministeriumslisten) auf den woertlich
  //      belegten Teil.
  // Die BETEILIGUNGSLISTEN parteien/ausschuesse, das Schema und der decision_level-Konflikt
  // laufen danach unveraendert fail closed; eine weiterhin ungueltige Antwort wird abgelehnt.
...
async function understandUpdate(cluster, deps, ctx = {}) {
  ...
  // (alle Listen), unbelegte Werte der Erwaehnungs-/Ministeriumslisten entfallen, belegte bleiben;
  // alle uebrigen Pruefungen laufen danach unveraendert streng.
  aiResult = akteurslistenQuellenbindung.ohneFalschTypisierteAkteure(aiResult, modellPrompt);
  aiResult = akteurslistenQuellenbindung.ohneUnbelegteAkteurswerte(aiResult, modellPrompt);
