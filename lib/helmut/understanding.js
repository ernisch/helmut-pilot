...
function buildUnderstandingPrompt(cluster = {}) {
  ...
  return [
    ...
    "- display_title: kurze STANDALONE Ueberschrift fuer die Oberflaeche, ca. 45 Zeichen (max. 70).",
    "  Muss ein vollstaendiger, in sich verstaendlicher Gedanke sein -- NIEMALS ein abgebrochener",
    "  Satzteil, keine Ellipsen/'...'. Darf frei umformuliert werden (muss nicht der Originaltitel sein).",
    "  Schlecht (abgebrochen): 'Friedrich Merz hat oeffentlich' / 'Reform soll Extremisten'",
    "  Gut (vollstaendig): 'Merz lehnt Vergesellschaftung ab' / 'Reform fuer Schoeffen geplant'",
...
// display_title darf NIE mitten im Wort/Satz gekappt werden (das war die
// Ursache abgebrochener Ueberschriften wie "Friedrich Merz hat oeffentlich").
// Ein zu langer Wert wird deshalb VERWORFEN statt abgeschnitten -- der Client
// faellt dann sauber auf den vollstaendigen Originaltitel zurueck (kleinere
// Schrift), nie auf ein abgebrochenes Fragment.
const DISPLAY_TITLE_MAX = 70; // Kulanz ueber dem ~45-Zeichen-Zielwert aus dem Prompt.
const DISPLAY_CATEGORY_MAX = 26;
function sanitizeDisplayTitle(value) {
  const v = String(value == null ? "" : value).replace(/\s+/g, " ").trim();
  return v && v.length <= DISPLAY_TITLE_MAX ? v : "";
}
function sanitizeDisplayCategory(value) {
  const v = String(value == null ? "" : value).replace(/\s+/g, " ").trim();
  return v && v.length <= DISPLAY_CATEGORY_MAX ? v : "";
}
...
function assembleKnowledgeObject(aiResult = {}, cluster = {}, vorgangId, opts = {}) {
  ...
  ko.display_summary = cleanEntry(aiResult.display_summary, 320);
  ko.why_relevant = cleanEntry(aiResult.why_relevant, 220);
  ko.recommendation = cleanEntry(aiResult.recommendation, 160);
