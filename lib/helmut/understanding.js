...
async function understandOneCluster(cluster, deps, opts = {}) {
  ...
  //   2) `ohneUnbelegteMinisterien` reduziert die beiden optionalen Ministeriumslisten auf den
  //      woertlich belegten Teil.
  // UNBELEGTE Werte in Parteien/Personen/Ausschuessen, das Schema und der decision_level-Konflikt
  // laufen danach unveraendert fail closed.
  aiResult = akteurslistenQuellenbindung.ohneFalschTypisierteAkteure(aiResult, modellPrompt);
  aiResult = akteurslistenQuellenbindung.ohneUnbelegteMinisterien(aiResult, modellPrompt);
...
async function understandUpdate(cluster, deps, ctx = {}) {
  ...
  // OPTIONALE MINISTERIUMSLISTEN (Production-Befund 2026-09-23): dieselbe deterministische
  // Reduktion wie im Erstverstehen — woertlich belegte, aber falsch typisierte Akteurswerte entfallen
  // (alle Listen), unbelegte Ministerienwerte entfallen, belegte bleiben; alle uebrigen Pruefungen
  // laufen danach unveraendert streng.
  aiResult = akteurslistenQuellenbindung.ohneFalschTypisierteAkteure(aiResult, modellPrompt);
  aiResult = akteurslistenQuellenbindung.ohneUnbelegteMinisterien(aiResult, modellPrompt);
