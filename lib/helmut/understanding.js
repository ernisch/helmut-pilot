...
async function understandOneCluster(cluster, deps, opts = {}) {
  ...
  // wird nie automatisch wiederholt. Ist der Vermerk nicht setzbar (Lease verloren,
  // Vertrag nicht pruefbar), wird NICHT aufgerufen: fail closed.
  if (vertrag && reservierung) {
    const start = await vertrag.modellstart({ vorgangId, fencing: reservierung.fencing });
    if (!start.erlaubt) {
      abgeschlossen = true;   // nichts freizugeben: das Lease gehoert uns nicht mehr
  ...
  try {
    ...
    return { vorgangId, status: "skipped-error", errorCode, ...spur };
  }
  ...
  if (!validation.valid) {
    await markFailed();
    deps.logSkip("skipped-understanding-invalid");
    return { vorgangId, status: "skipped-invalid", errors: validation.errors.slice(0, 5), ...spur };
  }
  ...
  if (vertrag && reservierung) {
    ...
    if (!recht.erlaubt) {
      abgeschlossen = true;
      return { vorgangId, status: "skipped-veraltet", reason: recht.grund || "lease-verloren", ...spur, begruendung: "schreibrecht-verloren" };
    }
    ko.verstehen_fencing = reservierung.fencing;
  }
  ...
  if (saved && saved.veraltet) {
    abgeschlossen = true;
    return { vorgangId, status: "skipped-veraltet", reason: "fencing-abgewehrt", ...spur, begruendung: "datenbank-hat-abgewehrt" };
  }
  ...
  if (vertrag && reservierung && saved && saved.saved) {
    // ABSCHLUSS: erst jetzt gilt der Vorgang als fertig — und zwar mit dem BELEG des
    // persistierten Ergebnisses (CLAUDE.md §4.10: eine Meldung behauptet nur, was die
    // Ablage traegt).
    const ende = await vertrag.abschluss({
      vorgangId, fencing: reservierung.fencing, ergebnisHash: verstehenVertragModul.ergebnisHash(ko)
    });
    abgeschlossen = true;
    if (!ende.ok) {
      return { vorgangId, status: "skipped-veraltet", reason: ende.grund || "abschluss-abgelehnt", ...spur, begruendung: "abschluss-uebernommen" };
    }
  }
  return {
    vorgangId,
  ...
  // sichtbar in `unbekannt`, statt einen zweiten bezahlten Aufruf auszuloesen.
    ...
    if (vertrag && reservierung && !abgeschlossen) {
      await Promise.resolve(vertrag.freigabe({
        vorgangId, fencing: reservierung.fencing, grund: "kein-ergebnis-bekannter-ausgang"
      })).catch(() => {});
    }
  }
}
...
async function understandUpdate(cluster, deps, ctx = {}) {
  ...
  // MODELLSTART VERMERKEN (OP-30 CAS) — siehe understandOneCluster. Fail closed.
  if (vertrag && reservierung) {
    const start = await vertrag.modellstart({ vorgangId, fencing: reservierung.fencing });
    if (!start.erlaubt) {
      abgeschlossen = true;
  ...
  try {
    ...
    deps.logSkip("skipped-understanding-error");
    return { vorgangId, status: "skipped-error", errorCode, documents: neueDocs.length, ...spur, begruendung: "aktualisierung-fehlgeschlagen", ...marker };
  }
  ...
  if (!aiResult || typeof aiResult !== "object" || Array.isArray(aiResult)) {
    await merkeUpdateOffen(true);
    deps.logSkip("skipped-understanding-invalid");
    return { vorgangId, status: "skipped-invalid", errors: ["ki-antwort-nicht-verwertbar"], documents: neueDocs.length, ...spur, begruendung: "aktualisierung-ungueltig", ...marker };
  }
  ...
  if (!validation.valid) {
    ...
    await merkeUpdateOffen(true);
    deps.logSkip("skipped-understanding-invalid");
    return { vorgangId, status: "skipped-invalid", errors: validation.errors.slice(0, 5), documents: neueDocs.length, ...spur, begruendung: "aktualisierung-ungueltig", ...marker };
  }
  ...
  if (vertrag && reservierung) {
    ...
    if (!recht.erlaubt) {
      ...
      return {
        ...
        documents: neueDocs.length, ...spur, begruendung: "schreibrecht-verloren", neueAnker, ...marker
      };
    }
    ko.verstehen_fencing = reservierung.fencing;
  }
  ...
  if (saved && saved.veraltet) {
    ...
    abgeschlossen = true;
    return {
      vorgangId, status: "skipped-veraltet", reason: "fencing-abgewehrt",
      documents: neueDocs.length, ...spur, begruendung: "datenbank-hat-abgewehrt", neueAnker, ...marker
    };
  }
  if (saved && saved.saved) await loeseVormerkungAuf();
  ...
  if (vertrag && reservierung && saved && saved.saved) {
    ...
    if (!ende.ok) {
      return {
        vorgangId, status: "skipped-veraltet", reason: ende.grund || "abschluss-abgelehnt",
        documents: neueDocs.length, ...spur, begruendung: "abschluss-uebernommen", neueAnker, ...marker
      };
    }
  }
  return {
    vorgangId,
    status: saved && saved.saved ? "updated" : "skipped-store",
    id: ko.id,
  ...
  // Ausdrueckliche Freigabe bei bekanntem Ausgang (siehe understandOneCluster).
  } finally {
    if (vertrag && reservierung && !abgeschlossen) {
      await Promise.resolve(vertrag.freigabe({
        vorgangId, fencing: reservierung.fencing, grund: "aktualisierung-ohne-ergebnis"
      })).catch(() => {});
    }
  }
}
