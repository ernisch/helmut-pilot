...
//
// Einzelne generische Woerter bleiben trotz ausreichender Laenge als ALLEINIGER
// Anker zu mehrdeutig ("Sanktionen" trifft sowohl Buergergeld-Sanktionen als auch
// aussenpolitische EU-Sanktionen) - zwei Dokumente, die nur dieses eine Wort teilen,
// wuerden sonst faelschlich demselben Vorgang zugeordnet. Ein spezifischerer
// Komposit-Anker (z. B. "buergergeld") bleibt fuer echte Duplikate weiterhin nutzbar.
const AMBIGUOUS_ANCHOR_STOPWORDS = new Set(["sanktion", "sanktionen", "persoenlich", "persönlich"]);

function anchorTokens(text) {
  return [...new Set(
    String(text || "").toLowerCase().replace(/[^a-z0-9äöüß]+/gi, " ").split(/\s+/)
      .filter((t) => t.length >= 8 && !AMBIGUOUS_ANCHOR_STOPWORDS.has(t))
  )];
}
