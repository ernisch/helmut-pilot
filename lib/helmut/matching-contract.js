...
// Rangfolge GESPEICHERTER Matching-Zeilen (Befund F-E2E, 2026-08-04): der
// BERECHNETE Rang entscheidet, nicht der Schreibzeitpunkt. `created_at` taugt
// dafuer nicht: alle Zeilen eines Laufs bekommen denselben Wert (ein Statement,
// ein `now()`) und bereits vorhandene Zeilen behalten beim Upsert ihren ALTEN
// Wert (helmut_publish_matching_run, Schritt 2) — die Reihenfolge waere damit
// „zuerst gesehen" statt „am relevantesten", und bei Gleichstand unbestimmt.
// Zeilen ohne lesbaren Rang (Altbestand) stehen hinten, nie vor einem belegten
// Rang; Gleichstand loest die Objektkennung byte-stabil auf. EINE Definition
// fuer den echten Lesepfad (lage.js) und das E2E-Testdoppel — kein Drift.
function storedMatchingRank(row) {
...
function compareStoredMatchingRows(a, b) {
  // Sind beide Zeilen ranglos, ist die Differenz NaN (falsy) und der
  // byte-stabile Tiebreak entscheidet.
  return (storedMatchingRank(a) - storedMatchingRank(b)) || compareIds(
