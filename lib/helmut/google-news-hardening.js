...
function googleHardeningConfig(env = process.env) {
  return {
    ...
    // Geteilte Abrufwege über Mandate hinweg (Incident 2026-07-25, siehe unten).
    // Kill-Switch: HELMUT_SHARED_PATH_DEDUP=off stellt das Alt-Verhalten her.
    sharedPathDedup: sharedPathDedupEnabled(env),
    sharedPathWindowMs: Math.floor(posNum(env.HELMUT_SHARED_PATH_WINDOW_MS, 15 * 60 * 1000))
  };
}

// ---------------------------------------------------------------------------
// Geteilte Abrufwege über Mandate hinweg (Root-Cause-Fix, Incident 2026-07-25)
// ---------------------------------------------------------------------------
// BEFUND (Production, crawl-20260725040036-uu3fk vs. crawl-20260725040353-3kste):
// 146 von 163 Abrufwegen sind Google-News-Suchen, und die große Mehrheit ist
// MANDANTENUNABHÄNGIG (gemeinsame Paket-/Planwege — identische URL, identische
// Antwort). Die Cron-Endpunkte durchlaufen alle aktiven Mandate sequenziell im
// SELBEN Prozess, also hinter DERSELBEN Egress-IP; jedes Mandat holte bisher
// dieselben ~138 Google-URLs erneut. Google drosselt pro IP, nicht pro Mandat:
// der erste Mandant lief gesund (145/145), jeder folgende lief in Timeouts/
// HTTP 503, der Breaker öffnete korrekt — und der Lauf wurde als
// "141/144 Quellen fehlgeschlagen" gemeldet, obwohl KEINE Quelle defekt war
// (alle 141 Wege waren im Lauf 3 Minuten davor erfolgreich).
//
// Warum der bestehende Schutz nicht greift: `fullCrawlMinSpacingMs` ist bewusst
// pro MANDAT gedacht (Doppel-Trigger-Schutz), die gedrosselte Ressource ist aber
// pro EGRESS-IP global. Der Degradations-Cooldown ist mandantsübergreifend, kann
// aber erst NACH einem degradierten Lauf greifen — der erste Mandant ist gesund,
// also schützt er den zweiten innerhalb desselben Durchlaufs nie.
//
// LÖSUNG (kleinste belastbare): ein prozessweites, zeitfensterbegrenztes
// Gedächtnis der bereits abgerufenen Google-Abruf-URLs. Wird eine IDENTISCHE URL
// innerhalb des Fensters erneut eingeplant, wird der Weg als 'skipped-shared'
// übersprungen — weder Erfolg noch Fehler. Das ist kein Datenverlust: der
// Dokumentenkorpus ist GLOBAL (raw_documents hat keine Mandantenspalte), die
// Dokumente liegen also bereits aus dem ersten Abruf vor. Mandantseigene Wege
// (eigene URL, z. B. die Personen-Newssuche) laufen unverändert weiter, ebenso
// ALLE direkten Quellen. Kein Quellenpaket-Eingriff, reversibel per Env.
function sharedPathDedupEnabled(env = process.env) {
  const v = String(env.HELMUT_SHARED_PATH_DEDUP || "").trim().toLowerCase();
  return !(v === "off" || v === "0" || v === "false" || v === "aus");
}

// Alle Google-News-Abruf-URLs einer Quelle als stabiler, sortierter Schlüssel.
// null = kein Google-Weg (dann greift die Dedup-Logik grundsätzlich nicht).
function sharedFetchKey(source = {}) {
  const urls = [source.rssUrl, source.url, ...(Array.isArray(source.rssUrls) ? source.rssUrls : [])]
    .map((u) => String(u || "").trim())
    .filter(isStrictGoogleNewsUrl);
  if (!urls.length) return null;
  return Array.from(new Set(urls)).sort().join("\n");
}

// Zeitfensterbegrenztes Gedächtnis abgerufener Abrufwege. Reine Logik mit
// injizierbarer Uhr (offline testbar); `maxEntries` verhindert unbegrenztes
// Wachstum in einem langlebigen warmen Container.
function createSharedFetchLedger({ windowMs = 15 * 60 * 1000, maxEntries = 1000, now = () => Date.now() } = {}) {
  const seen = new Map(); // key -> letzter Abrufzeitpunkt (ms)
  const prune = (nowMs) => {
    for (const [key, at] of seen) if (nowMs - at >= windowMs) seen.delete(key);
    while (seen.size > maxEntries) seen.delete(seen.keys().next().value);
  };
  return {
    // Wurde dieser Abrufweg innerhalb des Fensters schon abgerufen?
    seenRecently(source) {
      const key = sharedFetchKey(source);
      if (!key) return false;
      const nowMs = now();
      const at = seen.get(key);
      if (at == null) return false;
      if (nowMs - at >= windowMs) { seen.delete(key); return false; }
      return true;
    },
    // Abruf vermerken (nur sinnvoll für tatsächlich ausgeführte Abrufe).
    note(source) {
      const key = sharedFetchKey(source);
      if (!key) return false;
      const nowMs = now();
...
// Prozessweite Instanz: ein Cron-Durchlauf = eine Function-Invocation, alle
// Mandate teilen denselben Prozess und dieselbe Egress-IP. Wird bei geänderter
// Fensterbreite neu aufgebaut (Env-Änderung ohne Deploy).
let processLedger = null;
let processLedgerWindowMs = null;
function sharedFetchLedger(windowMs = 15 * 60 * 1000) {
  if (!processLedger || processLedgerWindowMs !== windowMs) {
    processLedger = createSharedFetchLedger({ windowMs });
    processLedgerWindowMs = windowMs;
  }
  return processLedger;
}

// Nur für Tests: prozessweites Gedächtnis zurücksetzen.
function resetSharedFetchLedger() {
  processLedger = null;
  processLedgerWindowMs = null;
}
...
function isGoogleNewsSource(source = {}) {
  const urls = [source.rssUrl, source.url, ...(Array.isArray(source.rssUrls) ? source.rssUrls : [])];
  return urls.some(isStrictGoogleNewsUrl);
}
