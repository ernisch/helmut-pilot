...
function erhebungsSql({ seitIso = "<STUFENBEGINN-UTC>" } = {}) {
  return [
    ...
    "  ) as \"haengendeLeases\",",
    "  -- Dubletten: derselbe Idempotenzschluessel mehr als einmal ABGESCHLOSSEN.",
    "  coalesce((",
    "    select count(*) from (",
    "      select idempotency_key from helmut_jobs",
    `      where finished_at >= '${seitIso}'`,
    ...
    "-- Quelle ist der Nutzungslog. Liegt die relationale Ablage vor",
    "-- (HELMUT_LLM_USAGE_RELATIONAL an + Migration 20260902121500), ist sie",
    "-- massgeblich; sonst helmut_store.data.llmUsage.",
    "select",
...
function baueBeobachtungen({
  ...
  setze("unbekannteModellaufrufe", zahl(modellaufrufe && modellaufrufe.unbekannteModellaufrufe));
  setze("drosselungen", zahl(modellaufrufe && modellaufrufe.drosselungen));
  ...
  setze("kommunikationsversuche", zahl(riegel && riegel.durchgelassen));
  ...
  if (startfenster && typeof startfenster === "object" && Array.isArray(startfenster.konflikte)) {
    setze("fensterKonflikte", startfenster.konflikte.length);
  }
  ...
  if (tagesplan && tagesplan.klassen && tagesplan.zuteilung) {
    ...
    if (typeof kl.realeVollstaendigBedient === "boolean" && typeof kl.real === "number") {
      ...
      const ohne = Object.entries(tagesplan.zuteilung)
        .filter(([id, z]) => !mandatsklasse.istSynthetischeKennung(id)
          && (!z || (Number(z.notwendig) || 0) < 1)).length;
      setze("realeMandateOhneZuteilung", ohne);
    }
  }
