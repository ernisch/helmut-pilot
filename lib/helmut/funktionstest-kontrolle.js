...
function baueBeobachtungen({
  ...
  // A01/A06 — KORRIGIERT 02.09. (adversariales Diff-Review, bestätigter Befund):
  // Die Zahlen kamen aus `public.llm_usage`. Diese Tabelle ist leer, solange
  // HELMUT_LLM_USAGE_RELATIONAL aus ist oder die Migration nicht angewendet
  // wurde — also im gesamten Zustand dieses Sprints. Beide Regeln meldeten
  // damit eine gemessene 0 gegen eine garantiert leere Quelle: falsches Grün.
  // Jetzt entsteht die Zahl NUR, wenn der Aufrufer die Quelle ausdrücklich als
  // tragfähig erklärt. Sonst bleibt die Beobachtung ABWESEND und die Regel
  // ausdrücklich „nicht bewertbar" (fail closed).
  const modellQuelleTraegt = modellaufrufe
    && (modellaufrufe.relationalAktiv === true || modellaufrufe.blobAusgezaehlt === true);
  if (modellQuelleTraegt) {
  ...
  // A10 — KORRIGIERT 02.09. (adversariales Diff-Review, bestätigter Befund):
  // Gemessen wurde `riegel.durchgelassen`. Am Testtag steht der Riegel auf
  // MODUS_TESTFENSTER und sperrt JEDEN Kanal — die Zahl ist dann strukturell
  // immer 0, unabhängig davon, ob etwas passiert ist. Und der Riegel führt
  // ohnehin KEINEN persistenten Zähler: es gab schlicht keine erhebbare Quelle.
  // Ehrliche Folge: die Beobachtung entsteht nur, wenn der Aufrufer eine
  // ausgezählte Quelle mitliefert (`gezaehlt: true`). Sonst bleibt A10
  // ausdrücklich unbewertbar statt grün.
  if (riegel && riegel.gezaehlt === true) {
    setze("kommunikationsversuche", zahl(riegel.durchgelassen));
  }
