...
function planeProvisionierung({ grundlinie, bestand, modus = MODUS_TROCKENLAUF, env = process.env } = {}) {
  ...
  return Object.freeze({
    ...
    hinweis: anzulegen.length === 0
      ...
      : `${anzulegen.length} synthetische Profile werden INAKTIV angelegt. Betroffen sind `
        + "ausschließlich Kohortenzeilen; die GETEILTE Blob-Zeile `main` wird dabei je Profil "
        + "neu geschrieben (Schreibziel: siehe Vorflug-Riegel)."
  });
}
  ...
  return Object.freeze({
    ...
    hinweis: zuDeaktivieren.length === 0
      ...
      : `${zuDeaktivieren.length} synthetische Profile werden deaktiviert; gelöscht wird nichts. `
        + "Die GETEILTE Blob-Zeile `main` wird dabei je Profil neu geschrieben (Schreibziel: "
        + "siehe Vorflug-Riegel)."
  });
}
