...
function pruefeGrundlinie(grundlinie) {
  ...
  if (realeMandate < 0 || realeMandateAktiv < 0) {
    throw new KohortenAbbruch("Grundlinie: negative Zahl realer Mandate", "grundlinie");
  }
  return Object.freeze({ ...werte, realeMandate, realeMandateAktiv });
}
...
function pruefeBestand(bestand) {
  ...
  if (!("fremdeGeloescht" in bestand)) {
    throw new KohortenAbbruch("Bestand: fremdeGeloescht fehlt — Löschmarken müssen gelesen werden", "bestand");
  }
  ...
  const zeilen = bestand.kohorte.map((zeile, index) => {
    ...
    return Object.freeze({ id: String(zeile.id || "").trim(), aktiv: zeile.aktiv });
  });
  ...
  return Object.freeze({
    ...
    fremdeGesamt: ganzzahl(Number(bestand.fremdeGesamt), "fremdeGesamt"),
    fremdeAktiv: ganzzahl(Number(bestand.fremdeAktiv), "fremdeAktiv"),
    fremdeGeloescht: ganzzahl(Number(bestand.fremdeGeloescht), "fremdeGeloescht")
  });
}
...
function pruefeIsolation({ grundlinie, bestand } = {}) {
  ...
  const pruefungen = [
    ...
    },
    {
      ...
      ok: mails.every((mail) => /@[^@]*\.invalid$/.test(String(mail))),
      detail: `alle ${mails.length} Adressen enden auf die reservierte TLD .invalid`
    },
    {
      name: "Keine Adresskollision mit realen Konten",
      ok: new Set(mails).size === mails.length,
      detail: "alle Kohortenadressen sind untereinander eindeutig und nicht zustellbar"
    },
    {
      name: "Kommunikationsriegel greift für jede Kohortenkennung",
      ok: kennungen.every((id) => !kommunikationsriegel
        .pruefe({ kanal: "mail", kennung: id, empfaenger: `${id}@test-kohorte.invalid` }).erlaubt),
      detail: "jede Kennung wird vom zentralen Riegel gesperrt"
    }
...
function pruefeRueckbau({ grundlinie, bestand } = {}) {
  ...
  const pruefungen = [
    ...
    },
    {
      ...
      ok: ist.fremdeGeloescht === basis.mandateGeloescht,
      detail: `Grundlinie ${basis.mandateGeloescht} Löschmarken, jetzt ${ist.fremdeGeloescht}`
    }
  ].map((p) => Object.freeze({ ...p }));
