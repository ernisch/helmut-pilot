...
function planeAktivierung({
  ...
  const fensterFrei = Boolean(
    startfensterBefund && startfensterBefund.startErlaubt === true
  );
  ...
  return Object.freeze({
    ...
    startfensterGeprueft: startfensterBefund !== null && startfensterBefund !== undefined,
    ...
    blockadeGruende: Object.freeze([
      ...
      ...(fensterFrei ? [] : [startfensterBefund ? "startfenster-konflikt" : "startfenster-nicht-geprueft"])
    ]),
    hinweis: blockiert
      ? (fensterFrei
...
function pruefeRueckbau({ grundlinie, bestand } = {}) {
  ...
  const pruefungen = [
    ...
    {
      // Der REALE Anteil der Identitätsprofile: alles ohne die Kohorte. Die
      // Grundlinie wurde bei 0 Kohortenzeilen erhoben, ihr Wert IST also der
      // reale Anteil und muss ihn nach dem Rückbau exakt wieder ergeben.
      name: "Zahl der realen Identitätsprofile unverändert",
      ok: (ist.identitaetenGesamt - ist.kohortenIdentitaeten) === basis.identitaetsprofile,
      detail: `Grundlinie ${basis.identitaetsprofile} real; jetzt ${ist.identitaetenGesamt} gesamt `
