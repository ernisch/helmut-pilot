...
function pruefeRueckbau({ grundlinie, bestand } = {}) {
  ...
  const pruefungen = [
    ...
    },
    {
      ...
      // ausdrücklich NICHT erzwingt — das Feld `kohortenProfile` darf dort > 0
      // sein (etwa bei einer Grundlinie aus einem zweiten Anlauf). Dann enthält
      // `identitaetsprofile` die Kohorte bereits, und der Vergleich zöge sie ein
      // zweites Mal ab: der Befund wäre still falsch. Er ist jetzt nur bei einer
      // NACHWEISLICH kohortenfreien Grundlinie bewertbar.
      name: "Zahl der realen Identitätsprofile unverändert",
      ok: basis.kohortenProfile === 0
        && (ist.identitaetenGesamt - ist.kohortenIdentitaeten) === basis.identitaetsprofile,
      detail: basis.kohortenProfile === 0
        ? `Grundlinie ${basis.identitaetsprofile} real; jetzt ${ist.identitaetenGesamt} gesamt `
          + `− ${ist.kohortenIdentitaeten} Kohorte = ${ist.identitaetenGesamt - ist.kohortenIdentitaeten}`
        : `NICHT BEWERTBAR: die Grundlinie trägt bereits ${basis.kohortenProfile} Kohortenprofile, `
          + `ihr Wert identitaetsprofile=${basis.identitaetsprofile} ist damit nicht der reine `
          + "reale Anteil. Eine kohortenfreie Grundlinie ist nötig (fail closed)."
    },
    {
