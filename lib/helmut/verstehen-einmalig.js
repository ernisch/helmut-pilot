...
const F17 = require("./verstehen-frische17-vertrag");
...
function pruefeErwartet(erwartet) {
  ...
  if (erwartet === F17.FRISCHE17) return F17.FRISCHE17;
...
async function pruefeUndPlane({ ids = null, deps = {}, commit = null, opts = {}, erwartet = null } = {}) {
  ...
  if (E === F17.FRISCHE17 ? !F17.istVersorgung(deps.artikelkontextVersorgung)
  ...
  if (E === F17.FRISCHE17 && !F17.pruefeInhalt(geladen)) {
    return { ok: false, grund: "verstehen-frische17-inhalt-oder-alter-abweichend" };
  }
...
async function fuehreAus({
  ...
  const quittung = E === F17.FRISCHE17
    ? { ok: quittungsschluessel === F17.QUITTUNG, schluessel: F17.QUITTUNG, grund: "verstehen-frische17-quittung-abweichend" }
    : E === F30.FRISCHE30
  ...
  const freigaben = E === F30.FRISCHE30 || E === F7.BUND7 || E === F17.FRISCHE17 ? new Set() : freigabenAntwort.freigaben;
  ...
  try {
    ...
    for (let i = 0; i < plan.geordnet.length; i += 1) {
      ...
      if ((E === F7.BUND7 || E === F17.FRISCHE17) && !["saved", "updated", "merged", "duplicate"].includes(r?.status)) {
        abbruch = { grund: E === F17.FRISCHE17 ? "verstehen-frische17-einzelergebnis-nicht-bestaetigt" : "verstehen-bund7-einzelergebnis-nicht-bestaetigt", vorgangId: r?.vorgangId || null };
        break;
      }
