...
//       `bilanz.unbekannt > 0` ⇒ `quittungStatus = "unbekannt"` und `ok = false`.
//     * KLASSE B — GLOBALER VERTRAGS-/INFRASTRUKTURFEHLER (`skipped-error`, `skipped-store`,
//       `skipped-veraltet`: Transport-/Modellfehler, nicht pruefbarer Speicher, verlorenes
//       Schreibrecht): hier steht die Verlaesslichkeit des weiteren Laufs selbst in Frage ⇒
...
async function fuehreAus({
  ...
  const quittung = quittungsschluesselVon(quittungsschluessel);
  if (!quittung.ok) {
    return {
      ok: false, reinLesend: !execute, ausgeloest: false, schutzvertrag: false,
      modellaufrufe: 0, quellenabrufe: 0, profilwrites: 0, kommunikation: 0,
  ...
  try {
    ...
    // „fachlich bestanden". Ein Lauf mit mindestens einem unbekannten Ausgang — auch wenn er alle
    // uebrigen Cluster abgearbeitet hat — bleibt sichtbar NICHT bestanden:
    //   * `bilanz.unbekannt > 0` ⇒ Quittung `unbekannt`, `ok = false`;
    //   * ohne unknown ist ein Abbruch `gestoppt`, ohne beides `abgeschlossen`.
    const bilanz = bilanzVon(ergebnisse);
    const hatUnbekannt = bilanz.unbekannt > 0;
    const fachlichBestanden = !abbruch && !hatUnbekannt;
    const status = hatUnbekannt ? "unbekannt" : (abbruch ? "gestoppt" : "abgeschlossen");
