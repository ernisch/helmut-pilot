...
async function pruefeUndPlane({ ids = null, deps = {}, commit = null, opts = {}, erwartet = null } = {}) {
  ...
  if (E === F7.BUND7 ? !F7.istVersorgung(deps.artikelkontextVersorgung)
...
async function fuehreAus({
  ...
  const quittung = E === F30.FRISCHE30
  ...
  const freigaben = E === F30.FRISCHE30 || E === F7.BUND7 ? new Set() : freigabenAntwort.freigaben;
  ...
  try {
    ...
    for (let i = 0; i < plan.geordnet.length; i += 1) {
      ...
      if (E === F7.BUND7 && !["saved", "updated", "merged", "duplicate"].includes(r?.status)) {
        abbruch = { grund: "verstehen-bund7-einzelergebnis-nicht-bestaetigt", vorgangId: r?.vorgangId || null };
        break;
      }
