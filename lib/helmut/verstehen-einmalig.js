...
// `verstehen169-<JJJJMMTT>-<suffix>`.
const QUITTUNG_MUSTER = /^verstehen169-\d{8}-[a-z0-9][a-z0-9-]{0,39}[a-z0-9]$/;
function quittungsschluesselVon(wert) {
  const roh = String(wert == null ? "" : wert).trim();
  if (roh === "") return { ok: true, schluessel: QUITTUNG, standard: true };
...
async function fuehreAus({
  ...
  const bindung = {
    version: 1, quittungsschluessel: quittung.schluessel,
  ...
  try {
    ...
    bericht.quittung = quittiert ? quittung.schluessel : null;
    ...
    return {
      ...
      quittung: geschlossen ? quittung.schluessel : null,
