...
async function pruefeUndPlane({ ids = null, deps = {}, commit = null, opts = {}, erwartet = null } = {}) {
  ...
  if (E === F16.FRISCHE16 ? !F16.istVersorgung(deps.artikelkontextVersorgung)
...
async function fuehreAus({
  ...
  const quittung = E === F16.FRISCHE16
  ...
  const freigaben = E === F30.FRISCHE30 || E === F7.BUND7 || E === F16.FRISCHE16 ? new Set() : freigabenAntwort.freigaben;
  ...
  try {
    ...
    for (let i = 0; i < plan.geordnet.length; i += 1) {
      ...
      if ((E === F7.BUND7 || E === F16.FRISCHE16) && !["saved", "updated", "merged", "duplicate"].includes(r?.status)) {
        abbruch = { grund: E === F16.FRISCHE16 ? "verstehen-frische16-einzelergebnis-nicht-bestaetigt" : "verstehen-bund7-einzelergebnis-nicht-bestaetigt", vorgangId: r?.vorgangId || null };
        break;
      }
