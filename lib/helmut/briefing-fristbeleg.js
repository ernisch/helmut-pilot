...
const RELATIV = new RegExp("\\b(?:" +
  ...
  "|bis\\s+ende\\s+(?:(?:des|dieses)\\s+)?(?:arbeits)?tages?" +
  "|(?:nachste|kommende)\\s+woche(?:\\s+mitte)?" +
  "|(?:bis\\s+)?(?:\\d+|ein(?:e[nmr]?)?|zwei|drei|vier|funf|sechs|sieben|acht|neun|zehn)\\s+(?:stunden?|werktage[n]?|tage[n]?|wochen?)" +
