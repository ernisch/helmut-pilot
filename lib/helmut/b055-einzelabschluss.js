...
function pruefeAuftrag(c, { commit, commandHash, now, neu = false }) {
  fordere(c && B.hash(c) === commandHash && /^[a-f0-9]{64}$/.test(commandHash || "")
    && c.version === (neu ? 2 : 1) && c.id === (neu ? COMMAND_NEU : COMMAND) && c.userId === MANDAT
...
async function ausfuehren({ commit, commandHash, confirmation, config, build,
  ...
  const neu = confirmation === CONFIRM_NEU;
  const commandId = neu ? COMMAND_NEU : COMMAND, startId = neu ? START_NEU : START_ID;
  ...
  const guard = async (ownTenant = false) => {
    ...
    pruefeAuftrag(c, { commit, commandHash, now: now(), neu });
  ...
  try {
    ...
    c = pruefeAuftrag(await readStore(commandId), { commit, commandHash, now: start, neu });
    ...
    for (const slot of (neu ? ["lage", B.SLOT] : [Q.SLOT, "lage", B.SLOT])) fordere(
    ...
    const startRow = { id: startId, user_id: MANDAT, slot: SLOT, generated_at: now().toISOString(),
      payload: { version: neu ? 2 : 1, ...(neu ? { vorgaenger: structuredClone(c.vorgaenger) } : {}), commandHash, runId: c.runId, productionCommit: commit,
...
module.exports = { ausfuehren, pruefeAuftrag, MANDAT, COMMAND, START_ID, SLOT, CONFIRM, MAX_MICRO_USD, COMMAND_NEU, START_NEU, CONFIRM_NEU };
