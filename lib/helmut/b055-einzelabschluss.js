...
const COMMAND_NEU = "b055-einzelabschluss-v2";
const START_NEU = `bf-${MANDAT}-${COMMAND_NEU}-start`;
const CONFIRM_NEU = "B055_NEUER_VERSUCH_INAKTIV_ZWEI_MODELLE_MAX_0424_USD";
const COMMAND_REDAKTION = "b055-redaktion-v1";
const START_REDAKTION = `bf-${MANDAT}-${COMMAND_REDAKTION}-start`;
const CONFIRM_REDAKTION = "B055_REDAKTION_INAKTIV_EIN_REVIEW_MAX_0424_USD";
...
function pruefeAuftrag(c, { commit, commandHash, now, neu = false, redaktion = false }) {
  fordere(c && B.hash(c) === commandHash && /^[a-f0-9]{64}$/.test(commandHash || "")
    && c.version === (redaktion ? 3 : neu ? 2 : 1) && c.id === (redaktion ? COMMAND_REDAKTION : neu ? COMMAND_NEU : COMMAND) && c.userId === MANDAT
  ...
  if (neu) fordere(c.vorgaenger && c.vorgaenger.runId !== c.runId
    && /^nachlauf500-[0-9]{5,20}$/.test(c.vorgaenger.runId || "")
    && /^\d{4}-\d{2}-\d{2}$/.test(c.vorgaenger.tag || "")
    && [c.vorgaenger.startHash, c.vorgaenger.entwurfHash, c.vorgaenger.urteilHash]
      .every(h => /^[a-f0-9]{64}$/.test(h || ""))
    && Number.isSafeInteger(c.vorgaenger.costMicroUsd) && c.vorgaenger.costMicroUsd >= 0,
    "einzelvorgaenger-bindung-fehlt");
...
async function ausfuehren({ commit, commandHash, confirmation, config, build,
  ...
  const redaktion = confirmation === CONFIRM_REDAKTION;
  const neu = confirmation === CONFIRM_NEU || redaktion;
  const commandId = redaktion ? COMMAND_REDAKTION : neu ? COMMAND_NEU : COMMAND,
    startId = redaktion ? START_REDAKTION : neu ? START_NEU : START_ID;
  ...
  const guard = async (ownTenant = false) => {
    ...
    pruefeAuftrag(c, { commit, commandHash, now: now(), neu, redaktion });
  ...
  try {
    fordere((confirmation === CONFIRM || neu) && typeof build === "function", "einzelfreigabe-fehlt");
    T.pruefeConfig(await config(), commit);
    get = deps.get || T.getReader(env, fetchFn);
    c = pruefeAuftrag(await readStore(commandId), { commit, commandHash, now: start, neu, redaktion });
    ...
    let existingFach = null;
    if (neu) {
      const base = await storage.getRenderedBriefingV3(MANDAT, Q.SLOT, c.tag, { strict: true });
      const gleich = base && B.hash(base.payload.urteil) === B.hash(c.urteil);
      existingFach = gleich ? base : await require("./briefing-urteilsnachfolger").lese({
        userId: MANDAT, tag: c.tag, ursprungHash: c.urteil.korrektur.ursprungHash, storage });
      if (existingFach) {
        fordere(B.hash(existingFach.payload.urteil) === B.hash(c.urteil), "einzelfachbasis-nicht-identisch");
        I.pruefeZeile(existingFach, await build(ctx.profile, MANDAT, { aussagenEingabe: true,
          aussagenKorrektur: c.urteil.korrektur, now: now() }), { gelesen: true, nachfolger: !gleich });
      } else fordere(base?.payload?.urteil?.korrektur?.ursprungHash !== c.urteil.korrektur.ursprungHash,
        "einzelfachbasis-nicht-identisch");
    }
    locked = await storage.acquirePipelineLock(LOCK, 360000);
    fordere(locked === true, "einzelstart-gesperrt");
    await guard();
    const startRow = { id: startId, user_id: MANDAT, slot: SLOT, generated_at: now().toISOString(),
      payload: { version: redaktion ? 3 : neu ? 2 : 1, ...(neu ? { vorgaenger: structuredClone(c.vorgaenger) } : {}), commandHash, runId: c.runId, productionCommit: commit,
    ...
    if (neu && !existingFach) {
      await guard();
      const result = await build(ctx.profile, MANDAT, { aussagenEingabe: true,
        aussagenKorrektur: c.urteil.korrektur, now: now() });
      importbericht = await require("./briefing-urteilsnachfolger").speichere({ userId: MANDAT,
        urteil: c.urteil, result, c, storage, now: now() });
    }
    importbericht = neu ? (importbericht || { verwendbar: true, gespeichert: false, wiederverwendet: true }) : await (deps.importiere || I.ausfuehren)({ userId: MANDAT, urteil: c.urteil, storage,
    ...
    if (redaktion) {
      const result = await require("./b055-redaktion").ausfuehren({ c, get, storage, profile: ctx.profile, now,
        generate: deps.redaktionGenerate,
        beforeReview: async () => {
          fordere(freigegebeneModelle === 0, "einzelmodellgrenze");
          await guard(); await fach();
          fordere(kostenBelege.length === 0 && now().getTime() + 90000 < end, "einzelkosten-unbestaetigt");
          freigegebeneModelle++;
        }, beforeSave: async () => {
          await guard(); await fach();
          fordere(freigegebeneModelle === 1 && kostenBelege.length === 1
            && kostenBelege[0].status === "abgerechnet" && kostenBelege[0].bezug.phase === "pruefung",
          "einzelkosten-unbestaetigt");
        } });
      lage = result.lage; briefing = result.briefing;
    } else {
...
module.exports = { ausfuehren, pruefeAuftrag, MANDAT, COMMAND, START_ID, SLOT, CONFIRM, MAX_MICRO_USD, COMMAND_NEU, START_NEU, CONFIRM_NEU, COMMAND_REDAKTION, START_REDAKTION, CONFIRM_REDAKTION };
