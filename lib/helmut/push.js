...
async function sendSmartBriefingPush(briefing, profile = {}) {
  // Kein Fallback auf "cem-ince": ohne erkennbare ID darf der Push nicht an den
  // falschen Nutzer gehen. Eine leere ID findet keine Subscriptions und wird
  // sicher zu keinem Versand fuehren (siehe sendPushToPolitician).
  const politicianId = profile.id || briefing?.politicianId || "";
...
async function sendLageChangePush(lageCheck = {}, profile = {}) {
  // Kein Fallback auf "cem-ince" - siehe sendSmartBriefingPush.
  const politicianId = profile.id || lageCheck.politicianId || "";
