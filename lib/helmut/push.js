...
async function sendPushToPolitician(politicianId, payload, options = {}) {
  // KOMMUNIKATIONSRIEGEL ZUERST — vor der Konfigurationspruefung (s. o.).
  const riegel = kommunikationsriegel.pruefe(
    { kanal: "push", kennung: politicianId },
    options.env || process.env
  );
  if (!riegel.erlaubt) {
    return { ok: false, skipped: true, gesperrt: true, reason: riegel.grund, riegel };
  }
  if (!isPushConfigured()) {
  ...
  for (const subscriptionEntry of subscriptions) {
    const result = await sendPush(subscriptionEntry.subscription, payload, {
      kennung: politicianId,
...
// KORREKTUR 02.09. (adversarialer Review): die zweite Lage prueft jetzt MIT der
// Mandatskennung. Vorher fragte sie nur nach dem Endpunkt — ein echter
// FCM-/Mozilla-Endpunkt traegt aber kein Synthetik-Signal, ergab also
// BEFUND_REAL und war im Standardmodus erlaubt. Damit war die Zusage des
// Kommentars ("auch ein Aufrufer, der sendPushToPolitician umgeht, kommt hier
// nicht am Riegel vorbei") fuer genau den Fall falsch, fuer den sie gedacht war:
// ein synthetisches Profil mit einem echten Push-Abo.
//
// FAIL CLOSED, ABER NICHT RUECKWIRKEND STRENGER: fehlt die Kennung, bleibt es
// bei der Endpunktpruefung von bisher — ein direkter Aufrufer ohne Kennung
// verhaelt sich unveraendert. Der produktive Weg (sendPushToPolitician) reicht
// die Kennung seit diesem Sprint durch und wird damit strikt geprueft.
async function sendPush(subscription, payload, opts = {}) {
  const endpoint = String(subscription?.endpoint || "");
  if (!endpoint) throw new Error("Push endpoint missing");
  const riegelEndpunkt = kommunikationsriegel.pruefe(
    { kanal: "push", ziel: endpoint, ...(opts.kennung ? { kennung: opts.kennung } : {}) },
    opts.env || process.env
  );
  if (!riegelEndpunkt.erlaubt) {
    return { ok: false, gesperrt: true, status: 0, reason: riegelEndpunkt.grund, riegel: riegelEndpunkt };
  }
...
module.exports = {
  ...
  // Exportiert, damit der Riegel an der Netzgrenze selbst pruefbar ist
  // (scripts/kommunikationsriegel-test.js §F2). Intern unveraendert genutzt.
  sendPush
};
