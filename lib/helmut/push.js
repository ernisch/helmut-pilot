...
async function sendSmartBriefingPush(briefing, profile = {}) {
  const politicianId = profile.id || briefing?.politicianId || "cem-ince";
...
async function sendLageChangePush(lageCheck = {}, profile = {}) {
  const politicianId = profile.id || lageCheck.politicianId || "cem-ince";
...
async function sendPushToPolitician(politicianId, payload, options = {}) {
  ...
  // Perf-Fix: die Sends selbst (Netzwerk-Calls an die Push-Dienste) laufen jetzt
  // parallel statt seriell — bei vielen Subscriptions (mehrere Geraete/Nutzer)
  // addierten sich vorher alle Einzel-Latenzen. Das anschliessende Entfernen
  // ungueltiger Subscriptions (404/410) bleibt bewusst SERIELL: removePushSubscription
  // ist ein read-modify-write auf denselben Politiker-Store-Blob (kein Lock), parallele
  // Schreibvorgaenge koennten sich sonst gegenseitig ueberschreiben.
  const sendResults = await Promise.all(subscriptions.map((subscriptionEntry) =>
    sendPush(subscriptionEntry.subscription, payload).catch((error) => ({
      ok: false,
      status: 0,
      error: error.message
    }))
  ...
  for (let i = 0; i < subscriptions.length; i += 1) {
    const subscriptionEntry = subscriptions[i];
...
// Perf/Stabilitaets-Fix: kein Timeout auf dem Push-Service-Call bedeutete, dass ein
// haengender/unresponsiver Push-Endpunkt (Browser-/OS-Push-Dienst) den kompletten,
// bislang seriellen Versand blockieren konnte. Gleiches AbortController-Muster wie
// bei supabaseRequest()/DIP.
const PUSH_REQUEST_TIMEOUT_MS = Number(process.env.HELMUT_PUSH_TIMEOUT_MS) || 8000;
