...
function zyklusPasstInsFenster({
  ...
  const fenster = fensterMinuten;
  const p = parallel;
  const dauer = laufzeitJeAufrufMs;
  ...
  // Fehlende optionale Grenzen bleiben optional. Explizit ungueltige Werte
  // duerfen dagegen weder zu 0 Bedarf noch zu unbegrenzter Kapazitaet werden.
  if (!Number.isSafeInteger(p) || p <= 0 || !Number.isSafeInteger(mandate) || mandate <= 0
    || (maxAnfragenJeMinute !== null
      && (!Number.isSafeInteger(maxAnfragenJeMinute) || maxAnfragenJeMinute <= 0))
    || (bedarfAufrufe !== null && (!Number.isSafeInteger(bedarfAufrufe) || bedarfAufrufe < 0))) {
    return Object.freeze({ bewertbar: false, grund: "Bedarf, Parallelitaet, Mandatszahl oder Minutengrenze ungueltig" });
  }
  ...
  const bedarfUebergeben = bedarfAufrufe !== null;
  const benoetigt = bedarfUebergeben
    ? bedarfAufrufe
    : (modell ? modell.gesamtBedarfProTag : NaN);
  if (!Number.isSafeInteger(benoetigt) || benoetigt < 0) {
  ...
  const ausRpm = maxAnfragenJeMinute !== null
    ? Math.floor(maxAnfragenJeMinute * fenster)
    : null;
  const moeglich = ausRpm === null ? ausLaufzeit : Math.min(ausLaufzeit, ausRpm);
  const scheiben = Math.floor(fensterMs / SCHEIBE_MS);
  const jeScheibe = Math.min(Math.floor(p * SCHEIBE_MS / dauer),
    maxAnfragenJeMinute === null ? Infinity : Math.floor(maxAnfragenJeMinute * SCHEIBE_MS / 60000));
  // Dieselben zwei Grenzen gelten auch fuer die noetige Zeit. Sonst konnte
  // die Ausgabe bei 500 Anfragen und 10 RPM zugleich "passt nicht in 10 min"
  // und "noetig: 1 min" behaupten. Das ist weiterhin ein Rechenmodell,
  // kein gemessener Laufzeitnachweis und keine Zusage fuer einzelne Slots.
  const benoetigteMinuten = Math.ceil(Math.max(benoetigt * dauer / (p * 60000),
    maxAnfragenJeMinute === null ? 0 : benoetigt / maxAnfragenJeMinute));
  if (![ausLaufzeit, scheiben, jeScheibe, benoetigteMinuten, ...(ausRpm === null ? [] : [ausRpm])]
    .every(n => Number.isSafeInteger(n) && n >= 0)) {
    return Object.freeze({ bewertbar: false, grund: "Kapazitaetsrechnung ausserhalb sicherer Zahlen" });
  }
  ...
  return Object.freeze({
    ...
    meldung: moeglich >= benoetigt
      ...
      : `Ein vollständiger Zyklus (${szenario}, ${benoetigt} Aufrufe) passt bei Parallelität ${p} `
        ...
        + `Nötig wären ${benoetigteMinuten} Minuten.`
  });
}
...
//
// AZURE IST SEIT 06.09. VOLLSTÄNDIG BELEGT. Im authentifizierten Microsoft
// Foundry Portal wurde das abonnementsweite TPM-Kontingent rein lesend erhoben:
// `gpt-5-mini`, Global Standard, hat 2.000.000 TPM Gesamtzuteilung; 250.000 TPM
// sind der Production-Bereitstellung `gpt-5-mini` auf `helmut-resource`
// zugeordnet (12,5 %, Portal rundet auf 13 %). Die bereits bekannte
// Deploymentgrenze von 250 RPM / 250.000 TPM bleibt unverändert. Es wurde
// weder Kontingent angefordert noch eine Azure-Ressource geändert.
const BELEGTE_MESSUNGEN = Object.freeze({
  ...
  "azure-kontingente-und-rate-limits": Object.freeze({
    belegt: true,
    wert: Object.freeze({ gesamtTpm: 2000000, zugeteiltTpm: 250000, deploymentRpm: 250 }),
    einheit: "TPM/RPM",
    herkunft: "Microsoft Foundry Portal, Azure subscription 1, rein lesend am 06.09.2026 "
      + "um 22:18 UTC: gpt-5-mini Global Standard 250.000/2.000.000 TPM; "
      + "Deploymentzuordnung 250.000 TPM. Deploymentgrenze 250 RPM bereits bestätigt (§17.5).",
    einschraenkung: "Kontingentmomentaufnahme des Abonnements. Keine Lastzusage, keine "
      + "Azure-Änderung und kein Ersatz für den Kostenstopp vor jedem scharfen Lauf."
  }),
