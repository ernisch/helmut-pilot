...
function quittung({
  ...
  return {
    ...
    grund: grund || null,
    ...(ausgabeBeleg === undefined ? {} : { ausgabeBeleg })
  };
}
...
async function schreibeQuittung(storage, q) {
  ...
  const verifiziert = Boolean(
    ...
    && (q.ausgabeBeleg === undefined || isDeepStrictEqual(gelesen.ausgabeBeleg, q.ausgabeBeleg))
  );
  return {
    gespeichert: true,
    verifiziert,
...
function istWiederholung(vorher, signatur, ausgabeBeleg) {
  if (!vorher || vorher.status !== STATUS_ERFOLG) return false;
  if (!signatur || !vorher.signatur) return false;
  return vorher.signatur === signatur && (ausgabeBeleg === undefined
    || (ausgabeBeleg !== null && isDeepStrictEqual(vorher.ausgabeBeleg, ausgabeBeleg)));
}
