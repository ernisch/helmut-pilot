...
async function erzeuge({ basis, faktenPlan, bereich, runId, beforeCall, onResponse }, deps = {}) {
  ...
  async function call(phase, prompt, schema) {
    ...
    await onResponse({ runId, phase, mandat, basisHash: vertrag.basisHash,
      antwort: structuredClone(antwort) });
    return antwort;
  }
  const entwurf = await call("entwurf", P.entwurfsPrompt(vertrag), ENTWURF_SCHEMA);
...
module.exports = { ENTWURF_SCHEMA, PRUEF_SCHEMA, erzeuge };
