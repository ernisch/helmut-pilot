...
// BEWUSST OHNE VORFLUG-RIEGEL (Entscheidung 04.09., SR §38.2).
// Der Rückweg ist die NOTBREMSE. `funktionstest-ablaufplan.js` führt ihn als
// `immerErlaubt: true` mit der ausdrücklichen Begründung, ein Rückbau dürfe nie
// an einer Vorbedingung scheitern — eine harte Umgebungsprüfung hier würde genau
// diese Zusage brechen und im Ernstfall die Deaktivierung verhindern.
// Der Schaden, gegen den der Riegel schützt, kann hier ohnehin nicht mehr
// entstehen: seit dem 04.09. verkleinert `compactStore` `crawlRuns` in KEINER
// Konfiguration mehr (storage.js). Das Schreibziel wird trotzdem ausgewiesen —
// das CLI druckt den Bericht, es bricht nur nicht ab.
//
// Zweiter Grund für die Nacharbeit (`entferneSchedulerSpur`): sie fasst
// `helmut_store.main` gar nicht an. Sie schreibt über `deleteCronFairnessTenant`
// ausschließlich die eigene Fairness-Zeile, und zwar bereits mit
// Compare-and-Set (storage.js:399ff). Ein Riegel mit der Begründung "sie
// schreibt den geteilten Blob" wäre hier schlicht falsch gewesen.
...
// EINE Freigabequelle: mit Stufe das stufengenaue Wort, ohne Stufe das
// Pauschalwort. Eine unbekannte Stufe faellt NIE auf das Pauschalwort zurueck —
// sonst waere ein Tippfehler eine Freigabe fuer alle 495.
function stufenErlaubnis(stufe, env) {
  ...
  if (!stufen.STUFEN.includes(s)) {
    return Object.freeze({
      schritt: SCHRITT, erteilt: false, flagAn: false, wortStimmt: false, erwartetesWort: null,
      meldung: `Unbekannte Stufe "${String(stufe).slice(0, 20)}" — keine Freigabe (fail closed).`
    });
  }
  const f = stufen.stufenFreigabe(s, SCHRITT, env);
  return Object.freeze({
    schritt: SCHRITT, stufe: s, erteilt: f.erteilt === true,
    flagAn: f.flagAn === true, wortStimmt: f.wortStimmt === true,
...
async function fuehreRueckbauAus({
  ...
  env = process.env,
  // ERGAENZT 02.09.: ohne Stufe unveraendertes Bestandsverhalten.
  stufe = null,
  deps = {}
} = {}) {
  // ZIELMENGE UND FREIGABE MUESSEN DIESELBE STUFE MEINEN (Abschlussreview,
  // BLOCKIEREND — selbst eingebaut). Ohne diese Pruefung haette das
  // stufengenaue Wort fuer Stufe A (20 Profile) einen Rueckbau ALLER 495
  // freigeschaltet, sobald `kennungen` fehlt oder eine groessere Menge traegt.
  // Genau die Verwechslung, die die stufengenauen Freigaben verhindern sollen.
  const stufenName = stufe === null || stufe === undefined
    ? "" : String(stufe).trim().toLowerCase();
  let ziel;
  if (stufenName) {
    const stufen = require("./testkohorte-stufen");
    if (!stufen.STUFEN.includes(stufenName)) {
      const fehler = new Error(`Rueckbau: unbekannte Stufe "${String(stufe).slice(0, 20)}"`);
      fehler.grund = "stufe";
      throw fehler;
    }
    // Ohne ausdrueckliche Liste ist die Zielmenge GENAU die Stufe — nicht alles.
    const roh = kennungen == null ? [...stufen.kennungenDerStufe(stufenName)] : kennungen;
    ziel = stufen.pruefeStufenZielmenge(stufenName, roh, "Rueckbau (stufengenau)");
  } else {
    ziel = zielmenge(kennungen);
  }
  // STUFENGENAU, WENN EINE STUFE GENANNT IST (Kreisschluss-Analyse 02.09.):
  // Der Stufenvertrag deklariert TESTKOHORTE_STUFE_<X>_DEAKTIVIERUNG_BESTAETIGT,
  // gelesen hat das aber KEIN Ausfuehrer — hier stand nur das Pauschalwort fuer
  // alle 495. Wer nach Stufe A nur die 20 abschalten wollte, musste die Freigabe
  // fuer alle setzen, und `alleStufenvertraege` zaehlte ein Wort mit, das nirgends
  // wirkte. Ohne Stufe bleibt es unveraendert beim Pauschalwort.
