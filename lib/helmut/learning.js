...
const NEGATIVE_INTERACTIONS = new Set(["ignored", "snoozed"]);
...
function buildLearningProfile(interactions = []) {
  const relevant = (interactions || [])
    ...
    .filter((interaction) => !isSyntheticInteraction(interaction))
...
function isSyntheticInteraction(interaction = {}) {
  const text = `${interaction.topic || ""} ${interaction.title || ""} ${interaction.recommendationId || ""} ${interaction.metadata?.test || ""}`.toLowerCase();
  return /\b(smoke|test|synthetic)\b/.test(text);
}
