...
function buildNotifications(recommendations = [], profile = {}, now = new Date().toISOString()) {
  ...
  if (top) {
    items.push({
      ...
      title: "Tagesbriefing verfügbar",
      message: `Heute zählt vor allem: ${top.title}.`,
      priority: "medium",
...
function scoreAgainstMandate(item, profile) {
  ...
  const citizenImpact = clamp(
    hasAnyTerm(text, ["million", "beschäftigte", "rentner", "pflegekräfte", "arbeitnehmer", "bürgergeld", "mindestlohn", "rente", "armut", "sozialstaat"]) ? 88 : 38
  );
