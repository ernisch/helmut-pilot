...
function personalizeItem(item, profile, memoryByTopic, seenBySignal, learningProfile, now) {
  ...
  const recommendation = {
    ...
    // Herkunft der Formulierung (Fokus: intern sauber erkennbar, ob KI oder
    // Regelmotor). item.aiEnhanced wird von ai.js gesetzt, ging beim Aufbau
    // dieses Objekts bisher verloren - jetzt explizit durchgereicht.
    aiEnhanced: Boolean(item.aiEnhanced),
    originNote: item.aiEnhanced
      ? "KI-verfeinerte Einschätzung."
      : "Regelbasiert erzeugt (keine KI-Verfeinerung für dieses Element).",
...
function finalRelevanceScore(item, scores, source, noGoPenalty, interactionBias, profile) {
  ...
  // Zwei-Engine-Konsistenz: Die Themen-Engine hat dieses Item bereits vorab
  // eingeordnet (item.decision). Wenn sie es schon als "Sofort reagieren"
  // eingestuft hat, darf diese zweite, unabhängige Mandatsbewertung das nicht
  // stillschweigend unter die eigene "Sofort reagieren"-Schwelle drücken (60)
  // - sonst widersprechen sich beide Engines sichtbar fuer denselben Nutzer.
  // Die Sicherheits-Caps direkt darunter (schwache Einzelquelle, No-Go-Thema)
  // bleiben Obergrenzen und gewinnen immer gegen diese Untergrenze.
  if (item.decision === "Sofort reagieren") score = Math.max(score, 60);
...
function taskFromRecommendation(item, profile, currentPriority, actionType, deadline) {
  return {
    ...
    // "Wichtig" (60-79) ist genau die Punktzahl-Spanne, die client-seitig als
    // "Sofort reagieren" angezeigt wird (siehe recommendationToDecisionItem) -
    // die Aufgabenpriorität muss das widerspiegeln, sonst zeigt dieselbe
    // Empfehlung "Sofort reagieren" bei gleichzeitig nur "medium"-Aufgabe.
    priority: ["Sofort handeln", "Kritisch", "Wichtig"].includes(currentPriority) ? "high" : currentPriority === "Relevant" ? "medium" : "low",
    dueDate: deadline,
...
function communicationFor(item, profile, channel) {
  ...
  const committeeLabel = profile.committee || (profile.committees || [])[0] || "";
  return committeeLabel
    ? `${topic} ist für deinen Ausschuss ${committeeLabel} relevant. Du kannst sachlich einordnen, was jetzt geprüft werden muss und welche Folgen das für die Menschen in deinem Zuständigkeitsbereich hat.`
    : `${topic} ist für dein Mandat relevant. Du kannst sachlich einordnen, was jetzt geprüft werden muss und welche Folgen das für die Menschen in deinem Zuständigkeitsbereich hat.`;
}
