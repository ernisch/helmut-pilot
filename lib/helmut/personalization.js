...
function personalizeBriefing(briefing, profile, memoryEntries = [], interactions = []) {
  ...
  const interactionBiasByTopic = buildInteractionBias(interactions);
  const recommendations = (briefing.items || [])
    .map((item) => personalizeItem(item, profile, memoryByTopic, seenBySignal, interactionBiasByTopic, now))
...
function personalizeItem(item, profile, memoryByTopic, seenBySignal, interactionBiasByTopic, now) {
  ...
  const interactionBias = interactionBiasByTopic.get(topicKey(item.topic || item.title)) || 0;
  ...
  const changeReason = changeReasonFor(item, scores, source, memory, statusChange);
  ...
  const communicationRecommendation = item.suggestedStatement || communicationFor(item, profile, recommendedChannel);
...
function finalRelevanceScore(item, scores, source, noGoPenalty, interactionBias) {
  ...
  if (item.decision === "Ignorieren" || text.includes("ignoriere das thema") || text.includes("keine aktion")) {
...
function buildReferentSummary(recommendations, profile) {
  ...
  return `Guten Morgen. Deine wichtigste Entscheidung ist heute: ${top.title}. ${top.recommended_action}`;
}
...
function buildInteractionBias(interactions = []) {
  ...
  interactions.forEach((interaction) => {
    const key = topicKey(interaction.topic || interaction.title || interaction.signalId || interaction.recommendationId);
