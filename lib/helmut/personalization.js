...
function personalizeBriefing(briefing, profile, memoryEntries = [], interactions = []) {
  ...
  const referentEngine = auditReferentEngine(recommendations, briefing, profile);
  ...
  return {
    ...
    referentEngine,
    learningProfile,
    homeSections: buildHomeSections(recommendations, briefing, profile),
...
function auditReferentEngine(recommendations = [], briefing = {}, profile = {}) {
  ...
  const audits = visible.map((item) => item.referent_audit || auditRecommendation(item, profile));
...
function buildHomeSections(recommendations, briefing, profile = {}) {
  ...
  const governmentPlans = [
    ...visible.filter((item) => contextTypeForRecommendation(item, profile) === "government"),
    ...situational.filter((item) => contextTypeForRecommendation(item, profile) === "government")
  ].filter(uniqueContextItem).slice(0, 3);
  const partyFaction = [
    ...visible.filter((item) => contextTypeForRecommendation(item, profile) === "party"),
    ...situational.filter((item) => contextTypeForRecommendation(item, profile) === "party")
  ].filter(uniqueContextItem).slice(0, 3);
...
function contextTypeForRecommendation(item = {}, profile = {}) {
  ...
  if (hasAnyTerm(text, ["bundesregierung", "bundeskabinett", "bmas", "bundesministerium", "regierungsvorhaben"])) return "government";
  // Partei/Fraktion aus dem Profil, nicht hartcodiert - sonst wuerde ein Artikel
  // ueber eine ANDERE Partei (z. B. "Die Linke fordert...") faelschlich als
  // eigene Partei-/Fraktionslage eines anderen Mandats einsortiert.
...
function inferActionType(text, priority) {
  ...
  if (hasAnyTerm(text, ["presse", "redaktion", "interview"])) return "answer_press";
  // Bare "x " (ohne linke Wortgrenze) traf jedes auf "x" endende Wort vor einem
  // Leerzeichen ("Komplex ist", "Reflex ist") - hasTerm verlangt eine echte
  // linke Wortgrenze vor dem Buchstaben.
  if (hasAnyTerm(text, ["social", "instagram", "linkedin"]) || hasTerm(text, "x")) return "post_social";
  if (hasAnyTerm(text, ["ausschuss", "frage", "sitzung"])) return "raise_in_committee";
  if (hasAnyTerm(text, ["termin", "später", "nachhalten"])) return "schedule_follow_up";
...
function communicationFor(item, profile, channel) {
  ...
  const committee = profile.committee || (profile.committees || [])[0] || "dein Fachgebiet";
  const style = String(profile.communicationStyle || "sachlich").toLowerCase();
  if (channel === "ausschuss") {
    return `Im Ausschuss solltest du bei ${topic} nach konkreter Umsetzung, Kontrolle und Wirkung fragen. Entscheidend ist, ob die Bundesregierung belastbare Antworten liefert.`;
  }
  ...
  return `${topic} ist für deinen Ausschuss ${committee} relevant. Du kannst sachlich einordnen, was jetzt geprüft werden muss und welche Folgen das für dein Fachgebiet hat.`;
}

function directActionText(text) {
  const value = String(text || "").trim();
  if (!value) return "Du solltest eine kurze Linie vorbereiten.";
  if (usesDirectAddress(value)) return value;
  const phrase = actionPhrase(value.replace(/\.$/, ""));
  // Nur ein sicher erkanntes Muster in "Du solltest ..." umbauen. Ein Imperativsatz
  // ist bereits eine direkte, grammatisch korrekte Handlungsanweisung - ein
  // erzwungenes "Du solltest" davor hat wiederholt kaputte Saetze erzeugt
  // ("Du solltest lege ... nach", "Du solltest vorbereiten eine kurze Linie ... vor").
  return phrase ? `Du solltest ${phrase}.` : value;
}

function directConsequenceText(text) {
  const value = String(text || "").trim();
  if (!value) return "Wenn du nicht reagierst, geht politischer Spielraum verloren.";
  // Wie bei directActionText: ein fremder Satz wird nicht in eine
  // "Wenn du nicht reagierst, ..."-Konstruktion gezwungen (falsche Wortstellung
  // bei drittperspektivischem Text). Ein vollstaendiger Satz bleibt unveraendert.
  return value;
}
...
function speaksAboutUser(value, profile = {}) {
  const firstName = String(profile.fullName || "").split(/\s+/).filter(Boolean)[0] || "";
  // Ohne Vornamen (kein Profil verfuegbar) darf NICHT stillschweigend "Cem"
  // angenommen werden - und ein leerer Name wuerde das \b\b-Muster so weit
  // oeffnen, dass es praktisch jeden Satz mit "kann/sollte/muss/..." trifft.
...
// Baut einen erkannten Imperativsatz in eine "Du solltest ..."-Phrase um. Gibt
// null zurueck, wenn kein Muster sicher passt (bewusst KEIN Textfragment/Fallback
// mehr) - der Aufrufer laesst den Originaltext dann unveraendert stehen, statt
// ein moeglicherweise falsches Ergebnis zu erzwingen.
function actionPhrase(value) {
  const text = lowerFirst(value).trim();
  const analyzeAndPrepare = text.match(/^analysiere\s+(.+?)\s+und\s+bereite\s+(.+?)\s+vor\b(.*)$/i);
  if (analyzeAndPrepare) {
    return `${analyzeAndPrepare[1]} analysieren und ${analyzeAndPrepare[2]} vorbereiten${analyzeAndPrepare[3] || ""}`;
  }

  const prepare = text.match(/^bereite\s+(.+?)\s+vor\b(.*)$/i);
  if (prepare) return `${prepare[1]} vorbereiten${prepare[2] || ""}`;

  // Mehrsaetzige oder mehrteilige Texte (zweiter, eigener Imperativ nach "und")
  // NICHT simple ans Satzende verschieben - das erzeugt Bruchstellen genau an der
  // Naht zum zweiten Teilsatz. Lieber gar nicht transformieren.
  const hasSecondClause = /\.\s+\S/.test(text) || /\sund\s+(bereite|analysiere|prüfe|stelle|lies|entwickle|formuliere)\b/i.test(text);
