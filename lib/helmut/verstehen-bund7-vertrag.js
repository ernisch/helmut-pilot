...
const QUITTUNG = "verstehen-bund7-20260925-a";
...
function ausGesichertenBelegen(docs, belege, now = Date.now()) {
  ...
  const clean = belege.map(b => A.pruefeArtikelkontext(docs, b)).sort((a, b) => a.dokumentId.localeCompare(b.dokumentId));
  ...
  const versorgung = async (selected) => {
    ...
    try {
      ...
      return { angefordert: true, ok: true, beleg: A.pruefeArtikelkontext(selected, b) };
...
module.exports = { BUND7, QUITTUNG, EINGABE, INHALT_HASH, BELEGE_HASH, pruefeInhalt, ausGesichertenBelegen, istVersorgung };
