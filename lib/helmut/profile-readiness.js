...
// Normalform fuer den Abgleich gegen VERALTETE_AUSSCHUSSNAMEN: gefaltet, alle
// Nicht-Wortzeichen zu einem Leerzeichen. Anders als `ausschussTokens` werden
// KEINE Stoppwoerter entfernt und die Reihenfolge bleibt erhalten (Begruendung
// in resolveBundestagsausschuss).
function veraltetSchluessel(text) {
  return falte(text).replace(/[^a-z0-9]+/g, " ").trim();
}
const VERALTET_NACH_SCHLUESSEL = new Map(
  VERALTETE_AUSSCHUSSNAMEN.map((v) => [veraltetSchluessel(v.name), v])
);
function veralteteBezeichnungNormalisiert(bezeichnung) {
  return VERALTET_NACH_SCHLUESSEL.get(veraltetSchluessel(bezeichnung)) || null;
}
...
function resolveBundestagsausschuss(bezeichnung) {
  ...
  //
  // Abgeglichen wird der amtliche Name der frueheren Wahlperiode, normalisiert
  // ueber Gross-/Kleinschreibung, Umlaute und Trennzeichen (`veraltetSchluessel`)
  // — sonst waere die neue Sperre durch " ausschuss fuer digitales " oder
  // "AUSSCHUSS FÜR DIGITALES" zu umgehen. Die WORTFOLGE bleibt dabei erhalten;
  // eine Zerlegung in eine Wortmenge waere zu grob: die Kennung `inneres-heimat`
  // (Kurzform des heutigen "Innenausschuss") traegt genau die Woerter der
  // veralteten Bezeichnung und wuerde faelschlich abgewiesen. Nachgemessen:
  // unter dieser Normalisierung kollidiert KEINER der 24 gueltigen Namen und
  // KEINE der 24 Kennungen mit einem der vier veralteten Namen (testgesichert,
  // profil-bereitschaft-test.js R2/R3/R7/R8 — die Sollmenge prueft das in
  // `validateSollmenge` ebenfalls, aber nur im Test, nicht beim Modulladen).
  //
  // REICHWEITE, ausdruecklich: dieser Resolver wird von ZWEI Einstiegen benutzt.
  // `pruefeNeuaktivierung` ist die harte Sperre und gilt nur dem NEUEN
  // Aktivierungsuebergang. `bewerteBundestagsprofil` ist die rein LESENDE
  // Bewertung und laeuft auch ueber BESTEHENDE Profile (Admin-Profilansicht,
  // Speicher-Antwort, Mandatsliste in server.js). Ein bestehendes Mandat, dessen
  // Ausschussfeld noch die amtliche Bezeichnung einer frueheren Wahlperiode
  // traegt, wird dort ab jetzt als „ungueltig" ANGEZEIGT. Das ist gewollt — es
  // ist genau der Hinweis, die Angabe auf die laufende Wahlperiode zu ziehen —,
  // aber es ist KEINE Sperre: es wird nichts deaktiviert, nichts umgeschrieben
  // und kein Verarbeitungsschritt blockiert.
  const veraltet = veralteteBezeichnungNormalisiert(bezeichnung);
  if (veraltet) {
    return {
      ok: false,
