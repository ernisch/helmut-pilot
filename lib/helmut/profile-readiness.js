...
function resolveBundestagsausschuss(bezeichnung) {
  ...
  return {
    ok: false,
    grund: treffer.length > 1 ? "mehrdeutig" : (veraltet ? `veraltet (WP ${veraltet.wahlperiode})` : "nicht in der Sollmenge"),
...
function bewerteBundestagsprofil(profile = {}) {
  ...
  // Angegebene Ausschuesse muessen in der WP-21-Sollmenge aufloesbar sein.
  for (const bezeichnung of ausschuesse) {
