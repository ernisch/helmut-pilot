...
// Landtag-Sprint (P2-3/P2-5): Persona/Regierungsbezug je Parlament aus der zentralen
// Registry (Bundestag-Strings byte-identisch zu den bisherigen Hardcodes).
...
- Jede Empfehlung muss Ausschuss, Mandatsprofil, Quelle oder ${resolveParlament(profile).regierung.vorhabenLabel} konkret verbinden.
...
async function generateCommunicationDraft({ prompt, decision, profile, channel, learningProfile }) {
  ...
  if (!isAiEnabled()) {
    return {
      ...
      text: fallbackStatement(decision, channelSpec, profile),
...
- Oppositionslogik mitdenken: ${resolveParlament(profile).regierung.name} konkret messen, Lücke benennen, Forderung oder Frage präzise formulieren.
...
async function assessParliamentaryItem({ item = {}, profile = {} }) {
  ...
  const prompt = `
Du bist Helmut, der persönliche politische Stabschef des Nutzers.
Bewerte kurz diesen ${resolveParlament(profile).persona.vorgangsBezug} fuer das Mandat.
...
function fallbackStatement(decision, channelSpec = communicationChannel("press"), profile = null) {
  ...
  if (channelSpec.id === "committee_question") {
    // Landtag-Sprint (P2-3): Frage-Adressat je Parlament (Bundestag: "die Bundesregierung",
    // byte-identisch; Berlin: "der Senat von Berlin"; Brandenburg: "die Landesregierung Brandenburg").
    return [
      `Welche konkreten Schritte plant ${resolveParlament(profile || {}).regierung.frageAdressat} bei ${topic}?`,
...
function buildLageBriefingPrompt(vorgaenge, profile = {}) {
  ...
  // Landtag-Sprint (P2-3): Persona je Parlament (Bundestag-Saetze byte-identisch).
  const persona = resolveParlament(profile).persona;
  return [
    persona.lageRolle,
    ...
    `- Ton: sachlich, praezise, nuechtern — ${persona.lageTon}.`,
...
// Landtag-Sprint: Persona konfigurierbar (opts.persona). Default = bisheriger Bundestag-
// Satz (byte-identisch), da dieser Pfad mandantenlos ueber ALLE KOs laeuft und heute
// ausschliesslich Bundesinhalte traegt. Bei Landes-KOs kann der Aufrufer eine passende
// Persona injizieren, ohne diesen Code zu aendern.
function buildKoTagsPrompt(ko = {}, opts = {}) {
  const text = [ko.headline, ko.was_ist_passiert, ko.warum_wichtig, ko.wer_ist_betroffen]
    .map((s) => String(s || "").trim()).filter(Boolean).join("\n");
  return [
    String(opts.persona || "Du bist wissenschaftlicher Mitarbeiter im Bundestag."),
