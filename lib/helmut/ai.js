...
function requestOpenAI(prompt, model = modelName(), meta = {}, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      (response) => {
        ...
        response.on("end", () => {
          ...
          try {
            ...
            reject(providerResponseError("ungueltiger-umschlag"));
            return;
          }
          try {
            ...
            logLlmUsage({ ...meta, model, usage: json.usage, durationMs, success: false, error: "response-not-completed" });
            reject(error);
            return;
          }
      ...
      reject(providerAufbauFehler(kode));
      return;
    }
...
async function generateLageBriefing(vorgaenge, profile = {}, meta = {}) {
  ...
  try {
    ...
    // unehrliche "KI nicht verfuegbar". Alle anderen Fehler wie bisher: null.
