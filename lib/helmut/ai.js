...
// Wiederverwendbarer Keep-Alive-Agent: mehrere LLM-Calls in derselben Invocation
// (z. B. sequenziell pro Briefing-Item in enrichBriefingWithAI) teilen sich eine
// offene TCP/TLS-Verbindung zum selben Host, statt bei jedem Call neu zu handshaken.
...
async function generateCommunicationDraft({ prompt, decision, profile, channel, learningProfile }) {
  ...
  // Kostenkontrolle: dieser Endpunkt hat sonst nur einen In-Memory-Rate-Limiter
  // (uebersteht keinen Cold-Start) und kein Tages-Budget wie der Briefing-Pfad.
  // Gleiches Gate wie runMorningBriefing, damit ein Mandant hier nicht unbegrenzt
  // KI-Kosten verursachen kann.
  const { canSpendLlm, recordLlmUsage } = require("./storage");
  const budget = await canSpendLlm(profile?.id || null);
  if (!budget.allowed) {
    await recordLlmUsage({ callType: "skipped-budget", politicianId: profile?.id || null, model: "none", success: true, error: `LLM-Tagesbudget erreicht (${budget.used}/${budget.limit})` });
    return {
      aiEnabled: false,
      channel: channelSpec.id,
      channelLabel: channelSpec.label,
      text: fallbackStatement(decision, channelSpec),
      sourceNote: "Regelbasiert erzeugt, weil das tägliche LLM-Budget erreicht ist."
    };
  }
...
async function assessParliamentaryItem({ item = {}, profile = {} }) {
  ...
  // Kostenkontrolle: sonst nur In-Memory-Rate-Limiter, kein Tages-Budget. Gleiches
  // Gate wie generateCommunicationDraft/runMorningBriefing.
  const { canSpendLlm, recordLlmUsage } = require("./storage");
  const budget = await canSpendLlm(profile?.id || null);
  if (!budget.allowed) {
    await recordLlmUsage({ callType: "skipped-budget", politicianId: profile?.id || null, model: "none", success: true, error: `LLM-Tagesbudget erreicht (${budget.used}/${budget.limit})` });
    return { aiEnabled: false, ...fallback };
  }
  const prompt = `
Du bist Helmut, der persönliche politische Stabschef des Nutzers.
