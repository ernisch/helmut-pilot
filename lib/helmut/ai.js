...
async function assessParliamentaryItem({ item = {}, profile = {} }) {
  ...
  try {
    ...
    if (!budget || !budget.allowed) {
      ...
      // budgetReason zusaetzlich zum Fallback: der Client kann so (wie im
      // Kommunikationspfad) zwischen Tageslimit und Mandanten-EUR-Budget
      // unterscheiden.
      return {
        aiEnabled: false,
        degraded: true,
...
`;
  try {
    ...
    // Stoerungswahrheit: Ein Budget-Stopp der atomaren Reservierung am
    // Choke-Point (requestOpenAI) ist KEIN KI-Erfolg — vorher schluckte der
    // generische catch auch LLM_BUDGET_EXHAUSTED und der Nutzer sah bei
    // erschoepftem Budget eine als KI markierte Regelantwort. Jetzt identisch
    // zu communicationDraft/office/lage ehrlich als Budget-Zustand ausgewiesen.
    if (error && error.code === "LLM_BUDGET_EXHAUSTED") {
      return {
        aiEnabled: false,
        degraded: true,
...
async function extractKnowledgeObjectTags(ko = {}, meta = {}) {
  ...
  try {
    ...
    // Budget-Stopp der Choke-Point-Reservierung DURCHREICHEN (wie im Lage-Pfad):
    // der Backfill-Loop soll bei erschoepftem Tagesbudget kontrolliert stoppen
    // (stop="daily-llm-budget"), statt jeden Kandidaten als "failed" zu zaehlen
    // und bis zu ~500 nutzlose Reservierungsversuche zu produzieren.
    if (error && error.code === "LLM_BUDGET_EXHAUSTED") throw error;
