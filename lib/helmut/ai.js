...
function fallbackStatement(decision, channelSpec = communicationChannel("press")) {
  ...
  if (channelSpec.id === "committee_question") {
    return [
      ...
      `Welche Wirkung auf die Betroffenen bei ${topic} erwartet sie?`,
  ...
  if (channelSpec.id === "internal_line") {
    return [
      ...
      "- Bei Rückfragen: konkrete Wirkung und Umsetzung betonen."
    ].join("\n");
  }
  // Kanaltexte duerfen keine politikfeldspezifischen Behauptungen (Arbeit,
  // soziale Sicherheit, #GuteArbeit etc.) erzwingen, wenn Thema/Ausschuss des
  // Mandats etwas anderes ist - lieber neutral auf ${topic} bezogen als
  // ueberzeugend falsch fuer ein anderes Politikfeld.
  if (channelSpec.id === "citizen_dialogue") return `${base} Wichtig ist, dass politische Entscheidungen bei ${topic} im Alltag der Menschen ankommen.`;
  if (channelSpec.id === "instagram") return `${base}\n\nPolitik muss konkret besser machen, was Menschen jeden Tag betrifft.`;
  if (channelSpec.id === "linkedin") return `${base}\n\nBei ${topic} zählt am Ende nicht die Ankündigung, sondern die konkrete Wirkung in der Umsetzung.`;
