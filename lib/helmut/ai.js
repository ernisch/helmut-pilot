...
function assembleLageParagraphs(rawResult, allowedIds) {
  ...
  if (list.length < 2 || list.length > 4) return [];
  const paragraphs = list.filter((p) => p && Array.isArray(p.vorgang_ids)
    && p.vorgang_ids.length > 0 && p.vorgang_ids.every((id) => allowed.has(id))).map((p) => ({
  ...
  })).filter((p) => p.text && !/\bvorgang_ids\b|\bvg-[a-z0-9][a-z0-9-]*\b/i.test(p.text));
  if (paragraphs.length !== list.length) return [];
...
async function generateLageBriefing(vorgaenge, profile = {}, meta = {}) {
  ...
  if (!paragraphs.length) throw lageAiFehler(null, "ai-text-invalid");
  return {
    paragraphs,
