...
function pruefeLageParagraphs(rawResult, allowedIds) {
  ...
  if (list.some(p => /\bvorgang_ids\b|\bvg-[a-z0-9][a-z0-9-]*\b/i.test(p.text)))
  ...
  const capped = capLageWords(paragraphs, 250);
  return capped.length >= 2 ? { paragraphs: capped, grund: null }
    : { paragraphs: [], grund: "ai-text-word-limit" };
}
