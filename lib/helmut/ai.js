...
- Gib AUSSCHLIESSLICH valides JSON zurueck: ein Objekt mit dem Schluessel "candidates" = Array.

Pro Kandidat exakt diese Felder:
{
...
`;
  const result = await requestJson(prompt, { callType: "v2ScoreCandidates", politicianId: profile?.id });
  ...
  // In Kandidatenreihenfolge zurueckgeben; fehlende KI-Antworten fallen weg.
  return list.map((c) => byId.get(String(c.id))).filter(Boolean).map(normalizeCandidateScore);
}
...
// (2) Finale Priorisierung der besten Kandidaten in EINEM Call. Liefert eine
// erklaerbare Rangfolge (warum Rang, warum Top 1). Fehlt/scheitert der Call,
// nutzt der Aufrufer die relevanceScore-Reihenfolge.
async function aiPrioritizeTop(topCandidates, profile) {
  const list = (topCandidates || []).map((c) => ({
    id: c.id, title: c.title, relevanceScore: c.aiRelevanceScore, reactOrObserve: c.reactOrObserve
  }));
  if (!list.length) return null;
  const prompt = `
Du bist Helmut, ein politischer Stabschef. Aus den folgenden bereits bewerteten
Kandidaten waehlst du die wichtigsten Handlungsempfehlungen und bringst sie in eine
klare Rangfolge. Wichtigstes zuerst.

Regeln:
- Keine Fakten erfinden. Nur die mitgelieferten Kandidaten nutzen.
- Beruecksichtige Mandatsbezug und Dringlichkeit (react vor observe).
...
Mandat: ${JSON.stringify({ fullName: profile?.fullName, party: profile?.party, committees: profile?.committees }, null, 2)}
Kandidaten:
${JSON.stringify(list, null, 2)}
`;
  const result = await requestJson(prompt, { callType: "v2PrioritizeTop", politicianId: profile?.id });
  if (!result || !Array.isArray(result.ranking)) return null;
  return {
    ranking: result.ranking
      .filter((r) => r && r.id != null)
      .map((r) => ({ id: String(r.id), rank: Number(r.rank) || 0, rankReason: typeof r.rankReason === "string" ? r.rankReason.trim() : "" })),
    top1Justification: typeof result.top1Justification === "string" ? result.top1Justification.trim() : ""
  };
}
...
async function enrichBriefingWithAiV2(briefing, profile) {
  ...
  const candidates = allItems.filter((it) => it.decision !== "Ignorieren").slice(0, maxCandidates);
  trace.candidates = candidates.length;

  try {
    const scores = await aiScoreCandidates(candidates, profile);
    if (!scores.length) throw new Error("no candidate scores");
    const scoreById = new Map(scores.map((s) => [s.id, s]));

    // Kandidaten mit KI-Score anreichern und nach KI-Relevanz sortieren -> Top 10.
    const enrichedCandidates = candidates
      .filter((it) => scoreById.has(it.id))
      .map((it) => applyV2ScoreToItem(it, scoreById.get(it.id)))
      .sort((a, b) => (b.aiRelevanceScore || 0) - (a.aiRelevanceScore || 0));
    const topTen = enrichedCandidates.slice(0, 10);

    // Finale Priorisierung (best effort). Bei Fehler: KI-Score-Reihenfolge behalten.
    let prioritization = null;
    try { prioritization = await aiPrioritizeTop(topTen, profile); } catch (_) { prioritization = null; }

    let orderedTop = topTen;
    if (prioritization && prioritization.ranking.length) {
      const rankById = new Map(prioritization.ranking.map((r) => [r.id, r]));
      orderedTop = [...topTen]
        .map((it) => ({ it, r: rankById.get(it.id) }))
        .sort((a, b) => (a.r?.rank || 999) - (b.r?.rank || 999))
        .map(({ it, r }) => (r ? { ...it, rank: r.rank, rankReason: r.rankReason } : it));
      trace.ranked = prioritization.ranking.length;
      trace.top1Justification = prioritization.top1Justification;
    }

    // Ignorierte Items unveraendert anhaengen, Reihenfolge: Top -> restliche Kandidaten -> Ignoriert.
    const topIds = new Set(orderedTop.map((it) => it.id));
    const remaining = enrichedCandidates.filter((it) => !topIds.has(it.id));
    const ignored = allItems.filter((it) => it.decision === "Ignorieren");
    const items = [...orderedTop, ...remaining, ...ignored];

    trace.scored = true;
    const nextBriefing = { ...briefing, items, ai: { enabled: true, engine: "v2", model: modelName(), enhancedAt: new Date().toISOString() }, v2: trace };
...
function requestJson(prompt, meta = {}) {
  return requestOpenAI(prompt, modelName(), meta).then((text) => parseJsonText(text));
}
...
function requestOpenAI(prompt, model = modelName(), meta = {}) {
  ...
  return new Promise((resolve, reject) => {
        ...
        response.on("end", () => {
          ...
          if (response.statusCode < 200 || response.statusCode >= 300) {
            ...
            logLlmUsage({
              ...
              error: `${isAzure() ? "Azure" : "OpenAI"} HTTP ${response.statusCode}: ${data.slice(0, 200)}`
            });
          ...
          try {
            ...
            logLlmUsage({ ...meta, model, durationMs, success: false, error: error.message });
            reject(error);
          }
        });
      }
    ...
    request.on("error", (error) => {
      logLlmUsage({ ...meta, model, durationMs: Date.now() - startedAt, success: false, error: error.message });
      reject(error);
    });
    request.write(body);
    request.end();
