...
- targetGroup und recommendedChannel sind bereits regelbasiert vorbelegt (siehe Kontext) - pruefe sie und korrigiere sie, falls sie zum konkreten Vorgang nicht passen; erfinde keine neue Zielgruppe ohne Bezug zum Kontext.
...
JSON Felder:
{
  ...
  "targetGroup": "Zielgruppe dieser Empfehlung, konkret benannt",
  "recommendedChannel": "Empfohlener Kanal (z. B. Presse, LinkedIn, Ausschussfrage)"
}
...
async function enrichBriefingWithAiV2(briefing, profile) {
  ...
  try {
    ...
    // SICHERHEIT: fester String statt error.message — dieses Objekt wird gespeichert und
    // im Briefing an den Client ausgeliefert; ein roher Fehlertext (kann u.U. Azure/OpenAI-
    // Antwortdetails enthalten, siehe requestAzure) darf dort nicht landen.
    return { ...briefing, ai: { enabled: true, engine: "v2", degraded: true, reason: "ai-scoring-failed" }, v2: { ...trace, scored: false, reason: "scoring-failed" } };
  }
}
...
function publicDecision(item) {
  return {
    ...
    targetGroup: capText(item.targetGroup, 200),
    recommendedChannel: capText(item.recommendedChannel, 200),
