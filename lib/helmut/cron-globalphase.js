...
// --- Pfadwahl (OP-25 K2.1) ----------------------------------------------------------------------
//
// Es gibt seit K2.1 DREI moegliche Pfade und damit zwei Flaggen. Die Wahl ist an genau EINER
// Stelle getroffen, damit sie testbar bleibt und nicht in `server.js` auseinanderfaellt:
//
//   `alt`         — der heutige Produktionspfad (`runSourceCrawl` je Mandat). DEFAULT.
//   `globalphase` — K1: global erfassen INKLUSIVE globaler Buendelung. Durch K2 als fachlich
//                   nicht aktivierungsfaehig belegt (`docs/betrieb/cron-globalphase.md` §8a);
//                   bleibt nur als Vergleichsmassstab im Repository.
//   `kontext`     — K2.1: global abrufen, Vorgangsbildung KONTEXTGEBUNDEN.
//
// FAIL CLOSED, ausdruecklich: sind BEIDE Flaggen gesetzt, ist die Absicht des Betreibers nicht
// eindeutig. Dann laeuft der ALTPFAD — nicht der neuere, nicht der „bessere". Eine
// widerspruechliche Konfiguration darf nie stillschweigend einen Schattenpfad scharf schalten.
// Sie wird laut protokolliert, damit sie nicht unbemerkt bleibt (CLAUDE.md §4.4).
function waehleCronPfad(env = process.env) {
  const vorgangskontext = require("./vorgangskontext");
  const k1 = globalPhaseEnabled(env);
  const k21 = vorgangskontext.kontextpfadEnabled(env);
  if (k1 && k21) {
    return { pfad: "alt", grund: "widersprechende-flaggen", widerspruch: true };
  }
  if (k21) return { pfad: "kontext", grund: "HELMUT_CRON_GLOBALABRUF", widerspruch: false };
  if (k1) return { pfad: "globalphase", grund: "HELMUT_CRON_GLOBALPHASE", widerspruch: false };
  return { pfad: "alt", grund: "default-aus", widerspruch: false };
}
...
function datenstandNeu({ laufId = null, startAt = 0, mandate = 0 } = {}) {
  return {
    ...
    // OP-25 E3: das Zeitbudget der Phase gehoert IN den versiegelten Datenstand, damit
    // Dauer und Grenze aus derselben Quelle belegbar sind (siehe datenstandVermerk).
    budgetMs: null
  };
}
...
function datenstandVermerk(datenstand) {
  ...
  return {
    ...
    // OP-25 K2.1 (additiv): welcher Buendelungsvertrag galt und wie viele Kontexte es gab.
    // Im K1-Pfad steht hier `"global"` / `null` — der Pfad kennt keinen Kontextbegriff.
    buendelung: datenstand.buendelung || "global",
    kontexte: datenstand.kontexte == null ? null : Number(datenstand.kontexte) || 0,
    // OP-25 E3 (additiv, 2026-08-04): die VERSIEGELTE Dauer und das Budget der globalen Phase.
    // `dauerMs` entsteht in `datenstandVersiegeln` (nowMs − startAt) und ist damit die einzige
    // Zahl, die die Phase VOLLSTAENDIG misst. Der Laufdatensatz-Wert `durationMs` wird dagegen
    // VOR dem Versiegeln gebildet (er steht im `saveCrawlRun`-Aufruf, der selbst noch Teil der
    // Phase ist) und unterzeichnet die Dauer systematisch — er darf den Budgetvertrag nicht
    // belegen. `budgetMs` steht daneben, damit Dauer und Grenze aus DERSELBEN versiegelten
    // Quelle stammen und kein Abgleich ueber zwei Ablagen noetig ist.
    dauerMs: datenstand.dauerMs == null ? null : Number(datenstand.dauerMs) || 0,
    budgetMs: datenstand.budgetMs == null ? null : Number(datenstand.budgetMs) || 0
  };
}
