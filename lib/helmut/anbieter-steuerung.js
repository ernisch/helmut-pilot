...
function anbieterAusUrl(url) {
  let host = "";
  try { host = new URL(String(url)).hostname.toLowerCase(); } catch (_) { return { anbieter: "quelle" }; }
  if (/(^|\.)news\.google\.[a-z.]+$/.test(host)) return { anbieter: "google", modell: "news" };
  if (/(^|\.)google\.[a-z.]+$/.test(host)) return { anbieter: "google", modell: "suche" };
