...
function erstelleTransport(env = process.env, deps = {}) {
  // KOMMUNIKATIONSRIEGEL VOR DER MODUSAUSWAHL: im scharfen Testfenster wird kein
  // Weck-/Warteschlangentransport mehr gebaut — auch kein konfigurierter.
  const riegel = kommunikationsriegel.pruefe({ kanal: "job-transport" }, env);
  if (!riegel.erlaubt) {
    return {
      name: "gesperrt", verfuegbar: false, grund: riegel.grund, riegel,
      sende: async () => ({ ok: false, grund: riegel.grund })
    };
  }
  const modus = dispatchModus(env);
...
async function versendeAbsichten({ env = process.env, deps = {}, limit = 50 } = {}) {
  ...
  // RIEGEL UNABHAENGIG VOM TRANSPORTURSPRUNG (ergaenzt 02.09., adversarialer
  // Review). Der Riegel sitzt in `erstelleTransport` — den Zweig betritt aber nur,
  // wer KEINEN `deps.transport` uebergibt. Ein injizierter Transport (Tests,
  // kuenftige Aufrufer, `outbox-relay`) haette die Sperre des scharfen
  // Testfensters vollstaendig umgangen. Deshalb hier noch einmal, VOR der ersten
  // Vergabe: kein Versand, keine Vergabe, kein Versuch, kein Backoff.
  const riegelVersand = kommunikationsriegel.pruefe({ kanal: "job-transport" }, env);
  if (!riegelVersand.erlaubt) {
    return {
      uebersprungen: true,
      grund: riegelVersand.grund,
      riegel: riegelVersand,
      vergeben: 0, versendet: 0, gesendet: 0, bestaetigt: 0, fehlgeschlagen: 0
    };
  }
