...
//   5. transport.verfuegbar/grund — ob dieser Weg JETZT versenden kann, und warum nicht
//   6. bereit              — Production-Bereitschaft des EREIGNIS-Antriebs (die eine Frage,
//                            die vor einer Umschaltung zaehlt)
//
// `bereit: true` heisst AUSSCHLIESSLICH: der Ereignis-Antrieb ist wirksam, seine Klassengrenzen
// sind an und sein Transport ist jetzt verfuegbar. Es heisst NICHT, dass ein Production-Nachweis
// existiert — der ist eine Betriebsfrage, keine Konfigurationsfrage.
//
...
async function versendeAbsichten({ env = process.env, deps = {}, limit = 50 } = {}) {
  ...
  // AKTIVIERUNGSVORLAUF STOPPT GESCHLOSSEN (Haertungssprint Selbstweck, 2026-08-24).
  // BELEGTE LUECKE: `HELMUT_JOB_DISPATCH_MODE=queue` OHNE `HELMUT_SCALABLE_PIPELINE` ergibt den
  // Antrieb `bestand` — `waehleAntrieb` nannte das als Widerspruch, der Dispatcher lebte ihn
  // aber trotzdem: er baute einen echten Transport, VERGAB Absichten (Versuchszaehler +
  // Backoff) und verbuchte sie als Fehlversuch, waehrend die Verbraucher-Route jedes Signal mit
  // 409 `antrieb-bestand` abwies. Ergebnis waere ein Versuchsverbrauch bis zur Quarantaene fuer
  // Arbeit, die nie haette zugestellt werden koennen. Jetzt gilt: kein Ereignis-Antrieb ⇒ kein
  // Versand und keine Vergabe — die Absichten bleiben unberuehrt liegen, der Cron-Rueckfallweg
  // traegt weiter, und der Widerspruch steht sichtbar in der Bilanz.
  if (modus === "queue") {
    const antrieb = waehleAntrieb(env);
    if (antrieb.antrieb !== "ereignis") {
      return {
        uebersprungen: true, modus, versendet: 0, vergeben: 0, fehlgeschlagen: 0,
