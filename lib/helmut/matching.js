...
function ausschussBelegZulaessig(profilZustaendigkeit, koZustaendigkeit) {
  ...
  // fallen. Verlangt wird deshalb, dass die BELEGTE Ebene des Vorgangs die
  // Bundesebene ist; jede andere belegte Ebene faellt fail-closed heraus, auch
  // eine unlesbare (eine Angabe, die wir nicht deuten koennen, ist kein Beleg).
  //
  // AUSNAHME, bewusst und benannt: ist die Ebene des Vorgangs GAR NICHT belegt
  // (leer oder `unknown`), bleibt das Verhalten unveraendert — genauso wie bei
  // einem Profil ohne belegte Mandatsebene. Dort ist nichts entscheidbar, und ein
  // Wegfall wuerde ECHTE Bundestagsbelege still entfernen (Altbestand ohne
  // `decision_level`; der Radar verlangt zusaetzlich die woertliche Nennung des
  // Ausschussnamens im Inhalt — `radar-committee-evidence-test.js` Faelle 1/1b/8).
  // In Production betrifft das heute 0 Paare: kein Objekt ohne belegte Ebene
  // traegt ueberhaupt eine Ausschussangabe (§51.9, §52.6). Ein striktes
  // fail-closed waere eine Zeile — es ist eine Betreiberentscheidung, keine
  // Codeentscheidung.
  if (pz.ebene === "bund") {
    if (!kz.ebene) return true;                                      // Ebene unbelegt -> nichts entscheidbar
