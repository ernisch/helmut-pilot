...
// Korrektur Mitgliedschafts-/Belegvergleiche schaerft. Harte Ausschussmerkmale
// und ihre Entscheidungspunkte verwenden ebenfalls diesen Belegschluessel;
// die gespeicherten Vektoren und ihre Aehnlichkeitsberechnung bleiben gleich.
...
function derivePolicyFields(ko = {}) {
  // Eine Hintergrundnennung belegt weder Beteiligung noch das Sachgebiet des
  // Vorgangs. Die Erwaehnung selbst bleibt in den separaten Merkmalen erhalten.
  const committees = [...(ko.ausschuesse || [])];
  const out = new Set();
...
function matchedFeatures(pf, kf) {
  ...
  if (ausschussBelegZulaessig(pf && pf.zustaendigkeit, kf && kf.zustaendigkeit)) {
    overlapLabels(pf.committees, kf.committees, committeeMatchKey).forEach((v) => feats.push({ type: "ausschuss", value: v }));
  }
