...
// --- Institutioneller Zustaendigkeitsraum (Befund 27A-1) --------------------
//
...
//
// GELTUNGSBEREICH (bewusst eng, siehe docs/matching-nachvollziehbarkeit.md §50):
// Die Verschaerfung greift ausschliesslich fuer Profile mit BESTIMMTEM
// Zustaendigkeitsraum eines Landesparlaments — Mandatsebene `landtag` UND belegtes
// Bundesland. Sonst bleibt das Verhalten byte-identisch. Fuer Bundesprofile ist die
// gleiche Verschaerfung eine getrennte Betreiberentscheidung (Befund 27A-2, §50.5):
// sie wuerde AKTIVE Bundestagsergebnisse veraendern. Fuer Profile ohne belegte
// Ebene bzw. ohne Bundesland ist gar nichts entscheidbar — dort wird deshalb auch
// nichts stillschweigend entfernt.
...
//
// DIE EINZIGE Bedingung, unter der die Regel ueberhaupt greift: das Profil hat
// einen BESTIMMTEN Zustaendigkeitsraum eines Landesparlaments (Mandatsebene
// `landtag` UND belegtes Bundesland). Nur dann ist ueberhaupt entscheidbar, ob
// eine Ausschussmitgliedschaft institutionell passt. Ist die Seite des Profils
// unbestimmt — Bundesmandat, fehlende Mandatsebene oder fehlendes Bundesland —
// bleibt das Verhalten byte-identisch wie vor diesem Sprint; es wird NICHTS
// stillschweigend entfernt, wo nichts entschieden werden kann.
//
// Greift die Regel, verlangt sie eine POSITIV belegte Uebereinstimmung
// (fail-closed): fehlende, fremde oder uebergeordnete Zustaendigkeit des
// Vorgangs erzeugt KEINEN Beleg — lieber ein ehrlicher Leerzustand als eine
// erfundene Mitgliedschaft.
function ausschussBelegZulaessig(profilZustaendigkeit, koZustaendigkeit) {
  const pz = profilZustaendigkeit;
  if (!pz || pz.ebene !== "land" || !pz.land) return true;           // unbestimmt -> unveraendert
  const kz = koZustaendigkeit;
  if (!kz) return false;
...
function matchedFeatures(pf, kf) {
  ...
  // BEFUND 27A-1: eine Ausschussueberschneidung ist nur dann eine
