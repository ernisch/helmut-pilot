...
// proximity 0..1: belegbasierte Nähe. optionaler match={similarity, matchedFeatures}
// (aus matching.js) verstärkt, ist aber nicht nötig — Kern ist der direkte Feldabgleich.
function proximityScore(ko = {}, profile = {}, match = null) {
  ...
  if (match && Number.isFinite(Number(match.similarity))) s += clamp01(Number(match.similarity)) * 0.15;
  return clamp01(s);
}
...
function personalRelevance(ko = {}, profile = {}, opts = {}) {
  ...
  const proximity = proximityScore(ko, profile, opts.match || null);
  const dynamics = dynamicsScore(ko, now);
  const score = proximity >= PROX_GATE ? round(100 * (0.65 * proximity + 0.35 * dynamics)) : 0;
  return { score, proximity: Number(proximity.toFixed(3)), dynamics: Number(dynamics.toFixed(3)) };
}
