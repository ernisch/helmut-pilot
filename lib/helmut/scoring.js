...
// Landtag-Sprint (P2-5/P2-7 vorbereitet): ebenen-gespiegelte Gewichte fuer Landtagsmandate.
// NICHT automatisch aktiv — Wichtigkeit bleibt per Design mandantenlos; die Nutzung
// (levelImportanceFor + opts.levelImportance) ist eine ausdrueckliche Scharfschalt-
// Entscheidung (Gruenderfreigabe E5), keine stille Verhaltensaenderung.
const LEVEL_IMPORTANCE_LANDTAG = {
  international: 1.0, eu: 0.8, bund: 0.6, land: 0.9, kommune: 0.5, unknown: 0.4
};

// Passende Ebenen-Gewichte fuer ein Profil: Bundestag/unbekannt -> Standard (unveraendert),
// Landtag -> landes-gespiegelte Gewichte. Reine Ableitung, aktiviert nichts.
function levelImportanceFor(profile = {}) {
  const ebene = String(profile.politische_ebene || profile.parliamentType || profile.politicalLevel || "").toLowerCase();
  if (ebene.includes("landtag") || ebene === "land") return LEVEL_IMPORTANCE_LANDTAG;
...
function levelFactor(ko, levelImportance = LEVEL_IMPORTANCE) {
  const lvl = normEnum(ko.decision_level);
  let base = levelImportance[lvl] != null ? levelImportance[lvl] : levelImportance.unknown;
...
function globalImportance(ko = {}, opts = {}) {
  const factors = {
    level: levelFactor(ko, opts.levelImportance || LEVEL_IMPORTANCE),
...
function rankForLage(kos = [], opts = {}) {
  const now = opts.now || Date.now();
  // opts.levelImportance (optional, Landtag-Sprint): abweichende Ebenen-Gewichte —
  // Nutzung nur per ausdruecklicher Scharfschalt-Entscheidung, Default unveraendert.
  const scored = asArray(kos).filter((k) => k && stableId(k)).map((ko) => ({
    ko, importance: globalImportance(ko, { levelImportance: opts.levelImportance }).score, ts: resolveTimestampMs(ko)
  }));
  scored.sort((a, b) =>
...
module.exports = {
  ...
  // Landtag-Sprint: ebenen-passende Gewichte (Ableitung, aktiviert nichts)
  levelImportanceFor,
  // Konstanten (single source of truth für Tests)
  LEVEL_IMPORTANCE, LEVEL_IMPORTANCE_LANDTAG, EVENT_IMPORTANCE, IMPORTANCE_WEIGHTS, ACTION_WEIGHTS, PROX, PROX_GATE, DEFAULT_STALE_HOURS
};
