...
async function awSearchPoliticians(name) {
  const url = `${AW_BASE}/politicians?politician[label][cn]=${encodeURIComponent(name)}&range_end=25`;
...
async function lookupMandate(opts = {}) {
  ...
  // 1) Gezielter Abruf nach Disambiguierung.
  if (forcedId) {
    try {
      const profile = await awBuildProfile({ id: forcedId, label: name || forcedId, party: "" }, requestedLevel, warnings);
  ...
  if (pool.length > 1) {
    ...
    const unique = [];
    for (const c of pool) {
      const key = `${c.label.toLowerCase()}|${normalizeParty(c.party)}`;
      if (seen.has(key)) continue;
      seen.add(key);
      unique.push(c);
    }
