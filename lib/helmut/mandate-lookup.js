...
async function awBuildProfile(candidate, requestedLevel, warnings) {
  // Kernmandat (bestimmt Ebene/Fraktion/Wahlkreis): ein TECHNISCHER Ausfall
  // (Netz/5xx) darf NICHT still zu einem hohlen „found" degradieren — sonst
  // verbürge sich ein Quellenausfall als leeres, aber „gefundenes" Profil.
  // Deshalb technischen Fehler weiterreichen (-> lookupMandate: source_down);
  // eine LEERE, aber erreichbare Antwort (kein Mandat hinterlegt) bleibt gültig.
  const mandate = await awMandateFor(candidate.id).catch((e) => { if (isTechnicalFailure(e)) throw e; return null; });
