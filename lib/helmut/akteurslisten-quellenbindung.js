...
// Vorgang, nicht nur eine Nennung. Fuer BEIDE gilt unveraendert: ein unbelegter Wert darf NICHT
// als belegt gespeichert werden.
//
// ANLASS `parteien` (Production-Befund 2026-09-24, 169er Run 35987448290): DREI lokale
// `quellenbeleg-parteien`-Fehler sperrten die gesamte, sonst brauchbare Antwort (`skipped-invalid`).
// Ein unbelegter `parteien`-Wert wird deshalb entfernt — aber NUR, wenn die Antwort nachweislich
// nicht von ihm abhaengt (`ohneUnbelegteAkteurswerte`, Erklaerung dort). Sonst bleibt die Antwort
// unveraendert fail closed. Der Wert entfaellt vollstaendig (in `parteien` UND in
// `mentioned_parties`); der Belegmassstab selbst bleibt woertlich derselbe. `ausschuesse` bleibt
// dagegen vollstaendig streng: dort ist kein Production-Fehler belegt, und die bestehenden
// Ausschuss-Vertraege bleiben unveraendert.
...
const PARTEI_BETEILIGUNGSFELD = "parteien";
...
//
// ANLASS 3 / BETEILIGUNGSLISTE `parteien` (Production-Befund 2026-09-24, Run 35987448290):
// DREI lokale `quellenbeleg-parteien`-Fehler sperrten die gesamte, sonst brauchbare Antwort.
// `parteien` wird deshalb ebenfalls reduziert — aber NUR unter einer BEDINGUNG. Der historische
// Grund fuer die Sonderstrenge war ausdruecklich das Risiko einer „stillen Listenbereinigung bei
// gleichzeitig erhaltener abhaengiger Empfehlung“ (docs/betrieb/parteien-quellenbindung-2026-
// 09-18.md). Genau dieses Risiko wird hier deterministisch geprueft:
//
//   * Kommt der unbelegte Parteiwert in KEINEM anderen gespeicherten Feld vor, ist die Antwort
//     nachweislich unabhaengig davon — dann entfaellt der Wert, die uebrige Antwort bleibt.
//   * Kommt er in irgendeinem anderen Feld vor (Prosa, Empfehlung, Risiko/Chance, strukturierte
//     Kommunikations- oder Handlungselemente, ...), dann ist eine abhaengige Aussage NICHT
//     auszuschliessen; `parteien` bleibt UNVERAENDERT und der strenge Validator weist die
//     Antwort wie bisher ab (fail closed). Es wird KEINE abhaengige Prosa entfernt oder
//     umgeschrieben — das waere Informationsverlust bzw. -verfaelschung.
//
// Der Vergleich benutzt exakt denselben Beleg (`istWertBelegt`, also Normalisierung, Wortgrenzen
// und die dokumentierte Partei-Artikelvariante) — kein Fuzzy, keine Synonyme, keine Ableitung.
// Grenze, die dabei ehrlich bleibt: eine umschreibende Prosa OHNE den Namen ist nicht
// erkennbar. Sie ist aber auch kein Merkmal DIESES Fixes — Prosa wird im bestehenden Vertrag
// grundsaetzlich nicht auf Akteursbeteiligung geprueft, und es wird nichts hinzugefuegt.
//
...
//
// ALLES ANDERE BLEIBT STRENG: die Beteiligungsliste `ausschuesse`, alle uebrigen Antwortfelder,
// das Schema und der `decision_level-Konflikt` laufen unveraendert fail closed. Der GOLDSET-
// Auswerter und der strenge Validator `pruefeAkteurslistenQuellenbindung` sind von der Reduktion
// ueberhaupt nicht beruehrt — sie pruefen weiter jeden Rohwert.
...
function ohneUnbelegteAkteurswerte(antwort, prompt) {
  ...
  const texte = quellentexte(prompt);
  const out = { ...antwort };
  // 1) Die unbedingt reduzierbaren Listen (Erwaeehnungen + Ministerien) — wie bisher.
  for (const feld of REDUZIERBARE_FELDER) {
    if (feld === PARTEI_BETEILIGUNGSFELD) continue;
  ...
  // 2) `parteien` NUR bei nachgewiesener Unabhaengigkeit reduzieren (siehe Kopf).
  if (Array.isArray(antwort[PARTEI_BETEILIGUNGSFELD])) {
    const entfernte = unbelegteBeteiligungsparteien(out, prompt);
    if (!parteienAbhaengig(out, entfernte)) {
      out[PARTEI_BETEILIGUNGSFELD] = antwort[PARTEI_BETEILIGUNGSFELD].filter((wert) =>
        typeof wert !== "string" || istWertBelegt(wert, texte, true));
    }
  }
  return out;
}

// Alle nicht-leeren Strings der Antwort AUSSER dem genannten Feld — normalisiert, wie der Beleg
// sie erwartet. Durchsucht auch Listen und verschachtelte Strukturen (Handlungs-/Kommunikations-
// elemente), damit eine abhaengige Aussage nicht uebersehen wird.
function textwerteAusser(antwort, ausgenommen) {
  const texte = [];
  const sammle = (wert) => {
    ...
    if (Array.isArray(wert)) { for (const el of wert) sammle(el); return; }
    if (wert && typeof wert === "object") { for (const v of Object.values(wert)) sammle(v); }
  };
  for (const [feld, wert] of Object.entries(antwort || {})) {
    if (ausgenommen.includes(feld)) continue;
    sammle(wert);
  }
  return texte;
}

// Die unbelegten STRINGS der Beteiligungsliste `parteien` (leer, wenn alles belegt ist).
function unbelegteBeteiligungsparteien(antwort, prompt) {
  const liste = antwort && antwort[PARTEI_BETEILIGUNGSFELD];
  if (!Array.isArray(liste)) return [];
  const texte = quellentexte(prompt);
  return liste.filter((wert) => typeof wert === "string" && !istWertBelegt(wert, texte, true));
}

// Haengt die Antwort an einem der entfernten Werte? (derselbe Beleg, dieselbe Artikelvariante)
function parteienAbhaengig(antwort, entfernte) {
  if (!Array.isArray(entfernte) || !entfernte.length) return false;
  const rest = textwerteAusser(antwort, [PARTEI_BETEILIGUNGSFELD]);
  return entfernte.some((wert) => istWertBelegt(wert, rest, true));
}
...
module.exports = { PROMPT_REGELN, pruefeAkteurslistenQuellenbindung, ohneUnbelegteAkteurswerte,
  ohneFalschTypisierteAkteure, FELD_TYPEN, PARTEI_BETEILIGUNGSFELD, unbelegteBeteiligungsparteien,
  parteienAbhaengig };
