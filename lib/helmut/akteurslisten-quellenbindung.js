...
// reduziert wird (siehe `ohneUnbelegteMinisterien`). Alle anderen Akteurslisten bleiben fuer
// UNBELEGTE Werte unveraendert streng: ein unbelegter Wert sperrt die Antwort weiterhin
// vollstaendig. Zusaetzlich entfernt `ohneFalschTypisierteAkteure` in ALLEN Listen woertlich
// BELEGTE, aber eindeutig FALSCH TYPISIERTE Werte (zentrale Entitaetsschicht, Befund 2026-09-23).
...
// Werte werden hier NICHT entfernt; sie laufen unveraendert durch den strengen Validator (die
// beiden optionalen Ministeriumslisten reduziert zusaetzlich `ohneUnbelegteMinisterien`).
...
function ohneFalschTypisierteAkteure(antwort, prompt) {
  ...
  for (const feld of FELDER) {
    ...
    out[feld] = antwort[feld].filter((wert) =>
      typeof wert !== "string"
      || !istWertBelegt(wert, texte, istParteifeld)
