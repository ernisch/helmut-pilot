...
function vergleicheLlmUsageProjektion(blobEintrag = {}, zeile = {}) {
  ...
  for (const feld of Object.keys(soll)) {
    ...
    const gleich = a === null || a === undefined
      ...
      // `Number(b) === a` meldete "gleich", wenn die relationale Spalte NULL ist
      // und der Blob 0 traegt — `Number(null)` ist 0. Genau die Abweichung, die
      // diese Pruefung finden soll (eine nicht geschriebene Zahlenspalte), sah
      // damit korrekt aus. NULL/undefined ist jetzt nie gleich einer Zahl.
      : (typeof a === "number"
        ? (b !== null && b !== undefined && b !== "" && Number(b) === a)
        : (typeof a === "boolean" ? b === a : String(b) === String(a)));
