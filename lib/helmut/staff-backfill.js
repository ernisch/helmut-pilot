...
function defaultDeps() {
  ...
  return {
    ...
    // Einzel-Getter fuer den GEZIELTEN Backfill (genau ein KO, kein Laden anderer Objekte).
    getKnowledgeObjectById: (id) => storage.getKnowledgeObjectById(id),
    getKnowledgeObjectByVorgang: (vid) => storage.getKnowledgeObjectByVorgang(vid),
...
async function backfillStaffFields(opts = {}, depsOverride = {}) {
  ...
  // Dann wird NUR dieses eine Objekt direkt geladen (kein listKnowledgeObjects,
  // kein anderes Objekt), und das Limit ist HART 1.
  const targetKoId = cleanStr(opts.targetKoId);
  const targetVorgangId = cleanStr(opts.targetVorgangId);
  const targeted = Boolean(targetKoId || targetVorgangId);
  const limit = targeted ? 1 : (Number(opts.limit) > 0 ? Math.floor(Number(opts.limit)) : 2000);
  ...
  if (targeted) {
    let ko = null;
    if (targetKoId) ko = await deps.getKnowledgeObjectById(targetKoId);
    if (!ko && targetVorgangId) ko = await deps.getKnowledgeObjectByVorgang(targetVorgangId);
    // Defensive: nur uebernehmen, wenn die IDs EXAKT passen (kein Fremdobjekt).
    const matches = ko && (!targetKoId || ko.id === targetKoId) && (!targetVorgangId || ko.vorgang_id === targetVorgangId);
    all = matches ? [ko] : [];
  } else {
    all = await deps.listKnowledgeObjects({ limit });
  }
  const understood = (all || []).filter((k) => k && k.vorgang_id && k.understanding_status === "complete");
  // Hartes Limit: nie mehr Kandidaten verarbeiten als das (im Zielmodus 1) Limit.
  const candidateKos = understood.filter(isBackfillCandidate).slice(0, limit);
  ...
  // Ziel-Bezug transparent im Plan (nur IDs, keine Inhalte).
  plan.targeted = targeted;
  plan.targetKoId = targetKoId || null;
  plan.targetVorgangId = targetVorgangId || null;
