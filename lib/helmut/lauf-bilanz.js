...
function laufBilanz(ergebnis = {}, { gruppenKarte = ERGEBNISGRUPPEN } = {}) {
  ...
  if (!clusterBrauchbar) {
    ...
    status = gespeichert > 0 ? STATUS.PARTIAL : STATUS.FAILED;
  } else if (Number(telemetrie.ergebnisse?.["skipped-artikelkontext"]) > 0) {
    // Angeforderter, aber fehlender Text ist eine Versorgungsluecke. Die Arbeit
    // bleibt vertagt, ihre Quittung darf jedoch keine vollstaendige Versorgung melden.
    fehlerklasse = "artikelkontext-offen";
    status = gespeichert > 0 ? STATUS.PARTIAL : STATUS.BLOCKED;
  } else if (Number(ergebnis.artikelkontextZeitVertagt) > 0) {
    // Der angeforderte Kontextlauf hat seine engere Frist erreicht. Er bleibt
    // offen; normale Zeitvertagungen ohne diesen Anschluss behalten ihren Vertrag.
    fehlerklasse = "artikelkontext-zeitbudget";
    status = gespeichert > 0 ? STATUS.PARTIAL : STATUS.BLOCKED;
  } else if (gesamt === 0) {
