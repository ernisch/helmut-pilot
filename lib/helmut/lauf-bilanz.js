...
function laufBilanz(ergebnis = {}, { gruppenKarte = ERGEBNISGRUPPEN } = {}) {
  ...
  const stimmig = cluster == null ? null : gesamt === cluster;

  const fehlerklasse = fehlgeschlagen > 0 ? fehlerklasseAus(telemetrie.ergebnisse, gruppenKarte) : null;
  ...
  if (fehlgeschlagen > 0) {
    ...
    // Ordnungsgemaess beendet, aber ohne jede Arbeitszeile.
