...
function pruefeManifest(m) {
  D.fordere(m && [1, 2].includes(m.version) && /^[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89ab][a-f0-9]{3}-[a-f0-9]{12}$/.test(m.laufId || ""),
  ...
  D.fordere(Array.isArray(ids) && ids.length === 500 && Array.isArray(aus) && aus.length === (m.version === 2 ? 0 : 4)
    && alle.every(id => typeof id === "string" && /^[a-zA-Z0-9_-]{1,200}$/.test(id))
    && new Set(alle).size === 500 + aus.length && D.ALLE_KENNUNGEN.every(id => ids.includes(id))
...
function plane(s, bestandsauswahl, vertrag) {
  const version = vertrag.version === undefined ? 1 : vertrag.version;
  const ziel = Z.pruefe(s, bestandsauswahl, version);
  const m = pruefeManifest({ ...vertrag, version, ids: ziel.ids, ausserhalb: ziel.ausserhalb, zielHash: ziel.zielHash });
...
select now() as zeit, (select data from public.helmut_store where id=${key}) as quittung,
  ...
  (select coalesce(jsonb_agg(user_id order by user_id),'[]'::jsonb) from public.mandate_profiles where not(user_id=any(${ziel}))) as ausserhalbKennungen;\n`;
...
${start ? `  if q is not null or exists(select 1 from public.helmut_store where id like '${PREFIX}%' and data->>'zustand'='aktiv') then
  ...
  if (select count(*) from public.mandate_profiles) <> ${500 + m.ausserhalb.length}
    or (select count(*) from public.profiles) <> ${501 + m.ausserhalb.length}
    or (select coalesce(jsonb_agg(user_id order by user_id),'[]'::jsonb) from public.mandate_profiles where not(user_id=any(ids))) is distinct from m->'ausserhalb'
...
function bewerteLesung(m, r) {
  ...
  const basis = zahlen && r.gesamt === 500 + m.ausserhalb.length && r.zielvorhanden === 500 && r.ausserhalbaktiv === 0
