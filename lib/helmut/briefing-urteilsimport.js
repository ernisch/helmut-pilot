...
function pruefeZeile(entry, result, { gelesen = false, nachfolger = false } = {}) {
  ...
  fordere(keys(b, ["version", "tag", "productionCommit", "freigabeHash", "urteilHash", "kontextHash"])
    ...
    && entry.id === (nachfolger
      ? require("./briefing-urteilsnachfolger").kennung(entry.user_id, b.tag, p.urteil?.korrektur?.ursprungHash)
      : `bf-${entry.user_id}-${Q.SLOT}-${b.tag}`)
