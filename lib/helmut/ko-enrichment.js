...
async function runKoEnrichmentBackfill(opts = {}, deps = {}) {
  ...
  const samples = [];
  // bypassBudget: umgeht NUR das tageweite App-Budget (canSpend) fuer diesen einen
  // bewussten Admin-Lauf. Der harte Euro-Deckel (capCents) bleibt die echte Grenze.
  ...
  for (const ko of candidates) {
    ...
    let ai;
    try { ai = await deps.extractTags(ko); } catch (_) { ai = null; }
