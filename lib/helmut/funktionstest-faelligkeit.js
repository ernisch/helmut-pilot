...
function faelligkeitsBefund({
  ...
  const bestandGemessen = roh !== null && roh.gemessen === true
  ...
  const bestandJeKlasse = klassen.map((k) => {
    ...
    const erledigtImTestfenster = rohImFenster === null ? null : Math.floor(rohImFenster);
    ...
    return Object.freeze({
      ...
      fachlichVollstaendig: fehlend === 0 && endgueltigFehlerhaft === 0
        ...
        && erledigt + Math.min(ausstehend, k.bisFensterendeBeanspruchbar) >= erwartet
    });
  });
