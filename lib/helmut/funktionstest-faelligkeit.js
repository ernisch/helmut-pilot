...
function faelligkeitsBefund({
  ...
  if (Array.isArray(rotation)) {
    rotationsListe = rotation.map((m) => String(m || "").trim()).filter(Boolean);
...
function erhebungsSql({
  ...
  // Frischefenster: entweder der Platzhalter oder exakt die Form `JJJJ-MM-TThhZ`,
  // die `fensterKennung` erzeugt. Alles andere wird ABGEWIESEN, nicht escaped.
  let fensterZeile = null;
  if (frischefenster !== null && frischefenster !== undefined && String(frischefenster).trim() !== "") {
    const roh = String(frischefenster).trim();
    if (!/^\d{4}-\d{2}-\d{2}T\d{2}Z$/.test(roh)) {
      throw new Error(`erhebungsSql: kein gueltiger Frischefensterschluessel: ${roh.slice(0, 40)}`);
    }
    fensterZeile = `  and freshness_window = '${roh}'`;
  }

  // Stufe: erzeugt die kumulative Praefixliste (C = a,b,c). Eine unbekannte
  // Stufe wird abgewiesen.
  let stufenZeile = null;
  if (stufe !== null && stufe !== undefined && String(stufe).trim() !== "") {
    const s = String(stufe).trim().toLowerCase();
    if (!stufen.STUFEN.includes(s)) {
      throw new Error(`erhebungsSql: unbekannte Stufe: ${s.slice(0, 20)}`);
    }
    const bis = stufen.STUFEN.slice(0, stufen.STUFEN.indexOf(s) + 1);
    stufenZeile = "  and (" + bis.map((x) => `tenant_id like '${praefix}${x}-%'`).join(" or ") + ")";
  }

  const warnung = [];
  if (!fensterZeile) {
    warnung.push("-- ACHTUNG: OHNE Frischefensterfilter. Zurueckgestellte Auftraege FRUEHERER");
    warnung.push("-- Tage stehen ebenfalls auf 'wartend' und werden mitgezaehlt. Das Ergebnis");
    warnung.push("-- ist dann KEIN Beleg fuer das geprueffte Fenster.");
  }
  if (!stufenZeile) {
    warnung.push("-- ACHTUNG: OHNE Stufenfilter. Alle provisionierten Kohortenkennungen zaehlen");
    warnung.push("-- mit, auch die einer noch nicht gestarteten Stufe.");
  }
  ...
  return [
    ...
    "--",
    "-- Sie uebernimmt die drei WHERE-Bedingungen des Claims",
    ...
    "-- NICHT abgebildet: der vorgelagerte Lease-Ruecklauf (die Zahl ist eine",
    "-- UNTERGRENZE), die Claim-Reihenfolge und das Claim-Limit.",
    "--",
    ...warnung,
    ...
    stufenZeile || `  and tenant_id like '${praefix}%'`,
    fensterZeile,
