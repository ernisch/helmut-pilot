...
function prompt(paragraphs, vorgaenge, profile) {
  return [
    ...
    "Wenn ein Absatz quellen_ids und auswahl enthaelt, pruefe ausschliesslich dieses eine ausgewaehlte Quelldokument und gib dieselbe quelle_id zurueck. Keine fehlende Aussage durch einen anderen Bericht derselben Vorgangsgruppe ersetzen. Die private Auswahlbegruendung ist nur eine Behauptung des Generators und kein Beleg; pruefe den Profilbezug eigenstaendig.",
    "Eine institutionelle Standardbeschreibung ist keine aktuelle Nachricht. Aus einer allgemeinen Ministeriumszustaendigkeit und einem Redetitel folgt nicht, dass diese Aussage in der Rede gefallen ist. Solche Vermischung ist nicht vollstaendig belegt; reine Organisationsbeschreibung ist fuelltext.",
...
function sachdetailsGebunden(paragraphs, vorgaenge) {
  ...
  return Array.isArray(paragraphs) && paragraphs.every(p => {
    ...
    return Boolean(q && require("./briefing-quellenqualitaet").quellengebunden(
      { display_summary: p.text }, [{ title: q.titel, summary: q.auszug }]));
  });
}
...
function pruefe(paragraphs, vorgaenge, urteil) {
  ...
  for (let index = 0; index < paragraphs.length; index++) {
    ...
    const bound = p.auswahl;
    const selected = bound && (vorgaenge || []).find(v => v.vorgang_id === p.vorgang_ids?.[0]);
    const source = selected?.quellenbelege?.find(q => q.quelle_id === r?.quelle_id);
    const q = bound ? (source && { ...source, vorgang_id: selected.vorgang_id }) : r && docs.get(r.quelle_id);
    const fehler = [];
    ...
    if (!q || (bound && (p.quellen_ids?.length !== 1 || p.quellen_ids[0] !== r?.quelle_id
      || bound.quelle_id !== r?.quelle_id))) fehler.push("quelle-unbekannt");
    ...
    gesehen.add(key);
    const { auswahl, ...sichtbar } = p; // Private Auswahl nie in die App uebernehmen.
    out.push({ ...sichtbar, quellen_ids: [q.quelle_id], belegstellen: [{ quelle_id: q.quelle_id, text: beleg }] });
  }
...
module.exports = { VERSION, SCHEMA, profilKontext, prompt, pruefe, sachdetailsGebunden, sichereDiagnose };
