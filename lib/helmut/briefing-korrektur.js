...
function wendeAn({ eingabe, profile, kos, sourcesByVorgang, korrektur }) {
  ...
  const mitErgaenzungen = Object.hasOwn(korrektur || {}, "ergaenzungen");
  const mitPriorisierung = Object.hasOwn(korrektur || {}, "priorisierung");
  const felder = ["version", "ursprungHash", "entwuerfe", "auslassungen"];
  if (mitErgaenzungen) felder.push("ergaenzungen");
  if (mitPriorisierung) felder.push("priorisierung");
  if (!eingabeHash || hash(inhalt) !== eingabeHash || profile?.id !== eingabe.mandat
    || !keys(korrektur, felder)
  ...
  // Ein bisher zurueckgehaltener Vorgang darf nach belegter Korrektur wieder
  // vorgelegt werden. Nur vorhandene, vollstaendig gelesene Eingabedaten;
  // keine fremden KOs, kein automatisches Nachruecken und kein Fachurteil hier.
  const ergaenzungen = mitErgaenzungen ? korrektur.ergaenzungen : [];
  const sichtbar = eingabe.korrekturKontext.sichtbareVorgaenge;
  if (!unique(ergaenzungen) || ergaenzungen.some(id => sichtbar.includes(id)
    || kos.filter(k => k.vorgang_id === id && k.status !== "pending"
      && k.understanding_status === "complete").length !== 1
    || !sourcesByVorgang[id]?.length)) fail();
  ...
  if (!sameSet(ids, sichtbar.concat(ergaenzungen))
    || ergaenzungen.some(id => !entwuerfe.some(r => r.vorgangId === id))) fail();
  const priorisierung = mitPriorisierung ? korrektur.priorisierung : null;
  // Explizites redaktionelles Urteil statt erfundener Modellkonfidenz oder
  // Dringlichkeit. Vollstaendige Reihenfolge, nur Beobachten/Ignorieren;
  // sie wird erst nach der unveraenderten Quellenpruefung angewendet.
  if (priorisierung && (!Array.isArray(priorisierung)
    || !sameSet(priorisierung.map(r => r?.vorgangId), entwuerfe.map(r => r.vorgangId))
    || priorisierung.some(r => !keys(r, ["vorgangId", "entscheidung", "begruendung"])
      || !["Beobachten", "Ignorieren"].includes(r.entscheidung)
      || !text(r.begruendung)))) fail();
  if (mitPriorisierung && !priorisierung) fail();
  const nextKos = [], nextSources = {};
  for (const r of entwuerfe) {
    const felder = ["vorgangId", "begruendung", "quellenIds", "inhalt"];
    const mitUrls = Object.hasOwn(r || {}, "quellenkorrekturen");
    if (mitUrls) felder.push("quellenkorrekturen");
    if (!keys(r, felder) || !text(r.begruendung)
      || !unique(r.quellenIds) || !r.quellenIds.length) fail();
    const urls = mitUrls ? r.quellenkorrekturen : [];
    if (!Array.isArray(urls) || !unique(urls.map(u => u?.documentId))
      || urls.some(u => !keys(u, ["documentId", "ursprungHash", "zielUrl", "begruendung"])
        || !r.quellenIds.includes(u.documentId) || !text(u.begruendung))) fail();
    ...
    const docs = r.quellenIds.map(id => {
      ...
      const doc = structuredClone(found[0]), url = urls.find(u => u.documentId === id);
      if (url) {
        if (url.ursprungHash !== hash(found[0])
          || !require("./lage-quellenbeleg").artikelUrl(url.zielUrl)) fail();
        const host = s => new URL(s).hostname.toLowerCase().replace(/^www\./, "");
        if (host(url.zielUrl) !== host(doc.url || doc.canonical_url)) fail();
        // Nur die ausdrücklich belegte Adresse derselben Publikation in der
        // Ausgabekopie berichtigen. Quelleninhalt und gespeicherte Zeile bleiben.
        doc.url = doc.canonical_url = url.zielUrl;
      }
      return doc;
    });
  ...
  return { kos: nextKos, sourcesByVorgang: nextSources, korrekturHash: hash(korrektur),
    ...(priorisierung ? { priorisierung: structuredClone(priorisierung) } : {}),
    umfang: { eingeleseneVorgaenge: kos.length, vorherSichtbar: sichtbar.length, entwuerfe: entwuerfe.length,
      ...(mitErgaenzungen ? { ergaenzteVorgaenge: [...ergaenzungen] } : {}),
