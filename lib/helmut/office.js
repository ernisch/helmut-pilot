...
// Landtag-Sprint (P2-3): Parlaments-Registry fuer den Template-Kontext (rede_art etc.).
...
// V3: Die Anlass-/Kontextfelder fallen robust auf die display_*-Felder zurück,
// damit der Büro-Entwurf einen konkreten Anlass + Quelle bekommt (nicht "Aktueller
// Vorgang"). Ein KO trägt display_* ODER die Prosa-Analyse — beides wird genutzt.
// Landtag-Sprint (P2-3): optionaler Parlamentskontext (profile) fuer die Templates —
// NUR Parlaments-Metadaten (Name/Redeart/Regierung), KEINE Personendaten. Ohne Profil
// greifen die Template-Defaults (Bundestag, bisheriges Verhalten).
function buildOfficeContext(channel, ko, profile = null) {
  const titel = String(ko.display_title || ko.headline || firstSentence(ko.was_ist_passiert, 120) || "").slice(0, 300);
  const parlament = resolveParlament(profile || {});
  return {
    channel: String(channel),
    mandat: {
      parlament: parlament.name,
      rede_art: parlament.persona.redeArt,
      regierung: parlament.regierung.name
    },
    vorgang: {
      titel,
      // headline bleibt als Template-Feld, jetzt mit Titel-Fallback (kein Leerwert).
      headline: titel,
      was_ist_passiert: String(ko.was_ist_passiert || ko.display_summary || "").slice(0, 800),
      warum_wichtig: String(ko.warum_wichtig || ko.why_relevant || "").slice(0, 500),
      wer_ist_betroffen: String(ko.wer_ist_betroffen || "").slice(0, 400),
      zeitdruck: String(ko.zeitdruck || "mittel"),
      handlungsempfehlung: String(ko.handlungsempfehlung || ko.recommendation || "").slice(0, 400),
      // Quelle als Anlass-Beleg (Direktlink, wenn vorhanden) — nur oeffentliche URL, keine PII.
      quelle: String(ko.best_source_url || "").slice(0, 500),
...
async function generateOfficeOutput(userId, vorgangId, channel, ko, deps = {}) {
  // deps.profile (optional): Mandatsprofil fuer den Parlamentskontext der Templates.
  ...
  const ctx = buildOfficeContext(channel, ko, deps.profile || null);
  let prompt;
  try {
  ...
  // KI-Call — meta traegt callType + vorgangId + politicianId (= Mandant, wie in
  // allen anderen Nutzerpfaden; Review-Fix: ohne die Zuordnung liefen Buero-Calls
  // als "mandantenlos" ins Kostenlog, per-Mandant-Budget und Admin-Aufteilung
  // sahen sie nicht). KEIN Prompt-Inhalt im Log.
  let content;
  try {
    content = await ai.requestText(prompt, { callType: "office-output", vorgangId, politicianId: userId });
  } catch (error) {
    // Budget-Stopp der Choke-Point-Reservierung: ehrlich als Budget-Skip
    // ausweisen (wie das Rate-Gate oben), NICHT als KI-Fehler — der Client
    // unterscheidet die beiden Zustaende in der Anzeige.
    if (error && error.code === "LLM_BUDGET_EXHAUSTED") {
      return { skipped: true, reason: "budget-denied" };
    }
...
module.exports = { OFFICE_CHANNELS, isOfficeEnabled, isValidChannel, generateOfficeOutput, buildOfficeContext };
