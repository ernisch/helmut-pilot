...
Pilotprofil (Blob **und** `mandate_profiles` identisch: `partei`/`fraktion` = Partei des Pilotmandanten), 195
...
- `vg-angleichung` rankt aber **Platz 84 von 130** in der Profil-Relevanz des Piloten (similarity 0,0538) — es
  geht um Polizei-/Feuerwehr-Zulagen, kaum Überschneidung mit dem Arbeit/Soziales/Rente-Profil des Piloten; nur
...
Beide betreffen **alle** Parteien/Fraktionen, keine Pilot-Sonderregel. Sie ändern die aktuelle Anzeige des Piloten
...
geändert, weil das den sichtbaren Ausschüsse-Reiter des Piloten verändern würde (1 → 12) und der angefragte Scope
...
Previews ist zu erwarten: **Der Partei-Reiter bleibt für den Pilotmandanten korrekt leer** (die Ursache — der
Top-50-Relevanz-Cut von `vg-angleichung` — ist unverändert; B1/B2 ändern die Anzeige des Piloten nicht). Prüfung:
