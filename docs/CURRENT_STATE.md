...
## 15 · Letzter Sprint (25.08. — Skalierung 25/50/100, Korrekturrunden 4 und 5)

**Sprintzustand: teilweise abgeschlossen.** Technische Vorbereitung, synthetischer Lastnachweis und die Korrektur von dreizehn bestätigten Widersprüchen sind erbracht; der realistische Nachweis (Z3) und jede Aktivierung bleiben offen und freigabepflichtig. Branch `claude/helmut-scale-25-50-100-t6fcj6`, **PR #270 (offen, nicht gemergt)**, Ausgangscommit `24a895ed`. **Keine Production-Änderung, keine Migration angewendet, keine Aktivierung, kein Import.**

**Status (verbindlich):** Werkzeuge und Änderungssatz **lokal vollständig belegt** · synthetischer Motorlasttest 25/50/100 **bestanden** · **Z1 weiterhin teilweise/offen** · **Z3/Z4/Z5 nicht erreicht, keine Production-Freigabe**. Kanonische Doku: [`betrieb/skalierung-25-50-100.md`](betrieb/skalierung-25-50-100.md) (§0.2 = Korrekturrunde 4, §0.3 = Korrekturrunde 5).

**Runde 4 — sieben Widersprüche behoben, zwei Ausgangsbefunde widerlegt:**

- **Abflussplätze:** **drei** reguläre Warteschlangenabflüsse/Tag (crawl 04:00, pipeline 16:00, crawl 20:00 UTC) — nicht elf; die 11 war die Zahl der Cron-**Einträge**. Der GitHub-Watchdog ist ein **bedingter Ersatzlauf** und zählt nicht als Kapazität. Testgesichert (§2a).
- **Anlage und Aktivierung technisch getrennt:** der Stapelpfad legte Mandate **aktiv** an und umging die Freigabe. Jetzt: kein stiller Vorgabewert (`buildProfile`/`provisionTenant` **werfen**), der Stapel legt ausschließlich **inaktiv** an, ein Aktivierungswunsch wird **abgelehnt statt umgedeutet**, ein bestehendes Mandat behält seinen Zustand in **beiden** Richtungen. **Kein Aktivierungspfad gebaut.** (§4.5a)
- **Indexfrage entschieden — besserer Hebel gefunden:** `helmut_job_ankunft` liest immer die **ganze** Tabelle. Nicht der Index fehlt: die **Form** der Abfrage (CTE-Join statt fester Zeitgrenze) sperrt einen **vorhandenen** Index aus. Inline gerechnet Faktor 2,5 bei **0 MB**, ein `created_at`-Index derselbe Faktor bei **18 MB**. **Kein Index, F9 unverändert.** Die 500-MB-Free-Grenze reißt ohnehin vorher (2,70 MB/Tag; heute 160 MB belegt); Ursache ist die fehlende Aufbewahrung (R5). **Selbst gefunden und behoben:** die erste Messung lief gegen ein Teilschema (7 statt 10 Indizes). (§4.9)
- **Zurückgenommen:** zu weite Aussagen zu Datenbankverbindungen (§4.3a) · der Alleinvertretungsanspruch von `skalierungsmodell.js` — es gibt **drei** Modelllinien (§2c) · 8/12/20 Läufe sind eine **Hochrechnung**, kein bewiesener Bedarf. **Widerlegt:** eine offene „Zweitquellenanforderung" der 20 BE/BB-Profile und eine Liste von „80 weiteren Kandidaten" (§8.0).

**Runde 5 — sechs verbliebene Widersprüche, alle gegen Code, Konfiguration oder Production geprüft:**

- **KI-Deckel widerspruchsfrei:** §4 sagte „100/30 live, reicht ab 25 nicht", §6 gleichzeitig „nicht nachprüfbar, 25 offen". Jetzt überall einheitlich: **100/30 sind dokumentiert, nicht live verifiziert**; **für 25 ist die Tragfähigkeit offen und muss gemessen werden**; **ab 50 reicht 100 in beiden Modelllinien nicht**. Reserve und Limit bleiben getrennt (nie 130; ohne Variable Schutzlimit 50). `kapazitaetsmodell-test.js` nennt den Deckel nicht mehr als bewiesenen bindenden Grund gegen jede Stufe ab 25.
- **Narrativslots berichtigt:** „alle drei sind inert" war **falsch**. `/api/cron/lage-briefing` (05:45) läuft bei ausgeschalteter Narrativwarteschlange über seinen **Direktpfad** weiter; inaktiv sind allein die **zwei** `lage-briefing-nachlauf`-Einträge. Keiner der drei ist ein **allgemeiner** Abfluss (typgebunden `tenant_narrative`) — **drei reguläre Abflussläufe bleibt korrekt**. Der Wächtertest prüft die Unterscheidung jetzt einzeln (Mutationsprobe: vier Riegel werden rot).
- **Migrationen eindeutig:** auf `main` offen ist **nur `20260720`**; **PR #270 bereitet zusätzlich F9 vor** — weder gemergt noch angewendet (§3, §12).
- **Profilzahlen getrennt** (rein lesend bestätigt): **9 Mandatsprofile** (5 aktiv), **10 Identitätsprofile** (1 ohne Mandatsprofil), **0 Testmandate**. Ein Identitätsprofil ist kein Mandat.
- **PostgreSQL-Versionslücke geschlossen:** die Datenbanktests liefen gegen 16.13, Production fährt **17.6**. Die drei relevanten Suiten laufen jetzt zusätzlich gegen eine lokale, isolierte **PostgreSQL 17.6** — gleicher Plan, gleicher Faktor 2,5, gleiche 18 MB. Der Indexbefund ist damit **auf Production übertragbar** (§4.9a).
- **F10 zurückgestellt:** optionale Optimierung · **nicht blockierend** · für den 7-Tage-Nachweis **nicht erforderlich** · erst bei **gemessenem Bedarf** · nach einer F9-Anwendung nur über eine **neue reguläre Migration**. Die frühere Reihenfolge „F10 vor F9" ist aus Bericht, PR-Beschreibung und §9a entfernt.

**Tests (Endwerte):** Gesamtlauf **281/281 Suiten grün, 0 FAIL**. Gegen echte lokale PostgreSQL **17.6** (Production-Hauptversion): Lasttest **60 PASS** (2×), Ankunftskennzahl **30 PASS**, Indexnachweis **36 PASS** — dieselben Zahlen zusätzlich gegen 16.13. Einzeln: `provision-stapel` **117**, `kapazitaetsmodell` **56**, `warteschlangen-abfluss` **31**. In keinem Testkindprozess war eine Production-Kennung sichtbar.

**Unverändert:** Production, Schema, Migrationen (F9 **nicht angewendet**), Vercel-Konfiguration, Flags, Crons, KI-Budget, Profile (keine importiert, keine aktiviert). Kein Merge, kein Deployment, kein manueller Cron-, Worker-, Watchdog- oder Gesundheitslauf. Production wurde ausschließlich **rein lesend** und **aggregiert** berührt; kein `EXPLAIN ANALYZE` gegen Production.
