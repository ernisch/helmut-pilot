...
- **`main`-Kopf (04.09.): `350d9015576c724711179322dddd6063fcf02fd8`** — Merge von **#300**, Eltern `92a0716` (alter Kopf) und `33154b4` (PR-Kopf). Zugehöriges Production-Deployment **`dpl_DFHbTQo5T4fButYGbEXUHgzSsfnP`, Status READY** (target `production`, `githubCommitSha` = Merge-Commit) — rein lesend bestätigt. Nach weiteren Merges rückt der Kopf vor; maßgeblich ist dann die Git-Historie.
...
- **Zuletzt gemergt: #300** (reine Doku zu §28, kein Code); letzter **fachlicher** Merge war **#297**. #275–#277 und #282 nach Konsolidierung **geschlossen, nicht gemergt** (Branches bleiben Auditbelege).
- **Offen: der Absicherungs-Code-PR** (§29) — Branch `claude/helmut-stufe-a-storage-protection-pd5i3x`, Basis `350d901`, **erster fachlicher PR seit #297**. Er ändert Anwendungscode und Tests, **keine Konfiguration, keine Migration, keine Production-Daten**, und ist **nicht gemergt**; bis dahin gilt die Sperre aus §7.11.
...
- **Crawl-Aufbewahrung:** `HELMUT_CRAWL_RUN_RETENTION=36` (Betreiberangabe, aus dem Code nicht belegbar). Der Blob-Ring `crawlRuns` steht seit 04.09. auf **20** (§28) und **füllt sich nicht nach**: seit 23.08. erreicht kein Cron mehr `saveCrawlRun` (SR §37.3). Stillgelegter Altpfad-Puffer; kein Entscheidungspfad liest über Position 20 hinaus. **Der Schutz gegen erneute Kürzung ist gebaut, aber NICHT in Production** (§29).
...
5. **20 zusätzliche Profile**: isolationsbelegt, **Aktivierung gesperrt** (§7.11). **Berliner Wahl 20.09.2026**: die zehn Berliner Profile gelten nur für die 19. WP — Terminrisiko für den 25er-Nachweis.
...
11. **(04.09.) — Profil-Schreibpfad: Absicherung gebaut, aber NICHT in Production.** Der Code-Sprint ist offline abgeschlossen (§29, Vollbeleg SR §38) und liegt als eigener PR vor. **Aktivierung der Stufe A, Stufe B und Stufe C bleiben gesperrt, bis dieser PR gemergt und deployt ist** — der laufende Production-Code trägt die Absicherung noch nicht. Danach bleibt eine **eigene** Aktivierungsfreigabe nötig. **Weiter offen, bewusst nicht angefasst:** die geteilte Zeile `main` wird unbedingt und vollständig ersetzt (**Last-Write-Wins ohne Compare-and-Set**) — Architekturschritt SR §37.5 (7), eigener Nachweis nötig.
...
2. **Erst danach Schritt 7: die acht Betreiberwerte setzen** (Werte: §23–25) — Betreiberaktion an der Vercel-Env, danach Riegel- (8) und Wirksamkeitsprüfung (9). Die **Aktivierung** (10) braucht eine **eigene** Freigabe. Plan: `node scripts/lokal.js -- node scripts/funktionstest-500-ablauf.js plan`.
...
4. **Gründerentscheidung Skalierung** (§6): KI-Deckel, AWS, Reihenfolge 10 → 25.
...
Vollbeleg: [SR §36/§37](betrieb/500-funktionstest-sicherheitsrahmen-2026-09-01.md). Als **#300** gemergt (Kopf und Deployment: §2); kein Anwendungscode, keine Production-Daten — unmittelbar danach rein lesend nachgeprüft (29/5/24, Stufe A 20/20 inaktiv, 0 Löschmarken, `helmut_jobs` 7205, `llm_used` 68, `crawlRuns` 20, `md5` der Zeile `main` **unverändert**). Die spätere Änderung dieser Zeile (16:04 UTC) stammt vom **regulären 16:00-Pipelinecron**: 223 Aufträge, **0 Kohortenaufträge**, **0 Profiländerungen**; jüngster `crawlRun` weiterhin vom 03.09.
...
Vollbeleg: **[SR §38](betrieb/500-funktionstest-sicherheitsrahmen-2026-09-01.md)**. Erfüllt SR §37.5. **Kein Production-Schreibvorgang** (nur `SELECT`), keine Umgebungsvariable, keine Migration, kein Merge, kein Deployment.

- **Wurzelursache beseitigt:** `compactStore` verkleinert `crawlRuns` **in keiner Konfiguration** mehr; die Aufbewahrung durchzusetzen ist allein Sache von `saveCrawlRun`, dem einzigen Schreiber. Eine wirksame Grenze liegt nie unter dem Lesefenster (20), sonst schaltet sich der Google-Cooldown **still ab**. **Preis:** eine gesenkte Aufbewahrung wirkt erst beim nächsten `saveCrawlRun` — heute vorerst gar nicht (SR §37.3).
- **Eine Wahrheit:** neues Modul `lib/helmut/speicherpfad-vorflug.js` (reine Logik); `storage.js` liest die Regel von dort, die Zweitwahrheit in `op25-production-nachweis.js` ist geschlossen (fail closed ohne belegte Aufbewahrung).
- **Vorflug-Riegel** in Kohorten-Provisionierung, -Aktivierung, -Entfernung **und `provision-tenant` gegen Production**: Exit 2 **vor dem ersten Schreibvorgang**, wenn Aufbewahrung, Backend-Kohärenz, Zeilenkennung oder wirksamer Profil-Schreibmodus fehlen — ein echter Mandantenvorgang kann nicht mehr **still blob-only** schreiben (Runbook mitgezogen). Nur eine **tatsächliche** Abweichung der Zeilenkennung blockiert; ein ausdrücklich auf die Vorgabe gesetzter Wert gilt als unverändert. **Bewusst ohne Riegel: der Rückweg** (Notbremse, darf nie an einer Vorbedingung scheitern; die Nacharbeit fasst `main` ohnehin nie an). `blob-purge`/`reverse-backfill` verlangen bereits hart `profileDbModeEnabled()` und bleiben reine Blob-Werkzeuge.
- **Umgehungsweg in `provision-tenant.js` geschlossen.** Riegel und Ausführung hatten **zwei** Einstufungen: die Prüfung stieg bei `--validate` aus, die Ausführung prüfte erst `--deactivate`/`--teardown`/`--paket` und `--validate` zuletzt. So erreichten u. a. `--validate --deactivate` und `--validate --paket --ausfuehren` einen echten Schreibpfad **ohne Riegel** (reproduziert). Jetzt bestimmt **eine** Funktion den wirksamen Vorgang, die Ausführung schaltet auf ihr Ergebnis, `--validate` kann keinen schreibenden Hauptmodus entwerten, und widersprüchliche Aufrufe enden **vor dem `require` des Provisionierers** mit Exit 2 — reihenfolgeunabhängig.
- **Schreibziel ausgewiesen:** Werkzeug · Stufe · Profilanzahl · Aktivierungsstatus · Blob-Backend · geteilte Zeile und Kontenzeile · beide Schreibmodi · wirksame Aufbewahrung. Die Meldung nennt **keine Zahl** als aktuellen Production-Wert.
- **Beide Werkzeugbefunde geschlossen:** `realeMandateBeruehrt`/`loeschtNichts` **ersatzlos entfernt** (ein Ersatzfeld wäre strukturell invariant, also wieder eine Behauptung); vier tautologische Tests prüfen jetzt Verhalten. `erhebungsSql()` liest die Adresse aus dem Auth-Blob statt aus `profiles.email` — rein lesend gegengeprüft: **20/20 `.invalid`**, vorher 20 Leerwerte.
- **Tests:** Offline **319/319**, Browser-Smoke **32/0**, neue Suite `speicherpfad-schutz-test.js` **115/115** — darunter der Stufe-A-Nachweis über alle **20** Kennungen der verbindlichen Stufendefinition und ein systematischer Durchlauf **aller 64 Argumentkombinationen** von `provision-tenant.js` (0 Schreibaufrufe; eine breitere Gegenprobe über 256 Kombinationen fand gegen den Stand davor 64 Lecks, jetzt 0).
