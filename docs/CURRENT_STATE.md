...
> **Stand 2026-07-27 (Hotfix-Sprint):** B4-3 ist **im Code behoben** und offline belegt —
> siehe §3, Zeile „Hotfix Vorgangsbildung B4-3". **In Production ist der Defekt weiterhin
> aktiv**, weil der Fix nicht gemergt und damit nicht deployt ist. Vor jedem weiteren
> Nachhollauf müssen Merge und Deployment erfolgt sein.
...
|---|---|---|
| **NEU** | **Hotfix B4-3: Beweisfamilien im Resolver** — Flexionsformen zählen einmal, generische Ereignisbegriffe tragen nichts, Ein-Dokument-Vorgänge sind fail-closed; Magnet-Analyse erkennt die Übernahme; `--ids` vor der Mengenkappung; Production-Schreibgate | **mergefähig, aber Merge = Deployment.** Reine Logikänderung, keine Migration, kein Flag, kein Schema. Rollback über `git revert` bzw. Vercel-Redeploy. Offline **166/166**, Smoke **32/32**, read-only Production-Vergleich vorhanden. **Ohne diesen Merge bleibt B4-3 in Production aktiv** — vor jedem weiteren Nachhollauf zu deployen. Branch `claude/hotfix-b4-3-vorgangsbildung-hzn92o` |
