...
*Teilweise abgeschlossen — offline bewiesen, keine Production-Wirkung.* Branch `claude/helmut-pilot-post-pr295-dveogp`, Basis `9079ac3`. Vollbeleg: [Sicherheitsrahmen](betrieb/500-funktionstest-sicherheitsrahmen-2026-09-01.md) **§22–§31**.

- **Nach-Merge-Beleg vollständig (§22):** #295 gemergt 02.09. 13:08:53 UTC, Merge `9079ac3`, beide Pflicht-Checks grün, Vercel `dpl_DHTnMxFsibaj3XxdkpgDzandursx` READY `production` exakt darauf. Rein lesend nachgezählt: 9/5/4 Mandats-, 10 Identitätsprofile, **0 synthetische Zeilen**, 35 Migrationen, 13 Crons — unverändert.
- **BETREIBERENTSCHEIDUNG 02.09. umgesetzt — das Startfenster-Tor prüft FÄLLIGKEIT (§30):** Maßgeblich ist, ob der Motor einen Auftrag nach `due_at <= jetzt` beanspruchen kann; die bisherige **Schnittmenge** mit dem Streuintervall beschrieb den Motor nicht. Neu `funktionstest-faelligkeit.js`: ruft die **echte** `planeMandatsarbeit` mit den **Eingaben von Production** auf (Rotationsrang), liefert **sieben Kennzahlen je Klasse**, datums- und kohortengenau, fail closed. **Kein pauschales „100 % fällig" eingebaut.**
...
- **Mitternacht — eine Annahme bestätigt, eine widerlegt (§30.5):** Bestätigt ist die harte Grenze: Planung **nach** 00:00 UTC legt die Fälligkeiten auf den Folgetag, dann sind im Nachtfenster **0,0 %** beanspruchbar (gemessen 00:30/02:00). Widerlegt ist „100 % ist selbst eine Annahme": bei Planung **vor oder zum Fensterbeginn** behalten die Aufträge ihre `due_at` (`on conflict do nothing`) und bleiben beanspruchbar — die 100 % sind gerechnet. Entscheidend ist der **Planungszeitpunkt**; als harte Startbedingung im Code.
- **Blocker 1 entfällt für das Nachtfenster, sonst nicht (§30.6).** Die alte Aussage bleibt richtig, die Prüfung war die falsche. 0,0 % und 55,2 % sind keine Kohortenabdeckung — das Tor verwirft beide Fenster weiter, jetzt mit einer Zahl. **Warum das Nachtfenster trägt (§30.4a):** es beginnt am **Ende** der Briefingphase, dort ist jeder Auftrag fällig — **unabhängig vom Rotationsrang** (100 % über fünf Rangkarten gemessen). Das Abendfenster schwankt dagegen zwischen **35 und 60 %** (Stufe A).
- **Blocker 2 entfällt NICHT durch das Tor (§30.7).** Im Nachtfenster (383 min, Parallelität 1) sind **2.522 ≥ 1.812** — es passt durch die **Fensterwahl**, die das Tor erst zulässt. Neu getrennt (`lastTrennung`, 500 Mandate konservativ): Warteschlangenarbeit **802** · nutzergetrieben + eigene Crons **1.000** · andere 10 = **1.812**; Deckel 2.416, Reserve 604. Die **802 ist keine Fenstergröße** — die nutzergetriebene Last zehrt vom selben Deckel.
- **Eigener Befund ZURÜCKGENOMMEN (§29):** §24.1 behauptete, die 1.000 mandatsgebundenen Aufrufe könnten im Fenster nicht entstehen. Zahlen richtig, **Begründung falsch** (dominiert von `communicationDraft` **ohne Fensterbindung**). **812 darf NICHT als Fenstergröße dienen**; die 802 tritt **nicht** an ihre Stelle.
- **Was weiterhin fehlt (§30.8):** Fälligkeit ist berechenbar, **Status nicht** — der 20:00-Crawl treibt dieselbe Warteschlange an. Deshalb bleibt `vollstaendigerZyklus` **`null` = NICHT BEWERTBAR**, bis die Zahl offener Aufträge rein lesend erhoben ist; `erhebungsSql()` liefert die Abfrage (nur `select`, mit Frischefenster- und Stufenfilter). **Nicht gemessen:** Laufzeit der KI-freien Klassen (990 Aufträge); die **vollständige Rangkarte** liegt dieser Sitzung nicht vor.
- **Drei von vier Schutzgrenzen sind NICHT hart (§23.4):** wirksam ist allein der Tagesdeckel (atomar); **RPM/TPM liest kein Ausführungspfad**, das Kostenbudget wirkt nur entdeckend. **Zwei Lücken geschlossen (§23.3):** stufengenaue Freigaben (**15**) und ein **vollständiger Entfernungsweg** (CLI, sechs Riegel).
- **Zwei unabhängige Reviews, 24 Defekte im EIGENEN neuen Code, alle behoben (§31).** **Blockierend:** die Teardown-Korrektur aus §28 wirkte in Production **gar nicht** — `readSupabaseStore` legt eine fehlende Zeile **beim Lesen** an; die 400 Zeilen wären weiter entstanden, dokumentiert als „behoben". **Hoch:** das Tor rechnete **ohne Rotationsrang**, Production plant **mit** (Stufe A abends 80,0 statt 60,0 %) · die Stufenhürde verlangte **fünf Freigabeworte gleichzeitig** aus einer Ein-Wort-Variablen und konnte nie grün werden · der `gelesen`-Fix machte eine **erklärte** Parallelität 2 zum Entscheider und kippte beide Fenster auf grün (jetzt nur mit `parallelitaetBelegt`) · die Erhebungsabfrage zählte **fremde Frischefenster und Stufen** mit · `mindestAbdeckung` koerzierte `null`/`0`/`""` zur Schwelle **0** · fehlende **Erlaubnisliste** und **Duplikatsperre** (§4.2). Dazu zehn kleinere (`Number([])`, ein trivial wahrer Test, drei neue Fundstellen des realen Mandats-Slugs). Neue **zwölfte Startbedingung** `rotationVollstaendig`; R4/R5 von Quelltextregex auf **Verhalten** umgestellt.
- **Fünf weitere Defekte aus dem ersten Sprintteil (§28, §30.3):** ein **Teilfehler** galt als „entfernt" · CLI-Exitcode **0** über gescheitertem Lauf · `Number(null)` als Planungszeitpunkt · escapen statt **abweisen** · das unbedingte Zurückschreiben (siehe oben). Nicht abgeräumt: `helmut_jobs`, Outbox, `crawl_runs`, Personenquellen-Rohdaten.
- **Schutz der fünf realen Mandate (§25.2):** Verdrängungsschutz **gebaut, nicht wirksam** — `HELMUT_TESTLAUF_VORRANG_REAL` ungesetzt, Reserve **0**; bei 0 synthetischen Zeilen eine Vorbedingung, kein Risiko. **05:45/05:48 (§26):** um 05:48 läuft nichts, kleinster Cronabstand **10 min**. **Telemetrie (§27):** Ring 5.000 trägt **einen** Testtag je Stufe; ein Lost Update bleibt unsichtbar. „Parallelität 2" unterbestimmt, Weg (c) nicht empfohlen (§25.1/§25.3).
- **Kein Fenster gewählt. Unverändert:** 5 reale Mandate, Gate `shadow`, 13 Crons, 35 Migrationen, alle Daten und Variablen, keine Migration angewendet.
