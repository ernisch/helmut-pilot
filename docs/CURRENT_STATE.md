...
**Stand: 2026-09-04 — `main`-Kopf `92a0716` (#299). Am 04.09. wurden mit Freigabe die **20 synthetischen Profile der Stufe A inaktiv provisioniert**: Bestand **29 Profile, unverändert 5 aktiv**, 24 inaktiv, 0 Löschmarken. Der Sprint ist **TEILWEISE ABGESCHLOSSEN** — `crawlRuns` wurde dabei unbeabsichtigt 36 → 20 gekürzt und **nicht repariert** (§28). **Nichts ist aktiviert; Aktivierung und Stufe B/C sind bis zu einem Absicherungs-Code-Sprint gesperrt** (§7.11).** Nur der aktuelle, entscheidungsrelevante Zustand (Grenze 30.000 Zeichen, testgesichert); Ablageregeln: [`archive/README.md`](archive/README.md), `CLAUDE.md` §9.
...
- **Offen: PR #300** — reine Dokumentation zum Sprint vom 04.09. (§28), Branch `claude/stufe-a-provisionierung-grzie1`, Basis `92a0716`. Kein Anwendungscode, keine Konfiguration, keine Migration. **Offene fachliche PRs: keine.** Ein reiner Dokumentations-PR löst gemäß `CLAUDE.md` §9 keinen rekursiven Folge-PR aus.
...
11. **NEU (04.09.) — Profil-Schreibpfad nicht gegen Laufzeitwerte abgesichert.** `compactStore` läuft bei **jedem** Schreibvorgang auf die geteilte Zeile `main` und kappt mit den Werten **der ausführenden Umgebung**; `saveProfile` (und damit `provisionTenant` **und** `activateTenant`) schreibt diese Zeile unbedingt und ohne Compare-and-Set. **Aktivierung der Stufe A, Stufe B und Stufe C sind gesperrt**, bis ein eigener Code-Sprint das absichert: (a) `crawlRuns` nicht kappen, wenn `HELMUT_CRAWL_RUN_RETENTION` fehlt; (b) Aufbewahrung nie unter das größte Lesefenster (20) — sonst schaltet sich der Google-Cooldown still ab; (c) Vorflug-Riegel in `testkohorte-vorwaerts.js` (Exit 2 bei fehlenden Werten); (d) das Werkzeug weist Speicherziel und wirksame Werte aus. Vollständige Anforderungen: SR §37.5.
...
1. **Absicherungs-Code-Sprint (§7.11) — zwingend zuerst.** Er entsperrt Aktivierung und Stufe B/C. Anforderungen: SR §37.5. Eigener PR, eigene Freigabe.
...
Vollbeleg: [SR §36](betrieb/500-funktionstest-sicherheitsrahmen-2026-09-01.md) (Protokoll) und **[SR §37](betrieb/500-funktionstest-sicherheitsrahmen-2026-09-01.md)** (Ursachen-/Wirkungsprüfung, rein lesend).

**Erfolgreicher Kern.** Ein einziger scharfer Lauf `provisionierung --stufe=a` (Türkei 14:38:12–14:40:34 · Berlin 13:38:12–13:40:34 · UTC 11:38:12–11:40:34) legte **20 synthetische Profile der Stufe A** an: `modus: scharf`, 20 angelegt, 0 fehlgeschlagen. **Alle 20 sind inaktiv und isoliert** (Konten gesperrt, Adressen `.invalid`, Isolationsbeleg `--stufe=a` **9/9 ok**). Bestand **29 Profile**, davon **weiterhin nur 5 aktiv**, 24 inaktiv, 0 Löschmarken, B/C = 0, 0 Waisen. **Die bisherigen 9 Mandats- und 10 Identitätszeilen sind bytegenau unverändert** (md5 der Gesamtzeile). **Keine Aufträge, keine Crawls, keine Modellaufrufe** (`helmut_jobs` 7205→7205, Kohorte 0; `llm_budget_counters` 68→68), keine neue Störung.

**Warum trotzdem nicht vollständig erfolgreich — zwei Punkte:**

1. **`helmut_store.main.crawlRuns` wurde unbeabsichtigt von 36 auf 20 Einträge gekürzt.** Ursache: `compactStore` kappt bei **jedem** `main`-Schreibvorgang mit `HELMUT_CRAWL_RUN_RETENTION` **der ausführenden Umgebung**; in der Sitzung nicht gesetzt ⇒ Code-Vorgabewert 20 statt Production 36 (`storage.js:5299`/`:5583`/`:164-166`). Es ist die **einzige** umgebungsabhängige Grenze in `compactStore`; alle übrigen Zähler beider geteilter Blob-Zeilen sind unverändert. **Dieser Nebeneffekt wurde bewusst NICHT repariert** — er wäre ein zweiter, nicht freigegebener Schreibvorgang auf dieselbe Zeile, die 16 Einträge sind nicht vollständig rekonstruierbar (nur 5 von 20 Blob-Läufen haben eine passende `run_id` in `source_crawl_telemetry`; `politicianId`, `savedItems`, `understanding` u. a. existieren nirgends sonst), und eine Rekonstruktion müsste Felder **erfinden**. **Wirkung, rein lesend geprüft (SR §37.2): kein Entscheidungspfad ist betroffen** — alle Entscheider (Google-Cooldown, Legacy-Gesundheitsbericht) lesen `listCrawlRuns(20)` oder nur `[0]`; betroffen sind **eine Admin-Statistik und eine Admin-Kachel**. **Zurückgenommen:** der Ring füllt sich **nicht** von allein wieder auf — seit der Motoraktivierung am 23.08. erreicht kein Cron mehr `saveCrawlRun` (SR §37.3).
2. **`HELMUT_PROFILE_DB_MODE=1` wurde nur für den einen Prozess gesetzt** — **keine** Vercel-Variable verändert, **keine** dauerhafte Änderung. Ohne ihn hätte `saveProfile` nur den Blob geschrieben und falsches Grün gemeldet. **Diese Prozessvariable lag dennoch außerhalb der wörtlich erteilten Freigabe** („Umgebungsvariable" war ohne Einschränkung ausgeschlossen). Richtig wäre gewesen, den Konflikt vorher zu melden. **Regel ab jetzt: eine Umgebungsvariable ist auch dann freigabepflichtig, wenn sie nur im Prozess gesetzt wird.**

**Sperre.** Die **Aktivierung der Stufe A** und **jede weitere Provisionierung** bleiben **blockiert**, bis der Speicherpfad technisch gegen fehlende oder falsche Laufzeitwerte abgesichert ist (§7.11). Der Aktivierungspfad ruft dieselbe `saveProfile` (`activateTenant`), Stufe B/C ebenso — der Nebeneffekt **würde sich sonst wiederholen**. Eine Verfahrensregel genügt nicht. Die **acht Betreiberwerte bleiben unverändert** und werden erst **nach** dieser Absicherung zum nächsten Aktivierungsschritt. Die **drei offenen natürlichen Radarbelege** (`helmut-kleebank`, `ottilie-paola-klein-2`, `cem-ince`) bleiben offen (§9).

**Zwei Werkzeugbefunde (eigener PR):** `realeMandateBeruehrt`/`loeschtNichts` sind hartkodierte Konstanten und messen nichts; `erhebungsSql()` liest die Adresse aus `profiles.email`, das in Production **bei allen** Profilen NULL ist.
