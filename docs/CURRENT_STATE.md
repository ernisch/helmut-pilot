...
**Teilweise abgeschlossen — Stufe 1 (Rückstandsschleife) deployt, Stufe 2 (Gate-Arm) lokal fertig.** Alle Messreihen + Arm: [`betrieb/understanding-kapazitaet-2026-08-31.md`](betrieb/understanding-kapazitaet-2026-08-31.md) (§12 = Arm). Production ausschließlich lesend vermessen (31.08.):
...
- **Stufe 1 (PR #284, gemergt 31.08. 08:58 UTC = `3244f073`, deployt):** Rückstandsschleife `/api/cron/understanding-rueckstand` (11:30/17:30 UTC), älteste zuerst, nicht priorisierte Buchung (Deckel−Reserve, atomar), Laufdeckel 20 + Budget-Boden 30 fail-closed, Slots selbstverankert überwacht; Suite `verstehen-rueckstand-test.js` 56/0, Offline-Gesamtlauf 285/287 (rot nur bekannte npm-Fehlstände, §19).
- **Stufe 2 (Gate-Arm, OP-18):** Prüfung ergab: Gate `on` war bisher **mechanisch wirkungslos** (nur Protokoll, `blockiert:0` hartkodiert; Pending-/Rückstands-/Lage-/Queue-Pfade ganz ohne Gate-Bezug) — Flag-Flip wäre falsches Grün, vom Betreiber untersagt. Stattdessen echter Arm: **eine** kanonische Prüfung in `understandOneCluster` vor Budget/CAS, Parkzustand `gate-geparkt` mit Beleg + `GATE_VERSION` + drei Rückwegen (Eingabeänderung, Wiedervorlage-Rotation, Admin-Freigabe), getrennte Zählung im Gesundheitsbericht, **keine Migration**. Schatten-Fehlerfreiheit erstmals über den Gesamtzeitraum belegt (109 480 Events, 0 amtlich/kuratiert geparkt). Suiten 48/0 + 19/0; Flag bleibt im Arm-PR **shadow** — Scharfschalten ist ein eigener Folgeschritt.
- **Offen:** natürliche Rückstandsläufe rein lesend prüfen (11:30/17:30 UTC), Arm-PR CI/Merge, Flag-Flip, Betreiberentscheidung Deckel (§8), OP-06 (1 769 Altfälle), Siebentagenachweis nicht begonnen.
