...
**Laufumgebungsprüfung Azure OpenAI**) ·
...
| **KI-Pfad in der Cloud-Sitzung nicht lauffähig** (gemessen 2026-07-27) | Die Azure-Ressource hinter `AZURE_OPENAI_ENDPOINT` (`helmut-openai-eu.openai.azure.com`) führt **0 Deployments** (`GET /openai/deployments` → `{"data":[]}`); jeder Aufruf über `ai.js` endet in `HTTP 404 DeploymentNotFound`. Zusätzlich ist `AZURE_OPENAI_DEPLOYMENT` in der Sitzung **nicht gesetzt**, `modelName()` fällt auf `gpt-5-mini` zurück. **Nicht** die Ursache: Netz (TLS/DNS zum Azure-Host grün), Schlüssel (Deployment-Liste antwortet `200`), Endpunkt-Schreibweise (kein Slash-Fehler), `isAiEnabled()` (`true`) | **Production ist nicht betroffen** (Vergleich 2026-07-27, nur lesend): der Kostenlog `helmut_store['main-auth'].llmUsage` zeigt für Production **35 erfolgreiche `gpt-5-mini`-Aufrufe allein am 2026-07-27**, letzter um **05:46:15 UTC**, **0 Fehler**; `Azure HTTP 404` gab es dort zuletzt am **2026-07-02** (10×, Einrichtungsphase). Der **effektive Deploymentname ist auf beiden Seiten `gpt-5-mini`** — die Sitzung erreicht ihn nur nicht. Da gleiches Paar (Endpunkt, Schlüssel) zwangsläufig gleiches Ergebnis liefert, zeigt die **Sitzung auf eine andere Azure-Ressource als Production**. **Ursache geklärt 2026-07-27:** das Deployment `gpt-5-mini` liegt nicht in `helmut-openai-eu`, sondern in der Foundry-Ressource **`helmut-resource`** (Projekt `helmut`, Sweden Central — EU). `ai.js:532` baut `${AZURE_OPENAI_ENDPOINT}/openai/v1/responses` **ohne `api-version`** und ist damit auf die **Azure-OpenAI-v1-Fläche** ausgelegt; der Hostname ist dem Code **egal**, erwartet wird ausschließlich die **Ressourcen-Basis-URL ohne Pfad und ohne abschließenden Slash** (ai.js kürzt nicht, anders als `server.js:6510`). Alle drei DNS-Namen von `helmut-resource` (`…services.ai.azure.com`, `…openai.azure.com`, `…cognitiveservices.azure.com`) zeigen auf dieselbe Ressource. **Nicht** verwendbar ist der Foundry-**Projekt**-Endpunkt (`…/api/projects/helmut`) — er ergäbe `/api/projects/helmut/openai/v1/responses`. Minimal nötig in den Claude-Code-Environment-Einstellungen: `AZURE_OPENAI_ENDPOINT` auf die Basis-URL von `helmut-resource` **und** `AZURE_OPENAI_KEY` auf deren Schlüssel (der vorhandene Schlüssel gehört zu `helmut-openai-eu` → sonst 401). `AZURE_OPENAI_DEPLOYMENT` kann leer bleiben (Default = `gpt-5-mini`). **Production bleibt unberührt.** Die Vercel-Env-Werte sind aus der Sitzung weiterhin nicht lesbar (Vercel-MCP hat kein Env-Werkzeug) |
...
|---|---|---|
| **Laufumgebungsprüfung Azure OpenAI (nur Diagnose)** | 2026-07-27 | **Blockiert — erster tatsächlicher Blocker: die Azure-Ressource führt 0 Deployments.** Geprüft wurde ausschließlich die Laufumgebung: `AZURE_OPENAI_ENDPOINT` = Host `helmut-openai-eu.openai.azure.com`, **ohne** abschließenden Slash, Pfad `/`, `https` — die in `ai.js:531` gebaute URL `…/openai/v1/responses` ist damit korrekt. `AZURE_OPENAI_DEPLOYMENT` **nicht gesetzt** → `activeModelName()`/`understandingModelName()` liefern den Fallback `gpt-5-mini`. `isAiEnabled()` = **true** (Schlüssel + Endpunkt vorhanden; `OPENAI_API_KEY` nicht gesetzt). Netzpfad grün: DNS auflösbar, TLS-Handshake `authorized: true` (`*.openai.azure.com`). Minimaler Live-Aufruf über **genau denselben Codepfad** (`ai.requestText` → `requestOpenAI`) → **`Azure HTTP 404 DeploymentNotFound`**; Gegenprobe mit demselben Schlüssel: `GET /openai/deployments` → **`200 {"data":[]}`**, `GET /openai/models` → `200`. Schlüssel und Netz sind also gültig, es existiert schlicht **kein Deployment**. **Keine DB-Schreibzugriffe:** `storage` wurde vor dem Laden von `ai.js` im require-Cache durch einen Stub ersetzt, damit `reserveLlmCall` (Budget-Reservierung) und `recordLlmUsage` (`llm_usage`) die Datenbank nicht erreichen — beide Aufrufe wurden abgefangen und protokolliert, nicht ausgeführt. Kein Code geändert, kein Cron, kein Flag, keine Migration. Diagnoseskript nur im Scratchpad, nicht im Repo. **Nachtrag Vergleich Production ↔ Sitzung (nur lesend, gleicher Tag):** Production ist **gesund** — 35 erfolgreiche `gpt-5-mini`-Aufrufe am 2026-07-27 (letzter 05:46:15 UTC, 0 Fehler), `Azure HTTP 404` dort zuletzt am 2026-07-02. Effektiver Deploymentname beidseitig `gpt-5-mini`; unterschiedlich ist folglich die **Azure-Ressource** (Endpunkt und/oder Schlüssel). Die Production-Env-Werte bleiben aus der Sitzung unlesbar. Blocker und nächster Schritt in §4 |
