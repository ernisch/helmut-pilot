...
| 2026-08-14 | **Sicherheitskorrektur OP-30-Zielarchitektur** (Belegdatei §17): (1) Route prüft das Wecksignal vollständig (Versand-Vertrag, 400/409); (2) Weckziel-Riegel — CRON_SECRET nie an ungeprüfte Ziele (16 adversariale Tests, 0 Netzaufrufe); (3) Aufrufverstärkung beseitigt (Türklingel-Bündelung, Timeout=unbestätigt, Slot-Freigabe vor Folgeklingel); (4) Migrationen auf eindeutige 14-stellige Stempel + `rollback_`-Präfix — **CLI-Nachweis**: die Altkonvention hätte Rollbacks als Vorwärtsmigrationen ausgeführt; Vertragstest-Harness awaited async-Fälle; (5) Route nach `/api/cron/worker-weck` verlegt (Zugriffs-Gates, CI-Befund, Belegdatei §17.6) | Auf PR #247 nachgeführt; neue Suiten: worker-weck-route (21), migrations-organisation (10); Production unangetastet |
