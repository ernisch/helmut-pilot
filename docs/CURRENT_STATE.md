...
- **Kreisschluss aufgelöst (§32/§33):** das Tor war falsch-negativ (ein erledigter Zyklus galt als gescheitert). Jetzt **sieben Statusmengen**, **zwei getrennte Urteile** (Fachzyklus · Lastbeweis); `startbereit` für alle drei Stufen erreichbar (A/B ab 143 min, C ab 276 min). **Kein neuer schreibender Planungscode** — der Bestandscron plant, sobald die Stufe aktiv ist.
- **Vier Blocker geschlossen** (Einzelbeleg §22–§33): u. a. das Deaktivierungswort für 20 hätte 495 freigegeben; `startbereit` war unerreichbar (jetzt belegt `true`). Messintegrität: `frischefenster`/`stufe` Pflicht, `mindestAbdeckung` ≥ 0,5, `--startbereit=ja` entfernt, Kapazitätshürde 25/100/500.
