...
- **Indexfrage lokal entschieden:** `helmut_job_ankunft` liest immer die **ganze** Tabelle (7 d 17 ms · 90 d 341 ms · 1 Jahr 1,6 s bei 2287 Aufträgen/Tag, echte PostgreSQL 16, Nutzlast auf die gemessenen 821 Byte geeicht). Ein Index brächte Faktor 756 — kostet aber 35 MB. **Kein Index, F9 unverändert:** kein heißer Pfad, und die 500-MB-Free-Grenze reißt vorher (bei 100 Mandaten **2,52 MB/Tag**; heute 160 MB belegt). Ursache ist die fehlende Aufbewahrung (R5), nicht der Plan. (§4.9)
...
- **Tests:** Gesamtlauf **281/281 Suiten grün, 0 FAIL**. Mit echter lokaler PostgreSQL zusätzlich: Lasttest 60 PASS (2×), Ankunftskennzahl 30 PASS, Indexnachweis 26 PASS. Einzeln: `provision-stapel` **117**, `kapazitaetsmodell` **53**, `warteschlangen-abfluss` **20**.
