...
**22.09.2026: Einmaliger Verstehenslauf fuer die169 gespeicherten Rohdokumente NUR VORBEREITET — kein Production-Lauf.** Neu: `lib/helmut/verstehen-einmalig.js` und Bedienweg `scripts/verstehen-einmalig-169.js`, fail closed und ohne zweite Fachlogik (Dedup, Clusterung, Vorgangsaufloesung, sameVorgang, neueErkenntnisse, CAS, Fencing, Budget-Gate und Restzeitwache bleiben die bestehenden Funktionen). Gebunden: Commit `ea84f26c…`, 169 Kennungen mit `idHash 5f387840…a2ed9`,122 Cluster, hoechstens113 Modellaufruf-Kandidaten, Laufdeckel0,80 USD und35 min innerhalb des unveraenderten4 USD-Tagesriegels, Aufruftyp `understanding-rueckstand`. Gezielte Tests65/65 gruen. Der **rein lesende Planlauf wurde ausgefuehrt** (Betreiber, Codespace Shell):169 Dokumente,169er Kennungsmenge, ID Hash exakt,122 Cluster, Groessen exakt110x1/5x2/2x3/1x4/2x8/1x11/1x12, Arten75 neu/20 update/19 pending-erst/8 merged, also114 Kandidaten, danach **fail closed `verstehen-kandidaten-ueber-deckel`**. Alle8 merged tragen `keine-neuen-fakten`; der fruehere eine `bestand-ohne-beleg`-Fall ist heute Teil der75 `neu` — welcher, ist noch nicht belegt. 0 Modellaufrufe, 0 Production Writes, 0 Quellenabrufe, 0 Profilwrites, 0 Kommunikation, keine Quittung. Der Abbruchpfad meldet die bereits berechneten Diagnosewerte mit, fuer `neu` zusaetzlich die Resolver-Spuren `vorgangId`/`gleich`/`grund` (nur Meldung; `ok`/`schutzvertrag` bleiben falsch,113 unveraendert). Ein scharfer Lauf bleibt gesperrt. [Beleg](betrieb/verstehen-einmalig-169-20260922.md).
