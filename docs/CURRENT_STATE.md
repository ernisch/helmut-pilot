...
9. **Zwei Testzeilen in Production** (24.08., §14) — **Zustand geändert am 25.08.**: Auftrag `371707a4…` wurde am 25.08. um **07:01 Uhr türkischer Zeit** (06:01 Berlin, 04:01 UTC) vom regulären Betrieb aufgenommen und ist nach fünf Versuchen **terminal `fehlgeschlagen`**; die Outbox-Zeile `24ba14ec…` steht auf `bestaetigt` (Transport `schatten`). **`endgueltig_fehler` ist damit tatsächlich um 1 verfälscht** — die frühere Aussage „nie beansprucht / heute nicht betroffen" gilt **nicht mehr**. **Noch nichts gelöscht, noch kein Production-Trockenlauf.** Der Einzeilenvertrag ist auf den Endzustand berichtigt; Trockenlauf und scharfer Lauf brauchen **je eine eigene Gründerfreigabe** (Runbook §31.8).
...
- **Zustand eingetreten (25.08., zweite Korrekturrunde):** um **07:01 Uhr türkischer Zeit** (06:01 Berlin, 04:01 UTC) hat der **reguläre Betrieb** die Testzeile aufgenommen; leere Nutzlast ⇒ endgültige Ablehnung ⇒ fünf Versuche in einem Slot ⇒ Auftrag **`fehlgeschlagen`**, Outbox **`bestaetigt`** (Transport `schatten`). Damit ist **`endgueltig_fehler` tatsächlich um 1 verfälscht**; „nie beansprucht" stimmt nicht mehr. Der Vertrag auf Commit `d4fa57d` hätte einen Trockenlauf **korrekt abgebrochen** (E7.1/E7.2/E7.7/E7.8, E2.7, E4.2/E4.5–E4.7) — **deshalb wurde nichts ausgeführt, sondern der Vertrag berichtigt** (zweite Fassung; Mengenverträge bytegleich unverändert, per SHA-256 belegt). Neu: der frühere Zustand `wartend`/`offen` wird ausdrücklich abgelehnt, und die Datensparsamkeit gilt jetzt **am Wert** (jeder sensible Vertragswert nur als rechte Seite seines eigenen Spaltenvergleichs). Nachweis: **61 PASS** (echte PostgreSQL 16). **Noch nichts gelöscht, kein Production-Trockenlauf** — einzige Berührung war die enge Leseprüfung (25.08. **07:48:39 türkischer Zeit** / 06:48:39 Berlin / 04:48:39 UTC), die jeden Vertragswert bestätigt. **Freigabe A ist jetzt neu prüfbar**, Freigabe B bleibt getrennt (Runbook §31.8). **Der Selbstweck bleibt deaktiviert.**
