...
**Letzte Aktualisierung:** 2026-07-31 (**Sprint OP-25 K2.2 — Production-Aktivierungsfreigabe für
den globalen Abruf. BLOCKIERT: die Aktivierung ist technisch und betrieblich vorbereitet, aber ein
zukünftiger regulärer Lauf und eine Betreiberprüfung fehlen. Rein lesend, keine Änderung an
Production.** **Entscheidung: `BLOCKIERT`, nicht `NO-GO`** — es fehlt nichts Gebautes, es fehlt
**Zeit und Reihenfolge**. **Deployment belegt:** `dpl_BkhKkPSAMEoYPzFbSQqUebkTSGzz`, Commit
`255df013` (= Merge-Commit PR #201), `target: production`, Region `fra1`, erstellt **14:44:56.862
UTC**, **`READY` 14:45:11.294 UTC**, `aliasError: null`, Alias `helmut-pilot.vercel.app`
zugewiesen; `project.latestDeployment` ist **dasselbe** Deployment — **kein neueres existiert**.
HEAD = `origin/main` = `255df013`, Arbeitsbaum sauber, und weil HEAD **der** Merge-Commit ist, hat
seither **kein Commit** die Flag- oder Pfadlogik berührt. **Kernbefund, der die Entscheidung
trägt:** dieses Deployment hat bis 15:1x UTC **keinen einzigen regulären Cron-Lauf** ausgeführt
(Runtime-Log: genau ein Eintrag, `GET / 200` um 14:45:46). Der nächste pfadrelevante Termin ist
`pipeline` **16:00 UTC**; nur `crawl` und `pipeline` betreten `cronSchwererPfad`. **Damit fällt der
erste reguläre Lauf des neuen Deployments mit dem ersten Lauf des neuen Pfades zusammen, sobald
aktiviert wird** — NO-GO-Kriterium 8. **Wirkung auf die drei offenen Nachweise, einzeln bewertet
(alle drei haben auf diesem Stand `0` vollständige reguläre Läufe):** **R-6** verlangt ≥ 24 h
Kadenz **mit mindestens einem Lauf am äußeren Zeitlimit** — genau diese Überziehung beseitigt K2.1
(gemessen n=6: alt **2/6 mit +37 585 ms** → K2.1 **6/6 ohne Überziehung**), der Nachweis wäre nach
einer Aktivierung **strukturell nicht mehr erreichbar**; zusätzlich fehlte bei einem Timeout
**innerhalb** der globalen Phase im Laufdatensatz `geplant`/`ausgaenge` (Prüfpunkt 2 verlangt
beides). **25B** wartet auf einen regulären Lauf des Pilotmandanten nach dem B25-2-Deployment
(PR #190, `bd7c889`, 10:43 UTC) — seither **0** Läufe; ein Lauf des neuen Pfades wäre kein sauberer
B25-2-Beleg, weil PR-#190- und K2.1-Wirkung nicht trennbar wären. **29B** misst in Punkt 1/2 genau
die Fairness-/Zeitbudget-/Lock-Größen, die K2.1 verändert — ein vorher begonnenes Fenster wäre
entwertet. **Flagbefund, ehrlich getrennt:** repositoryseitig **vollständig belegt** — Default AUS
und fail closed (nur `on`/`true`/`1`/`an`), beide Flaggen gleichzeitig → **Altpfad**
(`waehleCronPfad`, laut protokolliert), Aktivierung über `helmut-flags.json` **ausgeschlossen**
(`FILE_FLAG_ALLOWLIST` kennt nur `HELMUT_UNDERSTANDING_GATE`, `HELMUT_PARDOK_DISPATCH`,
`HELMUT_SOURCE_MODE`, `HELMUT_LANDESMODULE`; beide Cron-Flaggen lesen `process.env` direkt),
Mutationsprobe **M18 rot**. **Nicht belegbar** ist der tatsächliche Wert der Vercel-Production-Env
— das ist die bekannte Sechs-Kanäle-Grenze (§9): kein `VERCEL_TOKEN`, kein Env-Werkzeug im
Vercel-MCP, `api.vercel.com` proxy-gesperrt, App unauthentifiziert 401, und die
Admin-Konfigurationsdiagnose führt eine eingefrorene Whitelist **ohne** die beiden Cron-Flaggen.
**Auch der sonst übliche Ersatzbeleg „Wirkung statt Variable" trägt heute nicht** — der Code beider
Flaggen existiert in Production erst seit 13:17 bzw. 14:45 UTC, und seitdem lief kein Cron.
**Regressionsprüfung:** 50 Runtime-Fehlergruppen der letzten 7 Tage, **jüngster Zeitstempel
2026-07-31 10:00:30 UTC**, also **vor** dem Deployment — **kein einziges Vorkommen danach**, keine
neue Fehlerklasse; alles Bestand (Google-News-Timeout/503 · `[cron/*] Zeitbudget erschoepft` ·
`[understanding] OpenAI request timeout` · `Supabase storage timed out` · ein
`Vercel Runtime Timeout` mit letztem Vorkommen 2026-07-27, also vor der R-6-Behebung).
**CI gegen den aktuellen Commit verifiziert statt aus der PR übernommen:** Push-Lauf auf `main`
für `255df0133` = **`30639915579` `success`**; die Pflicht-Checks am gemergten Kopf `7b0a3e2`
stammen aus Lauf **`30639121215`** (beide `success`) — *die in PR #201 und in
`betrieb/vorgangskontext.md` genannte Laufnummer `30638964148` war überholt; die Aussage „beide
Pflicht-Checks grün" bleibt gültig.* **Lokal auf `255df013` nachgemessen (offline erzwungen):**
Offline-Suite **180/194 in 54 s** mit exakt den **14** umgebungsbedingten Fehlschlägen ·
`vorgangskontext` **102/102** · `cron-globalphase` **176/176** · `globalphase-buendelung`
**56/56** · `cron-fairness` **285/285** · `punkt29-fehlervertrag` **80/80** · `env-inventar`
**38/38** · Mutationsproben **18/18**, **17/17**, **15/15 rot, je 0 Löcher** — **alle Zahlen der
PR-Beschreibung reproduziert**. **Read-only gegen Production gemessen (~15:05 UTC, nur `SELECT`):**
**6 aktive Mandate, ausnahmslos `politische_ebene = bundestag`** (kein aktives Landtagsmandat,
**M8 unverändert inaktiv**) · **163** Abrufwege in unveränderter Verteilung · **9** Pakete ·
**alle 18 Landesmodul-Wege** (10 Berlin inkl. `rp-rbb24-politik`, 8 Brandenburg)
`needs_review` + `manual` — **Berlin und Brandenburg unverändert AUS**. `vercel.json` zuletzt
**2026-07-27** geändert (`d33f540`) → **Cron-Zeiten unverändert**; PR #201 fasst weder `flags.js`
noch `storage.js` noch einen Kostendeckel an → **Budgets unverändert**. **GO-Kriterien: 12 von 14
erfüllt** — offen sind Nr. 4 (Flagzustand, Betreiberprüfung) und Nr. 10 (Überlagerung bewertet,
**Ergebnis negativ**). **NO-GO-Kriterien: drei greifen** — 3 (unbekannt), 5 und 8 (zutreffend).
**Aktivierungsplan, Rollback (zwei Stufen, zehn sofortige Auslöser, kein Datenrückbau) und
Production-Nachweisplan sind vollständig festgehalten**, kanonisch in
[`betrieb/vorgangskontext.md`](betrieb/vorgangskontext.md) **§10** — dort auch die ehrliche
Feststellung, dass eine gültige **Altpfad-Basislinie auf `255df013` noch nicht existiert**; die
letzte gültige stammt vom 2026-07-31, 08:00 UTC und damit vor PR #190/#198/#199/#200/#201.
**Sicherheitsgrenzen eingehalten:** keine Aktivierung, keine Env-Änderung, kein Redeploy, kein
manueller Cron, keine Production-Schreibzugriffe, keine Migration, keine Budget-/Cron-/Quellen-
änderung, Berlin/Brandenburg/M8/Testmandate unverändert AUS, **0 KI-Aufrufe, 0,00 USD.
Production-Auswirkung: keine.** **Nächster Betreiberschritt:** die regulären Läufe ab
`pipeline` 16:00 UTC auf dem Altpfad laufen lassen und R-6/25B/29B abschließen — **erst danach**
aktivieren; oder ausdrücklich entscheiden, den R-6-Nachweis aufzugeben und sofort zu aktivieren.
Beides ist eine Betreiberentscheidung, keine Entwicklungsaufgabe.) ·
...
|---|---|---|
| **Sprint OP-25 K2.2 — Production-Aktivierungsfreigabe für den globalen Abruf** | 2026-07-31 | **Blockiert** (rein lesende Prüfung; Deployment `dpl_BkhKkPSAMEoYPzFbSQqUebkTSGzz` / `255df013` `READY` und enthält PR #201, keine neue Regression, CI und alle Testzahlen gegen den Commit reproduziert, Quellen/Budgets/Cron-Zeiten/Berlin/Brandenburg/M8 unverändert — aber **0** reguläre Läufe auf diesem Stand, R-6/25B/29B offen und durch eine Aktivierung überlagert oder unerreichbar, Vercel-Env aus der Cloud-Sitzung nicht lesbar. Entscheidung `BLOCKIERT`; Aktivierungs-, Rollback- und Nachweisplan in `betrieb/vorgangskontext.md` §10. Keine Aktivierung, keine Production-Änderung, 0 KI-Aufrufe, 0,00 USD) |
