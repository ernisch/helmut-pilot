...
**Letzte Aktualisierung:** 2026-07-26 (Reparatursprint Vorgangsbildung, B4) · **`main`-HEAD:** `b83d33f` (Merge #140)
...
|---|---|---|
| **#143** | **Reparatur der Vorgangsbildung (Betriebsbefund B4)** — kein politisch relevantes Ereignis geht mehr lautlos verloren | **Merge-Empfehlung: ja, aber mit bewusster Kostenentscheidung.** Basiert auf `b83d33f` (nach #140), **CI grün** (beide Pflicht-Checks, Run `30221808173`). Kein Schema, keine Migration, kein Flag, kein Cron — der Merge ist zugleich das Deployment und damit der eigentliche Nachweis. **Vorher lesen:** `befund-csd-2026-vorgangsverlust.md` §11 (KI-Aufrufe 115 → 159/Tag bei Budget 100) und §13 (Freigabeanfrage inkl. Abbruchkriterien). Branch `claude/helmut-vorgangsbildung-fehler-coolvq`, Commits `0f2df68` (Code) + `787a39d` (Doku) |
| **#140** | **Phase-1-Punkt 14B: Berliner Abnahmeprofil ausführbar vorbereiten, Sicherungslücke schließen** (aktiviert nichts) | **gemergt** (`b83d33f`). Basiert auf `4bc58dc` (nach #139). Reine Werkzeug-/Doku-Arbeit: 4 freigabepflichtige SQL-Dateien, die niemand ausführt, ein read-only Dry Run, ein neuer Backup-Umfang. Keine Production-Mutation, `helmut-flags.json` unverändert. Branch `claude/helmut-production-berlin-prep-l0lfbg`, Commit `a476f21` |
