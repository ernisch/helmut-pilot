...
**Stand: 2026-09-02 — PR #294 gemergt und deployt (`881739da`); darauf aufbauend der Vorbereitungssprint für den 500er-Funktionstest: Verdrängungsschutz der fünf realen Mandate, Vorwärts- und Rückweg, Stufenkontrolle, Fachzyklus-Startweg. Der Test ist **nicht** startbereit — zwei strukturelle Blocker (§24, Beleg §21). Neuer Draft-PR offen, nichts weiter gemergt.** Vollfassungen: [`archive/project_state/`](archive/project_state/). Diese Datei enthält nur den aktuellen, entscheidungsrelevanten Zustand (Grenze 30.000 Zeichen, testgesichert). Ablageregeln: [`archive/README.md`](archive/README.md), `CLAUDE.md` §9.
...
- **Der Kernbefund:** „synthetisch" existierte außerhalb des Kommunikationsriegels **nicht**; die fünf realen Mandate waren an **vier** Stellen verdrängbar (Budget, Priorisierung, Warteschlange, Laufzeit). **Gebaut:** `lib/helmut/mandatsklasse.js` (kein realer Slug) und darauf vier Schutzregeln — Vorrangreserve, reale Mandate zuerst in Rotation, +1 synthetisch, reale zuerst bei hartem Zeitbudget. **Bei 0 synthetischen Zeilen byte-identisch.** Synthetische Profile bauen zudem **keine eigenen Außenquellen** mehr (OP-15).
- **`recordLlmUsage` relational geschlossen (Phase 2):** Dual-Write, Flag **AUS**, Migration `20260902121500` + Rollback als Datei — **nicht angewendet**. Der Blob bleibt Lesequelle; **p95 170 bleibt Untergrenze**.
- **Kommunikationsriegel an vier Stellen gehärtet** (zweite Lage an der Netzgrenze). Der KI-Ausgang ist **kein** Riegelkanal — ihn begrenzt das Budget.
- **05:45/05:48 umgangen statt bewiesen:** Fenster **11:36–15:59 UTC**; Watchdog sperrt 05:30–08:30. Die Aktivierung verlangt einen bestandenen Fensterbefund (fail closed); der **Rückweg wird nie** durch ein Fenster blockiert.
- **Stufenkontrolle und Rückweg geschaffen:** `funktionstest-kontrolle.js` (die Regeln hatten **keinen** Aufrufer) — 15 Regeln, 6 Pflichtgrenzen. `testkohorte-rueckbau.js` ist der erste tatsächliche Ausführer des Rückbaus; die Rückbauprüfung bestätigte zuvor einen **leeren** Bestand als Erfolg (4 → 8 Befunde).
- **Vorbereitete Betreiberwerte (NICHTS gesetzt), acht Stück:** Deckel **2.416** · Verstehens-Reserve **702** · Vorrang real **200** · **RPM 82** (nicht 250) · TPM **250000** · Kostenbudget **10,00 USD/Tag** (≈ 213/Monat, Listenpreis, F7 offen) · Parallelität **1** · Kommunikation `gesperrt`. **`HELMUT_TENANT_LLM_CAP` NICHT einschalten.** Der Deckel wird **vor** der Vorrangreserve angehoben (§20.1). Sie genügen **nicht** für den relationalen Lesepfad — dafür zwei getrennte Freigaben (§21.8).
- **Tests:** Offline-Gesamtlauf **grün, Exit 0** · Browser-Smoke **32/0** · alle 14 Datenbankvertrags-Suiten grün. Sieben neue Suiten. Zahlen je Suite: §18.12/§19/§20/§21.
- **Zweiter Review — Teilsysteme (72 Agenten): 8 Befunde, 6 geschlossen (§19):** Einladung/Reset trugen **nie** eine Mandatskennung · EUR-Profildeckel für die Kohorte ein **No-op** · Fensterbefund **zeitlos** · Fairness-Zeile überlebte den Rückbau 90 Tage · der Ring kürzte Berichte **still** · `slotKapazitaetReicht` nie ausgewertet.
- **Dritter Review — Diff adversarial (54 Agenten): 20 Befunde, alle geschlossen (§20).** Der schwerste traf die eigene Änderung: die **Vorrangreserve** zog auch der GETEILTEN Verstehensarbeit Budget ab, während Code und **betreibersichtbare** Ausgabe das Gegenteil behaupteten — bei Deckel 100 gegen Vorrang 200 wäre `effectiveMax = 0` gewesen: der Datenmotor **auch der fünf realen Mandate** hätte stillgestanden. Behoben durch ehrliche Beschreibung, **Untergrenze** und eine Hürde, die Deckel/Reserve aus der LAUFENDEN Umgebung liest. Weiter: die Klassentrennung ließ **5 synthetische Profile 30 Tage verhungern** · **sechs** Regeln der Stufenkontrolle meldeten eine gemessene 0, ohne zu messen (A13 war eine **Tautologie**) · Rückbau meldete `ok:true` bei **leerer** Zielmenge · fünf Testverträge geschärft.
- **Dritter Review — Ausführbarkeit (02.09.): 6 Befunde, alle bestätigt, 4 geschlossen, 2 NICHT schließbar (Beleg §21).** Der frühere Satz „technisch vollständig vorbereitet" **war falsch und ist zurückgenommen.** Geschlossen: der **Vorwärtsweg** existierte nicht (nur der Rückweg war scharf ausführbar) ⇒ neu `testkohorte-vorwaerts.js` mit denselben drei Riegeln plus **Startfenster**, dazu `provisioning.activateTenant` (schreibt genau ein Feld, rührt Konten nicht an) · der **Fachzyklus hatte keinen Auslöser** (Pipeline-Cron 16:00, Fenster endet 15:59) ⇒ neu `funktionstest-500-zyklus.js`, ruft nur die bestehende Route `/api/cron/pipeline` in begrenzten Scheiben · **A01/A06/A10 nahmen menschliche Zusagen** ⇒ neu `funktionstest-nachweise.js` mit echten Auswertern; der A10-Auswerter musste **zweimal** gebaut werden (Auditeinträge schreibt die Route unabhängig vom Versand — Falschpositiv), jetzt zählt nur `versand=ja` plus `pushEvents.delivered` und `helmut_job_outbox`, drei Kanäle sind ausdrücklich **nicht messbar**.
- **ZWEI STRUKTURELLE BLOCKER, jetzt als erzwungene Hürden im Code (§21.4/§21.5):** (1) `briefing_materialization` ist erst ab **18:00 UTC** fällig — **über die Warteschlange** entsteht im Fenster 11:36–15:59 kein Briefing (Direktpfad `lage-briefing` unberührt, aber auf 240 s begrenzt und wirkt auf reale Mandate). (2) Der konservative Tagesbedarf **1.812** übersteigt die in 263 min bei Parallelität 1 möglichen **1.732** (nötig: 276 min). `startbereitschaft()` meldet deshalb von sich aus **nicht startbereit**, auch bei allen acht gesetzten Werten. **Einziger belegbarer Ablauf:** zwei Fenster nacheinander (11:36–15:59, dann **17:36–19:59**) **bei Parallelität 2** — beides ungedeckte, getrennte Betreiberentscheidungen.
