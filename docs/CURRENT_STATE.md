...
* **Repository `main`:** `a4507527f36f912131e83333a56bc269c4107fed`, Merge von PR522 (Diagnosewahrheit des gescheiterten 169er Laufs). Vercel Production READY ohne Runtime Fehler. Der manuelle GitHub-Actions-Ausfuehrungsweg fuer den einmaligen 169er Runner und der harte 0,80-USD-Laufdeckel stammen aus PR519/PR520.
...
* **Diagnose- und Ergebniswahrheit repariert und gemergt (PR522, main `a4507527`).** Der Motor traegt bei `skipped-invalid` jetzt die Fehlerklasse (`reason`) und die echte Clustergroesse (`documents`); der Runner uebernimmt hoechstens fuenf **sichere** Fehlercodes als `validierungsfehler` (auch in die Einmalquittung). Der historische konkrete Validierungsfehler bleibt **nicht rekonstruierbar**. Kein Retry, keine Quittungsaenderung.
* **Kleinster sicherer Weg fuer einen neuen 169er Versuch vorbereitet (dieser Sprint, PR offen).** Die Quittungskennung ist jetzt ein ausdruecklicher, streng gepruefter Parameter (`HELMUT_VERSTEHEN_169_QUITTUNG` bzw. Workflow-Input `quittungsschluessel`): ohne Kennung gilt die alte (alter Auftrag bleibt blockiert), die alte Kennung selbst und Fremdformate stoppen fail closed. Alle Grenzen unveraendert (169/122/113/0,80 USD/35 min/4 USD). Der Problemvorgang `vg-abschaffung-20260911-7420f6` bleibt CAS-gesperrt (`unbekannt`) und wird ohne Betreibereingriff ehrlich als `skipped-failed` gemeldet; erneutes Verstehen nur ueber den kanonischen Weg `helmut_verstehen_ausgang_aufloesen(…, 'erneut')` + Wiederaufnahmepfad. Neuer Test `verstehen-169-neuversuch-test.js` (18/18, offline, 0 Modellaufrufe, 0 Writes). **Branch:** `codex/verstehen-169-neuversuch-20260923`, PR offen — Merge braucht Betreiberfreigabe. [Beleg](betrieb/verstehen-einmalig-169-20260922.md) §17.
...
2. **Der kleinste sichere Weg zum neuen 169er Versuch liegt als PR zur Review** (`codex/verstehen-169-neuversuch-20260923`): Quittungskennung wird ausdruecklich uebergeben und streng geprueft, alle Grenzen unveraendert, Problemfall-Test 18/18, Workflow-Pruefung 70/70, Betroffenen-Suiten gruen. Merge braucht Betreiberfreigabe.
...
**Unmittelbar:** Den PR dieses Sprints reviewen (Quittungskennung als ausdruecklicher Parameter in `verstehen-einmalig.js`/`verstehen-einmalig-169.js`/Workflow, neuer Test `verstehen-169-neuversuch-test.js`, Beleg §17). Merge nach `main` braucht eine ausdrueckliche Betreiberfreigabe — Merge und automatisches Production Deployment sind eine gemeinsame kritische Wirkung.

**Danach getrennt, als eigene Betreiberentscheidungen:** (a) eine neue Quittungskennung vergeben (Format `verstehen169-<JJJJMMTT>-<suffix>`), (b) ggf. den Problemvorgang per kanonischem Weg `helmut_verstehen_ausgang_aufloesen('vg-abschaffung-20260911-7420f6', 'erneut')` freigeben und ueber den Wiederaufnahmepfad verstehen lassen, (c) erst danach einen neuen scharfen 169er Lauf freigeben — **kein Retry, kein Dispatch, keine Quittungs-, CAS- oder Budgetaenderung ohne Freigabe.**
