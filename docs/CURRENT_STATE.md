...
**80/80** · `punkt29-fix-mutationsprobe.js` **7/7 rot** · Offline-Suite **176/190**, exakt die
**identische** Basislinie und Fehlschlagliste wie `main` `4df9cf19` (14 umgebungsbedingte
Suiten, kein Delta) · Browser-/Mobile-Smoke **32/32**. **Beobachtung B29-F1 erweitert (kein
Befund dieses Sprints):** unter hoher Parallellast ist neben `berlin-e2e-vertrag-test.js` auch
`brandenburg-e2e-vertrag-test.js` flaky — in einem von drei Runner-Läufen rot, isoliert
**98/0** grün, im Wiederholungslauf wieder grün. Beide Suiten fasst dieser Sprint nicht an. **Sicherheitsgrenzen eingehalten:** keine
