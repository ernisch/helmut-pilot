...
| Kalender-Machbarkeit 1 (#209) | zuerst die Rechtsfrage ([`kalender-machbarkeit-1.md`](kalender-machbarkeit-1.md)) |
...
- **§21 500-Mandate-Reife** — *erfolgreich, Freigabe 01.09.* (#290). **Drei getrennte Urteile, unverändert gültig:** Warteschlangen-Aufnahmefähigkeit 500 **erbracht** · rechnerisch-architektonisch **vorbereitet, finale Dimensionierung offen** · operativer Mehrtagesbetrieb **NICHT BEWIESEN**. Minimal-Cron `18,48` vorbereitet, **nicht aktiviert**; Kohorte 495 validiert, inaktiv.
- **§14 Selbstweck-Härtung** (#269): `bereit` heißt Konfigurationsbereitschaft, **nicht** Zustellung; 3-s-Abbruch ungeprüft. Seither `CLAUDE.md` §6: jeder Testlauf über `scripts/lokal.js`. **§15 Skalierung** (#270): Anlage-Stapel legt nur **inaktiv** an, **kein Index nötig**; zuerst reißt die 500-MB-Free-Grenze (2,70 MB/Tag). **§16 Watchdog** (#271): **`partial` heißt nie „Slot fehlt"**; offen 5/5 personalisiert, nur **1/5** mit Push-Empfänger. **§17 Z3a** (#272): 92/92, 0 Fehler — **nicht** die echten Anbieter.
- **§18 Z22** — *erfolgreich* (29.08.): `helmut_jobs_offen` zählte mandatsblind, Z22 ergänzt `p_mandat`; Buchungen `20260829175642`/`20260829175749` — **Z22 nicht erneut anwenden.** **§19** (#283): `lauf-bilanz.js` als **eine** kanonische Zähler-/Statusableitung; offen drei `unbekannt`-Vorgänge. **§22 PR-Bereinigung** — #275–#277 und #282 nach Einzelprüfung **geschlossen, nicht gemergt** (Branches bleiben Auditbeleg).
...
- **Vier Reviews, 35 Befunde, 33 geschlossen.** Der schwerste: die Vorrangreserve zog auch der GETEILTEN Verstehensarbeit Budget ab — bei Deckel 100 gegen Vorrang 200 wäre `effectiveMax = 0` gewesen, der Datenmotor **auch der fünf realen Mandate** stillgestanden. Behoben.
...
*Teilweise abgeschlossen — offline bewiesen, keine Production-Wirkung.* Branch `claude/helmut-pilot-post-pr295-dveogp`, Basis `9079ac3`. Vollbeleg: [`betrieb/500-funktionstest-sicherheitsrahmen-2026-09-01.md`](betrieb/500-funktionstest-sicherheitsrahmen-2026-09-01.md) **§22–§30**.

- **Nach-Merge-Beleg vollständig (§22):** #295 gemergt 02.09. 13:08:53 UTC, Merge-Commit `9079ac3`, beide Pflicht-Checks grün (Lauf 33634007860), Vercel `dpl_DHTnMxFsibaj3XxdkpgDzandursx` READY `production` exakt auf `9079ac3`. Rein lesend nachgezählt: 9/5/4 Mandats-, 10 Identitätsprofile, **0 synthetische Zeilen**, 35 Migrationen, 13 Crons — unverändert.
- **BETREIBERENTSCHEIDUNG 02.09. umgesetzt — das Startfenster-Tor prüft ab jetzt FÄLLIGKEIT (§30):** Maßgeblich ist, ob der Motor einen Auftrag nach `due_at <= jetzt` beanspruchen kann; die bisherige **Schnittmenge** mit dem Streuintervall beschrieb den Motor nicht und ist ersetzt. Neu `funktionstest-faelligkeit.js`: ruft die **echte** `source-demand.planeMandatsarbeit` auf (keine zweite Phasenlogik, kein hartkodierter Ersatz), liefert **sieben Kennzahlen je Klasse**, rechnet datums- und kohortengenau, durchgehend fail closed. **Kein pauschales „100 % fällig" eingebaut.**
- **Gemessen, Tag 03.09., Plan zum Fensterbeginn** (Kohortenanteil, den der Motor bis Fensterende in der sichtbaren Produktstufe `briefing_materialization` beanspruchen könnte):

  | Fenster UTC | Türkei / Berlin | Stufe A (20) | Stufe B (95) | Stufe C (495) |
  |---|---|---|---|---|
  | 11:36–15:59 | 14:36–18:59 / 13:36–17:59 | **0,0 %** | **0,0 %** | **0,0 %** |
  | 17:36–19:59 | 20:36–22:59 / 19:36–21:59 | **80,0 %** | **57,9 %** | **55,1 %** |
  | 21:36–03:59 | 00:36–06:59 / 23:36–05:59 | **100 %** | **100 %** | **100 %** |

- **Mitternacht — eine Annahme bestätigt, eine widerlegt (§30.5):** Bestätigt ist die harte Grenze: Planung **nach** 00:00 UTC legt die Fälligkeiten auf den Folgetag, dann sind im Nachtfenster **0,0 %** beanspruchbar (gemessen 00:30 und 02:00). Widerlegt ist „100 % ist selbst nur eine Annahme": bei Planung **vor oder zum Fensterbeginn** behalten die Aufträge ihre `due_at` (`on conflict do nothing`) und bleiben über Mitternacht beanspruchbar — die 100 % sind gerechnet. Entscheidend ist der **Planungszeitpunkt**, nicht die Fensterlage; als harte Startbedingung im Code.
- **Blocker 1 entfällt für das Nachtfenster, sonst nicht (§30.6).** Die alte Aussage bleibt sachlich richtig, die Prüfung war die falsche. 0,0 % und 55,1 % sind keine Kohortenabdeckung — das Tor verwirft beide Fenster weiter, jetzt mit einer Zahl.
- **Blocker 2 entfällt NICHT durch das Tor (§30.7).** Im Nachtfenster (383 min, Parallelität 1) sind **2.522 ≥ 1.812** — es passt, aber durch die **Fensterwahl**, die das Tor erst zulässt. Neu getrennt (beschreibend, `kapazitaet-500.lastTrennung`, 500 Mandate konservativ): Warteschlangenarbeit **802** · nutzergetrieben + eigene Crons **1.000** · andere 10 = **1.812**; Deckel 2.416, Reserve 604. Die **802 ist keine Fenstergröße** — die nutzergetriebene Last hat keine Fensterbindung und zehrt vom selben Deckel.
- **Eigener Befund ZURÜCKGENOMMEN (§29):** §24.1 behauptete, die 1.000 mandatsgebundenen Aufrufe könnten im Fenster nicht entstehen. Zahlen richtig, **Begründung falsch** — dominiert von `communicationDraft`, einer Route **ohne Fensterbindung**. **812 darf NICHT als Fenstergröße dienen** (falsches Grün); die 802 tritt **nicht** an ihre Stelle.
- **Was weiterhin fehlt (§30.8):** Fälligkeit ist aus dem Plan berechenbar, **Status nicht** — der 20:00-Crawl findet in Stufe C 275/495 Briefings fällig und treibt dieselbe Warteschlange an. Deshalb bleibt `vollstaendigerZyklus` **`null` = NICHT BEWERTBAR**, bis die Zahl offener Aufträge rein lesend erhoben ist; `erhebungsSql()` gibt die Abfrage aus (nur `select`), das CLI endet dann mit Exitcode 1. **Nicht gemessen:** Laufzeit der KI-freien Warteschlangenklassen (990 Aufträge im Fenster).
- **Drei von vier Schutzgrenzen sind NICHT hart (§23.4):** laufzeitwirksam ist allein der Tagesdeckel (`reserveLlmCall`, atomar); **RPM/TPM liest kein Ausführungspfad**, das Kostenbudget wirkt nur entdeckend.
- **Zwei Lücken geschlossen (§23.3):** (1) Gestuft war nur die **Aktivierung** — neu `testkohorte-stufen.js`, **15 stufengenaue Freigaben**. (2) Kein Weg zur **vollständigen Entfernung** — neu `testkohorte-entfernung.js` + CLI, sechs Riegel, Restbestandsbefund über fünf gezählte Familien.
- **Fünf Defekte im NEU gebauten Code, alle behoben (§28, §30.3):** `deleteTenantScopedData` schrieb den leeren Store **unbedingt** zurück (hätte 400 Zeilen **angelegt**) · ein **Teilfehler** zählte als „entfernt" · CLI-Exitcode **0** über gescheitertem scharfem Lauf · `Number(null)` hätte einen fehlenden Planungszeitpunkt als 01.01.1970 gelesen · `erhebungsSql` escapte Eingaben, statt sie **abzuweisen**. Nicht abgeräumt bleiben `helmut_jobs`, Outbox, `crawl_runs`, Personenquellen-Rohdaten.
- **Schutz der fünf realen Mandate (§25.2):** Verdrängungsschutz **gebaut, heute nicht wirksam** — `HELMUT_TESTLAUF_VORRANG_REAL` ungesetzt, Reserve **0**. Bei 0 synthetischen Zeilen kein Risiko, sondern eine Vorbedingung.
- **05:45/05:48 (§26):** um 05:48 läuft heute nichts; kleinster Cronabstand **10 min** gegen 5 min Grenze. **Telemetrie (§27):** Ring 5.000 trägt **einen** Testtag je Stufe; ein Lost Update bleibt für die Vollständigkeitsprüfung strukturell unsichtbar. „Parallelität 2" bleibt unterbestimmt (§25.1), Weg (c) nicht empfohlen (§25.3).
- **Kein Fenster gewählt, keine Lösung ausgewählt. Unverändert:** 5 reale Mandate, Gate `shadow`, 13 Crons, 35 Migrationen, alle Production-Daten, jede Umgebungsvariable, keine Migration angewendet.
