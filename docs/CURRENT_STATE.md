...
## 16 · Letzter Sprint (26.08. — Watchdog: Slotlogik, Briefingstufen, Skalierung)

**Teilweise abgeschlossen** — gebaut und testbelegt; Merge, Deployment und der reguläre Production-Nachweis stehen aus. Branch `claude/watchdog-slot-briefing-korrektur`, **PR #271 (offen, nicht gemergt)**, Ausgangscommit `07bf7794`. **Keine Production-Änderung, keine Datenbank-Mutation, kein Cron, kein Flag, keine Env, keine Push-Abo-Änderung.** Vollständiger Bericht: [`betrieb/watchdog-korrektur-2026-08-26.md`](betrieb/watchdog-korrektur-2026-08-26.md).

**Anlass (rein lesend, 26.08.):** Motor grün (0 endgültige Fehler, 0 hängende Leases, 0 CAS-`unbekannt`), Bot meldete **„Gestört"** — einziger Grund `slot-fehlt:warteschlange-crawl@04:00Z`. Ein einmaliger `partial`-Lauf erfüllte seinen Slot nicht, obwohl er lief und zwei erfolgreiche Folgeläufe hatte: bis zu **24 h** Fehlalarm. Nachgerechnet ergibt die Korrektur **„Gesund mit Hinweisen"**.

- **Slotlogik:** Vorhandensein und Ergebnis getrennt — nur das vollständige Fehlen einer Quittung heißt „Slot fehlt", `partial` **nie**. Ein späterer Erfolg macht die Störung zu einem Hinweis; dauert eine Auswirkung an, bleibt sie rot (`slot-nachwirkend`).
- **Briefingstufen je Mandat:** Nenner sind die **aktiven Mandate**; Vorbereitung, Ablage, Personalisierung, Push-Ereignis, registrierter Empfänger, Versand und Empfang werden **getrennt** ausgewiesen. **Befund: 5 von 5 vorbereitet und personalisiert, aber nur 1 von 5 mit registriertem Push-Empfänger; Empfang am Endgerät nicht bestätigbar, Öffnung nicht messbar.** „5/5" belegte die Erzeugung, nicht die Ankunft. „Briefing erhalten" ist testgesichert ausgeschlossen.
- **Wortlaut:** `delivered` heißt **Annahme durch den Push-Dienst**, nie „zugestellt".
- **Drei echte Fehler gefunden und behoben** (durch den neuen echten Routenlauf, nicht durch Quelltextprüfung): eine **nicht lesbare Mandantenliste** ließ den Bot ganz **schweigen** · **offene terminale Arbeit** stand still auf „Gesund" · die 2000-Zeichen-Kappung zerschnitt Emojis und riss über `encodeURIComponent` den **ganzen Lauf** mit.
- **Skalierung:** kein Speicherzugriff wächst mehr mit jedem einzelnen Mandat. Neutrale Signale einmal je Lauf; Briefingbelege gebündelt; Lage und Push aus **derselben** `helmut_store`-Zeile, stapelweise über `id=in.(…)`. Gemessen **5 → 1 · 25 → 3 · 100 → 10** Stapelzugriffe statt **10 · 50 · 200**; Route bei 100 Mandaten **163 ms**. Ein gescheiterter oder zu langsamer Stapel bricht nichts ab — die Mandate bleiben im Nenner als „Status nicht bestimmbar".
- **Tests:** `health-report-route-test.js` **49** · `motor-health-test.js` **65** · `briefing-frische-audit-test.js` **38** · `mandantentrennung-test.js` **20** · `alarm-payload-test.js` **20** — je 0 FAIL. Gesamtlauf **280/282** (`scripts/lokal.js`, 709 s); die zwei Ausfälle `kalender-ics` und `lambda-paket` sind fehlende npm-Pakete der Cloud-Umgebung und bestanden vor dem Sprint unverändert.
