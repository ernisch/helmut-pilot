...
- **Kernbefund:** „synthetisch" existierte außerhalb des Kommunikationsriegels **nicht**; die fünf realen Mandate waren an **vier** Stellen verdrängbar. Gebaut: `mandatsklasse.js` und vier Schutzregeln. **Bei 0 synthetischen Zeilen byte-identisch.**
- **Gebaut:** Vorwärtsweg · Fachzyklus-Startweg (ruft nur `/api/cron/pipeline` in 280-s-Scheiben) · echte Auswerter (A10 zählt nur `versand=ja` + `pushEvents.delivered` + `helmut_job_outbox`, **drei Kanäle nicht messbar**) · Stufenkontrolle (15 Regeln) · Rückbau-Ausführer. `recordLlmUsage` relational als Dual-Write, Flag **AUS**, Migration `20260902121500` als Datei — **nicht angewendet**.
...
## 25 · Sprint 02.09. — Nachbereitung nach dem Merge von #295 (**Draft-PR, nicht gemergt**)

*Teilweise abgeschlossen — offline bewiesen, keine Production-Wirkung.* Branch `claude/helmut-pilot-post-pr295-dveogp`, Basis `9079ac3`. Vollbeleg: [`betrieb/500-funktionstest-sicherheitsrahmen-2026-09-01.md`](betrieb/500-funktionstest-sicherheitsrahmen-2026-09-01.md) **§22–§23**.
...
- **Beide Blocker unabhängig bestätigt, Zahlen exakt reproduziert (§23.1/§23.2):** `briefing_materialization` ist 18:00–21:36 UTC fällig, Schnitt mit 11:36–15:59 ist leer (0,0 %); die Werte 66,4 / 55,1 / 6,7 % stimmen. Konservativer Bedarf **1.812** gegen **1.732** mögliche in 263 min bei Parallelität 1, nötig 276 min. **Präzisierung:** Das Nachtfenster 21:36–03:59 trägt den Zyklus bei par 1 rechnerisch (2.522) — es scheitert daran, dass dort **keine** Arbeitsklasse fällig ist. Das Fenster ist nicht zu klein, sondern leer.
- **Neuer Befund — drei von vier Schutzgrenzen sind NICHT hart (§23.4):** Laufzeitwirksam ist allein der Tagesdeckel (`reserveLlmCall`, atomar, fail closed). **`HELMUT_TESTLAUF_MAX_RPM`/`_MAX_TPM` werden von keinem Ausführungspfad gelesen** — es gibt keinen Minutentakt-Begrenzer; das Kostenbudget wirkt nur entdeckend (A04, zwischen den Stufen). Die Annahme eines „harten RPM-/TPM-Schutzes" bei Parallelität 2 ist damit **falsch**.
- **Zwei neue Lücken geschlossen (§23.3):** (1) Gestuft war ausschließlich die **Aktivierung** — für die 20 Profile der Stufe A ließ sich kein Fachzyklus freigeben. Neu `testkohorte-stufen.js`: **15 stufengenaue Freigaben**, Reihenfolge erzwungen, die sieben Bestandsworte unverändert. (2) Es gab **keinen Weg zur vollständigen Entfernung** — der Rückweg deaktiviert nur; die 400er-Gruppe hätte 400 Einzelaufrufe ohne Erlaubnisliste gebraucht. Neu `testkohorte-entfernung.js` + CLI mit sechs Riegeln; aktive Profile werden übersprungen statt gelöscht, der Restbestandsbefund verlangt fünf **gezählte** Familien.
- **Zweiter, unabhängiger Review — der schwerste Befund (§24.1):** Von den 1.812 sind **1.000 mandatsgebundene Modellaufrufe, die im Fenster gar nicht entstehen können** — beide Warteschlangenklassen sind ausdrücklich **KI-frei** (`scalable-pipeline.js:1113/1175`). Im Fenster erzeugbar sind **812**; die passen bei **Parallelität 1** in beide Fenster (nötig 124 min). **Gilt dieses Framing, löst sich Blocker 2 auf und Parallelität 2 wird überflüssig.** Bewusst **nicht** in die Hürde eingebaut — ein Tor, das sich selbst grün rechnet, wäre das falsche Grün. Zusätzlich: Blocker 2 ist ohnehin szenarioabhängig (Kipppunkt 1,84; gemessen ist 1,2, konservativ rechnet 2,0).
- **Keine Lösung ausgewählt.** Vier Wege bewertet (§23.5). Kein riskanter Ersatz gebaut.
- **05:45/05:48 gemessen (§23):** genau **2** Überschneidungspaare, beide entstehen **nur** mit dem Minimal-Cron `18,48` — der ist nicht in `vercel.json`. Heute tritt die Überschneidung also nicht auf.
