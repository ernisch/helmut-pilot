...
**Stand: 2026-09-03 — PR #296 gemergt und deployt (`a839c1b`, Vercel „Deployment has completed", beide Pflichtprüfungen grün). Das Startfenster-Tor prüft Fälligkeit, der Startweg ist bedienbar (§25). Korrektursprint 03.09. (**PR #297 offen, nicht gemergt**, §26): der Betreiberweg der Provisionierung ignorierte `--stufe` und zielte auf alle 495 — jetzt stufenweise, Ablaufplan umgebaut; die Bundestagsreife-Sperre (§34.7) ist **geschlossen** (495/495 bestehen ihre Ebenenprüfung; die Sperre wurde dabei verschärft). Der 500er-Funktionstest bleibt **nicht startbereit** (Betreiberwerte, Vorrangreserve, Grundlinie, Sicherung offen).** Vollfassungen: [`archive/project_state/`](archive/project_state/). Diese Datei enthält nur den aktuellen, entscheidungsrelevanten Zustand (Grenze 30.000 Zeichen, testgesichert). Ablageregeln: [`archive/README.md`](archive/README.md), `CLAUDE.md` §9.
...
| **Stufenweise Provisionierung (§26)** | Merge und Production-Prüfung von #297 |
...
2. **Erst danach die Kette der Stufe A:** aktuelle Grundlinie → Sicherung der betroffenen Tabellen → Fenster → `provisionierung --stufe=a` (inaktiv, eigene Freigabe) → `isolation --stufe=a` → **acht Betreiberwerte setzen und wirksam prüfen** → Aktivierung A (eigene Freigabe) → Fachzyklus A → Kontrolle A. Stufe B und C jeweils getrennt, erst nach kontrollierter Vorstufe. Maschinenlesbar: `node scripts/lokal.js -- node scripts/funktionstest-500-ablauf.js plan`.
3. **Fenster:** nach Fälligkeit trägt allein **21:36–03:59 UTC** die volle Kohorte (§25); Planung **vor 00:00 UTC**. Die Zahl offener Aufträge ist erst nach Provisionierung und Planung messbar (`erhebungsSql()`, Beleg §30.8).
4. **Gründerentscheidung zur Skalierung** (Entscheidungsvorlage, §6): KI-Deckel, AWS-Frage, Reihenfolge 10 → 25.
5. **Selbstweck-Vorlauf entscheiden:** fünf Werte setzen (Betreiberaktion), Vorprüfung `/api/ops/jobqueue` → `ereignisbetrieb.bereit === true`, danach der **siebentägige Nachweis** mit 5 Mandaten. Davor empfohlen: kleinster Preview-Versuch zum 3-s-Abbruch (Zielarchitektur §27.3).
6. **Rückkehr zu den P0-Verkaufsblockern OP-01…OP-04.** **Betreiberprüfung Doppelkanal** (OP-07, §7 Punkt 10) vor jedem Kanalschritt.
...
- **§20 Kapazität Understanding — BLOCKIERT**, Gate-Flip und PR-B gestoppt ([Beleg](betrieb/understanding-kapazitaet-2026-08-31.md)): Verhungern und wachsender Rückstand belegt (31.08.: 9.080 pending; Ankunft Ø 307/Tag, Abfluss Ø 68/Tag). Gate bleibt `shadow`. **Offen:** natürlicher Nachweis, OP-06 (1.769 Altfälle), Siebentagenachweis.
- **§21 500-Mandate-Reife** — *erfolgreich, Freigabe 01.09.* (#290). **Drei getrennte Urteile, unverändert gültig:** Aufnahmefähigkeit 500 **erbracht** · rechnerisch **vorbereitet, Dimensionierung offen** · Mehrtagesbetrieb **NICHT BEWIESEN**. Minimal-Cron `18,48` vorbereitet, **nicht aktiviert**; Kohorte 495 gegen `validateSpec` validiert — die Bundestagsreife-Sperre kam erst am 03.09. dazu (§26).
- **§14** (#269): `bereit` heißt Konfigurationsbereitschaft, **nicht** Zustellung; seither jeder Testlauf über `scripts/lokal.js`. **§15** (#270): Anlage-Stapel legt nur **inaktiv** an; zuerst reißt die 500-MB-Free-Grenze. **§16 Watchdog** (#271): `partial` heißt nie „Slot fehlt"; nur 1/5 mit Push-Empfänger. **§17 Z3a** (#272): 92/92 — **nicht** die echten Anbieter.
- **§18 Z22** (29.08.): `helmut_jobs_offen` zählte mandatsblind; Buchungen `20260829175642`/`20260829175749` — **nicht erneut anwenden.** **§19** (#283): `lauf-bilanz.js` als **eine** kanonische Statusableitung. **§22**: #275–#277 und #282 **geschlossen, nicht gemergt**.
...
- **Kernbefund:** „synthetisch" existierte außerhalb des Kommunikationsriegels **nicht**; die fünf realen Mandate waren an **vier** Stellen verdrängbar. Gebaut: `mandatsklasse.js` und vier Schutzregeln, Vorwärtsweg, Fachzyklus-Startweg, Stufenkontrolle, Rückbau-Ausführer — **bei 0 synthetischen Zeilen byte-identisch**. Drei Auswerterkanäle **nicht messbar**. `recordLlmUsage` relational als Dual-Write, Flag **AUS**, Migration `20260902121500` **nicht angewendet**. Vier Reviews, 35 Befunde, 33 geschlossen.
...
- **Das Startfenster-Tor prüft FÄLLIGKEIT (§30):** maßgeblich ist `due_at <= jetzt`; `funktionstest-faelligkeit.js` ruft die echte `planeMandatsarbeit` mit Production-Eingaben auf, fail closed. Gemessen (03.09.): Nachtfenster **21:36–03:59 UTC** (Berlin 23:36–05:59) trägt **100 %** aller drei Stufen; Tagesfenster 11:36–15:59 **0 %**; Abendfenster 55–60 %. **Mitternacht:** Planung **vor 00:00 UTC** ist harte Startbedingung (§30.5). Kapazitätstor bei Parallelität 1: **2.522 ≥ 1.812** (§30.7).
- **Kreisschluss aufgelöst (§32/§33):** das Tor war falsch-negativ (ein erledigter Zyklus galt als gescheitert). Jetzt **sieben Statusmengen**, **zwei getrennte Urteile** (Fachzyklus · Lastbeweis); `startbereit` für alle drei Stufen erreichbar (A/B ab 143 min, C ab 276 min). **Kein neuer schreibender Planungscode.**
- **Vier Blocker geschlossen** (Einzelbeleg §22–§33): u. a. das Deaktivierungswort für 20 hätte 495 freigegeben; `startbereit` war unerreichbar. Messintegrität: `frischefenster`/`stufe` Pflicht, `mindestAbdeckung` ≥ 0,5, Kapazitätshürde 25/100/500.
- **Unverändert wahr:** drei von vier Schutzgrenzen sind nicht hart (§23.4); `HELMUT_TESTLAUF_VORRANG_REAL` ungesetzt ⇒ Verdrängungsschutz **nicht wirksam** (§25.2); Ring 5.000 trägt einen Testtag je Stufe (§27); Lastbeweis in keiner Stufe erbracht.

## 26 · Sprint 03.09. — Stufenweise Provisionierung + §34.7 (**PR #297 offen, nicht gemergt**)
...
- **Befund (reproduziert):** `provisionierung --stufe=a` ignorierte die Stufe still — zielGroesse **495**, Pauschalwort, Exit 0. Die Bibliothek war seit 02.09. stufenfähig; der Betreiberweg reichte es nicht durch.
- **Korrektur:** `--stufe=a|b|c` ist Pflicht; fehlend/leer/unbekannt → **Exit 2 vor jedem Zugriff, kein Rückfall auf 495**; `--ids=` nur in derselben Stufe. Gemessen: **a → 20 / b → 75 / c → 400** mit stufengenauem Wort. Bibliothek unverändert.
- **Ablaufplan stufenweise (28 Schritte, kein Sammelschritt):** Reifebeleg → je Stufe Anlage (inaktiv) → Isolation → Aktivierung → Fachzyklus → Kontrolle; B erst nach kontrollierter A, C nach B. **Die acht Betreiberwerte sind Vorbedingung jeder Aktivierung und keiner Provisionierung** (§34.5); rein lesend belegbar ist davon nur der Deckel. Rückweg nie gesperrt.
- **Verhaltensbelegt:** die inaktive Provisionierung (echter `provisionTenant`) erzeugt **0** Netzaufrufe, **0** Riegelanfragen, **0** KI-Aufrufe, keine Außenkanalmodule; der echte Planer plant für 20 inaktive Profile **0** Aufträge (Gegenprobe: 1 aktives → 2). **Dabei geschlossen:** die Verstehens-Interessenprüfung zählte inaktive Profile mit — jetzt Planer-Prädikat `isDisabled` (in Production ungesetzt, heute inert).
- **§34.7 geschlossen (Variante A, Betreiberentscheidung 03.09.):** die Reifesperre wies **18 von 20** Stufe-A-Profilen ab („Testausschuss N" fehlt in der Sollmenge). Behoben in der **Kohorte**, nicht in der Regel: die **434 Bundestagsprofile** tragen jetzt amtliche Ausschussnamen der 21. WP aus der vorhandenen Sollmenge (`seeds/bundestag-ausschuesse.js`, **keine zweite Namensliste**), die **61 Landtagsprofile** weiter synthetische — ein Bundestagsausschuss auf Landesebene wäre die falsche Ebene. Kennungen, Adressen, Testnamen, -parteien und -themen bleiben synthetisch. Der echte, unveränderte Pfad nimmt Stufe A **20/20** an, die Prüfung **495/495**.
- **Dabei verschärft, nicht gelockert:** veraltete Ausschussnamen früherer Wahlperioden liefen bisher als gültig durch (3 von 4 dokumentierten WP-20-Namen — die Negativliste wurde nur im Fehlerzweig gelesen). Jetzt weisen sie ab (36 Schreibvarianten geprüft); Kurzformen bleiben gültig (32 geprüft). **Reichweite:** harte Sperre nur bei Neuaktivierung; für Bestandsprofile ändert sich die **Anzeige** in der Admin-Ansicht. **Ungeprüft:** ob eines der 5 realen Mandate betroffen ist — lesender Production-Abgleich war in dieser Sitzung nicht möglich (§34.13.2/.6).
- **Lastfolge gemessen:** 42 der 495 Profile erhalten über „Arbeit und Soziales" **ein** Sachpaket zusätzlich (je +84 Quellen); kohortenweit **+84 `source_fetch` einmalig** (54 → 138), die mandatsgebundenen Klassen (je 495) unverändert — das Kapazitätsmodell (§30.7) bleibt unberührt. Gemessen gegen den Offline-Katalog.
- **Tests:** Offline-Gesamtlauf **318/318**, Browser-/Mobile-Smoke **32/0**. Neu/erweitert: `testkohorte-vorwaerts-cli` **55/0**, `testkohorte-provisionierung-inaktiv` **48/0** (A0 belegt den echten Pfad positiv), `test-kohorte-500` **54/0**, `profil-bereitschaft` **103/0**, `funktionstest-ablaufplan` **82/0**. Zwei Reviewrunden, alle Feststellungen behoben (§34.12/.13).
- **Gefunden, NICHT repariert (älterer, eigener Befund):** `seeds/entities.js` führt eine **zweite** kuratierte Ausschussliste (23) für den Radar-Ausschussbeleg — **21 von 24** WP-21-Namen haben einen Eintrag, **3** kuratierte Namen sind veraltete WP-20-Bezeichnungen. Stille Radarlücke für **reale** Mandate, nicht durch diesen Sprint entstanden, für die Kohorte keine Verschlechterung (vorher 0 von 24). Korrektur ändert Production-Klassifikation → eigener PR (§34.13.7).
- **Keine Production-Wirkung:** keine Provisionierung, Aktivierung, Umgebungsvariable, Migration, Cron-, Azure-, Budget- oder Reserveänderung, kein Modellaufruf, keine Nachricht.
