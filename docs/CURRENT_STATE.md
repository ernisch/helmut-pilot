...
**Stand: 2026-09-03 — #296 gemergt und deployt (`a839c1b`, §2). **PR #297 offen, nicht gemergt** (§26): Provisionierung stufenweise, Bundestagsreife-Sperre §34.7 **geschlossen** (495/495, dabei verschärft). **Neu:** ein Production-Ausschussbefund (Betreiberangabe, rein lesend) — Wirkung eng begrenzt, Merge nicht blockiert, **Auslegung offen** (§26). Der 500er-Funktionstest bleibt **nicht startbereit**.** Nur der aktuelle, entscheidungsrelevante Zustand (Grenze 30.000 Zeichen, testgesichert); Ablageregeln: [`archive/README.md`](archive/README.md), `CLAUDE.md` §9.
...
- **`main`-Kopf und letzter fachlich wirksamer Production-Code: `a839c1b19f55246bfe747efbfcfa2269f5e28842`** (Merge von #296, 03.09. 05:08:15 UTC). Vercel meldet für diesen Commit **„Deployment has completed"** ([Nachweis](https://vercel.com/nohut/helmut-pilot/CCbSsp58pxGNMqjuGQziVZYknaHd)); interne Details und `dpl_`-Kennung **nicht zusätzlich bestätigt**. Beide Pflicht-Checks grün (Lauf 33717626724).
- **Gemergt:** zuletzt **#296**; die vollständige Merge-Folge steht in der PR-/Git-Historie. #275–#277 und #282 nach Konsolidierung **geschlossen, nicht gemergt** (Branches bleiben Auditbelege).
- **Offen (NICHT gemergt):** **#297** (Korrektursprint 03.09.) auf `claude/stufenweise-provisionierung-fix-rg6sij`, Basis `a839c1b` (§26). Sonst keine offenen PRs.
...
| `HELMUT_PROFILE_DB_MODE` | Wirkung AN (laufzeitbelegt); Wert unbestätigt — relevant für F-P6/§26 |
...
9. **500er-Funktionstest: NICHT startbereit, nicht freigegeben.** Stand 03.09. (§26, Kette §11.2): **PR #297 offen** — Stufe A erst nach Merge **und** Production-Prüfung; Bundestagsreife-Sperre **geschlossen** (§34.7, Stufe A 20/20); **Grundlinie + Sicherung der betroffenen Tabellen vor** der Provisionierung; die **acht Betreiberwerte** erst vor der **Aktivierung**. Offen: Zahl offener Aufträge (erst nach Provisionierung messbar), Laufzeit der KI-freien Klassen, Azure-Gesamtkontingent, Verdrängungsschutz unter Last, Mehrtagesbetrieb.
...
| Punkt | Was fehlt |
|---|---|
| Profilreife (OP-29/OP-04) | 29B; relationale Profilzeilen = veraltete Schnappschüsse (F-P6, §26) |
...
1. **PR #297 (§26) prüfen und mergen** (Betreiberentscheidung), danach rein lesend die Wirkung belegen und diesen Status nachziehen (`CLAUDE.md` §9). Getrennt davon: den Ausschussbefund über den **wirksamen** Pfad klären (§26, SR §34.13.6b).
...
*Teilweise abgeschlossen — offline bewiesen, Merge und Production-Prüfung ausstehend.* Branch `claude/stufenweise-provisionierung-fix-rg6sij`, Basis `a839c1b`. Vollbeleg: [SR](betrieb/500-funktionstest-sicherheitsrahmen-2026-09-01.md) §34.
...
- **Production-Ausschussstand (Betreiberangabe 03.09., rein lesend; §34.13.6a):** **genau eines** der 5 aktiven Mandate trägt eine Ausschussangabe, deren Zuschnitt zur 21. WP aufgeteilt wurde und die deshalb nicht gegen die Sollmenge auflösbar ist; die übrigen Angaben aller 5 sind auflösbar, alle Stellvertretungen leer. **Nicht durch #297 entstanden:** der Resolver liefert am Basisstand `a839c1b` dasselbe Ergebnis (die Bezeichnung steht nicht auf der Veraltet-Liste); `server.js` ist nicht im Diff. Wirkung: **keine** Deaktivierung, **keine** Blockade, **keine** Datenänderung; die Admin-Ansicht zeigt das Mandat als nicht vollständig bereit, eine Neuaktivierung **über den CLI-Pfad** würde abgewiesen (der Admin-Schreibpfad nicht). **Blockiert den Merge technisch nicht.**
- **ABER — Widerspruch zu OP-29, ungeklärt (§34.13.6b):** OP-29/4 dokumentiert, dass **genau dieses Feld** am 04.08. mit Freigabe über den **Blob**-Pfad korrigiert wurde („alle fünf aktiven realen Profile BEREIT"). Der neue Befund nennt **relationale** Spaltennamen und exakt den Vorkorrekturstand — und OP-29 weist die relationalen Zeilen ausdrücklich als **veraltete Schnappschüsse (F-P6)** aus. (1) Gelesen wurde der veraltete Schnappschuss → **nichts zu korrigieren**. (2) Eine freigegebene Korrektur ist verloren → **eigener, ernster Vorfall**. **Vor jeder Profilkorrektur zuerst den wirksamen Pfad prüfen** (`profil-bereitschaft.js --production`) — sonst überschreibt eine „Korrektur" womöglich guten Blob-Stand. Danach ggf. **eigene Production-Freigabe**; amtlicher Stand ungeprüft, geraten wird nichts. Restliste OP-29 gekennzeichnet.
...
- **Keine Production-Wirkung:** keine Provisionierung, Aktivierung, Profiländerung, Umgebungsvariable, Migration, Cron-, Azure-, Budget-, Reserveänderung, kein Modellaufruf, keine Nachricht.
