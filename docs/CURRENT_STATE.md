...
- **Entscheidungsrelevant:** Telemetriequelle ist `helmut_store.data.llmUsage` (Ring 5.000), **nicht** `llm_usage`; Tagesbedarf p95 **170** / max 298 (Untergrenze). Azure 250.000 TPM / 250 RPM, `gpt-5-mini`, Sweden Central; **Gesamtkontingent des Kontos unbelegt**.
...
- **Startfenster-Tor prüft FÄLLIGKEIT** (`due_at <= jetzt`, fail closed): nur das Nachtfenster **21:36–03:59 UTC** trägt **100 %** aller drei Stufen. **Planung vor 00:00 UTC** ist harte Startbedingung. Kapazitätstor bei Parallelität 1: **2.522 ≥ 1.812**.
...
Vollbeleg: [SR §36](betrieb/500-funktionstest-sicherheitsrahmen-2026-09-01.md) (Protokoll) und **SR §37** (Ursachen-/Wirkungsprüfung, rein lesend). Als **#300** gemergt (Merge-Commit `350d901`, Deployment `dpl_DFHbTQo5T4fButYGbEXUHgzSsfnP` READY); der Merge änderte keinen Anwendungscode und keine Production-Daten — unmittelbar danach rein lesend nachgeprüft (29/5/24, Stufe A 20/20 inaktiv, 0 Löschmarken, `helmut_jobs` 7205, `llm_used` 68, `crawlRuns` 20, `md5` der Zeile `main` **unverändert**). Die spätere Änderung dieser Zeile (16:04 UTC) stammt vom **regulären 16:00-Pipelinecron** — 223 Aufträge (`source_fetch`/`document_understanding`), **0 Kohortenaufträge**, **0 Profiländerungen**; der jüngste `crawlRun` datiert weiterhin vom 03.09.

**Erfolgreicher Kern.** Ein scharfer Lauf `provisionierung --stufe=a` (UTC 11:38–11:40) legte **20 synthetische Profile der Stufe A** an: 20 angelegt, 0 fehlgeschlagen, **alle inaktiv und isoliert** (Isolationsbeleg 9/9 ok). Die bisherigen **9 Mandats- und 10 Identitätszeilen bytegenau unverändert**.
...
- **Wurzelursache beseitigt:** `compactStore` verkleinert `crawlRuns` **in keiner Konfiguration** mehr. Die erste Fassung kappte noch mit „belegter" Grenze — der eigene Review verwarf das: mit `HELMUT_CRAWL_RUN_RETENTION=20` wäre der Vorfall per Umgebungsvariable **wiederholbar** geblieben. Die Aufbewahrung durchzusetzen ist jetzt allein Sache von `saveCrawlRun`, dem einzigen Schreiber. **Preis:** eine gesenkte Aufbewahrung wirkt erst beim nächsten `saveCrawlRun` — heute also vorerst gar nicht (SR §37.3). Sichere Richtung, verliert nichts.
- **Eine Wahrheit:** neues Modul `lib/helmut/speicherpfad-vorflug.js` (reine Logik). `storage.js` liest die Regel von dort; die Zweitwahrheit in `scripts/op25-production-nachweis.js` ist geschlossen und blockiert fail closed ohne belegte Aufbewahrung.
- **Grenze nie unter dem Lesefenster (20)** — sonst schaltet sich der Google-Cooldown still ab, er verkürzt sich nicht.
- **Vorflug-Riegel** in Provisionierung, Aktivierung, Entfernung **und `provision-tenant` gegen Production**: Abbruch **vor dem ersten Schreibvorgang** mit **Exitcode 2**, wenn Aufbewahrung, Backend-Kohärenz, Zeilenkennung oder der wirksame Profil-Schreibmodus fehlen — ein echter Mandantenvorgang kann damit nicht mehr **still blob-only** schreiben (Runbook mitgezogen). Nur eine **tatsächliche** Abweichung der Zeilenkennung blockiert; ein ausdrücklich auf `main`/`main-auth` gesetzter Wert gilt als unverändert. **Bewusst ohne Riegel: der Rückweg** — er ist die Notbremse und darf nie an einer Vorbedingung scheitern; die Nacharbeit fasst `main` ohnehin nie an. `blob-purge` und `reverse-backfill` verlangen bereits hart `profileDbModeEnabled()` und bleiben reine Blob-Werkzeuge.
...
- **Beide Werkzeugbefunde geschlossen:** `realeMandateBeruehrt`/`loeschtNichts` **ersatzlos entfernt** (ein Ersatzfeld wäre strukturell invariant und damit wieder eine Behauptung); vier tautologische Tests prüfen jetzt Verhalten. `erhebungsSql()` liest die Adresse aus dem Auth-Blob statt aus `profiles.email` — rein lesend gegengeprüft: **20/20 `.invalid`**, vorher 20 Leerwerte.
- **Tests:** Offline-Gesamtlauf **319/319 grün**, Browser-/Mobile-Smoke **32/0**, neue Suite `scripts/speicherpfad-schutz-test.js` **88/88** (der Stufe-A-Nachweis läuft über alle **20** Kennungen der verbindlichen Stufendefinition, nicht über eine Teilmenge). Eigener adversarialer Review über fünf Linsen (30 Feststellungen, je zwei Widerleger) und eine zweite Betreiberprüfung mit **vier** Nachbesserungen (SR §38.8).
