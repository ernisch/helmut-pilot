...
- **Gebaut:** fail-closed **Kommunikationsriegel** vor allen sieben Außenkanälen (495/495 gesperrt, Netzzähler 0) · Kohortenwerkzeuge mit Erlaubnisliste · Kapazitäts-/Kostenriegel · Azure-**Endpunktguard** · Geheimnisredaktion an vier Leckwegen korrigiert.
- **Azure-Messungen** (je eigene Freigabe, `env -i`, `store: false`): 24/24 Aufrufe, **0,040841 USD**, 0 Fehler.
- **Maßgebliche Telemetriequelle korrigiert:** `helmut_store.data.llmUsage` (Ring 5.000), nicht die leere Tabelle `llm_usage` (K1–K6). Fenster 02.07.–01.09.: 3.673 Erfolge · **19 Azure-Fehler (0,51 %)** · 1.260 Budgetablehnungen (Bedarfsnachweis).
- **Gemessener Tagesbedarf:** p95 **170** / max 298 (UNTERGRENZE); Verstehen 82 · Büro 24 · Lage 7. **Ursache der Untererfassung bewiesen** (unbedingtes Lese-Ändere-Schreibe auf dem Blob; 7,9 % der Dual-Write-`processRuns` nur relational).
- **Azure-Deploymentkontingent bestätigt** (Betreiber 02.09.): 250.000 TPM / 250 RPM, `gpt-5-mini`, Global Standard, 2025-08-07, Sweden Central. **Gesamtkontingent des Kontos bleibt unbelegt.**

## 24 · Sprint 02.09. — Vorbereitungssprint 500er-Funktionstest (**Draft-PR, nicht gemergt**)

*Teilweise abgeschlossen — offline bewiesen, keine Production-Wirkung.* Branch `claude/helmut-500-test-prep-qpc12m`, Basis `881739da`. Vollbeleg: [`betrieb/500-funktionstest-sicherheitsrahmen-2026-09-01.md`](betrieb/500-funktionstest-sicherheitsrahmen-2026-09-01.md) **§18–§20**.

- **Der Kernbefund:** „synthetisch" existierte außerhalb des Kommunikationsriegels **nicht**. Die fünf realen Mandate waren an **vier** Stellen verdrängbar — KI-Budget (ein globaler Zähler, „wer zuerst kommt"), Priorisierung (Streuwert ⇒ rund **10 % Chance je reales Mandat und Tag**), Warteschlange (gleiche Prioritätszahl) und Laufzeit (feste Listenreihenfolge gegen 240 s).
- **Gebaut:** kanonische Klassifizierung (`lib/helmut/mandatsklasse.js`, kein realer Slug) und darauf vier Schutzregeln — Vorrangreserve im Budget (`HELMUT_TESTLAUF_VORRANG_REAL`, Default 0), reale Mandate zuerst in Rotation, Prioritätsaufschlag +1 synthetisch, reale Mandate zuerst bei hartem Zeitbudget. **Bei 0 synthetischen Zeilen byte-identisch** (gegen eine nachgebaute Kopie der alten Funktion belegt). Synthetische Profile bauen zudem **keine eigenen Außenquellen** mehr (sonst ~1.000 Google-News-Abrufe je Zyklus, OP-15).
- **`recordLlmUsage` relational geschlossen (Phase 2):** Dual-Write nach Muster W-2/`process_runs`, Flag **AUS**, Migration `20260902121500` + Rollback als Datei — **nicht angewendet**. Der Blob bleibt die Lesequelle. Phase 3/4 offen, **p95 170 bleibt Untergrenze**.
- **Kommunikationsriegel an vier Stellen gehärtet** (zweite Lage an der Netzgrenze in `sendPush`, `sendeMailpit`, `sendeResend`, `versendeAbsichten`); eine zu grüne Zusage zurückgenommen. Der KI-Ausgang ist **kein** Riegelkanal — ihn begrenzt das Budget.
- **05:45/05:48 umgangen statt bewiesen:** Testfenster **11:36–15:59 UTC (263 min)**; Watchdog sperrt 05:30–08:30 UTC. Die Aktivierung verlangt einen bestandenen Fensterbefund (fail closed); der **Rückweg wird nie** durch ein Zeitfenster blockiert. `laufzeitUeberschneidungen` rechnete nur in eine Richtung — es sind **zwei** Paare.
- **Stufenkontrolle und Rückweg geschaffen:** `funktionstest-kontrolle.js` (die Regeln hatten **keinen** Aufrufer) mit A13 Dubletten, A14 Verdrängung, A15 Mindestumfang — 15 Regeln, 6 Pflichtgrenzen. `testkohorte-rueckbau.js` ist der **erste tatsächliche Ausführer** des Rückbaus. Die Rückbauprüfung bestätigte zuvor einen **leeren** Bestand als Erfolg — aus 4 Befunden sind **8** geworden.
- **Vorbereitete Betreiberwerte (NICHTS gesetzt), acht Stück:** Deckel **2.416** · Verstehens-Reserve **702** · Vorrang real **200** · **RPM 82** (nicht 250 — bei 3.018 Token/Aufruf bräche 250 die TPM-Grenze) · TPM **250000** · Kostenbudget **10,00 USD/Tag** (≈ 213 USD/Monat, Listenpreis, F7 offen) · Parallelität **1** · Kommunikation `gesperrt`. **`HELMUT_TENANT_LLM_CAP` NICHT einschalten** (495 × Fallback 40 = 19.800, keine Summenprüfung). Der Deckel wird **vor** der Vorrangreserve angehoben (§20.1).
- **Ehrliche Grenze:** bei Parallelität 1 braucht der volle Deckel **367 min** > sicheres Fenster (263 min). Ein voller Tag endet an A05, nicht am Deckel.
- **Tests:** Offline-Gesamtlauf **grün, Exit 0** · Browser-Smoke **32/0** · alle 14 Datenbankvertrags-Suiten grün. Fünf neue Suiten, dreizehn erweiterte. Zahlen je Suite: §18.12/§19/§20.
- **Gegenanalyse (72 Agenten, 7 Teilsysteme): 8 Befunde, 6 hier geschlossen (Beleg §19):** Einladung/Reset trugen **nie** eine Mandatskennung (`politicianId` nur bei Rolle `abgeordneter`) ⇒ Auflösung über die echte Bindung · EUR-Profildeckel für die Kohorte ein **No-op** ⇒ 10 ct/Tag als Rückfallnetz · Fensterbefund **zeitlos** (gestern erhoben, 05:47 erlaubt) ⇒ `jetztUtc`+`gepruefteCrons` Pflicht · Fairness-Zeile überlebte den Rückbau 90 Tage ⇒ Nacharbeit `--spur` · der 5.000er-Ring kürzte 30-Tage-Berichte **still** ⇒ jetzt sichtbar · `slotKapazitaetReicht` nie ausgewertet ⇒ neue Bindung.
- **Adversariales Diff-Review (54 Agenten, 6 Dimensionen, gegengeprüft): 20 Befunde, alle geschlossen (Beleg §20).** Der schwerste traf die eigene Änderung: die **Vorrangreserve** zog auch der GETEILTEN Verstehensarbeit Budget ab, während `storage.js` und die **betreibersichtbare** Ausgabe das Gegenteil behaupteten — bei Deckel 100 gegen Vorrang 200 wäre `effectiveMax = 0` für jeden Verstehensaufruf gewesen: der Datenmotor **auch der fünf realen Mandate** hätte stillgestanden. Behoben: ehrliche Beschreibung + **Untergrenze** + neunte Hürde (Deckel/Reserve aus der LAUFENDEN Umgebung). Weiter: die Klassentrennung ließ **5 synthetische Profile 30 Tage verhungern** (Rotationsschritt nicht teilerfremd; nachgemessen, jetzt 0) · **sechs** Regeln der Stufenkontrolle meldeten eine gemessene 0, ohne zu messen (A13 durch UNIQUE-Index eine **Tautologie**, A01/A06 lasen die leere `llm_usage`, A10 die falsche Größe, A12 hielt „unbewertbar" für „frei", A14 **totale** Verdrängung für „in Ordnung") · Rückbau meldete `ok:true` bei **leerer** Zielmenge · fünf abgeschwächte Testverträge geschärft.
