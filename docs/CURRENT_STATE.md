...
**Stand: 2026-08-29 — zuletzt aktualisiert zur Z22-Vorwaertskorrektur (§18).** Vollständige
...
- **`main` = `0ddb73e9` = Merge von PR #272** (Realistiknachweis Z3a, §17); davor `ade1674e` (#271) und `07bf7794` (#270). Ältere Angaben sind überholt. Das Vercel-Production-Deployment ist erfolgreich, der öffentliche Alias liefert `0ddb73e9`.
- **PR #273** (Befund Z22, §18, Branch `claude/z22-tenant-isolation-after-z3a`) ist offen und **seit 28.08. direkt auf `main`** basiert (vorher der bereits gemergte Zweig von #272). **Nicht gemergt** — Merge = Production-Deployment und Betreiberentscheidung. Daneben offen: #274–#277.
...
- **Nachtrag 28.08. — zwei Nachweislücken geschlossen** (Basis jetzt `main`): der Datenbanknachweis wurde ohne `HELMUT_TEST_PG_HOST` still übersprungen und meldete trotzdem **grün**; er läuft jetzt fail-closed in einem eigenen CI-Schritt gegen einen kurzlebigen `postgres:17`-Dienst. Und eine **leere `tenant_id` auf der Zeile** behandelten SQL (2) und Attrappe (3) verschieden — maßgeblich ist der sichere Vertrag (4: keine brauchbare Kennung = geteilte Arbeit). Gegenprobe §8b ist gegen den alten Stand rot. Einzelheiten: Belegdatei §14.
- **Unverändert offen:** Z3 vollständig (Z3b) · Z4 nicht erteilt · Z5 nur fünf reale Mandate · Migration auf Production (freigabepflichtig) · Merge von #273 · §11 (echtes PostgREST) läuft mangels Binary nirgends · die übrigen ~35 Datenbanksuiten überspringen weiterhin still.
