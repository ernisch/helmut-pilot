...
**23.09.2026: Manueller GitHub-Actions-Ausfuehrungsweg fuer den einmaligen 169er Verstehenslauf vorbereitet; NICHT ausgefuehrt.** Neuer Workflow `.github/workflows/verstehen-169-einmalig.yml` (nur `workflow_dispatch` auf main, exaktes Bestaetigungswort, `run_attempt=1`, `contents: read`, Concurrency `helmut-500-kontrollierte-facharbeit` mit `cancel-in-progress:false`, Timeout 40 min, gepinnte Checkout-Actions ohne persistierte Credentials, direkt `node scripts/verstehen-einmalig-169.js` — nicht ueber `scripts/lokal.js`; GitHub-Secrets `SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY`/`AZURE_OPENAI_KEY`/`AZURE_OPENAI_ENDPOINT` plus bestehendes Azure-Deployment-Muster, Werte werden nie geloggt; fail-closed-Preflight unmittelbar vor dem Start). Keine Fachlogik geaendert: 169/122/Hash, hoechstens113 Kandidaten, 0,80 USD, 35 min und der 4-USD-Tagesriegel bleiben unveraendert im Runner. Neue statische Vertragspruefung `scripts/verstehen-169-workflow-test.js`; Branch `codex/verstehen-169-actions-20260923` (Deployment in `vercel.json` deaktiviert). **Kein Dispatch, kein scharfer Lauf, keine Production-Wirkung; der Start braucht die ausdrueckliche Betreiberfreigabe.** [Beleg](betrieb/verstehen-einmalig-169-20260922.md#15-manueller-github-actions-ausfuehrungsweg-vorbereitet-2026-09-23).
