...
- **Skalierung:** die Briefingbelege aller Mandate kommen gebündelt (`listRenderedBriefingsV3ForTenants`, eine Abfrage je 100 Zeilenkennungen), `getPushUebersicht` holt Ereignisse und Abos aus **einem** Zugriff, und die **mandantenneutralen** Motorsignale (Quittungen, Queue, CAS, Fehler, Nutzer, Feedback, Budget, Abdeckung) werden **einmal je Lauf** statt einmal je Mandat geholt. Gemessen bei 5 **und** 100 Mandaten: **1** neutrale Abfrage je Signal, **1–2** Briefingabfragen, **2** mandantenbezogene Abfragen je Mandat (Lage-Check, Push-Übersicht). Vorher waren es bei 100 Mandaten 100 identische Quittungsabfragen und 200 Einzelbelegabfragen.
- **Tests:** `briefing-frische-audit-test.js` **38 PASS / 0 FAIL** (F1g–F1j: die Bündelung trägt dieselbe Mandantenhärtung wie der Einzelpfad) · `motor-health-test.js` **65 PASS / 0 FAIL** (zehn Pflichtfälle plus Bestand plus Regressionsschutz K14) · `health-report-route-test.js` **38 PASS / 0 FAIL** (echter Routenlauf, sechs Pflichtlagen, Skalierung, Rückschau). Gesamtlauf **280/282** (`scripts/lokal.js`, 706 s); die zwei Ausfälle `kalender-ics` und `lambda-paket` sind fehlende npm-Pakete der Cloud-Umgebung (`ical.js`, `@aws-sdk/client-sqs`) und bestanden vor diesem Sprint unverändert.
