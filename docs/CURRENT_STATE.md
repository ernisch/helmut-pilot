...
- **Kapazitätsmodell, an der Messung geeicht.** `source_fetch` ist über Mandate dedupliziert: **139 geteilte Quellen** + **6 je Mandat**, je 2×/Tag. Die Formel liefert für M=5 **exakt die gemessenen 338**. Hochrechnung **841 / 1323 / 2287** Aufträge/Tag bei 25/50/100 — der Motor skaliert **unterlinear**.
- **Synthetischer Lasttest bestanden: 60 PASS / 0 FAIL** über alle drei Stufen (echter Motor, echte lokale PostgreSQL, echte Workerprozesse, echtes Fehler- und langsames Mandat; **Attrappen** für KI und Netz). 0 hängende Leases, 0 Dubletten, 0 unerwartete Fehler, Rückstau abgebaut, Fairness min = max. **Das misst die lokale Mechanik, nicht die Wirklichkeit** — Production ist ~2 300× langsamer.
...
- **Gesamtlauf: 279/279 Suiten grün in 729 s, 0 FAIL** (277 Bestandssuiten + 2 neue). Darin grün: alle vier vormals roten Befunde. Ergänzend mit echter lokaler PostgreSQL: Lasttest 60 PASS (zweimal), Ankunftskennzahl 24 PASS, `jobqueue-datenbank` 55 PASS, `jobqueue-lasttest` 19 PASS inkl. SIGKILL-Probe.
...
- **Review 2026-08-25/2 an PR #270 — drei Befunde bestätigt und behoben:** die neue Migration hätte `service_role` **kein** EXECUTE erteilt und wäre in Production **wirkungslos** gewesen (empirisch belegt); der **Trockenlauf meldete falsches Grün** und sagte Konto-/E-Mail-/Deaktivierungskonflikte nicht voraus; ein Abbruch **nach** der Auth-Kontrolle ließ ein halbes Konto zurück. Alle drei behoben und testgesichert (`provision-stapel-test` **66 PASS**, `jobqueue-ankunft-datenbank-test` **30 PASS**).
- **Beleggrenzen zurückgenommen:** Verbindungsspitze 5/300 ist ein **lokaler** Messwert, kein Supabase-Grenzwert; der synthetische Test beweist **nicht**, dass Supabase unter realistischer Last trägt; Vercel > 300 s gilt **erst mit bestätigtem Fluid Compute** (Projekt-API gibt dazu nichts her); 15/21/33 USD sind **angenommene Szenarien**. Von 139 Lücken sind **11 geschlossen**, eine vorbereitet (F9), der Rest erfasst ([`betrieb/skalierung-25-50-100.md`](betrieb/skalierung-25-50-100.md) §4.8).
