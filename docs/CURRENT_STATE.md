...
**Stand: 2026-08-31 — zuletzt aktualisiert zur Korrektur der Understanding-Laufmeldung (§19).** Vollständige
...
- **`main` = `afc807e0` = Merge von PR #274** (KI-Antwortumschlag, 29.08. 22:02 UTC nach Betreiberfreigabe); davor `bb0577a9` (#281), `87bed221` (#280), `4f019797` (#279), `ae74a9bc` (#273). Das Vercel-Production-Deployment zu `afc807e0` ist `READY` (rein lesend über die Vercel-API geprüft); alle Laufquittungen des 30.08. tragen genau diesen `commit_ref`.
- **PR #273, #274, #279, #280 und #281 sind gemergt.** Offen bleiben die gestapelten PRs #275–#277 (Basis war der #274-Branch) sowie **#282** (Beweisgatter Zehn-Mandate-Übergang, enthält die Grüntage-Auswertung); sie sind nicht Teil von Production.
...
**Teilweise abgeschlossen.** Vollständiger Befund mit allen Zahlen: [`betrieb/understanding-telemetrie-korrektur-2026-08-31.md`](betrieb/understanding-telemetrie-korrektur-2026-08-31.md). Branch `claude/understanding-telemetrie-korrektur-ty5gb2`, Kopf `ae5c870` — **PR #283 offen, Pflicht-CI vollständig grün, ungemergt; kein Merge, kein Deployment, keine Preview, keine Migration, keine Datenänderung**; Production ausschließlich lesend geprüft. Der Branch ist am 30.08. nach Betreiberfreigabe **nur zur Sicherung** nach GitHub gepusht, nachdem er sich in `vercel.json` selbst für Deployments gesperrt hatte; Wirksamkeit vorab empirisch belegt, danach rein lesend bestätigt (0 Deployments, keine Preview, Production unverändert `afc807e0`).

- **Meldefehler (lokal behoben).** Der natürliche 21:30-Lauf meldete `success` bei 18 gespeicherten Ergebnissen **und einem echten Fehlerfall**; `saved/skipped/failed_count` standen auf **0** — an **allen zehn** Quittungen des Tages. Zwei Ursachen: der Status kam allein aus `result.skipped` (`partial`/`failed` waren unerreichbar), und nicht übergebene Zähler wurden über `Number(null) === 0` in `blob-relational.js` zu einer **gemessenen Null**. Neu `lib/helmut/lauf-bilanz.js` als **eine** Ableitung aus der Fachtelemetrie; `understanding-cron` und `understanding-lage` nutzen sie. **Nachtrag 31.08. (unabhängige Prüfung):** eine nicht stimmige Bilanz, eine fehlende Arbeitsliste und ein ungültiger Zählerwert konnten weiterhin `success` bzw. eine stille Null ergeben — geschlossen mit den festen Fehlerklassen `zaehlerwiderspruch`/`telemetrie-unvollstaendig`; dafür musste auch `sanitizeProcessRun` denselben `num(null) === 0`-Fehler ablegen. Der Lauf ergibt jetzt `partial · 18/0/1/32`, Identität 18+0+1+32 = 51 = `cluster`.
- **Parserfall: nicht erfolgreich, aber andere Ursache.** Genau eine Reservierung wechselte im Lauffenster nach `unbekannt` (`versuche`/`ki_aufrufe` 2→3, zuerst bearbeitet, Protokoll `wiederaufnahmeFreigabe: true`); Ursache `KI-Antwort wurde nicht vollstaendig abgeschlossen` — die von PR #274 **gewollte** Ablehnung. **Der Parser bleibt unverändert.**
- **Neuer roter Befund:** ein **zweiter** `unbekannt` am 30.08. 20:02 UTC. Ein U+0000 aus dem Modelltext erreichte die Ablage und wurde von PostgreSQL mit `22P05` abgewiesen — der bezahlte Aufruf ging verloren. Synthetisch reproduziert; `cleanEntry` verdichtet jetzt C0-Steuerzeichen wie Leerraum.
- **217,5 s ist keine Laufzeitgrenze.** Der Wert ist **K9 (Tages-p95)** der Grüntage-Auswertung aus dem **ungemergten** PR #282 und steht in keinem deployten Code. Der Lauf endete 59,0 s vor der harten 280-s-Grenze; die Restzeitwache vertagte 32 Vorgänge korrekt, ohne weiteren Modellaufruf. **Keine Laufzeitkorrektur.** Der 30.08. ist dennoch **kein grüner Tag**: p95 = 279 563 ms, Treiber ist der 20:00-Crawlslot (K10 hielt mit 437 ms Abstand).
- **Tests (alle über `scripts/lokal.js`):** neue Suite `lauf-bilanz-test.js` **149 PASS / 0 FAIL**; Offline gesamt **284/286** — die zwei roten sind fehlende lokale npm-Pakete (`ical.js`, `@aws-sdk/client-sqs`) außerhalb des Änderungssatzes.
- **Kapazitätsbefund (unverändert offen, kein Meldefehler):** Ankunft ≫ Abfluss an jedem Tag 23.–30.08. (30.08.: 286 neue Wissensobjekte, 69 verstanden). Rückstand **9 065** `pending`, davon **8 836 älter als 24 h**, ältestes vom 02.07. Obergrenze ist der KI-Tagesdeckel: 84 Aufrufe am 30.08., **100 und damit am Deckel am 29.08.**
- **Offen:** Production-Nachweis der Korrektur (nur ein späterer **natürlicher** Lauf nach gesondert freigegebenem Merge), beide `unbekannt`-Vorgänge, **Siebentagenachweis nicht begonnen**.
