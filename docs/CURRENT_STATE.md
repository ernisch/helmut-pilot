...
**Stand: 2026-09-03 — `main`-Kopf `764a770` (#298, reine Doku). Der Production-Profilpfad ist relational wirkungsbelegt; die relationale Ausschussabweichung **aller fünf** aktiven realen Profile ist am 03.09. mit Freigabe korrigiert und rein lesend abgenommen (§27). Der 500er-Funktionstest bleibt **nicht startbereit**, Stufe A **nicht freigegeben**.** Nur der aktuelle, entscheidungsrelevante Zustand (Grenze 30.000 Zeichen, testgesichert); Ablageregeln: [`archive/README.md`](archive/README.md), `CLAUDE.md` §9.
...
9. **500er-Funktionstest: NICHT startbereit, nicht freigegeben.** Stand 03.09. (§26, Kette §11.2): **PR #297 ist gemergt und der Vercel-Status am Merge-Commit erfolgreich**; Stufe A 20/20 und Bundestagsreife-Sperre geschlossen. Vor einer inaktiven Provisionierung bleiben **aktuelle Grundlinie + Sicherung der betroffenen Tabellen** Pflicht und die Provisionierung braucht eine eigene Freigabe. **Die relationale Profilabweichung ist seit 03.09. aufgelöst (§27)** — sie blockiert die erste Kohortenaktivierung nicht mehr. Vor der ersten Aktivierung müssen die **acht Betreiberwerte** wirksam belegt sein. Offen: Zahl offener Aufträge (erst nach Provisionierung messbar), Laufzeit der KI-freien Klassen, Azure-Gesamtkontingent, Verdrängungsschutz unter Last, Mehrtagesbetrieb.
...
1. **Profilkorrektur erledigt (§27).** Nächster Schritt dort: den natürlichen Radar-Wirkungsbeleg abwarten (§9) — nichts auslösen.
2. **Kette der Stufe A (weiterhin NICHT freigegeben):** Grundlinie → Sicherung der betroffenen Tabellen → Fenster → `provisionierung --stufe=a` (inaktiv, eigene Freigabe) → `isolation --stufe=a` → **acht Betreiberwerte setzen und wirksam prüfen** → Aktivierung A (eigene Freigabe) → Fachzyklus A → Kontrolle A. B und C getrennt, erst nach kontrollierter Vorstufe. Plan: `node scripts/lokal.js -- node scripts/funktionstest-500-ablauf.js plan` (jeder Testlauf über `lokal.js`).
...
- **Dabei verschärft:** veraltete Ausschussnamen liefen bisher als gültig durch (3 von 4; Negativliste nur im Fehlerzweig gelesen). Jetzt weisen sie ab (36 Varianten geprüft), Kurzformen bleiben gültig (32). Harte Sperre nur bei Neuaktivierung; für Bestandsprofile ändert sich nur die **Anzeige**.
- **Production-Profilpfad (03.09., rein lesend; §34.13.6a/.6b):** Production verwendet nachweislich die **relationalen Profile aus `mandate_profiles`**. Zwei unabhängige Belege: die ausgeführten profilbezogenen Ausschussradare in `source_crawl_telemetry` entsprachen exakt den relationalen `committees[0]`, nie den abweichenden Blob-Werten; und eine Blob-Deaktivierung vom 04.08. blieb ~30 h wirkungslos, erst die relationale vom 06.08. beendete das Radar dauerhaft. Der rohe Wert von `HELMUT_PROFILE_DB_MODE` bleibt unbestätigt, die Wirkung ist eindeutig belegt.
- **Profilabweichung erkannt, NICHT durch #297 verursacht** (Befund; Auflösung siehe §27): die Korrektur vom 04.08. wurde nur im Blob vollzogen, nicht in die relationalen Zeilen übertragen. `mergeProfileLists` ersetzt bei gleicher Kennung das ganze Blob-Profil durch das relationale — SQL gewinnt, also war der veraltete Stand wirksam. `scripts/profil-bereitschaft.js --production` ist als Klärbeleg **unzulässig** (Lesepfad hängt am lokalen `process.env`; `readSupabaseStore` kann eine fehlende Blob-Zeile anlegen) und bleibt es.
- **Lastfolge (§34.13.4):** 42 der 495 Profile erhalten über „Arbeit und Soziales" **ein** Sachpaket zusätzlich; kohortenweit **54 → 138 `source_fetch` je 8-h-Fenster** (**+252/Tag**, nicht einmalig). Mandatsgebundene Klassen unverändert, Kapazitätsmodell (SR §30.7) unberührt; Folgeschritt über `document_understanding` **unbeziffert**. `scheduler.mandateNewsSources` Nr. 4 baut die Suchanfrage aus `committees[0]` — heute inert, mit `HELMUT_TESTKOHORTE_QUELLEN=aktiv` **1.802** statt 138. **Der Schalter bleibt AUS.**
- **Gefunden, NICHT repariert (§34.13.7):** `seeds/entities.js` führt eine **zweite** kuratierte Ausschussliste (23) für den Radar-Ausschussbeleg — 21 von 24 WP-21-Namen haben einen Eintrag, 3 sind WP-20-Bezeichnungen. Stille Radarlücke für **reale** Mandate → eigener PR.
...
*Erfolgreich abgeschlossen.* Amtliche WP-21-Belege (bundestag.de-Biografien aller fünf Mandate) vom Betreiber übergeben — **diese Sitzung hat sie nicht selbst abgerufen** (`*.bundestag.de` per Egress gesperrt). Vollbeleg: [SR §35](betrieb/500-funktionstest-sicherheitsrahmen-2026-09-01.md).

- **Befund:** Die Blob-Korrektur vom 04.08. blieb relational unwirksam — betroffen waren **alle fünf** aktiven Profile, nicht nur eines: `ausschuesse` bei **vier**, `stellvertretende_ausschuesse` bei **fünf** (alle leer). Nur `ottilie-paola-klein-2` trug zusätzlich eine **nicht auflösbare** Angabe; die übrigen Fehlwerte waren formal auflösbar und blieben deshalb still.
- **Ausgeführt** (Freigabe 03.09., **eine** atomare Transaktion, Compare-and-Set je Zeile): 5 Zeilen, **8 Feldänderungen**, ausschließlich `ausschuesse`/`stellvertretende_ausschuesse`. Bewusst nicht geschrieben: `cem-ince.ausschuesse` (bereits korrekt), `helmut-kleebank.stellvertretende_ausschuesse` (bleibt leer). Vorher Sicherung `backup-export.js --scope=profil` (`vollstaendig: true`, 2/2 Tabellen, 0 Fehler).
- **Ausgeschlossen** (kein ständiger Ausschuss der Sollmenge, Resolver weist ab): **Rechnungsprüfungsausschuss** (Stüwe), **Parlamentarischer Beirat für nachhaltige Entwicklung und Zukunftsfragen** (Kleebank) — dokumentierte Modelllücke, Datenmodell unverändert; „Schriftführer" bleibt Amt.
- **Abnahme rein lesend, alle bestanden:** fünf Zeilen exakt auf den Freigabewerten · 9 Profile / 5 aktiv / 4 inaktiv / 0 Löschmarken · `created_at` und `updated_at` **aller neun** Zeilen unverändert · vier inaktive Zeilen unverändert · `helmut_store` unverändert · **12/12 Zielwerte lösen gegen `resolveBundestagsausschuss` auf, 3/3 Ausschlüsse korrekt abgewiesen** (offline über `lokal.js`). Relational und Blob sind jetzt deckungsgleich.
- **Sachkorrektur:** „Bildung, Forschung und Technikfolgenabschätzung" wurde **nicht aufgeteilt**, sondern war ein **Zuschnittwechsel 2→2**: Nr. 18 gab *Bildung* ab und erhielt *Technologie/Raumfahrt*, Nr. 13 erhielt *Bildung*; beide behielten ihre Nummer, die Gesamtzahl sank 25 → 24.
- **Offene Codearbeiten (eigener PR):** (a) in `VERALTETE_AUSSCHUSSNAMEN` fehlen „Ausschuss für Bildung, Forschung und Technikfolgenabschätzung" und „Ausschuss für Familie, Senioren, Frauen und Jugend" — letztere löst sogar eindeutig auf Nr. 13 auf und ginge still als aktuell durch; (b) `scripts/profil-bereitschaft.js` behauptet in Z. 13/71 „rein lesend" — für `--production` strukturell zu stark.
- **Nicht enthalten:** keine Provisionierung, keine Aktivierung, **Stufe A nicht freigegeben**, keine Migration, keine Env-/Cron-/Azure-/Budgetänderung, kein Modellaufruf, kein Blob-Schreibvorgang.
