...
- **Befund (reproduziert):** `provisionierung --stufe=a` ignorierte die Stufe still — zielGroesse **495**, Pauschalwort `TESTKOHORTE_495_ANLEGEN_BESTAETIGT`, Exit 0. Die Bibliothek war seit 02.09. stufenfähig; der Betreiberweg reichte es nicht durch.
- **Korrektur:** `--stufe=a|b|c` ist Pflicht; fehlend/leer/unbekannt → **Exit 2 vor jedem Zugriff, kein Rückfall auf 495**; `--ids=` nur innerhalb derselben Stufe. Gemessen: **a → 20 / b → 75 / c → 400** mit stufengenauem Wort. Bibliothek unverändert (ohne `stufe` weiter 495).
- **Ablaufplan stufenweise (28 Schritte, kein Sammelschritt):** rein lesender Reifebeleg → je Stufe Anlage (inaktiv) → Isolation → Aktivierung → Fachzyklus → Kontrolle; B erst nach kontrollierter A, C nach B. **Die acht Betreiberwerte sind Vorbedingung jeder Aktivierung und keiner Provisionierung** (§34.5); rein lesend belegbar ist davon nur der Deckel. Rückweg nie gesperrt.
- **Verhaltensbelegt:** die inaktive Provisionierung (echter `provisionTenant`) erzeugt **0** Netzaufrufe, **0** Riegelanfragen, **0** KI-Aufrufe, Außenkanalmodule nicht geladen; der echte Planer plant für 20 inaktive Profile **0** Aufträge (Gegenprobe: 1 aktives → 2). **Dabei geschlossen:** die Verstehens-Interessenprüfung zählte inaktive Profile mit — jetzt Planer-Prädikat `isDisabled` (Flag in Production ungesetzt, heute inert).
- **§34.7 geschlossen (Variante A):** die Reifesperre wies **18 von 20** Stufe-A-Profilen ab („Testausschuss N" fehlt in der WP-21-Sollmenge). Behoben in der **Kohorte**, nicht in der Regel: die **434 Bundestagsprofile** tragen jetzt amtliche Ausschussnamen der 21. WP aus der vorhandenen Sollmenge (`seeds/bundestag-ausschuesse.js`, **keine zweite Namensliste**), die **61 Landtagsprofile** weiter synthetische — ein Bundestagsausschuss auf Landesebene wäre die falsche politische Ebene. Kennungen, `.invalid`-Adressen, Testnamen, -parteien und -themen bleiben synthetisch. Der echte, unveränderte Pfad nimmt Stufe A **20/20** an, die Reifeprüfung **495/495**.
- **Dabei verschärft, nicht gelockert:** veraltete Ausschussnamen früherer Wahlperioden liefen bisher als gültig durch (3 von 4 dokumentierten WP-20-Namen — die Negativliste wurde nur im Fehlerzweig gelesen). Jetzt weisen sie ab; Kurzformen bleiben gültig.
- **Lastfolge gemessen:** 42 der 495 Profile erhalten über „Arbeit und Soziales" **ein** Sachpaket zusätzlich (je +84 Quellen); kohortenweit **+84 `source_fetch` einmalig** (54 → 138), die mandatsgebundenen Klassen (je 495) unverändert — das Kapazitätsmodell (§30.7) bleibt unberührt. Gemessen gegen den Offline-Katalog (`lokal.js` erzwingt `HELMUT_SOURCE_MODE=off`).
- **Tests:** `testkohorte-vorwaerts-cli-test.js` **55/0** (echte Kindprozesse, Schreibspion), `testkohorte-provisionierung-inaktiv-test.js` **47/0** (A0 belegt jetzt positiv den echten Pfad), `test-kohorte-500-test.js` **54/0**, `profil-bereitschaft-test.js` **100/0**, `funktionstest-ablaufplan-test.js` **82/0**. Review: vier Linsen, 24 Feststellungen, zwei Widerleger je Feststellung — alle behoben (§34.12). Gesamtlauf und Pflichtprüfungen: PR-Text (§34.10).
- **Keine Production-Wirkung:** keine Provisionierung, Aktivierung, Umgebungsvariable, Migration, Cron-, Azure-, Budget- oder Reserveänderung, kein Modellaufruf, keine externe Nachricht.
