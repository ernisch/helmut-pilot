...
**Stand: 2026-08-29 — zuletzt aktualisiert zum Z22-Production-Abschluss (§18).** Vollständige
...
**Kernlage in sechs Sätzen:** Der Warteschlangenmotor (`HELMUT_SCALABLE_PIPELINE=on`) ist **seit 23.08.2026 in Production eingeschaltet** (Runbook §30.7). Die historische Abnahme der **fünf bestehenden Mandate** belegt 376 echte Abschlüsse, Morgenlauf 5/5 und Lagelauf effektiv 5/5. Damals gab es 0 Doppelarbeit, Verlust, endgültige Fehler, `unbekannt`, Lease- oder Fencing-Probleme. Der neuere natürliche Lauf vom 27.08. endete dagegen mit **1 unbekannten Vorgang; das aktuelle Fünfertor ist rot**. Der Modus bleibt **`HELMUT_JOB_DISPATCH_MODE=shadow`**; der Selbstweck ist nur lokal Ende-zu-Ende belegt und in Production nie ausgeführt (§14). Der R4-/GitHub-Actions-Watchdog-Nachweis bleibt grün.
...
- **`main` = `bb0577a9` = Merge von PR #281** (Z22-Production-Abschlussdoku); davor `87bed221` (#280), `4f019797` (#279), `ae74a9bc` (#273), `a966bf5c` (#278) und `0ddb73e9` (#272). Das automatische Vercel-Production-Deployment ist grün; der öffentliche Alias antwortete am 29.08. mit HTTP 200 und lieferte exakt `bb0577a9`.
- **PR #273, #278, #279, #280 und #281 sind gemergt.** Offen bleiben die gestapelten PRs #274–#277; sie wurden am 29.08. per normalen Merge-Commits auf den aktuellen `main` nachgeführt (#274 hängt jetzt an Basis `main`), sind nicht Teil von Production und brauchen eine eigene Betreiberentscheidung je Merge.
...
- **Datenbank:** Supabase **Free-Plan** — keine herunterladbaren nativen Tagesbackups, kein PITR (→ OP-01). Der alte Restore 40/40 deckt August nicht ab. Der lokale 51-Tabellen-Vertrag ist gehärtet, aber ohne Snapshot und echten Datenbank-Restore kein Vollrückweg.
...
- **Zugangsgrenze:** Supabase lesend. Der Vercel-Kanal sieht nur Team `Nohut`/`pro`, nicht Helmut-Projekt, Deployments, Env, Nutzung oder Logs; Änderungen bleiben Betreiberaktion ([`betrieb/env-inventar.md`](betrieb/env-inventar.md) §8). Parlamentsdomains sind per Egress-Proxy gesperrt; bytegenaue Prüfung läuft über Actions (§14).
...
5. **20 zusätzliche Profile**: gegen amtliche Live-Seiten abgeglichen und bestätigt (Actions 24.08.; maßgeblich ist der letzte PR-Lauf, [Prüfstand](../daten/mandatsprofile-berlin-brandenburg-2026-08-24-pruefstand.md)); Import/Aktivierung freigabepflichtig. **Berliner Wahl 20.09.2026**: zehn Profile gelten nur für die 19. WP; danach erneut prüfen.
...
- **F-3** Anker-basierter Understanding-Recovery-Pfad: in Production gescheitert; die historische Workflowdatei ist entfernt, Pfad **nie ausführen** (`CLAUDE.md` §5).
...
- Keine kostenverursachenden Läufe (Backfills, Recovery, Massen-Crawls); Recoverypfad F-3 nie ausführen; Retention nicht scharfschalten.
...
- **Indexfrage:** Die Abfrageform sperrt einen vorhandenen Index aus; F9 blieb unverändert und ist nicht angewendet. Aktuell 177.474.707 Byte Datenbank; Wachstum unter 500 und wirksame Aufbewahrung sind ungemessen.
- **Lokaler Versions-/Planvergleich:** PostgreSQL **17.6** wie Production, gleicher Plan/Faktor und 18 MB; kein Production-Übertragbarkeitsbeweis.
...
**Teilweise abgeschlossen** — der realistische Nachweis ist **als Z3a erbracht und als Z3 offen**. PR #272 ist als Merge `0ddb73e9` in `main`; dadurch wird Z3a nicht zu Z3. Vollständiger Bericht mit allen Zahlen: [`betrieb/z3-realistiknachweis-2026-08-26.md`](betrieb/z3-realistiknachweis-2026-08-26.md).
...
## 18 · Z22: Befund, Pflichtbeleg, isolierte Anwendung und Production-Abschluss

**Erfolgreich abgeschlossen (29.08.):** PR #273, #279 und #280 sind gemergt; `main` = `87bed221`, automatisch deployt, Alias liefert exakt diesen Commit. **Anschließend wurde Z22 mit ausdrücklicher Betreiberfreigabe auf das Production-Projekt `ddckuvvpcytqbyfmbvie` angewendet.** Alle Zahlen und Prüfsummen: [`betrieb/z3-realistiknachweis-2026-08-26.md`](betrieb/z3-realistiknachweis-2026-08-26.md) §13–§14; Production-Abschluss §14.6.
...
- **Pflichtbeleg:** aktueller Vertragsbeleg **48 PASS / 0 FAIL**. Pflichtlauf von PR #279 `33260203246`: **284/284 Offline-Suiten**, Browser/Mobil **32/0** und der echte Datenbanknachweis gegen PostgreSQL **17.11 mit 41 PASS / 0 FAIL**; §11 gegen echtes PostgREST blieb ausdrücklich übersprungen (1 übersprungen).
- **Vorgeschichte belegt:** isolierte Anwendung der Vorwärtskorrektur im Testprojekt (Buchung `20260829135455`; 900 synthetische Aufträge unverändert, 500 Kennungen geprüft, 0 Abweichungen — §14.4) und rein lesende Production-Vorprüfung vom 29.08. (grün, 4,011 ms Planprobe — §14.5).
- **Production-Anwendung 29.08.:** Basis `20260826190000` unter Production-Buchung `20260829175642`, Vorwärtskorrektur `20260829123132` unter `20260829175749`. Nachprüfung: **3.330 Aufträge** (3.122 erledigt, 208 wartend, 0 laufend, 0 fehlgeschlagen), 3.161 ohne Mandatskennung, 0 leere oder reine Weißraumkennungen, **5 brauchbare reale Mandate** — Bestand unverändert gegenüber der Vorprüfung. Genau **eine** dreistellige Funktion `public.helmut_jobs_offen(text[],text[],text)` mit drei Vorgabewerten, `stable`, `security invoker`, `search_path=public, pg_temp`; alte zweistellige Fassung entfernt; `anon`/`authenticated` gesperrt, `service_role` erlaubt. Fünf globale Gegenproben bestanden; alle fünf Mandate gegen die direkte SQL-Erwartung: **5 passend, 0 Abweichungen**. Lesende Laufzeitmessung **5,973 ms**, 922 Puffertreffer, 0 gelesene/veränderte/geschriebene Blöcke. Keine Auftragsdaten verändert, keine Mandatskennungen ausgegeben, Testprojekt und Azure unberührt. **Z22 nicht erneut anwenden.**
- **Beweisebenen:** Z22 ist lokal, im Pflicht-CI, isoliert gegen Supabase und **jetzt in Production bewiesen** — als Funktions- und Plattformzustand. Ein neuer **Mandatsnachweis** entstand nicht: Production bleibt bei fünf realen Mandaten; die 500 synthetischen Mandate belegen weiterhin ausschließlich den isolierten Warteschlangenmotor.
- **Offen:** §11 (echter PostgREST-Rückfallnachweis), die übrigen still überspringenden Datenbanksuiten, Z3b, Azure, Parserfix (PR #274) und die nächste natürliche Fünferprüfung. Der Fünferlauf vom 27.08. war wegen eines rohen Steuerzeichens im KI-Umschlag rot; die Fixes in PR #274 ([KI](betrieb/ki-antwortumschlag-parserfix-2026-08-28.md)), #275 ([Planung](betrieb/planungs-zeitbudget-hardening-2026-08-28.md)) und #276 ([Monitoring](betrieb/z3b-monitoring-ehrlichkeit-2026-08-28.md)) sind lokal grün, aber **ungemergt und nicht deployt** — in Production unbewiesen. Mergefolge #274 → #275 → #276.
- **Beweisgrenzen Z3b (PR #277):** lokal sind die Z3b-Schutzverträge grün; isoliert gegen Supabase nur der synthetische Plattformweg bis 500; im Fachweg 200/500 nichts bewiesen. Offen: Azure (Anmeldung gesperrt), Google-Sonderweg, Büro-Fachpfad, Beobachtung, Speicher, Aufbewahrung, Tarif, alle Realstufen ([Aktivierungsplan](betrieb/z3b-aktivierungsplan-2026-08-27.md)).
