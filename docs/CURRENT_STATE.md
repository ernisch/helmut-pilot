...
**Stand: 2026-09-04 — `main`-Kopf `8fa390d` (Merge von #301, Speicherpfad-Schutz). Production: **29 Profile, unverändert 5 aktiv**, 24 inaktiv (davon die **20 der Stufe A, alle inaktiv**), 0 Löschmarken; `crawlRuns` steht auf 20 und wurde **nicht wiederhergestellt** (§28). Der **Speicherpfad-Schutz ist gemergt und im Production-Deployment `dpl_BHEaHNmHC9xm9gSZtKCdZM6DbDTN` (READY) wirksam** (§29) — **offline verhaltensbasiert belegt, kein schreibender Production-Test**. **Nichts ist aktiviert; Aktivierung und Stufe B/C bleiben weiterhin gesperrt, bis die acht Betreiberwerte geprüft und gesondert freigegeben sind** (§7.11).** Nur der aktuelle, entscheidungsrelevante Zustand (Größengrenze testgesichert); Ablageregeln: [Archiv](archive/README.md), `CLAUDE.md` §9.
...
## 29 · Sprint 04.09. — Speicherpfad in Production abgesichert · **ERFOLGREICH ABGESCHLOSSEN** (gemergt **#301**, deployt)
