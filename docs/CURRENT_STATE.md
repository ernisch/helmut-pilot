...
Hotfix B4-3: Beweisfamilien im Resolver ·
CSD-Production-Nachweis Anlauf 3: ausgeführt, fachlich bestanden, durch das
Kostenlimit unvollständig ·
**Restlauf für die zwei offenen Dokumente: gescheitert und zurückgerollt, neuer Befund B4-4**) ·
**`main`-HEAD:** `27d7787` (Merge #147)

> **Der Berliner CSD hat seit 2026-07-27, 10:29 UTC erstmals einen verstandenen Vorgang
> in Production.** Der dritte, eng begrenzte Nachhollauf (`nachhol-20260727102907`,
> 10:29:11–10:29:31 UTC) auf dem Stand von PR #147 hat **19 der 21** Zielrohdokumente dem
> **fachlich richtigen** Vorgang `vg-csd-20260727-12aae0` zugeordnet (8 → **27** Dokumente,
> `pending` → `complete`, Inhalt trifft das Ereignis). **Kein** fachfremder Vorgang ist
> gewachsen — `vg-angriffen` steht unverändert bei **1** Dokument —, **kein** unbeteiligter
> Datensatz wurde verändert (genau **ein** geändertes Knowledge Object), **kein** zweiter
> CSD-Vorgang entstand, Magnet-Analyse danach **0 Magnete · 0 Übernahmen**. Damit ist der
> Hotfix B4-3 in Production belegt und der Vorgangsverlust B4 geschlossen.
> **Nicht erreicht ist die Vollständigkeit:** **2** Dokumente blieben unverarbeitet, weil
> das Sitzungs-Schutzlimit von 50 LLM-Aufrufen griff (Production stand bei 49/100; das
> Betriebsbudget war **nicht** ausgeschöpft). Beide stehen unverändert auf
> `ohne-endzustand`; sie brauchen einen zweiten eng begrenzten Lauf mit angehobenem
> `HELMUT_MAX_LLM_CALLS_PER_DAY` — **Betreiberentscheidung**, weil es eine Kostengrenze
> anhebt. **Production-Nettowirkung: +19 `ko_document_links`, 1 Vorgang von `pending` auf
> `complete`, +1 LLM-Aufruf — sonst nichts.** Es wurde **nicht** zurückgerollt; kein
> Abbruchkriterium ist eingetreten. Vollständige Beweiskette:
> [`befund-csd-2026-vorgangsverlust.md`](befund-csd-2026-vorgangsverlust.md) §17.
> **Ebenfalls belegt:** das Storage-Gate wirkt — Kosten (`llm_budget_counters` 49 → 50) und
> Lauftelemetrie liegen diesmal **in Production**, nicht lokal wie bei Anlauf 2.

> **Der Restlauf für die zwei offenen Dokumente ist am 2026-07-27, 10:47 UTC gescheitert und
> vollständig zurückgerollt — neuer Resolver-Defekt B4-4.** Beide Dokumente bekamen einen
> Endzustand, aber **keines im CSD-Vorgang**: das Wegner-Dokument landete im vorgemerkten
> Fremdvorgang `vg-tagesspiegel-20260519-f29ebd` (Bielefelder SPD-Brief, Pflegebedürftige),
> überschrieb dessen Inhalt und hob ihn von `pending` auf `neu` — **derselbe Schaden wie in
> Anlauf 2, über einen anderen Mechanismus**. **Ursache B4-4, an echten Daten nachgerechnet:**
> Google-News-Titel tragen den **Herausgebernamen** als Suffix („… - Tagesspiegel"). Der ist
> kein Ereignisbegriff, steht in keiner generischen Familie, hat Stärke 2 — und erfüllt
> `MIN_BEWEISGEWICHT=2` **allein**. Der Ein-Dokument-Riegel aus B4-3 greift nicht, weil der
> Bestandsvorgang 2 Dokumente trug. **Folge: jeder Vorgang mit ≥ 2 Artikeln desselben Mediums
> zieht beliebige weitere Artikel desselben Mediums an.** **B4-4 ist in Production aktiv** —
> eine Resolver-Änderung war in diesem Auftrag ausgeschlossen. Rückweg um 10:49 UTC ausgeführt
> und gegengemessen: `knowledge_objects` **1 140**, `ko_document_links` **4 141**, `pending`
> **426**, Fremdvorgang wieder `pending` und unsichtbar, neuer Vorgang entfernt, beide Ziel-IDs
> wieder ohne Verknüpfung — **netto 0 veränderte Zeilen**. Nicht rücknehmbar: **2 LLM-Aufrufe**
> (50 → 52) und ein `updated_at`. **Der Nachweis aus dem Kasten oben bleibt unberührt gültig.**
> Zwei eigenständige Werkzeugbefunde dabei: das Nachhol-Werkzeug meldet nach einem
> **fehlgeschlagenen** Lesevorgang „Nichts nachzuholen" mit Exit 0 (falsches Grün), und der
> ausgeführte Lauf fehlt in `processRuns`. Beweiskette:
> [`befund-csd-2026-vorgangsverlust.md`](befund-csd-2026-vorgangsverlust.md) §18.

> **Historisch, nicht löschen — CSD-Production-Nachweis Anlauf 2 am 2026-07-27 ausgeführt,
> gescheitert und vollständig zurückgenommen.** Der genehmigte Nachhollauf (08:41–08:42 UTC, 21 namentlich genannte
...
> **Erledigt:** B4-3 ist behoben, gemergt (**PR #147**, `27d7787`), deployt und seit dem
> 2026-07-27, 10:29 UTC **in Production belegt** — siehe den Kasten oben und §17 des
> Befunds. Der reguläre Lauf um 10:03 UTC bildete den CSD-Vorgang bereits von selbst.
...
5. **B4-4 ist in Production aktiv** (seit 2026-07-27 belegt): der **Herausgebername** aus
   dem Google-News-Titelsuffix zählt im Resolver als spezifischer Beweis und erfüllt die
   Belegschwelle allein. Jeder Vorgang mit ≥ 2 Artikeln desselben Mediums zieht damit
   themenunabhängig weitere Artikel dieses Mediums an. Reproduziert und zurückgerollt,
   **nicht behoben** — Fix-Sprint nötig, vorher kein weiterer Nachhollauf
   (`befund-csd-2026-vorgangsverlust.md` §18.4/§18.8).

## 8 · Offene Pull Requests (Stand 2026-07-27)

| PR | Inhalt | Einschätzung |
|---|---|---|
| **NEU** | **Dokumentation des CSD-Production-Nachweises (Anlauf 3)** — nur Doku: `befund-csd-2026-vorgangsverlust.md` §17 und die Statuszeilen hier | **rein dokumentarisch**, kein Code, keine Migration, kein Flag. Branch `claude/berliner-csd-safety-check-fscuc8` |
| ~~#147~~ | **Hotfix B4-3: Beweisfamilien im Resolver** — Flexionsformen zählen einmal, generische Ereignisbegriffe tragen nichts, Ein-Dokument-Vorgänge sind fail-closed; Magnet-Analyse erkennt die Übernahme; `--ids` vor der Mengenkappung; Production-Schreibgate | **gemergt** 2026-07-27 09:59 UTC (`27d7787`), Deployment `dpl_AQJU4Db1R95Gywc4bgT62oTrseDg` `READY`. **Production-Nachweis am selben Tag 10:29 UTC erbracht** (19 von 21 Dokumenten, 9 von 12 Abnahmekriterien) — Details in §3 und im Befund §17 |
...
- **Nachmessung 2026-07-27, 10:23–10:30 UTC (CSD-Nachweis Anlauf 3, mit genau einem Schreibschritt):**
  vorher `knowledge_objects` **1 140** · `ko_document_links` **4 122** · `raw_documents` **8 929** ·
  `pending` **427** · `failed` **7** · LLM **49/100** · 0 aktive Locks.
  Nachher: `ko_document_links` **4 141 (+19)** · `pending` **426** · LLM **50/100** ·
  `knowledge_objects` **unverändert 1 140** · `raw_documents` unverändert · 0 aktive Locks.
  Genau **ein** verändertes Knowledge Object (`ko-vg-csd-20260727-12aae0`, 8 → 27 Dokumente,
  `pending` → `complete`). **Nebenbeobachtung, nicht Gegenstand dieses Sprints:** der Crawl
  schreibt wieder Telemetrie — 07-27 08:xx **147 Zeilen / 147 distinct `source_id`** (Invariante
  B3 erfüllt), 07-27 10:xx **180 Zeilen / 92 distinct** (mehrere Teilläufe in derselben Stunde,
  **nicht** gegen B3 auswertbar). Die Aussage „Crawl steht seit 26.07. 20:03 still" ist damit
  überholt; eine saubere Abnahme von PR #144 steht weiterhin aus.
- **Nachmessung 2026-07-27, 10:45–10:50 UTC (Restlauf, ausgeführt und zurückgerollt):** Endstand
  **identisch** zur Messung von 10:30 — `knowledge_objects` **1 140** · `ko_document_links`
  **4 141** · `raw_documents` **8 929** · `pending` **426** · `failed` **7** ·
  `vg-csd-20260727-12aae0` **27** · `vg-angriffen` **1** · 0 aktive Locks. Einzige bleibende
  Abweichung: LLM **50 → 52** (zwei verbrauchte Aufrufe) und ein `updated_at`.
