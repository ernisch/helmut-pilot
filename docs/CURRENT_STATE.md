...
Matching lösen, was ein Gate nur verstecken würde. **PR offen, nicht gemergt.** Details:
...
> offline vollständig bewiesen; Production ausschließlich lesend geprüft, PR offen, Merge und
> Abnahme stehen aus).**
...
> bereits in `main`) ist erfolgt, offen ist der Merge des Sprints 23C-2A (Erklärungsabdeckung).
