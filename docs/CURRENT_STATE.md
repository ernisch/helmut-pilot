...
**Letzte Aktualisierung:** 2026-07-28 (**Sprint 22C1: Production-Shadow-Struktur für
semantische Embeddings + kontrollierte Backfill-Pipeline gebaut und getestet (Migration
finalisiert mit Rollback, harte Kosten-/Token-/Objektdeckel fail-closed, eigenes Lock mit
Erneuerung, Idempotenz/Wiederaufnahme testbewiesen, Offline-Suite 176/176) — teilweise
abgeschlossen: wartet auf Gate 1 (PR-Merge) und Gate 2 (Freigabe für Migration + Backfill),
kein Production-Write, keine Migration angewendet** · **Sprint 22B: externer Testlauf ausgeführt und
...
**`main`-HEAD:** `77e4de3` (Merge #164, in Production ausgerollt — Deployment
`dpl_9Kt9LpvYPCRNxeK4MhycsQmH8k29` `READY`, `target: production`; davor `7e881b8` = Merge #163,
`b6ca441` = Merge #162)

> **Sprint 22C1 am 2026-07-28 ausgeführt — teilweise abgeschlossen (alle Code-, Test-
> und Doku-Arbeiten fertig; wartet definitionsgemäß auf Gate 1 = PR-Merge und Gate 2 =
> ausdrückliche Freigabe für Production-Migration + Backfill).** Startprüfung vollständig
...
|---|---|---|
| offen | **Sprint 22C1: Production-Shadow-Struktur + Backfill-Pipeline für semantische Embeddings** — finalisierte Migration `20260728_embedding_shadow.sql` (+ Rollback, aus `entwuerfe/` überführt), Renew-Lock-RPC, Backfill-Lib+CLI mit harten fail-closed Deckeln und eigenem Lock, Testsuite 40/40, Env-Inventar + D7-Testpflege, Doku §14 inkl. Production-Runbook | **Merge = Deployment, ändert aber keine Production-Daten**: die Migration wird durch den Merge NICHT angewendet (kein Migration-Runner), das CLI läuft nur auf Aufruf, die App liest die Shadow-Tabelle nicht. Migration + Backfill bleiben je einzeln freigabepflichtig (Gate 2). Rollback per `git revert` |
