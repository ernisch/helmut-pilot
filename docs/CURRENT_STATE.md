...
- **Beide Blocker unabhängig bestätigt, alle Zahlen exakt reproduziert (§23):** `briefing_materialization` ist 18:00–21:36 UTC fällig, Schnitt mit 11:36–15:59 leer. Bedarf **1.812** gegen **1.732** in 263 min bei par 1, nötig 276 min.
- **Eigener Befund teilweise ZURÜCKGENOMMEN (§29):** §24.1 behauptete, die 1.000 mandatsgebundenen Aufrufe könnten im Fenster nicht entstehen. Die Zahlen stimmen, die **Begründung war falsch**: sie stammen nicht aus den (belegt KI-freien) Warteschlangenklassen, sondern aus einer Messreihe, die **`communicationDraft` dominiert** — eine nutzergetriebene HTTP-Route **ohne Fensterbindung**. **812 darf NICHT als Fenstergröße eingesetzt werden**; sie kippte beide Fenster auf „passt" und wäre ein falsches Grün. Dass §24.1 die Zahl **nicht** in die Hürde eingebaut hat, hat das verhindert.
- **Der stärkere Befund stattdessen (§29.3):** Das Tor prüft **Schnittmenge mit dem Streuintervall**, der Motor prüft **Fälligkeit** (`due_at <= now()`). Für ein Fenster **nach** einer Phase fällt beides auseinander. Betroffen ist ausgerechnet **21:36–03:59 UTC** — das **einzige** Fenster, das das Kapazitätstor bei **Parallelität 1** besteht (2.522 ≥ 1.812). Fehlerrichtung **fail closed** (kein falsches Grün), aber: träfe die Korrektur zu, wären Parallelität 2 **und** der Zwei-Fenster-Ablauf überflüssig. **Bewusst nicht geändert** — das Nachtfenster überschreitet 00:00 UTC, also die Frischefenstergrenze; „100 % fällig" wäre selbst eine Annahme. Blocker 2 ist zudem szenarioabhängig (Kipppunkt 1,84).
...
- **Zwei Lücken geschlossen (§23.3):** (1) Gestuft war nur die **Aktivierung** — neu `testkohorte-stufen.js` mit **15 stufengenauen Freigaben**. (2) Es gab **keinen Weg zur vollständigen Entfernung** — neu `testkohorte-entfernung.js` + CLI, sechs Riegel, Restbestandsbefund über fünf **gezählte** Familien.
- **Drei Defekte im NEU gebauten Code, alle behoben (§28):** (1) `deleteTenantScopedData` schrieb den leeren Mandanten-Store **unbedingt** zurück — der „Teardown" hätte für die 400er-Gruppe **400 Zeilen angelegt**. (2) Ein **Teilfehler** zählte als „entfernt". (3) Die CLI meldete Exitcode **0** über einem gescheiterten scharfen Lauf. Weiterhin **nicht** abgeräumt: `helmut_jobs`, Outbox, `crawl_runs`, Personenquellen-Rohdaten.
...
- **05:45/05:48 (§26):** Korrektur — um **05:48 läuft heute nichts**. Gemessen: kleinster Cronabstand **10 min** gegen 5 min Grenze ⇒ zwei Bestandscrons können sich heute **nicht** überschneiden. Frage „unbedenklich?" bleibt offen.
- **Telemetrie (§27):** Ring 5.000 trägt **einen** Testtag je Stufe (C ~2,8 Tage); die Stufenkette reißt ab Tag 3–5. **Blinder Fleck:** ein Lost Update ist für die Vollständigkeitsprüfung strukturell unsichtbar — jetzt benannt.
