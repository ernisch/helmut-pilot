...
| **#145** | **Qualitätssprint + Resolver-Härtung gegen Magnet-Vorgänge (B4-2)** — **der Kernbefund dieses Sprints.** `sameVorgang()` liess **einen einzigen** passenden Dokumentvergleich genügen; damit wuchs die Trefferwahrscheinlichkeit linear mit der Bestandsgröße und jede Aufnahme machte einen Vorgang anziehender. Production-Beleg: `vg-zeitung-…` wuchs in **einem Lauf von 13 auf 65 Dokumente** und enthielt Armutsbericht, Pflegereform, Verkehrsminister-Rücktritt **und** den CSD-Anschlag. **Ursache bewiesen, nicht vermutet:** dieselben 65 Dokumente ergeben bei Neuclusterung **16 getrennte Cluster** — die Clusterregel ist gesund, der Resolver war es nicht. **Lösung:** verglichen wird **Kern gegen Kern** statt Dokument gegen Dokument. Ein Vorgang ohne Kern nimmt nichts mehr auf — **der Magnet wird durch seine eigene Heterogenität unschädlich**; zweiter Riegel: ≥ 60 Dokumente. Anforderung skaliert mit der Kerngröße (`min(2, ⌈Kern/2⌉)`). Jede Entscheidung liefert eine **vollständige, reproduzierbare Spur** inkl. Ablehnungen. **Magnet-Analyse (36 Vorgänge, read-only):** objektive Definition über Kohärenz (größtes Cluster / alle Dokumente) — Magnete 0,07–0,55 mit **Kernanker 0**, gesunde 0,67–1,00 mit Kernanker 2–8; dazwischen liegt kein einziger Vorgang. 12 Magnete, größter **400 Dokumente**. **Production-Validierung (read-only): 12 von 12 Magneten blockiert, kein einziger Vorgang würde den CSD-Cluster aufnehmen** — unter dem alten Resolver hätten ihn 3 gezogen. Neues Werkzeug `scripts/vorgangs-magnet-analyse.js` (diagnostiziert, bereinigt **nicht**). Tests: **162/162**, neue Suite `vorgangs-resolver-test.js` 54 Assertions | die **Bereinigung** der 12 bestehenden Magnete — Production-Schreibzugriff mit KI-Kosten, eigene Freigabe. Sie können nur nichts mehr anziehen. Details: [`befund-csd-2026-vorgangsverlust.md`](befund-csd-2026-vorgangsverlust.md) §12b/§12c |
| ↳ | **derselbe PR, Teil 1: drei Korrekturen vor dem CSD-Nachweis** | **mergefähig, klein und ohne Verhaltensänderung im Normalbetrieb.** (1) `--karenz=<stunden>` im Nachhol-Werkzeug — Standard bleibt 24 h, wirkt nur dort; ohne diese Option blockierte die Karenz 16 von 20 CSD-Dokumenten. (2) Dokumentauswahl für den KI-Prompt: statt nach Dokumentkennung (faktisch zufällig) jetzt nach Faktenabdeckung mit festen Endpunkten — am echten CSD-Stapel **6/6 Schlüsselfakten statt 5/6**, „neueste zuerst" wurde geprüft und ist **nicht** optimal. (3) Telemetrie-Aggregation: `understanding-lagecheck` → Präfix `understanding-`; dabei **zwei weitere** Fehler derselben Klasse gefunden und behoben (`ERGEBNISGRUPPEN` ohne die zwei Pending-Ergebnisse; `verarbeitet = counts.saved` ohne `updated`/`merged`). Zwei neue **Strukturtests** prüfen künftig den Quelltext gegen die Zuordnungstabellen. Offline-Suite **160/160**, Browser-Smoke **32/32**. Details: [`befund-csd-2026-vorgangsverlust.md`](befund-csd-2026-vorgangsverlust.md) §12a. Branch `claude/helmut-vorgangsbildung-fehler-coolvq` |
