...
- **Beide Blocker unabhängig bestätigt, alle Zahlen exakt reproduziert (§23.1/§23.2):** `briefing_materialization` ist 18:00–21:36 UTC fällig, Schnitt mit 11:36–15:59 leer; 66,4 / 55,1 / 6,7 % stimmen. Bedarf **1.812** gegen **1.732** in 263 min bei par 1, nötig 276 min. **Präzisierung:** Das Nachtfenster 21:36–03:59 trägt den Zyklus bei par 1 rechnerisch (2.522) — es scheitert daran, dass dort **keine** Arbeitsklasse fällig ist: nicht zu klein, sondern leer.
- **Drei von vier Schutzgrenzen sind NICHT hart (§23.4):** Laufzeitwirksam ist allein der Tagesdeckel (`reserveLlmCall`, atomar). **`HELMUT_TESTLAUF_MAX_RPM`/`_MAX_TPM` liest kein Ausführungspfad** — es gibt keinen Minutentakt-Begrenzer; das Kostenbudget wirkt nur entdeckend (A04). Die Annahme eines „harten RPM-/TPM-Schutzes" ist **falsch**.
- **Zwei neue Lücken geschlossen (§23.3):** (1) Gestuft war nur die **Aktivierung** — für die 20 Profile der Stufe A ließ sich kein Fachzyklus freigeben. Neu `testkohorte-stufen.js`: **15 stufengenaue Freigaben**, Reihenfolge erzwungen. (2) Es gab **keinen Weg zur vollständigen Entfernung**; die 400er-Gruppe hätte 400 Einzelaufrufe ohne Erlaubnisliste gebraucht. Neu `testkohorte-entfernung.js` + CLI, sechs Riegel; aktive Profile werden übersprungen statt gelöscht, der Restbestandsbefund verlangt fünf **gezählte** Familien.
...
- **Schutzbeleg der fünf realen Mandate, ehrlich (§25.2):** Der Verdrängungsschutz ist **gebaut, aber heute nicht wirksam** — `HELMUT_TESTLAUF_VORRANG_REAL` ist ungesetzt, die Reserve ist **0**. Bei **0 synthetischen Zeilen** ist das kein Risiko, sondern eine Vorbedingung. Ebenfalls belegt: „Parallelität 2" ist **unterbestimmt** — Worker (Default 2) × Verstehen (1) ergeben heute 2, mit dem Vorschlag 4; das Kapazitätsmodell kennt nur **eine** Ebene (§25.1).
