...
9. **Zwei Testzeilen in Production** (24.08., §14) — **Stand 25.08.**: Auftrag `371707a4…` wurde um **07:01 Uhr türkischer Zeit** (06:01 Berlin, 04:01 UTC) vom regulären Betrieb aufgenommen und ist terminal **`fehlgeschlagen`**; die Outbox-Zeile `24ba14ec…` hat der **kanonische Abgleich** um **08:56:14 türkischer Zeit** (07:56:14 Berlin, 05:56:14 UTC) auf **`verzichtet`** gesetzt. **`endgueltig_fehler` ist um 1 verfälscht** (terminal, wächst nicht weiter). Ein **Production-Trockenlauf** lief um **08:55:30 türkischer Zeit** (07:55:30 Berlin, 05:55:30 UTC) erfolgreich und wurde **vollständig zurückgerollt** — **nichts gelöscht, nichts verändert**. Weil sich der Zustand danach änderte, trägt dieser Beleg **keine Freigabe B** mehr; der Vertrag ist auf `verzichtet` berichtigt, **Freigabe A ist erneut zu prüfen** (Runbook §31.9).
...
- **Zustand eingetreten (25.08., zweite Korrekturrunde):** um **07:01 Uhr türkischer Zeit** (06:01 Berlin, 04:01 UTC) hat der **reguläre Betrieb** die Testzeile aufgenommen; leere Nutzlast ⇒ endgültige Ablehnung ⇒ fünf Versuche in einem Slot ⇒ Auftrag **`fehlgeschlagen`**, Outbox zunächst **`bestaetigt`** (Transport `schatten`). Damit ist **`endgueltig_fehler` tatsächlich um 1 verfälscht**; „nie beansprucht" stimmt nicht mehr. Der Vertrag auf Commit `d4fa57d` hätte einen Trockenlauf **korrekt abgebrochen** (E7.1/E7.2/E7.7/E7.8, E2.7, E4.2/E4.5–E4.7) — **deshalb wurde nichts ausgeführt, sondern der Vertrag berichtigt** (zweite Fassung; Mengenverträge bytegleich unverändert, per SHA-256 belegt). Neu: der frühere Zustand `wartend`/`offen` wird ausdrücklich abgelehnt, und die Datensparsamkeit gilt jetzt **am Wert** (jeder sensible Vertragswert nur als rechte Seite seines eigenen Spaltenvergleichs). Nachweis: **61 PASS** (echte PostgreSQL 16). **Noch nichts gelöscht, kein Production-Trockenlauf** — einzige Berührung war die enge Leseprüfung (25.08. **07:48:39 türkischer Zeit** / 06:48:39 Berlin / 04:48:39 UTC), die jeden Vertragswert bestätigt. Freigabe A wurde daraufhin erteilt (siehe nächster Punkt) (Runbook §31.8). **Der Selbstweck bleibt deaktiviert.**
- **Production-Trockenlauf (Freigabe A) am 25.08., 08:55:30 türkischer Zeit** (07:55:30 Berlin, 05:55:30 UTC): aus dem committed Generator erzeugt (SHA-256 `eb6e0778…`), **genau einmal** ausgeführt, Ergebnis die kontrollierte Meldung **`TROCKENLAUF-OK`** — alle Riegel bestanden, 1 Auftrags- und 1 Outbox-Zeile *wären* entfernt worden, Transaktion **vollständig zurückgerollt**. Doppelt belegt: Quittung nennt vorher 1116/881, die Nachprüfung um 08:56:06 türkischer Zeit las **dieselben 1116/881**; 0 offene Transaktionen, 0 Sperren, 0 zurückgelassene Temp-Tabellen. **Nichts gelöscht, nichts dauerhaft gespeichert, kein zweiter Versuch.**
- **Kanonischer Abgleich 8 Sekunden später (25.08., 08:56:14 türkischer Zeit** / 07:56:14 Berlin / 05:56:14 UTC): `helmut_outbox_abgleich` setzte die Absicht auf **`verzichtet`**. **Ursache belegt** (Production-Definition rein lesend aus dem Katalog, **nicht aufgerufen**): der Abgleich schließt Absichten *terminaler* Aufträge — `j.status in (erledigt, fehlgeschlagen)` und `o.status in (offen, versendet, aufgegeben, bestaetigt)`. Das ist der kanonische Weg und **keine Wirkung des Trockenlaufs**, der zu diesem Zeitpunkt längst zurückgerollt war.
- **Dritte Vertragsfassung (25.08., dritte Korrekturrunde):** Outbox-Status auf **`verzichtet`**; **`updated_at` ist neuer Riegel E4.12** — belegt zulässig, weil kein Schreiber eine bereits `verzichtet`-Zeile erneut anfasst (Sweep führt `verzichtet` nicht, Wiedereröffnung verlangt `wartend`, Vergabe nur `offen`/`versendet`, Quittieren nur `versendet`, Zurücklegen und Erneut-Vorlegen brechen ab) und weil der Trigger `helmut_job_outbox_kappen_trg` `updated_at` bei **jedem** Update setzt — damit ein allgemeiner Änderungsmelder. Jeder frühere Zustand (`wartend`, `offen`, **`bestaetigt`**) wird **geschlossen abgelehnt**. Auftrag und alle übrigen Werte unverändert; Mengenverträge bytegleich (SHA-256 `e8a55ee3…`). Nachweis: **65 PASS / 0 FAIL** (echte PostgreSQL 16). **Der alte Trockenlaufbeleg ist verbraucht — Freigabe A ist erneut zu prüfen, Freigabe B bleibt getrennt.** **Der Selbstweck bleibt deaktiviert.**
