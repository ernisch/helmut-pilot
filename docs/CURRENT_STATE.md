...
**Status (Fassung 3, verbindlich):** **Werkzeuge und Änderungssatz lokal vollständig belegt** (279/279) · **synthetischer Motorlasttest 25/50/100 bestanden** · **Z1 (technische Gesamtbereitschaft) weiterhin teilweise/offen** — Ankunftskennzahl nicht angewendet, Fluid Compute unbestätigt, Supabase unter realistischer Last ungeprüft, KI-Deckel je Stufe offen · **Z3/Z4/Z5 nicht erreicht, keine Production-Freigabe**. Die frühere Angabe „Z1 und Z2 erreicht" war zu weit und ist zurückgenommen. Doku: [`betrieb/skalierung-25-50-100.md`](betrieb/skalierung-25-50-100.md) §0.1, Reihenfolge §9a.

- **Kapazitätsmodell, an der Messung geeicht:** **139 geteilte Quellen** + **6 je Mandat**, je 2×/Tag; die Formel trifft für M=5 **exakt die gemessenen 338**. Hochrechnung **841/1323/2287** Aufträge/Tag — **unterlinear**.
- **Synthetischer Lasttest: 60 PASS / 0 FAIL** über alle drei Stufen (echter Motor, echte lokale PostgreSQL, echte Workerprozesse, Fehler- und langsames Mandat; **Attrappen** für KI/Netz). 0 hängende Leases, 0 Dubletten, Rückstau abgebaut, Fairness min = max. **Misst die lokale Mechanik, nicht die Wirklichkeit** — Production ist ~2 300× langsamer.
- **Zwei echte Produktfehler behoben:** ein Wiederholungslauf löschte gepflegte Profilfelder (jetzt verschmelzend) und **reaktivierte still ein deaktiviertes Mandat** unter Umgehung der Freigabe (`CLAUDE.md` §5; jetzt fail-closed). Neu: **Stapelprovisionierung** mit Vorprüfung der ganzen Liste, Trockenlauf als Standard, ehrlicher Bilanz.
- **Die vier bekannten Testbefunde sind gelöst — reine Umgebungsfehler.** Per Bisektion isoliert: `HELMUT_V3_STORE=1` (drei Suiten) und `HELMUT_STORAGE_BACKEND=supabase` (profile-db); beidseitig belegt, CI an PR #269 unabhängig **277/277 grün**. `scripts/lokal.js` stellt die Speicherwahl jetzt deterministisch auf lokal — **keine Testzusicherung abgeschwächt**. Die Ursachenangabe in [`betrieb/op30-testbefunde-2026-08-08.md`](betrieb/op30-testbefunde-2026-08-08.md) war in 3 von 4 Fällen falsch und ist dort korrigiert.
- **Gesamtlauf: 279/279 Suiten grün, 0 FAIL** (277 Bestand + 2 neue); darin alle vier vormals roten Befunde. Mit echter lokaler PostgreSQL zusätzlich: Lasttest 60 PASS (zweimal), Ankunftskennzahl 30 PASS, `jobqueue-datenbank` 55 PASS, `jobqueue-lasttest` 19 PASS inkl. SIGKILL.
- **Härtester Blocker vor der AKTIVIERUNG weiterer Mandate (nicht vor dem Fünfernachweis) bleibt der KI-Tagesdeckel:** 100 Aufrufe/Tag tragen **keine** Stufe (25 Mandate brauchen schon im günstigsten Fall 113). Das ist **keine Kostenfrage** — 350 Aufrufe/Tag kosten unter 1,20 USD; Modellkosten liegen bei 25/50/100 realistisch bei **15 / 21 / 33 USD im Monat**. Anhebung ist eine Freigabeentscheidung.
...
- **Zwei Reviewrunden an PR #270, fünf bestätigte Befunde, alle behoben:** die Migration hätte `service_role` **kein** EXECUTE erteilt und wäre in Production **wirkungslos** gewesen (empirisch belegt); der **Trockenlauf meldete falsches Grün**; ein Abbruch nach der Auth-Kontrolle ließ ein halbes Konto zurück; zwei weitere Rückwege löschten **zu breit** (`deleteAuthDataForPolitician`) und verschluckten Löschfehler. Jetzt **ein** eng begrenzter Rückweg über `deleteUser(user.id)`, der einen Erfolg nur meldet, wenn er bestätigt ist (`provision-stapel-test` **83 PASS**, `jobqueue-ankunft-datenbank-test` **30 PASS**).
- **Beleggrenzen zurückgenommen:** Verbindungsspitze 5/300 ist ein **lokaler PostgreSQL-Wert**, kein Supabase-Grenzwert; der Test schließt Supabase unter realistischer Last **nicht** als Engpass aus; Vercel > 300 s gilt **erst mit bestätigtem Fluid Compute** (unbestätigt); 15/21/33 USD sind **Szenarien**, keine gemessenen Monatskosten. Von 139 Lücken **11 geschlossen**, eine vorbereitet (F9) ([`betrieb/skalierung-25-50-100.md`](betrieb/skalierung-25-50-100.md) §4.8).
