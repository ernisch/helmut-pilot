...
* **Offline-Pflichtlauf verschlankt (Testorganisation, kein Produktcode).** Der kanonische Standardlauf `node scripts/lokal.js -- node scripts/run-offline-tests.js` und das Pflicht-CI-Gate fuehren jetzt **453** Suiten aus (zuvor 456). Die vier historischen OP-30-Skalierungssuiten `narrativ-stress-1000-test.js`, `narrativ-stufen-test.js`, `skalierung-simulation-test.js` und `skalierung-stufen-test.js` sind aus dem Standardlauf entfernt (zusammen zuletzt ~290 s). Sie bleiben **unveraendert im Repo** und laufen bewusst nur ueber `node scripts/run-offline-tests.js --extended` bzw. `npm run test:offline:extended` — **nicht** automatisch im PR. Kein Sicherheitstest entfernt: Netzschutz, 500er-, Security- und Mandantentrennungssuiten bleiben unveraendert im Standardlauf (testgesichert durch `scripts/offline-suite-auswahl-test.js`). Kanonische Begruendung: Kopfkommentar `scripts/run-offline-tests.js`.
