...
| 13 | Quellenpakete | Vollständigkeit jedes Pakets prüfen | ✅ | Jedes Paket ist fachlich vollständig und nicht nur technisch angelegt | **Dieser Sprint.** Alle 8 Pakete haben jetzt ein ausführbares fachliches Kriterium (Pflichtklassen + Pflicht-Herausgeberklassen + Vollzähligkeit + begründete Überschneidungen). Ergebnis: **6 vollständig, 2 teilweise vollständig, 0 blockiert**; die beiden Teilweise-Fälle sind belegt und dauerhaft abgesichert (`regional-niedersachsen`: 0 benannte Herausgeber; `die-linke-brandenburg`: 2 Klassen fachlich unmöglich). 3 Lücken behoben, 3 benannt. **Ausschuss-Sollmenge extern verankert** (24 ständige Ausschüsse des 21. Bundestages, Drucksache 21/150) — die erste Fassung war katalogrelativ und führte nur 23; der Ausschuss für Wahlprüfung, Immunität und Geschäftsordnung fehlte, neun Bezeichnungen stammten aus der 20. WP. `bundestag-ausschuesse-test.js` 36/36, `paketvollstaendigkeit-test.js` 91/91, Offline-Suite 149/149 ([Nachweis](../quellenarchitektur/31-paketvollstaendigkeit.md) §2a) |
...
- **V-7** *behoben:* der Katalog führte **23 statt 24** ständige Bundestagsausschüsse und neun Bezeichnungen der 20. Wahlperiode. Ursache: die Ausschussliste war eine handgepflegte Politikfeld-Auswahl, und die Vollzähligkeitsregel leitete ihre Sollmenge aus demselben Katalog ab. Jetzt extern verankert in `seeds/bundestag-ausschuesse.js` (Drucksache 21/150), der fehlende Ausschuss ergänzt, alle Bezeichnungen amtlich.
- **V-8** *behoben:* der gezielte Seed-Restore deckte den Fall „Seed legt einen neuen Abrufweg an" nicht ab und hätte die Zeile stehen gelassen.
- **Offen extern:** die Sollmenge einmal direkt gegen den Volltext der Drucksache 21/150 abgleichen — `bundestag.de`/`dserver.bundestag.de` sind aus der Agentensitzung nicht erreichbar (`403` auf `CONNECT`); die 24 Bezeichnungen stammen aus amtlichen Ausschuss-Tagesordnungen der 21. WP. Ebenfalls offen: die **Fraktions**vollzähligkeit ist weiterhin katalogrelativ.
...
Bundespakete, **zwei** zusätzliche Paketzuordnungen (Ausschuss Arbeit und Soziales → `bund-basis`;
Ausschuss für Wahlprüfung, Immunität und Geschäftsordnung → `bund-basis`) und **einen** neuen
Abrufweg (der 24. ständige Ausschuss). Soll-Zahlen im Seed damit **147** Zuordnungen und **145**
Abrufwege ([Nachweis §7](../quellenarchitektur/31-paketvollstaendigkeit.md)).
