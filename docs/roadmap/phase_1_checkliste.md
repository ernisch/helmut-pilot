...
| 22 | Daten | Embeddings vollständig speichern und prüfen | ⏳ | Relevante Wissensobjekte besitzen dauerhafte Embeddings | **Sprint 22A (2026-07-28): Architektur bewiesen und geprüft** (Merkmalsvektor: 772/772 verstandene mit dauerhaftem Vektor, 0 Defekte). **Sprint 22B: semantischer Qualitätsnachweis erbracht** (Azure `text-embedding-3-small`, Top-5 38/38 vs. 33/38, mittlerer Rang 1,2 vs. 4,4). **Sprint 22C1: Production-Shadow-Migration + kontrollierte Backfill-Pipeline gebaut und getestet (40/40)** — der Punkt bleibt ⏳, bis der vollständige Production-Backfill ausgeführt und alle Abnahmekriterien bewiesen sind (Gate 2, freigabepflichtig). Kanonisch: [`embedding-architektur.md`](../embedding-architektur.md) §14. **Nicht** mit OP-22 (Scoring) verwechseln |
