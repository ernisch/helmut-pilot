...
- **Gebaut:** fail-closed **Kommunikationsriegel** vor allen sieben Außenkanälen · Kohortenwerkzeuge mit Erlaubnisliste · Kapazitäts-/Kostenriegel · Azure-**Endpunktguard** · Geheimnisredaktion.
...
- **Kernbefund:** „synthetisch" existierte außerhalb des Kommunikationsriegels **nicht**; die fünf realen Mandate waren an **vier** Stellen verdrängbar. Gebaut: `mandatsklasse.js` und vier Schutzregeln, Vorwärtsweg, Fachzyklus, Stufenkontrolle, Rückbau — **bei 0 synthetischen Zeilen byte-identisch**. Drei Auswerterkanäle **nicht messbar**. `recordLlmUsage` relational als Dual-Write, Flag **AUS**, Migration `20260902121500` **nicht angewendet**.
...
- **Befund und Korrektur (§34.2/§34.3):** `provisionierung --stufe=a` ignorierte die Stufe still (zielGroesse **495**, Pauschalwort, Exit 0). Jetzt ist `--stufe=a|b|c` Pflicht; fehlend/leer/unbekannt → **Exit 2 vor jedem Zugriff, kein Rückfall auf 495**; `--ids=` nur in derselben Stufe. Gemessen: **a → 20 / b → 75 / c → 400**. Bibliothek unverändert.
- **Ablaufplan stufenweise (28 Schritte, kein Sammelschritt):** Reifebeleg → je Stufe Anlage (inaktiv) → Isolation → Aktivierung → Fachzyklus → Kontrolle; B erst nach kontrollierter A, C nach B. **Die acht Betreiberwerte sind Vorbedingung jeder Aktivierung und keiner Provisionierung** (§34.5). Rückweg nie gesperrt.
- **Verhaltensbelegt:** die inaktive Provisionierung (echter `provisionTenant`) erzeugt **0** Netzaufrufe, **0** Riegelanfragen, **0** KI-Aufrufe, lädt kein Außenkanal-/KI-/Crawl-Modul nach; der echte Planer plant für 20 inaktive Profile **0** Aufträge (Gegenprobe: 1 aktives → 2). **Zwei Lücken geschlossen:** die Verstehens-Interessenprüfung zählte inaktive Profile mit (jetzt `isDisabled`); `provisionTenant` hätte `spec.reaktivieren:true` auch in einem inaktiven Lauf befolgt — jetzt Abbruch vor dem Schreibvorgang (der Stapelpfad wies es längst ab).
- **§34.7 geschlossen (Variante A, Betreiberentscheidung 03.09.):** die Reifesperre wies **18 von 20** Stufe-A-Profilen ab („Testausschuss N" fehlt in der Sollmenge). Behoben in der **Kohorte**, nicht in der Regel: die **434 Bundestagsprofile** tragen jetzt amtliche WP-21-Ausschussnamen aus der Sollmenge (**keine zweite Namensliste**), die **61 Landtagsprofile** weiter synthetische — ein Bundestagsausschuss auf Landesebene wäre die falsche Ebene. Kennungen, Adressen, Testnamen, -parteien und -themen bleiben synthetisch. Der echte Pfad nimmt Stufe A **20/20** an, die Prüfung **495/495**.
- **Dabei verschärft, nicht gelockert:** veraltete Ausschussnamen liefen bisher als gültig durch (3 von 4 dokumentierten WP-20-Namen; die Negativliste wurde nur im Fehlerzweig gelesen). Jetzt weisen sie ab (36 Schreibvarianten geprüft), Kurzformen bleiben gültig (32 geprüft). **Reichweite:** harte Sperre nur bei Neuaktivierung; für Bestandsprofile ändert sich nur die **Anzeige**. **Ungeprüft:** ob eines der 5 realen Mandate betroffen ist (§34.13.2/.6).
- **Lastfolge gemessen (§34.13.4, drei Aussagen berichtigt):** 42 der 495 Profile (Stufen A2/B6/C34) erhalten über „Arbeit und Soziales" **ein** Sachpaket zusätzlich; kohortenweit **54 → 138 `source_fetch` je 8-h-Fenster**, also **+252/Tag** — **nicht** einmalig. Mandatsgebundene Klassen (je 495) unverändert, Kapazitätsmodell (§30.7) unberührt; der Folgeschritt über `document_understanding` ist **unbeziffert**. **Zweite Auswertestelle:** `scheduler.mandateNewsSources` Nr. 4 baut die Suchanfrage aus `committees[0]` — heute inert, mit `HELMUT_TESTKOHORTE_QUELLEN=aktiv` **1.802** statt 138. **Der Schalter bleibt für den ganzen Test AUS.** Offline-Zahlen.
- **Tests:** Offline-Gesamtlauf **318/318**, Browser-/Mobile-Smoke **32/0**. Neu/erweitert: `testkohorte-provisionierung-inaktiv` **52/0** (A0 belegt den echten Pfad positiv), `test-kohorte-500` **54/0**, `profil-bereitschaft` **103/0**, `funktionstest-ablaufplan` **82/0**, `testkohorte-vorwaerts-cli` **55/0**. Vier Reviewrunden, alle Feststellungen behoben oder begründet offen (§34.12/.13).
- **Gefunden, NICHT repariert (älterer Befund, §34.13.7):** `seeds/entities.js` führt eine **zweite** kuratierte Ausschussliste (23) für den Radar-Ausschussbeleg — **21 von 24** WP-21-Namen haben einen Eintrag, **3** kuratierte Namen sind veraltete WP-20-Bezeichnungen. Stille Radarlücke für **reale** Mandate; für die Kohorte keine Verschlechterung (vorher 0 von 24). Korrektur ändert Production-Klassifikation → eigener PR.
- **Keine Production-Wirkung:** keine Provisionierung, Aktivierung, Umgebungsvariable, Migration, Cron-, Azure-, Budget-, Reserveänderung, kein Modellaufruf, keine Nachricht.
