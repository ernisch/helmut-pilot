...
**Stand: 2026-09-01 — 500-Mandate-Reifesprint lokal abgeschlossen (§21); davor Phase 3 des Understanding-Kapazitätssprints mit Sicherheitssperre (§20).** Vollständige
...
- **F-E2E — Ursache belegt und lokal geschlossen (01.09., ungemergt, §21):** Publish-Stapelzeitstempel im Audit-Pfad des Gerüsts + Totalordnung `created_at.desc,rank.asc.nullslast,id.asc` in `listMatchingResults` (Zweitschlüssel = berechneter Rang; 24 Gleichstandsgruppen in Production belegt). Nachweis 20/20+20/20 unter CPU-Fremdlast. PR #224 (Draft) ist damit überholt. Production-Nachweis steht bis zum Merge aus.
...
## 21 · Sprint 31.08./01.09. — 500-Mandate-Reife (lokal, ungepusht)

**Teilweise abgeschlossen — die theoretische Bereitschaft ist lokal „BEREIT", Merge/Push sind auftragsgemäß nicht erfolgt.** Kanonischer Beleg mit allen Zahlen und den **drei strikt getrennten Urteilen**: [`betrieb/500-mandate-theoretische-bereitschaft-2026-09-01.md`](betrieb/500-mandate-theoretische-bereitschaft-2026-09-01.md). Branch `claude/helmut-500-mandate-readiness-6hxden` (Basis `72d9ec5` = `main`), Commits `0381065` (Blocker 1–5) · `3950226` (F-E2E) · `e11b0ee` (Minimal-Cron) · `12357e6` (Kapazität) · `8e6f0a3` (Kohorte) · `5d34a86` (Review-Fixes) + Doku-Abschluss. Production ausschließlich rein lesend; Gate bleibt `shadow`, keine Migration, kein Modellaufruf, kein Lasttest.

- **Urteile:** Warteschlangen-Aufnahmefähigkeit 500 **erbracht** (Altbeleg, nicht wiederholt) · rechnerisch-architektonische Tragfähigkeit **BEREIT (lokal/theoretisch)** · operativer Mehrtagesbetrieb **NICHT BEWIESEN**.
- **Alle fünf Gate-Blocker aus §20 sind korrigiert** (fail-geschlossene Park-Verknüpfung; belegter Zustandswechsel 0/1/n Zeilen; ehrliche Drain-Bilanz mit echtem gate-würdigem Abfluss (Erstabschlüsse, `ko_version=1`) und CAS-Rückstandstrend; deterministische begrenzte Pagination; Dedup-Totalordnung). Zusätzlich vier bestätigte Befunde eines adversarialen Diff-Reviews (18 Agenten) behoben.
- **F-E2E geschlossen** (s. §9) · **Minimal-Cron `18,48 * * * *` vorbereitet** (Vertrag + Vorab-Bodenprüfung; vercel.json/SLOT_PLAN unverändert, Aktivierung = 6 dokumentierte Betreiberschritte) · **Kapazitätsmodell:** 455 = Erwartungswert bestätigt, **konservativer Zieldeckel 2.416** (Reserve 702, 48-Slot-Kapazität 984 trägt konservativ 802, Stress reißt ehrlich) · **Kohorte 495** (20/75/400) offline vollvalidiert, deterministisch, inaktiv, nichts provisioniert.
- **Naturlauf 21:30 UTC:** §20-Nachträge; keine Freigabegrundlage für einen Gate-Flip vor Merge + natürlichen Läufen der Korrekturen.
- **Nächster sicherer Schritt:** Betreiber prüft den lokalen Branch und gibt Push + PR frei (bis dahin existiert der Stand nur in der Cloud-Arbeitsumgebung); danach je eigene Freigabe: Merge → natürliche Läufe rein lesend → Gate-/Deckel-/Minimal-Cron-Entscheidung entlang Stufenplan.
