...
| **Atomizität** | Reihenfolge, **keine** DB-Funktion | Laufzeile → Projektion → Ablösung → **Abschluss zuletzt**. Damit kann es nie einen vollständigen Lauf ohne veröffentlichte Projektion geben. Eine `SECURITY DEFINER`-Funktion wäre eine neue privilegierte Fläche ohne zusätzliche Garantie und wurde verworfen |
