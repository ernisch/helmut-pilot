# Cem-Cutover-Plan — Blob → SQL (Phase 3, Production-Teil)
...
- Bei `HELMUT_PROFILE_DB_MODE` AUS (heutiger Zustand) ändert sich für Cem **exakt nichts** —
  weder Code-Pfad noch Daten.

## 2. Was noch aussteht, um Cem wirklich aus dem Code zu lösen
...
### Schritt B — Cems Profil einmalig in die SQL-Tabellen schreiben + Flag aktivieren
- **Was genau wird geschrieben:**
  - `public.profiles`: UPDATE der bestehenden Zeile `id='cem-ince'` — nur `name` (heute schon
    gesetzt) bleibt, kein neues Feld nötig dort.
  - `public.mandate_profiles`: **1 neue Zeile** `user_id='cem-ince'` mit dem vollständigen
    `cemInceProfile`-Inhalt, erzeugt über `storage.toMandateProfileRow(cemInceProfile)`
    (bereits Teil dieses PRs, in `scripts/profile-db-test.js` gegen `cemInceProfile` verifiziert
...
- **Wie geschrieben wird:** einmaliger, admin-gesicherter Endpoint (nach demselben Muster wie
  ...
  `storage.saveProfile(cemInceProfile)` mit `HELMUT_PROFILE_DB_MODE=1` aufruft. **Wird in diesem
...
- **Reihenfolge wichtig:** Erst Cems Zeile schreiben (Schritt B1), DANN das Flag aktivieren
  (Schritt B2) — nie umgekehrt, sonst würde `getProfile('cem-ince')` kurzzeitig nichts in der DB
...
- **Backup:** Der Blob (`helmut_store.data.profiles['cem-ince']`) bleibt unverändert bestehen
...
„Production-Migration" (Schritt A) und separat „Production-Datenübernahme Cem" (Schritt B)
vorliegt (siehe Auftrag, Stopp-Bedingungen 1 und 9/10). Bis dahin bleibt Cem vollständig auf dem
