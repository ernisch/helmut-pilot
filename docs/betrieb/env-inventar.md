...
| `HELMUT_PROCESS_RUNS_RELATIONAL` | O | **W-2 Werkzeug-Härtung, FREIGABEPFLICHTIG, Default AUS.** Dual-Write der Prozess-Lauftelemetrie in die relationale Tabelle `process_runs` (Migration `20260727` — NICHT auf Prod angewendet); atomarer Upsert je `(run_id, process)`, dadurch parallel-sicher und idempotent (der Blob verliert Läufe per Last-Write-Wins, Befund W-2). Ohne Flag/Migration bleibt der Blob-Pfad aktiv — mit sichtbaren Fehlern statt `.catch(() => {})`. Übergangsplan: `docs/betrieb/blob-relational-migration-plan.md`. | storage.js · Prod nach Freigabe · fail-safe (Blob-Fallback, Fehler sichtbar) · — |
