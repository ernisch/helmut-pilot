...
| `HELMUT_SECURE_READ_ROLE` | O | Secure Read Path (Sprint 9): Name der **read-only** DB-Rolle (Allowlist: `secure_reader`/`readonly`/`read_only`/`authenticated_readonly`; `service_role` u. ä. werden hart abgewiesen). **Nicht gesetzt in Prod** → Lesepfad inert (fail-closed). | secure-read-path.js · ungesetzt · ohne: kein sicherer Read möglich · zusammen mit `HELMUT_SECURE_READ_GRANT` erst nach Freigabe |
| `HELMUT_SECURE_READ_GRANT` | O·(S) | Secure Read Path (Sprint 9): read-only Credential der o. g. Rolle. Darf **niemals** ein `service_role`-Schlüssel sein (wird timing-safe geprüft und abgewiesen). **Nicht gesetzt in Prod** → Lesepfad inert (fail-closed). | secure-read-path.js · ungesetzt · ohne: kein sicherer Read möglich · **Geheimnis**, nur mit echter read-only-Rolle (freigabepflichtig) |
...
| `HELMUT_RECOVERY_EXECUTE` | O | **Understanding-Recovery, FREIGABEPFLICHTIG, Default AUS.** Schaltet den eng begrenzten Recovery-Pfad (`scripts/understanding-recovery-execute.js`) für genau die 6 bestätigten Allowlist-Vorgänge scharf; zusätzlich exaktes Bestätigungstoken (`HELMUT_RECOVERY_CONFIRM`) nötig. Ohne Flag: reiner Plan-Ausdruck (read-only), kein Write, kein KI-Call. Der schreibende/KI-Schritt ist bewusst noch nicht verdrahtet → auch mit Flag kein Prod-Write, bis nach Freigabe verdrahtet. Analyse: `docs/betrieb/understanding_recovery_trockenlauf.md`. | understanding-recovery.js · Prod nach Freigabe · fail-safe (No-Op) · — |
