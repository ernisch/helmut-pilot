...
| `HELMUT_CRON_PROJEKTION_MS` (Default 8000) · `HELMUT_GLOBALPHASE_ABRUF_STUFE` (Default 20) | O | Feinstellung der K1-Globalphase, nur wirksam bei gesetztem `HELMUT_CRON_GLOBALPHASE`. `…PROJEKTION_MS`: Reserve **je Mandat**, die vom Laufbudget für die Mandatsphase zurückgehalten wird (Production gemessen: ein Matchinglauf ≈ 1,05 s). `…ABRUF_STUFE`: Zahl der Abrufwege je Stufe — zwischen zwei Stufen wird die Restzeit geprüft, damit das globale Budget den Abruf wirklich begrenzt. **Beide heben kein Zeitbudget an**; die globale Phase erhält höchstens `HELMUT_CRAWL_GESAMTBUDGET_MS` und nie weniger als 50 % der Restzeit. | cron-globalphase.js/scheduler.js · alle · Default greift · — |
