...
| `HELMUT_DARK_LAUNCH` | O | Dark Launch der neuen Quellenplattform: off (Default) / shadow. Rein additive, deterministische Analyse (Legacy- vs. Neu-Ergebnis je Mandat, Vergleich/Difference-Report/Metriken/Telemetrie/Admin-Report). Verändert NIE das sichtbare Verhalten und aktiviert die neue Plattform NIE (autoActivateNewAllowed strukturell false). Dynamisch über `flagValue`. | dark-launch.js · nur Analyse · Default off · NICHT im Live-Pfad verdrahtet |
