...
| `HELMUT_TENANT_LLM_LIMITS` | O | Per-Mandant-Overrides als JSON-Karte (schlagen das uniforme Limit), z. B. `{"cem-ince":150,"tenant-x":30}`. Ungültiges JSON → ignoriert (uniformer Default greift), einmalige Log-Warnung. Keine PII (nur Mandant-IDs + Zahlen). | storage.js (`tenantLlmLimitsMap`) · Prod=leer · Default kein Override · — |
...
| `HELMUT_LANDESMODULE` | O | Landtag-Sprint (P2-1): freigegebene Landesmodule als Kommaliste (`berlin,brandenburg`). **Default LEER = alle Landesmodule gesperrt** (BE/BB-Wege werden wie bisher vor jeder Status-/Paketprüfung ausgeschlossen). Ein Wert ist eine ausdrückliche Gründer-Freigabe (Aktivierungscheckliste docs/landtag/). Dynamisch über `flagValue`/helmut-flags.json. | landesmodule.js/source-mode.js/pardok-dispatch.js · alle · Default leer = gesperrt · — |
