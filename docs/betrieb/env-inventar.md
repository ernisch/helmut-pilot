...
| `HELMUT_MAIL_FROM` | O | Absender für Invite-/Reset-Mails (**die einzige Absenderkonfiguration**, es gibt bewusst keine zweite). Default Platzhalter `Helmut <no-reply@…de>` — ohne gültige Domain und deshalb vom lokalen Mailpit-Transport **ehrlich abgelehnt** (`mailpit-absender-ungueltig`). Für lokale Tests eine reservierte Adresse setzen, z. B. `Helmut <noreply@helmut.test>`. Ein **Production**-Versand existiert weiterhin nicht (Domain/Dienst offen). | invite-mail.js · lokal (Prod: unwirksam) · Versand aus (ehrlicher sent=false-Status) · — |
| `HELMUT_MAIL_TRANSPORT` | O | **LOKAL, Default AUS.** Einziger wirksamer Wert `mailpit` (Groß-/Kleinschreibung und Leerraum egal); alles andere lässt den Versand aus. Schaltet die Übergabe von Einladungs-/Reset-Mails an ein **lokales Mailpit-Testpostfach** frei. **In Production und auf Vercel technisch gesperrt** — `NODE_ENV=production`, `VERCEL` oder `VERCEL_ENV` (jeder nicht leere Wert) verweigern den Transport **vor** jeder Zielprüfung. Ohne diese Variable ist das Verhalten byte-identisch zu vorher (`sent:false`, Grund `mail-versand-nicht-konfiguriert`). **Gehört nie nach Vercel.** Anleitung: [`lokale-mailtests-mailpit.md`](lokale-mailtests-mailpit.md) | mail-transport.js/invite-mail.js · nur lokal · fail-closed (Versand aus) · — |
