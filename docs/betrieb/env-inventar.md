...
| `PILOT_SECRET` (auch `HELMUT_PILOT_SECRET`) | P·S | Geteilter Pilot-Zugangscode. **Fail-closed:** auf Vercel ohne diesen Wert kein Zugang. Rotation = Freigabepunkt F1 (am 2026-07-15 ausgeführt, `/api/pilot/unlock` → HTTP 200 verifiziert). | server.js · Prod · fail-closed · Rot: F1 erledigt / sonst bei Verdacht |
...
| `HELMUT_RECOVERY_EXECUTE` | O | **Anker-basierter Understanding-Recovery, FREIGABEPFLICHTIG, Default AUS.** Schaltet den eng begrenzten Recovery-Pfad (`scripts/understanding-recovery-execute.js`) scharf; zusätzlich exaktes Token (`HELMUT_RECOVERY_CONFIRM`) nötig. Allowlist inzwischen auf **genau 1 Vorgang** (`vg-sozialwohnungen`) verengt. **Hinweis:** dieser anker­basierte Pfad zog über Teilstring-Anker themenfremde Zusatzdokumente ein (3-Themen-Digest) und wurde zurückgerollt; für die saubere Wiederherstellung wird stattdessen der **Einzel-Dokument-Pfad** (`HELMUT_SINGLEDOC_RECOVERY_EXECUTE`) genutzt. Ohne Flag: read-only Plan. Analyse: `docs/betrieb/understanding_recovery_trockenlauf.md`, `docs/betrieb/recovery_rollback_rec_29569461715.md`. | understanding-recovery.js · Prod nach Freigabe · fail-safe (No-Op) · — |
| `HELMUT_SINGLEDOC_RECOVERY_EXECUTE` | O | **Einzel-Dokument-Recovery, FREIGABEPFLICHTIG, Default AUS.** Schaltet den Einzel-Dokument-Pfad (`scripts/understanding-single-doc-recovery.js`, `lib/helmut/single-doc-recovery.js`) scharf: rekonstruiert **genau 1** Wissensobjekt aus **genau 1** per exakter `raw_document_id` fest verdrahtetem Seed-Dokument — **keine** Anker-/Teilstring-/Themen-/Cluster-Suche. Zusätzlich exaktes Token `HELMUT_SINGLEDOC_RECOVERY_CONFIRM` (= `RECOVER_SOZIALWOHNUNGEN_SINGLEDOC`) nötig; Lauf-Kennung via `HELMUT_SINGLEDOC_RECOVERY_RUNID`. Ohne Flag+Token: read-only Vorschau, kein Write/KI. Fail-closed bei jeder Vorbedingungsverletzung. Vorschau: `docs/betrieb/recovery_vorschau_vg_sozialwohnungen.md`. | single-doc-recovery.js · Prod nach Freigabe · fail-safe (No-Op) · — |
