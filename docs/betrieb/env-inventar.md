...
| `HELMUT_LANDESMODUL_FREIGABE` | O | Freigabe der Landesmodule **je Bundesland** als Komma-Liste (`berlin`, `brandenburg`). Leer/nicht gesetzt = **kein** Landesmodul freigegeben (heutiger Prod-Zustand). Unbekannte Token — auch `all`/`*`/`on`/`true` — wirken wie leer (**fail-closed, keine Wildcard**). Erlaubnis, kein Auslöser: Paketstatus, Wegabnahme und Referenzzählung gelten unverändert weiter. Via helmut-flags.json/Env (`flagValue`). | flags.js/source-mode.js · Prod=**nicht gesetzt** · Default leer · freigabepflichtig (`CLAUDE.md` §5) |
