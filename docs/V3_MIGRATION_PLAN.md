...
| Flags `HELMUT_V3_STORE/_MATCHING/_LAZY_UNDERSTANDING/_OFFICE` + `HELMUT_UNDERSTANDING_LOCK` | env | ✅ **Step 6 (Audit) entschieden: ALLE behalten.** Keiner ist ein *totes* Flag (alle gaten aktiven Code). V3 ist **bereits** der einzige produktive Pfad (V2 entfernt) — die Flags schalten den *einen* V3-Pfad an/aus, kein Parallelpfad. Erhalten als **Not-Aus** (`HELMUT_V3_STORE` = Master, → Leerzustand statt V2), **Kosten-Schutz** (`HELMUT_UNDERSTANDING_LOCK`) und **Feature-Toggle** (`HELMUT_V3_OFFICE`). Betreiber-Entscheidung (bewusst kein Hart-Verdrahten). Dead-Code-Sweep (Aufgabe 8): 5 tote `server.js`-Imports entfernt. |
...
4. ~~`runMorningBriefing` + `personalization.js` + `runtime.js`-Briefing-Familie löschen.~~
   ✅ **teilweise erledigt (Step 4):** `runDailyPipeline` (tot) + `runMorningBriefing` (kein aktiver
