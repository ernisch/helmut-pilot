...
| Flags `HELMUT_V3_STORE/_MATCHING/_LAZY_UNDERSTANDING/_OFFICE` + `HELMUT_UNDERSTANDING_LOCK` | env | **Step 5 geprüft: NICHT ungenutzt** — alle gaten aktiv den V3-Store (`v3StoreReady`/`v3MatchingEnabled`/… mit 40+ Call-Sites). „Unbedingt-live schalten + entfernen" ist ein **separater Verhaltens-Schritt** (V3 immer AN), kein „ungenutzte Flags löschen" → bewusst nicht angefasst. |
