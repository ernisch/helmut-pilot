...
| **B4** — Vorgangsbildung verwirft Ereignisse lautlos bei `vorgang_id`-Kollision (Diagnose-Sprint 2026-07-26, CSD-2026-Fall) | **belastbar bewiesen**, read-only gegen Production: `deriveVorgangId()` reduziert einen Cluster auf **ein einzelnes Wort**; trifft dieses Wort ein bereits bestehendes, thematisch fremdes KO, verwirft `understandOneCluster` den neuen Cluster über `skipped-exists` — **kein KI-Aufruf, kein `ko_document_links`-Eintrag, kein Fehler, kein Protokolleintrag**. Am belegten Fall (tödlicher Anschlag auf den Berliner CSD, 25./26.07.) entstand dadurch **0 Knowledge Objects** trotz 21 gecrawlter Rohdokumente. Hochgerechnet über 7 Produktionstage: **21,7 % der Cluster / 47,0 % aller Rohdokumente** treffen eine Kollision dieser Art. Vollständige Beweiskette, Reparaturplan (nicht ausgeführt) und Regressionstest (`scripts/vorgangsbildung-verlust-test.js`, reproduziert den Ist-Zustand) in [`befund-csd-2026-vorgangsverlust.md`](befund-csd-2026-vorgangsverlust.md) | OP-14 (schärft es), neuer Reparaturbedarf noch ohne eigene OP-Nummer — nächster Sprint entscheidet Einordnung |
