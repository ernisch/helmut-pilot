...
const G = require("./prosa-modellversuch"), T = require("./privater-nachweis-transport");
...
function harness(options = {}) {
  ...
  const fetchFn = async (url, config) => {
    ...
    return { status: 200, json: async () => ({ ok: true, reinLesend: true, schemaVersion: 1, production: true,
      commit: require("./gipfel-modellversuch").MAIN, storageSupabase: true,
...
(async () => {
  ...
  await test("Aktive Profile, offene Kommunikation und unklarer Claim erzeugen keinen Modellaufruf", async () => {
    for (const options of [{ active: true }, { communicationOpen: true }, { unknownWrite: true }]) {
  ...
  await test("Workflow bindet neuen Branch und haelt Gipfeljob, Crons und Anwendung unveraendert", () => {
    ...
    const own = yaml.slice(yaml.indexOf("  prosa-fachnachweis:"));
    A(own.includes(`refs/heads/${G.BRANCH}`)); A(own.includes("github.run_attempt == 1"));
    A(own.includes("timeout-minutes: 3")); A(own.includes("node scripts/prosa-modellversuch.js"));
    A(yaml.includes("!startsWith(inputs.confirm_text, 'PROSA_EINMAL:')"));
