...
const V = require("./prosa-restfaelle-versuch");
...
function setup(change = {}) {
  ...
  let count = 1, reads = 0, writes = 0; const logs = [], calls = [];
...
(async () => {
  await test("Fester neuer Auftrag bindet nur12 ungesendete Sollfaelle, Code, Production und Empfaenger", () => {
    A.deepEqual(V.eingabe(encode(a)), a); A.equal(V.MAX_COST, 1272000);
  ...
  await test("Sechs Zweiergruppen,12 Vergleiche, vollstaendige Antwortsicherung und keine Produktfreigabe", async () => {
    const s = setup(), old1 = structuredClone(s.auth().prosaEinordnung20260921),
      old2 = structuredClone(s.auth().prosaProfilvertrag20260921);
    const out = await V.ausfuehren(s.deps); A.equal(out.ok, true, JSON.stringify(s.decode(s.auth()[V.KEY].ergebnis))); A.equal(s.calls.length, 6);
    A.deepEqual(s.calls.map(c => c.meta.testKostenPhase), Array(6).fill("pruefung"));
    A.deepEqual(s.calls.map(c => c.meta.politicianId), Array.from({length:6}, (_,i) => "synthetisch-restfaelle-" + (i+1)));
    ...
    A.equal(result.urteile.length, 12); A.equal(result.restvergleichBestanden, true); A.equal(result.methodenvergleichBestanden, false); A.equal(result.ausgeschlosseneFaelle, 6);
    ...
    await A.rejects(V.ausfuehren(s.deps), /BEREITS_BEGONNEN/); A.equal(s.calls.length, 6);
  });
  await test("Jeder fachliche Fehlbefund stoppt nach gesicherter erster Gruppe ohne Retry", async () => {
    ...
    A.equal(result.urteile.length, 2); A.equal(result.sollFaelle, 12);
  ...
  await test("Alte Versuche muessen geschlossen sein und bleiben geschuetzt", async () => {
    for (const key of ["prosaEinordnung20260921", "prosaProfilvertrag20260921", "prosaPraemissen20260921"]) {
  ...
  await test("Fehlende volle Reserve fuer sechs Aufrufe, fehlender Tagesbeleg und fremde offene Reserve sperren", async () => {
  ...
  await test("Kosten oder fremde gemeinsame Daten abweichend verhindern die zweite Gruppe", async () => {
    for (const edit of [a => a.llmUsage[0].politicianId = "fremd", a => a.sessions.push({ id: "fremd" }),
      a => a.testKostenTage[V.TAG].calls.call2.bezug.phase = "fremd"]) {
  ...
  await test("Sechs verdrängte Telemetriezeilen werden vor dem Start dauerhaft archiviert", async () => {
    const s = setup(); s.auth().llmUsage = Array.from({ length: 5000 }, (_, i) => ({ id: "alt" + i }));
    const tail = structuredClone(s.auth().llmUsage.slice(-6));
    A.equal((await V.ausfuehren(s.deps)).ok, true);
    A.deepEqual(s.auth()[V.KEY].archivierteAufruftelemetrie, tail);
    A.equal(s.auth().llmUsage.length, 5000); A.equal(s.auth().llmUsage.at(-1).id, "alt4993");
  });
  await test("Sechs verlorene Faelle sind selbst bei neuem Commit niemals erneut in der Nutzlast", () => {
    const p = V.paket(), old = Alt.paket();
    A.deepEqual(p.ausgeschlossen, old.gruppen[0]); A.deepEqual(p.gruppen.flat(), old.gruppen.slice(1).flat());
  ...
  await test("Freigegebene, fremde oder unvollstaendige Altreserve und fehlende Deckung stoppen vor Sendung", async () => {
    const edits = [s => delete s.testKostenTage[V.TAG].calls[V.ALT_TICKET],
      ...
      s => { s.testKostenTage[V.TAG].baseline = s.testKostenTage[V.TAG].spent = 2516001; },
    ...
    const s = setup(); s.auth().testKostenTage[V.TAG].baseline = s.auth().testKostenTage[V.TAG].spent = 2516000;
    A.equal((await V.ausfuehren(s.deps)).ok, true);
  });
