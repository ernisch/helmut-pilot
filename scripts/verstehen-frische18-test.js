...
const B = require("./verstehen-frische18");
let count = 0;
...
(async () => {
  ...
  await test("Runtime ist an ersten manuellen Main-Lauf und exakten Commit gebunden", () => {
    const commit = "a".repeat(40), env = { HELMUT_FRISCHE18_RUNTIME_COMMIT: commit, GITHUB_ACTIONS: "true",
    ...
    for (const change of [{ GITHUB_RUN_ATTEMPT: "2" }, { GITHUB_SHA: "b".repeat(40) }, { GITHUB_REF: "refs/heads/feature" },
      { GITHUB_EVENT_NAME: "schedule" }, { HELMUT_FRISCHE18_RUNTIME_COMMIT: "" }]) assert.throws(() => B.pruefeRuntime({ ...env, ...change }, commit));
  });
  await test("Echter Kostenadapter bindet 18er-Reserve und Abrechnung an genau einen Lauf", async () => {
    ...
    const deps = { env, storage, now: () => new Date("2026-09-26T13:40:00.000Z"), id: () => "offline-" + ++seq };
    const runId = "verstehen18-123456789";
    ...
    assert.equal(auth.testKostenTage["2026-09-26"].calls[t.id].bezug.runId, runId);
    ...
    for (const id of ["verstehen-frische18-12", "verstehen-frische18-fremd", "verstehen-bund8-123456789"])
  ...
  await test("Neuer UTC-Tag braucht vollstaendige Kostenhistorie; Vorpruefung schreibt nichts", async () => {
    ...
    const ticket = await K.reserviere({ model: "gpt-5-mini", maxOutputTokens: 3000, runId: "verstehen18-123456789" }, deps);
  ...
  await test("Workflow haelt Plan ohne Modellzugang und eindeutigen Erstlauf fest", () => {
    const text = require("node:fs").readFileSync(require("node:path").join(__dirname, "../.github/workflows/verstehen-frische18.yml"), "utf8");
    for (const rule of ["github.run_attempt == 1", "inputs.runtime_commit == github.sha", "contents: read",
      ...
      "HELMUT_TESTLAUF_KOMMUNIKATION: gesperrt", "node scripts/verstehen-frische18.js --plan"])
  ...
  await test("Fremde Quellen, offene Belege und ungesicherte Lieferfunktionen bleiben gesperrt", async () => {
    ...
    const r = await V.pruefeUndPlane({ ids: [], deps: {}, commit: F.FRISCHE18.commit, erwartet: F.FRISCHE18 });
    assert.equal(r.ok, false);
    const q = await V.fuehreAus({ ids: [], deps: {}, erwartet: F.FRISCHE18, quittungsschluessel: "verstehen30-20260925-a" });
    assert.equal(q.grund, "verstehen-frische18-quittung-abweichend");
  });
  await test("Gleiche UTC-Zeitpunkte bleiben nach PostgreSQL-Lesung gebunden; echte Aenderungen nicht", () => {
    ...
    const doc = { id: "rd-zeitprobe", title: "Amtlicher Quellenbeleg zur Zeitbindung",
      ...
      published_at: "2026-09-26T11:00:00.000Z", retrieved_at: "2026-09-26T12:44:58.165Z" };
    const beleg = { version: 2, dokumentId: doc.id, quellenHash: A.quellenstandHash(doc),
      ...
      gelesenAm: "2026-09-26T12:45:00.000Z", absatzPosition: 1, text: "Der unveraenderte Originalabsatz wird an die konkrete Quellenzeile gebunden.",
    ...
    const pg = { ...doc, published_at: "2026-09-26T11:00:00+00:00", retrieved_at: "2026-09-26T12:44:58.165000+00:00" };
    ...
    for (const change of [{ published_at: "2026-09-26T11:00:00.001Z" },
      { retrieved_at: "2026-09-26T12:44:58.165001Z" }, { published_at: "2026-09-26" },
  ...
  if (process.argv[2]) {
    ...
    if (process.argv[3]) await test("Alle 18 realen Production-Zeilen bestehen mit unveraenderten Importbelegen", async () => {
    ...
    await test("Alle 18 echten Eingaben und Belege erreichen unveraendert ihren Einzelcluster", async () => {
    ...
    await test("Gebundener 18er-Lauf: einmalig, Kostenreserve, erster Fachfehler stoppt", async () => {
      ...
      async function run({ failAt = 0, price = 0.003 } = {}) {
        ...
        const options = { ids: payload.rows.map(d => d.id), deps, execute: true, commit: F.FRISCHE18.commit,
          erwartet: F.FRISCHE18, quittungsschluessel: F.QUITTUNG, runId: "isoliert" };
      ...
      try {
        const full = await run(); assert.equal(full.result.ok, true); assert.equal(full.calls, 18);
        const failed = await run({ failAt: 2 }); assert.equal(failed.result.ok, false); assert.equal(failed.calls, 2);
        assert.equal(failed.receipt.abbruchGrund, "verstehen-frische18-einzelergebnis-nicht-bestaetigt");
        const expensive = await run({ price: 0.1 }); assert.equal(expensive.result.ok, false); assert.equal(expensive.calls, 5);
