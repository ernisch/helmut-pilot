...
const G = require("./aussagenabdeckung-versuch"), T = require("./privater-nachweis-transport");
...
function harness(options = {}) {
  ...
  const request = async (prompt, schema, meta, model, settings) => {
    ...
    if (options.referenceBad) result.quellen[0].aussagen.pop();
...
(async () => {
  ...
  await test("Workflow bindet neuen Branch und haelt Gipfeljob, Crons und Anwendung unveraendert", () => {
    ...
    const own = yaml.slice(yaml.indexOf("  aussagenabdeckung-eingang:"));
    A(own.includes(`refs/heads/${G.BRANCH}`)); A(own.includes("github.run_attempt == 1"));
    A(own.includes("timeout-minutes: 3")); A(own.includes("node scripts/aussagenabdeckung-versuch.js"));
    A(yaml.includes("!startsWith(inputs.confirm_text, 'ABDECKUNG_EINMAL:')"));
