...
function szenarioUeberschreiben() {
  ...
  psql(`insert into public.knowledge_objects(id,vorgang_id,status,verstehen_fencing)`
    + ` values ('ko-vg-o','vg-o','neu',${u.fencing})`);
...
async function main() {
  ...
  const m2 = szenarioAbloesung();
  const m2Abschluss = psql(`select public.helmut_verstehen_abschluss('vg-m','alt',${m2.erst.fencing},'ergebnis-des-alten')`).out;
  mutation("M2 fehlender Fencing-Wert -> der abgeloeste Arbeiter schliesst den Vorgang mit SEINEM Ergebnis ab",
    m2.uebernahme.fencing === m2.erst.fencing && m2Abschluss === "t",
    `erst=${m2.erst.fencing} uebernahme=${m2.uebernahme.fencing} abschluss=${m2Abschluss}`);
...
comment on function public.helmut_verstehen_schreibrecht`,
...
comment on function public.helmut_verstehen_schreibrecht`);
  ...
  if (loecher > 0) {
    ...
    console.log("Alle sechs geforderten Schutzmechanismen sind nachweislich wirksam.");
  }
