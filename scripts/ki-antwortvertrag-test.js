...
// keine Datenbank und kein echtes Geheimnis. Belegt vier Zusagen:
...
// 3. Der reale Steuerzeichenfehler wird an der produktiven HTTP-Engstelle gerettet.
// 4. Nur status completed liefert Modelltext aus und wird als Erfolg gebucht.
...
async function main() {
  ...
  // Reproduktion des Production-Pfads: Das rohe Steuerzeichen sitzt bereits im
  // JSON-Umschlag des Anbieters. Nach dessen Rettung enthaelt der extrahierte
  // Modelltext dasselbe rohe Zeichen; auch der getrennte Modellparser muss es
  // deshalb retten. Dieser Test laeuft bewusst durch requestStructuredJson und
  // nicht nur gegen die exportierten Parserhelfer.
  naechsteAntwort = '{"status":"completed","output_text":"{\\"text\\":\\"Zeile eins\nZeile zwei\\"}","usage":{"input_tokens":13,"output_tokens":8,"total_tokens":21}}';
  const steuerzeichenErgebnis = await ai.requestStructuredJson(
    "offline", schema, { callType: "vertrag-steuerzeichen" }, "gpt-4.1");
  const steuerzeichenLog = protokoll.find((eintrag) => eintrag.callType === "vertrag-steuerzeichen");
  check("7 Rohes Steuerzeichen wird an der produktiven HTTP-Engstelle gerettet",
    steuerzeichenErgebnis.text === "Zeile eins\nZeile zwei");
  check("8 Der gerettete Aufruf behaelt die echte Usage",
    steuerzeichenLog && steuerzeichenLog.success === true
      && steuerzeichenLog.usage && steuerzeichenLog.usage.total_tokens === 21);
  ...
  check("9 status completed liefert den strukturierten Modelltext aus", ergebnis.text === "vollstaendig");
  check("10 Nur der vollstaendige Abschluss wird als Erfolg gebucht",
