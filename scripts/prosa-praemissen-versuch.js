...
const KEY = "prosaPraemissen20260921", PREFIX = "PRAEMISSEN_EINMAL:";
const BRANCH = "codex/prosa-praemissenpruefung-20260921", TAG = "2026-09-21";
const MAX_COST = 636000, MAX_MS = 300000, RING = 5000;
...
function kostenbeleg(before, after, meta, counterBefore, counterAfter) {
  ...
  fordere(c.status === "abgerechnet" && c.reserved === 212000 && c.maxOutputTokens === 3000
    ...
    && u.callType === "prosaPraemissen" && counterAfter.used === counterBefore.used + 1,
...
async function ausfuehren({ env = process.env, now = () => new Date(), fetchFn = global.fetch,
  ...
  await storage.mutateAuthStore(auth => {
    ...
    expected = { version: 1, runId, commit: a.commit, productionCommit: a.productionCommit,
      ...
      archivierteAufruftelemetrie: structuredClone(auth.llmUsage.slice(Math.max(0, RING - 3))),
  ...
  let fehler = null;
  const result = { urteile: [], sollFaelle: 18, methodenvergleichBestanden: false,
  ...
  try {
    ...
    for (let i = 0; i < p.gruppen.length; i++) {
      ...
      const meta = { runId, phase: "pruefung", gruppe: i + 1, mandat: "synthetisch-praemissen-" + (i + 1),
      ...
      const observed = await observe(() => ai.requestStructuredJson(prompt, schema, {
        callType: "prosaPraemissen", politicianId: meta.mandat, runId, testKostenPhase: meta.phase
    ...
    fordere(expected.phasen.length === 3 && result.urteile.length === 18
