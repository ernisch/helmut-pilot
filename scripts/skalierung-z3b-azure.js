...
function liesKonfiguration(env = process.env, { heuteUtc = new Date().toISOString().slice(0, 10) } = {}) {
  ...
  // KEINE AUTOMATISCHE FORTSETZUNG: fuer die Stichprobe muss die Laufkennung der
  ...
  if (modus === "stichprobe") {
    if (!/^[a-z0-9]{6,32}$/.test(vorprobeLauf)) {
      throw new Z3bAzureAbbruch(
        "Z3b Azure Stichprobe verlangt die Laufkennung der getrennt freigegebenen gruenen Vorprobe",
        "paketkette"
      );
    }
    if (vorprobeLauf === laufKennung) {
      throw new Z3bAzureAbbruch(
        "Z3b Azure Stichprobe darf nicht die eigene Laufkennung als Vorprobebeleg fuehren",
        "paketkette"
      );
    }
...
async function fuehreMesslauf(konfiguration, { fetchImpl = globalThis.fetch, jetztMs = () => Date.now() } = {}) {
  ...
  return Object.freeze({
    ...
    vorprobeLauf: konfiguration.vorprobeLauf,
    erhobenUtc,
...
// PROZESSWEITE PAKETSPERRE. Der Einstieg fuehrt hoechstens EIN Messpaket aus.
// Es gibt keinen Ruecksetzer: eine automatische Fortsetzung von der Vorprobe zur
// Stichprobe ist im selben Prozess technisch unmoeglich. Der eigentliche,
// nicht umgehbare Riegel zwischen den Paketen ist die Freigabekennung der
// Stichprobe — sie muss die Laufkennung einer bereits gelaufenen Vorprobe
// tragen, die es vor der Vorprobe nicht geben kann.
let prozessPaketGelaufen = false;

async function main() {
  try {
    if (prozessPaketGelaufen) {
      throw new Z3bAzureAbbruch(
        "Z3b Azure fuehrt je Prozess hoechstens ein Freigabepaket aus",
        "paketkette"
      );
    }
    prozessPaketGelaufen = true;
