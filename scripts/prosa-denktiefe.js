...
// Isolierter, einmaliger Neunfallvergleich mit reasoning low. Keine Appinhalte oder Profile schreiben.
...
// Feste Production-Bindung nach dem Dokumentationsmerge PR446.
const MAIN = "05feca37476e23d892ee45ef08d4706923486279";
...
const KEY = "prosaDenkFachnachweis20260919", PREFIX = "PROSA_DENKEN:";
const BRANCH = "codex/prosa-fachnachweis-20260919";
const MANIFEST = "303ceda18e3a6b0574e5210719ccc329cb3f5ebbfc442c97c1ac96dfc31bd2dd";
...
const PROMPTS = [
  ...
  "8f44c0aef535d4558f8d02023e0bb76ff8a45f9ea6370aec48941b4cada9418d"
];
...
const MAX_MS = 30 * 60000, RESERVE_MS = 180000, MAX_COST = 1908000;
...
function paket() {
  const raw = F.readFileSync(P.join(__dirname, "../docs/betrieb/prosa-denktiefe-2026-09-19.json"));
  fordere(sha(raw) === MANIFEST && sha(JSON.stringify(SCHEMA)) === SCHEMA_HASH, "PROSA_PAKET");
  const m = JSON.parse(raw);
  fordere(m.faelle.length === 9, "PROSA_PAKET");
...
function eingabe(text) {
  ...
  fordere(a && Object.keys(a).sort().join(",") === "commit,position,previous,publicKey"
    && /^[a-f0-9]{40}$/.test(a.commit) && Number.isInteger(a.position) && a.position >= 1 && a.position <= 9,
  "PROSA_BINDUNG");
...
async function vorflug(env, storage, fetchFn, now) {
  ...
  fordere(r.ok === true && r.reinLesend === true && r.schemaVersion === 1
    ...
    && K.tagespolitikGueltig(r.testKosten) && r.testKosten.aktiv === true,
  "PROSA_LAUFZEIT");
  ...
  await storage.mutateAuthStore(auth => {
    ...
    if (a.position === 1) {
      ...
      r = { version: 1, manifestHash: MANIFEST, schemaHash: SCHEMA_HASH, commit: a.commit,
        ...
        archivierteAufruftelemetrie: structuredClone(auth.llmUsage.slice(Math.max(0, USAGE_MAX - 9))),
    ...
    fordere(r.faelle.length < 9 && r.faelle.reduce((s, f) => s + f.kosten, 0) + 212000 <= MAX_COST,
      "PROSA_AUFTRAGSLIMIT");
...
function telemetrieErhalten(before, after, usage, claim) {
  if (usage.length !== 1 || !Array.isArray(claim.archivierteAufruftelemetrie)
    || claim.archivierteAufruftelemetrie.length > 9) return false;
...
async function mitTransportbeleg(call, { prompt, env, deadline, emit, now = () => Date.now() }, https = H) {
  ...
  const expected = { model: "gpt-5-mini", input: prompt, max_output_tokens: 3000,
    reasoning: { effort: "low" }, text: { format: { type: "json_schema", name: "knowledge_object", schema: SCHEMA, strict: false } } };
...
async function ausfuehren({ env = process.env, now = () => new Date(), fetchFn = global.fetch,
  ...
  const before = await storage.readAuthStore(), runId = `prosa-denken-${env.GITHUB_RUN_ID}`;
  ...
  try {
    ...
    result = await observe(() => request(f.prompt, SCHEMA,
      { callType: "understanding", pipelineStep: "prosa-denktiefe", politicianId: null, runId },
      "gpt-5-mini", { strict: false, reasoningEffort: "low" }),
