...
const BESTAETIGUNG = "DIE_15_UNBEGONNENEN_QUELLEN_EINMAL_VERSTEHEN";
const fordere = (v, g) => { if (!v) throw new Error("rest15-" + g); };
...
function pruefeRuntime(env, gitCommit) {
  const commit = env.HELMUT_REST15_RUNTIME_COMMIT;
...
async function main(args = process.argv.slice(2), env = process.env) {
  ...
  const vorgaenger = await read("helmut_store", "select=data&id=eq.verstehen-frische16-20260926-a&limit=2");
  const cas = await read("helmut_verstehen_reservierungen", "select=*&vorgang_id=eq." + F.VORGANG + "&limit=2");
  fordere(vorgaenger.length === 1 && cas.length === 1 && F.pruefeVorgaenger(vorgaenger[0].data, cas[0]), "vorgaenger-abweichend");
  const ids = (eingabe.rows || []).map(d => d.id).filter(id => id !== F.AUSGESCHLOSSEN);
  fordere(ids.length === 15 && V.idsHash(ids) === F.REST15.idHash, "ids-abweichend");
  const originale = await storage.getRawDocumentsByIds((eingabe.rows || []).map(d => d.id));
  ...
  fordere(new Date(Date.now() + F.REST15.maxMs).toISOString().slice(0, 10) === day, "tageswechsel");
  ...
  fordere(neueKosten === 0 && reserve <= F.REST15.maxUsd
  ...
  deps.acquireLock = () => storage.acquireGlobalUnderstandingLock(F.REST15.maxMs + 60000);
  const timer = execute ? setTimeout(() => { console.error("rest15-harte-laufzeit; nur nachlesen, kein Retry"); process.exit(1); }, F.REST15.maxMs) : null;
  try {
    const out = await V.fuehreAus({ ids: liste.ids, deps, execute, erwartet: F.REST15,
      commit: F.REST15.commit, runtimeCommit, runId, quittungsschluessel: F.QUITTUNG, env });
    ...
    const nachCas = await read("helmut_verstehen_reservierungen", "select=*&vorgang_id=eq." + F.VORGANG + "&limit=2");
    fordere(JSON.stringify(nachCas) === JSON.stringify(cas), "gesperrter-vorgang-veraendert");
    out.gesperrterVorgangUnveraendert = true;
    out.profileUnveraendert = profilHash === nachHash;
    out.vollstaendigVerstanden = execute && out.ergebnisse?.length === F.REST15.cluster
    ...
    out.quellenkontextCommit = F.REST15.commit;
...
if (require.main === module) main().then(code => { process.exitCode = code; }).catch(e => {
  console.log(JSON.stringify({ ok: false, grund: /^rest15-[a-z-]+$/.test(e.message || "") ? e.message : "rest15-technischer-fehler", automatischeWiederholung: false })); process.exitCode = 1;
});
