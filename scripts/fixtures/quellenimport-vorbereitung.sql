-- NICHT FREIGEGEBEN, NICHT AUSGEFUEHRT. Nur exakt diese31 Quellen und Fundstellen.
-- Vor einer Freigabe: PR462 integriert/READY und Schutzstand frisch lesen.
...
begin
  ...
  if jsonb_array_length(eingabe->'rows')<>31 or jsonb_array_length(eingabe->'findings')<>31 then
  ...
  if n<>31 then raise exception 'quellenimport-rohzahl'; end if;
  ...
  get diagnostics n = row_count;
  if n<>31 then raise exception 'quellenimport-fundzahl'; end if;
  if exists(select 1 from jsonb_array_elements(eingabe->'rows') x
