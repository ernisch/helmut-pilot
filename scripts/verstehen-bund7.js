...
const BESTAETIGUNG = "DIE_7_AMTLICHEN_QUELLEN_EINMAL_VERSTEHEN";
const fordere = (v, g) => { if (!v) throw new Error("bund7-" + g); };
...
function pruefeRuntime(env, gitCommit) {
  const commit = env.HELMUT_BUND7_RUNTIME_COMMIT;
...
async function main(args = process.argv.slice(2), env = process.env) {
  ...
  fordere(ids.length === 7 && V.idsHash(ids) === F.BUND7.idHash, "ids-abweichend");
  ...
  // Sperre deckt das volle Zehnminutenfenster plus Abschlussreserve ab.
  deps.acquireLock = () => storage.acquireGlobalUnderstandingLock(F.BUND7.maxMs + 60000);
  const timer = execute ? setTimeout(() => { console.error("bund7-harte-laufzeit; nur nachlesen, kein Retry"); process.exit(1); }, F.BUND7.maxMs) : null;
  try {
    const out = await V.fuehreAus({ ids: liste.ids, deps, execute, erwartet: F.BUND7,
      commit: F.BUND7.commit, runtimeCommit, runId, quittungsschluessel: F.QUITTUNG, env });
    ...
    out.vollstaendigVerstanden = execute && out.ergebnisse?.length === F.BUND7.cluster
