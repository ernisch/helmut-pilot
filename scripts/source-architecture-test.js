...
check("WBSB: neue Herausgeber (BMWSB/BBSR/BBR/recht.bund.de/Staedtebaufoerderung/Foerderdatenbank) vorhanden", (() => {
  const doms = new Set(M.publishers.map((p) => p.canonical_domain));
  return ["bmwsb.bund.de", "bbsr.bund.de", "bbr.bund.de", "recht.bund.de", "staedtebaufoerderung.info", "foerderdatenbank.de"].every((d) => doms.has(d));
})());
