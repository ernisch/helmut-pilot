...
let aiCalls = 0;
function fixtureUnderstanding(prompt) {
  aiCalls += 1;
  const treffer = AI_FIXTURES.filter((f) => String(prompt).includes(f.marker));
  if (treffer.length !== 1) {
    // Fail loud: ein Cluster ohne (oder mit mehrdeutiger) Fixture-Zuordnung waere ein
    // stiller Teilerfolg — genau die Fehlerklasse, die dieser Vertrag ausschliesst.
...
function neuerStore() {
  ...
  st.api = {
    ...
    listKnowledgeObjects: (opts = {}) => {
      const alle = [...st.knowledgeObjects.values()].map((k) => JSON.parse(JSON.stringify(k)));
      return alle.slice(0, Number(opts.limit) > 0 ? Number(opts.limit) : 500);
    },
...
(async () => {
  ...
  if (!relevantRoh || !irrelevantRoh || !bbRoh) {
    ...
    console.log(`\n${passed} bestanden, ${failed + 1} fehlgeschlagen`);
    process.exit(1);
  }
  ...
  check("L1 kein einziger echter KI-Aufruf (alle Analysen aus Fixtures)", aiCalls >= 4 && typeof fixtureUnderstanding === "function");
  ...
  console.log(`\n${passed} bestanden, ${failed + 1} fehlgeschlagen (Abbruch)`);
  process.exit(1);
});
