...
// ============================ PFLICHTFALL 6 ============================
console.log("== 6) Kein aktives Profil -> nur dauerhaft aktive Basisquellen ==");
const none = pp.computeGlobalActivation({ ...base, profiles: [] });
check("kein Profil: nur bund-basis technisch aktiv", JSON.stringify(none.packageStatus.filter((p) => p.activation === "active").map((p) => p.key)) === JSON.stringify(["bund-basis"]));
check("kein Profil: aktive Abrufwege = 54 (Bund Basis, always_on)", none.activePathCount === 54);
check("kein Profil: optionale Pakete NICHT aktiv (arbeit-und-soziales/die-linke)", none.packageStatus.filter((p) => ["arbeit-und-soziales", "die-linke-bund", "regional-niedersachsen"].includes(p.key)).every((p) => p.activation !== "active"));
...
check("bund-basis ist always_on (laeuft ohne Profil)", M.packages.find((p) => p.key === "bund-basis").always_on === true);
