...
function neuerStore(opts) {
  ...
  st.api = {
    ...
    saveMatchingResults: (rows) => {
      ...
      //
      // KORREKTUR DES KOMMENTARS (Abschlussreview 2026-08-08): hier stand „dort jetzt
      // `order=created_at.desc,id.desc`". Das ist NICHT so — `storage.listMatchingResults`
      // sendet unveraendert nur `&order=created_at.desc`, und der PR begruendet in
      // storage.js ausdruecklich, warum er das NICHT aendert (welcher Vorgang zuerst
      // angezeigt wird, ist eine Produktentscheidung). Der Kommentar behauptete also eine
      // Korrektur am Production-Lesepfad, die es nicht gibt.
      //
      ...
      // J8 weiterhin. Im Abschlussreview am 2026-08-08 in EINEM von sechs vollstaendigen
      // Offline-Laeufen aufgetreten (`brandenburg-e2e-vertrag-test.js` J8); in 12
      // Einzellaeufen auf diesem Branch und 8 auf unveraendertem `origin/main` nie.
      // F-E2E bleibt damit offen (CURRENT_STATE §9, Entwurf PR #224).
