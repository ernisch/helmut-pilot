...
const KEY = "prosaRestfaelle20260921", PREFIX = "RESTFAELLE_EINMAL:";
const BRANCH = "codex/prosa-restfaelle-20260921", TAG = "2026-09-21";
const MAX_COST = 1272000, MAX_MS = 300000, RING = 5000;
...
function paket() {
  const alt = Alt.paket();
  fordere(alt.paketHash === ALT_PAKET, "EINORDNUNG_ALTPAKET");
  const rest = alt.gruppen.slice(1).flat(), ausgeschlossen = alt.gruppen[0];
  fordere(rest.length === 12 && ausgeschlossen.length === 6
    && rest.every(r => !ausgeschlossen.some(a => a.eingabe.id === r.eingabe.id
      || hash(a.eingabe) === hash(r.eingabe))), "EINORDNUNG_WIEDERHOLUNG");
  const gruppen = Array.from({ length: 6 }, (_, i) => rest.slice(i * 2, i * 2 + 2));
  const vertraege = gruppen.map(g => P.binde(g.map(r => r.eingabe)));
  return { gruppen, ausgeschlossen, altPaketHash: ALT_PAKET, schema: P.SCHEMA,
    prompts: vertraege.map(v => P.prompt(v)),
    paketHash: hash({ gruppen, schema: P.SCHEMA, prompts: vertraege.map(v => P.prompt(v)),
      auswertung: auswertung.toString(), ausgeschlossen, altPaketHash: ALT_PAKET, scope: "12-ungesendete-Restfaelle-6-verlorene-Faelle-gesperrt" }) };
}
...
function kostenbeleg(before, after, meta, counterBefore, counterAfter) {
  ...
  fordere(c.status === "abgerechnet" && c.reserved === 212000 && c.maxOutputTokens === 3000
    ...
    && u.callType === "prosaRestfaelle" && counterAfter.used === counterBefore.used + 1,
...
async function ausfuehren({ env = process.env, now = () => new Date(), fetchFn = global.fetch,
  ...
  await storage.mutateAuthStore(auth => {
    ...
    expected = { version: 1, runId, commit: a.commit, productionCommit: a.productionCommit,
      ...
      archivierteAufruftelemetrie: structuredClone(auth.llmUsage.slice(Math.max(0, RING - 6))),
  ...
  let fehler = null;
  const result = { urteile: [], sollFaelle: 12, ausgeschlosseneFaelle: 6, restvergleichBestanden: false, methodenvergleichBestanden: false,
  ...
  try {
    ...
    for (let i = 0; i < p.gruppen.length; i++) {
      ...
      const meta = { runId, phase: "pruefung", gruppe: i + 1, mandat: "synthetisch-restfaelle-" + (i + 1),
        basisHash: vertrag.eingabeHash };
      await schreibe(r => { r.phasen.push({ ...meta, status: "begonnen" }); });
      const before = await storage.readAuthStore();
      const prompt = p.prompts[i], schema = p.schema;
      const observed = await observe(() => ai.requestStructuredJson(prompt, schema, {
        callType: "prosaRestfaelle", politicianId: meta.mandat, runId, testKostenPhase: meta.phase
    ...
    fordere(expected.phasen.length === 6 && result.urteile.length === 12
...
module.exports = { paket, auswertung, eingabe, konfiguration, vorflug, grundlinie, kostenbeleg, transport, ausfuehren,
  KEY, PREFIX, BRANCH, TAG, MAX_COST, MAX_MS, ALT_TICKET };
