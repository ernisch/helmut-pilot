...
const KEY = "quellenfaktenEingang20260920", PREFIX = "FAKTEN_EINMAL:";
const BRANCH = "codex/quellenfakten-versuch-20260920";
const MANIFEST = "261452f2a264571351e27d831948ff56d0f4c91bf7f9f2c5d3de6f90d3bba825";
const SCHEMA_HASH = "c6ab1f88fecf2e2e4f1ea87b2e882944859e735cc2a30406d7019246adf2b537";
const PROMPTS = [
  "70a8b88b5a2b798e7f7ff00ed4162dc449d0fd031a1f15f60335d7aa20f02427",
  "37a0142f0a2cb42fb2608ae9798ee36838ed0285688f63eb1d930c45e4a44388",
  "500c6d4d74ef083fea09ccd8c5bbcb47d4654fdae7d48c211230eec2aa9f3ef4",
  "28a4b053a81cc1f4b23098365d1a2bb4f5edf360dc7654601b429d41b1a3645f",
  "0c98e6a15298e1950ad51fc4f3325d4d6e3d8c36e8f8d5ad66e4aac93ce37723",
  "03393b39e9d779794cf21169f1563a4194d04599f283052288684d898bf42123"
];
...
function paket() {
  fordere(Eingang.korpus().hash === MANIFEST && sha(JSON.stringify(SCHEMA)) === SCHEMA_HASH, "FAKTEN_PAKET");
  return PROMPTS.map((h, i) => {
    const b = Eingang.block(i + 1);
    fordere(b.promptHash === h, "FAKTEN_PROMPT");
    return { ...b, id: b.klasse };
  });
}
function eingabe(text) {
  fordere(typeof text === "string" && text.startsWith(PREFIX) && text.length < 6000, "FAKTEN_EINGABE");
  const encoded = text.slice(PREFIX.length), bytes = Buffer.from(encoded, "base64");
  fordere(bytes.toString("base64") === encoded, "FAKTEN_EINGABE");
  ...
  fordere(a.position === 1 ? a.previous === null : a.previous
    ...
    && [a.previous.responseHash, a.previous.reviewHash].every(x => /^[a-f0-9]{64}$/.test(x)), "FAKTEN_VORPRUEFUNG");
...
function konfiguration(a, env, now) {
  fordere(env.GITHUB_REPOSITORY === "ernisch/helmut-pilot" && env.GITHUB_REF === `refs/heads/${BRANCH}`
    ...
    && env.GITHUB_SHA === a.commit && /^\d{5,20}$/.test(env.GITHUB_RUN_ID || ""), "FAKTEN_AUSFUEHRUNG");
  fordere(now.toISOString().slice(0, 10) === "2026-09-20" && now.getUTCHours() < 23, "FAKTEN_TAG");
...
async function vorflug(env, storage, fetchFn, now) {
  ...
  fordere(response.status === 200, "FAKTEN_LAUFZEIT");
  ...
  for (const path of ["mandate_profiles?select=user_id&aktiv=eq.true&limit=1",
    ...
    try {
      ...
      fordere(response.status === 200, "FAKTEN_BETRIEBSLESUNG");
      rows = await response.json();
      fordere(Array.isArray(rows), "FAKTEN_BETRIEBSLESUNG");
    } catch (_) {
      // Keine Zugangsdaten, Antwortinhalte oder fremden Fehlertexte ins Log.
      fordere(false, "FAKTEN_BETRIEBSLESUNG");
    }
    fordere(rows.length === 0, "FAKTEN_AKTIVER_BETRIEB");
  }
  ...
  fordere(counter.ok === true && Number.isSafeInteger(counter.used) && counter.used >= 0
    && counter.limit === 2416, "FAKTEN_ZAEHLER");
...
async function beanspruche(storage, a, runId, now, counter) {
  ...
  await storage.mutateAuthStore(auth => {
    for (const [key, max] of Object.entries({ sessions: 2000, auditEvents: 1000, systemErrors: 500,
      ...
      fordere(Array.isArray(auth[key]) && auth[key].length <= max, "FAKTEN_SPEICHERSTAND");
    let r = auth[KEY];
    const day = now.toISOString().slice(0, 10);
    let bound;
    if (auth.testKostenTage?.[day]) {
      const t = K.pruefeTag(auth.testKostenTage[day], day); K.kontrolliere(auth, day, 0, counter.used);
      fordere(!Object.values(t.calls).some(c => ["reserviert", "ungeklaert"].includes(c.status)), "FAKTEN_KOSTEN_OFFEN");
      bound = K.belegt(t);
    } else bound = null;
    ...
    if (bound === null) {
      fordere(counter.used === 0 && !auth.llmUsage.some(x => x.createdAt?.startsWith(day)), "FAKTEN_KOSTEN_GRUNDLAGE");
      bound = 0;
    }
    if (a.position === 1) {
      fordere(!Object.hasOwn(auth, KEY) && bound + MAX_COST <= 4000000, "FAKTEN_START_GESPERRT");
      ...
      fordere(r?.version === 1 && r.manifestHash === MANIFEST && r.schemaHash === SCHEMA_HASH
        ...
        && r.faelle?.length === a.position - 1 && r.status === "wartet-auf-fachpruefung", "FAKTEN_FORTSETZUNG");
      ...
      fordere(r.faelle.every(f => f.fachpruefung?.bestanden && f.kosten <= 212000), "FAKTEN_VORPRUEFUNG");
    }
    zeit(r, now, RESERVE_MS);
    fordere(r.faelle.length < 6 && r.faelle.reduce((s, f) => s + f.kosten, 0) + 212000 <= MAX_COST,
      "FAKTEN_AUFTRAGSLIMIT");
  ...
  fordere(equal((await storage.readAuthStore())[KEY], expected), "FAKTEN_START_UNBESTAETIGT");
  return expected;
}
...
async function mitTransportbeleg(call, { prompt, env, deadline, emit, now = () => Date.now() }, https = H) {
  ...
  https.request = function(url, options, cb) {
    fordere(++count === 1 && String(url) === String(expectedUrl) && options.method === "POST"
      && now() + 30000 <= deadline, "FAKTEN_TRANSPORT");
    let req, timer;
    req = original.call(https, url, options, response => {
      ...
      response.on("data", chunk => {
        ...
        else req.destroy(new Error("FAKTEN_ANTWORTGROESSE"));
      });
      response.on("end", () => {
    ...
    req.write = function(body, ...args) {
      fordere(!written && typeof body === "string" && equal(JSON.parse(body), expected), "FAKTEN_NUTZLAST");
    ...
    req.end = function(...args) { fordere(written && !args.some(x => typeof x === "string" || Buffer.isBuffer(x)), "FAKTEN_NUTZLAST"); return end(...args); };
    timer = setTimeout(() => req.destroy(new Error("FAKTEN_TRANSPORTZEIT")), Math.min(20000, deadline - now()));
...
async function ausfuehren({ env = process.env, now = () => new Date(), fetchFn = global.fetch,
  ...
  const before = await storage.readAuthStore(), runId = `quellenfakten-einmal-${env.GITHUB_RUN_ID}`;
  ...
  try {
    ...
    result = await observe(() => request(f.prompt, SCHEMA,
      { callType: "understanding", pipelineStep: "quellenfakten-eingang", politicianId: null, runId },
      "gpt-5-mini", { strict: false, reasoningEffort: "minimal" }),
    { prompt: f.prompt, env, deadline: Date.parse(claim.ende),
      emit: value => { transport = value; send("quellenfakten-transport", { position: a.position, runId, ...value }); } });
    fordere(result.count === 1 && transport?.rawResponse && transport.statusCode === 200, "FAKTEN_TRANSPORTBELEG");
    const pruefung = Eingang.pruefe(a.position, result.answer);
    fordere(pruefung.annotationsgleich, "FAKTEN_ANNOTATIONSABWEICHUNG");
  } catch (e) { errorCode = /^[A-Z_]{3,60}$/.test(e?.code || "") ? e.code : "FAKTEN_AUSGANG_OFFEN"; }
  send("quellenfakten-antwort", { runId, position: a.position, fall: f, schemaHash: SCHEMA_HASH,
  ...
  if (!kostenBestaetigt || !geschuetzt || !usage[0]?.success) errorCode ||= "FAKTEN_NACHKONTROLLE";
  try { zeit(claim, now()); await vorflug(env, storage, fetchFn, now()); }
  catch { errorCode ||= "FAKTEN_NACHKONTROLLE"; }
  ...
  send("quellenfakten-kosten", beleg);
  let savedExpected;
  await storage.mutateAuthStore(auth => {
    const r = auth[KEY];
    fordere(equal(r, claim), "FAKTEN_ABSCHLUSSKONFLIKT");
  ...
  fordere(equal((await storage.readAuthStore())[KEY], savedExpected), "FAKTEN_ABSCHLUSS_UNBESTAETIGT");
...
if (require.main === module) ausfuehren().then(r => {
  ...
  console.error(JSON.stringify({ ok: false, grund: /^FAKTEN_[A-Z_]+$/.test(e?.code || "") ? e.code : "FAKTEN_UNBESTAETIGT",
