...
function erhebungsSql() {
  return [
    ...
    "--",
    "-- KORRIGIERT 04.09.2026 (SR §36.9 (2)): Bis hierher las diese Abfrage die",
    "-- Adresse als `left join profiles p on p.id = m.user_id` -> `p.email`. Diese",
    "-- Spalte ist in Production bei ALLEN Profilen NULL, auch bei den realen:",
    "-- `buildProfile` (provisioning.js) uebertraegt `email` nie ins Profilobjekt,",
    "-- `saveProfileToDb` schreibt deshalb `email: null`, und `mandate_profiles` hat",
    "-- gar keine E-Mail-Spalte. Die Kontoadresse lebt AUSSCHLIESSLICH im Auth-Blob",
    "-- (helmut_store, Zeile 'main-auth', data->users[] mit email + politicianId).",
    "-- Folge: `pruefeBestand` brach an jeder Zeile ab (\"fuehrt keine gelesene",
    "-- E-Mail-Adresse\") — strukturell, fuer Stufe A wie B und C. Gelesen wird jetzt",
    "-- die Ablage, welche die Adresse tatsaechlich fuehrt; dieselbe Zeile, aus der",
    "-- weiter unten schon `kohortenKontenAktiv` kommt.",
    "select",
    "  coalesce(",
    "    (select json_agg(json_build_object(",
    "       'id', m.user_id, 'aktiv', m.aktiv is true, 'email', coalesce(k.email, ''))",
    "     order by m.user_id)",
    "     from mandate_profiles m",
    "     left join lateral (",
    "       select u->>'email' as email",
    "       from helmut_store hs, jsonb_array_elements(coalesce(hs.data->'users','[]'::jsonb)) u",
    "       where hs.id = 'main-auth' and (u->>'politicianId') = m.user_id",
    "       limit 1",
    "     ) k on true",
