...
async function main() {
  ...
  abschnitt("11 · Gedeckelte Grundmenge ist Stichprobe, keine Gesamtverteilung");
  {
    ...
    // Leere `last_error` Werte sind trotz `not.is.null` moeglich. Der Storagepfad
    // verwirft sie aus `texte`, hat sie aber tatsaechlich gelesen. Der Stichprobenanteil
    // muss deshalb durch `gelesen`, nicht durch die kuerzere Textliste teilen.
    const mitLeerenZeilen = await status(kennzahlen({
      wartend: 2000,
      aeltester_faelliger_s: 19 * H,
      aeltester_offener_s: 19 * H,
      erledigt_im_zeitraum: 25
      ...
      limit: 1000,
      vollstaendig: false,
      texte: Array(500).fill("zurueckgestellt: verstehen-uebersprungen (understanding-locked)")
    }) });
    check("11.8 Leere gelesene Zeilen erhoehen den Stichprobenanteil nicht",
      mitLeerenZeilen.zurueckgestellt
      && mitLeerenZeilen.zurueckgestellt.anteilBlockierendStichprobe === 0.5,
      JSON.stringify(mitLeerenZeilen.zurueckgestellt));
  }
