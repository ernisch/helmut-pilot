...
const PAYLOAD_HASH = "0d453a5366824c675eaa63091c59c1f6ae6e32ea2a2e9182436bcf197507fa69";
...
function plane(payload) {
  if (typeof payload !== "string" || hash(payload) !== PAYLOAD_HASH || payload.includes("$eingabe$")) throw Error("frische17-payload-abweichend");
  const data = JSON.parse(payload);
  F.ausGesichertenBelegen(data.rows, data.belege);
  if (data.findings.length !== 17) throw Error("frische17-fundzahl");
...
lock table ${TABELLEN.map(t => "public." + t).join(",")} in share row exclusive mode;`;
  let sql = fs.readFileSync(path.join(__dirname, "fixtures/quellenfrische-30-vorbereitung.sql"), "utf8")
    .replace(/^[\s\S]*?begin;/, "-- Genau17 neue Quellen; atomar, keine Wiederholung bei unklarem Ausgang.\nbegin;")
    .replace(/lock table[^;]*;\nlock table[^;]*;/, locks)
    .replace(/<>30\b/g, "<>17").replace("<>504", "<>500")
  ...
  if not exists(select 1 from public.helmut_store where id='${F.EINGABE}'
    and data-'status'-'payloadHash'-'angelegtAm'=eingabe) then raise exception 'frische17-belegablage-abweichend'; end if;
  if grundlinie is distinct from ${grundlinie("eingabe")} then`);
  let rueckweg = require("./quellenfrische-30-plan").baueRueckweg(payload)
    .replace(/<>30\b/g, "<>17").replaceAll("frische30", "frische17")
    .replace(/lock table[^;]*;\nlock table[^;]*;/, locks)
 ...
 if (select count(*) from public.mandate_profiles)<>500 or (select count(*) from public.profiles)<>501
  ...
  or not exists(select 1 from public.helmut_store where id='${F.EINGABE}'
   ...
   raise exception 'frische17-rueckweg-belegstand'; end if;
 ...
 get diagnostics n=row_count; if n<>1 then raise exception 'frische17-rueckweg-belegzahl'; end if;
 if exists(select 1 from public.document_findings where raw_document_id in(select x->>'id' from jsonb_array_elements(e->'rows')x))
  or grundlinie is distinct from ${grundlinie("e")} then raise exception 'frische17-rueckweg-fremde-wirkung'; end if;
end $rueckweg$;`);
  if (!sql.includes("insert into public.helmut_store") || sql.includes("__PAYLOAD__")) throw Error("frische17-vorlage-abweichend");
  return { sql, rueckweg, bericht: { dokumente: 17, belege: 17, payloadHash: PAYLOAD_HASH,
    importHash: hash(sql), rueckwegHash: hash(rueckweg), ausgefuehrt: false } };
}
if (require.main === module) {
  const [input, prefix, extra] = process.argv.slice(2);
  if (!input || !prefix || extra) throw Error("frische17-aufruf");
