...
const G = require("./prosa-quellenpruefung"), T = require("./privater-nachweis-transport");
...
function answerFor(position = 1) {
  const f = G.paket()[position - 1];
  return { pruefungen: f.input.faelle.map(c => ({ fall: c.fall, urteile: c.aussagen.map(a => ({
    aussage: a.i, deckung: f.expect.rejected_paths[c.fall]?.includes(a.pfad) ? "unbelegt" : "belegt",
    quellen: [0], grund: "Synthetisches Urteil fuer den technischen Kontrollfluss." })) })) };
}
...
function harness(options = {}) {
  ...
  const request = async (prompt, schema, meta, model, settings) => {
    ...
    A.equal(G.sha(JSON.stringify(schema)), G.SCHEMA_HASH); A.deepEqual(settings, { strict: true, reasoningEffort: "minimal" });
    ...
    if (options.semanticBad) {
      const r = answerFor(auth[G.KEY].faelle.length);
      r.pruefungen.forEach(c => c.urteile.forEach(u => u.deckung = "belegt")); return r;
    }
    if (options.falseReject) {
      const r = answerFor(auth[G.KEY].faelle.length), id = G.paket()[auth[G.KEY].faelle.length - 1].expect.accept[0];
      r.pruefungen.find(c => c.fall === id).urteile[0].deckung = "unbelegt"; return r;
    }
    return options.schemaBad ? {} : answerFor(auth[G.KEY].faelle.length);
  };
...
(async () => {
  await test("Zwei gebundene Bloecke enthalten alle neun Faelle und keine Erwartung im Modellauftrag", () => {
    A.equal(G.paket().length, 2); A.deepEqual(G.paket().map(f => f.promptHash), G.PROMPTS);
    A.equal(G.paket().reduce((n, f) => n + f.input.faelle.length, 0), 9);
    A.equal(G.MAX_COST, 424000);
    for (const f of G.paket()) {
      A.deepEqual([...f.expect.reject, ...f.expect.accept].sort(), f.input.faelle.map(c => c.fall).sort());
      A(!f.prompt.includes('"expect"')); A(!f.prompt.includes('"rejected_paths"'));
    }
    A.deepEqual(G.eingabe(encode(input)), input);
    for (const a of [{ ...input, position: 0 }, { ...input, position: 3 }, { ...input, previous: {} },
  ...
  await test("Zwei verschiedene Bloecke einmalig und nur nach gebundener Vorbewertung", async () => {
    ...
    A.equal(h.state().calls, 2); A.equal(h.state().auth[G.KEY].faelle.length, 2);
    A.equal(h.state().auth[G.KEY].faelle.reduce((s, f) => s + f.kosten, 0), 1700);
    await A.rejects(h.run()); A.equal(h.state().calls, 2);
  });
  ...
  await test("Voller echter Nutzungsring behaelt alle Altbelege ueber zwei neue Bloecke", async () => {
    ...
    for (let position = 1; position <= 2; position++) {
      ...
      A.deepEqual(archive, old.slice(-2)); A.equal(auth.llmUsage.length, 5000);
      A.deepEqual(auth.llmUsage.slice(position), old.slice(0, 5000 - position));
      A.deepEqual([...auth.llmUsage.slice(position), ...archive.slice(2 - position)], old);
    }
    A.equal(h.state().calls, 2);
  });
  await test("Fehlende Archivzeile und fremde Telemetrieaenderung bestehen nicht", () => {
    ...
    const claim = { archivierteAufruftelemetrie: old.slice(-2) };
  ...
  await test("Modellfehler, fehlendes Urteil, falsche Annahme/Ablehnung, Kosten und fremde Mutation stoppen", async () => {
    for (const opts of [{ modelError: true }, { schemaBad: true }, { semanticBad: true }, { falseReject: true }, { costUnknown: true }, { foreignWrite: true }]) {
  ...
  await test("Fehlende Kostenhistorie und volle Tagesreserve sperren vor Claim", async () => {
    ...
    await s.storage.mutateAuthStore(auth => { delete auth[G.KEY]; const t = auth.testKostenTage["2026-09-19"];
      t.baseline = 3800000; t.spent += t.baseline; });
  ...
  await test("Echter KI Pfad reserviert vor HTTPS und archiviert vollstaendige Rohantwort", async () => {
    ...
    try {
      ...
      A.equal(JSON.parse(requestBeleg.requestRaw).reasoning.effort, "minimal");
  ...
  await test("Workflow bindet neuen Branch und haelt Gipfeljob, Crons und Anwendung unveraendert", () => {
    ...
    const own = yaml.slice(yaml.indexOf("  prosa-quellenpruefung:"));
    A(own.includes(`refs/heads/${G.BRANCH}`)); A(own.includes("github.run_attempt == 1"));
    A(own.includes("timeout-minutes: 3")); A(own.includes("node scripts/prosa-quellenpruefung.js"));
    A(yaml.includes("!startsWith(inputs.confirm_text, 'PROSA_QUELLEN:')"));
    A.equal(require("../vercel.json").git.deploymentEnabled[G.BRANCH], false);
  });
  await test("Alle drei geschlossenen alten Serien bleiben byteinhaltlich geschuetzt", async () => {
    const h = harness();
    const first = {status:"fachlich-gestoppt",beleg:"erste Serie"}, second = {status:"fachlich-gestoppt",beleg:"zweite Serie"};
    await h.storage.mutateAuthStore(a => { a.prosaFachnachweis20260919=first; a.prosaFolgenFachnachweis20260919=second; a.prosaDenkFachnachweis20260919={status:"fachlich-gestoppt",beleg:"dritte Serie"}; });
    ...
    A.deepEqual(h.state().auth.prosaDenkFachnachweis20260919,{status:"fachlich-gestoppt",beleg:"dritte Serie"});
