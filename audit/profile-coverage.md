...
**Bias-Prüfung:** Kein hartkodiertes „cem ince", kein „arbeit soziales"-Sonderscore, kein committee-92 im Rechenkern (decisions.js/matching.js). Der Bias sitzt eine Ebene höher: `cemInceProfile` (config.js:7, 130 Zeilen) ist in `server.js`/`scheduler.js` ~30× als Default-`politicianId` verdrahtet; nur `cem-ince` erhält im Nicht-Auth-Modus dieses reiche Profil (scheduler.js:1294), jedes andere Mandat bekommt `neutralProfileDefaults` (leere committees/topics/regions, scheduler.js:1311).

---

## 3. Profil-Konfiguration (verifiziert)

**Reiche Profildaten leben nur im Code** (`cemInceProfile`, config.js:7-139). Die DB-Tabelle `profiles` (3 Zeilen) hat **alle inhaltlichen Felder NULL**:

| id | party | committee | focustopics | embedding |
|---|---|---|---|---|
| cem-ince | NULL | NULL | NULL | NULL |
| james-brown | NULL | NULL | NULL | NULL |
| angela-merkel | NULL | NULL | NULL | NULL |
...
- `james-brown`/`angela-merkel` sind **leere Platzhalter** (0 decisions, 0 embedding).
- `profile_embeddings`: **1 Zeile** (nur cem-ince, dim=256).

---

## 4. Versorgungskette pro Profil (gemessen)

### 4.1 `cem-ince` (das einzige reale Profil)
...
1. **Themen-Dimension tot:** KO-`topics` speist sich aus `tags`+`policy_field` (matching.js:128) — **`tags` bei allen 217 KOs leer, `embedding` bei allen 217 NULL** (gemessen). Der thema-Treffer (12 P.) feuert nie; der Kosinus verliert alle Themen-Tokens. Cem-Inces 16 focusTopics matchen ins Leere.
...
### 4.2 `james-brown` / `angela-merkel` (leere Platzhalter)
...
| T3 | Linke-MdB Arbeit/Soziales | Die Linke | Opposition | Bundestag | ländlich (Salzgitter) | Arbeit & Soziales | mittel | ja | Pilot-Klon, Label-Mismatch |
...
| Reiche Profildaten nur in `config.js`, DB-`profiles` leer | **C** | nur `cem-ince` (Code) versorgt | config.js:7; DB |
| Nicht-cem-ince-Mandate erhalten `neutralProfileDefaults` (leere Merkmale) | **C** | jedes echte Mandat startet ohne Fachmerkmale | scheduler.js:1294/1311 |
...
- `james-brown`/`angela-merkel` haben **kein** konfiguriertes Mandat → dass sie nichts Personalisiertes bekommen, ist korrekt (Profil ist leer, Klasse C, kein Datenverlust).
...
**Dünn, aber korrekt:** cem-ince erhält **10 belegte Karten** — nicht leer, aber ohne echte Priorisierung. Die Kartenzahl ist ausreichend; der Mangel ist **Relevanz-/Personalisierungsqualität**, nicht Menge.
